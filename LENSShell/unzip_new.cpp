...
static ZRESULT MapMzError(int32_t mz_err)
{
    switch (mz_err) {
        ...
        case MZ_DATA_ERROR:
            return 0x00000003;  // ZR_CORRUPT
        case MZ_MEM_ERROR:
            return 0x00000004;  // ZR_MEMORY
        case MZ_PARAM_ERROR:
            return 0x00000001;  // ZR_ARGS
        case MZ_PASSWORD_ERROR:
            return 0x00000008;  // ZR_PASSWORD
        case MZ_OPEN_ERROR:
            return 0x00000012;  // ZR_NOFILE
        case MZ_READ_ERROR:
            return 0x0000000B;  // ZR_READ
        default:
            return 0x00000010;  // ZR_FAILED
    }
}
        ...
        if (file_info && file_info->filename) {
            ...
            if (match) {
                if (index)
                    *index = current_index;
                if (ze)
                    return GetZipItem(hz, current_index, ze);
                return 0x00000000;  // ZR_OK
            }
        }
        current_index++;
