...
class TexturePool
{
  ...
  public:
    TexturePool() : m_allocations(0), m_reuses(0), m_evictions(0), m_totalMemoryBytes(0) {}
    ...
    HRESULT AcquireTexture(UINT width, UINT height, DXGI_FORMAT format, ID3D11Texture2D** outTexture,
                           ID3D11ShaderResourceView** outSRV, ID3D11UnorderedAccessView** outUAV)
    {
        ...
        for (auto& entry : m_pool) {
            if (!entry->inUse && entry->width == width && entry->height == height && entry->format == format) {
                ...
                *outTexture = entry->texture.Get();
                ...
                std::string poolInfo = "Texture reused: " + std::to_string(width) + "x" + std::to_string(height)
                                       + " (use count: " + std::to_string(entry->useCount) + ")";
        ...
        HRESULT hr = CreateNewTexture(width, height, format, outTexture, outSRV, outUAV);
        if (SUCCEEDED(hr)) {
            m_allocations++;
        }
    ...
    void EvictOldTextures()
    {
        ...
        m_pool.erase(std::remove_if(m_pool.begin(), m_pool.end(),
                                    [&](const std::unique_ptr<PooledTexture>& entry) {
                                        ...
                                        auto age =
                                            std::chrono::duration_cast<std::chrono::seconds>(now - entry->lastUsed);
                                        ...
                                        if (age > threshold) {
                                            ...
                                            std::string evictInfo = "Evicted texture: " + std::to_string(entry->width)
                                                                    + "x" + std::to_string(entry->height)
        ...
        if (evicted > 0) {
            std::string evictSummary = "Evicted " + std::to_string(evicted) + " textures from pool";
    ...
    void CheckMemoryPressure()
    {
        ...
        if (m_totalMemoryBytes > maxBytes) {
            std::string pressureInfo = "Memory pressure detected: " + std::to_string(m_totalMemoryBytes / (1024 * 1024))
                                       + " MB / " + std::to_string(MAX_MEMORY_MB) + " MB";
    ...
    std::string GetStatistics() const
    {
        ...
        double reuseRate =
            (m_allocations + m_reuses) > 0 ? (static_cast<double>(m_reuses) / (m_allocations + m_reuses) * 100.0) : 0.0;
    ...
    void Cleanup()
    {
        ...
        if (!m_pool.empty()) {
            std::string cleanupInfo = "Cleaning up texture pool: " + std::to_string(m_pool.size()) + " textures";
  ...
  private:
    ...
    HRESULT CreateNewTexture(UINT width, UINT height, DXGI_FORMAT format, ID3D11Texture2D** outTexture,
                             ID3D11ShaderResourceView** outSRV, ID3D11UnorderedAccessView** outUAV)
    {
        ...
        if (FAILED(hr)) {
            DT_LOG_HRESULT(LogLevel::LVL_ERROR, LogCategory::GPU, "Failed to create pooled texture", hr);
            return hr;
        }
        ...
        if (outSRV) {
            ...
            if (FAILED(hr)) {
                DT_LOG_HRESULT(LogLevel::LVL_ERROR, LogCategory::GPU, "Failed to create SRV for pooled texture", hr);
                return hr;
            }
        }
        ...
        if (outUAV) {
            hr = m_device->CreateUnorderedAccessView(entry->texture.Get(), nullptr, &entry->uav);
            if (FAILED(hr)) {
                DT_LOG_HRESULT(LogLevel::LVL_ERROR, LogCategory::GPU, "Failed to create UAV for pooled texture", hr);
                return hr;
            }
        }
        ...
        *outTexture = entry->texture.Get();
        ...
        std::string allocInfo = "Created new texture: " + std::to_string(width) + "x" + std::to_string(height) + " ("
    ...
    void EvictLRU()
    {
        ...
        if (oldest != m_pool.end()) {
            ...
            std::string evictInfo =
                "LRU evicted: " + std::to_string((*oldest)->width) + "x" + std::to_string((*oldest)->height);
            DT_LOG_DEBUG(LogCategory::GPU, evictInfo);
    ...
    uint64_t CalculateTextureSize(const PooledTexture* entry) const
    {
        ...
        return static_cast<uint64_t>(entry->width) * entry->height * bitsPerPixel / 8;
    }
};
