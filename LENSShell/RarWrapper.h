...
class CUnrarWrapper
{
  public:
    ...
    {
        return ExtractFirst([](const RarEntryInfo& e) { return IsImageExtension(e.filename); });
    }
    ...
    static bool IsImageExtension(const std::wstring& name)
    {
        ...
        return ext == L".jpg" || ext == L".jpeg" || ext == L".png" || ext == L".bmp" || ext == L".gif"
               || ext == L".webp" || ext == L".tiff" || ext == L".tif";
    }
    ...
    static const char* ErrorCodeToString(int code)
    {
        switch (code) {
            ...
            case ERAR_BAD_DATA:
                return "Bad data (CRC error)";
            case ERAR_BAD_ARCHIVE:
                return "Bad archive format";
  ...
  private:
    void LoadUnRARDLL()
    {
        ...
        if (!m_dll.hModule)
            return;
        m_dll.OpenArchiveEx = (UnRARDLLFunctions::RAROpenArchiveExFn)GetProcAddress(m_dll.hModule, "RAROpenArchiveEx");
        m_dll.CloseArchive = (UnRARDLLFunctions::RARCloseArchiveFn)GetProcAddress(m_dll.hModule, "RARCloseArchive");
        m_dll.ReadHeaderEx = (UnRARDLLFunctions::RARReadHeaderExFn)GetProcAddress(m_dll.hModule, "RARReadHeaderEx");
        m_dll.ProcessFileW = (UnRARDLLFunctions::RARProcessFileWFn)GetProcAddress(m_dll.hModule, "RARProcessFileW");
        m_dll.SetCallback = (UnRARDLLFunctions::RARSetCallbackFn)GetProcAddress(m_dll.hModule, "RARSetCallback");
    }
