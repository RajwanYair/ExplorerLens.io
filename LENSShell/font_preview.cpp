...
HBITMAP FontPreview::GenerateFontPreview(const std::wstring& fontPath, int width, int height)
{
...
HBITMAP FontPreview::RenderUsingDirectWrite(const std::wstring& fontPath, int width, int height)
{
    // Initialize COM
    CoInitialize(NULL);
    ...
    hr = pWICFactory->CreateBitmap(width, height, GUID_WICPixelFormat32bppPBGRA, WICBitmapCacheOnDemand, &pWICBitmap);
    ...
    hr = pD2DFactory->CreateWicBitmapRenderTarget(pWICBitmap, &props, &pRenderTarget);
...
HBITMAP FontPreview::RenderUsingGDI(const std::wstring& fontPath, int width, int height)
{
    // Add font resource temporarily
    int fontCount = AddFontResourceEx(fontPath.c_str(), FR_PRIVATE, NULL);
    ...
    HFONT hFont = CreateFont(height / 3,                   // Height
                             0,                            // Width
                             0,                            // Escapement
                             0,                            // Orientation
    ...
    DrawText(hdcMem, sampleText, -1, &textRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    SelectObject(hdcMem, hOldFont);
    DeleteObject(hFont);

    // Draw smaller character set preview
    hFont = CreateFont(height / 8,  // Smaller size
                       0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                       CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, fontFamily.c_str());
    ...
    DrawText(hdcMem, charSet, -1, &charRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    SelectObject(hdcMem, hOldFont);
    DeleteObject(hFont);

    // Draw font name at bottom
    hFont = CreateFont(height / 12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
    ...
    DrawText(hdcMem, fontFamily.c_str(), -1, &nameRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
...
// Get font metadata
bool FontPreview::GetFontMetadata(const std::wstring& fontPath, FontMetadata& metadata)
{
    ZeroMemory(&metadata, sizeof(FontMetadata));

    // Try to get metadata from file properties first
    CComPtr<IPropertyStore> pPropertyStore;
    HRESULT hr = SHGetPropertyStoreFromParsingName(fontPath.c_str(), NULL, GPS_DEFAULT, IID_PPV_ARGS(&pPropertyStore));
    ...
    metadata.isMonospace =
        (lowerName.find(L"mono") != std::wstring::npos || lowerName.find(L"code") != std::wstring::npos
         || lowerName.find(L"console") != std::wstring::npos || lowerName.find(L"courier") != std::wstring::npos);
    ...
    metadata.isItalic =
        (lowerName.find(L"italic") != std::wstring::npos || lowerName.find(L"oblique") != std::wstring::npos);
