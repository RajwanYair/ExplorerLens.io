...
HRESULT GPUAccelerator::Initialize(bool allowWARP)
{
    ...
    if (FAILED(hr)) {
        DT_LOG_HRESULT(LogLevel::LVL_ERROR, LogCategory::GPU, "Create WIC factory", hr);
        OutputDebugString(L"[GPU] Failed to create WIC factory\n");
        return hr;
    }

    DT_LOG_DEBUG(ExplorerLens::LogCategory::GPU, "WIC factory created successfully");
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    DT_LOG_INFO(LogCategory::GPU, "GPU acceleration initialized successfully");
                    std::ostringstream oss;
                    oss << "Device: " << std::string(CW2A(m_deviceInfo.name.c_str()))
                        << ", VRAM: " << (m_deviceInfo.dedicatedVideoMemory / (1024.0 * 1024.0)) << " MB"
                        << ", Feature Level: 0x" << std::hex << m_deviceInfo.featureLevel;
                    DT_LOG_INFO(LogCategory::GPU, oss.str());

                    OutputDebugStringW(L"[GPU] GPU acceleration initialized successfully\n");
                    ...
                    swprintf_s(msg, _countof(msg), L"[GPU] VRAM: %.1f MB, Feature Level: 0x%04X\n",
                               ...
                               static_cast<unsigned>(m_deviceInfo.featureLevel));
                    OutputDebugStringW(msg);
                } else {
                    DT_LOG_HRESULT(LogLevel::LVL_ERROR, LogCategory::GPU, "Detect capabilities", hr);
                }
            } else {
                DT_LOG_HRESULT(LogLevel::LVL_ERROR, LogCategory::GPU, "Compile shaders", hr);
            }
        } else {
            DT_LOG_HRESULT(LogLevel::LVL_ERROR, LogCategory::GPU, "Create resources", hr);
        }
    } else {
        DT_LOG_HRESULT(LogLevel::LVL_WARNING, LogCategory::GPU, "Create D3D11 device", hr);
    }

    if (!m_gpuAvailable) {
        OutputDebugString(L"[GPU] GPU acceleration not available, using CPU fallback\n");
...
HRESULT GPUAccelerator::CreateDevice(bool allowWARP)
{
    ...
    hr = D3D11CreateDevice(nullptr,  // Default adapter
                           D3D_DRIVER_TYPE_HARDWARE, nullptr, createDeviceFlags, featureLevels,
    ...
    if (FAILED(hr) && allowWARP) {
        ...
        hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_WARP, nullptr, createDeviceFlags, featureLevels,
                               ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, &m_device, &featureLevel, &m_context);
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        if (desc.VendorId == 0x8086) {
                            ...
                            if (gpuName.find(L"Iris") != std::wstring::npos || gpuName.find(L"Arc") != std::wstring::npos
...
HRESULT GPUAccelerator::CompileShaders()
{
    ...
    hr = D3DCompile(shaderSource, strlen(shaderSource), "ThumbnailResize", nullptr, nullptr, "CSResizeLanczos3",
    ...
    if (FAILED(hr)) {
        ...
        OutputDebugString(L"[GPU] Failed to compile compute shader, will use CPU fallback\n");
        return hr;
    }
...
HRESULT GPUAccelerator::DetectCapabilities()
{
    ...
    if (m_deviceInfo.featureLevel >= D3D_FEATURE_LEVEL_11_0) {
        m_deviceInfo.capabilities = m_deviceInfo.capabilities | GPUCapability::ComputeShaders;
    }

    // Check for D3D11.1 features
    if (m_deviceInfo.featureLevel >= D3D_FEATURE_LEVEL_11_1) {
        m_deviceInfo.capabilities = m_deviceInfo.capabilities | GPUCapability::FastSemantics;
    }

    // Check for dedicated GPU
    if (m_deviceInfo.isHardwareDevice && m_deviceInfo.dedicatedVideoMemory > 256 * 1024 * 1024) {
        m_deviceInfo.capabilities = m_deviceInfo.capabilities | GPUCapability::HighPerformance;
    }
...
{
    return HasCapability(m_deviceInfo.capabilities, GPUCapability::ComputeShaders);
}

bool GPUAccelerator::IsHighPerformance() const
{
    return HasCapability(m_deviceInfo.capabilities, GPUCapability::HighPerformance);
}
...
HRESULT GPUAccelerator::CreateThumbnail(IWICBitmapSource* pSource, UINT targetWidth, UINT targetHeight,
                                        IWICBitmap** ppThumbnail)
{
    ...
    *ppThumbnail = nullptr;
    ...
    if (!m_gpuAvailable || m_deviceLost) {
        DT_LOG_DEBUG(ExplorerLens::LogCategory::GPU, "Using CPU fallback (GPU unavailable or device lost)");
    ...
    if (FAILED(hr)) {
        DT_LOG_HRESULT(LogLevel::LVL_WARNING, LogCategory::GPU, "CreateTextureFromWIC", hr);
        OutputDebugString(L"[GPU] Failed to create texture from WIC, using CPU fallback\n");
        return CreateThumbnailCPU(pSource, targetWidth, targetHeight, ppThumbnail);
    }
    ...
    std::string resizeInfo = std::string("Resizing texture: ") + std::to_string(srcDesc.Width) + "x"
                             + std::to_string(srcDesc.Height) + " -> " + std::to_string(targetWidth) + "x"
                             + std::to_string(targetHeight);
    DT_LOG_DEBUG(ExplorerLens::LogCategory::GPU, resizeInfo);

    // Resize texture using GPU
    hr = ResizeTexture(sourceSRV.Get(), srcDesc.Width, srcDesc.Height, targetWidth, targetHeight, &outputTexture);
    if (FAILED(hr)) {
        DT_LOG_HRESULT(LogLevel::LVL_WARNING, LogCategory::GPU, "ResizeTexture", hr);
    ...
    if (FAILED(hr)) {
        DT_LOG_HRESULT(LogLevel::LVL_WARNING, LogCategory::GPU, "TextureToWICBitmap", hr);
        OutputDebugString(L"[GPU] Failed to convert to WIC bitmap, using CPU fallback\n");
        return CreateThumbnailCPU(pSource, targetWidth, targetHeight, ppThumbnail);
    }
    ...
    std::string perfInfo = std::string("GPU thumbnail generated in ") + std::to_string(elapsedMs) + " ms";
...

    {
        ...
        m_stats.avgGpuTimeMs = (m_stats.avgGpuTimeMs * (m_stats.gpuThumbnails - 1) + elapsedMs) / m_stats.gpuThumbnails;
    }
    ...
    // Periodic texture pool cleanup
    {
        ...
        if (m_poolFrameCounter >= POOL_CLEANUP_INTERVAL) {
            ...
            while (it != m_texturePool.end()) {
                if (!it->inUse) {
                    auto age = std::chrono::duration_cast<std::chrono::milliseconds>(now - it->lastUsed).count();
...
HRESULT GPUAccelerator::CreateThumbnailFromFile(const std::wstring& filePath, UINT targetWidth, UINT targetHeight,
                                                IWICBitmap** ppThumbnail)
{
    if (!m_wicFactory) {
...
HRESULT GPUAccelerator::CreateThumbnailCPU(IWICBitmapSource* pSource, UINT targetWidth, UINT targetHeight,
                                           IWICBitmap** ppThumbnail)
{
    ...
    hr = scaler->Initialize(pSource, targetWidth, targetHeight, WICBitmapInterpolationModeFant);
    if (FAILED(hr)) {
        return hr;
    }
...

    {
        ...
        m_stats.avgCpuTimeMs = (m_stats.avgCpuTimeMs * (m_stats.cpuFallbacks - 1) + elapsedMs) / m_stats.cpuFallbacks;
    }
...
HRESULT GPUAccelerator::CreateTextureFromWIC(IWICBitmapSource* pSource, ID3D11Texture2D** ppTexture,
                                             ID3D11ShaderResourceView** ppSRV)
{
    ...
    if (!IsEqualGUID(pixelFormat, GUID_WICPixelFormat32bppBGRA)) {
        ...
        hr = converter->Initialize(pSource, GUID_WICPixelFormat32bppBGRA, WICBitmapDitherTypeNone, nullptr, 0.0,
    ...
    hr =
        AcquireTexture(width, height, DXGI_FORMAT_B8G8R8A8_UNORM, D3D11_BIND_SHADER_RESOURCE, &texture, ppSRV, nullptr);
    ...
    m_context->UpdateSubresource(texture.Get(), 0, nullptr, pixels.data(), stride, 0);

    *ppTexture = texture.Detach();
    return S_OK;
}

HRESULT GPUAccelerator::ResizeTexture(ID3D11ShaderResourceView* pSourceSRV, UINT sourceWidth, UINT sourceHeight,
                                      UINT targetWidth, UINT targetHeight, ID3D11Texture2D** ppOutput)
{
    ...
    if (m_resizeCS) {
        return ResizeTextureComputeShader(pSourceSRV, sourceWidth, sourceHeight, targetWidth, targetHeight, ppOutput);
    }

    // Fallback: Use WIC scaler (Phase 1 behavior)
    OutputDebugString(L"[GPU] Compute shader not available, using WIC fallback\n");
    ...
    hr = m_wicFactory->CreateBitmapFromMemory(sourceWidth, sourceHeight, GUID_WICPixelFormat32bppBGRA, mapped.RowPitch,
    ...
    hr = scaler->Initialize(sourceBitmap.Get(), targetWidth, targetHeight, WICBitmapInterpolationModeFant);
    if (FAILED(hr)) {
        return hr;
    }
    ...
    hr = scaler->CopyPixels(nullptr, scaledStride, scaledBufferSize, scaledPixels.data());
    if (FAILED(hr)) {
        return hr;
    }

    // Update output texture
    m_context->UpdateSubresource(outputTexture.Get(), 0, nullptr, scaledPixels.data(), scaledStride, 0);

    *ppOutput = outputTexture.Detach();
    return S_OK;
}

HRESULT GPUAccelerator::ResizeTextureComputeShader(ID3D11ShaderResourceView* pSourceSRV, UINT sourceWidth,
                                                   ...
                                                   ID3D11Texture2D** ppOutput)
{
    ...
    hr = AcquireTexture(targetWidth, targetHeight, DXGI_FORMAT_B8G8R8A8_UNORM,
                        D3D11_BIND_UNORDERED_ACCESS | D3D11_BIND_SHADER_RESOURCE, &outputTexture, nullptr, &outputUAV);
...
HRESULT GPUAccelerator::TextureToWICBitmap(ID3D11Texture2D* pTexture, IWICBitmap** ppBitmap)
{
    ...
    // Create WIC bitmap from mapped data
    hr =
        m_wicFactory->CreateBitmapFromMemory(desc.Width, desc.Height, GUID_WICPixelFormat32bppBGRA, mapped.RowPitch,
                                             mapped.RowPitch * desc.Height, static_cast<BYTE*>(mapped.pData), ppBitmap);
...
void GPUAccelerator::QueueThumbnail(const std::wstring& filePath, UINT targetWidth, UINT targetHeight, int priority,
...
void GPUAccelerator::ProcessQueue()
{
    while (m_queueRunning) {
...

        {
            ...
            m_queueCV.wait(lock, [this] { return !m_queue.empty() || !m_queueRunning; });
        ...
        HRESULT hr = CreateThumbnailFromFile(request.sourcePath, request.targetWidth, request.targetHeight, &thumbnail);
...
HRESULT GPUAccelerator::AcquireTexture(UINT width, UINT height, DXGI_FORMAT format, UINT bindFlags,
                                       ...
                                       ID3D11UnorderedAccessView** ppUAV)
{
    ...
    *ppTexture = nullptr;
    ...
    if (ppSRV && (bindFlags & D3D11_BIND_SHADER_RESOURCE)) {
        ...
        hr = m_device->CreateShaderResourceView(newTexture.Get(), &srvDesc, &newSRV);
        if (FAILED(hr)) {
    ...
    if (ppUAV && (bindFlags & D3D11_BIND_UNORDERED_ACCESS)) {
        ...
        hr = m_device->CreateUnorderedAccessView(newTexture.Get(), &uavDesc, &newUAV);
        if (FAILED(hr)) {
    ...
    // Add to pool if not full
    {
        ...
        if (m_texturePool.size() < MAX_POOL_SIZE) {
            ...
            wchar_t msg[128];
            swprintf_s(msg, 128, L"[GPU] Texture pool: Added new texture (pool size: %zu/%zu)\n", m_texturePool.size(),
                       MAX_POOL_SIZE);
            OutputDebugString(msg);
        } else {
...
HRESULT ThumbnailGenerator::Generate(IWICBitmapSource* pSource, UINT targetWidth, UINT targetHeight,
                                     IWICBitmap** ppThumbnail)
{
    return GPUAccelerator::Instance().CreateThumbnail(pSource, targetWidth, targetHeight, ppThumbnail);
}

HRESULT ThumbnailGenerator::GenerateFromFile(const std::wstring& filePath, UINT targetWidth, UINT targetHeight,
                                             IWICBitmap** ppThumbnail)
{
    return GPUAccelerator::Instance().CreateThumbnailFromFile(filePath, targetWidth, targetHeight, ppThumbnail);
}
...
void GetOptimalSize(UINT srcWidth, UINT srcHeight, UINT maxSize, UINT& outWidth, UINT& outHeight)
{
