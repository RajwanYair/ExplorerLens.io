...
HRESULT CLENSShell::GetThumbnail_Internal(UINT cx, HBITMAP *phBmpThumbnail,
  ...
  *pdwAlpha = WTSAT_ARGB; // Use alpha channel for modern Windows
  ...
  // Only reachable if: Engine unavailable AND LENSShell_LEGACY_DECODERS compiled in
  ...
  if (FAILED(hr)) {
    DT_LOG_HRESULT(ExplorerLens::LogLevel::LVL_ERROR,
                   ExplorerLens::LogCategory::DECODER, 
...
STDMETHODIMP CLENSShell::GetThumbnail(UINT cx, HBITMAP *phBmpThumbnail,
  ...
  __try {
    ...
    sprintf_s(errorMsg, sizeof(errorMsg),
              "[ExplorerLens] CRITICAL: SEH exception in GetThumbnail - Code: 0x%08X\n",
              exceptionCode);
