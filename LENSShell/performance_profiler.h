// Performance Profiling System for ExplorerLens
...
struct PerformanceMetric
{
    ...
    std::string ToString() const
    {
        ...
        oss << std::fixed << std::setprecision(3) << name << ": calls=" << callCount << ", avg=" << avgTimeMs << "ms"
            << ", min=" << minTimeMs << "ms"
            << ", max=" << maxTimeMs << "ms"
  ...
  private:
    ...
    PerformanceProfiler() : m_enabled(false)
    {
        ...
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Settings", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
            ...
            if (RegQueryValueExW(hKey, L"EnableProfiling", nullptr, nullptr, (LPBYTE)&profiling, &size)
                == ERROR_SUCCESS) {
...
class LENSScopedTimer
{
  ...
  public:
    explicit LENSScopedTimer(const std::string& metricName)
    ...
    // Non-copyable
    LENSScopedTimer(const LENSScopedTimer&) = delete;
    LENSScopedTimer& operator=(const LENSScopedTimer&) = delete;
};

// Convenience macro
#define PROFILE_SCOPE(name) ExplorerLens::LENSScopedTimer __profiler_timer__(name)
