...
public:
  ...
  static HBITMAP GenerateFontPreview(const std::wstring &fontPath,
  ...
  static bool GetFontMetadata(const std::wstring &fontPath,
...
private:
  ...
  static HBITMAP RenderUsingDirectWrite(const std::wstring &fontPath, int width,
                                        int height);

  // GDI+ fallback rendering
  static HBITMAP RenderUsingGDI(const std::wstring &fontPath, int width,
                                int height);

  // Load font file into DirectWrite
  static IDWriteFontFace *LoadFontFile(IDWriteFactory *factory,
                                       const std::wstring &fontPath);

  // Render sample text with given font
  static void RenderSampleText(ID2D1RenderTarget *renderTarget,
  ...
  static void DrawGlyphGrid(ID2D1RenderTarget *renderTarget,
                            IDWriteTextFormat *textFormat,
                            const D2D1_RECT_F &rect,
                            ID2D1SolidColorBrush *brush);

  // Draw font metadata overlay
  static void DrawMetadataOverlay(ID2D1RenderTarget *renderTarget,
