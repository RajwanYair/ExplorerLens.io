...
class CUnzipWrapper
{
  public:
    ...
    {
        return ExtractFirst([](const ZipEntryInfo& e) { return IsImageExtension(e.filename); });
    }
    ...
    static bool IsImageExtension(const std::wstring& name)
    {
        ...
        return ext == L".jpg" || ext == L".jpeg" || ext == L".png" || ext == L".bmp" || ext == L".gif"
               || ext == L".webp" || ext == L".tiff" || ext == L".tif" || ext == L".avif" || ext == L".jxl"
    ...
    void SortEntriesByName()
    {
        std::sort(m_entries.begin(), m_entries.end(),
                  [](const ZipEntryInfo& a, const ZipEntryInfo& b) { return a.filename < b.filename; });
    }
