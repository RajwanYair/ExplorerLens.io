...
public:
  ...
  static HBITMAP GenerateWaveform(const std::wstring &audioPath,
  ...
  static bool GetMetadata(const std::wstring &audioPath,
...
private:
  ...
  static HBITMAP GenerateWaveformWMF(const std::wstring &audioPath, int width,
                                     int height);

  // Generate waveform using direct PCM reading (WAV files)
  static HBITMAP GenerateWaveformPCM(const std::wstring &audioPath, int width,
                                     int height);

  // Draw waveform from audio samples
  static HBITMAP DrawWaveform(const std::vector<float> &samples, int width,
                              int height,
                              COLORREF waveColor = RGB(0, 128, 255));

  // Extract audio samples for visualization
  static bool ExtractAudioSamples(const std::wstring &audioPath,
