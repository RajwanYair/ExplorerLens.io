...
static const CLSID CLSID_LENSShellContextMenu_Internal = {
    0xA7B3F4E1, 0x9C2D, 0x4E8F, {0xB6, 0xA1, 0x3D, 0x5E, 0x7F, 0x9C, 0x2B, 0x4A}};
...
void CLENSExplorerSubCommand::Initialize(LENSContextAction action, const wchar_t* title, const wchar_t* icon,
...
IFACEMETHODIMP CLENSExplorerSubCommand::GetTitle(IShellItemArray* /*pItems*/, LPWSTR* ppszName)
{
    return SHStrDupW(m_title.c_str(), ppszName);
}

IFACEMETHODIMP CLENSExplorerSubCommand::GetIcon(IShellItemArray* /*pItems*/, LPWSTR* ppszIcon)
{
    if (m_icon.empty()) {
        *ppszIcon = nullptr;
        return E_NOTIMPL;
...
IFACEMETHODIMP CLENSExplorerSubCommand::GetToolTip(IShellItemArray* /*pItems*/, LPWSTR* ppszInfotip)
{
...
IFACEMETHODIMP CLENSExplorerSubCommand::GetState(IShellItemArray* /*pItems*/, BOOL /*fOkToBeSlow*/,
...
IFACEMETHODIMP CLENSExplorerSubCommand::EnumSubCommands(IEnumExplorerCommand** ppEnum)
{
    *ppEnum = nullptr;
    return E_NOTIMPL;  // Leaf command — no sub-commands
}

std::wstring CLENSExplorerSubCommand::GetSelectedFilePath(IShellItemArray* pItems)
{
    if (!pItems)
        return L"";
    DWORD count = 0;
...
IFACEMETHODIMP CLENSExplorerSubCommand::Invoke(IShellItemArray* pItems, IBindCtx* /*pBindCtx*/)
{
    ...
    switch (m_action) {
        case LENSContextAction::Regenerate: {
            ...
            SHChangeNotify(SHCNE_UPDATEITEM, SHCNF_PATHW, filePath.c_str(), nullptr);
            break;
        }
        ...
        case LENSContextAction::Settings: {
            ...
            std::wstring managerPath = std::wstring(modulePath) + L"\\LENSManager.exe";
            ShellExecuteW(nullptr, L"open", managerPath.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
            break;
        }
...
void CLENSEnumExplorerCommand::Initialize(const std::vector<CComPtr<IExplorerCommand>>& commands)
{
    m_commands = commands;
    m_current = 0;
}

IFACEMETHODIMP CLENSEnumExplorerCommand::Next(ULONG celt, IExplorerCommand** pUICommand, ULONG* pceltFetched)
{
    ULONG fetched = 0;
    while (fetched < celt && m_current < static_cast<ULONG>(m_commands.size())) {
...
{
    m_current += celt;
    return (m_current <= static_cast<ULONG>(m_commands.size())) ? S_OK : S_FALSE;
}
...
IFACEMETHODIMP CLENSEnumExplorerCommand::Clone(IEnumExplorerCommand** ppEnum)
{
    *ppEnum = nullptr;
    return E_NOTIMPL;
}
...
void CLENSShellContextMenu::BuildSubCommands()
{
    ...
    static const SubCmdDef defs[] = {
        {LENSContextAction::Regenerate, L"Regenerate Thumbnail", L"Force regeneration of the cached thumbnail"},
        {LENSContextAction::ClearCache, L"Clear Thumbnail Cache", L"Remove cached thumbnail for this file"},
        {LENSContextAction::ShowInfo, L"File Properties", L"Show file details and metadata"},
        {LENSContextAction::CopyThumbnail, L"Copy File Path", L"Copy the file path to clipboard"},
        {LENSContextAction::ExportAs, L"Show in Explorer", L"Open containing folder and select file"},
        {LENSContextAction::Settings, L"ExplorerLens Settings...", L"Open the ExplorerLens configuration utility"},
    };

    for (const auto& def : defs) {
        CComObject<CLENSExplorerSubCommand>* pSub = nullptr;
        if (SUCCEEDED(CComObject<CLENSExplorerSubCommand>::CreateInstance(&pSub))) {
            pSub->AddRef();
...
IFACEMETHODIMP CLENSShellContextMenu::GetTitle(IShellItemArray* /*pItems*/, LPWSTR* ppszName)
{
    return SHStrDupW(L"ExplorerLens", ppszName);
}

IFACEMETHODIMP CLENSShellContextMenu::GetIcon(IShellItemArray* /*pItems*/, LPWSTR* ppszIcon)
{
    // Use the ExplorerLens icon from resources
    wchar_t modulePath[MAX_PATH] = {};
    GetModuleFileNameW(_AtlBaseModule.GetModuleInstance(), modulePath, MAX_PATH);
    // Icon resource index 0 (IDI_ICON1)
    std::wstring iconRef = std::wstring(modulePath) + L",0";
    return SHStrDupW(iconRef.c_str(), ppszIcon);
}

IFACEMETHODIMP CLENSShellContextMenu::GetToolTip(IShellItemArray* /*pItems*/, LPWSTR* ppszInfotip)
{
    return SHStrDupW(L"ExplorerLens thumbnail operations", ppszInfotip);
}

IFACEMETHODIMP CLENSShellContextMenu::GetCanonicalName(GUID* pguidCommandName)
{
...
IFACEMETHODIMP CLENSShellContextMenu::GetState(IShellItemArray* pItems, BOOL /*fOkToBeSlow*/, EXPCMDSTATE* pCmdState)
{
...
IFACEMETHODIMP CLENSShellContextMenu::EnumSubCommands(IEnumExplorerCommand** ppEnum)
{
    if (!ppEnum)
        return E_POINTER;
    *ppEnum = nullptr;
    ...
    HRESULT hr = CComObject<CLENSEnumExplorerCommand>::CreateInstance(&pEnum);
    if (FAILED(hr))
        return hr;
...
IFACEMETHODIMP CLENSShellContextMenu::Invoke(IShellItemArray* /*pItems*/, IBindCtx* /*pBindCtx*/)
{
