  ...
  public:
    ...
    void RecordAttempt(const std::string& format, bool success, SIZE_T memoryUsed = 0)
    {
        ...
        if (format == "zip" || format == ".zip")
            ...
            m_metrics.rarCount++;
        else if (format == "cbz" || format == ".cbz")
            m_metrics.cbzCount++;
        else if (format == "cbr" || format == ".cbr")
            m_metrics.cbrCount++;
        else if (format == "7z" || format == ".7z")
            m_metrics.sevenzCount++;
        else if (format == "webp" || format == ".webp")
            m_metrics.webpCount++;
        else if (format == "avif" || format == ".avif")
            m_metrics.avifCount++;
        else if (format == "jxl" || format == ".jxl")
            m_metrics.jxlCount++;
        ...
        if (memoryUsed > 0) {
            ...
            while (memoryUsed > current && !m_metrics.peakMemoryBytes.compare_exchange_weak(current, memoryUsed)) {}
        }
    }
    ...
    void UpdateMemoryPeak()
    {
        ...
        while (current > peak && !m_metrics.peakMemoryBytes.compare_exchange_weak(peak, current)) {}
    }
    ...
    std::string GetSummary() const
    {
        ...
        oss << "ExplorerLens Performance Metrics\n";
        oss << "==============================\n\n";
        ...
        oss << "  Successful:      " << m_metrics.successCount.load() << " (" << m_metrics.GetSuccessRate() << "%)\n";
        ...
        if (m_metrics.zipCount > 0)
            oss << "  ZIP:   " << m_metrics.zipCount.load() << "\n";
        if (m_metrics.rarCount > 0)
            oss << "  RAR:   " << m_metrics.rarCount.load() << "\n";
        if (m_metrics.cbzCount > 0)
            oss << "  CBZ:   " << m_metrics.cbzCount.load() << "\n";
        if (m_metrics.cbrCount > 0)
            oss << "  CBR:   " << m_metrics.cbrCount.load() << "\n";
        if (m_metrics.sevenzCount > 0)
            oss << "  7Z:    " << m_metrics.sevenzCount.load() << "\n";
        if (m_metrics.webpCount > 0)
            oss << "  WebP:  " << m_metrics.webpCount.load() << "\n";
        if (m_metrics.avifCount > 0)
            oss << "  AVIF:  " << m_metrics.avifCount.load() << "\n";
        if (m_metrics.jxlCount > 0)
            oss << "  JXL:   " << m_metrics.jxlCount.load() << "\n";
        oss << "\n";
...
class ThumbnailMetricsScope
{
  ...
  public:
    explicit ThumbnailMetricsScope(const std::string& format) : m_format(format), m_success(false)
    {
        PROCESS_MEMORY_COUNTERS pmc;
        m_memoryBefore = 0;
        if (GetProcessMemoryInfo(GetCurrentProcess(), &pmc, sizeof(pmc))) {
    ...
    ~ThumbnailMetricsScope()
    {
        ...
        SIZE_T memoryUsed = (memoryAfter > m_memoryBefore) ? (memoryAfter - m_memoryBefore) : 0;
