...
class SVGDecoder
{
  public:
    ...
    static HRESULT DecodeToHBITMAP(const BYTE* svgData, size_t dataSize, HBITMAP* phBitmap, int maxWidth = 256,
                                   int maxHeight = 256)
    {
        ...
        *phBitmap = NULL;
        ...
        HRESULT hr = RenderSVGDirect2D(svgData, dataSize, phBitmap, maxWidth, maxHeight);
  ...
  private:
    ...
    static HRESULT RenderSVGDirect2D(const BYTE* svgData, size_t dataSize, HBITMAP* phBitmap, int width, int height)
    {
        HRESULT hr = S_OK;
        ...
        hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, createFlags, nullptr, 0, D3D11_SDK_VERSION,
                               &pD3DDevice, &featureLevel, nullptr);
        if (FAILED(hr)) {
            // Try WARP (software) fallback
            hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_WARP, nullptr, createFlags, nullptr, 0, D3D11_SDK_VERSION,
                                   &pD3DDevice, &featureLevel, nullptr);
            if (FAILED(hr))
                return hr;
        }
        ...
        hr = D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof(ID2D1Factory3), &factoryOptions,
        ...
        hr = pD2DDevice->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_NONE, &pDC2);
        if (FAILED(hr))
            return hr;
        ...
        hr = pWICFactory->CreateBitmap(width, height, GUID_WICPixelFormat32bppPBGRA, WICBitmapCacheOnDemand,
                                       &pWICBitmap);
        if (FAILED(hr))
            return hr;
        ...
        D2D1_SIZE_F viewportSize = D2D1::SizeF(static_cast<float>(width), static_cast<float>(height));
    ...
    static HRESULT CreateSVGPlaceholderBitmap(HBITMAP* phBitmap, int width, int height)
    {
        CComPtr<IWICImagingFactory> pWICFactory;
        HRESULT hr =
    ...
    static HRESULT ConvertWICBitmapToHBITMAP(IWICImagingFactory* pFactory, IWICBitmap* pWICBitmap, HBITMAP* phBitmap)
    {
