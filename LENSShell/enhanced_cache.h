// Enhanced Multi-Level Cache System for ExplorerLens
...
class EnhancedCacheManager
{
  private:
    ...
    {
        HKEY hKey;
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Cache", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
            DWORD value = 0;
    ...
    void InitializeDiskCache()
    {
        ...
        if (SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, localAppData) == S_OK) {
            ...
            m_diskCachePath += L"\\ExplorerLens\\cache";
    ...
    bool LoadFromDisk(const std::string& key, std::vector<BYTE>& data)
    {
        ...
        DWORD bytesRead = 0;
        bool success = ReadFile(hFile, data.data(), static_cast<DWORD>(fileSize.QuadPart), &bytesRead, nullptr);
  ...
  public:
    ...
    std::string GetStatistics() const
    {
        ...
        double totalHitRate =
            totalRequests > 0 ? (static_cast<double>(m_memoryHits + m_diskHits) / totalRequests * 100.0) : 0.0;
