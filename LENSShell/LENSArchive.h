...
// v8.4.0 (Sprint 175+)
//////////////////////////////////////////////
// CLENSShell functionality implementation
...
// Legacy decoders are gated behind LENSShell_LEGACY_DECODERS and excluded from build by default.
// To re-enable for debugging: add LENSShell_LEGACY_DECODERS to Preprocessor Definitions.
// ============================================================================
...
// Video formats (v5.0+) - Deprecated individual types, use LENSTYPE_VIDEO
// #define LENSTYPE_MP4 31	// .mp4 MPEG-4 video - use LENSTYPE_VIDEO
// #define LENSTYPE_MKV 32	// .mkv Matroska video - use LENSTYPE_VIDEO
// #define LENSTYPE_AVI 33	// .avi Audio Video Interleave - use LENSTYPE_VIDEO
// #define LENSTYPE_MOV 34	// .mov QuickTime movie - use LENSTYPE_VIDEO
// #define LENSTYPE_WEBM 35 // .webm WebM video - use LENSTYPE_VIDEO
...
#define LENSTYPE_DOCUMENT 81	// Generic document type (DOCX/PPTX/etc. fall here if not specific)
...
#define LENSTYPE_JP2 86	// .jp2/.j2k/.j2c/.jpx JPEG 2000 (Sprint 181)
#define LENSTYPE_EPS 87	// .eps/.epsf/.ps Encapsulated PostScript (Sprint 183)
#define LENSTYPE_KTX 88	// .ktx/.ktx2 Khronos KTX texture (Sprint 184)
#define LENSTYPE_VTF 89	// .vtf Valve Texture Format (Sprint 184)
#define LENSTYPE_ORA 90	// .ora OpenRaster image (Sprint 185)
#define LENSTYPE_XCF 91	// .xcf GIMP native image (Sprint 185)
#define LENSTYPE_SGI 92	// .sgi/.rgb/.rgba/.bw SGI image (Sprint 186)
#define LENSTYPE_XPM 93	// .xpm X PixMap image (Sprint 186)
...
namespace __LENS
{
	...
	public:
		...
		HRESULT OnExtract(HBITMAP *phBmpThumbnail)
		{
			*phBmpThumbnail = NULL;
			...
			{
				HBITMAP cachedBitmap = ExplorerLens::ThumbnailCache::LoadFromCache(archivePath, imageName, (ULONGLONG)fileSize, lastModified);
				if (cachedBitmap)
				{
			...
			try
			{
					...
					if (collageMode != ExplorerLens::ThumbnailCollage::MODE_SINGLE)
					{
						...
						if (!images.empty())
						{
							...
							*phBmpThumbnail = ExplorerLens::ThumbnailCollage::CreateCollage(
							...
							if (*phBmpThumbnail)
							{
								ExplorerLens::ThumbnailCache::SaveToCache(archivePath, imageName, (ULONGLONG)fileSize, lastModified, *phBmpThumbnail);
							}
					...
					while (ExplorerLens::LibArchiveWrapper::ReadNextEntry())
					{
						if (currentIndex == thumbindex)
						{
							...
							if (hG)
							{
								...
								if (pBuf)
								{
									SSIZE_T bytesRead = ExplorerLens::LibArchiveWrapper::ExtractEntryToMemory(pBuf, (size_t)entrySize);
				...
				case LENSTYPE_AUDIO:
				{
					...
					HBITMAP hWaveform = ExplorerLens::AudioThumbnail::GenerateWaveform(audioPath, m_thumbSize.cx, m_thumbSize.cy);
						...
						if (SUCCEEDED(hr) && bytesRead == fileSize64)
						{
							...
							if (S_OK == CreateStreamOnHGlobal(hG, TRUE, (LPSTREAM *)&pIs))
							{
								*phBmpThumbnail = ThumbnailFromIStream(pIs, &m_thumbSize);
								...
								if (*phBmpThumbnail)
								{
									ExplorerLens::ThumbnailCache::SaveToCache(archivePath, imageName, (ULONGLONG)fileSize64, lastModified, *phBmpThumbnail);
								}
	...
	private:
		CString m_LENSFile; // overcome MAX_PATH limit?
		SIZE m_thumbSize;
		int i, j; // helpers
		LENSTYPE m_LENSTYPE;
		IStream *m_pIs;
	...
	private:
		...
		inline LENSTYPE GetLENSTYPE(LPCTSTR szExt)
		{
			...
			if (StrEqual(szExt, _T(".cbz")))
				return LENSTYPE_CBZ;
			// if (StrEqual(szExt, _T(".cbr"))) return LENSTYPE_CBR;  // CBR removed
			...
			// Fixed in Sprint 175: djvu/djv now correctly route to LENSTYPE_DJVU
				...
				if (SUCCEEDED(pIs->Stat(&stat, STATFLAG_NONAME)))
				{
					...
					if (streamSize > 0 && streamSize < LENSMEM_MAXBUFFER_SIZE)
					{
						...
						if (SUCCEEDED(pIs->Read(buffer.data(), static_cast<ULONG>(streamSize), &bytesRead)) && bytesRead == streamSize)
						{
							...
							{
								if (SUCCEEDED(ExplorerLens::WebPDecoder::DecodeToHBITMAP(buffer.data(), streamSize, &hModernBitmap)))
								{
							...
							if (ExplorerLens::HEIFDecoderNative::IsHEIFFormat(buffer.data(), streamSize))
							{
								...
								if (SUCCEEDED(ExplorerLens::HEIFDecoderNative::DecodeToHBITMAP(buffer.data(), streamSize, &hModernBitmap, isDarkMode)))
								{
									return ScaleBitmapToThumbnail(hModernBitmap, pThumbSize);
								}
							}
							...
							{
								if (SUCCEEDED(ExplorerLens::AVIFDecoder::DecodeToHBITMAP(buffer.data(), streamSize, &hModernBitmap)))
								{
									return ScaleBitmapToThumbnail(hModernBitmap, pThumbSize);
								}
							}
							...
							{
								if (SUCCEEDED(ExplorerLens::JXLDecoder::DecodeToHBITMAP(buffer.data(), streamSize, &hModernBitmap)))
								{
									return ScaleBitmapToThumbnail(hModernBitmap, pThumbSize);
								}
							}
							...
							if (ExplorerLens::PDFDecoder::IsPDFFormat(buffer.data(), streamSize))
							{
								...
								if (SUCCEEDED(ExplorerLens::PDFDecoder::DecodeToHBITMAP(buffer.data(), streamSize, &hModernBitmap,
							...
							{
								if (SUCCEEDED(ExplorerLens::SVGDecoder::DecodeToHBITMAP(buffer.data(), streamSize, &hModernBitmap,
																					  pThumbSize ? pThumbSize->cx : 256, pThumbSize ? pThumbSize->cy : 256)))
								{
									return ScaleBitmapToThumbnail(hModernBitmap, pThumbSize);
								}
							...
							// Try RAW camera decoder (LibRaw-based, Sprint 13)
							{
								...
								HRESULT hrRaw = ExplorerLens::RAWDecoder::DecodeToHBITMAP(buffer.data(), streamSize, &hModernBitmap,
