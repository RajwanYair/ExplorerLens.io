...
class DocumentThumbnail
{
  public:
    ...
    static HBITMAP ExtractDocumentThumbnail(const std::wstring& filePath, int width = 256, int height = 256);
    ...
    static bool GetMetadata(const std::wstring& filePath, DocumentMetadata& metadata);
  ...
  private:
    ...
    static HBITMAP ExtractUsingStorageAPI(const std::wstring& filePath, int width, int height);

    // Strategy 2: IShellItemImageFactory (Windows Vista+)
    static HBITMAP ExtractUsingShellItem(const std::wstring& filePath, int width, int height);

    // Strategy 3: Embedded thumbnail from Office Open XML
    static HBITMAP ExtractEmbeddedThumbnail(const std::wstring& filePath);

    // Strategy 4: Generate generic document icon with metadata
    static HBITMAP GenerateGenericThumbnail(const std::wstring& filePath, DocumentType type, int width, int height);

    // Helper: Extract thumbnail from ZIP-based Office file (docx, pptx, xlsx)
    static HBITMAP ExtractFromOpenXML(const std::wstring& filePath, DocumentType type);

    // Helper: Read document properties using OLE Automation
    static bool ReadOLEProperties(const std::wstring& filePath, DocumentMetadata& metadata);
    ...
    static HBITMAP CreateDocumentBackground(int width, int height, DocumentType type);

    // Helper: Draw metadata overlay on thumbnail
    static void DrawMetadataOverlay(HDC hdc, const RECT& rect, const DocumentMetadata& metadata, DocumentType type);
};
