// Error Logging System for ExplorerLens
...
class Logger
{
  private:
    ...
    Logger() : m_minLevel(LogLevel::LVL_WARNING), m_enabled(false), m_debugMode(false)
    {
        ...
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Settings", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
            ...
            if (RegQueryValueExW(hKey, L"DebugLogging", nullptr, nullptr, (LPBYTE)&debugMode, &size) == ERROR_SUCCESS) {
    ...
    void InitializeLogFile()
    {
        ...
        if (SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, localAppData) == S_OK) {
            ...
            filename << m_logPath << L"\\ExplorerLens_" << std::put_time(&tm_now, L"%Y%m%d_%H%M%S") << L".log";
  ...
  public:
    ...
    void Log(LogLevel level, LogCategory category, const std::string& message, const char* file = nullptr, int line = 0)
    {
    ...
    void LogHRESULT(LogLevel level, LogCategory category, const std::string& operation, HRESULT hr,
                    const char* file = nullptr, int line = 0)
    {
        std::ostringstream msg;
        msg << operation << " failed with HRESULT 0x" << std::hex << std::uppercase << hr;
...
#define DT_LOG_DEBUG(category, msg) \
    ExplorerLens::Logger::Instance().Log(ExplorerLens::LogLevel::LVL_DEBUG, category, msg, __FILE__, __LINE__)
#define DT_LOG_INFO(category, msg) \
    ExplorerLens::Logger::Instance().Log(ExplorerLens::LogLevel::LVL_INFO, category, msg, __FILE__, __LINE__)
#define DT_LOG_WARNING(category, msg) \
    ExplorerLens::Logger::Instance().Log(ExplorerLens::LogLevel::LVL_WARNING, category, msg, __FILE__, __LINE__)
#define DT_LOG_ERROR(category, msg) \
    ExplorerLens::Logger::Instance().Log(ExplorerLens::LogLevel::LVL_ERROR, category, msg, __FILE__, __LINE__)
#define DT_LOG_CRITICAL(category, msg) \
    ExplorerLens::Logger::Instance().Log(ExplorerLens::LogLevel::LVL_CRITICAL, category, msg, __FILE__, __LINE__)
#define DT_LOG_HRESULT(level, category, operation, hr) \
    ExplorerLens::Logger::Instance().LogHRESULT(level, category, operation, hr, __FILE__, __LINE__)
