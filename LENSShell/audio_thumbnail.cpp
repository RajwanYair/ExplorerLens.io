...
AudioThumbnail::ExtractAlbumArtPropertySystem(const std::wstring& audioPath)
{
    ...
    HRESULT hr = SHGetPropertyStoreFromParsingName(audioPath.c_str(), NULL, GPS_DEFAULT, IID_PPV_ARGS(&propStore));
...
bool AudioThumbnail::ExtractAudioSamples(const std::wstring& audioPath, std::vector<float>& samples, int maxSamples)
{
    ...
    while (ReadFile(hFile, chunkID, 4, &bytesRead, NULL) && bytesRead == 4) {
        ...
        if (memcmp(chunkID, "data", 4) == 0) {
            ...
            while (samplesRead < maxSamples && ReadFile(hFile, buffer.data(), (DWORD)buffer.size(), &bytesRead, NULL)
                   && bytesRead > 0) {
