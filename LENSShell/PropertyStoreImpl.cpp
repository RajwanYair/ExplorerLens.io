...
HRESULT CLENSPropertyStore::InitializeProperties(const wchar_t* filePath, int lensType)
{
    if (m_initialized)
        return S_OK;
...
HRESULT CLENSPropertyStore::PropertyStore_GetAt(DWORD iProp, PROPERTYKEY* pkey)
{
    if (!pkey)
        return E_POINTER;
...
HRESULT CLENSPropertyStore::PropertyStore_GetValue(REFPROPERTYKEY key, PROPVARIANT* pv)
{
    if (!pv)
        return E_POINTER;
    PropVariantInit(pv);
...
HRESULT CLENSPropertyStore::PropertyStore_SetValue(REFPROPERTYKEY /*key*/, REFPROPVARIANT /*propvar*/)
{
...
HRESULT CLENSPropertyStore::PropertyStoreCapabilities_IsPropertyWritable(REFPROPERTYKEY /*key*/)
{
...
void CLENSPropertyStore::AddStringProperty(REFPROPERTYKEY key, const wchar_t* value)
{
    StoredProperty prop;
    prop.key = key;
...
void CLENSPropertyStore::AddStringVectorProperty(REFPROPERTYKEY key, const wchar_t* value)
{
    StoredProperty prop;
    prop.key = key;
...
const CLENSPropertyStore::StoredProperty* CLENSPropertyStore::FindProperty(REFPROPERTYKEY key) const
{
...
std::wstring CLENSPropertyStore::GetMimeTypeForExtension(const wchar_t* filePath)
{
    if (!filePath)
        return L"";
...
bool CLENSPropertyStore::IsImageType(int lensType)
{
    return (lensType >= LENSTYPE_WEBP && lensType <= LENSTYPE_RAW)
           ...
           || (lensType >= LENSTYPE_TGA && lensType <= LENSTYPE_XPM) || lensType == LENSTYPE_HDR
           || lensType == LENSTYPE_EXR;
}

bool CLENSPropertyStore::IsArchiveType(int lensType)
{
    return lensType == LENSTYPE_ZIP || lensType == LENSTYPE_CBZ || lensType == LENSTYPE_7Z || lensType == LENSTYPE_CB7
           || lensType == LENSTYPE_TAR || lensType == LENSTYPE_CBT
           || (lensType >= LENSTYPE_BZIP2 && lensType <= LENSTYPE_LZMA) || lensType == LENSTYPE_LZ4
           || (lensType >= LENSTYPE_TAR_GZ && lensType <= LENSTYPE_CPIO) || lensType == LENSTYPE_ISO
           || lensType == LENSTYPE_XAR || lensType == LENSTYPE_AR || lensType == LENSTYPE_DEB
