...
{
    return static_cast<GPUCapability>(static_cast<UINT>(a) | static_cast<UINT>(b));
}
...
class GPUAccelerator
{
  public:
    ...
    HRESULT CreateThumbnail(IWICBitmapSource* pSource, UINT targetWidth, UINT targetHeight, IWICBitmap** ppThumbnail);

    // Thumbnail generation from file (synchronous)
    HRESULT CreateThumbnailFromFile(const std::wstring& filePath, UINT targetWidth, UINT targetHeight,
                                    IWICBitmap** ppThumbnail);

    // Async thumbnail queue
    void QueueThumbnail(const std::wstring& filePath, UINT targetWidth, UINT targetHeight, int priority,
  ...
  private:
    ...
    // Rendering helpers
    HRESULT CreateTextureFromWIC(IWICBitmapSource* pSource, ID3D11Texture2D** ppTexture,
                                 ID3D11ShaderResourceView** ppSRV);

    HRESULT ResizeTexture(ID3D11ShaderResourceView* pSourceSRV, UINT sourceWidth, UINT sourceHeight, UINT targetWidth,
                          UINT targetHeight, ID3D11Texture2D** ppOutput);

    HRESULT ResizeTextureComputeShader(ID3D11ShaderResourceView* pSourceSRV, UINT sourceWidth, UINT sourceHeight,
    ...
    // CPU fallback
    HRESULT CreateThumbnailCPU(IWICBitmapSource* pSource, UINT targetWidth, UINT targetHeight, IWICBitmap** ppThumbnail);

    // Texture pool management
    HRESULT AcquireTexture(UINT width, UINT height, DXGI_FORMAT format, UINT bindFlags, ID3D11Texture2D** ppTexture,
                           ID3D11ShaderResourceView** ppSRV = nullptr, ID3D11UnorderedAccessView** ppUAV = nullptr);
...
{
  public:
    static HRESULT Generate(IWICBitmapSource* pSource, UINT targetWidth, UINT targetHeight, IWICBitmap** ppThumbnail);

    static HRESULT GenerateFromFile(const std::wstring& filePath, UINT targetWidth, UINT targetHeight,
                                    IWICBitmap** ppThumbnail);
};
