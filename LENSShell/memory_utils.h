...
class CoTaskMemPtr
{
  ...
  public:
    ...
    explicit CoTaskMemPtr(size_t count) : m_ptr(static_cast<T*>(CoTaskMemAlloc(count * sizeof(T)))) {}
  ...
  private:
    ...
    MemoryTracker() : m_totalAllocated(0), m_peakAllocated(0), m_enabled(false)
    {
        ...
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Settings", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
            ...
            if (RegQueryValueExW(hKey, L"EnableMemoryTracking", nullptr, nullptr, (LPBYTE)&tracking, &size)
                == ERROR_SUCCESS) {
  ...
  public:
    ...
    std::string GetReport() const
    {
        ...
        for (const auto& pair : categoryTotals) {
            report << "  " << pair.first << ": " << (pair.second / 1024.0 / 1024.0) << " MB\n";
        }
...
class TrackedPtr
{
  ...
  public:
    ...
    // Move semantics
    TrackedPtr(TrackedPtr&& other) noexcept : m_ptr(other.m_ptr), m_category(std::move(other.m_category))
    {
        other.m_ptr = nullptr;
    }
