...
namespace ExplorerLens
{
    ...
    HBITMAP DocumentThumbnail::ExtractDocumentThumbnail(
    ...
    HBITMAP DocumentThumbnail::ExtractUsingShellItem(
        const std::wstring &filePath,
        int width,
        int height)
    {
    ...
    HBITMAP DocumentThumbnail::ExtractEmbeddedThumbnail(
        const std::wstring &filePath)
    {
    ...
    HBITMAP DocumentThumbnail::GenerateGenericThumbnail(
        const std::wstring &filePath,
        DocumentType type,
        int width,
        int height)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            int lineWidth = (i == 3) ? (docRect.right - docRect.left - margin) / 2 : (docRect.right - docRect.left - margin);
        ...
        if (!ext.empty() && ext[0] == L'.')
        {
            ...
            DrawText(hdcMem, ext.c_str(), -1, &textRect,
    ...
    bool DocumentThumbnail::GetMetadata(
        const std::wstring &filePath,
    ...
    DocumentThumbnail::DocumentType DocumentThumbnail::GetDocumentType(
        const std::wstring &filePath)
    {
        std::wstring ext = PathFindExtension(filePath.c_str());
        if (ext.empty())
