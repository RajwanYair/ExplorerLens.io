...
std::wstring ThumbnailCache::GetCacheDirectory()
{
    ...
    if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, localAppData))) {
        std::wstring cachePath = std::wstring(localAppData) + L"\\ExplorerLens\\cache";
        return cachePath;
    }
    return L"";
}
...
std::wstring ThumbnailCache::GenerateCacheKey(const std::wstring& archivePath, const std::wstring& imageName,
                                              ULONGLONG fileSize, FILETIME lastModified)
{
    // Create unique key: path + image + size + mtime
    std::wstringstream ss;
    ss << archivePath << L"|" << imageName << L"|" << fileSize << L"|"
...
bool ThumbnailCache::IsCached(const std::wstring& archivePath, const std::wstring& imageName, ULONGLONG fileSize,
...
HBITMAP ThumbnailCache::LoadFromCache(const std::wstring& archivePath, const std::wstring& imageName,
                                      ULONGLONG fileSize, FILETIME lastModified)
{
    std::wstring cacheKey = GenerateCacheKey(archivePath, imageName, fileSize, lastModified);
    std::wstring cachePath = GetCachePath(cacheKey);
...
bool ThumbnailCache::SaveToCache(const std::wstring& archivePath, const std::wstring& imageName, ULONGLONG fileSize,
