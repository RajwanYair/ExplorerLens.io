...
# Installation script for ExplorerLens thumbnail provider
...
$SourceFiles = @{
    "LENSShell.dll"         = "x64\$BuildConfig\LENSShell.dll"
    "LENSManager.exe"       = "x64\$BuildConfig\LENSManager.exe"
    "ExplorerLensEngine.dll" = "build\bin\$BuildConfig\ExplorerLensEngine.dll"
}
...
Write-Host "  ExplorerLens Installer v$Version" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if ($missingFiles.Count -gt 0) {
    ...
    Write-Host "  msbuild LENSShell.sln /p:Configuration=Release /p:Platform=x64" -ForegroundColor Gray
    exit 1
}
...
if ($Upgrade -or (Test-Path $InstallPath)) {
    Write-Host "UPGRADING ExplorerLens..." -ForegroundColor Yellow
} else {
    Write-Host "INSTALLING ExplorerLens..." -ForegroundColor Green
}
Write-Host ""
...
if ((Test-Path $InstallPath) -and (-not $SkipBackup)) {
    $backupPath = Backup-Installation -BackupPath (Join-Path $env:TEMP "ExplorerLens_Backups")
}
...
try {
    ...
    if (Test-Installation) {
        ...
        Write-Host "============================================" -ForegroundColor Green
        Write-Host ""
        Write-Host "ExplorerLens $Version is now installed at:" -ForegroundColor White
