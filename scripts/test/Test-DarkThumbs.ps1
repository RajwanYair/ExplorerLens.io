...
Write-Host "|    DarkThumbs v5.2.0+ Test Suite       |" -ForegroundColor Cyan
Write-Host "+========================================+" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $systemDll) {
    Add-TestResult "Installation" "CBXShell.dll exists" "Pass"
    ...
    Add-TestResult "Installation" "CBXShell.dll exists" "Fail" "DLL not found"
}
...
if ($explorerProc) {
    ...
    if (([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        $modules = $explorerProc | ForEach-Object { $_.Modules } | Where-Object { $_.FileName -like "*CBXShell.dll" }
        if ($modules) {
