...
function Show-Banner {
    ...
    Write-Host "|   DarkThumbs v5.2.0+ Deployment Tool   |" -ForegroundColor Cyan
    Write-Host "+========================================+" -ForegroundColor Cyan
    Write-Host ""
}
...
# Uninstall DarkThumbs
function Uninstall-DarkThumbs {
    Write-Info "Uninstalling DarkThumbs..."
    
    $systemDll = "$env:SystemRoot\System32\CBXShell.dll"
    $managerExe = "$env:ProgramFiles\DarkThumbs\CBXManager.exe"
    $managerDir = "$env:ProgramFiles\DarkThumbs"
    ...
    if (Test-Path $systemDll) {
        ...
        try {
            ...
            Write-Success "Removed CBXShell.dll"
        } catch {
            Write-ErrorMsg "Could not remove CBXShell.dll: $_"
        }
    } else {
        Write-Info "CBXShell.dll not found (already uninstalled)"
    }
    ...
    if (Test-Path $managerDir) {
        try {
            ...
            Write-Success "Removed DarkThumbs directory"
        } catch {
            Write-Warning "Could not remove DarkThumbs directory: $_"
        }
    }
}

# Install DarkThumbs
function Install-DarkThumbs {
    param([string]$source)
    
    Write-Info "Installing DarkThumbs..."
    
    $dllSource = Join-Path $source "CBXShell.dll"
    $exeSource = Join-Path $source "CBXManager.exe"
    
    # Verify source files exist
    if (-not (Test-Path $dllSource)) {
        Write-ErrorMsg "CBXShell.dll not found in $source"
        return $false
    }
    
    if (-not (Test-Path $exeSource)) {
        Write-ErrorMsg "CBXManager.exe not found in $source"
        return $false
    }
    ...
    try {
        ...
        Write-Success "Copied CBXShell.dll to System32"
    ...
    try {
        Copy-Item $exeSource "$managerDir\CBXManager.exe" -Force
        Write-Success "Installed CBXManager.exe"
    } catch {
        Write-Warning "Could not install CBXManager.exe: $_"
    }
...
function Test-Installation {
    ...
    if (-not (Test-Path $systemDll)) {
        Write-ErrorMsg "CBXShell.dll not found"
        return $false
    }
    
    Write-Success "CBXShell.dll present"
...
function Show-Status {
    ...
    if (Test-Path $systemDll) {
        Write-Success "DarkThumbs is installed"
        ...
        $managerPath = "$env:ProgramFiles\DarkThumbs\CBXManager.exe"
        ...
        Write-Info "DarkThumbs is not installed"
    }
}
...
try {
    ...
    elseif ($Uninstall) {
        ...
        if (Install-DarkThumbs -source $SourcePath) {
            Clear-ThumbnailCache
