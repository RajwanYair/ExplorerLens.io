...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    ...
    if (!hResMod)
        hResMod = ::GetModuleHandle(NULL);

    HICON hIcon = (HICON)::LoadImage(hResMod, MAKEINTRESOURCE(IDR_MAINFRAME), IMAGE_ICON, ::GetSystemMetrics(SM_CXICON),
    ...
    HICON hIconSmall =
        (HICON)::LoadImage(hResMod, MAKEINTRESOURCE(IDR_MAINFRAME), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON),
    ...
    int allCheckboxes[] = {IDC_CB_CBZ,       IDC_CB_CBR,     IDC_CB_CB7,         IDC_CB_CBT,         IDC_CB_EPUB,
...
void CMainDlg::RelayoutControls(int clientWidth, int clientHeight)
{
    ...
    for (const auto& anchor : m_anchors) {
        ...
        if (anchor.id == IDC_STATUSBAR) {
            ...
            hDwp = ::DeferWindowPos(hDwp, hCtrl, NULL, 0, clientHeight - sbHeight, clientWidth, sbHeight,
        ...
        hDwp = ::DeferWindowPos(hDwp, hCtrl, NULL, newX, newY, newW, newH, SWP_NOZORDER | SWP_SHOWWINDOW);
    }
...
LRESULT CMainDlg::OnApply(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    ...
    if (result == IDC_BTN_RESTORE && dlg.ShouldRestore()) {
        ...
        MessageBox(_T("Previous configuration has been restored."), _T("Configuration Restored"),
...
{
    auto& darkCtrl = ExplorerLens::DarkModeController::Instance();
    return reinterpret_cast<LRESULT>(darkCtrl.OnCtlColor(reinterpret_cast<HDC>(wParam), reinterpret_cast<HWND>(lParam)));
}
...
void CMainDlg::InitTooltips()
{
    ...
    AddTooltipWithStatus(IDC_CB_AVIF, LENS_AVIF,
    ...
    // Media & Documents
    AddTooltipWithStatus(IDC_CB_VIDEO, LENS_VIDEO,
    ...
    AddTooltipWithStatus(IDC_CB_RAW, LENS_RAW,
    ...
    AddTooltipWithStatus(IDC_CB_HDR, LENS_HDR,
    ...
    m_tooltip.AddTool(GetDlgItem(IDC_RADIO_1X1), _T("Single Page Mode - Show only the first page\nDefault ")
                                                 _T("for most formats"));
    m_tooltip.AddTool(GetDlgItem(IDC_RADIO_2X2),
...
void CMainDlg::UpdateStatusBar()
{
    ...
    int conflictCount = 0;
    const int allFormats[] = {LENS_CBZ,     LENS_CBR,         LENS_CB7,         LENS_CBT,  LENS_EPUB,  LENS_MOBI,
                              ...
                              LENS_TGA,     LENS_AUDIO,       LENS_DOCUMENT,    LENS_FONT, LENS_MODEL, LENS_EXT_IMAGE,
                              LENS_TEXTURE, LENS_EXT_ARCHIVE, LENS_EXT_DOCUMENT};
    ...
    if (conflictCount > 0) {
        ...
        statusText.Format(_T("Ready - %d of 200+ formats enabled | %s"), enabledCount, perfSummary.c_str());
    }
...
void CMainDlg::InitStatusIcons()
{
    ...
    m_checkboxStatus[IDC_CB_EXT_IMAGE] = m_reg.GetHandlerStatus(LENS_EXT_IMAGE, m_reg.GetExtension(LENS_EXT_IMAGE));
    m_checkboxStatus[IDC_CB_TEXTURE] = m_reg.GetHandlerStatus(LENS_TEXTURE, m_reg.GetExtension(LENS_TEXTURE));
...
LRESULT CMainDlg::OnCategoryAllClicked(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    struct CategoryMap
    {
        int allID;
...
void CMainDlg::UpdateCategoryAllState(int allCheckboxID, const int* memberIDs, int count)
{
    ...
    Button_SetCheck(GetDlgItem(allCheckboxID), (checked == count) ? BST_CHECKED : BST_UNCHECKED);
}
...
void CMainDlg::RecreateFont(int pointSize)
{
    ...
    m_hFont = ::CreateFont(lfHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                           CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, _T("Segoe UI"));

    if (!m_hFont) {
        // Fallback to MS Shell Dlg
        m_hFont = ::CreateFont(lfHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                               CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, _T("MS Shell Dlg"));
    }
...
void CMainDlg::OnSelectAll()
{
    ...
    int formatCheckboxes[] = {IDC_CB_CBZ,       IDC_CB_CBR,     IDC_CB_CB7,         IDC_CB_CBT,         IDC_CB_EPUB,
                              IDC_CB_MOBI,      IDC_CB_AZW,     IDC_CB_AZW3,        IDC_CB_ZIP,         IDC_CB_RAR,
                              IDC_CB_7Z,        IDC_CB_TAR,     IDC_CB_PHZ,         IDC_CB_FB2,         IDC_CB_WEBP,
                              IDC_CB_HEIF,      IDC_CB_AVIF,    IDC_CB_JXL,         IDC_CB_VIDEO,       IDC_CB_PDF,
                              IDC_CB_TIFF,      IDC_CB_SVG,     IDC_CB_RAW,         IDC_CB_PSD,         IDC_CB_DDS,
...
bool CMainDlg::LoadConfigFromFile(LPCTSTR filename, ConfigSnapshot& outConfig)
{
    ...
    auto getInt = [&jsonData](const char* key) -> int {
        ...
        while (valuePos < jsonData.GetLength()
               && (jsonData[valuePos] == ':' || jsonData[valuePos] == ' ' || jsonData[valuePos] == '\t'))
            valuePos++;
    ...
    if (outConfig.collageMode != 1 && outConfig.collageMode != 4 && outConfig.collageMode != 9
        && outConfig.collageMode != 16) {
    ...
    if (dlg.DoModal() == IDOK) {
        ...
        if (isRegFile) {
            ...
            if (result == IDYES) {
                ...
                if (CreateProcess(NULL, cmdLine.GetBuffer(), NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si,
                                  &pi)) {
                    ...
                    // Refresh Explorer
                    SHChangeNotify(SHCNE_ASSOCCHANGED, SHCNF_IDLIST | SHCNF_FLUSHNOWAIT | SHCNF_NOTIFYRECURSIVE, NULL,
                                   NULL);
                ...
                if (LoadConfigFromRegFile(dlg.m_szFileName, loadedConfig)) {
                    ...
                    MessageBox(_T("Failed to parse registry file."), _T("Load Error"), MB_OK | MB_ICONERROR);
                }
            }
        } else {
 ...
 */
LRESULT CMainDlg::OnResetDefaults(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    int result = MessageBox(_T("This will reset all thumbnail handler settings to their default state ")
    ...
    const int formatCheckboxes[] = {
        IDC_CB_CBZ,     IDC_CB_CBR,         IDC_CB_CB7,         IDC_CB_CBT,  IDC_CB_ZIP,   IDC_CB_RAR,
        IDC_CB_7Z,      IDC_CB_TAR,         IDC_CB_EPUB,        IDC_CB_MOBI, IDC_CB_AZW,   IDC_CB_AZW3,
        IDC_CB_PHZ,     IDC_CB_FB2,         IDC_CB_WEBP,        IDC_CB_HEIF, IDC_CB_AVIF,  IDC_CB_JXL,
        IDC_CB_PSD,     IDC_CB_DDS,         IDC_CB_HDR,         IDC_CB_EXR,  IDC_CB_VIDEO, IDC_CB_PDF,
        IDC_CB_TIFF,    IDC_CB_SVG,         IDC_CB_RAW,         IDC_CB_PPM,  IDC_CB_ICO,   IDC_CB_QOI,
        IDC_CB_TGA,     IDC_CB_AUDIO,       IDC_CB_DOCUMENT,    IDC_CB_FONT, IDC_CB_MODEL, IDC_CB_EXT_IMAGE,
        IDC_CB_TEXTURE, IDC_CB_EXT_ARCHIVE, IDC_CB_EXT_DOCUMENT};
    ...
    MessageBox(_T("All settings have been reset to defaults.\r\n\r\n")
 ...
 */
LRESULT CMainDlg::OnExportConfig(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    CFileDialog dlg(FALSE, _T("json"), _T("ExplorerLens-Config"), OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY,
    ...
    if (_wfopen_s(&fp, dlg.m_szFileName, L"w") != 0 || !fp) {
        MessageBox(_T("Failed to create export file."), _T("Export Error"), MB_OK | MB_ICONERROR);
        return 0;
    }
    ...
    FmtEntry fmts[] = {{"cbz", config.cbz},
                       ...
                       {"font", config.font},
                       {"model", config.model},
                       {"extImage", config.extImage},
                       {"texture", config.texture},
                       {"extArchive", config.extArchive},
                       {"extDocument", config.extDocument}};
...
LRESULT CMainDlg::OnToggleTheme(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    auto& darkCtrl = ExplorerLens::DarkModeController::Instance();
...
LRESULT CMainDlg::OnExportDiagnostics(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    CFileDialog dlg(FALSE, _T("txt"), _T("ExplorerLens-Diagnostics"), OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY,
    ...
    FILE* fp = nullptr;
    if (_wfopen_s(&fp, dlg.m_szFileName, L"w") != 0 || !fp) {
        MessageBox(_T("Failed to create diagnostics file."), _T("Export Error"), MB_OK | MB_ICONERROR);
        return 0;
    }
    ...
    if (hNtdll) {
        auto fnRtlGetVersion = (RtlGetVersionPtr)GetProcAddress(hNtdll, "RtlGetVersion");
        if (fnRtlGetVersion) {
    ...
    for (const auto& info : healthResults) {
        fprintf(fp, "%s %S (%u ext)\n", info.isAvailable ? "[OK]" : "[--]", info.name.c_str(), info.extensionCount);
...
LRESULT CMainDlg::OnTrayMessage(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CMainDlg::OnTrayOpen(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
...
LRESULT CMainDlg::OnTrayAbout(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
...
LRESULT CMainDlg::OnAboutBtn(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    CAboutDlg dlg;
    dlg.DoModal(m_hWnd);
    return 0;
...
LRESULT CMainDlg::OnBenchmark(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    ...
    for (int i = 0; i < ITERATIONS; ++i) {
        for (int dy = 0; dy < 256; ++dy) {
            for (int dx = 0; dx < 256; ++dx) {
                ...
                for (int c = 0; c < 4; ++c) {
                    int v = (int)buffer[(sy * WIDTH + sx) * 4 + c] + (int)buffer[(sy * WIDTH + sx + 1) * 4 + c]
    ...
    swprintf_s(msg, 1024,
               ...
               ITERATIONS, fillPerImage, grayPerImage, scalePerImage, totalPerImage, imagesPerSec);
