...
void CMainDlg::OnApplyImpl()
{
	...
	if (bRet!=m_reg.HasTH(LENS_CBZ))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_CBZ, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_CBR)));
	if (bRet!=m_reg.HasTH(LENS_CBR))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_CBR, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_CB7)));
	if (bRet!=m_reg.HasTH(LENS_CB7))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_CB7, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_CBT)));
	if (bRet!=m_reg.HasTH(LENS_CBT))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_CBT, bRet);
	}
	
	// E-Book Formats
	bRet = (BST_CHECKED == Button_GetCheck(GetDlgItem(IDC_CB_EPUB)));
	if (bRet != m_reg.HasTH(LENS_EPUB))
	{
		bRefresh = TRUE;
		m_reg.SetHandlers(LENS_EPUB, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_MOBI)));
	if (bRet!=m_reg.HasTH(LENS_MOBI))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_MOBI, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_AZW)));
	if (bRet!=m_reg.HasTH(LENS_AZW))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_AZW, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_AZW3)));
	if (bRet!=m_reg.HasTH(LENS_AZW3))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_AZW3, bRet);
	}
	
	// Archive Formats
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_ZIP)));
	if (bRet!=m_reg.HasTH(LENS_ZIP))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_ZIP, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_RAR)));
	if (bRet!=m_reg.HasTH(LENS_RAR))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_RAR, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_7Z)));
	if (bRet!=m_reg.HasTH(LENS_7Z))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_7Z, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_TAR)));
	if (bRet!=m_reg.HasTH(LENS_TAR))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_TAR, bRet);
	}
	
	// Photo & Other Formats
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_PHZ)));
	if (bRet!=m_reg.HasTH(LENS_PHZ))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_PHZ, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_FB2)));
	if (bRet!=m_reg.HasTH(LENS_FB2))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_FB2, bRet);
	}
	
	// Modern Image Formats
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_WEBP)));
	if (bRet!=m_reg.HasTH(LENS_WEBP))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_WEBP, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_HEIF)));
	if (bRet!=m_reg.HasTH(LENS_HEIF))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_HEIF, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_AVIF)));
	if (bRet!=m_reg.HasTH(LENS_AVIF))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_AVIF, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_JXL)));
	if (bRet!=m_reg.HasTH(LENS_JXL))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_JXL, bRet);
	}
	
	// Media & Documents
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_VIDEO)));
	if (bRet!=m_reg.HasTH(LENS_VIDEO))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_VIDEO, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_PDF)));
	if (bRet!=m_reg.HasTH(LENS_PDF))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_PDF, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_TIFF)));
	if (bRet!=m_reg.HasTH(LENS_TIFF))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_TIFF, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_SVG)));
	if (bRet!=m_reg.HasTH(LENS_SVG))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_SVG, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_RAW)));
	if (bRet!=m_reg.HasTH(LENS_RAW))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_RAW, bRet);
	}
	
	// Sprint 8: Professional & Specialized Formats
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_PSD)));
	if (bRet!=m_reg.HasTH(LENS_PSD))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_PSD, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_DDS)));
	if (bRet!=m_reg.HasTH(LENS_DDS))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_DDS, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_HDR)));
	if (bRet!=m_reg.HasTH(LENS_HDR))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_HDR, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_EXR)));
	if (bRet!=m_reg.HasTH(LENS_EXR))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_EXR, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_PPM)));
	if (bRet!=m_reg.HasTH(LENS_PPM))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_PPM, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_ICO)));
	if (bRet!=m_reg.HasTH(LENS_ICO))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_ICO, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_QOI)));
	if (bRet!=m_reg.HasTH(LENS_QOI))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_QOI, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_TGA)));
	if (bRet!=m_reg.HasTH(LENS_TGA))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_TGA, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_AUDIO)));
	if (bRet!=m_reg.HasTH(LENS_AUDIO))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_AUDIO, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_DOCUMENT)));
	if (bRet!=m_reg.HasTH(LENS_DOCUMENT))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_DOCUMENT, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_FONT)));
	if (bRet!=m_reg.HasTH(LENS_FONT))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_FONT, bRet);
	}
	bRet=(BST_CHECKED==Button_GetCheck(GetDlgItem(IDC_CB_MODEL)));
	if (bRet!=m_reg.HasTH(LENS_MODEL))
	{
		bRefresh=TRUE;
		m_reg.SetHandlers(LENS_MODEL, bRet);
	}
...
LRESULT CMainDlg::OnAppHelp(LPHELPINFO lphi)
{
	switch (lphi->iCtrlId)
	{
		...
		case IDC_CB_RAR:
			...
			HtmlHelp(m_hWnd, _T("LENSShellHelp.chm::manager.html#optth"), HH_DISPLAY_TOPIC, NULL);
		break;

		case IDC_SORT_ADVOPTGROUP:
			HtmlHelp(m_hWnd, _T("LENSShellHelp.chm::manager.html#advopt"), HH_DISPLAY_TOPIC, NULL);
		break;

		case IDC_CB_SORT:
		case IDC_SORT_DESC:
			ATLTRACE("HH sort opt\n");
			HtmlHelp(m_hWnd, _T("LENSShellHelp.chm::FAQ.html#custth"), HH_DISPLAY_TOPIC, NULL);
		break;

	default:
			ATLTRACE("HH default\n");
			HtmlHelp(m_hWnd, _T("LENSShellHelp.chm::manager.html"), HH_DISPLAY_TOPIC, NULL);//about?
		break;
	}
return 0;
}
...
void CMainDlg::InitTooltips()
{
	...
	AddTooltipWithStatus(IDC_CB_CBZ, LENS_CBZ, _T("CBZ - Comic Book ZIP Archive\nExtensions: .cbz"));
	AddTooltipWithStatus(IDC_CB_CBR, LENS_CBR, _T("CBR - Comic Book RAR Archive\nExtensions: .cbr\nRequires: UnRAR library"));
	AddTooltipWithStatus(IDC_CB_CB7, LENS_CB7, _T("CB7 - Comic Book 7-Zip Archive\nExtensions: .cb7"));
	AddTooltipWithStatus(IDC_CB_CBT, LENS_CBT, _T("CBT - Comic Book TAR Archive\nExtensions: .cbt"));
	
	// E-Book Formats
	AddTooltipWithStatus(IDC_CB_EPUB, LENS_EPUB, _T("EPUB - Electronic Publication\nExtensions: .epub\nSupports: Cover extraction from metadata"));
	AddTooltipWithStatus(IDC_CB_MOBI, LENS_MOBI, _T("MOBI - Mobipocket E-Book\nExtensions: .mobi"));
	AddTooltipWithStatus(IDC_CB_AZW, LENS_AZW, _T("AZW - Amazon Kindle Format\nExtensions: .azw"));
	AddTooltipWithStatus(IDC_CB_AZW3, LENS_AZW3, _T("AZW3 - Kindle Format 8 (KF8)\nExtensions: .azw3"));
	
	// Archive Formats
	AddTooltipWithStatus(IDC_CB_ZIP, LENS_ZIP, _T("ZIP - Standard ZIP Archive\nExtensions: .zip\nThumbnails from first image inside"));
	AddTooltipWithStatus(IDC_CB_RAR, LENS_RAR, _T("RAR - WinRAR Archive\nExtensions: .rar\nRequires: UnRAR library"));
	AddTooltipWithStatus(IDC_CB_7Z, LENS_7Z, _T("7Z - 7-Zip Archive\nExtensions: .7z\nHigh compression ratio"));
	AddTooltipWithStatus(IDC_CB_TAR, LENS_TAR, _T("TAR - Tape Archive\nExtensions: .tar, .tar.gz, .tar.bz2"));
	
	// Photo Albums & Other
	AddTooltipWithStatus(IDC_CB_PHZ, LENS_PHZ, _T("PHZ - Photo ZIP Album\nExtensions: .phz\nOptimized for photo collections"));
	AddTooltipWithStatus(IDC_CB_FB2, LENS_FB2, _T("FB2 - FictionBook E-Book\nExtensions: .fb2\nXML-based format"));
	
	// Modern Image Formats
	AddTooltipWithStatus(IDC_CB_WEBP, LENS_WEBP, _T("WebP - Modern Image Format\nExtensions: .webp\nPerformance: Fast, built-in decoder"));
	AddTooltipWithStatus(IDC_CB_HEIF, LENS_HEIF, _T("HEIF/HEIC - High Efficiency Image\nExtensions: .heif, .heic\nUsed by: iPhone photos\nRequires: HEIF Image Extensions (Microsoft Store)"));
	AddTooltipWithStatus(IDC_CB_AVIF, LENS_AVIF, _T("AVIF - AV1 Image Format\nExtensions: .avif\nRequires: AV1 Video Extension (Microsoft Store)\nPerformance: Fast via WIC"));
	AddTooltipWithStatus(IDC_CB_JXL, LENS_JXL, _T("JPEG XL - Next-Gen Image Format\nExtensions: .jxl\nRequires: Windows Imaging Component support"));
	
	// Media & Documents
	AddTooltipWithStatus(IDC_CB_VIDEO, LENS_VIDEO, _T("Video Files - Extract Frame Thumbnails\nExtensions: .mp4, .avi, .mkv, .mov, .wmv\nRequires: DirectShow codecs\nPerformance: Cached for speed"));
	AddTooltipWithStatus(IDC_CB_PDF, LENS_PDF, _T("PDF Documents\nExtensions: .pdf\nPerformance: Fast, uses embedded thumbnails when available"));
	AddTooltipWithStatus(IDC_CB_TIFF, LENS_TIFF, _T("TIFF Images\nExtensions: .tif, .tiff\nSupports: Multi-page TIFF files"));
	AddTooltipWithStatus(IDC_CB_SVG, LENS_SVG, _T("SVG Vector Graphics\nExtensions: .svg, .svgz\nScalable vector format"));
	AddTooltipWithStatus(IDC_CB_RAW, LENS_RAW, _T("RAW Camera Photos\nExtensions: .dng, .cr2, .cr3, .nef, .arw, .orf\nRequires: Microsoft Camera Codec Pack (free)\nPerformance: Fast (uses embedded preview)"));
	
	// Sprint 8: Professional & Specialized Formats
	AddTooltipWithStatus(IDC_CB_PSD, LENS_PSD, _T("PSD - Adobe Photoshop Document\nExtensions: .psd, .psb\nShows: Composite preview layer"));
	AddTooltipWithStatus(IDC_CB_DDS, LENS_DDS, _T("DDS - DirectDraw Surface\nExtensions: .dds\nUsed by: Games, 3D applications"));
	AddTooltipWithStatus(IDC_CB_HDR, LENS_HDR, _T("HDR - Radiance High Dynamic Range\nExtensions: .hdr\nUsed by: Professional photography, 3D rendering"));
	AddTooltipWithStatus(IDC_CB_EXR, LENS_EXR, _T("EXR - OpenEXR High Dynamic Range\nExtensions: .exr\nUsed by: VFX, film production"));
	AddTooltipWithStatus(IDC_CB_PPM, LENS_PPM, _T("PPM - Portable Pixmap\nExtensions: .ppm, .pgm, .pbm\nUsed by: Scientific imaging, CLI tools"));
	AddTooltipWithStatus(IDC_CB_ICO, LENS_ICO, _T("ICO - Windows Icon\nExtensions: .ico, .cur\nShows: Largest available icon size"));
	AddTooltipWithStatus(IDC_CB_QOI, LENS_QOI, _T("QOI - Quite OK Image Format\nExtensions: .qoi\nFast lossless compression"));
	AddTooltipWithStatus(IDC_CB_TGA, LENS_TGA, _T("TGA - Targa Image\nExtensions: .tga\nUsed by: Games, 3D applications"));
	AddTooltipWithStatus(IDC_CB_AUDIO, LENS_AUDIO, _T("Audio Cover Art\nExtensions: .mp3, .flac, .ogg, .wma, .aac\nShows: Embedded album art thumbnail"));
	AddTooltipWithStatus(IDC_CB_DOCUMENT, LENS_DOCUMENT, _T("Document Thumbnails\nExtensions: .docx, .xlsx, .pptx\nShows: First page preview via OLE/COM"));
	AddTooltipWithStatus(IDC_CB_FONT, LENS_FONT, _T("Font Preview Thumbnails\nExtensions: .ttf, .otf, .woff\nShows: Sample text rendering"));
	AddTooltipWithStatus(IDC_CB_MODEL, LENS_MODEL, _T("3D Model Thumbnails\nExtensions: .stl, .obj, .ply\nShows: Wireframe/solid preview"));
...
void CMainDlg::AddTooltipWithStatus(int ctrlID, int LENSTYPE, LPCTSTR formatName)
{
	// Sprint 18A: Enhanced with program name detection
	CString programName;
	HandlerStatus status = m_reg.GetHandlerStatusEx(LENSTYPE, m_reg.GetExtension(LENSTYPE), programName);
	CString tooltip;
	
	switch (status)
	{
	case HANDLER_ExplorerLens:
		tooltip.Format(_T("%s\n\n\xE2\x9C\x85 Active Handler: ExplorerLens\nStatus: Enabled and working"), formatName);
		break;
		
	case HANDLER_NATIVE:
		tooltip.Format(_T("%s\n\n\xF0\x9F\x94\xB7 Current Handler: %s (Windows built-in)\nNote: Enable ExplorerLens to use enhanced features"), formatName, (LPCTSTR)programName);
		break;
		
	case HANDLER_NONE:
		tooltip.Format(_T("%s\n\n\xE2\xAD\x95 No Handler: No thumbnail provider installed\nAction: Enable ExplorerLens to generate thumbnails"), formatName);
		break;
		
	case HANDLER_THIRD_PARTY:
		tooltip.Format(_T("%s\n\n\xE2\x9A\xA0 Current Handler: %s (Third-party)\nAction: Click Apply to use ExplorerLens instead"), formatName, (LPCTSTR)programName);
		break;
...
void CMainDlg::UpdateStatusBar()
{
	...
	const int allFormats[] = {
		LENS_CBZ, LENS_CBR, LENS_CB7, LENS_CBT,
		LENS_EPUB, LENS_MOBI, LENS_AZW, LENS_AZW3,
		LENS_ZIP, LENS_RAR, LENS_7Z, LENS_TAR,
		LENS_PHZ, LENS_FB2,
		LENS_WEBP, LENS_HEIF, LENS_AVIF, LENS_JXL,
...
int CMainDlg::GetEnabledFormatCount()
{
	...
	if (m_reg.HasTH(LENS_CBZ)) count++;
	if (m_reg.HasTH(LENS_CBR)) count++;
	if (m_reg.HasTH(LENS_CB7)) count++;
	if (m_reg.HasTH(LENS_CBT)) count++;
	
	// E-Book Formats (4)
	if (m_reg.HasTH(LENS_EPUB)) count++;
	if (m_reg.HasTH(LENS_MOBI)) count++;
	if (m_reg.HasTH(LENS_AZW)) count++;
	if (m_reg.HasTH(LENS_AZW3)) count++;
	
	// Archive Formats (4)
	if (m_reg.HasTH(LENS_ZIP)) count++;
	if (m_reg.HasTH(LENS_RAR)) count++;
	if (m_reg.HasTH(LENS_7Z)) count++;
	if (m_reg.HasTH(LENS_TAR)) count++;
	
	// Photo & Other Formats (2)
	if (m_reg.HasTH(LENS_PHZ)) count++;
	if (m_reg.HasTH(LENS_FB2)) count++;
	
	// Modern Image Formats (4)
	if (m_reg.HasTH(LENS_WEBP)) count++;
	if (m_reg.HasTH(LENS_HEIF)) count++;
	if (m_reg.HasTH(LENS_AVIF)) count++;
	if (m_reg.HasTH(LENS_JXL)) count++;
	
	// Media & Documents (5)
	if (m_reg.HasTH(LENS_VIDEO)) count++;
	if (m_reg.HasTH(LENS_PDF)) count++;
	if (m_reg.HasTH(LENS_TIFF)) count++;
	if (m_reg.HasTH(LENS_SVG)) count++;  
	if (m_reg.HasTH(LENS_RAW)) count++;
	
	// Sprint 8: Professional & Specialized Formats (12)
	if (m_reg.HasTH(LENS_PSD)) count++;
	if (m_reg.HasTH(LENS_DDS)) count++;
	if (m_reg.HasTH(LENS_HDR)) count++;
	if (m_reg.HasTH(LENS_EXR)) count++;
	if (m_reg.HasTH(LENS_PPM)) count++;
	if (m_reg.HasTH(LENS_ICO)) count++;
	if (m_reg.HasTH(LENS_QOI)) count++;
	if (m_reg.HasTH(LENS_TGA)) count++;
	if (m_reg.HasTH(LENS_AUDIO)) count++;
	if (m_reg.HasTH(LENS_DOCUMENT)) count++;
	if (m_reg.HasTH(LENS_FONT)) count++;
	if (m_reg.HasTH(LENS_MODEL)) count++;
...
void CMainDlg::InitStatusIcons()
{
	...
	// Comic Book Formats
	m_checkboxStatus[IDC_CB_CBZ] = m_reg.GetHandlerStatus(LENS_CBZ, m_reg.GetExtension(LENS_CBZ));
	m_checkboxStatus[IDC_CB_CBR] = m_reg.GetHandlerStatus(LENS_CBR, m_reg.GetExtension(LENS_CBR));
	m_checkboxStatus[IDC_CB_CB7] = m_reg.GetHandlerStatus(LENS_CB7, m_reg.GetExtension(LENS_CB7));
	m_checkboxStatus[IDC_CB_CBT] = m_reg.GetHandlerStatus(LENS_CBT, m_reg.GetExtension(LENS_CBT));
	
	// E-Book Formats
	m_checkboxStatus[IDC_CB_EPUB] = m_reg.GetHandlerStatus(LENS_EPUB, m_reg.GetExtension(LENS_EPUB));
	m_checkboxStatus[IDC_CB_MOBI] = m_reg.GetHandlerStatus(LENS_MOBI, m_reg.GetExtension(LENS_MOBI));
	m_checkboxStatus[IDC_CB_AZW] = m_reg.GetHandlerStatus(LENS_AZW, m_reg.GetExtension(LENS_AZW));
	m_checkboxStatus[IDC_CB_AZW3] = m_reg.GetHandlerStatus(LENS_AZW3, m_reg.GetExtension(LENS_AZW3));
	
	// Archive Formats
	m_checkboxStatus[IDC_CB_ZIP] = m_reg.GetHandlerStatus(LENS_ZIP, m_reg.GetExtension(LENS_ZIP));
	m_checkboxStatus[IDC_CB_RAR] = m_reg.GetHandlerStatus(LENS_RAR, m_reg.GetExtension(LENS_RAR));
	m_checkboxStatus[IDC_CB_7Z] = m_reg.GetHandlerStatus(LENS_7Z, m_reg.GetExtension(LENS_7Z));
	m_checkboxStatus[IDC_CB_TAR] = m_reg.GetHandlerStatus(LENS_TAR, m_reg.GetExtension(LENS_TAR));
	
	// Photo & Other Formats
	m_checkboxStatus[IDC_CB_PHZ] = m_reg.GetHandlerStatus(LENS_PHZ, m_reg.GetExtension(LENS_PHZ));
	m_checkboxStatus[IDC_CB_FB2] = m_reg.GetHandlerStatus(LENS_FB2, m_reg.GetExtension(LENS_FB2));
	
	// Modern Image Formats
	m_checkboxStatus[IDC_CB_WEBP] = m_reg.GetHandlerStatus(LENS_WEBP, m_reg.GetExtension(LENS_WEBP));
	m_checkboxStatus[IDC_CB_HEIF] = m_reg.GetHandlerStatus(LENS_HEIF, m_reg.GetExtension(LENS_HEIF));
	m_checkboxStatus[IDC_CB_AVIF] = m_reg.GetHandlerStatus(LENS_AVIF, m_reg.GetExtension(LENS_AVIF));
	m_checkboxStatus[IDC_CB_JXL] = m_reg.GetHandlerStatus(LENS_JXL, m_reg.GetExtension(LENS_JXL));
	
	// Media & Documents
	m_checkboxStatus[IDC_CB_VIDEO] = m_reg.GetHandlerStatus(LENS_VIDEO, m_reg.GetExtension(LENS_VIDEO));
	m_checkboxStatus[IDC_CB_PDF] = m_reg.GetHandlerStatus(LENS_PDF, m_reg.GetExtension(LENS_PDF));
	m_checkboxStatus[IDC_CB_TIFF] = m_reg.GetHandlerStatus(LENS_TIFF, m_reg.GetExtension(LENS_TIFF));
	m_checkboxStatus[IDC_CB_SVG] = m_reg.GetHandlerStatus(LENS_SVG, m_reg.GetExtension(LENS_SVG));
	m_checkboxStatus[IDC_CB_RAW] = m_reg.GetHandlerStatus(LENS_RAW, m_reg.GetExtension(LENS_RAW));
	
	// Sprint 8: Professional & Specialized Formats
	m_checkboxStatus[IDC_CB_PSD] = m_reg.GetHandlerStatus(LENS_PSD, m_reg.GetExtension(LENS_PSD));
	m_checkboxStatus[IDC_CB_DDS] = m_reg.GetHandlerStatus(LENS_DDS, m_reg.GetExtension(LENS_DDS));
	m_checkboxStatus[IDC_CB_HDR] = m_reg.GetHandlerStatus(LENS_HDR, m_reg.GetExtension(LENS_HDR));
	m_checkboxStatus[IDC_CB_EXR] = m_reg.GetHandlerStatus(LENS_EXR, m_reg.GetExtension(LENS_EXR));
	m_checkboxStatus[IDC_CB_PPM] = m_reg.GetHandlerStatus(LENS_PPM, m_reg.GetExtension(LENS_PPM));
	m_checkboxStatus[IDC_CB_ICO] = m_reg.GetHandlerStatus(LENS_ICO, m_reg.GetExtension(LENS_ICO));
	m_checkboxStatus[IDC_CB_QOI] = m_reg.GetHandlerStatus(LENS_QOI, m_reg.GetExtension(LENS_QOI));
	m_checkboxStatus[IDC_CB_TGA] = m_reg.GetHandlerStatus(LENS_TGA, m_reg.GetExtension(LENS_TGA));
	m_checkboxStatus[IDC_CB_AUDIO] = m_reg.GetHandlerStatus(LENS_AUDIO, m_reg.GetExtension(LENS_AUDIO));
	m_checkboxStatus[IDC_CB_DOCUMENT] = m_reg.GetHandlerStatus(LENS_DOCUMENT, m_reg.GetExtension(LENS_DOCUMENT));
	m_checkboxStatus[IDC_CB_FONT] = m_reg.GetHandlerStatus(LENS_FONT, m_reg.GetExtension(LENS_FONT));
	m_checkboxStatus[IDC_CB_MODEL] = m_reg.GetHandlerStatus(LENS_MODEL, m_reg.GetExtension(LENS_MODEL));
}
	...
	if (ctrlPressed) {
		...
		switch (wParam) {
			case VK_F1:  // F1: Show help
				...
				MessageBox(
					_T("ExplorerLens Shell Manager - Quick Help\n\n")
