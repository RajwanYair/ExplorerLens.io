...
class PerformanceDashboard
{
  public:
    ...
    void RefreshMetrics()
    {
        ...
        // GPU metrics
        AddMetric(L"GPU Decode Active", m_gpuDecodeActive ? 100.0 : 0.0, 100.0, L"bool");
    ...
    // ====================================================================
    void RecordDecode(const std::wstring& format, double decodeMs, bool fromCache, bool gpuAccel)
    {
        m_totalDecodes++;
        if (fromCache)
            m_cacheHits++;
        ...
        m_cacheHitRate = (m_totalDecodes > 0) ? (100.0 * m_cacheHits / m_totalDecodes) : 0.0;
        ...
        for (auto& fs : m_formatStats) {
            if (fs.formatName == format) {
                fs.decodeCount++;
                fs.avgDecodeMs = ((fs.avgDecodeMs * (fs.decodeCount - 1)) + decodeMs) / fs.decodeCount;
    ...
    {
        wchar_t buf[256];
        swprintf_s(buf, L"Avg: %.1fms | Cache: %.0f%% | Decodes: %llu", m_avgThumbnailMs, m_cacheHitRate,
                   m_totalDecodes);
        return buf;
    }
  ...
  private:
    ...
    void AddMetric(const std::wstring& name, double value, double target, const std::wstring& unit)
    {
        PerformanceMetric m;
        m.name = name;
        m.value = value;
        m.target = target;
        m.unit = unit;
        m.meetsTarget = (unit == L"img/s" || unit == L"%") ? (value >= target) : (value <= target);
