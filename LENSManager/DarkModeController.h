...
class DarkModeController
{
  public:
    ...
    void Initialize()
    {
        ...
        m_colors = m_isDarkMode ? DarkMode::GetDarkTheme() : DarkMode::GetLightTheme();
    ...
    void ToggleTheme(HWND hWnd)
    {
        ...
        if (std::find(m_managedWindows.begin(), m_managedWindows.end(), hWnd) == m_managedWindows.end()) {
            ApplyToWindow(hWnd);
        }
    }
    ...
    void DrawCheckBox(DRAWITEMSTRUCT* dis, bool checked, const wchar_t* text)
    {
        ...
        // Checkbox square
        RECT boxRc = {dis->rcItem.left, dis->rcItem.top + 2, dis->rcItem.left + 13, dis->rcItem.top + 15};
        HPEN pen = CreatePen(PS_SOLID, 1, m_colors.border);
        HPEN oldPen = (HPEN)SelectObject(dis->hDC, pen);
        HBRUSH boxBrush = CreateSolidBrush(checked ? m_accentColor : m_colors.buttonFace);
        ...
        if (checked) {
            ...
            RECT chkRc = boxRc;
            DrawTextW(dis->hDC, L"\x2713", -1, &chkRc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        }

        // Label text
        RECT textRc = {boxRc.right + 5, dis->rcItem.top, dis->rcItem.right, dis->rcItem.bottom};
        SetTextColor(dis->hDC, (dis->itemState & ODS_DISABLED) ? m_colors.disabledText : m_colors.text);
        SetBkMode(dis->hDC, TRANSPARENT);
        DrawTextW(dis->hDC, text, -1, &textRc, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
    }
    ...
    const DarkMode::ThemeColors& Colors() const
    {
        return m_colors;
    }
