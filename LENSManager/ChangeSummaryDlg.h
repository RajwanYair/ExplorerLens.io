...
struct ConfigSnapshot
{
    ...
    CString GetCollageModeString() const
    {
        switch (collageMode) {
            case 1:
                return _T("Single Page (1x1)");
            case 4:
                return _T("2x2 Grid (4 pages)");
            case 9:
                return _T("3x3 Grid (9 pages)");
            case 16:
                return _T("4x4 Grid (16 pages)");
            default:
                return _T("Unknown");
        }
    }
    ...
    CString GetTimestamp() const
    {
        ...
        GetLocalTime(&st);
        CString timestamp;
        timestamp.Format(_T("%04d-%02d-%02d %02d:%02d:%02d"), st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute,
                         st.wSecond);
        return timestamp;
    }
};
...
class CChangeSummaryDlg : public CDialogImpl<CChangeSummaryDlg>
{
  public:
    ...
    LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
    {
        ...
        if (m_oldConfig.collageMode != m_newConfig.collageMode) {
            hasOptionsChanges = true;
            optionsInfo.AppendFormat(_T("• Collage Mode: %s -> %s\r\n"), m_oldConfig.GetCollageModeString(),
                                     m_newConfig.GetCollageModeString());
        }
    ...
    LRESULT OnSaveConfig(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
    {
        ...
        CFileDialog dlg(
            FALSE, _T("reg"), _T("ExplorerLens_Config"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
    ...
    {
        if (MessageBox(
                _T("This will restore the previous configuration and undo all changes made in this session.\r\n\r\n")
                _T("Do you want to continue?"),
                _T("Confirm Restore"), MB_YESNO | MB_ICONQUESTION)
            == IDYES) {
    ...
    const ConfigSnapshot& GetOldConfig() const
    {
        return m_oldConfig;
    }
  ...
  private:
    ...
    static bool SaveConfigToRegFile(LPCTSTR filename, const ConfigSnapshot& config)
    {
        ...
        auto addHandler = [&regData](const char* ext, bool enabled,
                                     const char* guid = "{9E6ECB90-5A61-42BD-B851-D3297D9C7F39}") {
            // Thumbnail handler
            regData.AppendFormat(
                "[HKEY_CURRENT_USER\\SOFTWARE\\Classes\\.%s\\shellex\\{BB2E617C-0920-11d1-9A0B-00C04FC2D6C1}]\r\n", ext);
            if (enabled) {
            ...
            // InfoTip handler
            regData.AppendFormat(
                "[HKEY_CURRENT_USER\\SOFTWARE\\Classes\\.%s\\shellex\\{00021500-0000-0000-C000-000000000046}]\r\n", ext);
            if (enabled) {
                regData.AppendFormat("@=\"%s\"\r\n\r\n", guid);
            } else {
                regData += "@=-\r\n\r\n";
