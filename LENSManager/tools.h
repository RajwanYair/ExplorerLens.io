...
template <class T>
class CDropFileTarget  // implements dropfile target window
{
  public:
  ...
  protected:
    virtual BOOL StartDropFile(UINT uNumFiles)
    {
        ATLTRACE("CDropFileTarget::StartDropFile NumFiles: %d\n", uNumFiles);
        return TRUE;
    }
    virtual BOOL ProcessDropFile(LPCTSTR szDropFile, UINT nFile)
    {
        ATLTRACE(_T("CDropFileTarget::ProcessDropFile: %s\n"), szDropFile);
        return TRUE;
    }
    virtual void FinishDropFile()
    {
  ...
  private:
    virtual LRESULT OnDropFiles(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        for (i = 0; i < uNumFiles; i++) {
            ...
            if (!ProcessDropFile(szFilename, i))
                break;
        }
    ...
    #endif
{
  public:
    ...
    #endif
    {
        ...
        GetCursorPos(&snap_cur_pos);
        OffsetRect(snap_prc, snap_cur_pos.x - (snap_prc->left + snap_x), snap_cur_pos.y - (snap_prc->top + snap_y));
        ...
        else if (isSnapClose(snap_wa.right, snap_prc->right))
        ...
        else if (isSnapClose(snap_wa.bottom, snap_prc->bottom))
...
class CTimer
{
  public:
    ...
    BOOL resetTimer()
    {
        return ::QueryPerformanceCounter(&m_startTick);
    }
