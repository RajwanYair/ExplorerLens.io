...
!define MUI_FINISHPAGE_RUN_TEXT "Launch DarkThumbs Configuration Manager"
...
OutFile "DarkThumbs-v${PRODUCT_VERSION}-x64-Setup.exe"
...
Section "MainSection" SEC01
    ...
    IfFileExists "$SYSDIR\CBXShell.dll" 0 +3
        DetailPrint "Unregistering previous version..."
        nsExec::ExecToLog 'regsvr32 /s /u "$SYSDIR\CBXShell.dll"'
    ...
    CreateShortcut "$SMPROGRAMS\${PRODUCT_NAME}\Configuration Manager.lnk" "$INSTDIR\CBXManager.exe"
    CreateShortcut "$SMPROGRAMS\${PRODUCT_NAME}\GPU Validator.lnk" "$INSTDIR\GPUValidator.exe"
    CreateShortcut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe"
    
    ; Create Desktop shortcut (optional)
    CreateShortcut "$DESKTOP\DarkThumbs Config.lnk" "$INSTDIR\CBXManager.exe"
...
Section Uninstall
    ...
    Delete "$DESKTOP\DarkThumbs Config.lnk"
