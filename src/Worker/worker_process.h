...
struct WorkerConfig {
  ...
  std::wstring workerExePath = L"ExplorerLensWorker.exe";
...
public:
  ...
  bool SubmitRequest(const IPC::ThumbnailRequest &request,
                     IPC::ThumbnailResponse &response, uint32_t timeoutMs = 0);
...
public:
  explicit ExponentialBackoffPolicy(uint32_t maxRestarts = 5,
