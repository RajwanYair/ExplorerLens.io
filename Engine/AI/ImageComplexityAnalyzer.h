...
enum class ComplexityLevel : uint8_t { Trivial, Low, Medium, High, Extreme };
...
public:
    ...
    ComplexityEstimate Estimate(uint32_t width, uint32_t height,
        uint32_t bitsPerPixel, const std::string& format) const {
        ...
        if (format == "JPEG-XL" || format == "JXL") formatMultiplier = 3.0;
        else if (format == "HEIF" || format == "HEIC") formatMultiplier = 2.5;
        else if (format == "AVIF") formatMultiplier = 2.5;
        else if (format == "RAW" || format == "CR2" || format == "NEF") formatMultiplier = 4.0;
        else if (format == "TIFF") formatMultiplier = 1.5;
        else if (format == "BMP" || format == "PNG") formatMultiplier = 1.0;
        else if (format == "JPEG" || format == "JPG") formatMultiplier = 0.8;
        else if (format == "GIF" || format == "ICO") formatMultiplier = 0.5;
        else if (format == "PDF") formatMultiplier = 5.0;
        else if (format == "glTF" || format == "FBX" || format == "OBJ") formatMultiplier = 8.0;
        ...
        if (pixelCount > 4000000 || est.score > 6.0) {
            ...
            est.recommendedThreads = std::min(4u, std::max(2u,
                static_cast<uint32_t>(pixelCount / 4000000) + 1));
        }
        ...
        // Classify
        if (est.score < 1.0) est.level = ComplexityLevel::Trivial;
        else if (est.score < 3.0) est.level = ComplexityLevel::Low;
        else if (est.score < 6.0) est.level = ComplexityLevel::Medium;
        else if (est.score < 8.5) est.level = ComplexityLevel::High;
        else est.level = ComplexityLevel::Extreme;

        m_stats.filesAnalyzed++;
        m_stats.avgScore = (m_stats.avgScore * (m_stats.filesAnalyzed - 1) + est.score)
            / m_stats.filesAnalyzed;
        if (est.level == ComplexityLevel::Trivial) m_stats.trivialCount++;
        if (est.level == ComplexityLevel::Extreme) m_stats.extremeCount++;
