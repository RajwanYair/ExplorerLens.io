...
public:
    ...
    std::optional<ComPtr<ID3D12Resource>> ApplySuperResolution(ID3D12Resource* pSourceTexture, int scale = 2);

    std::optional<SmartCropRegion> DetectSalientRegion(ID3D12Resource* pSourceTexture, uint32_t targetWidth,
    ...
    std::optional<ComPtr<ID3D12Resource>> ApplyContentAwareCrop(ID3D12Resource* pSourceTexture, uint32_t targetWidth,
                                                                uint32_t targetHeight);
...
private:
    ...
    ComPtr<ID3D12Resource> TensorToTexture(const std::vector<float>& tensor, uint32_t width, uint32_t height);

    HRESULT ExecuteDirectMLOperator(IDMLOperator* pOperator);

    void PreprocessImageForONNX(const std::vector<float>& input, std::vector<float>& output, uint32_t width,
                                uint32_t height);

    SmartCropRegion FindBestCropRegion(const std::vector<float>& saliencyMap, uint32_t mapWidth, uint32_t mapHeight,
