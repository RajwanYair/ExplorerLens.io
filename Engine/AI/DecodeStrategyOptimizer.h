...
public:
    ...
    void RecordTrial(const std::string& format, DecodeStrategy strategy,
        ...
        for (auto& p : perfs) {
            if (p.strategy == strategy) { sp = &p; break; }
        }
        if (!sp) {
        ...
        // Success rate update
        sp->successRate = ((sp->successRate * (sp->trialCount - 1)) + (success ? 1.0 : 0.0))
            / sp->trialCount;
    ...
    StrategyRecommendation Recommend(const std::string& format) const {
        ...
        for (const auto& p : perfs) totalTrials += p.trialCount;
    ...
    static const char* StrategyName(DecodeStrategy s) {
        switch (s) {
        case DecodeStrategy::CPUSingleThread:  return "CPUSingleThread";
        case DecodeStrategy::CPUMultiThread:    return "CPUMultiThread";
        case DecodeStrategy::GPUDirect:         return "GPUDirect";
        case DecodeStrategy::GPUWithFallback:   return "GPUWithFallback";
        case DecodeStrategy::MemoryMapped:      return "MemoryMapped";
        case DecodeStrategy::Streaming:         return "Streaming";
        case DecodeStrategy::Cached:            return "Cached";
