...
public:
    inline bool BuildGraph(const GraphConfig& config,
        ...
        for (const auto& node : nodes) {
            if (m_nodes.size() >= config.maxNodes) break;
    ...
    inline bool UpdateIncremental(uint64_t fileId, const std::vector<float>& embedding) {
        if (!m_built || m_nodes.size() >= m_config.maxNodes) return false;
    ...
    inline uint64_t GetEdgeCount() const {
        uint64_t count = 0;
        for (const auto& [_, edges] : m_adjacency) count += edges.size();
        return count;
    }
...
private:
    inline std::vector<SimilarityResult> ComputeNeighbors(uint64_t sourceId,
        ...
        std::partial_sort(candidates.begin(),
                          candidates.begin() + std::min(static_cast<size_t>(m_config.k), candidates.size()),
                          candidates.end(),
                          [](const SimilarityResult& a, const SimilarityResult& b) { return a.score > b.score; });
        if (candidates.size() > m_config.k) candidates.resize(m_config.k);
        return candidates;
    }
