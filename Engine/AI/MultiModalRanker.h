  ...
  public:
    ...
    inline std::vector<RankedResult> Rank(const std::vector<RankCandidate>& candidates) const
    {
        ...
        for (const auto& c : candidates) {
            float score = m_weights.clip * c.clipScore + m_weights.bm25 * c.bm25Score
                          + m_weights.recency * c.recencyScore + m_weights.size * c.sizeScore;

            if (score < m_config.minScore)
                continue;
    ...
    inline std::vector<RankedResult> GetTopK(const std::vector<RankCandidate>& candidates, uint32_t k) const
    {
