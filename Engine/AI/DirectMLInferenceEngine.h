...
struct DMLInferenceSession {
    ...
    double AvgLatencyUs() const {
        return inferenceCount > 0 ?
            static_cast<double>(totalLatencyUs) / inferenceCount : 0.0;
    }
};
...
public:
    ...
    bool RunInference(uint64_t sessionId, const std::vector<float>& input,
        std::vector<float>& output) {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto it = FindSession(sessionId);
        if (it == m_sessions.end() || !it->active) return false;
        ...
        for (uint32_t dim : it->outputShape) outputSize *= dim;
    ...
    bool Validate() const {
        ...
        for (const auto& s : m_sessions) {
            ...
            if (s.modelName.empty()) return false;
            if (s.active && s.inputShape.empty()) return false;
        }
        return true;
    }
