...
public:
    ...
    {
        DedupResult result;
        if (!m_initialized || candidates.empty())
            return result;
        ...
        for (size_t i = 0; i < candidates.size(); ++i) {
            ...
            for (size_t j = i + 1; j < candidates.size(); ++j) {
                if (clustered[j])
                    continue;
                uint32_t dist = HammingDistance(candidates[i].perceptualHash, candidates[j].perceptualHash);
    ...
    inline uint64_t ComputePerceptualHash(const uint8_t* grayPixels, uint32_t width, uint32_t height) const
    {
        if (!grayPixels || width < DCT_BLOCK_SIZE || height < DCT_BLOCK_SIZE)
            return 0;
