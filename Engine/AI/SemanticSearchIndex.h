...
class SemanticSearchIndex
{
  public:
    ...
    {
        if (!m_initialized || embedding.size() != m_config.dimension)
            return false;
    ...
    {
        if (!m_initialized || query.size() != m_config.dimension)
            return {};
        ...
        std::partial_sort(results.begin(), results.begin() + std::min(static_cast<size_t>(topK), results.size()),
                          results.end(), [](const SearchResult& a, const SearchResult& b) { return a.score > b.score; });
