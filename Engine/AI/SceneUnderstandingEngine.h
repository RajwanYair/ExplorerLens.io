...
public:
    ...
    static SceneClassification ClassifyByHeuristics(const uint8_t* rgb,
        ...
        double saturation = 0;
        {
            double maxC = meanR > meanG ? (meanR > meanB ? meanR : meanB) :
                (meanG > meanB ? meanG : meanB);
            double minC = meanR < meanG ? (meanR < meanB ? meanR : meanB) :
                (meanG < meanB ? meanG : meanB);
            if (maxC > 0) saturation = (maxC - minC) / maxC;
        }
        ...
        for (uint32_t y = 1; y < height - 1; ++y) {
            for (uint32_t x = 1; x < width - 1; ++x) {
                ...
                if (gx * gx + gy * gy > 2500) ++edgePixels;
            }
        }
        double edgeDensity = static_cast<double>(edgePixels) /
            ((width - 2) * (height - 2));
        ...
        if (result.score > 0.7f) result.confidence = SceneConfidence::High;
        else if (result.score > 0.5f) result.confidence = SceneConfidence::Medium;
        else result.confidence = SceneConfidence::Low;
