...
void PluginHostServer::HandleRequestThumbnail(uint64_t correlation_id, 
    ...
    const auto* request = reinterpret_cast<const IPC::ThumbnailRequest*>(payload.data());
...
void PluginHostServer::HandleCanDecodeQuery(uint64_t correlation_id, 
                                            const std::vector<uint8_t>& payload) {
    // Parse query
