...
class PolicyVersionControl
{
  public:
    ...
    // Commit operations
    std::string CommitPolicy(const std::string& policyJson, const CommitOptions& options);
    ...
    std::vector<PolicyDiff> DiffVersions(const std::string& hashA, const std::string& hashB) const;
    ...
    // Tag support
    bool TagVersion(const std::string& hash, const std::string& tag);
