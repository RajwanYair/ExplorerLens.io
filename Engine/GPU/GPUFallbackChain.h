...
public:
    ...
    void Initialize(const GPUFallbackConfig& config = {}) {
        ...
        m_chain.clear();
        if (config.enableDX12)  m_chain.push_back(GPUBackendId::DX12);
        if (config.enableDX11)  m_chain.push_back(GPUBackendId::DX11);
        if (config.enableVulkan) m_chain.push_back(GPUBackendId::Vulkan);
        if (config.alwaysFallbackCPU) m_chain.push_back(GPUBackendId::CPU);
    ...
    GPUBackendSelection Select() {
        ...
        for (uint32_t i = 0; i < static_cast<uint32_t>(m_chain.size()); ++i) {
            ...
            if (sel.isFallback) m_stats.fallbackSelections++;
            return sel;
        }
    ...
    void RecordSuccess(GPUBackendId id) {
        auto idx = static_cast<uint32_t>(id);
        if (idx >= static_cast<uint32_t>(GPUBackendId::Count)) return;
    ...
    GPUBackendStatus GetBackendStatus(GPUBackendId id) const {
        auto idx = static_cast<uint32_t>(id);
        if (idx < static_cast<uint32_t>(GPUBackendId::Count)) return m_backendStatus[idx];
        return {};
    }

    // Get chain length
    uint32_t GetChainLength() const { return static_cast<uint32_t>(m_chain.size()); }

    // Get stats
    GPUFallbackStats GetStats() const { return m_stats; }
