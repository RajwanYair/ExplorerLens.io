...
struct ShaderPerfRecord {
    ...
    double bestMs = 1e9;
    double worstMs = 0.0;
    double AvgMs() const { return invocations > 0 ? totalMs / invocations : 0.0; }
};

class AdaptiveShaderSelection {
public:
    ShaderSelectionResult Select(uint32_t shaderModel, bool hasWaveOps,
        bool hasFP16, uint32_t imageWidth) const {
    ...
    void RecordPerformance(ShaderVariant variant, double ms) {
        ...
        rec.invocations++;
        rec.totalMs += ms;
        if (ms < rec.bestMs) rec.bestMs = ms;
        if (ms > rec.worstMs) rec.worstMs = ms;
    }
