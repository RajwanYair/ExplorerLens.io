...
public:
    ...
    inline DecompressResult DispatchDecompress(DecompressRequest& request) {
        ...
        if (elapsedMs > 0.0) {
            m_stats.lastThroughputMBps = (static_cast<double>(request.decompressedSize) / (1024.0 * 1024.0)) / (elapsedMs / 1000.0);
        }
    ...
    inline double GetThroughputMBps() const { return m_stats.lastThroughputMBps; }
    inline bool IsHardwareAccelerated() const { return m_stats.hardwareAccelerated; }
    inline GPUDecompressVendor GetVendor() const { return m_vendor; }
    inline const SchedulerStats& GetStats() const { return m_stats; }
