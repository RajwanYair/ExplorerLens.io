...
class MetalPipelineV2
{
  public:
    ...
    MetalRenderResult RenderThumbnail(const uint8_t* sourceData, uint32_t srcWidth, uint32_t srcHeight,
                                      uint32_t dstWidth, uint32_t dstHeight)
    {
        ...
        NearestNeighborScale(sourceData, srcWidth, srcHeight, result.pixelData.data(), dstWidth, dstHeight);
        result.success = true;
#endif
        return result;
    }
    ...
    MetalPipelineStatus GetStatus() const
    {
        return m_status;
    }
    ...
    {
        return m_status == MetalPipelineStatus::Ready;
    }
    void Shutdown()
    {
        m_status = MetalPipelineStatus::NotInitialized;
    }
...
#ifdef __APPLE__
    static void NearestNeighborScale(const uint8_t* src, uint32_t srcW, uint32_t srcH, uint8_t* dst, uint32_t dstW,
                                     uint32_t dstH)
    {
