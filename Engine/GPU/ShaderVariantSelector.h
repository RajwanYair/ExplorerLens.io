...
public:
    ...
    ShaderVariantSelectorConfig GetConfig() const { return m_config; }
    std::string GetName() const { return m_config.label; }

    enum class ShaderModel : uint8_t { SM_5_0, SM_5_1, SM_6_0, SM_6_5 };
    ...
    bool RegisterVariant(const Variant& v) {
        if (m_variants.size() >= m_config.maxVariants) return false;
