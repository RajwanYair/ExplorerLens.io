...
public:
    ...
    [[nodiscard]] std::vector<VulkanDecodeResult> DecodeBatch(
        const std::vector<VulkanDecodeJob>& jobs);

    [[nodiscard]] VulkanBackendStatus  Status()     const noexcept { return m_status; }
    [[nodiscard]] const VulkanDeviceInfo& DeviceInfo() const noexcept { return m_deviceInfo; }
