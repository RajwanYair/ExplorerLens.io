...
class D3D11Renderer : public IGPURenderer
{
  public:
    ...
    HRESULT ScaleBitmap(HBITMAP hSource, uint32_t targetWidth, uint32_t targetHeight, HBITMAP* phResult);
  ...
  private:
    ...
    // Rendering pipeline
    HRESULT CreateTextureFromHBITMAP(HBITMAP hBitmap, Microsoft::WRL::ComPtr<ID3D11Texture2D>& pTexture,
                                     Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>& pSRV, uint32_t& width,
                                     uint32_t& height);

    HRESULT ScaleTexture(ID3D11ShaderResourceView* pSourceSRV, uint32_t sourceWidth, uint32_t sourceHeight,
                         uint32_t targetWidth, uint32_t targetHeight, Microsoft::WRL::ComPtr<ID3D11Texture2D>& pOutput);

    HRESULT TextureToHBITMAP(ID3D11Texture2D* pTexture, uint32_t width, uint32_t height, HBITMAP* phBitmap);
