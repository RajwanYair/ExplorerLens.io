...
public:
    ...
    inline bool SubmitRead(const DSReadRequest& request) {
        if (m_status != DSStatus::Available) return false;
        if (!request.filePath || request.readSize == 0) return false;
    ...
    inline DSStatus GetStatus() const { return m_status; }
    inline bool IsAvailable() const { return m_status == DSStatus::Available; }
    inline const DSStatistics& GetStatistics() const { return m_stats; }
    inline uint64_t GetPendingReads() const { return m_pendingReads.load(std::memory_order_relaxed); }
