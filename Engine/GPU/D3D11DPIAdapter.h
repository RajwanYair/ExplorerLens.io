...
class D3D11DPIAdapter
{
  public:
    ...
    D3D11DPISwapChain CreateSwapChain(void* hwnd, const D3D11SwapChainDPIConfig& cfg) const;

    // Update an existing swap chain's physical buffer size on DPI change.
    bool ResizeSwapChain(IDXGISwapChain1* swapChain, uint32_t newPhysicalWidth, uint32_t newPhysicalHeight) const;

    // Blit a staging texture (BGRA) to the swap chain with DPI scaling.
    bool BlitToSwapChain(IDXGISwapChain1* swapChain, const uint8_t* bgraPixels, uint32_t srcWidth,
