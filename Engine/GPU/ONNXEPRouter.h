...
class ONNXEPRouter
{
  public:
    ...
    void RegisterEP(const EPCapability& cap)
    {
        m_eps.push_back(cap);
    }
    ...
    ONNXExecutionProvider Select() const
    {
        ...
        for (const auto& ep : m_eps) {
            ...
            if (m_cfg.policy == EPSelectionPolicy::LowestPower) {
                if (ep.scorePoints < best->scorePoints)
                    best = &ep;
        ...
        if (best)
            return best->ep;
        return m_cfg.allowCPUFallback ? ONNXExecutionProvider::CPU : ONNXExecutionProvider::CPU;
    }
    ...
    EPSelectionPolicy GetPolicy() const
    {
        return m_cfg.policy;
    }
    void SetPolicy(EPSelectionPolicy p)
    {
        m_cfg.policy = p;
    }
    const EPRouterConfig& GetConfig() const
    {
        return m_cfg;
    }
    void Clear()
