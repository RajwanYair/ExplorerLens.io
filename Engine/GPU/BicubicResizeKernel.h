...
public:
    static BicubicResizeKernel& Instance() { static BicubicResizeKernel s; return s; }
    ...
    bool Configure(const BicubicParams& params) {
        ...
        if (params.channels == 0 || params.channels > 4) return false;
        m_params = params;
        m_configured = true;
        return true;
    }
    ...
    std::vector<BicubicWeightEntry> ComputeWeights(float center, uint32_t srcSize) const {
        ...
        // Normalize
        if (wSum > 0.0f) {
            for (auto& e : entries) e.weight /= wSum;
        }
        return entries;
    }
    ...
    bool VerifyKernel() const {
        ...
        if (std::abs(v2) > 1e-5f) return false;
    ...
    const BicubicParams& GetParams() const { return m_params; }

    bool Validate() const {
        if (!m_configured) return true;
        if (m_params.srcWidth == 0 || m_params.dstWidth == 0) return false;
        if (m_params.B < 0.0f || m_params.B > 1.0f) return false;
        if (m_params.C < 0.0f || m_params.C > 1.0f) return false;
        return VerifyKernel();
    }
