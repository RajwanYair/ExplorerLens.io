...
enum class NCHWABackend { NVDEC, AMF, QuickSync, Software };
...
public:
    NCHWAEncodeResult Encode(const NCHWAEncodeRequest& req) {
        ...
        if (!IsAvailable(chosen)) chosen = NCHWABackend::Software;
    ...
    static std::string BackendName(NCHWABackend backend) {
        switch (backend) {
            case NCHWABackend::NVDEC:     return "NVDEC";
            case NCHWABackend::AMF:       return "AMF";
            case NCHWABackend::QuickSync: return "QuickSync";
            case NCHWABackend::Software:  return "Software";
        }
        return "Unknown";
    }
    NCHWABackend PreferredBackend() const { return NCHWABackend::QuickSync; }
};
