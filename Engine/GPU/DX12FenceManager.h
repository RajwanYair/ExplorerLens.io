...
inline const char* FenceStateName(FenceState s) noexcept
{
    switch (s) {
        ...
        case FenceState::Signaled:
            return "Signaled";
        case FenceState::Waiting:
            return "Waiting";
        case FenceState::Timeout:
            return "Timeout";
...
{
    switch (m) {
        case FenceWaitMode::Blocking:
            return "Blocking";
        case FenceWaitMode::Polling:
            return "Polling";
        case FenceWaitMode::Event:
            return "Event";
        case FenceWaitMode::Hybrid:
            return "Hybrid";
        case FenceWaitMode::BusyWait:
            return "BusyWait";
        default:
            return "Unknown";
    }
  ...
  public:
    ...
    uint64_t GetCompletedValue(uint64_t fenceId) const
    {
        for (const auto& f : m_fences) {
            if (f.id == fenceId)
                return f.completedValue;
        }
        return 0;
    }

    /// Get number of active fences
    size_t GetActiveFenceCount() const noexcept
    {
        return m_fences.size();
    }

    /// Get total signal count
    uint64_t GetSignalCount() const noexcept
    {
        return m_signalCount;
    }

    /// Get total wait count
    uint64_t GetWaitCount() const noexcept
    {
        return m_waitCount;
    }

    /// Apply configuration
    void SetConfig(const FenceManagerConfig& cfg) noexcept
    {
        m_config = cfg;
    }

    /// Get wait mode
    FenceWaitMode GetWaitMode() const noexcept
    {
        return m_config.defaultWaitMode;
    }
