...
enum class NPUVendor     { Intel, Qualcomm, MediaTek, AMD, Unknown };
enum class OVPrecision   { FP32, FP16, INT8, BF16 };
enum class NPUBackendState { Uninitialized, Ready, Running, Degraded, Faulted };
...
public:
    ...
    bool              Initialize()     { m_state = NPUBackendState::Ready; return true; }
    bool              IsAvailable()    const { return m_state == NPUBackendState::Ready ||
                                                      m_state == NPUBackendState::Running; }
    NPUBackendState   GetState()       const { return m_state; }
    NPUVendor         GetVendor()      const { return NPUVendor::Intel; }
    OVPrecision       GetPrecision()   const { return m_cfg.precision; }

    bool              Infer(const float* input, size_t size, float* output) {
        (void)input; (void)output;
        if (!IsAvailable() || !input || size == 0) return false;
        ++m_metrics.framesProcessed;
        m_metrics.inferenceMs = 1;
        return true;
    }

    NPUInferenceMetrics GetMetrics()   const { return m_metrics; }
    void                ResetMetrics()       { m_metrics = {}; }
    const IntelNPUConfig& GetConfig()  const { return m_cfg; }
    void                SetConfig(const IntelNPUConfig& cfg) { m_cfg = cfg; }
    void                Shutdown()          { m_state = NPUBackendState::Uninitialized; }
