...
public:
    void Configure(const ProfilerConfig& config) { m_config = config; }
    ...
    void RecordDispatch(const DispatchRecord& record) {
        ...
        if (record.gpuTimeMs < summary.minGpuMs) summary.minGpuMs = record.gpuTimeMs;
        if (record.gpuTimeMs > summary.maxGpuMs) summary.maxGpuMs = record.gpuTimeMs;
        summary.totalInputBytes += record.inputBytes;
        summary.totalOutputBytes += record.outputBytes;
        if (summary.totalGpuMs > 0) {
            summary.throughputMBps = (summary.totalInputBytes + summary.totalOutputBytes) /
                (1024.0 * 1024.0) / (summary.totalGpuMs / 1000.0);
        }
    }
