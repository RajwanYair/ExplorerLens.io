...
D3D12ComputePipeline::D3D12ComputePipeline(const ComputePipelineConfig& config)
 : m_config(config)
{
}
...
ComputeResult D3D12ComputePipeline::Resize(
 ...
 uint32_t outputHeight)
{
 ...
 auto result = ResizeCPU(inputData, inputWidth, inputHeight,
 ...
 m_stats.totalPixelsProcessed += 
 static_cast<uint64_t>(outputWidth) * outputHeight;

 return result;
}

ComputeResult D3D12ComputePipeline::ToneMap(
 const uint8_t* inputData,
 uint32_t width,
 uint32_t height,
 ToneMapOperator op)
...
ComputeResult D3D12ComputePipeline::ConvertColorSpace(
 const uint8_t* inputData,
 uint32_t width,
 uint32_t height,
 GPUColorSpace srcSpace,
 GPUColorSpace dstSpace)
{
 ...
 // sRGB to linear
 converted = (v <= 0.04045f) ? (v / 12.92f) :
 std::pow((v + 0.055f) / 1.055f, 2.4f);
 } else if (srcSpace == GPUColorSpace::LinearRGB && dstSpace == GPUColorSpace::SRGB) {
 // Linear to sRGB
 converted = (v <= 0.0031308f) ? (v * 12.92f) :
 (1.055f * std::pow(v, 1.0f / 2.4f) - 0.055f);
 }

 result.outputData[i * 4 + c] = static_cast<uint8_t>(
 (std::min)(255.0f, (std::max)(0.0f, converted * 255.0f)));
 }
...
{
 switch (backend) {
 case GPUBackend::Auto: return L"Auto";
 case GPUBackend::D3D12: return L"D3D12";
 case GPUBackend::D3D11: return L"D3D11";
 case GPUBackend::CPU: return L"CPU";
 case GPUBackend::Disabled: return L"Disabled";
...
{
 switch (algo) {
 case ScalingAlgorithm::NearestNeighbor: return L"NearestNeighbor";
 case ScalingAlgorithm::Bilinear: return L"Bilinear";
 case ScalingAlgorithm::Bicubic: return L"Bicubic";
 case ScalingAlgorithm::Lanczos3: return L"Lanczos3";
 case ScalingAlgorithm::Adaptive: return L"Adaptive";
 default: return L"Unknown";
 }
}
...
{
 switch (cs) {
 case GPUColorSpace::SRGB: return L"sRGB";
 case GPUColorSpace::LinearRGB: return L"LinearRGB";
 case GPUColorSpace::HDR10: return L"HDR10";
 case GPUColorSpace::HLG: return L"HLG";
 case GPUColorSpace::DolbyVision: return L"DolbyVision";
 default: return L"Unknown";
 }
}
...
{
 switch (op) {
 case ToneMapOperator::None: return L"None";
 case ToneMapOperator::Reinhard: return L"Reinhard";
 case ToneMapOperator::ReinhardExtended: return L"ReinhardExtended";
 case ToneMapOperator::ACES: return L"ACES";
 case ToneMapOperator::Uncharted2: return L"Uncharted2";
 case ToneMapOperator::HableFilmic: return L"HableFilmic";
 default: return L"Unknown";
 }
}
...
ComputeResult D3D12ComputePipeline::ResizeCPU(
 const uint8_t* inputData,
 uint32_t inputWidth,
 uint32_t inputHeight,
 uint32_t outputWidth,
 uint32_t outputHeight)
{
 ...
 result.outputData[dstIdx + c] = static_cast<uint8_t>(
 (std::min)(255.0f, (std::max)(0.0f, v)));
 }
 }
 }
...
void D3D12ComputePipeline::CalculateDispatch(
