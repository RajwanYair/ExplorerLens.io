...
enum class HexagonVariant { V65, V66, V68, V69, V73, Unknown };
enum class QNNBackend      { HTP, HTA, CPU };
...
public:
    ...
    HexagonVariant GetVariant()  const { return m_cfg.variant; }
    QNNBackend     GetQNNBackend() const { return m_cfg.preferredBE; }
    bool          HVXEnabled()   const { return m_cfg.enableHVX; }
    bool          HMXEnabled()   const { return m_cfg.enableHMX; }

    HexagonRunResult RunInference(const uint8_t* input, size_t inputSize,
                                  uint8_t* output,       size_t outputSize) {
        (void)input; (void)output; (void)outputSize;
        HexagonRunResult r;
        if (!m_ready || !input || inputSize == 0) { r.errorMsg = "Not ready"; return r; }
        r.success   = true;
        r.latencyUs = 800;
        r.powerMw   = 150.0f;
        ++m_runCount;
    ...
    const HexagonDSPConfig& GetConfig() const { return m_cfg; }
    void             SetConfig(const HexagonDSPConfig& cfg) { m_cfg = cfg; }
    void             Reset()         { m_ready = false; m_runCount = 0; }
