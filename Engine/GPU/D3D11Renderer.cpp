...
HRESULT D3D11Renderer::CreateDevice(bool allowWARP)
{
    ...
    hr = D3D11CreateDevice(nullptr,                   // Default adapter
                           ...
                           createDeviceFlags, featureLevels, ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, &m_device,
...
HRESULT D3D11Renderer::CompileShaders()
{
 ...
 targetTexture[DTid.xy] = sum;
    ...
    HRESULT hr = D3DCompile(shaderSource, strlen(shaderSource), "ThumbnailResize", nullptr, nullptr, "CSResizeLanczos3",
...
HRESULT D3D11Renderer::ScaleBitmap(HBITMAP hSource, uint32_t targetWidth, uint32_t targetHeight, HBITMAP* phResult)
{
...
HRESULT D3D11Renderer::CreateTextureFromHBITMAP(HBITMAP hBitmap, ComPtr<ID3D11Texture2D>& pTexture,
                                                ComPtr<ID3D11ShaderResourceView>& pSRV, uint32_t& width,
                                                uint32_t& height)
{
    BITMAP bm;
...
HRESULT D3D11Renderer::ScaleTexture(ID3D11ShaderResourceView* pSourceSRV, uint32_t sourceWidth, uint32_t sourceHeight,
                                    uint32_t targetWidth, uint32_t targetHeight, ComPtr<ID3D11Texture2D>& pOutput)
{
...
HRESULT D3D11Renderer::TextureToHBITMAP(ID3D11Texture2D* pTexture, uint32_t width, uint32_t height, HBITMAP* phBitmap)
{
    ...
    for (uint32_t y = 0; y < height; y++) {
        memcpy(static_cast<BYTE*>(pBits) + y * pitch, static_cast<BYTE*>(mapped.pData) + y * mapped.RowPitch, pitch);
    }
...
HRESULT D3D11Renderer::RenderThumbnailBatch(BatchRenderRequest* requests, uint32_t requestCount)
{
    ...
    for (uint32_t i = 0; i < requestCount; ++i) {
        ...
        HRESULT hr = RenderThumbnail(req.imageData, req.imageWidth, req.imageHeight, req.thumbWidth, req.thumbHeight,
                                     req.outBitmap);
