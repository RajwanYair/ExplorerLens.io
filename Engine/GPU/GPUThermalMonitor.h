...
public:
    ...
    ThermalReading GetLastReading() const { return m_lastReading; }
    ThermalMetrics GetMetrics() const { return m_metrics; }
    bool IsThrottling() const { return m_metrics.currentThrottlePercent > 0.0f; }
    float GetThrottlePercent() const { return m_metrics.currentThrottlePercent; }

    void SetPolicy(const ThermalPolicy& policy) { m_policy = policy; }
    ThermalPolicy GetPolicy() const { return m_policy; }

private:
    ThermalZone ClassifyTemperature(float tempC) const {
        if (tempC >= m_policy.emergencyThresholdC) return ThermalZone::Emergency;
        if (tempC >= m_policy.criticalThresholdC) return ThermalZone::Critical;
        if (tempC >= m_policy.hotThresholdC) return ThermalZone::Hot;
        if (tempC >= m_policy.warmThresholdC) return ThermalZone::Warm;
        return ThermalZone::Cool;
    }

    float GetThrottlePercent(ThermalZone zone) const {
        switch (zone) {
        case ThermalZone::Hot: return m_policy.throttleStepPercent;
        case ThermalZone::Critical: return m_policy.throttleStepPercent * 2;
        case ThermalZone::Emergency: return 100.0f;
        default: return 0.0f;
        }
    }
