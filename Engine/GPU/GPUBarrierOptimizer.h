...
public:
    ...
    GPUBarrierOptimizerConfig GetConfig() const { return m_config; }
    std::string GetName() const { return m_config.label; }

    enum class BarrierType : uint8_t { Transition, UAV, Aliasing };
    ...
    bool QueueBarrier(const PendingBarrier& b) {
        if (m_pending.size() >= m_config.maxBatchSize) return false;
        m_pending.push_back(b);
        return true;
    }

    size_t GetPendingCount() const { return m_pending.size(); }
    void FlushBarriers() { m_pending.clear(); }
