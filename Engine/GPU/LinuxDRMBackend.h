...
class LinuxDRMBackend
{
  public:
    ...
    DRMReadbackResult RenderOffscreen(const uint8_t* sourceData, uint32_t srcWidth, uint32_t srcHeight,
                                      uint32_t outWidth, uint32_t outHeight)
    {
        ...
        if (m_status != DRMBackendStatus::Ready || !sourceData)
            return result;
        ...
        BilinearScale(sourceData, srcWidth, srcHeight, result.pixels.data(), outWidth, outHeight);
        result.success = true;
#endif
        return result;
    }
    ...
    DRMReadbackResult ReadbackPixels(uint32_t x, uint32_t y, uint32_t width, uint32_t height) const
    {
        ...
        if (m_status != DRMBackendStatus::Ready)
            return result;
        if (x + width > m_surfaceWidth || y + height > m_surfaceHeight)
            return result;
    ...
    DRMBackendStatus GetStatus() const
    {
        return m_status;
    }
    const DRMDeviceInfo& GetDeviceInfo() const
    {
        return m_deviceInfo;
    }
    bool IsAvailable() const
    {
        return m_status == DRMBackendStatus::Ready;
    }
...
#ifdef __linux__
    static void BilinearScale(const uint8_t* src, uint32_t srcW, uint32_t srcH, uint8_t* dst, uint32_t dstW,
                              uint32_t dstH)
    {
