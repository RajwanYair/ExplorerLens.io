...
HRESULT GDIRenderer::RenderThumbnail(
    ...
    HBITMAP* outBitmap)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        double durationMs = std::chrono::duration_cast<std::chrono::microseconds>(
...
HRESULT GDIRenderer::CreateBitmapFromRGBA(
    const uint8_t* imageData,
    uint32_t width,
    uint32_t height,
    Bitmap** outBitmap)
...
HRESULT GDIRenderer::ScaleBitmap(
    Bitmap* source,
...
HRESULT GDIRenderer::BitmapToHBITMAP(
...
HRESULT GDIRenderer::GetGPUInfo(
    wchar_t* outName,
    uint32_t nameSize,
    uint32_t* outMemoryMB) const
{
