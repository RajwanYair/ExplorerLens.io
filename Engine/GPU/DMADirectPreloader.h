...
public:
    ...
    bool IsPreloaded(const std::wstring& path) const { return m_entries.count(path) > 0; }
    bool Evict(const std::wstring& path)          { return m_entries.erase(path) > 0; }
    uint32_t PreloadedCount() const { return static_cast<uint32_t>(m_entries.size()); }
