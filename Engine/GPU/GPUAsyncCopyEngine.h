...
enum class CopyDirection : uint8_t {
    HostToDevice, DeviceToHost, DeviceToDevice, COUNT
};

enum class CopyPriority : uint8_t {
    Low, Normal, High, Realtime, COUNT
};
...
public:
    ...
    bool Submit(const CopyRequest& req) {
        ...
        if (m_pending.size() >= m_maxInFlight) return false;
        ...
        for (auto it = m_pending.begin(); it != m_pending.end(); ++it) {
            if (it->requestId == requestId) {
                ...
                result.bandwidthGBps = (it->sizeBytes > 0)
                    ? (static_cast<double>(it->sizeBytes) / (1024.0 * 1024.0 * 1024.0)) / (result.elapsedMs / 1000.0)
                    : 0.0;
    ...
    const AsyncCopyStats& GetStats() const { return m_stats; }

    static size_t DirectionCount() { return static_cast<size_t>(CopyDirection::COUNT); }
    static size_t PriorityCount() { return static_cast<size_t>(CopyPriority::COUNT); }
