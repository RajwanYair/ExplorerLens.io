...
public:
 ...
 bool Dispatch(const ComputeDispatch &params, const uint8_t *srcData,
 size_t srcSize, uint8_t *dstData, size_t dstSize);

 /// Execute resize operation
 std::vector<uint8_t> Resize(const uint8_t *srcBGRA, uint32_t srcW,
 uint32_t srcH, uint32_t dstW, uint32_t dstH);

 /// Get pipeline statistics
 const VulkanComputeStats &GetStats() const { return m_stats; }
...
private:
 ...
 void CPUResize(const uint8_t *src, uint32_t srcW, uint32_t srcH, uint8_t *dst,
