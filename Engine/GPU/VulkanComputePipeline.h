...
/// GPU API backend — uses the shared GPUBackend enum from
/// D3D12PipelineActivation.h
...
/// Compute pipeline statistics (Vulkan)
struct VulkanComputeStats
{
...
class VulkanComputePipeline
{
  public:
    ...
    bool Dispatch(const ComputeDispatch& params, const uint8_t* srcData, size_t srcSize, uint8_t* dstData,
                  size_t dstSize);

    /// Execute resize operation
    std::vector<uint8_t> Resize(const uint8_t* srcBGRA, uint32_t srcW, uint32_t srcH, uint32_t dstW, uint32_t dstH);

    /// Get pipeline statistics
    const VulkanComputeStats& GetStats() const
    {
        return m_stats;
    }
  ...
  private:
    ...
    VulkanComputeStats m_stats;
    std::vector<PipelineCacheEntry> m_pipelineCache;

    bool InitializeVulkan();
    bool InitializeFallback();
    void CPUResize(const uint8_t* src, uint32_t srcW, uint32_t srcH, uint8_t* dst, uint32_t dstW, uint32_t dstH);
};
