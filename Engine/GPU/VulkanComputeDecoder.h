...
public:
    ...
    bool Initialize() {
        ...
        auto pfnCreate = reinterpret_cast<int32_t(*)()>(
            GetProcAddress(m_vulkanDll, "vkCreateInstance"));
        if (!pfnCreate) {
    ...
    bool Decode(VulkanDecodeJob& job) {
        ...
        if (!EnsurePipeline(job.format)) return false;
    ...
    bool SupportsFormat(VulkanDecodeFormat fmt) const {
        return fmt == VulkanDecodeFormat::RGBA8Unorm ||
               fmt == VulkanDecodeFormat::BC1 ||
               fmt == VulkanDecodeFormat::BC3 ||
