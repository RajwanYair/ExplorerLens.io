...
bool VulkanComputePipeline::InitializeVulkan() {
    ...
    // Populate device info (simplified — real impl would use vkEnumeratePhysicalDevices)
...
//------------------------------------------------------------------------------
bool VulkanComputePipeline::Dispatch(const ComputeDispatch& params,
    ...
    if (m_backend == GPUBackend::CPU || m_backend == GPUBackend::None) {
        CPUResize(srcData, params.srcWidth, params.srcHeight,
                  dstData, params.dstWidth, params.dstHeight);
    }
...
//------------------------------------------------------------------------------
std::vector<uint8_t> VulkanComputePipeline::Resize(const uint8_t* srcBGRA,
...
//------------------------------------------------------------------------------
void VulkanComputePipeline::CPUResize(const uint8_t* src, uint32_t srcW, uint32_t srcH,
...
const wchar_t* VulkanComputePipeline::GetShaderName(ComputeShaderType type) {
    switch (type) {
        case ComputeShaderType::BilinearResize:    return L"Bilinear Resize";
        case ComputeShaderType::LanczosResize:     return L"Lanczos Resize";
        case ComputeShaderType::ColorSpaceConvert: return L"Color Space Convert";
        case ComputeShaderType::GammaCorrect:      return L"Gamma Correct";
        case ComputeShaderType::Sharpen:           return L"Sharpen";
        case ComputeShaderType::ToneMap:           return L"Tone Map";
...
const wchar_t* VulkanComputePipeline::GetBackendName(GPUBackend backend) {
    switch (backend) {
        case GPUBackend::None:   return L"None";
        case GPUBackend::Vulkan: return L"Vulkan";
        case GPUBackend::D3D12:  return L"Direct3D 12";
        case GPUBackend::D3D11:  return L"Direct3D 11";
