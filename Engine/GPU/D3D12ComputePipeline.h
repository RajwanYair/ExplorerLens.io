...
enum class GPUColorSpace : uint8_t {
    SRGB,
    LinearRGB,
    HDR10,
    HLG,
...
class D3D12ComputePipeline
{
  public:
    ...
    ComputeResult Resize(const uint8_t* inputData, uint32_t inputWidth, uint32_t inputHeight, uint32_t outputWidth,
                         uint32_t outputHeight);

    /// Apply tone mapping to HDR image
    ComputeResult ToneMap(const uint8_t* inputData, uint32_t width, uint32_t height, ToneMapOperator op);

    /// Convert color space
    ComputeResult ConvertColorSpace(const uint8_t* inputData, uint32_t width, uint32_t height, GPUColorSpace srcSpace,
    ...
    /// Get configuration
    const ComputePipelineConfig& GetConfig() const
    {
        return m_config;
    }
  ...
  private:
    ...
    ComputeResult ResizeCPU(const uint8_t* inputData, uint32_t inputWidth, uint32_t inputHeight, uint32_t outputWidth,
                            uint32_t outputHeight);
    ...
    void CalculateDispatch(uint32_t width, uint32_t height, uint32_t& groupsX, uint32_t& groupsY) const;
