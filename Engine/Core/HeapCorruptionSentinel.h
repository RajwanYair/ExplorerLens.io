...
struct HeapCorruptionReport
{
    ...
    {
        return type == HeapCorruptionType::None;
    }
    std::string TypeName() const noexcept
    {
        switch (type) {
            case HeapCorruptionType::None:
                return "None";
            case HeapCorruptionType::HeadCanaryOverwrite:
                return "HeadCanaryOverwrite";
            case HeapCorruptionType::TailCanaryOverwrite:
                return "TailCanaryOverwrite";
            case HeapCorruptionType::DoubleFree:
                return "DoubleFree";
        }
        return "Unknown";
    }
...
class HeapCorruptionSentinel
{
  public:
    ...
    int BlockCount() const noexcept
    {
        return (int)m_blocks.size();
    }
    bool IsHealthy() const
    {
        return ScanAll().empty();
    }
