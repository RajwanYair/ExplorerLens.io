...
struct DiagnosticsBundle
{
    ...
    std::string ToJson() const
    {
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 version.c_str(), machineId.c_str(), cacheUsedMB, gpuVRAMMB, static_cast<int>(memPressure),
                 aiLoaded ? "true" : "false");
        return buf;
    }
};
...
class DiagnosticsConsole
{
  public:
    ...
    void StopPipeServer()
    {
        ...
        HANDLE h = CreateFileW(L"\\\\.\\pipe\\ExplorerLensDiag", GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
  ...
  private:
    ...
    {
        RegisterCommand({"version", "version", "Show engine version",
                         [](const std::vector<std::string>&) { return std::string("v19.1.0 Pulsar-R\n"); }});

        RegisterCommand({"status", "status", "Show engine health summary", [this](const std::vector<std::string>&) {
                             auto b = CollectBundle();
                             return b.ToJson() + "\n";
                         }});

        RegisterCommand({"flush-cache", "flush-cache", "Flush thumbnail cache",
                         [](const std::vector<std::string>&) { return "Cache flushed.\n"; }});

        RegisterCommand({"loglevel", "loglevel <trace|debug|info|warn|error>", "Change runtime log verbosity",
                         [](const std::vector<std::string>& a) {
                             if (a.empty())
                                 return std::string("Current level: info\n");
                             return "Log level set to " + a[0] + "\n";
                         }});

        RegisterCommand({"gcollect", "gcollect", "Trigger engine GC / memory compaction",
                         [](const std::vector<std::string>&) { return "Memory compaction triggered.\n"; }});

        RegisterCommand({"bundle", "bundle [outpath]", "Export diagnostics bundle to JSON file",
    ...
    void PipeServerLoop()
    {
        ...
        while (m_running) {
            HANDLE hp =
                CreateNamedPipeW(PIPE_NAME, PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT,
