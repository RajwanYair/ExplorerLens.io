...
inline const char* SearchPropertyTypeName(SearchPropertyType t) noexcept {
    switch (t) {
    case SearchPropertyType::Thumbnail:  return "Thumbnail";
    case SearchPropertyType::Dimensions: return "Dimensions";
    case SearchPropertyType::FileType:   return "FileType";
    case SearchPropertyType::ColorDepth: return "ColorDepth";
    case SearchPropertyType::Duration:   return "Duration";
...
inline const char* IndexingStateName(IndexingState s) noexcept {
    switch (s) {
    case IndexingState::Idle:     return "Idle";
    case IndexingState::Indexing: return "Indexing";
    case IndexingState::Paused:   return "Paused";
    case IndexingState::Error:    return "Error";
    case IndexingState::Complete: return "Complete";
    default:                      return "Unknown";
    }
}
...
public:
    ...
    bool RegisterProvider(const std::wstring& scopePath) {
        if (scopePath.empty()) return false;
    ...
    IndexingState GetIndexingState() const noexcept { return m_state; }
    ...
    std::vector<SearchPropertyValue> QueryProperties(const std::wstring& filePath) const {
        ...
        if (filePath.empty() || !m_isRegistered) return results;
    ...
    const std::wstring& GetScopePath() const noexcept { return m_scopePath; }

    /// Set indexing state (for testing/simulation)
    void SetState(IndexingState state) noexcept { m_state = state; }

    /// Apply configuration
    void SetConfig(const SearchIntegrationConfig& cfg) noexcept { m_config = cfg; }
