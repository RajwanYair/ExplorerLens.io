...
{
  public:
    void Configure(const ResourceThrottleCfg& config)
    {
        m_config = config;
    }
    ...
    {
        if (cpuPercent > m_config.cpuCeilingPercent * 1.5 || memBytes > m_config.memoryBudgetBytes * 2)
            return ThrottleLevel::Paused;
        if (cpuPercent > m_config.cpuCeilingPercent * 1.2 || memBytes > m_config.memoryBudgetBytes * 1.5)
            return ThrottleLevel::Heavy;
        if (cpuPercent > m_config.cpuCeilingPercent || memBytes > m_config.memoryBudgetBytes)
    ...
    uint32_t AllowedConcurrency(ThrottleLevel level) const
    {
        switch (level) {
            case ThrottleLevel::None:
                return m_config.maxConcurrentDecodes;
            case ThrottleLevel::Light:
                return std::max(1u, m_config.maxConcurrentDecodes * 3 / 4);
            case ThrottleLevel::Moderate:
                return std::max(1u, m_config.maxConcurrentDecodes / 2);
    ...
    ThrottleSnapshot GetSnapshot() const
    {
        return m_snapshot;
    }
