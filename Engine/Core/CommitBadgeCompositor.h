...
enum class BadgePosition { TopLeft, TopRight, BottomLeft, BottomRight };
enum class BadgeSize     { Tiny, Small, Medium, Large };
...
public:
    ...
    CommitBadgeInfo  Build(const std::string& hash,
                           const std::string& author,
                           uint32_t ageDays) const {
        CommitBadgeInfo b;
        if (hash.empty()) return b;
    ...
    static std::string  FormatAge(uint32_t days) {
        ...
        if (days < 30)  return std::to_string(days / 7) + "w";
        if (days < 365) return std::to_string(days / 30) + "m";
        return std::to_string(days / 365) + "y";
    }

    const CommitBadgeConfig& GetConfig() const { return m_cfg; }
    void  SetPosition(BadgePosition p)         { m_cfg.position = p; }
