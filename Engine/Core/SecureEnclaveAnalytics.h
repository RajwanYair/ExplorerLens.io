...
enum class EnclaveBackend { VBS_HVCI, SGX, TrustZone, Simulation };
...
public:
    explicit SecureEnclaveAnalytics(const EnclaveAnalyticsConfig& config)
        : m_config(config) {}

    EnclaveAnalyticsResult Aggregate(const std::vector<double>& values) {
        EnclaveAnalyticsResult r;
        if (values.empty()) { r.errorMsg = "No values"; return r; }
        r.aggregateValue = std::accumulate(values.begin(), values.end(), 0.0)
    ...
    bool IsEnclaveAvailable() const {
        return m_config.backend == EnclaveBackend::VBS_HVCI ||
               m_config.backend == EnclaveBackend::Simulation;
    }
    static std::string BackendName(EnclaveBackend backend) {
        switch (backend) {
            case EnclaveBackend::VBS_HVCI:   return "VBS_HVCI";
            case EnclaveBackend::SGX:         return "SGX";
            case EnclaveBackend::TrustZone:   return "TrustZone";
            case EnclaveBackend::Simulation:  return "Simulation";
        }
        return "Unknown";
    }
