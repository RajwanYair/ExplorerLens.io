...
struct PhysicalThumbnailSize {
    ...
    uint32_t TotalBytes() const { return stride * height; }
};
...
public:
    ...
    ScaleFactorInfo GetDisplayScale() const {
        ...
        info.dpiY   = BASE_DPI;
#endif
        info.isFractional = (info.factor != 1.0f && info.factor != 2.0f &&
                             info.factor != 3.0f);
        return info;
    }
    ...
    uint32_t GetPreferredThumbnailSize(float scaleFactor) const {
        if (scaleFactor <= 1.0f)  return 256;
        if (scaleFactor <= 1.5f)  return 384;
        if (scaleFactor <= 2.0f)  return 512;
        if (scaleFactor <= 3.0f)  return 768;
        return 1024;
    }
    ...
    void SetScalingMode(ScalingMode mode) { m_mode = mode; }
    ScalingMode GetScalingMode() const { return m_mode; }
    ...
    float GetEffectiveScale() const {
        if (m_mode == ScalingMode::Manual) return m_manualScale;
