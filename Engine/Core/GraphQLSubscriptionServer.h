...
public:
    ...
    void Publish(const GQLSubEvent& event) {
        for (auto& [id, sub] : m_subscriptions)
            if (sub.first == event.topic || sub.first == "*") sub.second(event);
        ++m_publishedCount;
    }
    uint32_t SubscriberCount() const { return static_cast<uint32_t>(m_subscriptions.size()); }
    uint32_t PublishedCount()  const { return m_publishedCount; }
