...
public:
    ...
    uint32_t GetCapabilityBitmask() const { return m_bitmask; }
    void Refresh() {
        ++m_stats.probesCacheMiss;
#ifdef _WIN32
        m_bitmask = static_cast<uint32_t>(PlatformCapability::D3D12)
                  | static_cast<uint32_t>(PlatformCapability::VulkanCompute);
#endif
    }
    ...
    CapabilityBrokerStats GetStats() const { return m_stats; }

private:
    CrossPlatformCapabilityBroker() { Refresh(); }
