...
enum class StreamProtoType : uint8_t {
    HTTP,
    WebSocket,
    GRPC,
    NamedPipe,
...
enum class ThumbnailStreamState : uint8_t {
    Idle,
    Connecting,
    Streaming,
    Paused,
...
enum class StreamRouteStrategy : uint8_t {
    RoundRobin,
    LeastLoaded,
    FormatAffinity
};
...
{
    switch (v) {
        case ThumbnailVersion::Original:
            return "Original";
        case ThumbnailVersion::Modified:
            return "Modified";
        case ThumbnailVersion::Current:
            return "Current";
        case ThumbnailVersion::Rollback:
            return "Rollback";
        case ThumbnailVersion::Draft:
            return "Draft";
        default:
            return "Unknown";
    }
...
inline const char* VersionActionName(VersionAction a) noexcept
{
    switch (a) {
        ...
        case VersionAction::Delete:
            return "Delete";
        case VersionAction::Purge:
            return "Purge";
        case VersionAction::Archive:
            return "Archive";
        default:
            return "Unknown";
    }
...
{
    switch (t) {
        case TSProtoType::HTTP:
            return "HTTP";
        case TSProtoType::WebSocket:
            return "WebSocket";
        case TSProtoType::GRPC:
            return "GRPC";
        case TSProtoType::NamedPipe:
            return "NamedPipe";
        case TSProtoType::SharedMemory:
            return "SharedMemory";
        default:
            return "Unknown";
    }
...
{
    switch (s) {
        case ThumbnailStreamState::Idle:
            return "Idle";
        case ThumbnailStreamState::Connecting:
            return "Connecting";
        case ThumbnailStreamState::Streaming:
            return "Streaming";
        case ThumbnailStreamState::Paused:
            return "Paused";
        case ThumbnailStreamState::Error:
            return "Error";
        case ThumbnailStreamState::Closed:
            return "Closed";
        default:
            return "Unknown";
    }
...
class ThumbnailVersionControl
{
  public:
    bool CreateVersion(const std::wstring& label, uint32_t crc32, uint32_t sizeBytes)
    {
        ...
        e.label.clear();
        for (wchar_t c : label)
            e.label.push_back(static_cast<char>(c));
    ...
    const std::vector<VersionEntry>& GetHistory() const noexcept
    {
        return m_history;
    }
    uint32_t GetRollbackCount() const noexcept
    {
