...
AsyncShellExtension::AsyncShellExtension(uint32_t threadCount) : m_threadCount(threadCount > 0 ? threadCount : 4) {}
...
AsyncDecodeState AsyncShellExtension::GetRequestState(uint64_t requestId) const
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
//------------------------------------------------------------------------------
void AsyncShellExtension::BoostPriority(uint64_t requestId, DecodePriority newPriority)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    for (auto& req : m_queue) {
        if (req.requestId == requestId && req.state == AsyncDecodeState::Queued) {
...
uint32_t AsyncShellExtension::GetQueueDepthForPriority(DecodePriority priority) const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    uint32_t count = 0;
    for (const auto& req : m_queue) {
