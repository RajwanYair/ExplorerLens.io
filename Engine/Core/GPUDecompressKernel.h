...
public:
    ...
    GPUDecompressVendor GetActiveVendor() const { return m_activeVendor; }

    const GPUDecompressCapability& GetCapabilities() const { return m_caps; }

    static const char* VendorName(GPUDecompressVendor v) {
        switch (v) {
        case GPUDecompressVendor::NvidiaGDeflate:    return "NVIDIA-GDeflate";
        case GPUDecompressVendor::IntelDSB:          return "Intel-DSB";
        case GPUDecompressVendor::AmdComputeShader:  return "AMD-ComputeShader";
        case GPUDecompressVendor::CPUFallback:       return "CPU-Fallback";
        default:                                      return "Auto";
        }
    }

    static bool IsFormatSupported(GPUCompressedFormat fmt) {
        return fmt == GPUCompressedFormat::ZStandard ||
               fmt == GPUCompressedFormat::GDeflate  ||
               fmt == GPUCompressedFormat::LZ4Frame   ||
               fmt == GPUCompressedFormat::Deflate    ||
               fmt == GPUCompressedFormat::Uncompressed;
    }

    static uint32_t EstimateOutputSize(uint32_t compressedSize, GPUCompressedFormat fmt) {
        switch (fmt) {
        case GPUCompressedFormat::Uncompressed: return compressedSize;
        case GPUCompressedFormat::GDeflate:     return compressedSize * 5;   // ~5:1 typical
        case GPUCompressedFormat::ZStandard:    return compressedSize * 8;   // up to 8:1
        case GPUCompressedFormat::LZ4Frame:     return compressedSize * 4;
        case GPUCompressedFormat::Deflate:      return compressedSize * 6;
        default:                                return compressedSize * 4;
        }
    }
...
private:
    ...
    static GPUDecompressVendor SelectBestVendor(
        GPUDecompressVendor pref, const GPUDecompressCapability& caps)
    {
        if (pref != GPUDecompressVendor::Auto) return pref;
        if (caps.gdeflateSupported) return GPUDecompressVendor::NvidiaGDeflate;
        if (caps.dsbSupported)      return GPUDecompressVendor::IntelDSB;
        if (caps.amdCSSupported)    return GPUDecompressVendor::AmdComputeShader;
