...
HighDPIScaling::HighDPIScaling(const DPIConfig& config)
    : m_config(config) {}
    ...
    if (hMod) {
        ...
        if (pFunc) {
            ...
            if (SUCCEEDED(pFunc(nullptr, &awareness))) {
                ...
                case 1: return DPIAwareness::SystemAware;
                case 2: return DPIAwareness::PerMonitorV1;
                }
            }
        }
    }
...
bool HighDPIScaling::SetProcessAwareness(DPIAwareness mode) {
    ...
    if (hMod) {
        ...
        if (pFunc) {
            int awareness = 0;
            switch (mode) {
            case DPIAwareness::Unaware:       awareness = 0; break;
            case DPIAwareness::SystemAware:   awareness = 1; break;
            case DPIAwareness::PerMonitorV1:  awareness = 2; break;
            case DPIAwareness::PerMonitorV2:  awareness = 2; break; // Fallback to V1
            }
...
DPIAwareThumbnailRequest HighDPIScaling::ScaleRequest(
    uint32_t logicalWidth, uint32_t logicalHeight, uint32_t targetDPI) const {
    ...
    req.physicalWidth = std::clamp(req.physicalWidth,
                                   m_config.minPhysicalSize,
                                   m_config.maxPhysicalSize);
    req.physicalHeight = std::clamp(req.physicalHeight,
                                    m_config.minPhysicalSize,
                                    m_config.maxPhysicalSize);
...
DPIScale HighDPIScaling::GetNearestScale(uint32_t dpi) {
    if (dpi <= 108) return DPIScale::Scale100;  // ≤112%
    if (dpi <= 132) return DPIScale::Scale125;
    if (dpi <= 156) return DPIScale::Scale150;
    if (dpi <= 180) return DPIScale::Scale175;
    if (dpi <= 216) return DPIScale::Scale200;
    if (dpi <= 264) return DPIScale::Scale250;
    if (dpi <= 312) return DPIScale::Scale300;
    if (dpi <= 360) return DPIScale::Scale350;
    return DPIScale::Scale400;
}

uint32_t HighDPIScaling::GetDPIForScale(DPIScale scale) {
    switch (scale) {
    case DPIScale::Scale100: return 96;
    case DPIScale::Scale125: return 120;
    case DPIScale::Scale150: return 144;
    case DPIScale::Scale175: return 168;
    case DPIScale::Scale200: return 192;
    case DPIScale::Scale250: return 240;
    case DPIScale::Scale300: return 288;
    case DPIScale::Scale350: return 336;
    case DPIScale::Scale400: return 384;
    default: return 96;
    }
}
...
const wchar_t* HighDPIScaling::GetScaleName(DPIScale scale) {
    ...
    case DPIScale::Scale400: return L"400%";
    case DPIScale::Custom:   return L"Custom";
...
const wchar_t* HighDPIScaling::GetAwarenessName(DPIAwareness awareness) {
    switch (awareness) {
    case DPIAwareness::Unaware:      return L"Unaware";
    case DPIAwareness::SystemAware:  return L"SystemAware";
    case DPIAwareness::PerMonitorV1: return L"PerMonitorV1";
    case DPIAwareness::PerMonitorV2: return L"PerMonitorV2";
    default: return L"Unknown";
    }
}
