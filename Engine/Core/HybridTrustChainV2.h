...
enum class ChainAlgo   { ClassicX509, MLDSA65, MLDSA87, Hybrid };
...
public:
    ...
    TrustChainResult Validate(const std::vector<TrustCertificate>& chain,
                               int64_t nowMs = 0) const {
        TrustChainResult res;
        if (chain.empty()) { res.errorCode = "EMPTY_CHAIN"; return res; }

        for (const auto& cert : chain) {
            if (nowMs > 0 && cert.validTo > 0 && nowMs > cert.validTo) {
                res.errorCode = "CERT_EXPIRED"; return res;
            }
            if (cert.publicKey.empty() || cert.signature.empty()) {
                res.errorCode = "MISSING_KEY_OR_SIG"; return res;
            }
        }
        ...
        for (const auto& anchor : m_anchors) {
            if (anchor.subjectDN == root.issuerDN) { foundAnchor = true; break; }
        }
        if (!foundAnchor && !m_anchors.empty()) {
            res.errorCode = "ANCHOR_NOT_FOUND"; return res;
        }
