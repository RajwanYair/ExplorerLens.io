...
class SharedStateCoordinator
{
  public:
    ...
    explicit SharedStateCoordinator(SyncStrategy strategy) noexcept : m_strategy(strategy) {}

    [[nodiscard]] SyncStrategy GetStrategy() const noexcept
    {
        return m_strategy;
    }
    ...
    bool Set(const std::wstring& key, const std::string& value) noexcept
    {
        if (key.empty())
            return false;
        if (static_cast<int>(m_entries.size()) >= MAX_STATE_ENTRIES)
            return false;
    ...
    {
        switch (strategy) {
            case SyncStrategy::Optimistic:
                return L"Optimistic";
            case SyncStrategy::Pessimistic:
                return L"Pessimistic";
            case SyncStrategy::EventualConsistency:
                return L"EventualConsistency";
            default:
                return L"Unknown";
        }
    }
    ...
    {
        switch (reason) {
            case StateChangeReason::UserAction:
                return L"UserAction";
            case StateChangeReason::ProcessEvent:
                return L"ProcessEvent";
            case StateChangeReason::Timeout:
                return L"Timeout";
            case StateChangeReason::Conflict:
                return L"Conflict";
            default:
                return L"Unknown";
        }
