  ...
  public:
    ...
    ColdStartReport PreloadAll()
    {
        ...
        // Phase 2: Pre-read registry configuration
        {
            ...
            auto t0 = std::chrono::high_resolution_clock::now();
            HKEY hKey = nullptr;
            LONG rc = RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens", 0, KEY_READ, &hKey);
            if (rc == ERROR_SUCCESS) {
                // Read a few common values to warm the registry cache
                DWORD val = 0, size = sizeof(val);
                RegQueryValueExW(hKey, L"EnableGPU", nullptr, nullptr, reinterpret_cast<LPBYTE>(&val), &size);
                RegCloseKey(hKey);
                r.success = true;
                report.componentsLoaded++;
            } else {
        ...
        m_stats.preloadRuns++;
        m_stats.avgPreloadTimeMs = m_stats.avgPreloadTimeMs * 0.8 + report.totalPreloadMs * 0.2;
    ...
    {
        return {L"libwebp.dll", L"libjxl.dll", L"libheif.dll", L"libavif.dll", L"dav1d.dll", L"libde265.dll"};
    }
