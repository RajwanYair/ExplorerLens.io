...
{
    switch (d) {
        case StructuredErrorDomain::Decode:
            return "Decode";
        case StructuredErrorDomain::Pipeline:
            return "Pipeline";
        case StructuredErrorDomain::Cache:
            return "Cache";
        case StructuredErrorDomain::GPU:
            return "GPU";
        case StructuredErrorDomain::Plugin:
            return "Plugin";
        case StructuredErrorDomain::IO:
            return "IO";
        case StructuredErrorDomain::Memory:
            return "Memory";
        case StructuredErrorDomain::Network:
            return "Network";
        case StructuredErrorDomain::Config:
            return "Config";
        case StructuredErrorDomain::Security:
            return "Security";
        default:
            return "Unknown";
    }
...
#define ELENS_SOURCE_LOCATION()                                 \
    ::ExplorerLens::Engine::ErrorSourceLocation                 \
    {                                                           \
...
class StructuredError
{
  public:
    ...
    StructuredError(StructuredErrorDomain domain, ErrorSeverity severity, HRESULT code, const std::string& message,
    ...
    StructuredErrorDomain GetDomain() const noexcept
    {
        return m_domain;
    }
    ...
    const ErrorSourceLocation& GetLocation() const noexcept
    {
        return m_location;
    }
    ...
    const StructuredError* GetInnerError() const noexcept
    {
        return m_innerError.get();
    }

    /// Whether this error has an inner error chain
    bool HasInnerError() const noexcept
    {
        return m_innerError != nullptr;
    }
    ...
    {
        return m_severity == ErrorSeverity::Error || m_severity == ErrorSeverity::Fatal;
    }
  ...
  private:
    void FormatErrorImpl(std::ostringstream& oss, uint32_t indentLevel) const
    {
        ...
        oss << indent << "[" << ErrorSeverityName(m_severity) << "] " << StructuredErrorDomainName(m_domain) << ": "
            << m_message;
        oss << " (HRESULT=0x" << std::hex << std::setw(8) << std::setfill('0') << static_cast<uint32_t>(m_code)
            << std::dec << ")";
        if (m_location.IsValid()) {
            oss << " at " << m_location.fileName << ":" << m_location.lineNumber;
...
class ErrorChain
{
  public:
    ...
    static StructuredError Wrap(const StructuredError& inner, StructuredErrorDomain domain, ErrorSeverity severity,
                                HRESULT code, const std::string& contextMessage, ErrorSourceLocation location = {})
    {
    ...
    static StructuredError AddContext(const StructuredError& inner, const std::string& contextMessage,
                                      ErrorSourceLocation location = {})
    {
        return Wrap(inner, inner.GetDomain(), inner.GetSeverity(), inner.GetCode(), contextMessage, location);
    }
};
