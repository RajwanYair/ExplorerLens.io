...
public:
    ...
    AnnotationSchemaMigrator() {
        ...
        RegisterStep({ 1, 2, "Add colorLabel field",
            [](const std::string& s) {
        ...
        RegisterStep({ 2, 3, "Add author field",
            [](const std::string& s) {
        ...
        RegisterStep({ 3, 4, "Add schemaVersion field",
            [](const std::string& s) {
    ...
    SchemaMigrationResult Migrate(const std::string& jsonData,
        ...
        while (v < toVersion) {
            ...
            if (it->second.migrate) current = it->second.migrate(current);
    ...
    static SchemaVersion CurrentVersion() noexcept { return CURRENT_SCHEMA_VERSION; }
