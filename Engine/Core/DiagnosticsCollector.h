...
class DiagnosticsCollector
{
  public:
    ...
    inline bool SaveReport(const std::wstring& path, const DiagReport& report) const
    {
        ...
        if (len > 0) {
            ...
            WideCharToMultiByte(CP_UTF8, 0, text.c_str(), static_cast<int>(text.size()), utf8.data(), len, nullptr,
                                nullptr);
    ...
    {
        std::wstring path = ReadRegistryString(HKEY_LOCAL_MACHINE, L"SOFTWARE\\ExplorerLens", L"InstallPath");
    ...
    inline std::vector<std::wstring> GetRegisteredExtensions() const
    {
        ...
        while (RegEnumKeyExW(hKey, index++, name, &nameLen, nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS) {
            if (name[0] == L'.') {
                ...
                std::wstring subkeyPath = std::wstring(name) + L"\\ShellEx\\{E357FCCD-A995-4576-B01F-234630154E96}";
                HKEY hSub = nullptr;
                if (RegOpenKeyExW(HKEY_CLASSES_ROOT, subkeyPath.c_str(), 0, KEY_READ, &hSub) == ERROR_SUCCESS) {
                    wchar_t val[128]{};
                    DWORD valSize = sizeof(val);
                    DWORD type = 0;
                    if (RegQueryValueExW(hSub, nullptr, nullptr, &type, reinterpret_cast<LPBYTE>(val), &valSize)
                        == ERROR_SUCCESS) {
  ...
  private:
    ...
    static std::wstring CollectOSVersion()
    {
        ...
        std::wostringstream ss;
        ss << L"Windows " << osvi.dwMajorVersion << L"." << osvi.dwMinorVersion << L" (Build " << osvi.dwBuildNumber
           << L")";
        return ss.str();
    }
    ...
    {
        return ReadRegistryString(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0",
    ...
    static std::wstring CollectDirectXVersion()
    {
        ...
        HRESULT hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, featureLevels,
                                       _countof(featureLevels), D3D11_SDK_VERSION, &device, &obtainedLevel, nullptr);

        if (FAILED(hr)) {
            // Try WARP fallback
            hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_WARP, nullptr, 0, featureLevels, _countof(featureLevels),
        ...
        if (SUCCEEDED(hr) && device) {
            switch (obtainedLevel) {
                case D3D_FEATURE_LEVEL_12_1:
                    result = L"DirectX 12.1";
                    break;
                case D3D_FEATURE_LEVEL_12_0:
                    result = L"DirectX 12.0";
                    break;
                case D3D_FEATURE_LEVEL_11_1:
                    result = L"DirectX 11.1";
                    break;
                case D3D_FEATURE_LEVEL_11_0:
                    result = L"DirectX 11.0";
                    break;
                case D3D_FEATURE_LEVEL_10_1:
                    result = L"DirectX 10.1";
                    break;
                case D3D_FEATURE_LEVEL_10_0:
                    result = L"DirectX 10.0";
                    break;
                default:
                    result = L"Unknown";
                    break;
    ...
    static std::wstring ReadRegistryString(HKEY root, const wchar_t* subkey, const wchar_t* valueName)
    {
        ...
        if (RegQueryValueExW(hKey, valueName, nullptr, &type, reinterpret_cast<LPBYTE>(buffer), &bufSize)
                == ERROR_SUCCESS
            && type == REG_SZ) {
            result = buffer;
        }
    ...
    static std::wstring FormatBytes(uint64_t bytes)
    {
        ...
        if (bytes >= 1073741824ULL) {
            ss << std::fixed << std::setprecision(1) << (static_cast<double>(bytes) / 1073741824.0) << L" GB";
        } else if (bytes >= 1048576ULL) {
            ss << std::fixed << std::setprecision(1) << (static_cast<double>(bytes) / 1048576.0) << L" MB";
