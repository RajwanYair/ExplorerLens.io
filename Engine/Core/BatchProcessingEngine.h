#pragma once
// Sprint 233: Batch Processing Engine — multi-file batch operations with progress
#include <string>
#include <vector>
#include <cstdint>
#include <functional>
...
public:
    ...
    static uint32_t GetOperationCount() { return static_cast<uint32_t>(BatchOperation::COUNT); }

    /// Create a new batch job
    size_t CreateJob(const std::wstring& name, BatchOperation op, const std::vector<std::wstring>& files);
    /// Get a job by index
    const BatchJob& GetJob(size_t index) const;
    /// Get total job count
