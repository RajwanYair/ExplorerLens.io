...
public:
    ...
    static bool RemoveWorldAccess(const std::wstring& path) {
        ...
        if (GetNamedSecurityInfoW(path.c_str(), SE_FILE_OBJECT,
                DACL_SECURITY_INFORMATION, nullptr, nullptr, &pOldDACL, nullptr, &pSD) != ERROR_SUCCESS)
            return false;
        ...
        deny.grfAccessPermissions = GENERIC_WRITE | GENERIC_EXECUTE | FILE_WRITE_DATA |
                                    FILE_WRITE_ATTRIBUTES | FILE_APPEND_DATA;
    ...
    static bool ApplyDefaultPolicy(ACLTarget target, const std::wstring& path) {
        switch (target) {
        case ACLTarget::AuditLog:      return LockToCurrentUser(path);
        case ACLTarget::CacheDatabase: return RemoveWorldAccess(path);
        case ACLTarget::Config:        return RemoveWorldAccess(path);
        case ACLTarget::PluginDir:     return RemoveWorldAccess(path);
        default:                       return RemoveWorldAccess(path);
        }
    }
    ...
    static ACLVerifyResult Verify(const std::wstring& path) {
        ACLVerifyResult res;
        PSID pOwner = nullptr; PACL pDACL = nullptr; PSECURITY_DESCRIPTOR pSD = nullptr;

        if (GetNamedSecurityInfoW(path.c_str(), SE_FILE_OBJECT,
                OWNER_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION,
                &pOwner, nullptr, &pDACL, nullptr, &pSD) != ERROR_SUCCESS) {
            res.message = L"GetNamedSecurityInfo failed";
            return res;
        }

        // Owner name
        wchar_t name[256] = {}, domain[256] = {}; DWORD nSz = 256, dSz = 256;
        SID_NAME_USE use;
        ...
        HANDLE hToken = nullptr; PSID pCurrentUser = nullptr;
        ...
        if (pDACL) {
            ...
            for (DWORD i = 0; i < res.aceCount; ++i) {
                void* ace = nullptr;
                if (!GetAce(pDACL, i, &ace)) continue;
                ...
                if (hdr->AceType == ACCESS_ALLOWED_ACE_TYPE) {
                    ...
                    if (EqualSid(sid, pWorldSid) &&
                        (allowed->Mask & (GENERIC_WRITE | FILE_WRITE_DATA))) {
