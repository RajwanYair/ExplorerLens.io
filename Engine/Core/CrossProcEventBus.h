...
public:
    ...
    explicit CrossProcEventBus(EventBusMode mode) noexcept
        : m_mode(mode) {}

    [[nodiscard]] EventBusMode GetMode()       const noexcept { return m_mode; }
    [[nodiscard]] bool         IsRunning()     const noexcept { return m_running; }
    [[nodiscard]] int          GetQueueDepth() const noexcept { return m_queueDepth; }

    void SetMode(EventBusMode mode) noexcept { m_mode = mode; }
    ...
    [[nodiscard]] SubscriptionHandle Subscribe(const std::wstring& topic, EventCallback cb) noexcept {
        if (!cb) return {};
        if (static_cast<int>(m_subs.size()) >= MAX_SUBSCRIBERS) return {};
        SubEntry entry;
    ...
    bool Publish(const BusEvent& event) noexcept {
        if (event.topic.empty()) return false;
        if (m_queueDepth >= MAX_QUEUE_DEPTH) return false;
        ++m_queueDepth;
        for (auto& sub : m_subs) {
        ...
        if (m_queueDepth > 0) --m_queueDepth;
        return true;
    }

    [[nodiscard]] int GetSubscriberCount(const std::wstring& topic = L"") const noexcept {
        if (topic.empty()) return static_cast<int>(m_subs.size());
        int count = 0;
        for (const auto& sub : m_subs) {
            if (sub.handle.topic == topic) ++count;
        }
        return count;
    }
    ...
    static const wchar_t* GetModeName(EventBusMode mode) noexcept {
        switch (mode) {
            case EventBusMode::InProcess:    return L"InProcess";
            case EventBusMode::CrossProcess: return L"CrossProcess";
            case EventBusMode::Hybrid:       return L"Hybrid";
    ...
    static const wchar_t* GetPriorityName(EventPriority prio) noexcept {
        switch (prio) {
            case EventPriority::Low:      return L"Low";
            case EventPriority::Normal:   return L"Normal";
            case EventPriority::High:     return L"High";
            case EventPriority::Critical: return L"Critical";
            default:                      return L"Unknown";
        }
    }
