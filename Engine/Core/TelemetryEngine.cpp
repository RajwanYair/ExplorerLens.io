...
TelemetryEngine::TelemetryEngine()
{
    ...
    m_sessionStartMs = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
}
...
void TelemetryEngine::RecordEvent(const TelemetryEvent& event)
{
    ...
    if (m_privacyMode && !event.piiSafe)
        return;
    auto evt = event;
    if (evt.timestamp == 0) {
        auto now = std::chrono::steady_clock::now();
        evt.timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
    }
    m_events.push_back(evt);
}

void TelemetryEngine::RecordMetric(TelemetryCategory category, const std::wstring& name, double value,
...
HealthScore TelemetryEngine::ComputeHealthScore() const
{
    ...
    for (const auto& e : m_events) {
        if (e.severity == TelemetrySeverity::Error || e.severity == TelemetrySeverity::Critical) {
            errors++;
        }
    }
...
TelemetrySummary TelemetryEngine::GetSummary() const
{
    ...
    for (const auto& e : m_events) {
        if (e.severity == TelemetrySeverity::Error || e.severity == TelemetrySeverity::Critical)
            s.errorCount++;
        if (e.severity == TelemetrySeverity::Warning)
            s.warningCount++;
    }
    auto now = std::chrono::steady_clock::now();
...
std::wstring TelemetryEngine::ExportJSON() const
{
    ...
    for (size_t i = 0; i < m_events.size(); i++) {
        ...
        if (i > 0)
            ss << L",";
        ss << L"{\"name\":\"" << e.eventName << L"\",\"severity\":\"" << GetSeverityName(e.severity)
           << L"\",\"category\":\"" << GetCategoryName(e.category) << L"\",\"value\":" << e.value << L"}";
    }
    ss << L"]}";
    return ss.str();
}
...
{
    switch (category) {
        case TelemetryCategory::Decode:
            return L"Decode";
        case TelemetryCategory::Render:
            return L"Render";
        case TelemetryCategory::Cache:
            return L"Cache";
        case TelemetryCategory::Plugin:
            return L"Plugin";
        case TelemetryCategory::Shell:
            return L"Shell";
        case TelemetryCategory::GPU:
            return L"GPU";
        case TelemetryCategory::Memory:
            return L"Memory";
        case TelemetryCategory::IO:
            return L"IO";
        case TelemetryCategory::Network:
            return L"Network";
        case TelemetryCategory::System:
            return L"System";
        default:
            return L"Unknown";
    }
...
{
    switch (dimension) {
        case HealthDimension::Performance:
            return L"Performance";
        case HealthDimension::Reliability:
            return L"Reliability";
        case HealthDimension::Compatibility:
            return L"Compatibility";
        case HealthDimension::Coverage:
            return L"Coverage";
        case HealthDimension::Memory:
            return L"Memory";
        case HealthDimension::GPU:
            return L"GPU";
        default:
            return L"Unknown";
    }
