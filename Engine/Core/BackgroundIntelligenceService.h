...
enum class BISState    { Idle, Running, Suspended, Paused };
enum class BISJobStatus { Queued, Active, Complete, Failed };
...
public:
    ...
    void SetWorker(BISWorkerFn fn) { m_worker = std::move(fn); }
    void SetIdleThresholdCPU(float pct) noexcept { m_idleCPUThreshold = pct; }
    ...
    int DrainNext(int maxJobs = 4, float currentCPUPct = 0.0f) {
        if (currentCPUPct > m_idleCPUThreshold) return 0;
        int done = 0;
        for (int i = 0; i < maxJobs; ++i) {
            ...
            if (ok) m_metrics.completed++; else m_metrics.failed++;
            done++;
        }
        return done;
    }

    BISState State() const noexcept { return m_state; }
    void SetState(BISState s) noexcept { m_state = s; }
    int QueueDepth() {
        std::lock_guard<std::mutex> lock(m_mutex);
        return static_cast<int>(m_queue.size());
    }
    const BISMetrics& Metrics() const noexcept { return m_metrics; }
