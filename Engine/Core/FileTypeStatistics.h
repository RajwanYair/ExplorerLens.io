...
{
    switch (r) {
        case StatTimeRange::LastHour:
            return "LastHour";
        case StatTimeRange::Today:
            return "Today";
        case StatTimeRange::ThisWeek:
            return "ThisWeek";
        case StatTimeRange::ThisMonth:
            return "ThisMonth";
        case StatTimeRange::AllTime:
            return "AllTime";
        default:
            return "Unknown";
    }
...
class FileTypeStatistics
{
  public:
    ...
    void RecordDecode(const std::string& extension, double timeMs, uint64_t sizeBytes, bool cacheHit, bool error)
    {
    ...
    FileTypeStat GetStats(const std::string& extension, StatCategory category) const
    {
        ...
        switch (category) {
            ...
            case StatCategory::CacheHitRate:
                ...
                break;
            case StatCategory::FileSizeAvg:
                stat.value = (e.totalDecodes > 0) ? static_cast<double>(e.totalSizeBytes) / e.totalDecodes : 0.0;
                break;
        }
        return stat;
    }
    ...
    size_t GetTrackedFormatCount() const
    {
        return m_data.size();
    }
