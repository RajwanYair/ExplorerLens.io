...
enum class SanitizeFlags : uint32_t {
    ...
    All = NullByteInjection | UnicodeNormalize | StripADS | MaxPathEnforce | ControlCharStrip | TrailingDotSpace
          | EncodingValidation
};
  ...
  public:
    ...
    SanitizeResult SanitizePath(const std::wstring& inputPath, SanitizeFlags flags = SanitizeFlags::All)
    {
        ...
        if (result.issuesFound > 0)
            m_issuesDetected += result.issuesFound;
    ...
    {
        if (path.empty() || path.size() > m_maxPathLen)
            return false;
    ...
    // Stats
    uint64_t GetPathsProcessed() const
    {
        return m_pathsProcessed;
    }
    uint64_t GetIssuesDetected() const
    {
        return m_issuesDetected;
    }
