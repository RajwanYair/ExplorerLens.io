...
class ZeroCopyTextureUploader
{
  public:
    ...
    ~ZeroCopyTextureUploader()
    {
        Destroy();
    }

    bool Initialize(ID3D12Device* device, int poolSize = DEFAULT_POOL_SIZE, size_t slotBytes = DEFAULT_SLOT_BYTES)
    {
        if (!device)
            return false;
        m_device = device;
        ...
        for (int i = 0; i < poolSize; ++i) {
            ...
            HRESULT hr =
                device->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_GENERIC_READ,
    ...
    bool Upload(const uint8_t* pixelData, size_t bytes, UploadSlot** outSlot)
    {
        ...
        if (bytes > m_stats.peakUsageBytes)
            m_stats.peakUsageBytes = bytes;
    ...
    const ZeroCopyUploadStats& Stats() const
    {
        return m_stats;
    }
