...
struct TrustVerification {
    ...
    bool             ok() const { return result == TrustCheckResult::Trusted ||
                                         result == TrustCheckResult::NoPinConfigured; }
};
...
public:
    ...
    TrustVerification Verify(const std::wstring& hostname,
                             PCCERT_CONTEXT certCtx) const {
        ...
        if (!certCtx) { tv.result = TrustCheckResult::TLSError; return tv; }
    ...
    void RegisterDefaults() {
        ...
        AddPin({
            L"activate.explorerlens.io",
            { "47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=" }, // placeholder
            90, true
        });
        AddPin({
            L"feedback.explorerlens.io",
            { "47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=" },
            90, true
        });
        AddPin({
            L"telemetry.explorerlens.io",
            { "47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=" },
            90, true
        });
    }
    ...
    std::vector<std::wstring> PinnedHosts() const {
        ...
        for (const auto& [h, _] : m_pins) hosts.push_back(h);
        return hosts;
    }

private:
    NetworkTrustManager() { RegisterDefaults(); }
    ...
    static std::string ExtractSPKIHash(PCCERT_CONTEXT ctx) {
        ...
        uint8_t hash[32] = {}; DWORD hashSz = 32;
        ...
        CryptBinaryToStringA(hash, 32, CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF,
                &b64[0], &b64Sz);
