...
enum class GitFileStatus {
    Clean, Modified, Staged, Untracked, Conflicted, Ignored, Unknown
};
...
public:
    ...
    bool  ShouldRender(const GitStatusResult& r) const {
        if (!r.inRepo) return false;
        if (r.status == GitFileStatus::Clean   && !m_cfg.showOnClean)   return false;
        if (r.status == GitFileStatus::Ignored && !m_cfg.showOnIgnored) return false;
        return true;
    }

    static const wchar_t* StatusLabel(GitFileStatus s) {
        switch (s) {
            case GitFileStatus::Modified:   return L"M";
            case GitFileStatus::Staged:     return L"S";
            case GitFileStatus::Untracked:  return L"+";
            case GitFileStatus::Conflicted: return L"!";
            case GitFileStatus::Ignored:    return L"I";
            default:                        return L"";
        }
    }

    const GitOverlayConfig& GetConfig() const { return m_cfg; }
