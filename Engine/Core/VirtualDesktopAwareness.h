...
class VirtualDesktopAwareness
{
  public:
    ...
    VirtualDesktopId GetCurrentDesktopId() const noexcept
    {
        return m_currentId;
    }
    void SetCurrentDesktopId(VirtualDesktopId id) noexcept
    {
        m_currentId = id;
    }

    void RegisterDesktop(const VirtualDesktopInfo& vd)
    {
        m_desktops[vd.id] = vd;
    }
    ...
    {
        switch (policy) {
            case VDScopePolicy::PerDesktop:
                return m_currentId;
            case VDScopePolicy::ActiveOnly:
                return m_currentId;
            case VDScopePolicy::Global:
                return VDID_GLOBAL;
        }
        return VDID_GLOBAL;
    }
    ...
    {
        if (policy == VDScopePolicy::Global)
            return "global";
        return "vd:" + std::to_string(GetCacheScopeId(policy));
    }

    int DesktopCount() const noexcept
    {
        return (int)m_desktops.size();
    }
    bool HasDesktop(VirtualDesktopId id) const noexcept
    {
        return m_desktops.count(id) > 0;
    }

    std::vector<VirtualDesktopInfo> AllDesktops() const
    {
        std::vector<VirtualDesktopInfo> v;
        for (const auto& kv : m_desktops)
