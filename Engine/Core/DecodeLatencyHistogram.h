...
public:
    ...
    static constexpr double BucketBounds[NUM_BUCKETS] = {
        0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0, 200.0, 500.0, 1000.0, 2000.0, 5000.0
    };
    ...
    static const wchar_t* GetName() { return L"DecodeLatencyHistogram"; }
    ...
    void Record(double latencyMs) {
        ...
        if (latencyMs < m_min) m_min = latencyMs;
        if (latencyMs > m_max) m_max = latencyMs;
    ...
    LatencyPercentiles ComputePercentiles() const {
        ...
        if (m_stats.totalSamples == 0) return p;
        ...
        for (uint32_t i = 0; i < NUM_BUCKETS; ++i) {
            ...
            if (cumulative >= target50 && p.p50 == 0.0) p.p50 = BucketBounds[i];
            if (cumulative >= target90 && p.p90 == 0.0) p.p90 = BucketBounds[i];
            if (cumulative >= target95 && p.p95 == 0.0) p.p95 = BucketBounds[i];
            if (cumulative >= target99 && p.p99 == 0.0) p.p99 = BucketBounds[i];
            if (cumulative >= target999 && p.p999 == 0.0) p.p999 = BucketBounds[i];
        }
        return p;
    }
