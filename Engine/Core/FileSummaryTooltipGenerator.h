...
class FileSummaryTooltipGenerator
{
  public:
    ...
    {
        return L"FileSummaryTooltipGenerator";
    }
    ...
    TooltipContent Generate(const FileMetadata& meta) const
    {
        ...
        if (meta.rawSizeBytes > 0 && meta.fileSizeBytes > 0) {
            ...
            body << L"Compression: " << std::fixed << std::setprecision(1) << ratio << L":1\n";
            lines++;
        }
        ...
        if (meta.decodeTimeMs > 0) {
            footer << L"Decoded in " << std::fixed << std::setprecision(1) << meta.decodeTimeMs << L" ms";
        }
