...
{
    return static_cast<CodecCapability>(static_cast<uint32_t>(a) | static_cast<uint32_t>(b));
}
...
class CodecPlatformV2
{
  public:
    ...
    [[nodiscard]] const CodecRegistration* FindForExtension(const std::string& ext) const noexcept;

    // Auto-probe by magic bytes (ignores file extension).
    [[nodiscard]] const CodecRegistration* ProbeByMagic(std::span<const uint8_t> header) const noexcept;

    // Decode using best codec (extension first, then magic probe fallback).
    [[nodiscard]] std::optional<CodecDecodeResult> Decode(const std::string& ext, const CodecDecodeRequest& req) const;
