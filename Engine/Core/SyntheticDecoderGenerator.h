...
class SyntheticDecoderGenerator
{
  public:
    ...
    GeneratorSynthesisResult Generate(const SyntheticDecoderSpec& spec)
    {
        ...
        r.quality = spec.family == DecoderFamily::Unknown ? StubQuality::Placeholder : StubQuality::Partial;
    ...
    DecoderFamily InferFamily(const std::string& ext) const
    {
        if (ext == "png" || ext == "jpg" || ext == "webp")
            return DecoderFamily::Image;
        if (ext == "pdf" || ext == "docx")
            return DecoderFamily::Document;
        if (ext == "zip" || ext == "rar")
            return DecoderFamily::Archive;
