...
public:
 ...
 m_isDarkMode = enable;
 if (!LoadUxThemeAPIs()) return false;

 if (m_pfnSetPreferredAppMode) {
 m_pfnSetPreferredAppMode(enable ? PreferredAppMode::AllowDark
 ...
 const DarkColorScheme& GetDarkScheme() const { return m_darkScheme; }
 const LightColorScheme& GetLightScheme() const { return m_lightScheme; }
 ...
 COLORREF GetAccent() const { return m_darkScheme.accent; }
 ...
 void DrawText(HDC hdc, const wchar_t* text, RECT& rc,
 ...
 void DrawRoundedBorder(HDC hdc, const RECT& rc, int radius = 4,
...
private:
 ...
 if (m_apisLoaded) return true;

 m_hUxTheme = LoadLibraryW(L"uxtheme.dll");
 if (m_hUxTheme) {
 // Ordinal 135: SetPreferredAppMode (Win10 1903+)
 m_pfnSetPreferredAppMode = reinterpret_cast<FnSetPreferredAppMode>(
 GetProcAddress(m_hUxTheme, MAKEINTRESOURCEA(135)));
 // Ordinal 133: AllowDarkModeForWindow
 m_pfnAllowDarkModeForWindow = reinterpret_cast<FnAllowDarkModeForWindow>(
 GetProcAddress(m_hUxTheme, MAKEINTRESOURCEA(133)));
 // Ordinal 104: RefreshImmersiveColorPolicyState
 m_pfnRefreshPolicy = reinterpret_cast<FnRefreshImmersiveColorPolicyState>(
 GetProcAddress(m_hUxTheme, MAKEINTRESOURCEA(104)));
 }

 m_hDwmApi = LoadLibraryW(L"dwmapi.dll");
 if (m_hDwmApi) {
 DwmSetWindowAttribute = reinterpret_cast<FnDwmSetWindowAttribute>(
 GetProcAddress(m_hDwmApi, "DwmSetWindowAttribute"));
 }
