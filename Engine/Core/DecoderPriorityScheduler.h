...
enum class DecoderTaskPriority : uint8_t { Urgent = 0, Normal = 1, Background = 2, Deferred = 3 };

inline const char* DecoderTaskPriorityName(DecoderTaskPriority p) noexcept {
    switch (p) {
    case DecoderTaskPriority::Urgent:     return "Urgent";
    case DecoderTaskPriority::Normal:     return "Normal";
...
enum class SchedulerPolicy : uint8_t { FIFO = 0, Weighted = 1, StrictPriority = 2 };
...
inline const char* SchedulerPolicyName(SchedulerPolicy p) noexcept {
    ...
    case SchedulerPolicy::StrictPriority: return "StrictPriority";
    default:                              return "Unknown";
    }
}
...
public:
    ...
    bool Cancel(uint32_t id) {
        auto it = std::find_if(m_queue.begin(), m_queue.end(),
            [id](const DecoderTask& t) { return t.id == id; });
        if (it == m_queue.end()) return false;
        m_queue.erase(it);
        return true;
    }

    size_t GetQueueDepth() const noexcept { return m_queue.size(); }
