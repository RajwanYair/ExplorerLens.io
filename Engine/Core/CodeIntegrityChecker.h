...
struct IntegrityResult {
    ...
    bool           revoked = false;
    bool           ok() const { return status == IntegrityStatus::Valid; }
};
...
public:
    ...
    IntegrityResult VerifySignature(const std::wstring& path) const {
        ...
        switch (result) {
        case ERROR_SUCCESS:          res.status = IntegrityStatus::Valid;      break;
        case TRUST_E_NOSIGNATURE:
        case CERT_E_UNTRUSTEDROOT:   res.status = IntegrityStatus::NoSignature; break;
        case TRUST_E_EXPLICIT_DISTRUST:
        case CERT_E_EXPIRED:         res.status = IntegrityStatus::InvalidSig; break;
        default:                     res.status = IntegrityStatus::VerifyError; break;
        }
...
private:
    void ExtractSignerInfo(const std::wstring& path, IntegrityResult& res) const {
        ...
        if (!CryptQueryObject(CERT_QUERY_OBJECT_FILE, path.c_str(),
                ...
                0, &encode, &content, &format, &hStore, &hMsg, nullptr))
            return;
