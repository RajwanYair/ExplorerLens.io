...
enum class ShellCheckId : uint8_t {
    COMRegistration, ThumbnailHandler, FileAssociation, ContextMenu,
    PropertyHandler, OverlayIcon, PreviewHandler, ShellFolder, COUNT
};

enum class ShellCheckResult : uint8_t {
    Pass, Fail, Warning, NotApplicable, COUNT
};
...
public:
    ...
    ShellValidationReport ValidateAll() const {
        ...
        for (uint8_t i = 0; i < static_cast<uint8_t>(ShellCheckId::COUNT); ++i) {
            ...
            switch (v.result) {
            case ShellCheckResult::Pass: report.passCount++; break;
            case ShellCheckResult::Fail:
                report.failCount++;
                report.fullyRegistered = false;
                break;
            case ShellCheckResult::Warning: report.warningCount++; break;
            default: break;
            }
        }
        return report;
    ...
    static size_t CheckIdCount() { return static_cast<size_t>(ShellCheckId::COUNT); }
    static size_t ResultCount() { return static_cast<size_t>(ShellCheckResult::COUNT); }
