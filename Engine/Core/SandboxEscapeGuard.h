...
class SandboxEscapeGuard
{
  public:
    ...
    {
        m_policy = policy;
        if (policy.level == JobObjectLevel::None)
            return true;
        ...
        if (policy.level >= JobObjectLevel::Standard) {
            ...
            uiRestr.UIRestrictionsClass = JOB_OBJECT_UILIMIT_HANDLES | JOB_OBJECT_UILIMIT_READCLIPBOARD
            ...
            SetInformationJobObject(m_hJob, JobObjectBasicUIRestrictions, &uiRestr, sizeof(uiRestr));
        }

        // --- CPU rate cap (Win8+) ---
        if (policy.maxCpuRatePercent > 0 && policy.maxCpuRatePercent < 100) {
            JOBOBJECT_CPU_RATE_CONTROL_INFORMATION cpuCtrl = {};
            cpuCtrl.ControlFlags = JOB_OBJECT_CPU_RATE_CONTROL_ENABLE | JOB_OBJECT_CPU_RATE_CONTROL_HARD_CAP;
            cpuCtrl.CpuRate = policy.maxCpuRatePercent * 100;  // In units of 0.01%
            SetInformationJobObject(m_hJob, JobObjectCpuRateControlInformation, &cpuCtrl, sizeof(cpuCtrl));
        }
    ...
    {
        if (!m_hJob)
            return true;  // No job = no restriction
    ...
    void RefreshStats()
    {
        ...
        if (QueryInformationJobObject(m_hJob, JobObjectBasicAccountingInformation, &acct, sizeof(acct), nullptr)) {
    ...
    const SandboxEscapeStats& Stats() const
    {
        return m_stats;
    }
    const SandboxEscapePolicy& Policy() const
    {
        return m_policy;
    }
    HANDLE JobHandle() const
    {
        return m_hJob;
    }
