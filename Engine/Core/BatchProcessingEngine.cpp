...
const wchar_t *BatchProcessingEngine::GetOperationName(BatchOperation op) {
 switch (op) {
 case BatchOperation::GenerateThumbnails:
 return L"Generate Thumbnails";
 case BatchOperation::ConvertFormats:
 return L"Convert Formats";
 case BatchOperation::ValidateFiles:
 return L"Validate Files";
 case BatchOperation::CleanCache:
 return L"Clean Cache";
 case BatchOperation::ExportMetadata:
 return L"Export Metadata";
 default:
 return L"Unknown";
 }
}
...
size_t
BatchProcessingEngine::CreateJob(const std::wstring &name, BatchOperation op,
...
const BatchJob &BatchProcessingEngine::GetJobByIndex(size_t index) const {
...
bool BatchProcessingEngine::RunJob(size_t index) {
 ...
 (static_cast<double>(i + 1) / job.progress.totalFiles) * 100.0;
 if (m_progressCb)
 m_progressCb(job.progress);
 }
