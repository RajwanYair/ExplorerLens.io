...
{
    switch (c) {
        case DiagCategory::SystemInfo:
            return "SystemInfo";
        case DiagCategory::DecoderHealth:
            return "DecoderHealth";
        case DiagCategory::ETWLogs:
            return "ETWLogs";
        case DiagCategory::Configuration:
            return "Configuration";
        case DiagCategory::PerformanceData:
            return "PerformanceData";
        case DiagCategory::ErrorLogs:
            return "ErrorLogs";
        case DiagCategory::RegistrySnapshot:
            return "RegistrySnapshot";
        case DiagCategory::CacheStats:
            return "CacheStats";
        case DiagCategory::GPUDiagnostics:
            return "GPUDiagnostics";
        case DiagCategory::PluginStatus:
            return "PluginStatus";
        default:
            return "Unknown";
    }
...
struct DiagExportConfig
{
    ...
    static DiagExportConfig Default()
    {
        return {};
    }
...
inline const char* ExportStatusName(ExportStatus s)
{
    switch (s) {
        case ExportStatus::Success:
            return "Success";
        case ExportStatus::PartialSuccess:
            return "PartialSuccess";
        case ExportStatus::NoData:
            return "NoData";
        case ExportStatus::PathError:
            return "PathError";
        case ExportStatus::SizeExceeded:
            return "SizeExceeded";
        case ExportStatus::InternalError:
...
struct ExportResult
{
    ...
    std::string Summary() const
    {
        ...
        if (skippedCount > 0)
            s += ", skipped=" + std::to_string(skippedCount);
        return s;
    }
};
  ...
  public:
    ...
    std::vector<DiagEntry> FilteredEntries() const
    {
        ...
        for (const auto& e : m_entries) {
            if (e.sensitive && !m_config.includeSensitive)
                continue;
            if (e.category == DiagCategory::ETWLogs && !m_config.includeETW)
                continue;
            if (e.category == DiagCategory::PerformanceData && !m_config.includePerformance)
                continue;
            if (e.category == DiagCategory::RegistrySnapshot && !m_config.includeRegistry)
                continue;
            if (e.category == DiagCategory::GPUDiagnostics && !m_config.includeGPU)
                continue;
            if (e.sizeBytes > m_config.maxLogSizeBytes)
                continue;
    ...
    ExportResult Export() const
    {
        ...
        for (const auto& e : m_entries) {
            ...
            if (!found)
                result.skippedFiles.push_back(e.filename);
        }
    ...
    const DiagExportConfig& Config() const
    {
        return m_config;
    }
