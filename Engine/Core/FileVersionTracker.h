...
public:
    ...
    FileVersionTrackerConfig GetConfig() const { return m_config; }
    ...
    bool TrackFile(const std::string& path, uint64_t writeTime, uint64_t size) {
        if (m_versions.size() >= m_config.maxTrackedFiles) return false;
    ...
    bool IsStale(const std::string& path, uint64_t currentWriteTime) const {
        ...
        if (it == m_versions.end()) return true;
        return it->second.lastWriteTime != currentWriteTime;
    }

    size_t GetTrackedCount() const { return m_versions.size(); }
