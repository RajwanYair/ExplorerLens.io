...
enum class GovernorMode     { Balanced, Performance, PowerSave, ThermalThrottle };
enum class GovernorMetric   { ThermalC, PowerW, MemoryMB, GPUPct, CPUPct };
...
public:
    explicit AdaptivePerformanceGovernorV2(GovernorLimits limits = {})
        : m_limits(std::move(limits)) {}

    GovernorDecision Evaluate(const GovernorSample& sample) {
        GovernorDecision decision;
        float pressure = ComputePressure(sample);
        if      (pressure > 0.9f) { decision.mode = GovernorMode::ThermalThrottle; decision.maxDecodeSlots = 2;  decision.gpuClockScale = 0.5f; }
        else if (pressure > 0.7f) { decision.mode = GovernorMode::PowerSave;       decision.maxDecodeSlots = 4;  decision.gpuClockScale = 0.75f; }
        else if (pressure > 0.4f) { decision.mode = GovernorMode::Balanced;        decision.maxDecodeSlots = 8;  decision.gpuClockScale = 1.0f; }
        else                      { decision.mode = GovernorMode::Performance;     decision.maxDecodeSlots = 16; decision.gpuClockScale = 1.0f; }
        m_lastDecision = decision;
        m_evalCount++;
        return decision;
    }

    const GovernorDecision& LastDecision() const noexcept { return m_lastDecision; }
    int EvalCount() const noexcept { return m_evalCount.load(); }
    const GovernorLimits& Limits() const noexcept { return m_limits; }
    ...
    static std::string ModeName(GovernorMode m) noexcept {
        ...
        case GovernorMode::ThermalThrottle: return "ThermalThrottle";
        }
        return "Unknown";
    }
