//==============================================================================
// ExplorerLens Engine — SIMD Acceleration
...
enum class SIMDLevel : uint8_t {
  ...
  SSE42 = 3,  // SSE4.2 — CRC32, string compare
  AVX = 4,    // AVX — 256-bit float ops
  AVX2 = 5,   // AVX2 — 256-bit integer ops
  AVX512 = 6, // AVX-512 — 512-bit ops
  NEON = 7    // ARM NEON — ARM64 SIMD
};
...
struct SIMDCapabilities {
  ...
  uint32_t l1CacheKB = 32;      // L1 in KB
  uint32_t l2CacheKB = 256;     // L2 in KB
};
...
public:
  ...
  static const wchar_t *OperationName(SIMDOperation op) {
    switch (op) {
    case SIMDOperation::BilinearResize:
      return L"Bilinear Resize";
    case SIMDOperation::LanczosResize:
      return L"Lanczos Resize";
    case SIMDOperation::BoxResize:
      return L"Box Resize";
    case SIMDOperation::AlphaBlend:
      return L"Alpha Blend";
    case SIMDOperation::ColorConvert:
      return L"Color Convert";
    case SIMDOperation::GammaCorrect:
      return L"Gamma Correct";
    case SIMDOperation::Sharpen:
      return L"Sharpen";
    case SIMDOperation::Dither:
      return L"Dither";
    default:
      return L"Unknown";
    }
  ...
  static SIMDLevel SelectLevel(const SIMDCapabilities &caps,
                               const SIMDConfig &cfg) {
    SIMDLevel best = SIMDLevel::None;
    if (caps.hasSSE2)
      best = SIMDLevel::SSE2;
    if (caps.hasSSE41)
      best = SIMDLevel::SSE41;
    if (caps.hasSSE42)
      best = SIMDLevel::SSE42;
    if (caps.hasAVX)
      best = SIMDLevel::AVX;
    if (caps.hasAVX2)
      best = SIMDLevel::AVX2;
  ...
  /// Level count (x64 SIMD levels, excluding ARM NEON)
  static constexpr size_t LevelCount() { return 7; }

  /// Operation count
  static constexpr size_t OperationCount() {
    return static_cast<size_t>(SIMDOperation::COUNT);
  }
};
