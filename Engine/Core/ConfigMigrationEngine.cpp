// =============================================================================
// ConfigMigrationEngine.cpp — Sprint 247: Settings Migration Between Versions
...
void ConfigMigrationEngine::AddSetting(const std::wstring& key, const std::wstring& value) {
...
MigrationReport ConfigMigrationEngine::Migrate() {
    ...
    for (const auto& rule : m_rules) {
        ...
        if (rResult.success) report.successCount++;
        else if (rResult.errorMessage.find(L"not found") == std::wstring::npos) report.failureCount++;

        report.results.push_back(rResult);
    }

    m_status = (report.failureCount == 0) ? MigrationStatus::Completed : MigrationStatus::Failed;
...
const wchar_t* ConfigMigrationEngine::GetActionName(MigrationAction action) {
    switch (action) {
        case MigrationAction::Copy:       return L"Copy";
        case MigrationAction::Rename:     return L"Rename";
        case MigrationAction::Transform:  return L"Transform";
        case MigrationAction::Delete:     return L"Delete";
        case MigrationAction::SetDefault: return L"Set Default";
...
const wchar_t* ConfigMigrationEngine::GetStatusName(MigrationStatus status) {
    switch (status) {
        case MigrationStatus::NotStarted: return L"Not Started";
        case MigrationStatus::InProgress: return L"In Progress";
        case MigrationStatus::Completed:  return L"Completed";
        case MigrationStatus::Failed:     return L"Failed";
        case MigrationStatus::RolledBack: return L"Rolled Back";
        default:                          return L"Unknown";
    }
}
