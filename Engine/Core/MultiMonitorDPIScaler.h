...
enum class DPIScaleMode : uint8_t { Unaware = 0, SystemAware = 1, PerMonitor = 2, PerMonitorV2 = 3 };

inline const char* DPIScaleModeName(DPIScaleMode m) noexcept {
    switch (m) {
    case DPIScaleMode::Unaware:      return "Unaware";
    case DPIScaleMode::SystemAware:  return "SystemAware";
...
enum class MonitorProfile : uint8_t { Standard = 0, HiDPI = 1, UltraHiDPI = 2 };
...
public:
    ...
    MonitorProfile GetMonitorProfile(uint32_t dpi) const noexcept {
        if (dpi >= 192) return MonitorProfile::UltraHiDPI;
