...
class DeadCodeAudit
{
  public:
    ...
    const std::vector<DeadCodeFinding>& GetFindings() const
    {
        return m_findings;
    }
    ...
    {
        for (const auto& f : m_findings) {
            if (f.severity >= DeadCodeSeverity::High && f.status != DeadCodeStatus::Cleaned
  ...
  private:
    DeadCodeAudit()
    {
        ...
        m_findings = {
            ...
            {"Engine/Decoders/WMFDecoder_old.cpp", "Obsolete WMF decoder superseded by WMFDecoder.cpp",
             DeadCodeType::ObsoleteFile, DeadCodeSeverity::High, DeadCodeStatus::Cleaned},

            {"Engine/Decoders/unzip.cpp", "Legacy unzip implementation replaced by minizip-ng",
             DeadCodeType::ObsoleteFile, DeadCodeSeverity::High, DeadCodeStatus::Cleaned},

            {"LENSShell/LENSArchive.h", "Commented-out CBuffer template class (~30 lines)",
             DeadCodeType::CommentedOutCode, DeadCodeSeverity::Medium, DeadCodeStatus::Cleaned},

            {"Engine/CMakeLists.txt", "Commented-out add_subdirectory(PluginHost)", DeadCodeType::CommentedOutCode,
             DeadCodeSeverity::Medium, DeadCodeStatus::Cleaned},

            {"LENSShell/LENSArchive.h", "LENSTYPE constants duplicated in LENSTypes.h ", DeadCodeType::DeprecatedAPI,
             DeadCodeSeverity::Low, DeadCodeStatus::Deferred},

            {"external/compression-libs/unrar/UnRAR.dll", "Binary blob in source tree — should be built from source",
             DeadCodeType::StaleBackup, DeadCodeSeverity::Medium, DeadCodeStatus::Confirmed},
            ...
            {"Engine/Shell/COMApartmentAudit.h", "Zero #include refs — entire Shell/ directory unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Medium, DeadCodeStatus::Confirmed},

            {"Engine/Shell/ContextMenuHandler.h", "Zero #include refs — entire Shell/ directory unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Medium, DeadCodeStatus::Confirmed},

            {"Engine/Shell/PreviewPaneHandler.h", "Zero #include refs — entire Shell/ directory unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Medium, DeadCodeStatus::Confirmed},

            // Cloud directory — 2/2 headers unused
            {"Engine/Cloud/CloudThumbnailProvider.h", "Zero #include refs — Cloud/ headers not integrated",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Medium, DeadCodeStatus::Confirmed},

            {"Engine/Cloud/NetworkThumbnailProvider.h", "Zero #include refs — Cloud/ headers not integrated",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Medium, DeadCodeStatus::Confirmed},

            // Memory directory — 5/10 headers unused
            {"Engine/Memory/BufferPoolAllocator.h", "Zero #include refs — registered but unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Memory/DecoderHotsetManager.h", "Zero #include refs — registered but unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Memory/DirectoryFormatProfiler.h", "Zero #include refs — registered but unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Memory/MemoryOptimizationEngine.h", "Zero #include refs — registered but unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Memory/MemorySoakValidator.h", "Zero #include refs — registered but unused",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            // Utils umbrella headers with zero consumers
            {"Engine/Utils/CIPipeline.h", "Umbrella header never included by any file", DeadCodeType::UnusedInclude,
             DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Utils/CodeCoverage.h", "Excluded from TestInfrastructure.h due to type conflicts",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Medium, DeadCodeStatus::Deferred},

            {"Engine/Utils/Enterprise.h", "Umbrella header never included by any file", DeadCodeType::UnusedInclude,
             DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Utils/Installer.h", "Umbrella header never included by any file", DeadCodeType::UnusedInclude,
             DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Utils/Observability.h", "Umbrella header never included by any file", DeadCodeType::UnusedInclude,
             DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Utils/SecurityTesting.h", "Umbrella header never included by any file",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Low, DeadCodeStatus::Confirmed},

            {"Engine/Utils/WindowsUI.h", "Excluded from WindowsCompat.h due to DPIScale enum conflict",
             DeadCodeType::UnusedInclude, DeadCodeSeverity::Medium, DeadCodeStatus::Deferred},
