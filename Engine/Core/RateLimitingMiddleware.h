...
enum class RLMAlgorithm { TokenBucket, SlidingWindow, FixedWindow };
...
public:
    ...
    RLMCheckResult Check(const std::string& clientId) {
        ...
        if (r.allowed) ++bucket.count;
        r.remaining = m_config.maxRequests - bucket.count;
        r.resetAtMs = bucket.windowStartMs + m_config.windowMs;
        return r;
    }
    void Reset(const std::string& clientId) { m_buckets.erase(clientId); }

private:
    RLMConfig m_config;
    struct BucketState { uint32_t count = 0; uint64_t windowStartMs = 0; };
