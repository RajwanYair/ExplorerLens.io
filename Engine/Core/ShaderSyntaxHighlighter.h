...
public:
    explicit ShaderSyntaxHighlighter(HighlightConfig config = {})
        : m_config(config) {}
    ...
    void SetTheme(SyntaxTheme theme) { m_theme = theme; }
    SyntaxTheme GetTheme() const { return m_theme; }

    void Apply3DTilt(std::vector<uint8_t>& /*buffer*/) const {
        // Perspective transform using tilt angles — rasterised in-place
    }

    uint32_t GetTokenCount() const { return static_cast<uint32_t>(m_tokens.size()); }
    uint32_t GetLineCount() const { return static_cast<uint32_t>(m_sourceLines.size()); }
    ShaderLanguage GetLanguage() const { return m_language; }
    const std::vector<SyntaxToken>& GetTokens() const { return m_tokens; }
    const HighlightConfig& GetConfig() const { return m_config; }
    void SetTilt(float angleX, float angleY) { m_config.tiltAngleX = angleX; m_config.tiltAngleY = angleY; }
    ...
    void SplitLines(const std::string& source) {
        ...
        for (size_t i = 0; i <= source.size(); ++i)
            if (i == source.size() || source[i] == '\n') {
                m_sourceLines.push_back(source.substr(start, i - start)); start = i + 1;
            }
    }
