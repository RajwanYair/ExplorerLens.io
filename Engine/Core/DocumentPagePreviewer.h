...
public:
    explicit DocumentPagePreviewer(PreviewConfig config = {})
        : m_config(config) {}

    ~DocumentPagePreviewer() { Close(); }
    ...
    bool RenderPage(uint32_t pageIndex, std::vector<uint8_t>& outputRGBA) const {
        if (!m_isOpen || pageIndex >= m_pages.size()) return false;
        ...
        if (m_pageCallback) m_pageCallback(pageIndex, outputRGBA.data(), w, h);
        return true;
    }
    ...
    std::vector<uint32_t> GetPageThumbnails(uint32_t startPage, uint32_t count) const {
        ...
        for (uint32_t i = startPage; i < end; ++i) indices.push_back(i);
        return indices;
    }

    void SetTransitionEffect(TransitionEffect effect) { m_config.transition = effect; }
    uint32_t GetCurrentPage() const { return m_currentPage; }
    DocumentType GetDocumentType() const { return m_docType; }
    const PageInfo* GetPageInfo(uint32_t index) const {
        return (index < m_pages.size()) ? &m_pages[index] : nullptr;
    }

    void AddPage(const PageInfo& page) { m_pages.push_back(page); }
    void SetPageCallback(PageRenderedCallback cb) { m_pageCallback = std::move(cb); }
...
private:
    void PreloadAdjacent(uint32_t center) {
        ...
        for (uint32_t i = start; i < end; ++i) m_preloadedPages.push_back(i);
    }
