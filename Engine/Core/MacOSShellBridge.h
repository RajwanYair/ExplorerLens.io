...
public:
    ...
    QLThumbnailResponse HandleQLRequest(const QLThumbnailRequest& req) {
        ...
        if (!m_ready) { resp.errorCode = "NOT_INITIALIZED"; return resp; }
        if (!m_platformOk) { resp.errorCode = "MACOS_ONLY"; return resp; }
        if (req.filePath.empty()) { resp.errorCode = "EMPTY_PATH"; return resp; }
    ...
    bool CanHandleExtension(const std::string& ext) const {
        static const char* supported[] = {
            "jpg","jpeg","png","gif","webp","avif","heic","raw","cr2","nef",
            "pdf","zip","cbz",nullptr
        };
