...
enum class CryptoCategory { KEM, Signature, Hash, MAC };
enum class CryptoAlgoId   {
    ECDH_X25519, ECDH_P256,
    ECDSA_P256, RSA_PSS_2048,
    ML_KEM_768, ML_KEM_1024,
    ML_DSA_65,  ML_DSA_87,
    SLH_DSA_128, SLH_DSA_256,
    SHA2_256,   SHA3_256,
    HMAC_SHA256
};
...
public:
    ...
    CryptoAlgoId GetPreferred(CryptoCategory cat) const {
        ...
        if (it != m_preferred.end()) return it->second;
    ...
    std::vector<AlgoCapability> ListByCategory(CryptoCategory cat) const {
        ...
        for (const auto& kv : m_capabilities) {
            if (kv.second.category == cat) out.push_back(kv.second);
        }
        return out;
    }
...
private:
    ...
    void RegisterDefaults() {
        auto reg = [&](CryptoAlgoId id, CryptoCategory cat, const std::string& name,
                       bool pqc, uint32_t bits, bool fips) {
            AlgoCapability c; c.id = id; c.category = cat; c.name = name;
            c.pqcSafe = pqc; c.securityBits = bits; c.fipsApproved = fips;
