...
uint32_t ResourcePoolEngine::EvictExpired()
{
    uint64_t now = NowMs();
    uint32_t evicted = 0;
    auto it = std::remove_if(m_resources.begin(), m_resources.end(), [&](const PooledResource& r) {
...
uint32_t ResourcePoolEngine::GetInUseCount(ResourceType type) const
{
    uint32_t count = 0;
    for (const auto& r : m_resources) {
        if (r.type == type && r.state == ResourceState::InUse)
            count++;
    }
    return count;
}

PoolStats ResourcePoolEngine::GetStats() const
{
    return m_stats;
}
...
{
    switch (type) {
        case ResourceType::DecoderContext:
            return L"Decoder Context";
        case ResourceType::GPUTexture:
            return L"GPU Texture";
        case ResourceType::RenderTarget:
            return L"Render Target";
        case ResourceType::ComputeBuffer:
            return L"Compute Buffer";
        case ResourceType::StagingBuffer:
            return L"Staging Buffer";
        case ResourceType::CommandList:
            return L"Command List";
        default:
            return L"Unknown";
    }
}
