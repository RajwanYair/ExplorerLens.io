...
class BatchDecodeScheduler
{
  public:
    ...
    explicit BatchDecodeScheduler(Config cfg = {}) : m_cfg(cfg) {}
    ...
    void SetCompleteCallback(BatchDecodeCompleteCallback cb)
    {
    ...
    [[nodiscard]] uint64_t TotalCompleted() const noexcept
    {
        return m_completed.load();
    }
    [[nodiscard]] uint64_t TotalCancelled() const noexcept
    {
  ...
  private:
    ...
    std::priority_queue<PrioritizedRequest, std::vector<PrioritizedRequest>, std::greater<PrioritizedRequest>> m_queue;
