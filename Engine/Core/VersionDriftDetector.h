...
{
  public:
    explicit VersionDriftDetector(DriftScanPolicy policy = DefaultPolicy()) : m_policy(std::move(policy)) {}
    ...
    std::vector<DriftEntry> ScanContent(const std::string& path, const std::string& content, ArtifactKind kind) const
    {
    ...
    static std::string FormatReport(const DriftScanResult& r)
    {
        ...
        if (!r.entries.empty()) {
            ...
            for (auto& e : r.entries) {
                report += "[" + SeverityLabel(e.severity) + "] " + e.filePath + ":" + std::to_string(e.lineNumber)
                          + " — found " + e.foundVersion + " expected " + e.expectedVersion + "\n";
            }
        }
        return report;
    }

    const DriftScanPolicy& Policy() const
    {
        return m_policy;
    }
