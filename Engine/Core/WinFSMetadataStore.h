...
enum class MetadataStoreBackend { NTFS_ADS, WindowsPropertySystem, InMemory };
enum class MetadataValueType    { String, Integer, Double, Boolean, DateTime };
...
struct MetadataRecord {
    ...
    void Set(const std::string& key, const std::string& val) {
        for (auto& p : properties) { if (p.key == key) { p.value = val; return; } }
        properties.push_back({ key, val, MetadataValueType::String });
    }
    std::string Get(const std::string& key) const noexcept {
        for (const auto& p : properties) if (p.key == key) return p.value;
        return {};
    }
};
...
public:
    explicit WinFSMetadataStore(MetadataStoreBackend backend = MetadataStoreBackend::InMemory)
        : m_backend(backend) {}
    ...
    MetadataStoreStats Stats() const noexcept {
        ...
        for (const auto& [_, rec] : m_store) s.totalProps += static_cast<int>(rec.properties.size());
        return s;
    }

    MetadataStoreBackend Backend() const noexcept { return m_backend; }

    std::vector<std::wstring> AllPaths() const {
        std::vector<std::wstring> result;
        for (const auto& [path, _] : m_store) result.push_back(path);
        return result;
    }
