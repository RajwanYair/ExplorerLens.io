...
CloudSyncResult CloudSyncProvider::SyncThumbnails(StorageCloudProvider provider, const std::wstring& path)
{
    (void)provider;
...
bool CloudSyncProvider::IsCloudPath(const std::wstring& path) const
{
    ...
    return lower.find(L"onedrive") != std::wstring::npos || lower.find(L"sharepoint") != std::wstring::npos
           || lower.find(L"dropbox") != std::wstring::npos || lower.find(L"google drive") != std::wstring::npos;
}
