...
class FileSignatureDetector
{
  public:
    ...
    SignatureMatch Detect(const uint8_t* data, size_t dataSize) const
    {
        ...
        for (const auto& sig : m_signatures) {
            if (sig.offset + sig.patternLen > dataSize)
                continue;
    ...
    SignatureMatch DetectFile(const wchar_t* filePath) const
    {
        ...
        HANDLE hFile = CreateFileW(filePath, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
  ...
  private:
    ...
    bool MatchPattern(const uint8_t* data, const uint8_t* pattern, const uint8_t* mask, uint32_t len) const
    {
        if (mask) {
    ...
    void BuildSignatureTable()
    {
        ...
        m_signatures.push_back(
            {"PSD", "image/vnd.adobe.photoshop", SignatureCategory::Image, 0, kPSD, 4, nullptr, 0.90f, false});
        ...
        m_signatures.push_back(
            {"HDR", "image/vnd.radiance", SignatureCategory::Image, 0, kHDR, 5, nullptr, 0.90f, false});
        ...
        m_signatures.push_back(
            {"RAR5", "application/x-rar", SignatureCategory::Archive, 0, kRAR5, 8, nullptr, 0.95f, true});
        m_signatures.push_back(
            {"RAR4", "application/x-rar", SignatureCategory::Archive, 0, kRAR4, 7, nullptr, 0.95f, true});
        m_signatures.push_back(
            {"7Z", "application/x-7z-compressed", SignatureCategory::Archive, 0, k7Z, 6, nullptr, 0.95f, true});
        ...
        m_signatures.push_back(
            {"ZSTD", "application/zstd", SignatureCategory::Archive, 0, kZSTD, 4, nullptr, 0.90f, true});
        m_signatures.push_back(
            {"LZ4", "application/x-lz4", SignatureCategory::Archive, 0, kLZ4, 4, nullptr, 0.90f, true});

        // Document formats
        m_signatures.push_back(
            {"PDF", "application/pdf", SignatureCategory::Document, 0, kPDF, 5, nullptr, 0.95f, false});

        // LENSTYPE mappings (values from LENSTypes.h)
        m_formatToLENSType = {
            {"ZIP", 1},   {"RAR4", 2},  {"RAR5", 2},  {"7Z", 3},     {"GZ", 4},     {"TAR", 5},
            {"JPEG", 10}, {"PNG", 11},  {"BMP", 12},  {"GIF87", 13}, {"GIF89", 13}, {"TIFF", 14},
            {"WEBP", 15}, {"ICO", 16},  {"PSD", 17},  {"SVG", 18},   {"PDF", 20},   {"EXR", 30},
            {"HDR", 31},  {"AVIF", 40}, {"HEIC", 41}, {"JXL", 42},
        };
    }
};
