...
public:
    ...
    DecoderFallbackChainConfig GetConfig() const { return m_config; }
    ...
    bool AddDecoder(const ChainEntry& entry) {
        if (m_chain.size() >= m_config.maxChainLength) return false;
    ...
    size_t GetChainLength() const { return m_chain.size(); }
