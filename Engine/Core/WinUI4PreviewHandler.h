...
class WinUI4PreviewHandler
{
  public:
    ...
    WinUI4RenderFrame RenderPreviewPanel(const std::string& filePath, const std::vector<uint8_t>& thumbBGRA,
                                         uint32_t thumbW, uint32_t thumbH, const FilePreviewMetadata& meta)
    {
        WinUI4RenderFrame frame;
        if (!m_ready || filePath.empty()) {
            frame.errorCode = "INVALID_INPUT";
            return frame;
        }
        (void)thumbBGRA;
        (void)thumbW;
        ...
        frame.composited.assign(static_cast<size_t>(frame.width) * frame.height * 4, 0x20);
        frame.success = true;
        return frame;
    }

    void SetConfig(const WinUI4PanelConfig& cfg)
    {
        m_config = cfg;
    }
    const WinUI4PanelConfig& GetConfig() const
    {
        return m_config;
    }
