...
struct DecoderDiagnostics {
    ...
    double AverageTimeMs() const noexcept {
        return (totalDecodes > 0)
            ? totalTimeMs / static_cast<double>(totalDecodes)
            : 0.0;
    }
};
...
struct DiagnosticSnapshot {
    ...
    std::string ToJson() const {
        ...
        for (size_t i = 0; i < recentErrors.size(); ++i) {
            ...
            oss << "    {\"timestamp\": " << entry.timestampMs
                << ", \"domain\": \"" << StructuredErrorDomainName(entry.domain)
                << "\", \"severity\": \"" << ErrorSeverityName(entry.severity)
                << "\", \"message\": \"" << EscapeJson(entry.message)
                << "\", \"code\": " << static_cast<int32_t>(entry.code) << "}";
            if (i + 1 < recentErrors.size()) oss << ",";
            oss << "\n";
        }
        oss << "  ],\n";
        ...
        for (size_t i = 0; i < decoderStats.size(); ++i) {
            ...
            oss << "    {\"name\": \"" << EscapeJson(ds.decoderName)
                << "\", \"totalDecodes\": " << ds.totalDecodes
                << ", \"failures\": " << ds.totalFailures
                << ", \"failureRate\": " << std::fixed << std::setprecision(4)
                << ds.FailureRate()
                << ", \"avgTimeMs\": " << std::fixed << std::setprecision(2)
                << ds.AverageTimeMs() << "}";
            if (i + 1 < decoderStats.size()) oss << ",";
            oss << "\n";
        }
        oss << "  ],\n";
        ...
        for (size_t i = 0; i < summary.subsystems.size(); ++i) {
            ...
            oss << "    {\"name\": \"" << EscapeJson(sub.name)
                << "\", \"status\": \"" << DiagnosticStatusName(sub.status)
                << "\", \"errors\": " << sub.errorCount
                << ", \"warnings\": " << sub.warningCount << "}";
            if (i + 1 < summary.subsystems.size()) oss << ",";
            oss << "\n";
        }
        oss << "  ]\n";
...
public:
    ...
    explicit DiagnosticCollector(size_t ringBufferCapacity = 256)
        : m_ringCapacity(ringBufferCapacity)
    {
        m_ringBuffer.resize(ringBufferCapacity);
    }

    /// Record an error in the ring buffer
    void RecordError(StructuredErrorDomain domain,
    ...
    void RecordDecoderOperation(const std::string& decoderName,
                                bool succeeded,
                                double elapsedMs) {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    size_t GetCapacity() const noexcept { return m_ringCapacity; }
