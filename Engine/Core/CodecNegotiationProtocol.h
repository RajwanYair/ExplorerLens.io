...
{
  public:
    void SetPreferences(const std::vector<CNPCodec>& codecs)
    {
        m_preferences = codecs;
    }

    CNPNegotiateResult Negotiate(const CNPOffer& peerOffer)
    {
        CNPNegotiateResult r;
        if (peerOffer.supportedCodecs.empty()) {
            r.errorMsg = "No codecs offered";
            return r;
        }
        // Pick best match from our preferences
        for (const auto& preferred : m_preferences) {
            auto it = std::find(peerOffer.supportedCodecs.begin(), peerOffer.supportedCodecs.end(), preferred);
    ...
    uint32_t PreferenceCount() const
    {
        return static_cast<uint32_t>(m_preferences.size());
    }
