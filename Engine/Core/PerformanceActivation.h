...
{
    return static_cast<SIMDCapability>(static_cast<uint32_t>(a) | static_cast<uint32_t>(b));
}
...
class PerformanceActivation
{
  public:
    ...
    PerformanceProfile DetectAndConfigure()
    {
        ...
        profile.simdScalerEnabled = HasCapability(profile.simdCaps, SIMDCapability::SSE41);
    ...
    std::string GetActivationReport() const
    {
        ...
        report += " Zero-Copy Pipeline: " + std::string(m_profile.zeroCopyEnabled ? "ON" : "OFF") + "\n";
        report += " Parallel I/O: " + std::string(m_profile.parallelIOEnabled ? "ON" : "OFF");
        report += " (" + std::to_string(m_profile.ioThreadCount) + " threads)\n";
        report += " SIMD Scaler: " + std::string(m_profile.simdScalerEnabled ? "ON" : "OFF") + "\n";
        report += " PSO Cache: " + std::string(m_profile.psoCacheEnabled ? "ON" : "OFF");
        report += " (" + std::to_string(m_profile.psoCacheMaxEntries) + " entries)\n";
        report += " Cache Warming: " + std::string(m_profile.cacheWarmingEnabled ? "ON" : "OFF");
        report += " (" + std::to_string(m_profile.warmingPrefetchCount) + " prefetch)\n";
    ...
    const PerformanceProfile& Profile() const
    {
        return m_profile;
    }
