...
enum class FileProviderType { LocalDisk, UNCShare, OneDrivePlaceholder,
                               SharePointVFS, WebDAV, Custom };
...
public:
    ...
    FileProviderInfo Probe(const std::string& path) const {
        ...
        info.uri = path;
        if (path.empty()) { info.online = false; return info; }
    ...
    FileReadResult Read(const std::string& path,
                         uint64_t offset = 0, uint64_t maxBytes = 0) {
        FileReadResult r;
        if (!m_ready || path.empty()) { r.errorCode = "INVALID_PATH"; return r; }
        (void)offset;
