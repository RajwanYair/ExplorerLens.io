  ...
  public:
    ...
    static bool ValidateConfig(const AsyncProviderConfig& cfg)
    {
        if (cfg.maxConcurrent == 0 || cfg.maxConcurrent > 16)
            return false;
        if (cfg.queueCapacity == 0 || cfg.queueCapacity > 4096)
            return false;
        if (cfg.defaultTimeoutMs < 1000 || cfg.defaultTimeoutMs > 30000)
            return false;
        return true;
    }
    ...
    static uint32_t EffectiveTimeout(const AsyncProviderConfig& cfg, DecodePriority priority)
    {
        switch (priority) {
            case DecodePriority::Critical:
                return cfg.criticalTimeoutMs;
            case DecodePriority::High:
                return cfg.defaultTimeoutMs;
            case DecodePriority::Normal:
                return cfg.defaultTimeoutMs;
            case DecodePriority::Low:
                return cfg.defaultTimeoutMs / 2;
            case DecodePriority::Idle:
                return cfg.defaultTimeoutMs / 4;
            default:
                return cfg.defaultTimeoutMs;
        }
    }
