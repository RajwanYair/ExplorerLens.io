...
{
  public:
    ClusterAutoScaler(uint32_t minNodes, uint32_t maxNodes) : m_min(minNodes), m_max(maxNodes) {}
    ...
    CASDecision Evaluate(const CASMetrics& metrics)
    {
        ...
        if (metrics.queueDepth > 20 || metrics.avgCpuPercent > 80.0f) {
            d.action = (metrics.activeNodes < m_max) ? CASScaleAction::ScaleUp : CASScaleAction::NoChange;
            d.targetNodes = std::min(m_max, metrics.activeNodes + 2u);
            d.reason = "High load";
        } else if (metrics.queueDepth < 2 && metrics.avgCpuPercent < 20.0f) {
            d.action = (metrics.activeNodes > m_min) ? CASScaleAction::ScaleDown : CASScaleAction::NoChange;
