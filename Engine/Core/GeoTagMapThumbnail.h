...
class GeoTagMapThumbnail
{
  public:
    ...
    bool HasCoordinates() const noexcept
    {
        return m_hasCoords;
    }
    ...
    static MercatorPoint GetMercatorProjection(double lat, double lon, uint32_t zoom) noexcept
    {
        MercatorPoint pt{};
        ...
        pt.y = (1.0 - std::log(std::tan(latRad) + 1.0 / std::cos(latRad)) / kPI) / 2.0 * n * 256.0;
        pt.valid = true;
        return pt;
    }

    // Return the slippy-map tile coordinate for a lat/lon at a zoom level.
    static TileCoord GetTileCoord(double lat, double lon, uint32_t zoom) noexcept
    {
        TileCoord tc{};
        if (!IsValidCoordinate(lat, lon))
            return tc;
        ...
        tc.tileY = static_cast<uint32_t>((1.0 - std::log(std::tan(latRad) + 1.0 / std::cos(latRad)) / kPI) / 2.0 * n);
    ...
    static double GetDistanceKm(double lat1, double lon1, double lat2, double lon2) noexcept
    {
