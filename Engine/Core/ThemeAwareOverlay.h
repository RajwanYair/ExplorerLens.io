...
class ThemeAwareOverlay
{
  public:
    ...
    void AddElement(const OverlayElement& elem)
    {
    ...
    OverlayRenderResult Render(uint32_t thumbWidth, uint32_t thumbHeight, bool isDarkBackground) const
    {
        (void)thumbWidth;
        (void)thumbHeight;
    ...
    static uint32_t OverlayBackgroundForTheme(bool isDark, float opacity)
    {
        ...
        if (isDark)
            return (0x30u << 24) | (0x30u << 16) | (0x30u << 8) | alpha;
        return (0xF0u << 24) | (0xF0u << 16) | (0xF0u << 8) | alpha;
    }

    static size_t PositionCount()
    {
        return static_cast<size_t>(ThemeOverlayPosition::COUNT);
    }
    static size_t StyleCount()
    {
        return static_cast<size_t>(OverlayStyle::COUNT);
    }

    static const wchar_t* PositionName(ThemeOverlayPosition p)
    {
        switch (p) {
            case ThemeOverlayPosition::TopLeft:
                return L"TopLeft";
            case ThemeOverlayPosition::TopRight:
                return L"TopRight";
            case ThemeOverlayPosition::BottomLeft:
                return L"BottomLeft";
            case ThemeOverlayPosition::BottomRight:
                return L"BottomRight";
            case ThemeOverlayPosition::TopCenter:
                return L"TopCenter";
            case ThemeOverlayPosition::BottomCenter:
