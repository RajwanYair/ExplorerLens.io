...
public:
 ...
 switch (t) {
 case GPUHeapType::Default: return L"Default (VRAM)";
 case GPUHeapType::Upload: return L"Upload (CPU→GPU)";
 case GPUHeapType::Readback: return L"Readback (GPU→CPU)";
 case GPUHeapType::Custom: return L"Custom (UMA)";
 ...
 switch (p) {
 case GPUResidencyPriority::Low: return L"Low";
 case GPUResidencyPriority::Normal: return L"Normal";
 case GPUResidencyPriority::High: return L"High";
 case GPUResidencyPriority::Critical: return L"Critical";
 default: return L"Unknown";
 }
 }
 ...
 switch (s) {
 case GPUAllocStrategy::BestFit: return L"Best Fit";
 case GPUAllocStrategy::FirstFit: return L"First Fit";
 case GPUAllocStrategy::PoolSlab: return L"Pool Slab";
 default: return L"Unknown";
 }
 }

 static constexpr size_t HeapTypeCount() { return static_cast<size_t>(GPUHeapType::COUNT); }
 static constexpr size_t ResidencyCount() { return static_cast<size_t>(GPUResidencyPriority::COUNT); }
 static constexpr size_t AllocStrategyCount() { return static_cast<size_t>(GPUAllocStrategy::COUNT); }

 // Default VRAM budget: 512 MB
 static constexpr uint64_t DefaultBudgetBytes() { return 512ULL * 1024 * 1024; }
 ...
 if (s.budgetBytes == 0) return 0.0f;
