...
struct FormatDetectionReport {
    ...
    static DetectionVerdict VerdictFromConfidence(float c) {
        if (c >= 0.90f) return DetectionVerdict::Confirmed;
        if (c >= 0.70f) return DetectionVerdict::Probable;
...
public:
    ...
    void AddVote(DetectionSource source, const std::string& formatId,
                 const std::string& mimeType, float confidence) {
        DetectionVote v;
        v.source     = source;
        v.formatId   = formatId;
        v.mimeType   = mimeType;
