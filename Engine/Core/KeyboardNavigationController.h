...
enum class NavKey {
    ...
    Enter, Space, Escape, Tab, ShiftTab
};
...
public:
    ...
    void SetTotalItems(uint32_t n) { m_state.totalItems = n; }
    void SetColumns(uint32_t c)    { m_state.columnsPerRow = c; }
    void SetFocus(uint32_t idx)    { m_state.focusedIndex = idx; }
    const NavState& GetState()     const { return m_state; }

    void SetActionCallback(NavActionCallback cb) { m_callback = std::move(cb); }
