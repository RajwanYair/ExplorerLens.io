...
{
    switch (type) {
        case BenchmarkType::SingleDecode:
            return L"Single Decode";
        case BenchmarkType::BatchDecode:
            return L"Batch Decode";
        case BenchmarkType::CacheHit:
            return L"Cache Hit";
        case BenchmarkType::GPURender:
            return L"GPU Render";
        case BenchmarkType::FormatConvert:
            return L"Format Convert";
        case BenchmarkType::EndToEnd:
            return L"End to End";
        default:
            return L"Unknown";
    }
...
BenchmarkResult PerformanceBenchmarkV2::ComputeStats(const std::wstring& label, BenchmarkType type,
                                                     const std::vector<double>& samplesMs)
{
    ...
    result.medianMs = (sorted.size() % 2 == 0) ? (sorted[mid - 1] + sorted[mid]) / 2.0 : sorted[mid];
