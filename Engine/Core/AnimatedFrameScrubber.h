...
public:
    explicit AnimatedFrameScrubber(uint32_t fpsCap = 30)
        : m_fpsCap(fpsCap) {}
    ...
    bool DecodeFrame(uint32_t index, std::vector<uint8_t>& outputRGBA) const {
        if (!m_isLoaded || index >= m_frames.size()) return false;
        const auto& f = m_frames[index];
        outputRGBA.resize(static_cast<size_t>(f.width) * f.height * 4);
        if (m_frameCallback) m_frameCallback(index, outputRGBA.data(), f.width * 4);
        return true;
    }
    ...
    uint32_t GetFrameAtProgress(float progress) const {
        ...
        for (uint32_t i = 0; i < m_frames.size(); ++i) {
            ...
            if (accum >= targetMs) return i;
        }
        return static_cast<uint32_t>(m_frames.size() - 1);
    }

    void SetFpsCap(uint32_t fps) { m_fpsCap = (fps > 0) ? fps : 1; }
    uint32_t GetFpsCap() const { return m_fpsCap; }

    uint64_t GetTotalDurationMs() const {
        if (m_metadata.totalDurationMs > 0) return m_metadata.totalDurationMs;
    ...
    uint32_t GetEffectiveFps() const {
        ...
        if (dur == 0 || m_frames.empty()) return 0;
        uint32_t natural = static_cast<uint32_t>((m_frames.size() * 1000ULL) / dur);
        return (natural < m_fpsCap) ? natural : m_fpsCap;
    }

    const AnimationMetadata& GetMetadata() const { return m_metadata; }
    const std::vector<FrameInfo>& GetFrames() const { return m_frames; }
    void SetFrameCallback(FrameDecodedCallback cb) { m_frameCallback = std::move(cb); }
    void AddFrame(const FrameInfo& frame) { m_frames.push_back(frame); }
