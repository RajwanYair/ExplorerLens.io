...
public:
  ...
  SecurityAuditResult
  RunAudit(SecurityLevel requiredLevel = SecurityLevel::Standard);
  SecurityCheckResult RunCheck(IntegrityCheck check) const;

  bool VerifyCodeSignature(const std::wstring &filePath) const;
  bool VerifyFileHash(const std::wstring &filePath,
  ...
  static uint32_t GetCheckCount() {
    return static_cast<uint32_t>(IntegrityCheck::CheckCount);
  }
  static uint32_t GetLevelCount() {
    return static_cast<uint32_t>(SecurityLevel::LevelCount);
  }
