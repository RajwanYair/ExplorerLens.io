...
enum class AnnotationOpType { Set, Delete, AddTag, RemoveTag, SetStar, SetColor };
...
public:
    ...
    void SetDeltaReceiver(DeltaReceiver fn) { m_receiver = std::move(fn); }
    void SetPeerCount(int count) noexcept { m_peerCount = count; }
    ...
    BroadcastResult Broadcast(const AnnotationDelta& delta) {
        ...
        if (m_receiver) m_receiver(delta);
    ...
    uint64_t LamportClock() const noexcept { return m_clock; }
    int HistorySize() const noexcept { return static_cast<int>(m_history.size()); }

    std::vector<AnnotationDelta> GetHistory() const { return m_history; }

    static std::string OpName(AnnotationOpType op) noexcept {
        switch (op) {
        case AnnotationOpType::Set:        return "Set";
        case AnnotationOpType::Delete:     return "Delete";
        case AnnotationOpType::AddTag:     return "AddTag";
        case AnnotationOpType::RemoveTag:  return "RemoveTag";
        case AnnotationOpType::SetStar:    return "SetStar";
        case AnnotationOpType::SetColor:   return "SetColor";
        }
        return "Unknown";
    }
