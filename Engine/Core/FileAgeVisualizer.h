...
public:
    ...
    static AgeCategory GetAgeCategory(FILETIME fileTimeUtc) noexcept {
        ...
        if (ageDays < 1)   return AgeCategory::Fresh;
        if (ageDays < 7)   return AgeCategory::Recent;
        if (ageDays < 90)  return AgeCategory::Old;
        if (ageDays < 365) return AgeCategory::Stale;
        return AgeCategory::Archive;
    }
    ...
    static AgeColor GetAgeColorGradient(int64_t ageMs) noexcept {
        ...
        const double ageDays = static_cast<double>(ageMs) /
    ...
    static const wchar_t* GetAgeLabelString(AgeCategory category) noexcept {
        switch (category) {
        case AgeCategory::Fresh:   return L"Fresh";
        case AgeCategory::Recent:  return L"Recent";
        case AgeCategory::Old:     return L"Old";
        case AgeCategory::Stale:   return L"Stale";
        case AgeCategory::Archive: return L"Archive";
        }
        return L"Unknown";
    }
    ...
    static double ComputeFreshnessScore(FILETIME createdTime,
        FILETIME modifiedTime) noexcept {
        FILETIME nowFt{};
        ::GetSystemTimeAsFileTime(&nowFt);
        ...
        const double creationAgeDays =
            static_cast<double>((std::max)(nowMs - cMs, 0LL)) /
            static_cast<double>(kMsPerDay);

        // Recency of last edit (days ago)
        const double modAgeDays =
            static_cast<double>((std::max)(nowMs - mMs, 0LL)) /
            static_cast<double>(kMsPerDay);
