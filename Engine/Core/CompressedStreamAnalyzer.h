...
{
  public:
    CompressedStreamAnalyzer() : m_compressedSize(0), m_originalSize(0) {}
    ...
    {
        if (m_compressedSize == 0 || m_originalSize == 0)
            return 1.0;
    ...
    double GetCompressionEfficiency() const
    {
        if (m_originalSize == 0)
            return 0.0;
        if (m_compressedSize >= m_originalSize)
            return 0.0;
    ...
    CompressionClass ClassifyCompression() const
    {
        ...
        if (savings <= 0.0)
            return CompressionClass::None;
        if (savings < 10.0)
            return CompressionClass::Minimal;
        if (savings < 30.0)
            return CompressionClass::Low;
        if (savings < 60.0)
            return CompressionClass::Moderate;
        if (savings < 80.0)
            return CompressionClass::High;
    ...
    {
        switch (cls) {
            case CompressionClass::None:
                return "None";
            case CompressionClass::Minimal:
                return "Minimal";
            case CompressionClass::Low:
                return "Low";
            case CompressionClass::Moderate:
                return "Moderate";
            case CompressionClass::High:
                return "High";
            case CompressionClass::Extreme:
                return "Extreme";
            default:
                return "Unknown";
        }
