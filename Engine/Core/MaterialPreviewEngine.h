...
enum class MaterialPreviewShape  { Sphere, Cube, Plane, CornellBox };
enum class MaterialFileFormat    { MTL, MaterialX, MDL, Substance, Unknown };
...
public:
    ...
    static MaterialFileFormat DetectFormat(const std::wstring& path) noexcept {
        if (path.size() > 4) {
            ...
            if (ext == L".mtl") return MaterialFileFormat::MTL;
            if (ext == L".mdl") return MaterialFileFormat::MDL;
        }
        if (path.size() > 6) {
            auto ext = path.substr(path.size() - 6);
            if (ext == L".mtlx") return MaterialFileFormat::MaterialX;
        }
    ...
    static std::string FormatName(MaterialFileFormat fmt) noexcept {
        switch (fmt) {
        case MaterialFileFormat::MTL:        return "MTL";
        case MaterialFileFormat::MaterialX:  return "MaterialX";
        case MaterialFileFormat::MDL:        return "MDL";
        case MaterialFileFormat::Substance:  return "Substance";
        case MaterialFileFormat::Unknown:    return "Unknown";
        }
        return "Unknown";
    }
};
