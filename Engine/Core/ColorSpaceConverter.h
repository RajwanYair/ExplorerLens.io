...
enum class CSCColorSpace : uint8_t {
    sRGB, AdobeRGB, DisplayP3, ProPhotoRGB, Rec2020, Rec709, CIEXYZ, COUNT
};

enum class RenderIntent : uint8_t {
    Perceptual, RelativeColorimetric, Saturation, AbsoluteColorimetric, COUNT
};
...
public:
    void Configure(const ConversionConfig& cfg) { m_config = cfg; }
    const ConversionConfig& GetConfig() const { return m_config; }

    ColorTriple Convert(const ColorTriple& input) const {
        if (m_config.source == m_config.destination) return input;
        ...
        if (m_config.clampOutput) Clamp(result);
        return result;
    }
    ...
    float GamutCoverage(CSCColorSpace wider, CSCColorSpace narrower) const {
        ...
        if (wider == narrower) return 1.0f;
        if (wider == CSCColorSpace::Rec2020 && narrower == CSCColorSpace::sRGB) return 0.41f;
        if (wider == CSCColorSpace::DisplayP3 && narrower == CSCColorSpace::sRGB) return 0.79f;
        if (wider == CSCColorSpace::AdobeRGB && narrower == CSCColorSpace::sRGB) return 0.68f;
        return 0.5f;
    }

    static size_t SpaceCount() { return static_cast<size_t>(CSCColorSpace::COUNT); }
    static size_t IntentCount() { return static_cast<size_t>(RenderIntent::COUNT); }
...
private:
    ...
    ColorTriple ToXYZ(ColorTriple c, CSCColorSpace) const {
        return { c.r * 0.4124f + c.g * 0.3576f + c.b * 0.1805f,
    ...
    ColorTriple FromXYZ(ColorTriple c, CSCColorSpace) const {
        return { c.r * 3.2406f + c.g * -1.5372f + c.b * -0.4986f,
                 c.r * -0.9689f + c.g * 1.8758f + c.b * 0.0415f,
                 c.r * 0.0557f + c.g * -0.2040f + c.b * 1.0570f };
    }
    ColorTriple ApplyGamma(ColorTriple c, CSCColorSpace) const {
        auto g = [](float v) { return (v > 0.0031308f) ? (1.055f * std::pow(v, 1.0f / 2.4f) - 0.055f) : v * 12.92f; };
        return { g(c.r), g(c.g), g(c.b) };
    }
    static void Clamp(ColorTriple& c) {
        auto cl = [](float& v) { v = (v < 0) ? 0.0f : ((v > 1.0f) ? 1.0f : v); };
        cl(c.r); cl(c.g); cl(c.b);
    }
