...
public:
    ...
    void SetParams(const ToneMappingParams& p) { m_params = p; }
    const ToneMappingParams& GetParams() const { return m_params; }

    BitDepthConversionResult Convert(
        const void*   sourceData,
        uint32_t      width,
        uint32_t      height,
        BitDepthSource srcFormat,
        uint8_t*       outputBGRA)
    {
        ...
        if (srcFormat == BitDepthSource::SDR_8bit) {
            ...
            for (uint32_t i = 0; i < pixels * 4; ++i) outputBGRA[i] = src[i];
            r.success         = true;
        ...
        for (uint32_t i = 0; i < pixels; ++i) {
            ...
            if (lum * m_params.peakNits > maxNits) maxNits = lum * m_params.peakNits;

            r_f = ToneMap(r_f); g_f = ToneMap(g_f); b_f = ToneMap(b_f);
    ...
    static BitDepthSource DetectSourceFormat(uint32_t bitsPerChannel, bool isPQ, bool isHLG) {
        if (bitsPerChannel == 8)  return BitDepthSource::SDR_8bit;
        if (bitsPerChannel == 16) return BitDepthSource::HDR_16bit_FP;
        if (bitsPerChannel == 10) return isPQ ? BitDepthSource::HDR_10bit_PQ
                                              : (isHLG ? BitDepthSource::HDR_10bit_HLG
                                                       : BitDepthSource::SDR_8bit);
        if (bitsPerChannel == 12) return BitDepthSource::WCG_12bit;
        return BitDepthSource::SDR_8bit;
    }

    static const char* SourceFormatName(BitDepthSource s) {
        switch (s) {
        case BitDepthSource::SDR_8bit:     return "SDR-8bit";
        case BitDepthSource::HDR_16bit_FP: return "HDR-16bit-FP";
        case BitDepthSource::HDR_10bit_PQ: return "HDR-10bit-PQ";
        case BitDepthSource::HDR_10bit_HLG:return "HDR-10bit-HLG";
        case BitDepthSource::WCG_12bit:    return "WCG-12bit";
        case BitDepthSource::RAW_Linear:   return "RAW-Linear";
    ...
    static const char* ToneMappingName(ToneMappingOperator op) {
        ...
        case ToneMappingOperator::HejlDawson:       return "Hejl-Dawson";
        case ToneMappingOperator::Uncharted2:       return "Uncharted2";
        case ToneMappingOperator::Clamp:            return "Clamp";
        default:                                    return "Unknown";
        }
    }
