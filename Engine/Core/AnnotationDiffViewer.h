...
struct AnnotationDiffResult
{
    ...
    {
        return additions == 0 && removals == 0 && changes == 0;
    }
};
...
class AnnotationDiffViewer
{
  public:
    ...
    AnnotationDiffResult Diff(const AnnotationSnapshot& before, const AnnotationSnapshot& after,
                              const std::wstring& author = {}) const
    {
        AnnotationDiffResult result;
        if (before.starRating != after.starRating) {
            result.entries.push_back({DiffOperation::Modify, L"starRating", std::to_wstring(before.starRating),
                                      std::to_wstring(after.starRating), author});
            result.changes++;
        }
        if (before.comment != after.comment) {
            result.entries.push_back({DiffOperation::Modify, L"comment", before.comment, after.comment, author});
            result.changes++;
        }
        if (before.colorLabel != after.colorLabel) {
            result.entries.push_back({DiffOperation::Modify, L"colorLabel", std::to_wstring(before.colorLabel),
                                      std::to_wstring(after.colorLabel), author});
            result.changes++;
        }
        // Tags diff
        for (const auto& t : after.tags) {
            bool found = false;
            for (const auto& bt : before.tags)
                if (bt == t) {
                    found = true;
                    break;
                }
            if (!found) {
                result.entries.push_back({DiffOperation::Add, L"tag", {}, t, author});
                result.additions++;
            }
        }
        for (const auto& t : before.tags) {
            bool found = false;
            for (const auto& at : after.tags)
                if (at == t) {
                    found = true;
                    break;
                }
            if (!found) {
                result.entries.push_back({DiffOperation::Remove, L"tag", t, {}, author});
                result.removals++;
            }
        }
        return result;
