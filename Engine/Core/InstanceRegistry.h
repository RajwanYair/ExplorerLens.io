...
public:
    ...
    void Unregister(InstanceId id)        { m_records.erase(id); }

    int  PruneStale(int thresholdMs = 5000) {
        int pruned = 0;
        for (auto it = m_records.begin(); it != m_records.end(); ) {
            if (it->second.IsStale(thresholdMs)) { it = m_records.erase(it); pruned++; }
            else ++it;
        }
        return pruned;
    }
    ...
    std::vector<InstanceRecord> AllInstances() const {
        ...
        for (const auto& kv : m_records) v.push_back(kv.second);
        return v;
    }

    int  Count()     const noexcept { return (int)m_records.size(); }
