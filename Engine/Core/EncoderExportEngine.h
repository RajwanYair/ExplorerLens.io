...
class EncoderExportEngine
{
  public:
    ...
    // Export operations
    EncoderExportResult ExportThumbnail(const uint8_t* rgbaData, uint32_t width, uint32_t height,
                                        const ExportConfig& config, const std::wstring& outputPath);
    EncoderExportResult ExportToMemory(const uint8_t* rgbaData, uint32_t width, uint32_t height,
                                       const ExportConfig& config, std::vector<uint8_t>& outputBuffer);

    // Batch export
    std::vector<EncoderExportResult> BatchExport(const uint8_t* rgbaData, uint32_t width, uint32_t height,
  ...
  private:
    bool WritePNG(const uint8_t* data, uint32_t w, uint32_t h, const ExportConfig& config, std::vector<uint8_t>& output);
    bool WriteBMP(const uint8_t* data, uint32_t w, uint32_t h, const ExportConfig& config, std::vector<uint8_t>& output);
};
