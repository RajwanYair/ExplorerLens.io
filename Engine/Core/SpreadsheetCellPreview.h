...
public:
    ...
    bool CalculateCellLayout(int canvasW, int canvasH) {
        ...
        int totalUnits = 0;
        for (int w : m_colWidths) totalUnits += w;
        if (totalUnits <= 0) totalUnits = m_cols;
        ...
        for (int c = 0; c < m_cols; ++c) {
            m_scaledColWidths[c] = (std::max)(1,
                (m_colWidths[c] * usableW) / totalUnits);
        }
    ...
    int GetVisibleCellCount() const {
        if (!m_layoutComputed || m_rowHeight <= 0)
            return 0;
        const int visibleRows = (std::min)(m_rows,
            (std::max)(1, (m_canvasH - m_headerHeight) / m_rowHeight));
    ...
    bool IsLayoutComputed() const { return m_layoutComputed; }
