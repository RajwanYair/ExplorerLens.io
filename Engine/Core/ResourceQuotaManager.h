...
inline const char* QuotaResourceName(QuotaResource r) noexcept {
    switch (r) {
    case QuotaResource::Memory:    return "Memory";
    case QuotaResource::CPU:       return "CPU";
    case QuotaResource::DiskRead:  return "DiskRead";
    case QuotaResource::DiskWrite: return "DiskWrite";
    case QuotaResource::GPUMemory: return "GPUMemory";
...
inline const char* QuotaEnforcementName(QuotaEnforcement e) noexcept {
    switch (e) {
    case QuotaEnforcement::SoftLimit: return "SoftLimit";
    case QuotaEnforcement::HardLimit: return "HardLimit";
    case QuotaEnforcement::Warning:   return "Warning";
    case QuotaEnforcement::Throttle:  return "Throttle";
    case QuotaEnforcement::Deny:      return "Deny";
    default:                          return "Unknown";
    }
}
...
public:
    ...
    void SetQuota(QuotaResource resource, QuotaEnforcement enforcement,
        uint64_t limitValue, const std::string& unit = "bytes") {
        for (auto& q : m_quotas) {
    ...
    uint64_t GetUsage(QuotaResource resource) const noexcept {
        for (const auto& q : m_quotas) {
            if (q.resource == resource) return q.currentUsage;
        }
        return 0;
    }
    ...
    bool IsOverQuota() const noexcept {
        for (const auto& q : m_quotas) {
            if (q.currentUsage > q.limitValue) return true;
        }
        return false;
    }

    /// Get all configured quotas
    const std::vector<ResourceQuota>& GetQuotas() const noexcept { return m_quotas; }

    /// Reset all usage counters to zero
    void ResetUsage() noexcept {
        for (auto& q : m_quotas) q.currentUsage = 0;
    }
