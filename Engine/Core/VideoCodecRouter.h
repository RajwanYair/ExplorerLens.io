...
class VideoCodecRouter
{
  public:
    ...
    VideoRouteDecision Route(const wchar_t* filePath) const
    {
        ...
        if (ext == L".mp4" || ext == L".m4v" || ext == L".mov" || ext == L".wmv" || ext == L".avi") {
        ...
        else if (ext == L".mkv" || ext == L".webm" || ext == L".flv" || ext == L".ogv" || ext == L".ogg"
                 || ext == L".rmvb" || ext == L".rm" || ext == L".ts" || ext == L".mts") {
    ...
    std::vector<std::wstring> GetExtensionsForBackend(VideoBackend backend) const
    {
        ...
        switch (backend) {
            ...
            case VideoBackend::FFmpeg:
                exts = {L".mkv", L".webm", L".flv", L".ogv", L".ogg", L".rmvb", L".rm", L".ts", L".mts"};
                break;
    ...
    void SetGPUDecodeAvailable(bool available)
    {
        m_gpuDecodeAvailable = available;
    }
    bool IsGPUDecodeAvailable() const
    {
        return m_gpuDecodeAvailable;
    }
