...
public:
    ...
    ThumbResolution SelectResolution(uint64_t monitorHandle) const {
        ...
        const double factor = static_cast<double>(
            static_cast<uint32_t>(it->second.scalePercent)) / 100.0;
    ...
    bool IsMixedDPI() const noexcept {
        if (m_monitors.size() < 2) return false;
        uint32_t firstDpi = 0;
        for (const auto& [_, p] : m_monitors) {
            if (firstDpi == 0) { firstDpi = p.dpiX; continue; }
            if (p.dpiX != firstDpi) return true;
        }
        return false;
    }
