...
class SecureStringPool
{
  public:
    ...
    bool Store(const std::wstring& keyName, const std::wstring& secret)
    {
        ...
        SecureSlot slot{};
        slot.page = static_cast<uint8_t*>(VirtualAlloc(nullptr, pageSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
        if (!slot.page)
            return false;
        ...
        if (encLen <= pageSize) {
            if (CryptProtectMemory(slot.page, static_cast<DWORD>(encLen), CRYPT_PROTECT_MEMORY_SAME_PROCESS))
                slot.encrypted = true;
        }
    ...
    bool Retrieve(const std::wstring& keyName, std::wstring& outSecret)
    {
        ...
        if (slot.encrypted) {
            ...
            CryptUnprotectMemory(slot.page, static_cast<DWORD>(encLen), CRYPT_PROTECT_MEMORY_SAME_PROCESS);
        }
        ...
        if (slot.encrypted) {
            DWORD blockSize = 16;
            size_t encLen = (slot.dataLen + blockSize - 1) & ~(static_cast<size_t>(blockSize) - 1);
            CryptProtectMemory(slot.page, static_cast<DWORD>(encLen), CRYPT_PROTECT_MEMORY_SAME_PROCESS);
        }
        return true;
    }
