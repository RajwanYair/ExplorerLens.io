...
public:
    ...
    // ---------------------------------------------------------------
    bool AddActivity(uint32_t year, uint32_t month, uint32_t day,
    ...
    // ---------------------------------------------------------------
    HeatmapCell GetCellRect(uint32_t dayOfYear, uint32_t canvasW,
        uint32_t canvasH) const noexcept {
        if (dayOfYear == 0 || dayOfYear > 366) return {};
        ...
        return {
            static_cast<int32_t>(weekCol * cellW),
            static_cast<int32_t>(dayRow * cellH),
            effectiveW,
            effectiveH
        };
    }
    ...
    static HeatLevel GetHeatLevel(uint32_t count, uint32_t maxCount) noexcept {
        if (maxCount == 0 || count == 0) return HeatLevel::None;
        if (count >= maxCount) return HeatLevel::Max;

        // Quartile-based thresholds
        const uint32_t quarter = maxCount / 4;
        if (quarter == 0) return (count > 0) ? HeatLevel::Max : HeatLevel::None;

        if (count <= quarter)          return HeatLevel::Low;
        if (count <= quarter * 2)      return HeatLevel::Medium;
        if (count <= quarter * 3)      return HeatLevel::High;
    ...
    // ---------------------------------------------------------------
    static uint32_t GetDayOfWeek(uint32_t year, uint32_t month,
        uint32_t day) noexcept {
        if (month < 1 || month > 12 || day < 1 || year == 0) return 0;
    ...
    uint32_t GetActivityCount(uint32_t dayOfYear) const noexcept {
        if (dayOfYear == 0 || dayOfYear > 366) return 0;
