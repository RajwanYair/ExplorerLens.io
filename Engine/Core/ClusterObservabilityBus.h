...
enum class COBSpanStatus { Ok, Error, Timeout };
...
public:
    ...
    COBMetricSnapshot Snapshot() const {
        ...
        for (const auto& sp : m_spans) {
            ...
            if (sp.status != COBSpanStatus::Ok) ++s.errorSpans;
        }
        s.avgDurationMs = (s.totalSpans > 0)
            ? totalDur / static_cast<float>(s.totalSpans)
            : 0.0f;
        return s;
    }
