...
public:
    void SetLayout(PreviewLayout layout) { m_layout = layout; }
    PreviewLayout GetLayout() const { return m_layout; }

    void SetMaxPages(uint32_t n) { m_maxPages = n; }
    uint32_t GetMaxPages() const { return m_maxPages; }

    void AddPage(const MultiPageInfo& page) { m_pages.push_back(page); }
    ...
    static PreviewLayout SuggestLayout(uint32_t totalPages) {
        if (totalPages <= 1) return PreviewLayout::SinglePage;
        if (totalPages <= 3) return PreviewLayout::FanStack;
        return PreviewLayout::Grid2x2;
    }

    static const wchar_t* LayoutName(PreviewLayout l) {
        switch (l) {
        case PreviewLayout::SinglePage:      return L"SinglePage";
        case PreviewLayout::Grid2x2:         return L"Grid2x2";
        case PreviewLayout::FanStack:        return L"FanStack";
        case PreviewLayout::StripHorizontal: return L"StripHorizontal";
        case PreviewLayout::StripVertical:   return L"StripVertical";
        default: return L"Unknown";
        }
    }
    static size_t LayoutCount() { return static_cast<size_t>(PreviewLayout::COUNT); }
