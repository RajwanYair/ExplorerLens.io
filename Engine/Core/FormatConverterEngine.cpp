...
BatchConversionResult FormatConverterEngine::ConvertBatch(
...
ConvertFormat FormatConverterEngine::DetectFormat(const std::wstring& filePath) {
    ...
    if (dot == std::wstring::npos) return ConvertFormat::PNG;

    auto ext = filePath.substr(dot + 1);
    for (auto& c : ext) c = towlower(c);

    if (ext == L"png") return ConvertFormat::PNG;
    if (ext == L"jpg" || ext == L"jpeg") return ConvertFormat::JPEG;
    if (ext == L"webp") return ConvertFormat::WebP;
    if (ext == L"bmp") return ConvertFormat::BMP;
    if (ext == L"tiff" || ext == L"tif") return ConvertFormat::TIFF;
    if (ext == L"avif") return ConvertFormat::AVIF;
    if (ext == L"jxl") return ConvertFormat::JXL;
    return ConvertFormat::PNG;
}

const wchar_t* FormatConverterEngine::GetFormatName(ConvertFormat format) {
    switch (format) {
        case ConvertFormat::PNG:  return L"PNG";
        case ConvertFormat::JPEG: return L"JPEG";
        case ConvertFormat::WebP: return L"WebP";
        case ConvertFormat::BMP:  return L"BMP";
        case ConvertFormat::TIFF: return L"TIFF";
        case ConvertFormat::AVIF: return L"AVIF";
        case ConvertFormat::JXL:  return L"JPEG XL";
...
const wchar_t* FormatConverterEngine::GetFormatExtension(ConvertFormat format) {
    switch (format) {
        case ConvertFormat::PNG:  return L".png";
        case ConvertFormat::JPEG: return L".jpg";
        case ConvertFormat::WebP: return L".webp";
        case ConvertFormat::BMP:  return L".bmp";
        case ConvertFormat::TIFF: return L".tif";
        case ConvertFormat::AVIF: return L".avif";
        case ConvertFormat::JXL:  return L".jxl";
...
const wchar_t* FormatConverterEngine::GetPresetName(QualityPreset preset) {
    switch (preset) {
        ...
        case QualityPreset::Thumbnail: return L"Thumbnail";
        default: return L"Unknown";
    }
}
...
uint32_t FormatConverterEngine::GetQualityValue(QualityPreset preset) {
    switch (preset) {
        ...
        case QualityPreset::High:      return 90;
        case QualityPreset::Medium:    return 75;
        case QualityPreset::Low:       return 50;
        case QualityPreset::Thumbnail: return 60;
        default: return 75;
    }
}
