...
public:
    ...
    VideoStreamInfo GetStreamInfo(const wchar_t* filePath) {
        ...
        if (!m_loaded || !filePath) return info;
    ...
    VideoFrameResult ExtractThumbnailFrame(const wchar_t* filePath,
    ...
    VideoFrameResult ExtractFrameAt(const wchar_t* filePath,
                                     double timestampSec,
                                     uint32_t cx, uint32_t cy) {
        VideoFrameResult result;
        if (!m_loaded) {
        ...
        double pos = (info.durationSec > 0) ?
            timestampSec / info.durationSec : 0.0;
    ...
    HBITMAP GenerateThumbnail(const wchar_t* filePath, uint32_t cx, uint32_t cy) {
        ...
        if (!result.success || result.pixelData.empty()) return nullptr;
    ...
    static ContainerFormat DetectFormatFromExtension(const wchar_t* filePath) {
        if (!filePath) return ContainerFormat::Unknown;
        std::wstring path(filePath);
        auto dot = path.rfind(L'.');
        if (dot == std::wstring::npos) return ContainerFormat::Unknown;
        std::wstring ext = path.substr(dot);
        for (auto& c : ext) c = static_cast<wchar_t>(towlower(c));

        if (ext == L".mkv")  return ContainerFormat::MKV;
        if (ext == L".webm") return ContainerFormat::WebM;
        if (ext == L".mp4" || ext == L".m4v") return ContainerFormat::MP4;
        if (ext == L".avi")  return ContainerFormat::AVI;
        if (ext == L".flv")  return ContainerFormat::FLV;
        if (ext == L".ts" || ext == L".mts") return ContainerFormat::MPEG_TS;
        if (ext == L".ogv" || ext == L".ogg") return ContainerFormat::OGV;
        if (ext == L".rmvb" || ext == L".rm") return ContainerFormat::RMVB;
        if (ext == L".mov")  return ContainerFormat::MOV;
        if (ext == L".wmv" || ext == L".asf") return ContainerFormat::WMV_ASF;
        if (ext == L".3gp")  return ContainerFormat::GP3;
    ...
    static const char* CodecFamilyName(FFmpegCodecFamily c) {
        switch (c) {
            case FFmpegCodecFamily::Unknown:     return "Unknown";
            case FFmpegCodecFamily::H264:        return "H.264";
            case FFmpegCodecFamily::H265:        return "H.265/HEVC";
            case FFmpegCodecFamily::VP8:         return "VP8";
            case FFmpegCodecFamily::VP9:         return "VP9";
            case FFmpegCodecFamily::AV1:         return "AV1";
            case FFmpegCodecFamily::MPEG4:       return "MPEG-4";
            case FFmpegCodecFamily::MPEG2:       return "MPEG-2";
            case FFmpegCodecFamily::WMV:         return "WMV";
            case FFmpegCodecFamily::Theora:      return "Theora";
            case FFmpegCodecFamily::ProRes:      return "ProRes";
            case FFmpegCodecFamily::DNxHD:       return "DNxHD";
            case FFmpegCodecFamily::FFV1:        return "FFV1";
            case FFmpegCodecFamily::MotionJPEG:  return "Motion JPEG";
            case FFmpegCodecFamily::RealVideo:   return "RealVideo";
            default:                             return "?";
        }
    }
    ...
    static const char* ContainerName(ContainerFormat f) {
        switch (f) {
            ...
            case ContainerFormat::FLV:       return "Flash Video";
            case ContainerFormat::MPEG_TS:   return "MPEG-TS";
            case ContainerFormat::MPEG_PS:   return "MPEG-PS";
            case ContainerFormat::OGV:       return "Ogg Video";
            case ContainerFormat::RMVB:      return "RealMedia";
            case ContainerFormat::MOV:       return "QuickTime";
            case ContainerFormat::WMV_ASF:   return "WMV/ASF";
            case ContainerFormat::GP3:       return "3GP";
            default:                         return "?";
        }
    }

    static constexpr uint32_t GetCodecFamilyCount() { return 15; }
    static constexpr uint32_t GetContainerFormatCount() { return 13; }

private:
    FFmpegIntegration() { m_loaded = TryLoadDLLs(); }
    ~FFmpegIntegration() { UnloadDLLs(); }
    ...
    VideoFrameResult ExtractFrameInternal(const wchar_t* filePath,
                                           uint32_t cx, uint32_t cy, double pos) {
        VideoFrameResult result;
        (void)filePath; (void)cx; (void)cy; (void)pos;
        return result;
    }
    ...
    HBITMAP CreateBitmapFromPixels(const uint8_t* pixels, uint32_t w, uint32_t h) {
        ...
        if (hbmp && bits) memcpy(bits, pixels, static_cast<size_t>(w) * h * 4);
        return hbmp;
    }
