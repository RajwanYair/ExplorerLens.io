...
{
  public:
    static OpenJPEGRenderer& Instance()
    {
        static OpenJPEGRenderer s;
        return s;
    }
    ...
    {
        J2KImageInfo info{};
        if (!m_initialized || !data || dataSize < 12)
            return info;
    ...
    const J2KDecodeConfig& GetConfig() const
    {
        return m_config;
    }
    ...
    bool Validate() const
    {
        if (m_config.maxWidth == 0 || m_config.maxHeight == 0)
            return false;
        if (m_config.numThreads == 0 || m_config.numThreads > 16)
            return false;
        if (m_config.maxWidth > 32768 || m_config.maxHeight > 32768)
            return false;
        return true;
    }
