...
public:
    ...
    DSBackend GetActiveBackend() const {
        if (m_caps.directStorage12Available) return DSBackend::DirectStorage12;
        return DSBackend::CPUFallback;
    }

    const DSCapabilities& GetCapabilities() const { return m_caps; }

    int GetQueueDepth() const { return m_queueDepth.load(); }

    void SetPriorityBoost(bool enable) { m_priorityBoost = enable; }

    static bool IsSupportedCompressionFormat(DSCompressionFormat fmt) {
        return fmt == DSCompressionFormat::GDeflate ||
               fmt == DSCompressionFormat::ZStandard ||
               fmt == DSCompressionFormat::LZ4;
    }

    static const char* BackendName(DSBackend b) {
        switch (b) {
        case DSBackend::DirectStorage12: return "DirectStorage-1.2";
        case DSBackend::DirectStorage10: return "DirectStorage-1.0";
        case DSBackend::CPUFallback:     return "CPU-Fallback";
