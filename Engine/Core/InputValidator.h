    ...
    const wchar_t* ErrorMessage() const
    {
        switch (error) {
            case ValidationError::Ok:
                return L"OK";
            case ValidationError::PathTraversal:
                return L"Path traversal detected";
            case ValidationError::UNCPath:
                return L"UNC paths not allowed";
            case ValidationError::DevicePath:
                return L"Device paths not allowed";
            case ValidationError::NullByte:
                return L"Embedded null character";
            case ValidationError::ExcessiveLength:
                return L"Path exceeds maximum length";
            case ValidationError::InvalidChars:
                return L"Invalid characters in path";
            case ValidationError::ThumbnailSizeRange:
                return L"Thumbnail size out of range (16–2048)";
            case ValidationError::RegistryKeyInvalid:
                return L"Invalid registry key name";
            case ValidationError::PluginIdInvalid:
                return L"Invalid plugin identifier";
            case ValidationError::EmptyInput:
                return L"Empty input";
            default:
...
class InputValidator
{
  public:
    ...
    PathValidationResult ValidatePath(const std::wstring& path) const
    {
        ...
        if (path.empty()) {
            res.error = ValidationError::EmptyInput;
            return res;
        }

        // Embedded null bytes
        if (path.find(L'\0') != std::wstring::npos) {
            res.error = ValidationError::NullByte;
            return res;
        }

        // Length check
        if (path.size() > 32767) {
            res.error = ValidationError::ExcessiveLength;
            return res;
        }
        ...
        if (path.size() >= 2 && path[0] == L'\\' && path[1] == L'\\') {
            ...
            if (path.size() >= 4 && path[2] == L'.' && path[3] == L'\\') {
                res.error = ValidationError::DevicePath;
                return res;
            }
        ...
        if (path.find(L"..") != std::wstring::npos) {
            ...
            res.error = ValidationError::PathTraversal;
            return res;
        }

        // Forbidden Windows device names (CON, AUX, COM1-9, LPT1-9, NUL, PRN)
        static const wchar_t* const FORBIDDEN[] = {L"CON",  L"AUX",  L"NUL",  L"PRN",  L"COM0", L"COM1", L"COM2",
                                                   L"COM3", L"COM4", L"COM5", L"COM6", L"COM7", L"COM8", L"COM9",
                                                   L"LPT0", L"LPT1", L"LPT2", L"LPT3", L"LPT4", L"LPT5", L"LPT6",
                                                   L"LPT7", L"LPT8", L"LPT9", nullptr};
        ...
        if (dot != std::wstring::npos)
            fnameNoExt = fnameNoExt.substr(0, dot);
        for (const wchar_t* const* d = FORBIDDEN; *d; ++d) {
            if (_wcsicmp(fnameNoExt.c_str(), *d) == 0) {
                res.error = ValidationError::InvalidChars;
                return res;
            }
        }
    ...
    PathValidationResult ValidateRegistryKey(const std::wstring& key) const
    {
        ...
        for (wchar_t c : key) {
            if (c == L'\0' || c == L'\\') {
                res.error = ValidationError::RegistryKeyInvalid;
                return res;
            }
        }
        res.canonical = key;
    ...
    PathValidationResult ValidatePluginId(const std::wstring& id) const
    {
        ...
        if (id.empty() || id.size() > 64) {
            res.error = id.empty() ? ValidationError::EmptyInput : ValidationError::ExcessiveLength;
            return res;
        }
        for (wchar_t c : id) {
            bool ok = (c >= L'A' && c <= L'Z') || (c >= L'a' && c <= L'z') || (c >= L'0' && c <= L'9') || c == L'-'
                      || c == L'_';
            if (!ok) {
                res.error = ValidationError::PluginIdInvalid;
                return res;
            }
        }
        res.canonical = id;
