...
enum class DecoderFormat : uint16_t {
    JPEG = 0, PNG, WebP, AVIF, JXL, HEIF, PSD, PDF, RAW,
    GeoTIFF, DICOM, FITS, HDF5, BMP, GIF, TIFF, SVG, ICO,
...
public:
    UniversalDecoderFacade()
        : m_initialized(false), m_gpuEnabled(true), m_maxConcurrent(4) {
        Initialize();
    }
    ...
    DecoderFormat IdentifyFormat(const std::string& filePath) const {
        ...
        if (dot == std::string::npos) return DecoderFormat::Unknown;
        std::string ext = filePath.substr(dot);
        for (auto& c : ext) c = static_cast<char>(c >= 'A' && c <= 'Z' ? c + 32 : c);
        for (const auto& entry : m_registeredDecoders)
            if (entry.extension == ext) return entry.format;
    ...
    std::string GetDecoderForFormat(DecoderFormat format) const {
        for (const auto& entry : m_registeredDecoders)
            if (entry.format == format) return entry.decoderName;
        return "none";
    }

    static const char* GetVersion() { return "30.5.0"; }
    ...
    size_t GetDecoderCount() const { return m_registeredDecoders.size(); }
...
private:
    ...
    void Initialize() {
        m_registeredDecoders = {
            {DecoderFormat::JPEG, "LibJPEG-Turbo", ".jpg"},
            {DecoderFormat::PNG,  "LibPNG",        ".png"},
            {DecoderFormat::WebP, "LibWebP",       ".webp"},
            {DecoderFormat::AVIF, "LibAVIF",       ".avif"},
            {DecoderFormat::JXL,  "LibJXL",        ".jxl"},
            {DecoderFormat::HEIF, "LibHEIF",       ".heif"},
            {DecoderFormat::PSD,  "PSDDecoder",    ".psd"},
            {DecoderFormat::PDF,  "MuPDF",         ".pdf"},
            {DecoderFormat::RAW,  "LibRaw",        ".cr3"},
        };
        m_initialized = true;
    }

    void Shutdown() { m_registeredDecoders.clear(); m_initialized = false; }
