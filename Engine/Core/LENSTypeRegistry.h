...
public:
    static LENSTypeRegistry& Instance() { static LENSTypeRegistry s; return s; }

    bool Register(const TypeRegistryEntry& entry) {
        if (entry.extension.empty()) return false;
        if (m_byId.count(entry.typeId) > 0) return false;
    ...
    const TypeRegistryEntry* LookupByMime(const std::string& mime) const {
        ...
        if (it == m_byMime.end()) return nullptr;
        return &m_entries[it->second];
    }
    ...
    std::vector<const TypeRegistryEntry*> GetAllTypes() const {
        ...
        for (const auto& e : m_entries) result.push_back(&e);
        return result;
    }
    ...
    bool Validate() const {
        if (m_entries.size() != m_byId.size()) return false;
        for (const auto& entry : m_entries) {
            if (entry.extension.empty()) return false;
            if (entry.category == LENSRegistryCategory::Unknown &&
                entry.typeId != 0) {
        ...
        for (const auto& e : m_entries) {
            if (++idCount[e.typeId] > 1) return false;
        }
        return true;
    }
