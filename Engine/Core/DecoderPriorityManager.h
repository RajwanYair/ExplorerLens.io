...
public:
    ...
    void RegisterDecoder(const std::wstring& decoderName,
    ...
    std::wstring GetPrimaryDecoder(const std::wstring& ext) const {
        ...
        if (it == m_extDecoders.end() || it->second.empty()) return {};
        const DecoderEntry* best = nullptr;
        for (auto& e : it->second) {
            if (!IsAvailableLocked(e.name)) continue;
    ...
    std::wstring GetFallbackDecoder(const std::wstring& ext, const std::wstring& primary) const {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto it = m_extDecoders.find(ext);
        if (it == m_extDecoders.end()) return {};
        const DecoderEntry* fallback = nullptr;
        for (auto& e : it->second) {
            if (e.name == primary) continue;
            if (!IsAvailableLocked(e.name)) continue;
            if (!fallback || e.priority < fallback->priority) fallback = &e;
        }
...
enum class DecodeUrgency : uint8_t { Immediate = 0, Soon = 1, Background = 2, Idle = 3 };
