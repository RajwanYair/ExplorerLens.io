...
public:
    static FFmpegExtractor& Instance() { static FFmpegExtractor s; return s; }
    ...
    VideoFrameInfo ExtractFrame(const std::wstring& filePath) {
        ...
        if (!m_initialized || filePath.empty()) return frame;
        ...
        if (frame.width == 0)  frame.width = 320;
        if (frame.height == 0) frame.height = 240;
    ...
    uint64_t GetDuration() const { return m_metadata.durationMs; }

    void SetMetadata(const FFVideoMetadata& meta) { m_metadata = meta; }
    const FFVideoMetadata& GetMetadata() const { return m_metadata; }
    ...
    uint64_t GetExtractCount() const { return m_extractCount; }

    bool Validate() const {
        if (m_config.maxWidth == 0 || m_config.maxHeight == 0) return false;
        if (m_config.seekPercent < 0.0f || m_config.seekPercent > 100.0f) return false;
        if (m_config.maxWidth > 7680 || m_config.maxHeight > 4320) return false;
        if (m_config.timeoutMs == 0) return false;
        return true;
    }
