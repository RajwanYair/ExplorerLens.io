...
class VulkanComputeActivation
{
  public:
    ...
    {
        switch (f) {
            case VulkanFeature::ComputeShader:
                return L"Compute Shader";
            case VulkanFeature::StorageImage:
                return L"Storage Image";
            case VulkanFeature::SampledImage:
                return L"Sampled Image";
            case VulkanFeature::PushConstants:
                return L"Push Constants";
            case VulkanFeature::Descriptor:
                return L"Descriptor Binding";
            case VulkanFeature::Timeline:
                return L"Timeline Semaphore";
            case VulkanFeature::DynamicRendering:
                return L"Dynamic Rendering";
            default:
                return L"Unknown";
        }
    }
    ...
    {
        return static_cast<size_t>(VulkanFeature::COUNT);
    }

    /// Queue type count
    static constexpr size_t QueueTypeCount()
    {
        return static_cast<size_t>(VulkanQueueType::COUNT);
    }
    ...
    {
        if (cfg.workGroupSizeX == 0 || cfg.workGroupSizeY == 0)
            return false;
        if (cfg.maxImageDimension == 0 || cfg.maxImageDimension > 65536)
            return false;
        return true;
    }
};
