...
public:
    explicit DifferentialPrivacyEngine(const DPParameters& params)
        : m_params(params), m_budgetUsed(0.0), m_rng(42) {}

    DPQueryResult Query(double trueValue) {
        DPQueryResult r;
        double remaining = m_params.epsilon - m_budgetUsed;
        r.budgetExceeded = remaining <= 0.0;
        if (r.budgetExceeded) { r.noisyValue = 0.0; r.privacyBudgetUsed = m_budgetUsed; return r; }
    ...
    double RemainingBudget() const { return std::max(0.0, m_params.epsilon - m_budgetUsed); }
    void   ResetBudget()           { m_budgetUsed = 0.0; }
    const DPParameters& Parameters() const { return m_params; }
