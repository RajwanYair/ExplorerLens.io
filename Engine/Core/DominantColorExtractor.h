...
public:
    ...
    /// pixel (0xAARRGGBB).
    std::vector<DominantColor> ExtractTopColors(
        ...
        std::vector<uint32_t> clusterSize(count, 0);
        for (uint32_t a : assignments) ++clusterSize[a];
    ...
    RGBColor GetAverageColor(const uint32_t* pixels,
    ...
    RGBColor QuantizeColor(RGBColor color, uint32_t levels) const {
        ...
        auto q = [step](uint8_t v) -> uint8_t {
            ...
            return static_cast<uint8_t>(
                (std::max)(0.0f, (std::min)(255.0f, qv)));
            };
    ...
    bool IsGrayscale(RGBColor color, uint8_t tolerance = 12) const {
        int maxC = (std::max)({ static_cast<int>(color.r),
                                static_cast<int>(color.g),
                                static_cast<int>(color.b) });
        int minC = (std::min)({ static_cast<int>(color.r),
                                static_cast<int>(color.g),
                                static_cast<int>(color.b) });
...
private:
    ...
    static std::vector<RGBColor> SubSample(
        const uint32_t* pixels, size_t totalPixels, size_t maxSamples) {
