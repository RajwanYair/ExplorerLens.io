...
class VCSBadgeAdapter
{
  public:
    ...
    VCSProvider DetectProvider(const std::string& path) const
    {
        ...
        while (!dir.empty()) {
            if (fs::exists(dir / ".git"))
                return VCSProvider::Git;
            if (fs::exists(dir / ".svn"))
                return VCSProvider::SVN;
            if (fs::exists(dir / ".hg"))
                return VCSProvider::Mercurial;
            if (fs::exists(dir / ".p4config"))
                return VCSProvider::Perforce;
    ...
    void SetConfig(const VCSBadgeConfig& cfg)
    {
        m_cfg = cfg;
    }
    const VCSBadgeConfig& GetConfig() const
    {
        return m_cfg;
    }
    ...
    {
        switch (p) {
            case VCSProvider::Git:
                return L"Git";
            case VCSProvider::SVN:
                return L"SVN";
            case VCSProvider::Mercurial:
                return L"Hg";
            case VCSProvider::Perforce:
                return L"P4";
            default:
                return L"Unknown";
        }
