...
public:
  ...
  static GPUBackend SelectBackend(const GPUAdapterInfo &adapter,
                                  const D3D12ActivationConfig &cfg) {
    if (cfg.preferD3D12 && adapter.supportsD3D12 &&
        adapter.dedicatedVideoMemory >= cfg.minVRAM)
  ...
  /// Backend count
  static constexpr size_t BackendCount() { return 9; }
