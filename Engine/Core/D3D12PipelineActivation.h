//==============================================================================
// DarkThumbs Engine — Sprint 262: D3D12 Pipeline Activation
...
enum class GPUBackend : uint8_t {
    ...
    GDI,            // GDI+ — CPU fallback
    Vulkan,         // Vulkan — future/Linux path
    Software,       // Pure CPU — last resort
...
public:
    ...
    static const wchar_t* BackendName(GPUBackend b) {
        switch (b) {
            ...
            case GPUBackend::GDI:       return L"GDI+ (CPU)";
            case GPUBackend::Vulkan:    return L"Vulkan";
            case GPUBackend::Software:  return L"Software";
    ...
    static const wchar_t* FeatureLevelName(D3DFeatureLevel fl) {
        switch (fl) {
            case D3DFeatureLevel::Level_11_0: return L"11.0";
            case D3DFeatureLevel::Level_11_1: return L"11.1";
            case D3DFeatureLevel::Level_12_0: return L"12.0";
            case D3DFeatureLevel::Level_12_1: return L"12.1";
            case D3DFeatureLevel::Level_12_2: return L"12.2";
            default: return L"Unknown";
        }
    }
    ...
    /// Backend count
    static constexpr size_t BackendCount() { return 5; }

    /// Feature level count
    static constexpr size_t FeatureLevelCount() { return 5; }

    /// Validate activation config
    static bool ValidateConfig(const D3D12ActivationConfig& cfg) {
        if (cfg.minVRAM < 128 * 1024 * 1024ULL) return false;
        if (cfg.computeThreadGroupX == 0 || cfg.computeThreadGroupY == 0) return false;
        if (cfg.maxConcurrentDispatches == 0 || cfg.maxConcurrentDispatches > 16) return false;
        return true;
    }
};
