...
{
    uint32_t purged = 0;
    auto it = std::remove_if(m_entries.begin(), m_entries.end(), [&purged](const ContentIndexEntry& e) {
        if (e.state == IndexState::Removed) {
            purged++;
            return true;
        }
        return false;
...
{
    if (ext == L".jpg" || ext == L".jpeg" || ext == L".png" || ext == L".bmp" || ext == L".gif" || ext == L".webp"
        || ext == L".tiff" || ext == L".avif" || ext == L".heif" || ext == L".heic" || ext == L".jxl"
        || ext == L".svg") {
        return ContentType::Image;
    }
    if (ext == L".zip" || ext == L".rar" || ext == L".7z" || ext == L".tar" || ext == L".gz" || ext == L".cbz"
        || ext == L".cbr" || ext == L".cb7") {
