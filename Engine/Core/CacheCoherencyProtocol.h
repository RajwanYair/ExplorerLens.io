...
class CacheCoherencyProtocol
{
  public:
    ...
    bool Initialize(const CoherencyConfig& config = {})
    {
        ...
        m_mapping = CreateFileMappingW(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sharedSize,
                                       m_config.sharedMemoryName.c_str());
        if (!m_mapping) {
    ...
    bool Lookup(uint64_t fileHash, CacheEntryHeader& outEntry)
    {
        ...
        auto* header = reinterpret_cast<SharedCacheHeader*>(m_sharedBase);
        auto* entries =
            reinterpret_cast<CacheEntryHeader*>(static_cast<uint8_t*>(m_sharedBase) + sizeof(SharedCacheHeader));
    ...
    bool Store(uint64_t fileHash, const CacheEntryHeader& entry, const void* thumbnailData, uint32_t dataSize)
    {
        if (!m_initialized || !thumbnailData)
            return false;
