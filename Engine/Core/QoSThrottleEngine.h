...
struct QoSThrottleDecision
{
    ...
    {
        return mode != ThrottleMode::Suspend;
    }
};
...
{
  public:
    explicit QoSThrottleEngine(QoSPolicy policy = {}) : m_policy(std::move(policy)) {}
    ...
    QoSThrottleDecision Evaluate(const QoSRequest& req)
    {
        ...
        switch (req.qosClass) {
            ...
            case QoSClass::BatchDecode:
                d.mode = active > m_policy.interactiveSlots / 2 ? ThrottleMode::SoftThrottle : ThrottleMode::None;
    ...
    const QoSPolicy& Policy() const noexcept
    {
        return m_policy;
    }
