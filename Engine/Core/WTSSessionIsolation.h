...
enum class WTSSessionState { Active, Disconnected, Remote, Idle };
...
struct WTSSessionInfo {
    ...
    std::string StateName() const noexcept {
        switch (state) {
        case WTSSessionState::Active:       return "Active";
        case WTSSessionState::Disconnected: return "Disconnected";
        case WTSSessionState::Remote:       return "Remote";
        case WTSSessionState::Idle:         return "Idle";
        }
        return "Unknown";
    }
};
...
public:
    ...
    WTSSessionId     CurrentSessionId()    const noexcept { return m_currentSession; }
    void             SetCurrentSession(WTSSessionId id) noexcept { m_currentSession = id; }

    void RegisterSession(const WTSSessionInfo& info) { m_sessions[info.sessionId] = info; }
    ...
    bool IsRDSEnvironment() const noexcept { return m_sessions.size() > 1; }
    int  SessionCount()     const noexcept { return (int)m_sessions.size(); }

    std::vector<WTSSessionInfo> ActiveSessions() const {
        std::vector<WTSSessionInfo> v;
        for (const auto& kv : m_sessions)
            if (kv.second.state == WTSSessionState::Active) v.push_back(kv.second);
        return v;
    }
