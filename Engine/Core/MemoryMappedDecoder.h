...
public:
    ...
    const MMapStats& GetStats() const { return m_stats; }

    void ResetStats() { m_stats = {}; }

    MMapBackend GetBackend() const { return m_backend; }
    ...
    static uint64_t RecommendedMappingSize(uint64_t fileSizeBytes) {
        ...
        if (fileSizeBytes < 8 * 1024 * 1024) return 0;
        return fileSizeBytes;
    }

    static const char* BackendName(MMapBackend b) {
        switch (b) {
        case MMapBackend::WindowsMMapV1:  return "Windows-MMapV1";
        case MMapBackend::WindowsFileCopy:return "Windows-FileCopy";
        case MMapBackend::Unavailable:    return "Unavailable";
