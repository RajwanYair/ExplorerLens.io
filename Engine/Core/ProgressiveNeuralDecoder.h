...
public:
    ...
    PNDDecodeStep NextStep() {
        ...
        step.rgbaData.assign(static_cast<size_t>(w) * h * 4,
                             static_cast<uint8_t>(0x20 + m_level * 0x30));
        step.qualityEstimate  = std::min(1.0f, static_cast<float>(m_level + 1) * 0.25f);
        ++m_level;
        return step;
    }
    bool IsComplete() const { return m_level >= 4; }
    void Reset()            { m_level = 0; m_encoded.clear(); }
