...
public:
    ...
    RenderedAnnotationOverlay Render(const AnnotationOverlayData& data,
                                     uint8_t* /*thumbPixels*/,
                                     int thumbW, int thumbH) const {
        RenderedAnnotationOverlay result;
        if (!data.hasAnnotations()) return result;
    ...
    const AnnotationOverlayConfig& Config() const noexcept { return m_cfg; }
    void SetConfig(const AnnotationOverlayConfig& c) noexcept { m_cfg = c; }
