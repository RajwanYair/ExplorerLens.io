...
class TabbedExplorerSync
{
  public:
    ...
    void RegisterTab(const ExplorerTabState& tab)
    {
        m_tabs[tab.tabId] = tab;
    }
    ...
    void SetPolicy(TabSyncPolicy p) noexcept
    {
        m_policy = p;
    }
    TabSyncPolicy GetPolicy() const noexcept
    {
        return m_policy;
    }
    ...
    void OnTabStateChange(const ExplorerTabState& updated)
    {
        ...
        if (m_policy == TabSyncPolicy::Independent)
            return;
        SyncEvent evt{updated.tabId, updated.folderPath, updated};
        for (auto& [id, tab] : m_tabs) {
            if (id == updated.tabId)
                continue;
            bool shouldSync = (m_policy == TabSyncPolicy::SyncAll) || (tab.folderPath == updated.folderPath);
            if (shouldSync) {
    ...
    int TabCount() const noexcept
    {
        return (int)m_tabs.size();
    }
    bool HasTab(uint64_t id) const noexcept
    {
        return m_tabs.count(id) > 0;
    }
