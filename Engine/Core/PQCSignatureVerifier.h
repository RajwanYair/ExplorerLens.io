...
enum class PQCAlgorithm { MLDSA65, MLDSA87, SLHDSA_SHA2_128s, SLHDSA_SHA2_256s };
...
public:
    ...
    PQCVerifyResult Verify(const PQCVerifyRequest& req) {
        ...
        if (ok)  ++m_stats.verifiesOk;
        else    { ++m_stats.verifiesFail; res.errorCode = "INVALID_INPUT"; }

        float n = static_cast<float>(m_stats.verifiesOk + m_stats.verifiesFail);
        if (n > 0) m_stats.avgLatencyUs = (m_stats.avgLatencyUs * (n-1) + 120.0f) / n;
        return res;
    }

    bool VerifyBatch(const std::vector<PQCVerifyRequest>& reqs,
                     std::vector<PQCVerifyResult>& out) {
        out.clear();
        out.reserve(reqs.size());
        bool allOk = true;
        for (const auto& r : reqs) {
            out.push_back(Verify(r));
            if (!out.back().valid) allOk = false;
        }
        return allOk;
    }

    PQCVerifierStats GetStats() const { return m_stats; }
