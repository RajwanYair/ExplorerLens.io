...
class WinUI3Research
{
  public:
    ...
    {
        if (index >= ASSESSMENT_COUNT)
            return {};

        static const WinUI3Assessment assessments[] = {
            {"LENSManager GUI", "WTL 10.0 (ATL Dialog)", "WinUI 3 XAML", MigrationFeasibility::PartialOnly,
             "Admin elevation conflicts with WinUI 3 app model; "
             "WinUI 3 doesn't support running elevated by default",
             "Hybrid approach using XAML Islands is the recommended path", 30},
            {"Shell Extension DLL", "ATL COM (IThumbnailProvider)", "Keep as-is (COM required)",
             ...
             "Shell extension stays as native COM DLL — non-negotiable", 0},
            {"Settings Dialog", "WTL CPropertySheet", "WinUI 3 NavigationView", MigrationFeasibility::FullyFeasible,
             nullptr,
             "Settings page is the best candidate for XAML modernization. "
             "Can use ContentDialog + NavigationView for tabbed layout",
             15},
            {"Format Registration", "Win32 CheckBoxes + Registry", "WinUI 3 ToggleSwitch + TreeView",
             MigrationFeasibility::FullyFeasible, nullptr,
             "TreeView with ToggleSwitches would greatly improve UX. "
             "Category grouping becomes natural with TreeViewItem hierarchy",
             10},
            {"Theme Support", "Owner-draw dark mode (uxtheme hacks)", "WinUI 3 native dark/light theme",
             MigrationFeasibility::Recommended, nullptr,
             "WinUI 3 handles dark mode natively with RequestedTheme. "
             "Eliminates need for DarkModeRendererV2 hacks",
             5},
            {"Performance Charts", "GDI+ custom drawing", "WinUI 3 + Win2D charts", MigrationFeasibility::FullyFeasible,
             nullptr,
             "Win2D provides GPU-accelerated 2D rendering for charts. "
             "Significantly better than GDI+ for real-time stats",
             12},
            {"COM Registration", "RegSvr32 + manual registry", "MSIX sparse package + AppExtension",
             ...
             20},
            {"Installer", "WiX 6 MSI + Inno Setup", "MSIX + MSI bridge", MigrationFeasibility::PartialOnly,
             "MSIX doesn't support all COM registration scenarios. "
             "Shell extension COM needs SxS manifest or sparse package",
             "Recommended: Keep MSI as primary, add MSIX as optional modern path", 15}};
