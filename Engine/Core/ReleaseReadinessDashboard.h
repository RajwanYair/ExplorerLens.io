...
{
    switch (c) {
        case GateCategory::Build:
            return "Build";
        case GateCategory::Tests:
            return "Tests";
        case GateCategory::Performance:
            return "Performance";
        case GateCategory::VersionDrift:
            return "VersionDrift";
        case GateCategory::CodeQuality:
            return "CodeQuality";
        case GateCategory::Packaging:
            return "Packaging";
        case GateCategory::Documentation:
...
struct DashboardResult
{
    ...
    {
        return overall == ReadinessLevel::Green;
    }
...
class ReleaseReadinessDashboard
{
  public:
    ...
    static std::string FormatReport(const DashboardResult& r)
    {
        ...
        rpt += "Overall: " + LevelLabel(r.overall) + " " + LevelIcon(r.overall) + "\n\n";
        ...
        for (auto& g : r.gates) {
            rpt += LevelIcon(g.level) + " " + GateCategoryName(g.category) + " — " + g.summary;
            if (g.totalChecks > 0) {
                char buf[32];
    ...
    const std::vector<ChecklistItem>& Checklist() const
    {
        return m_checklist;
    }
  ...
  private:
    ...
    {
        m_checklist = {{"Zero build warnings in Release", GateCategory::Build, false, true},
