...
inline const wchar_t* ToString(BatchState state) {
    switch (state) {
    case BatchState::Idle:      return L"Idle";
    case BatchState::Running:   return L"Running";
    case BatchState::Paused:    return L"Paused";
    case BatchState::Completed: return L"Completed";
    case BatchState::Failed:    return L"Failed";
    case BatchState::Cancelled: return L"Cancelled";
...
public:
    ...
    bool Submit(const std::vector<SmartBatchTask>& tasks) {
        if (!m_initialized || tasks.empty()) return false;
        m_pendingCount += static_cast<uint64_t>(tasks.size());
        return true;
    }

    bool Start() {
        if (!m_initialized || m_pendingCount == 0) return false;
    ...
    bool Pause() {
        if (m_state != BatchState::Running) return false;
        m_state = BatchState::Paused;
        return true;
    }

    bool Resume() {
        if (m_state != BatchState::Paused) return false;
        m_state = BatchState::Running;
        return true;
    }

    bool Cancel() {
        if (m_state == BatchState::Idle || m_state == BatchState::Completed) return false;
        m_state = BatchState::Cancelled;
        return true;
    }

    BatchState GetState() const { return m_state; }
    uint32_t GetMaxConcurrency() const { return m_maxConcurrency; }
    BatchProcessorStats GetStats() const { return m_stats; }
