...
{
  public:
    explicit AdaptiveBitrateSelector(ABRStrategy strategy) : m_strategy(strategy), m_bandwidthKbps(5000.0f) {}

    void SetProfiles(const std::vector<ABRProfile>& profiles)
    {
        m_profiles = profiles;
    }
    ...
    ABRSelectResult Select(float measuredBandwidthKbps, uint32_t bufferMs)
    {
        ...
        for (const auto& p : m_profiles)
            if (p.bitrateKbps <= static_cast<uint32_t>(effectiveBw))
                best = p;
    ...
    float EstimatedBandwidth() const
    {
        return m_bandwidthKbps;
    }
