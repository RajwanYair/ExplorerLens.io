...
public:
    ...
    static SizeCategory ClassifySize(uint64_t bytes) noexcept {
        ...
        if (bytes < KB)             return SizeCategory::Tiny;
        if (bytes < 100 * KB)       return SizeCategory::Small;
        if (bytes < 10 * MB)        return SizeCategory::Medium;
        if (bytes < GB)             return SizeCategory::Large;
        if (bytes < 100 * GB)       return SizeCategory::Huge;
    ...
    static BadgeColor GetBadgeColor(SizeCategory category) noexcept {
        switch (category) {
        case SizeCategory::Tiny:    return { 160, 160, 160, 255 };  // Gray
        case SizeCategory::Small:   return { 76, 175,  80, 255 };  // Green
        case SizeCategory::Medium:  return { 33, 150, 243, 255 };  // Blue
        case SizeCategory::Large:   return { 255, 152,   0, 255 };  // Orange
        case SizeCategory::Huge:    return { 244,  67,  54, 255 };  // Red
        case SizeCategory::Massive: return { 156,  39, 176, 255 };  // Purple
