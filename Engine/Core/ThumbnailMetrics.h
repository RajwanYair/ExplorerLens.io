...
struct PipelineProfile
{
    ...
    {
        return stageUs[static_cast<size_t>(ProfileStage::Decode)];
    }
};
...
{
  public:
    SizeNegotiationResult NegotiateSize(uint32_t srcW, uint32_t srcH, uint32_t requested, float dpiScale = 1.0f) const
    {
    ...
    HistogramData ComputeHistogram(const uint8_t* channel, uint32_t pixelCount) const
    {
        ...
        for (uint32_t i = 0; i < 256; ++i)
            sum += static_cast<double>(i) * h.counts[i];
