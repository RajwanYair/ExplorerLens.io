...
bool EngineConfig::ReadRegistryBool(HKEY key, const wchar_t* valueName, bool defaultValue)
{
    ...
    if (RegQueryValueExW(key, valueName, nullptr, &type, reinterpret_cast<BYTE*>(&value), &size) == ERROR_SUCCESS) {
        return (value != 0);
    }
    return defaultValue;
}
...
bool EngineConfig::WriteRegistryBool(HKEY key, const wchar_t* valueName, bool value)
{
    ...
    return RegSetValueExW(key, valueName, 0, REG_DWORD, reinterpret_cast<const BYTE*>(&dword), sizeof(DWORD))
           == ERROR_SUCCESS;
}

bool EngineConfig::WriteRegistryDWORD(HKEY key, const wchar_t* valueName, uint32_t value)
{
    return RegSetValueExW(key, valueName, 0, REG_DWORD, reinterpret_cast<const BYTE*>(&value), sizeof(DWORD))
           == ERROR_SUCCESS;
}
...
bool EngineConfig::SaveToRegistry(HKEY rootKey, const wchar_t* subKey) const
{
    ...
    if (RegCreateKeyExW(rootKey, subKey, 0, nullptr, 0, KEY_WRITE, nullptr, &hKey, &disposition) != ERROR_SUCCESS) {
        return false;
    }
...
ENGINE_API EngineConfig& GetEngineConfig()
{
    ...
    std::call_once(initFlag, []() { instance.LoadFromRegistry(); });
