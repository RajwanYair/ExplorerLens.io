...
struct ThrottleStats {
    ...
    double coalesceRatio() const {
        return eventsReceived > 0
            ? static_cast<double>(eventsCoalesced) / eventsReceived
            : 0.0;
    }
};
...
public:
    explicit FileChangeThrottler(ThrottleConfig cfg = {})
        : m_config(cfg) {
    }
    ...
    ThrottleStats GetStats() const { return m_stats; }
    const ThrottleConfig& Config() const { return m_config; }
