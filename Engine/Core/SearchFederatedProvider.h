...
{
    static const char* names[] = {"FullText", "Property", "Visual", "Combined", "Metadata"};
...
class SearchFederatedProvider
{
  public:
    ...
    bool IndexFile(const std::wstring& filePath, const std::wstring& format, uint32_t width, uint32_t height,
                   uint64_t fileSize)
    {
        IndexEntry entry;
    ...
    const FederatedSearchStats& GetStats() const
    {
        return m_stats;
    }
    uint64_t GetIndexedCount() const
    {
        return m_stats.indexedFiles;
    }
    ...
    bool MatchesFilters(const IndexEntry& entry, const std::vector<PropertyFilter>& filters) const
    {
        for (const auto& filter : filters) {
            if (filter.propertyName == L"format") {
                if (filter.op == PropertyOperator::Equals && entry.formatName != filter.value)
                    return false;
            } else if (filter.propertyName == L"width") {
                uint32_t val = static_cast<uint32_t>(std::stoul(filter.value));
                if (filter.op == PropertyOperator::GreaterThan && entry.width <= val)
                    return false;
                if (filter.op == PropertyOperator::LessThan && entry.width >= val)
                    return false;
            }
        }
        return true;
