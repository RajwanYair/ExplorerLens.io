...
enum class PredictorScrollDirection {
    Unknown,
    Forward,
    Backward,
    Random
  ...
  public:
    ...
    std::vector<PrefetchCandidate> OnViewport(const ViewportState& vs)
    {
        ...
        if (dir == PredictorScrollDirection::Unknown) {
            if (delta > 0)
                dir = PredictorScrollDirection::Forward;
            else if (delta < 0)
                dir = PredictorScrollDirection::Backward;
            else
                dir = PredictorScrollDirection::Random;
        }
        ...
        if (dir == PredictorScrollDirection::Backward)
            base = firstIdx - 1;
    ...
    float HitRate() const
    {
        return (m_prefetchCount > 0) ? static_cast<float>(m_hitCount) / m_prefetchCount : 0.0f;
    }
