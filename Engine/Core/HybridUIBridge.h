...
public:
    ...
    bool CreatePanel(HWND hwndParent, XamlPanelId panelId, const RECT& bounds) {
        ...
        for (auto& p : m_panels) {
            if (p.id == panelId) {
                if (p.hwndHost) DestroyWindow(p.hwndHost);
                p = panel;
    ...
    void ResizePanel(XamlPanelId panelId, const RECT& newBounds) {
        for (uint32_t i = 0; i < m_panelCount; ++i) {
            if (m_panels[i].id == panelId && m_panels[i].hwndHost) {
                ...
                SetWindowPos(m_panels[i].hwndHost, nullptr,
    ...
    XamlHostState GetState() const { return m_state; }
    const HybridUIConfig& GetConfig() const { return m_config; }
    ...
    static const char* StateName(XamlHostState s) {
        switch (s) {
            case XamlHostState::NotInitialized: return "NotInitialized";
            case XamlHostState::Initializing:   return "Initializing";
            case XamlHostState::Ready:          return "Ready";
            case XamlHostState::Active:         return "Active";
            case XamlHostState::Error:          return "Error";
            case XamlHostState::Unsupported:    return "Unsupported";
