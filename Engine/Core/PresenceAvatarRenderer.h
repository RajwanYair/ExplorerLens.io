...
public:
    explicit PresenceAvatarRenderer(AvatarRenderConfig config = {})
        : m_config(std::move(config)) {}

    AvatarFrame Render(const std::vector<AvatarParticipant>& participants,
    ...
    const AvatarRenderConfig& Config() const noexcept { return m_config; }
    void SetConfig(AvatarRenderConfig cfg) noexcept { m_config = std::move(cfg); }

private:
    void PaintBadge(std::vector<uint8_t>& rgba, int cx, int cy,
                    int w, int h, const std::array<uint8_t,3>& color) const {
