...
enum class OverlayBadge : uint8_t {
    None, CloudSync, CloudOffline, Locked, Warning, Error, Favorite
};

// -- Annotation types (from Annotation) ---------------------------------------

enum class AnnotationType : uint8_t { Resolution, FileSize, Format, Duration, PageCount };

enum class AnnotationPosition : uint8_t { TopLeft, TopRight, BottomLeft, BottomRight, Center };

enum class AnnotationStyle : uint8_t { Plain, Corner, Badge, Overlay, Tooltip };
...
inline const char* AnnotationStyleName(AnnotationStyle s) noexcept {
    switch (s) {
    case AnnotationStyle::Plain:   return "Plain";
...
enum class WatermarkPosition : uint8_t { TopLeft, TopRight, BottomLeft, BottomRight, Center };

enum class WatermarkType : uint8_t { FormatBadge, SizeBadge, ResolutionTag, StatusIcon, CustomBranding, Text, QRCode, None };
...
inline const char* WatermarkTypeName(WatermarkType t) noexcept {
    switch (t) {
    case WatermarkType::FormatBadge:      return "FormatBadge";
    case WatermarkType::SizeBadge:        return "SizeBadge";
    case WatermarkType::ResolutionTag:    return "ResolutionTag";
    case WatermarkType::StatusIcon:       return "StatusIcon";
    case WatermarkType::CustomBranding:   return "CustomBranding";
...
enum class SpriteLayout : uint8_t { Grid, Strip, Atlas, TreePack, Custom };

// -- Stitch orientation (from Stitcher) ---------------------------------------

enum class StitchOrientation : uint8_t { Horizontal, Vertical, Grid };
...
struct WatermarkConfig {
    WatermarkType     type     = WatermarkType::FormatBadge;
...
public:
    void SetConfig(const WatermarkConfig& cfg) noexcept { m_config = cfg; }
    bool IsActive() const noexcept { return m_config.type != WatermarkType::None; }
    bool Apply(uint8_t* pixels, uint32_t w, uint32_t h) noexcept {
        if (!IsActive() || !pixels || w == 0 || h == 0) return false;
        ++m_appliedCount;
        return true;
    }
    uint32_t GetAppliedCount() const noexcept { return m_appliedCount; }
    static std::vector<WatermarkType> GetSupportedTypes() {
        return { WatermarkType::FormatBadge, WatermarkType::SizeBadge,
                 WatermarkType::Text, WatermarkType::QRCode };
    }
private:
...
public:
    ...
    bool RemoveAnnotation(size_t index) {
        if (index >= m_annotations.size()) return false;
        m_annotations.erase(m_annotations.begin() + static_cast<ptrdiff_t>(index));
        return true;
    }
    bool IsFull() const noexcept { return m_annotations.size() >= MAX_ANNOTATIONS; }
    size_t GetAnnotationCount() const noexcept { return m_annotations.size(); }
...
public:
    bool ApplyOverlay(const std::vector<uint8_t>& thumbRGBA,
    ...
    std::vector<StitchRect> ComputeStitchLayout(
        const std::vector<std::pair<uint32_t, uint32_t>>& segments,
        StitchOrientation orient = StitchOrientation::Horizontal, uint32_t padding = 2) const {
        std::vector<StitchRect> rects;
        int32_t offset = 0;
        for (auto& [w, h] : segments) {
            StitchRect r; r.x = (orient == StitchOrientation::Vertical) ? 0 : offset;
    ...
    std::vector<SpriteEntry> PackSprites(uint32_t count, uint32_t cellW, uint32_t cellH,
        ...
        for (uint32_t i = 0; i < count; ++i) {
            SpriteEntry e;
            e.index = i;
            if (layout == SpriteLayout::Strip) { e.x = i * cellW; e.y = 0; }
            else { e.x = (i % cols) * cellW; e.y = (i / cols) * cellH; }
            e.w = cellW; e.h = cellH;
            entries.push_back(e);
        }
        return entries;
    ...
    void ApplyLensEffect(double cx, double cy, double radius, double power,
                         double srcX, double srcY, double& outX, double& outY) const {
        double dx = srcX - cx, dy = srcY - cy;
        double dist = std::sqrt(dx * dx + dy * dy);
        if (dist >= radius || dist < 1e-6) { outX = srcX; outY = srcY; return; }
