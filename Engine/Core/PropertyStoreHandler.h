...
class PropertyStoreHandler
{
  public:
    ...
    static const GUID& GetPropertyHandlerCLSID()
    {
        ...
        static const GUID clsid = {0xB2E7F4A1, 0x3C8D, 0x4E9F, {0xA5, 0xB6, 0x1D, 0x2E, 0x3F, 0x4A, 0x5B, 0x6C}};
        return clsid;
    }
    ...
    static constexpr uint32_t GetPropertyIDCount()
    {
        return 16;
    }
  ...
  private:
    ...
    void RegisterFormat(const wchar_t* ext, const wchar_t* desc, PropertyCapability caps, ExtractFn fn)
    {
    ...
    static std::vector<PropertyValue> ExtractImageProperties(const wchar_t* filePath)
    {
        ...
        if (bytesRead >= 2 && header[0] == 0xFF && header[1] == 0xD8) {
            ...
            for (DWORD off = 2; off + 1 < bytesRead;) {
                ...
                if (off + 4 > bytesRead)
                    break;
                uint16_t segLen = (static_cast<uint16_t>(header[off + 2]) << 8) | header[off + 3];
        ...
        else if (bytesRead >= 26 && header[0] == 0x89 && header[1] == 0x50 && header[2] == 0x4E && header[3] == 0x47) {
            formatName = L"PNG";
        ...
        // ---- TIFF: II (LE) or MM (BE) with magic 0x002A ----
            ...
            auto rd16 = [&](DWORD o) -> uint16_t {
                if (o + 1 >= bytesRead)
                    return 0;
                return bigEndian ? (static_cast<uint16_t>(header[o]) << 8 | header[o + 1])
                                 : (header[o] | static_cast<uint16_t>(header[o + 1]) << 8);
            };
            auto rd32 = [&](DWORD o) -> uint32_t {
                if (o + 3 >= bytesRead)
                    return 0;
                return bigEndian ? (static_cast<uint32_t>(header[o]) << 24 | static_cast<uint32_t>(header[o + 1]) << 16
        ...
        } else if (bytesRead >= 7 && header[0] == 0x52 && header[1] == 0x61 && header[2] == 0x72 && header[3] == 0x21) {
            formatName = L"RAR Archive";
        } else if (bytesRead >= 6 && header[0] == 0x37 && header[1] == 0x7A && header[2] == 0xBC && header[3] == 0xAF) {
