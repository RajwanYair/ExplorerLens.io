...
class PerformanceDashboard
{
  public:
    ...
    inline void RecordMetric(const std::string& category, const std::string& name, double value)
    {
        MetricSample sample;
        sample.value = value;
    ...
    inline void RecordLatency(const std::string& category, const std::string& name, uint64_t microseconds)
    {
        RecordMetric(category, name, static_cast<double>(microseconds));
    }

    /// Convenience: record a throughput measurement (items/second).
    inline void RecordThroughput(const std::string& category, const std::string& name, double itemsPerSecond)
    {
        RecordMetric(category, name, itemsPerSecond);
    }

    /// Convenience: increment a counter metric by 1.
    inline void RecordCounter(const std::string& category, const std::string& name)
    {
        AcquireSRWLockExclusive(&m_lock);
        auto& ring = m_metrics[category][name];
        double prev = 0.0;
    ...
    inline MetricSummary GetMetricSummary(const std::string& category, const std::string& name) const
    {
        ...
        for (const auto& catPair : m_metrics) {
            for (const auto& metPair : catPair.second) {
                ...
                for (size_t i = 0; i < count; ++i) {
                    ...
                    auto us = std::chrono::duration_cast<std::chrono::microseconds>(s.timestamp - epoch).count();
                    ofs << catPair.first << "," << metPair.first << "," << us << "," << s.value << "\n";
                }
            }
        }
        ReleaseSRWLockShared(const_cast<PSRWLOCK>(&m_lock));
  ...
  private:
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, MetricRing>> m_metrics;
