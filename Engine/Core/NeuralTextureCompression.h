...
class NeuralTextureCompression
{
  public:
    ...
    static constexpr double DecodeTimeFactor(NeuralModelTier tier, NeuralCodecBackend backend)
    {
        ...
        if (backend == NeuralCodecBackend::DirectML || backend == NeuralCodecBackend::TensorRT)
            baseFactor *= 0.15;
        else if (backend == NeuralCodecBackend::ONNX_Runtime || backend == NeuralCodecBackend::OpenVINO)
            baseFactor *= 0.3;
        return baseFactor;
    }
};
