...
public:
    ...
    const E& Error() const& noexcept { return m_storage.error; }
    E& Error() & noexcept { return m_storage.error; }
    E&& Error() && noexcept { return std::move(m_storage.error); }
...
template <typename T>
Result<T> MakeError(StructuredErrorDomain domain,
...
#define TRY_RESULT(varDecl, expr) \
    ...
    if (_tryResult_##__LINE__.IsErr()) \
        return decltype(expr)(::ExplorerLens::Engine::ErrTag{}, \
                              std::move(_tryResult_##__LINE__.Error())); \
