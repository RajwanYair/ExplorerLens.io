...
{
    switch (value) {
        case FileIntegrityCheckType::Hash:
            return "Hash";
        case FileIntegrityCheckType::Timestamp:
            return "Timestamp";
        case FileIntegrityCheckType::Attribute:
            return "Attribute";
        case FileIntegrityCheckType::FullScan:
            return "FullScan";
        case FileIntegrityCheckType::Quick:
            return "Quick";
        default:
            return "Unknown";
    }
...
inline const char* FileIntegrityStatusName(FileIntegrityStatus value)
{
    switch (value) {
        case FileIntegrityStatus::Valid:
            return "Valid";
        case FileIntegrityStatus::Modified:
            return "Modified";
        case FileIntegrityStatus::Corrupted:
            return "Corrupted";
        case FileIntegrityStatus::Missing:
            return "Missing";
        case FileIntegrityStatus::Quarantined:
            return "Quarantined";
        default:
            return "Unknown";
    }
    ...
    uint32_t ScanDirectory(const std::wstring& directoryPath, FileIntegrityCheckType checkType)
    {
        ...
        do {
            ...
            if (it == m_records.end()) {
                ...
                record.lastCheckType = checkType;
                record.lastCheckMs = GetCurrentTimeMs();
                record.fileSizeBytes = (static_cast<uint64_t>(findData.nFileSizeHigh) << 32) | findData.nFileSizeLow;
                if (checkType == FileIntegrityCheckType::Hash || checkType == FileIntegrityCheckType::FullScan) {
