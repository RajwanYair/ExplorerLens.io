...
enum class TrackedResourceType : uint8_t {
    GDIObject, COMRef, GPUTexture, GPUBuffer, FileHandle, HeapAlloc, Event, Mutex, COUNT
};
...
public:
    ...
    bool HasLeaks() const {
        for (uint32_t i = 0; i < m_recordCount; ++i) {
            if (!m_records[i].freed) return true;
        }
        return false;
    }
    ...
    static size_t TypeCount() { return static_cast<size_t>(TrackedResourceType::COUNT); }
