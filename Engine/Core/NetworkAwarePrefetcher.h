...
class NetworkAwarePrefetcher
{
  public:
    ...
    // Per-policy settings.
    bool PrefetchOnMetered() const
    {
        return m_prefetchOnMetered;
    }
    void SetPrefetchOnMetered(bool v)
    {
        m_prefetchOnMetered = v;
    }

    uint64_t MaxBandwidthBps() const
    {
        return m_maxBandwidthBps;
    }
    void SetMaxBandwidthBps(uint64_t v)
    {
        m_maxBandwidthBps = v;
    }
