...
enum class RJSPriority { Critical = 0, High, Normal, Low, Background };
enum class RJSJobState { Pending, Running, Complete, Failed };
...
public:
    uint32_t Enqueue(const std::string& description,
                     RJSPriority priority = RJSPriority::Normal) {
        RJSJob j;
        j.id          = ++m_counter;
        j.priority    = priority;
    ...
    bool Complete(uint32_t jobId) {
        ...
        if (it == m_running.end()) return false;
    ...
    uint32_t RunningCount()   const { return static_cast<uint32_t>(m_running.size()); }
    uint32_t CompletedCount() const { return static_cast<uint32_t>(m_completed.size()); }
