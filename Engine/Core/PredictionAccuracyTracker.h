...
public:
    void Record(const PATSample& sample) { m_samples.push_back(sample); }

    PATAccuracyReport Compute() const {
        PATAccuracyReport r;
        r.totalPredictions = static_cast<uint32_t>(m_samples.size());
        if (r.totalPredictions == 0) return r;
        float confSum = 0.0f;
        for (const auto& s : m_samples) {
            if (s.wasViewed) ++r.hits;
    ...
    uint32_t SampleCount() const { return static_cast<uint32_t>(m_samples.size()); }
