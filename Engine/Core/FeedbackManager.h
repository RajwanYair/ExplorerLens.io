...
enum class FeedbackStatus { Draft, Submitted, Failed, Throttled };
...
public:
    ...
    bool Validate(const FeedbackItem& item, std::wstring& outError) const {
        if (item.title.empty()) { outError = L"Title is required."; return false; }
        if (item.title.size() > 200) { outError = L"Title must be ≤ 200 characters."; return false; }
        if (item.body.size() > 4000) { outError = L"Body must be ≤ 4000 characters."; return false; }
        if (item.rating < 0 || item.rating > 5) { outError = L"Rating must be 0-5."; return false; }
        return true;
    }

    // Submit feedback asynchronously (callback on completion)
    void Submit(FeedbackItem item,
                std::function<void(const FeedbackItem&)> onDone) {
        ...
        HINTERNET hReq  = WinHttpOpenRequest(hConn, L"POST", L"/api/v1/feedback",
    ...
    void SaveDraft(const FeedbackItem& item) {
        HKEY hk = nullptr;
        RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\FeedbackDraft",
                0, nullptr, 0, KEY_WRITE, nullptr, &hk, nullptr);
        if (!hk) return;
        RegSetValueExW(hk, L"Title", 0, REG_SZ,
            reinterpret_cast<const BYTE*>(item.title.c_str()),
            static_cast<DWORD>((item.title.size()+1)*sizeof(wchar_t)));
        RegSetValueExW(hk, L"Body", 0, REG_SZ,
            reinterpret_cast<const BYTE*>(item.body.c_str()),
            static_cast<DWORD>((item.body.size()+1)*sizeof(wchar_t)));
        DWORD cat = static_cast<DWORD>(item.category);
        RegSetValueExW(hk, L"Category", 0, REG_DWORD,
                reinterpret_cast<const BYTE*>(&cat), sizeof(DWORD));
        RegCloseKey(hk);
    }
    ...
    FeedbackItem LoadDraft() const {
        FeedbackItem item{};
        HKEY hk = nullptr;
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\FeedbackDraft",
                0, KEY_READ, &hk) != ERROR_SUCCESS) return item;
        wchar_t buf[4096] = {}; DWORD sz = sizeof(buf);
        if (RegQueryValueExW(hk, L"Title", nullptr, nullptr,
                reinterpret_cast<BYTE*>(buf), &sz) == ERROR_SUCCESS)
            item.title = buf;
        sz = sizeof(buf);
        if (RegQueryValueExW(hk, L"Body", nullptr, nullptr,
                reinterpret_cast<BYTE*>(buf), &sz) == ERROR_SUCCESS)
            item.body = buf;
        RegCloseKey(hk);
        return item;
...
private:
    ...
    void RecordSubmission() { m_recentSubmissions++; }
    ...
    void EnrichItem(FeedbackItem& item) {
        ...
        if (pfn) pfn(&osv);
        item.osVersion = std::to_wstring(osv.dwMajorVersion) + L"." +
    ...
    static std::wstring BuildJson(const FeedbackItem& item) {
        return L"{\"category\":" + std::to_wstring(static_cast<int>(item.category)) +
               L",\"rating\":" + std::to_wstring(item.rating) +
               L",\"title\":\"" + Escape(item.title) + L"\"" +
               L",\"body\":\"" + Escape(item.body) + L"\"" +
               L",\"version\":\"" + item.version + L"\"" +
               L",\"os\":\"" + item.osVersion + L"\"}";
    }
};
