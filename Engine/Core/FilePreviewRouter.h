...
inline const char* PreviewHandlerName(PreviewHandler h) noexcept {
    switch (h) {
    case PreviewHandler::Internal:     return "Internal";
    case PreviewHandler::WICFallback:  return "WICFallback";
    case PreviewHandler::ShellDefault: return "ShellDefault";
    case PreviewHandler::Plugin:       return "Plugin";
    case PreviewHandler::External:     return "External";
...
inline const char* RoutingPriorityName(RoutingPriority p) noexcept {
    switch (p) {
    case RoutingPriority::Speed:           return "Speed";
    case RoutingPriority::Quality:         return "Quality";
    case RoutingPriority::Both:            return "Both";
    case RoutingPriority::CacheFirst:      return "CacheFirst";
    case RoutingPriority::ForceRegenerate: return "ForceRegenerate";
    default:                               return "Unknown";
    }
}
...
public:
    ...
    PreviewHandler GetBestHandler(const std::wstring& /*filePath*/,
        RoutingPriority priority) const noexcept {
    ...
    bool RegisterHandler(const std::string& extension, PreviewHandler handler) {
        if (m_registeredCount >= MAX_REGISTRATIONS) return false;
    ...
    uint32_t GetRegisteredCount() const noexcept { return m_registeredCount; }
...
private:
    ...
    double EstimateDecodeTime(PreviewHandler handler) const noexcept {
        switch (handler) {
        case PreviewHandler::Internal:     return 12.0;
        case PreviewHandler::WICFallback:  return 25.0;
        case PreviewHandler::ShellDefault: return 40.0;
        case PreviewHandler::Plugin:       return 30.0;
        case PreviewHandler::External:     return 80.0;
        default:                           return 50.0;
        }
    }
