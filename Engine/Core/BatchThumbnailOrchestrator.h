  ...
  public:
    ...
    bool Initialize(const BatchConfig& config = {})
    {
        ...
        if (m_config.maxConcurrency == 0) {
            ...
            m_config.maxConcurrency =
                (si.dwNumberOfProcessors > 1) ? static_cast<uint32_t>(si.dwNumberOfProcessors - 1) : 1;
        }
    ...
    bool WaitForAll(uint32_t timeoutMs = INFINITE)
    {
        ...
        return m_completionCv.wait_until(lock, deadline, [this]() {
            return m_stats.totalCompleted + m_stats.totalFailed + m_stats.totalCancelled >= m_stats.totalSubmitted;
        });
    }

    /// Set callback for individual item completion
    void SetCompletionCallback(BatchCompletionCallback cb)
    {
        m_completionCb = std::move(cb);
    }

    /// Set callback for progress updates
    void SetProgressCallback(OrchestratorProgressCallback cb)
    {
    ...
    void WorkerLoop(uint32_t /*threadId*/)
    {
        while (m_running.load()) {
            ...
            if (result.success) {
                ...
                double total =
                    m_totalDecodeTimeMs.fetch_add(static_cast<uint64_t>(result.decodeTimeMs * 1000)) / 1000.0;
            ...
            if (m_progressCb) {
                m_progressCb(m_stats.totalCompleted + m_stats.totalFailed, m_stats.totalSubmitted);
            }
