...
enum class DetectedFormat : uint8_t {
    Unknown = 0,
    JPEG, PNG, GIF89, GIF87, BMP, TIFF_LE, TIFF_BE,
    WebP, AVIF, HEIF, HEIC, JXL,
    PDF, PSD, PSB,
    ZIP, RAR4, RAR5, SevenZ, XZ, GZIP, ZSTD, LZ4,
    EXE_PE, ELF, MACHO,
...
public:
    ...
    FormatMatch Detect(const uint8_t* headerBytes, uint32_t length) const {
        ...
        if (length >= 4 && headerBytes[0] == 0x89 && headerBytes[1] == 0x50 &&
            headerBytes[2] == 0x4E && headerBytes[3] == 0x47)
            return { DetectedFormat::PNG, SignatureConfidence::Definitive, "image/png", ".png", false };

        // GIF89a
        if (length >= 6 && headerBytes[0] == 'G' && headerBytes[1] == 'I' &&
            headerBytes[2] == 'F' && headerBytes[3] == '8' && headerBytes[4] == '9')
        ...
        if (length >= 12 && headerBytes[0] == 'R' && headerBytes[1] == 'I' &&
            headerBytes[2] == 'F' && headerBytes[3] == 'F' &&
            headerBytes[8] == 'W' && headerBytes[9] == 'E' &&
            headerBytes[10] == 'B' && headerBytes[11] == 'P')
            return { DetectedFormat::WebP, SignatureConfidence::Definitive, "image/webp", ".webp", false };

        // PDF: 25 50 44 46
        if (length >= 4 && headerBytes[0] == '%' && headerBytes[1] == 'P' &&
        ...
        if (length >= 4 && headerBytes[0] == 0x50 && headerBytes[1] == 0x4B &&
        ...
        if (length >= 7 && headerBytes[0] == 0x52 && headerBytes[1] == 0x61 &&
        ...
        if (length >= 4 && headerBytes[0] == 0x37 && headerBytes[1] == 0x7A &&
            headerBytes[2] == 0xBC && headerBytes[3] == 0xAF)
            return { DetectedFormat::SevenZ, SignatureConfidence::Definitive, "application/x-7z-compressed", ".7z", false };

        // TIFF LE: 49 49 2A 00
        if (length >= 4 && headerBytes[0] == 0x49 && headerBytes[1] == 0x49 &&
            headerBytes[2] == 0x2A && headerBytes[3] == 0x00)
            return { DetectedFormat::TIFF_LE, SignatureConfidence::Definitive, "image/tiff", ".tiff", false };

        // TIFF BE: 4D 4D 00 2A
        if (length >= 4 && headerBytes[0] == 0x4D && headerBytes[1] == 0x4D &&
            headerBytes[2] == 0x00 && headerBytes[3] == 0x2A)
            return { DetectedFormat::TIFF_BE, SignatureConfidence::Definitive, "image/tiff", ".tiff", false };

        // PSD: 38 42 50 53
        if (length >= 4 && headerBytes[0] == '8' && headerBytes[1] == 'B' &&
            headerBytes[2] == 'P' && headerBytes[3] == 'S')
            return { DetectedFormat::PSD, SignatureConfidence::Definitive, "image/vnd.adobe.photoshop", ".psd", false };

        // OpenEXR: 76 2F 31 01
        if (length >= 4 && headerBytes[0] == 0x76 && headerBytes[1] == 0x2F &&
            headerBytes[2] == 0x31 && headerBytes[3] == 0x01)
    ...
    static bool ExtensionMatchesDetected(const std::wstring& path, DetectedFormat fmt) {
        if (path.empty() || fmt == DetectedFormat::Unknown) return false;
        auto ext = path.substr(path.rfind(L'.') + 1);
        for (auto& c : ext) c = static_cast<wchar_t>(towlower(c));
        switch (fmt) {
        case DetectedFormat::JPEG: return ext == L"jpg" || ext == L"jpeg";
        case DetectedFormat::PNG:  return ext == L"png";
        case DetectedFormat::PDF:  return ext == L"pdf";
        case DetectedFormat::ZIP:  return ext == L"zip";
        default:                   return true;
        }
    }
    ...
    static const char* FormatName(DetectedFormat f) {
        switch (f) {
        case DetectedFormat::JPEG:    return "JPEG";
        case DetectedFormat::PNG:     return "PNG";
        case DetectedFormat::GIF89:   return "GIF89a";
        case DetectedFormat::BMP:     return "BMP";
        case DetectedFormat::WebP:    return "WebP";
        case DetectedFormat::PDF:     return "PDF";
        case DetectedFormat::ZIP:     return "ZIP";
        case DetectedFormat::RAR4:    return "RAR4";
        case DetectedFormat::SevenZ:  return "7-Zip";
        case DetectedFormat::TIFF_LE: return "TIFF-LE";
        case DetectedFormat::TIFF_BE: return "TIFF-BE";
        case DetectedFormat::PSD:     return "PSD";
        case DetectedFormat::EXR:     return "OpenEXR";
