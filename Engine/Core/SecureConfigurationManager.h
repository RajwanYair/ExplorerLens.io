...
public:
    ...
    bool Get(const std::string& key, std::string& plaintext) {
        ...
        plaintext.assign(it->second.encryptedValue.begin(),
                         it->second.encryptedValue.end());
        ++m_stats.readsOk;
        return true;
    }

    SecureConfigBackend GetBackend() const { return m_backend; }
    const SecureConfigManagerStats& GetStats() const { return m_stats; }
