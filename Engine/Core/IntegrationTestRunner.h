...
class IntegrationTestRunner
{
  public:
    ...
    using DecodeCallback =
        std::function<bool(const std::wstring&, uint32_t, uint32_t, std::vector<uint8_t>&, uint32_t&, uint32_t&)>;
    ...
    inline void AddTestCasesFromDirectory(const std::wstring& dir, uint32_t targetW, uint32_t targetH)
    {
        WIN32_FIND_DATAW fd{};
    ...
    inline std::wstring GenerateReport(const std::vector<TestResult>& results) const
    {
        ...
        for (auto& r : results) {
            ws << L"| " << r.name << L" | " << (r.passed ? L"PASS" : L"**FAIL**") << L" | " << r.actualWidth << L" | "
               << r.actualHeight << L" | " << r.timeMs << L" | " << r.outputSize << L" |\n";
        }
    ...
    inline bool SaveReport(const std::wstring& path, const std::vector<TestResult>& results) const
    {
  ...
  private:
    ...
    inline TestResult ExecuteTestCase(const TestCase& tc, const DecodeCallback& cb) const
    {
        ...
        HANDLE hFile = CreateFileW(tc.inputFile.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                                   FILE_ATTRIBUTE_NORMAL, nullptr);
        if (hFile == INVALID_HANDLE_VALUE) {
            result.passed = !tc.shouldSucceed;
            result.error =
                tc.shouldSucceed ? L"File not found or inaccessible" : L"Expected failure: file inaccessible";
            return result;
        }
        CloseHandle(hFile);
        ...
        auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(tc.maxTimeMs);
        ...
        if (tc.shouldSucceed) {
            if (!ok) {
                ...
                if (outputRGBA.size() != expectedSize) {
                    result.passed = false;
                    result.error = L"RGBA buffer size mismatch: expected " + std::to_wstring(expectedSize) + L" got "
                                   + std::to_wstring(outputRGBA.size());
                } else if (tc.expectedWidth > 0 && outW != tc.expectedWidth) {
                    result.passed = false;
                    result.error = L"Width mismatch: expected " + std::to_wstring(tc.expectedWidth) + L" got "
                                   + std::to_wstring(outW);
                } else if (tc.expectedHeight > 0 && outH != tc.expectedHeight) {
                    result.passed = false;
                    result.error = L"Height mismatch: expected " + std::to_wstring(tc.expectedHeight) + L" got "
                                   + std::to_wstring(outH);
                } else {
                    result.passed = true;
                }
