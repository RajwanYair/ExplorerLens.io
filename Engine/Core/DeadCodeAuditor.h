...
class DeadCodeAuditor
{
  public:
    ...
    enum class DeadCodeAuditSeverity {
        Info,
        Warning,
        Error,
        COUNT
    ...
    static std::vector<AuditFinding> RunAudit()
    {
        return {
            ...
            {L"LENSArchive.h CBuffer", AuditCategory::CommentedCode, DeadCodeAuditSeverity::Info,
             L"Commented-out template class", true},
        };
    }
    ...
    {
        return ResolvedCount() == RunAudit().size();
    }
};
