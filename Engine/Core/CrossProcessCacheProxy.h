...
public:
    ...
    explicit CrossProcessCacheProxy(const ProxyConfig& cfg) noexcept
        : m_config(cfg) {}

    [[nodiscard]] CacheProxyMode GetMode()        const noexcept { return m_mode; }
    [[nodiscard]] ProxyState     GetState()       const noexcept { return m_state; }
    [[nodiscard]] bool           IsConnected()    const noexcept { return m_state == ProxyState::Connected; }
    [[nodiscard]] const ProxyConfig& GetConfig()  const noexcept { return m_config; }

    void SetMode(CacheProxyMode mode) noexcept { m_mode = mode; }
    ...
    bool Disconnect() noexcept {
        if (m_state == ProxyState::Disconnected) return false;
    ...
    bool Put(const std::string& key, const void* data, size_t size) noexcept {
        if (key.empty() || !data || size == 0) return false;
        if (m_state != ProxyState::Connected) return false;
        if (size > MAX_VALUE_SIZE_BYTES) return false;
    ...
    ProxyCacheResult Get(const std::string& key) noexcept {
        ProxyCacheResult r;
        if (key.empty()) { r.error = "Empty key"; return r; }
        if (m_state != ProxyState::Connected) { r.error = "Not connected"; return r; }
        auto it = m_store.find(key);
        if (it == m_store.end()) { r.error = "Key not found: " + key; return r; }
        r.success = true;
        r.data    = it->second;
        return r;
    }
    ...
    [[nodiscard]] size_t GetEntryCount() const noexcept { return m_store.size(); }

    static const wchar_t* GetModeName(CacheProxyMode mode) noexcept {
        switch (mode) {
            case CacheProxyMode::SharedMemory: return L"SharedMemory";
            case CacheProxyMode::NamedPipe:    return L"NamedPipe";
            case CacheProxyMode::COM:          return L"COM";
            case CacheProxyMode::Auto:         return L"Auto";
    ...
    static const wchar_t* GetStateName(ProxyState state) noexcept {
        switch (state) {
            case ProxyState::Disconnected: return L"Disconnected";
            case ProxyState::Connecting:   return L"Connecting";
            case ProxyState::Connected:    return L"Connected";
            case ProxyState::Error:        return L"Error";
            default:                       return L"Unknown";
        }
    }
