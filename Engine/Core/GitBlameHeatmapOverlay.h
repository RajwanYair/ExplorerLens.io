...
enum class HeatmapColorScheme { HotCold, Viridis, Grayscale };
...
public:
    ...
    BlameHeatmapData  ComputeHeat(const std::string& path) const {
        BlameHeatmapData d; d.path = path;
    ...
    float  ComputeScore(uint32_t ageDays) const {
        if (m_cfg.maxAgeDays == 0) return 0.0f;
        float ratio = static_cast<float>(ageDays) / static_cast<float>(m_cfg.maxAgeDays);
        return 1.0f - (ratio > 1.0f ? 1.0f : ratio);
    }

    const BlameHeatmapConfig& GetConfig() const { return m_cfg; }
    void  SetScheme(HeatmapColorScheme s)        { m_cfg.scheme = s; }
