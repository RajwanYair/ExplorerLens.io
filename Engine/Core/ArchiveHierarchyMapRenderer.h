...
class ArchiveHierarchyMapRenderer
{
  public:
    ...
    void AddEntry(const std::wstring& path, uint64_t fileSize)
    {
        ...
        for (size_t i = 0; i < parts.size(); ++i) {
            ...
            if (child == SIZE_MAX) {
                child = CreateNode(parts[i], parent, isLast ? false : true, isLast ? fileSize : 0);
            }
            parent = child;
        }
    }
    ...
    static uint32_t ColorForDepth(uint32_t depth)
    {
        ...
        static const uint32_t palette[] = {0xFF3498DB, 0xFF2ECC71, 0xFFE67E22, 0xFF9B59B6,
                                           0xFFE74C3C, 0xFF1ABC9C, 0xFFF39C12, 0xFF2980B9};
  ...
  private:
    ...
    size_t CreateNode(const std::wstring& name, size_t parentIdx, bool isDir, uint64_t size)
    {
    ...
    void LayoutChildren(size_t idx, const TreemapRect& bounds)
    {
        ...
        std::sort(sorted.begin(), sorted.end(),
                  [this](size_t a, size_t b) { return m_nodes[a].totalSize > m_nodes[b].totalSize; });
        ...
        for (size_t ci : sorted) {
            float frac = static_cast<float>(static_cast<double>(m_nodes[ci].totalSize) / static_cast<double>(total));
