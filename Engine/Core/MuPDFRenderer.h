...
class MuPDFRenderer {
public:
    static MuPDFRenderer& Instance() { static MuPDFRenderer s; return s; }
    ...
    uint32_t BytesPerPixel() const {
        ...
        case MuPDFColorMode::Grayscale: return 1;
        case MuPDFColorMode::RGB:       return 3;
        case MuPDFColorMode::BGRA:      return 4;
        case MuPDFColorMode::CMYK:      return 4;
        default:                        return 4;
        }
    }
    ...
    MuPDFPageResult RenderPage(uint32_t pageIndex) {
        ...
        if (!m_initialized || pageIndex >= m_pageCount) return result;
    ...
    const MuPDFRenderConfig& GetConfig() const { return m_config; }

    bool Validate() const {
        if (m_config.targetDPI < 36 || m_config.targetDPI > 1200) return false;
        if (m_config.maxWidth == 0 || m_config.maxHeight == 0) return false;
        if (m_config.gammaCorrection < 0.1f || m_config.gammaCorrection > 5.0f) return false;
