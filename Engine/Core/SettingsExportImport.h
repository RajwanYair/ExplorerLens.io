...
{
    return static_cast<SettingsSection>(static_cast<uint32_t>(a) | static_cast<uint32_t>(b));
}
...
class SettingsExportImport
{
  public:
    ...
    SettingsIOResult ExportToFile(const wchar_t* filePath, SettingsSection sections = SettingsSection::All)
    {
        ...
        strftime(meta.exportDate, sizeof(meta.exportDate), "%Y-%m-%dT%H:%M:%S", &tmBuf);
    ...
    SettingsIOResult ImportFromFile(const wchar_t* filePath)
    {
        ...
        if (content.find("\"metadata\"") == std::string::npos || content.find("\"settings\"") == std::string::npos) {
