...
struct LayoutMetrics {
    ...
    int ToPhysical(int dp) const { return static_cast<int>(dp * scaleFactor + 0.5f); }

    // Scale a dimension relative to 96 DPI baseline
    int Scale(int baseline) const { return ToPhysical(baseline); }
};
...
public:
    ...
    RECT ControlRect(int xDp, int yDp, int wDp, int hDp) const {
        return RECT{
            m_metrics.ToPhysical(xDp),
            m_metrics.ToPhysical(yDp),
            m_metrics.ToPhysical(xDp + wDp),
            m_metrics.ToPhysical(yDp + hDp)
        };
    }

    // Whether to use compact (single-column) layout
    bool IsCompact() const { return m_metrics.breakpoint == LayoutBreakpoint::Compact; }

private:
    ResponsiveLayoutManager() { m_metrics.scaleFactor = 1.f; }
    ...
    LayoutBreakpoint ComputeBreakpoint(HWND hwnd) const {
        ...
        if (effectivePx < 800)  return LayoutBreakpoint::Compact;
        if (effectivePx < 1280) return LayoutBreakpoint::Normal;
        if (effectivePx < 1920) return LayoutBreakpoint::Wide;
    ...
    void FireCallbacks() {
        for (auto& fn : m_cbs) fn(m_metrics);
    }
