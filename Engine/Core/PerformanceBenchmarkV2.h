...
class PerformanceBenchmarkV2
{
  public:
    ...
    static uint32_t GetBenchmarkTypeCount()
    {
        return static_cast<uint32_t>(BenchmarkType::COUNT);
    }

    /// Record a set of timing samples for a benchmark
    BenchmarkResult ComputeStats(const std::wstring& label, BenchmarkType type, const std::vector<double>& samplesMs);
