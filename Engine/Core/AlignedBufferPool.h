...
inline const char* TierName(BufferTier tier) {
    switch (tier) {
        case BufferTier::Small:  return "64KB";
        case BufferTier::Medium: return "256KB";
        case BufferTier::Large:  return "1MB";
        case BufferTier::Huge:   return "4MB";
...
inline BufferTier TierForSize(size_t requestedBytes) {
    if (requestedBytes <= ALIGNED_POOL_TIER_SIZES[0]) return BufferTier::Small;
    if (requestedBytes <= ALIGNED_POOL_TIER_SIZES[1]) return BufferTier::Medium;
    if (requestedBytes <= ALIGNED_POOL_TIER_SIZES[2]) return BufferTier::Large;
    if (requestedBytes <= ALIGNED_POOL_TIER_SIZES[3]) return BufferTier::Huge;
...
public:
    ...
    FreeNode* Pop() {
        ...
        while (oldHead) {
            if (m_head.compare_exchange_weak(oldHead, oldHead->next,
    ...
    size_t Count() const { return m_count.load(std::memory_order_relaxed); }
...
public:
    ...
    AlignedPooledBuffer(void* data, size_t capacity, BufferTier tier,
                 AlignedBufferPool* pool, FreeNode* node)
        : m_data(data), m_capacity(capacity), m_size(0),
          m_tier(tier), m_pool(pool), m_node(node) {}
    ...
    AlignedPooledBuffer(AlignedPooledBuffer&& other) noexcept
        : m_data(other.m_data), m_capacity(other.m_capacity),
          m_size(other.m_size), m_tier(other.m_tier),
          m_pool(other.m_pool), m_node(other.m_node) {
    ...
    BufferTier Tier() const { return m_tier; }

    void SetSize(size_t sz) { m_size = (sz <= m_capacity) ? sz : m_capacity; }

    /// Typed access to the buffer
    template<typename T>
    T* As() { return static_cast<T*>(m_data); }
...
public:
    ...
    /// Get pool statistics
    const AlignedPoolStats& GetStats() const { return m_stats; }

    /// Get free-list depth for a tier
    size_t FreeCount(BufferTier tier) const {
        if (tier >= BufferTier::COUNT) return 0;
