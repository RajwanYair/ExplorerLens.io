...
class LibWebPConfig
{
  public:
    ...
    LibWebPConfig() : useDynamicCRT(true), enableSIMD(true)
    {
        ...
        m_config.buildScript = "build-scripts/external-libs/Build-LibWebP-NMake.ps1";
    ...
    const LibraryBuildConfig& GetConfig() const
    {
        return m_config;
    }

    /// Check if libwebp was built with correct CRT (/MD)
    bool IsCorrectCRT() const
    {
    ...
    LinkerFlags GetRequiredLinkerFlags() const
    {
        ...
        if (m_config.crt == CRTLinkage::StaticMT) {
            ...
            flags.description = "/NODEFAULTLIB:LIBCMT required (libwebp /MT in /MD host)";
        } else {
