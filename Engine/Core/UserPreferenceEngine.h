...
class UserPreferenceEngine
{
  public:
    ...
    PreferenceStats ComputeStats() const
    {
        ...
        for (uint8_t i = 0; i < static_cast<uint8_t>(PreferenceKey::COUNT); ++i) {
            ...
            switch (m_prefs[i].source) {
                case PreferenceSource::UserSetting:
                    stats.userOverrides++;
                    break;
                case PreferenceSource::EnterprisePolicy:
                    stats.policyOverrides++;
                    break;
                default:
                    stats.defaultsUsed++;
                    break;
    ...
    {
        return static_cast<size_t>(PreferenceKey::COUNT);
    }
    static size_t SourceCount()
    {
        return static_cast<size_t>(PreferenceSource::COUNT);
    }
