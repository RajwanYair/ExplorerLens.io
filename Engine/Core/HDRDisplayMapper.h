...
class HDRDisplayMapper
{
  public:
    ...
    inline std::vector<uint8_t> ToneMapToSDR(const float* hdrPixels, uint32_t width, uint32_t height, uint32_t channels,
                                             DisplayToneMapOp op = DisplayToneMapOp::ACES) const
    {
        size_t pixelCount = static_cast<size_t>(width) * height;
        std::vector<uint8_t> sdrOutput(pixelCount * 3);
        if (!hdrPixels || pixelCount == 0 || channels == 0)
            return sdrOutput;
    ...
    {
        switch (op) {
            case DisplayToneMapOp::Reinhard:
                return ReinhardToneMap(pixel);
            case DisplayToneMapOp::ReinhardExtended:
                return ReinhardExtendedToneMap(pixel, 4.0f);
            case DisplayToneMapOp::ACES:
                return ACESToneMap(pixel);
            case DisplayToneMapOp::Uncharted2:
                return Uncharted2ToneMap(pixel);
            case DisplayToneMapOp::Linear:
                return pixel;
            default:
                return ACESToneMap(pixel);
        }
    ...
    {
        if (!hdrPixels || width == 0 || height == 0 || channels == 0)
            return 1.0f;
        size_t pixelCount = static_cast<size_t>(width) * height;
        ...
        for (size_t i = 0; i < pixelCount; ++i) {
            ...
            if (channels > 1)
                luminance += 0.7152f * hdrPixels[idx + 1];
            if (channels > 2)
                luminance += 0.0722f * hdrPixels[idx + 2];
  ...
  private:
    ...
    inline ColorPixel ReinhardExtendedToneMap(ColorPixel p, float whitePoint) const
    {
        ...
        return {p.r * (1.0f + p.r / wp2) / (1.0f + p.r), p.g * (1.0f + p.g / wp2) / (1.0f + p.g),
    ...
    {
        auto uc2 = [this](float x) -> float {
            return Uncharted2Func(x);
        };
        float exposureBias = 2.0f;
        float whiteScale = 1.0f / Uncharted2Func(11.2f);
        return {uc2(exposureBias * p.r) * whiteScale, uc2(exposureBias * p.g) * whiteScale,
