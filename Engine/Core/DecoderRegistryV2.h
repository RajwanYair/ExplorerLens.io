...
public:
    ...
    virtual bool DecodeToBuffer(const wchar_t* /*filePath*/, uint8_t* /*buffer*/,
...
public:
    ...
    bool RegisterDecoder(const char* name, DecoderCreator factory,
        ...
        if (m_factories.count(name)) return false; // Already registered
    ...
    DecoderPtr FindBestDecoder(const char* extension) {
        ...
        if (it == m_extensionIndex.end() || it->second.empty()) return nullptr;
        ...
        if (bestName.empty()) return nullptr;
    ...
    std::vector<DecoderPtr> FindDecoders(const char* extension) {
        ...
        auto it = m_extensionIndex.find(lowerExt);
        if (it == m_extensionIndex.end()) return result;
