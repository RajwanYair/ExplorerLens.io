...
enum class HolographicDisplayTarget { Standard2D, HoloLens2, QuestPro, SpatialIsland };
enum class DepthCueMode             { None, DepthGradient, EdgeHighlight, DepthFog };
...
public:
    ...
    HolographicProjectionResult Project(const HolographicProjectionRequest& req) const {
        ...
        if (stereo) result.rightRGBA.assign(frameSize, 0xE8);
        return result;
    }

    static std::string TargetName(HolographicDisplayTarget t) noexcept {
        switch (t) {
        case HolographicDisplayTarget::Standard2D:   return "Standard2D";
        case HolographicDisplayTarget::HoloLens2:    return "HoloLens2";
        case HolographicDisplayTarget::QuestPro:     return "QuestPro";
        case HolographicDisplayTarget::SpatialIsland: return "SpatialIsland";
        }
        return "Unknown";
    }
};
