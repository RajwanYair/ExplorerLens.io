...
enum class SyncMode     { Disabled, VSync, AdaptiveSync, LowLatency };
enum class RefreshRate  { Hz30 = 30, Hz60 = 60, Hz90 = 90, Hz120 = 120, Hz144 = 144 };
...
struct FrameSyncResult {
    ...
    bool Ok() const noexcept { return presented; }
};
...
public:
    explicit FrameRateSynchronizer(FrameSyncConfig config = {})
        : m_config(std::move(config)) {
        m_frameTimeMs = 1000.0 / static_cast<double>(static_cast<int>(m_config.targetHz));
    }

    void SetPresentCallback(FramePresentCallback cb) { m_callback = std::move(cb); }
    ...
    double TargetFrameTimeMs() const noexcept { return m_frameTimeMs; }
    int64_t FrameNumber()     const noexcept { return m_frameNumber.load(); }
    const FrameSyncConfig& Config() const noexcept { return m_config; }
    void SetMode(SyncMode mode) noexcept { m_config.mode = mode; }

    static std::string ModeName(SyncMode m) noexcept {
        switch (m) {
        case SyncMode::Disabled:     return "Disabled";
        case SyncMode::VSync:        return "VSync";
        case SyncMode::AdaptiveSync: return "AdaptiveSync";
        case SyncMode::LowLatency:   return "LowLatency";
        }
        return "Unknown";
    }
