...
class ParallelBatchProcessor
{
  public:
    ...
    {
        switch (p) {
            case BatchPolicy::RoundRobin:
                return L"Round Robin";
            case BatchPolicy::FormatGrouped:
                return L"Format Grouped";
            case BatchPolicy::SizeOrdered:
                return L"Size Ordered";
            case BatchPolicy::PriorityBased:
                return L"Priority Based";
    ...
    static bool ValidateConfig(const BatchDecodeConfig& cfg)
    {
        ...
        if (cfg.maxQueueDepth == 0)
            return false;
        if (cfg.batchSize == 0 || cfg.batchSize > cfg.maxQueueDepth)
            return false;
        if (cfg.timeoutPerFileMs < 500)
            return false;
        return true;
    }
