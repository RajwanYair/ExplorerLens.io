...
inline const char* KpiName(PerfKPI kpi) {
    switch (kpi) {
        case PerfKPI::SingleThumbnailMs:      return "SingleThumbnailMs";
        case PerfKPI::SingleThumbnailP95Ms:   return "SingleThumbnailP95Ms";
        case PerfKPI::BatchThroughputImgSec:  return "BatchThroughputImgSec";
        case PerfKPI::CacheHitMs:             return "CacheHitMs";
        case PerfKPI::ColdStartMs:            return "ColdStartMs";
        case PerfKPI::MemoryPeakMB:           return "MemoryPeakMB";
        case PerfKPI::GpuFrameMs:             return "GpuFrameMs";
        case PerfKPI::DecoderInitMs:          return "DecoderInitMs";
    }
    return "Unknown";
}
...
struct GateResult {
    ...
    bool Passed() const { return overall != GateVerdict::Fail; }
};
...
public:
    PerfRegressionGate() { SetDefaultThresholds(); }
    ...
    void SetDefaultThresholds() {
        m_thresholds[PerfKPI::SingleThumbnailMs]     = {PerfKPI::SingleThumbnailMs,     15.0, 25.0, ThresholdDirection::LowerIsBetter};
        m_thresholds[PerfKPI::SingleThumbnailP95Ms]  = {PerfKPI::SingleThumbnailP95Ms,  25.0, 40.0, ThresholdDirection::LowerIsBetter};
        m_thresholds[PerfKPI::BatchThroughputImgSec] = {PerfKPI::BatchThroughputImgSec, 200.0, 150.0, ThresholdDirection::HigherIsBetter};
    ...
    GateResult Evaluate(const std::map<PerfKPI, double>& current) const {
        ...
        for (auto& [kpi, value] : current) {
            ...
            switch (r.verdict) {
                case GateVerdict::Pass: gate.passCount++; r.detail = "Within limits"; break;
                case GateVerdict::Warn: gate.warnCount++; r.detail = "Near threshold"; break;
                case GateVerdict::Fail: gate.failCount++; r.detail = "Threshold exceeded"; break;
            }
            gate.results.push_back(r);
        }
    ...
    TrendStats ComputeTrend(PerfKPI kpi) const {
        ...
        for (auto& s : samples) vals.push_back(s.value);
        ...
        if (vals.size() > 1) {
            double sq_sum = 0;
            for (auto v : vals) sq_sum += (v - ts.mean) * (v - ts.mean);
            ...
            if (den > 0) ts.trendSlope = num / den;
        }
    ...
    static std::string FormatReport(const GateResult& gate) {
        ...
        rpt += "Pass: " + std::to_string(gate.passCount)
            + "  Warn: " + std::to_string(gate.warnCount)
            + "  Fail: " + std::to_string(gate.failCount) + "\n\n";

        for (auto& r : gate.results) {
            rpt += "[" + VerdictLabel(r.verdict) + "] "
                + KpiName(r.kpi) + ": "
                + FormatDouble(r.measured);
            if (r.baseline > 0)
                rpt += " (baseline=" + FormatDouble(r.baseline)
                    + ", delta=" + FormatDouble(r.regressionPct) + "%)";
            rpt += " — " + r.detail + "\n";
        }
        return rpt;
    }

    const std::map<PerfKPI, KpiThreshold>& Thresholds() const { return m_thresholds; }
...
private:
    ...
    static std::string VerdictLabel(GateVerdict v) {
        switch (v) {
            ...
            case GateVerdict::Warn: return "WARN";
            case GateVerdict::Fail: return "FAIL";
        }
        return "?";
    }
