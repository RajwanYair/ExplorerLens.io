...
{
    switch (m) {
        case FixApplicationMode::Auto:
            return "Auto";
        case FixApplicationMode::Manual:
            return "Manual";
        case FixApplicationMode::PreserveOriginal:
            return "PreserveOriginal";
        case FixApplicationMode::BatchMode:
            return "BatchMode";
        case FixApplicationMode::Silent:
            return "Silent";
        default:
            return "Unknown";
    }
  ...
  public:
    ...
    bool ApplyFix(ExifOrientation orientation, uint32_t& width, uint32_t& height)
    {
        ...
        if (orientation == ExifOrientation::Rotate90 || orientation == ExifOrientation::Rotate270
            || orientation == ExifOrientation::Transpose || orientation == ExifOrientation::Transverse) {
    ...
    void SetMode(FixApplicationMode mode) noexcept
    {
        m_mode = mode;
    }

    /// Get current mode
    FixApplicationMode GetMode() const noexcept
    {
        return m_mode;
    }

    /// Get total fix count
    uint64_t GetFixCount() const noexcept
    {
        return m_fixCount;
    }
