...
void SIMDAccelerator::DetectCPUFeatures()
{
    ...
    // Determine max level
    if (m_caps.hasAVX512)
        m_maxLevel = SIMDLevel::AVX512;
    else if (m_caps.hasAVX2)
        m_maxLevel = SIMDLevel::AVX2;
    else if (m_caps.hasAVX)
        m_maxLevel = SIMDLevel::AVX;
    else if (m_caps.hasSSE42)
        m_maxLevel = SIMDLevel::SSE42;
    else if (m_caps.hasSSE41)
        m_maxLevel = SIMDLevel::SSE41;
    else if (m_caps.hasSSE2)
        m_maxLevel = SIMDLevel::SSE2;
    else if (m_caps.hasNEON)
        m_maxLevel = SIMDLevel::NEON;
...
//------------------------------------------------------------------------------
bool SIMDAccelerator::ResizeBilinear(const uint8_t* src, uint32_t srcW, uint32_t srcH, uint8_t* dst, uint32_t dstW,
                                     uint32_t dstH, uint32_t channels)
{
    ...
    for (uint32_t y = 0; y < dstH; y++) {
        ...
        for (uint32_t x = 0; x < dstW; x++) {
            ...
            for (uint32_t c = 0; c < channels; c++) {
                ...
                dst[(y * dstW + x) * channels + c] = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, val)));
            }
        }
    }
    return true;
...
bool SIMDAccelerator::AlphaBlend(const uint8_t* fg, const uint8_t* bg, uint8_t* dst, uint32_t width, uint32_t height)
{
    if (!fg || !bg || !dst)
        return false;
...
bool SIMDAccelerator::ColorConvertRGBAToBGRA(const uint8_t* src, uint8_t* dst, uint32_t pixelCount)
{
    if (!src || !dst)
        return false;
...
bool SIMDAccelerator::ApplySharpen(uint8_t* data, uint32_t width, uint32_t height, uint32_t channels, float strength)
{
...
const wchar_t* SIMDAccelerator::GetOpName(SIMDOp op)
{
    switch (op) {
        ...
        case SIMDOp::AlphaBlend:
            return L"AlphaBlend";
        case SIMDOp::Sharpen:
            return L"Sharpen";
        case SIMDOp::GammaCorrect:
            return L"GammaCorrect";
        case SIMDOp::BilinearFilter:
            return L"BilinearFilter";
        case SIMDOp::BoxFilter:
            return L"BoxFilter";
        case SIMDOp::Transpose:
            return L"Transpose";
        default:
            return L"Unknown";
    }
