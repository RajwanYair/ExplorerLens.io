...
enum class DiffSeverity : uint8_t {
    Identical = 0,
    None = 0,
    Minor = 1,
    Moderate = 2,
...
{
    switch (a) {
        case DiffAlgorithm::PixelWise:
            return "PixelWise";
        case DiffAlgorithm::SSIM:
            return "SSIM";
        case DiffAlgorithm::Perceptual:
            return "Perceptual";
        case DiffAlgorithm::Histogram:
            return "Histogram";
        case DiffAlgorithm::EdgeDetect:
            return "EdgeDetect";
        default:
            return "Unknown";
    }
...
{
  public:
    BatchExportResult ExportBatch(const std::vector<std::wstring>& files, const BatchExportConfig& /*cfg*/)
    {
        BatchExportResult r;
    ...
    DiffResult ComputeDiff(const uint8_t* a, const uint8_t* b, uint32_t w, uint32_t h,
                           DiffAlgorithm algo = DiffAlgorithm::PixelWise) const
    {
        DiffResult r;
        r.algorithm = algo;
        if (!a || !b || w == 0 || h == 0) {
            r.severity = DiffSeverity::Critical;
            return r;
        }
        uint32_t diff = 0;
        ...
        r.severity = (ratio < 0.01f)   ? DiffSeverity::Identical
                     ...
                     : (ratio < 0.40f) ? DiffSeverity::Major
...
class ThumbnailDiffEngine
{
  public:
    ...
    void SetThresholds(const DiffThresholdConfig& cfg) noexcept
    {
        m_cfg = cfg;
    }
    const DiffThresholdConfig& GetThresholds() const noexcept
    {
        return m_cfg;
    }
    DiffResult Compare(const uint8_t* a, const uint8_t* b, uint32_t w, uint32_t h,
                       DiffAlgorithm algo = DiffAlgorithm::PixelWise) const
    {
        DiffResult r;
        r.algorithm = algo;
        ...
        r.severity = (ratio < m_cfg.minorThreshold)      ? DiffSeverity::Identical
                     ...
                     : (ratio < m_cfg.criticalThreshold) ? DiffSeverity::Major
                                                         : DiffSeverity::Critical;
        return r;
    }
...
{
    switch (s) {
        case CompareSource::Memory:
            return "Memory";
        case CompareSource::File:
            return "File";
        case CompareSource::Cache:
            return "Cache";
        case CompareSource::Network:
            return "Network";
        default:
            return "Unknown";
    }
...
class ThumbnailCompareView
{
  public:
    ...
    bool Compare(const uint8_t* a, uint32_t aw, uint32_t ah, const uint8_t* b, uint32_t bw, uint32_t bh,
                 CompareMode mode = CompareMode::SideBySide) noexcept
    {
        if (!a || !b || aw == 0 || ah == 0 || aw != bw || ah != bh)
            return false;
        ...
        m_result.matchPercent =
            (diff == 0) ? 100.0f : (1.0f - static_cast<float>(diff) / static_cast<float>(total)) * 100.0f;
        m_result.mode = mode;
        m_hasResult = true;
        return true;
    }
    CompareResult GetResult() const noexcept
    {
        return m_result;
    }
...
{
    switch (f) {
        case ThumbnailExportFormat::PNG:
            return "PNG";
        case ThumbnailExportFormat::JPEG:
            return "JPEG";
        case ThumbnailExportFormat::WebP:
            return "WebP";
        case ThumbnailExportFormat::BMP:
            return "BMP";
        case ThumbnailExportFormat::TIFF:
            return "TIFF";
        default:
            return "Unknown";
    }
...
inline const char* ExportDestinationName(ExportDestination d) noexcept
{
    switch (d) {
        ...
        case ExportDestination::Email:
            return "Email";
        case ExportDestination::Network:
            return "Network";
        case ExportDestination::Cloud:
            return "Cloud";
        default:
            return "Unknown";
    }
...
{
  public:
    bool Export(const uint8_t* pixels, uint32_t w, uint32_t h, const ThumbnailExportConfig& cfg,
    ...
    uint32_t GetExportCount() const noexcept
    {
        return m_exportCount;
    }
    ThumbnailExportFormat GetLastFormat() const noexcept
    {
        return m_lastFormat;
    }
    static std::vector<ThumbnailExportFormat> GetSupportedFormats()
    {
        return {ThumbnailExportFormat::PNG, ThumbnailExportFormat::JPEG, ThumbnailExportFormat::WebP,
                ThumbnailExportFormat::BMP, ThumbnailExportFormat::TIFF};
    }
