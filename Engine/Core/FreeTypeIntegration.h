...
public:
  ...
  FontRenderResult RenderPreview(const wchar_t *filePath,
                                 const FontRenderConfig &config = {}) {
  ...
  static FontFormat DetectFormat(const uint8_t *header, size_t len) {
    ...
    if (header[0] == 0x00 && header[1] == 0x01 && header[2] == 0x00 &&
        header[3] == 0x00)
      return FontFormat::TrueType;
    // OpenType: OTTO
    if (header[0] == 'O' && header[1] == 'T' && header[2] == 'T' &&
        header[3] == 'O')
      return FontFormat::OpenType;
    // TTC collection: ttcf
    if (header[0] == 't' && header[1] == 't' && header[2] == 'c' &&
        header[3] == 'f')
      return FontFormat::Collection;
    // WOFF: wOFF
    if (header[0] == 'w' && header[1] == 'O' && header[2] == 'F' &&
        header[3] == 'F')
  ...
  static constexpr uint32_t GetFormatCount() { return 9; }
  static constexpr uint32_t GetBackendCount() { return 3; }
...
private:
  ...
  FontRenderResult RenderWithFreeType(const wchar_t *filePath,
                                      const FontRenderConfig &config);
#endif
  ...
  FontRenderResult RenderWithGDI(const wchar_t *filePath,
                                 const FontRenderConfig &config) {
    FontRenderResult result;
    (void)filePath;
    (void)config;
...
inline FontStyle operator|(FontStyle a, FontStyle b) {
  return static_cast<FontStyle>(static_cast<uint32_t>(a) |
