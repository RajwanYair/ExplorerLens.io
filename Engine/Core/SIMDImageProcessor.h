...
class SIMDImageProcessor
{
  public:
    ...
    void BoxDownscale(const uint8_t* src, uint32_t srcW, uint32_t srcH, uint8_t* dst, uint32_t dstW, uint32_t dstH)
    {
    ...
    void ApplyGammaLUT(uint8_t* PRGBA, uint32_t pixelCount, bool toLinear = true)
    {
  ...
  private:
    ...
    void BuildGammaLUTs()
    {
        for (int i = 0; i < 256; ++i) {
            ...
            // sRGB to linear
            float lin = (linear <= 0.04045f) ? linear / 12.92f : std::pow((linear + 0.055f) / 1.055f, 2.4f);
