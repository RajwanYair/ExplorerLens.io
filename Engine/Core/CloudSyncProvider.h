...
struct CloudFileInfo {
    ...
    bool isPlaceholder = false;   // Cloud-only file (not downloaded)
...
public:
    ...
    CloudSyncResult SyncThumbnails(CloudProvider provider, const std::wstring& path);
    ...
    static uint32_t GetProviderCount() { return static_cast<uint32_t>(CloudProvider::ProviderCount); }
