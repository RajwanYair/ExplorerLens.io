...
public:
    ...
    void SetConfig(const AnnotationConfig& cfg) { m_config = cfg; }
    const AnnotationConfig& GetConfig() const { return m_config; }
    ...
    static uint8_t BadgeMaskForFile(const std::wstring& filePath, bool isReadOnly, bool isDRM) {
        uint8_t mask = 0;
        if (isDRM)      mask |= static_cast<uint8_t>(AnnotationBadge::DRMLocked);
        if (isReadOnly) mask |= static_cast<uint8_t>(AnnotationBadge::ReadOnly);
    ...
    static BadgeSize RecommendedBadgeSize(uint32_t thumbSize) {
        if (thumbSize >= 256) return BadgeSize::Large;
        if (thumbSize >= 128) return BadgeSize::Medium;
        return BadgeSize::Small;
    }

    uint64_t GetTotalBadgesRendered() const { return m_totalBadgesRendered; }

private:
    ThumbnailAnnotationOverlay() = default;

    enum class BadgeType : uint8_t { Lock, Cloud, Key, ReadOnly };
    ...
    void DrawBadge(uint8_t* pixels, uint32_t w, uint32_t h, BadgeType type, BadgePosition pos) {
        ...
        switch (pos) {
        case BadgePosition::BottomRight: bx = w - bw - 2; by = h - bw - 2; break;
        case BadgePosition::TopLeft:     bx = 2;           by = 2;          break;
        case BadgePosition::TopRight:    bx = w - bw - 2; by = 2;          break;
        case BadgePosition::BottomLeft:  bx = 2;           by = h - bw - 2; break;
        default:                         bx = (w - bw) / 2; by = (h - bw) / 2; break;
        }
