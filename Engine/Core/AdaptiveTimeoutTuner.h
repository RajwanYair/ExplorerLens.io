...
public:
    ...
    void RecordLatency(const std::string& format, double latencyMs) {
        ...
        if ((int)window.size() >= MAX_WINDOW) window.erase(window.begin());
    ...
    double GetTimeoutMs(const std::string& format) const {
        ...
        if (it == m_policies.end()) return DEFAULT_TIMEOUT;
    ...
    int FormatCount() const noexcept { return (int)m_windows.size(); }

private:
    void UpdatePolicy(const std::string& format) {
        auto& window = m_windows[format];
        if ((int)window.size() < MIN_SAMPLES) return;
        ...
        if (p99idx >= (int)sorted.size()) p99idx = (int)sorted.size() - 1;
