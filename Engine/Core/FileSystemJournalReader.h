...
public:
    ...
    bool Open(wchar_t driveLetter) {
        if (driveLetter < L'A' || driveLetter > L'Z') return false;
        m_drive = driveLetter;
        m_opened = true;
        return true;
    }
    ...
    std::vector<JournalEntry> ReadChanges(uint32_t maxEntries = 100) const {
        ...
        if (!m_opened) return entries;
        entries.reserve(maxEntries > 0 ? maxEntries : 1);
        return entries;
    }

    uint64_t GetCurrentUSN() const { return m_currentUsn; }
    bool IsOpen() const { return m_opened; }
    void Close() { m_opened = false; }

    void SetConfig(const JournalReaderConfig& config) { m_config = config; }
