...
struct SurfaceDimensions
{
    ...
    {
        return static_cast<uint32_t>(widthPx * scaleFactor);
    }
    uint32_t PhysicalHeight() const
    {
        return static_cast<uint32_t>(heightPx * scaleFactor);
    }
};
...
{
  public:
    OffscreenSurface(uint32_t w, uint32_t h) : m_dims{w, h, 1.0f}, m_valid(w > 0 && h > 0) {}

    void* GetNativeHandle() const override
    {
        return nullptr;
    }
    SurfaceDimensions GetDimensions() const override
    {
        return m_dims;
    }
    SurfaceBackend GetBackend() const override
    {
...
{
  public:
    Win32Surface(uint32_t w, uint32_t h, const std::string& title) : m_dims{w, h, 1.0f}
    {
        m_hwnd = CreateWindowExA(0, "STATIC", title.c_str(), WS_OVERLAPPED, 0, 0, w, h, nullptr, nullptr,
