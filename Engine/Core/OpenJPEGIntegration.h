...
class OpenJPEGIntegration
{
  public:
    ...
    JP2DecodeResult Decode(const wchar_t* filePath, uint32_t targetWidth = 0, uint32_t targetHeight = 0)
    {
    ...
    static constexpr uint32_t GetProfileCount()
    {
        return 6;
    }
    static constexpr size_t ProfileCount()
    {
        return 6;
    }
    static constexpr uint32_t GetColorSpaceCount()
    {
        return 7;
    }
...
#ifdef HAS_OPENJPEG
    JP2DecodeResult DecodeInternal(const wchar_t* filePath, uint32_t tw, uint32_t th);
#endif
