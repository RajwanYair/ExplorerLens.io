...
public:
    ...
    std::wstring GetLanguageName(CodeLanguage lang) const {
        ...
        case CodeLanguage::CSharp:     return L"C#";
        case CodeLanguage::Java:       return L"Java";
        case CodeLanguage::Python:     return L"Python";
        case CodeLanguage::JavaScript: return L"JavaScript";
        case CodeLanguage::TypeScript: return L"TypeScript";
        case CodeLanguage::Rust:       return L"Rust";
        case CodeLanguage::Go:         return L"Go";
        case CodeLanguage::Html:       return L"HTML";
        case CodeLanguage::Css:        return L"CSS";
        case CodeLanguage::Json:       return L"JSON";
        case CodeLanguage::Xml:        return L"XML";
    ...
    bool IsKeyword(CodeLanguage lang, const std::wstring& word) const {
        auto it = m_keywords.find(lang);
        if (it == m_keywords.end()) return false;
    ...
    SyntaxTokenType ClassifyToken(CodeLanguage lang, const std::wstring& token) const {
        if (token.empty()) return SyntaxTokenType::Whitespace;
        if (token[0] == L'#') return SyntaxTokenType::Preprocessor;
        if (token[0] == L'"' || token[0] == L'\'') return SyntaxTokenType::String;
        if (token.size() >= 2 && token[0] == L'/' &&
            (token[1] == L'/' || token[1] == L'*'))
            return SyntaxTokenType::Comment;
        if (IsNumber(token)) return SyntaxTokenType::Number;
        if (IsOperator(token)) return SyntaxTokenType::Operator;
        if (IsPunctuation(token)) return SyntaxTokenType::Punctuation;
        if (IsKeyword(lang, token)) return SyntaxTokenType::Keyword;
        if (IsTypeName(token)) return SyntaxTokenType::Type;
...
private:
    ...
    void InitLanguageMap() {
        ...
        add(L".cpp", CodeLanguage::Cpp);   add(L".cxx", CodeLanguage::Cpp);
        add(L".cc", CodeLanguage::Cpp);   add(L".c", CodeLanguage::Cpp);
        add(L".h", CodeLanguage::Cpp);   add(L".hpp", CodeLanguage::Cpp);
        ...
        add(L".html", CodeLanguage::Html);  add(L".htm", CodeLanguage::Html);
        add(L".css", CodeLanguage::Css);
        add(L".json", CodeLanguage::Json);
        add(L".xml", CodeLanguage::Xml);
        add(L".sql", CodeLanguage::Sql);
        add(L".sh", CodeLanguage::Shell); add(L".bash", CodeLanguage::Shell);
    ...
    void InitKeywords() {
        m_keywords[CodeLanguage::Cpp] = {
            ...
            L"return",L"class",L"struct",L"namespace",L"template",L"typename",
            L"const",L"static",L"virtual",L"override",L"public",L"private",
            L"protected",L"new",L"delete",L"void",L"int",L"float",L"double",
            L"bool",L"auto",L"using",L"typedef",L"enum",L"sizeof",L"nullptr",
            L"true",L"false",L"constexpr",L"noexcept",L"inline",L"extern"
        };
        m_keywords[CodeLanguage::Python] = {
            L"def",L"class",L"if",L"elif",L"else",L"for",L"while",L"return",
            L"import",L"from",L"as",L"with",L"try",L"except",L"finally",
            L"raise",L"pass",L"lambda",L"yield",L"True",L"False",L"None",
            L"and",L"or",L"not",L"in",L"is",L"async",L"await"
        };
        m_keywords[CodeLanguage::JavaScript] = {
            L"function",L"var",L"let",L"const",L"if",L"else",L"for",L"while",
            L"return",L"class",L"new",L"this",L"import",L"export",L"default",
            L"async",L"await",L"try",L"catch",L"throw",L"typeof",L"instanceof"
        };
        m_keywords[CodeLanguage::Rust] = {
            L"fn",L"let",L"mut",L"if",L"else",L"for",L"while",L"loop",
            L"match",L"return",L"struct",L"enum",L"impl",L"trait",L"pub",
            L"use",L"mod",L"self",L"super",L"crate",L"unsafe",L"async",L"await"
        };
    }
