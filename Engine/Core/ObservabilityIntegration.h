...
class ObservabilityIntegration
{
  public:
    ...
    // ─── Configuration ───────────────────────────────────────────
    void SetPrivacyMode(PathPrivacy mode)
    {
        m_privacy = mode;
    }
    PathPrivacy GetPrivacyMode() const
    {
        return m_privacy;
    }

    void SetMinLevel(ObservabilityLevel level)
    {
        m_minLevel = level;
    }
    ObservabilityLevel GetMinLevel() const
    {
        return m_minLevel;
    }
    ...
    void OnRequestComplete(uint64_t requestId, const wchar_t* filePath, HRESULT status, double elapsedMs,
    ...
    {
        return m_totalFailures.load();
    }
    uint64_t GetTotalCacheHits() const
    {
        return m_totalCacheHits.load();
    }
  ...
  private:
    ...
    void Emit(ObservabilityLevel level, const PipelineEvent& evt, const wchar_t* eventName)
    {
        ...
        // Update counters
        if (wcscmp(eventName, L"RequestStart") == 0)
            m_totalRequests++;
        if (wcscmp(eventName, L"DecodeFail") == 0)
            m_totalFailures++;
        if (wcscmp(eventName, L"CrashCaught") == 0)
            m_totalFailures++;
        if (wcscmp(eventName, L"CacheHit") == 0)
            m_totalCacheHits++;
