...
class ContentInspectionGateway
{
  public:
    ...
    static bool ShouldBlock(const InspectionResult& result, const InspectionConfig& config)
    {
        if (!config.enabled)
            return false;
        if (result.classification == ContentClassification::Blocked)
            return true;
        if (config.blockOnFailure && result.classification >= ContentClassification::Restricted)
            return true;
        return false;
    }
};
