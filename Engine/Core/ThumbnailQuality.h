...
inline bool HasQualityFlag(QualityCheckFlag set, QualityCheckFlag flag)
{
    return HasFlag(set, flag);
}
...
class ThumbnailQualityEngine
{
  public:
    QualityScore Analyze(const uint8_t* rgba, uint32_t w, uint32_t h) const
    {
        QualityScore s;
        if (!rgba || w == 0 || h == 0) {
            s.verdict = QualityVerdict::Fail_Corrupt;
            return s;
        }
        s.sharpness = static_cast<float>(ComputeLaplacianVariance(rgba, w, h));
        s.confidence = (s.sharpness > 200.0f) ? 1.0f : s.sharpness / 200.0f;
        if (IsBlank(rgba, w * h * 4))
            s.verdict = QualityVerdict::Fail_Blank;
        else if (s.sharpness < 50.0f)
            s.verdict = QualityVerdict::Fail_TooBlurry;
        return s;
    }
    ...
    BlurAnalysisResult DetectBlur(const uint8_t* gray, uint32_t w, uint32_t h) const
    {
        ...
        r.level = (r.laplacianVariance < 50)     ? SharpnessLevel::VeryBlurry
                  ...
                  : (r.laplacianVariance < 1000) ? SharpnessLevel::Sharp
                                                 : SharpnessLevel::VerySharp;
    ...
    static double ComputeLaplacianVariance(const uint8_t* px, uint32_t w, uint32_t h)
    {
        ...
        for (uint32_t y = 1; y < h - 1; ++y) {
            for (uint32_t x = 1; x < w - 1; ++x) {
                int lap = -4 * px[y * w + x] + px[(y - 1) * w + x] + px[(y + 1) * w + x] + px[y * w + x - 1]
                          + px[y * w + x + 1];
                sum += lap;
...
struct ThumbnailQualityReport
{
    ...
    {
        return grade >= QualityGrade::Acceptable;
    }
};
...
{
  public:
    static size_t MetricCount() noexcept
    {
        return 8;
    }  // canonical 8-metric count
    static const wchar_t* MetricName(QualityMetric m) noexcept
    {
        switch (m) {
            ...
            case QualityMetric::Clarity:
                return L"Clarity";
            case QualityMetric::ColorAccuracy:
                return L"Color Accuracy";
            case QualityMetric::Saturation:
                return L"Saturation";
            case QualityMetric::NoiseLevel:
                return L"Noise Level";
            case QualityMetric::CompressionArtifact:
                return L"Compression Artifact";
            default:
                return L"Unknown";
        }
    }
...
{
  public:
    void SetThresholds(const QualityValidatorThresholds& t) noexcept
    {
        m_thresholds = t;
    }
    const QualityValidatorThresholds& GetThresholds() const noexcept
    {
        return m_thresholds;
    }

    QualityValidationResult Validate(const uint8_t* rgba, uint32_t w, uint32_t h, uint32_t /*stride*/) const
    {
