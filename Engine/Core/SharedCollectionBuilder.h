...
struct SharedCollection {
    ...
    int  ItemCount() const noexcept { return (int)items.size(); }
...
public:
    ...
    bool AddItem(const std::wstring& collectionName, const CollectionItem& item) {
        for (auto& col : m_collections)
            if (col.name == collectionName) { col.items.push_back(item); return true; }
        return false;
    }

    const SharedCollection* FindByToken(const std::string& token) const {
        for (const auto& col : m_collections)
            if (col.shareToken == token) return &col;
        return nullptr;
    }
    ...
    bool MergeCollection(const SharedCollection& remote) {
        for (auto& local : m_collections) {
            if (local.name == remote.name && local.folderPath == remote.folderPath) {
                for (const auto& item : remote.items) local.items.push_back(item);
    ...
    int  CollectionCount() const noexcept { return (int)m_collections.size(); }
    const std::vector<SharedCollection>& All() const noexcept { return m_collections; }
