...
class STLMeshDecoder
{
  public:
    ...
    STLMeshInfo ParseASCII(const uint8_t* data, size_t size) const
    {
        ...
        info.isASCII = true;
        std::string text(reinterpret_cast<const char*>(data), std::min(size, static_cast<size_t>(1024)));
    ...
    STLMeshInfo Parse(const uint8_t* data, size_t size) const
    {
        if (IsASCII(data, size))
            return ParseASCII(data, size);
        return ParseBinary(data, size);
    }

    STLStats GetStats() const
    {
        return m_stats;
    }
