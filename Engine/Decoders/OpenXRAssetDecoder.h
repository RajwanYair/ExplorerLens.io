...
enum class XRAssetFormat { XRB, GLTF_XR, GLB_XR, Unknown };
...
public:
    static XRAssetFormat DetectFormat(const std::wstring& path) {
        ...
        if (dot == std::wstring::npos) return XRAssetFormat::Unknown;
        std::wstring ext = path.substr(dot + 1);
        std::transform(ext.begin(), ext.end(), ext.begin(), ::towlower);
        if (ext == L"xrb")  return XRAssetFormat::XRB;
        if (ext == L"gltf") return XRAssetFormat::GLTF_XR;
        if (ext == L"glb")  return XRAssetFormat::GLB_XR;
    ...
    bool IsSupported(XRAssetFormat fmt) const { return fmt != XRAssetFormat::Unknown; }
};
