...
#pragma pack(pop)
    ...
    HRESULT DecodeQOI(const uint8_t* data, size_t size, uint8_t** ppPixels, uint32_t* pWidth, uint32_t* pHeight,
                      uint8_t* pChannels);
    ...
    // Bitmap creation
    HBITMAP CreateBitmapFromRGBA(const uint8_t* pixels, uint32_t width, uint32_t height, uint8_t channels);
