...
// using GDI+ PlayMetaFile/PlayEnhMetaFile. Outputs rasterized HBITMAP thumbnail.
//==============================================================================
...
/// Decoder for WMF (Windows Metafile) and EMF (Enhanced Metafile) vector formats.
///
...
public:
    ...
    static constexpr const wchar_t* s_extensions[] = {
        L".wmf", L".emf"
    };
...
private:
    ...
    HRESULT RenderMetafile(const wchar_t* filePath, uint32_t size, HBITMAP& hBitmap);

    /// Calculate aspect-ratio-preserving render rect
    static Gdiplus::RectF CalculateRenderRect(
        uint32_t targetSize, float sourceWidth, float sourceHeight);
};
