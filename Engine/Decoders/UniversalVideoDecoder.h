...
public:
    ...
    VideoDecodeResult DecodeFrame(const uint8_t* data, size_t dataSize,
        const std::string& codec, double timestampSec = 0.0) {
    ...
    bool Validate() const {
        ...
        for (const auto& c : codecs) {
            if (c.codecName == "h264") { h264Found = true; break; }
        }
        if (!h264Found) return false;
...
private:
    ...
    bool DecodeViaMF(const uint8_t* /*data*/, size_t /*size*/, VideoDecodeResult& result) {
        ...
        size_t frameSize = static_cast<size_t>(result.frameWidth) *
            result.frameHeight * result.frameChannels;
        if (frameSize > 256 * 1024 * 1024) return false;
        result.framePixels.resize(frameSize, 0);
        return true;
    }

    bool DecodeViaFFmpeg(const uint8_t* /*data*/, size_t /*size*/,
        VideoDecoderBackend /*backend*/, VideoDecodeResult& result) {
        result.frameWidth = 1920;
        result.frameHeight = 1080;
        result.frameChannels = 4;
        size_t frameSize = static_cast<size_t>(result.frameWidth) *
