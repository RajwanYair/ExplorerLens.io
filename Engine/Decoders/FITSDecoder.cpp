...
bool FITSDecoder::IsFITSFile(const uint8_t* data, size_t size) {
    // FITS files start with "SIMPLE  = " in the first 80-char record
...
bool FITSDecoder::ParseHeader(const uint8_t* data, size_t size) {
    ...
    while (offset + 80 <= size) {
        ...
        if (keyword == "BITPIX") {
            int bp = 0;
            try { bp = std::stoi(value); } catch (...) {}
            m_info.bitpix = static_cast<FITSBitpix>(bp);
        } else if (keyword == "NAXIS") {
            try { m_info.naxis = static_cast<uint32_t>(std::stoul(value)); } catch (...) {}
        } else if (keyword == "NAXIS1") {
            try { m_info.naxis1 = static_cast<uint32_t>(std::stoul(value)); } catch (...) {}
        } else if (keyword == "NAXIS2") {
            try { m_info.naxis2 = static_cast<uint32_t>(std::stoul(value)); } catch (...) {}
        } else if (keyword == "NAXIS3") {
            try { m_info.naxis3 = static_cast<uint32_t>(std::stoul(value)); } catch (...) {}
        } else if (keyword == "BZERO") {
            try { m_info.bzero = std::stod(value); } catch (...) {}
        } else if (keyword == "BSCALE") {
            try { m_info.bscale = std::stod(value); } catch (...) {}
        } else if (keyword == "DATAMIN") {
            try { m_info.datamin = std::stod(value); m_info.hasMinMax = true; } catch (...) {}
        } else if (keyword == "DATAMAX") {
            try { m_info.datamax = std::stod(value); m_info.hasMinMax = true; } catch (...) {}
