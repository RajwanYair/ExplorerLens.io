...
bool FITSDecoder::IsFITSFile(const uint8_t* data, size_t size)
{
    if (!data || size < 6)
        return false;
    return data[0] == 'S' && data[1] == 'I' && data[2] == 'M' && data[3] == 'P' && data[4] == 'L' && data[5] == 'E';
}

bool FITSDecoder::ParseHeader(const uint8_t*, size_t)
{
    return false;
}
...
std::vector<uint8_t> FITSDecoder::ExtractThumbnail(const uint8_t*, size_t, uint32_t, uint32_t) const
{
    return {};
}
...
const wchar_t* FITSDecoder::GetBitpixName(FITSBitpix bp)
{
    switch (bp) {
        ...
        case FITSBitpix::Int16:
            return L"16-bit integer";
        case FITSBitpix::Int32:
            return L"32-bit integer";
...
std::string FITSDecoder::ParseKeyword(const char*, std::string&) const
{
    return {};
}
double FITSDecoder::ReadPixelValue(const uint8_t*, size_t) const
{
    return 0.0;
}
