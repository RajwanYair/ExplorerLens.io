...
DecoderEBookFormat EBookDecoder::DetectFormat(const uint8_t* data, size_t size)
{
    ...
    if (IsEPUB(data, size))
        return DecoderEBookFormat::EPUB;
    if (IsMOBI(data, size))
        return DecoderEBookFormat::MOBI;
    // FB2 is XML-based
    if (size >= 5 && memcmp(data, "<?xml", 5) == 0) {
        // Check for FictionBook root element
        std::string content(reinterpret_cast<const char*>(data), std::min(size, (size_t)1024));
...
bool EBookDecoder::ExtractEPUBCover(const uint8_t* /*data*/, size_t /*size*/, std::vector<uint8_t>& /*coverOut*/) const
{
...
{
    switch (format) {
        case DecoderEBookFormat::EPUB:
            return L"EPUB";
        case DecoderEBookFormat::MOBI:
            return L"MOBI";
        case DecoderEBookFormat::FB2:
            return L"FB2";
        case DecoderEBookFormat::AZW3:
            return L"AZW3";
        case DecoderEBookFormat::CBZ:
            return L"CBZ";
        case DecoderEBookFormat::DJVU:
            return L"DJVU";
        case DecoderEBookFormat::LIT:
            return L"LIT";
        default:
            return L"Unknown";
    }
...
const wchar_t* EBookDecoder::GetFormatMimeType(DecoderEBookFormat format)
{
    switch (format) {
        ...
        case DecoderEBookFormat::FB2:
            return L"application/x-fictionbook+xml";
        case DecoderEBookFormat::AZW3:
