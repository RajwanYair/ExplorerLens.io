...
bool ArchiveDecoder::IsArchiveFormat(const void* pData, size_t dataSize)
{
    ...
    return (bytes[0] == ZIP_SIGNATURE[0] && bytes[1] == ZIP_SIGNATURE[1] && bytes[2] == ZIP_SIGNATURE[2]
...
bool ArchiveDecoder::IsImageFile(const std::wstring& filename)
{
    ...
    static const wchar_t* imageExts[] = {L".jpg",  L".jpeg", L".png",  L".bmp",  L".gif",  L".tif",
                                         L".tiff", L".webp", L".avif", L".heic", L".heif", L".jxl"};
...
bool ArchiveDecoder::IsCoverImage(const std::wstring& filename)
{
    ...
    return (lower.find(L"cover") != std::wstring::npos || lower.find(L"folder") != std::wstring::npos
            || lower.find(L"front") != std::wstring::npos || lower.find(L"thumb") != std::wstring::npos
            || lower.find(L"poster") != std::wstring::npos || lower.find(L"001") != std::wstring::npos
...
int ArchiveDecoder::GetImagePriority(const std::wstring& filename)
{
    ...
    if (lower.find(L"cover") != std::wstring::npos)
        return 100;
    if (lower.find(L"folder") != std::wstring::npos)
        return 90;
    if (lower.find(L"front") != std::wstring::npos)
        return 80;
    if (lower.find(L"poster") != std::wstring::npos)
        return 70;
    if (lower.find(L"thumb") != std::wstring::npos)
        return 60;
    if (lower.find(L"001.") != std::wstring::npos)
        return 50;
    if (lower.find(L"01.") != std::wstring::npos)
        return 40;
    if (lower.find(L"00.") != std::wstring::npos)
        return 45;
...
HRESULT ArchiveDecoder::ExtractBestCoverImage(const wchar_t* archivePath, std::vector<unsigned char>& imageData,
                                              std::wstring& imageName)
{
...
HRESULT ArchiveDecoder::ExtractFirstImage(const wchar_t* archivePath, std::vector<unsigned char>& imageData,
                                          std::wstring& imageName)
{
    // Convert wstring to UTF-8
    int utf8Size = WideCharToMultiByte(CP_UTF8, 0, archivePath, -1, nullptr, 0, nullptr, nullptr);
...
HRESULT ArchiveDecoder::DecodeImageData(const std::vector<unsigned char>& imageData, const std::wstring& imageName,
                                        UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
    ...
    hr = pStream->InitializeFromMemory(const_cast<BYTE*>(imageData.data()), static_cast<DWORD>(imageData.size()));
    if (FAILED(hr)) {
        pStream->Release();
        return hr;
    }
    ...
    if ((targetWidth > 0 && targetHeight > 0) && (width != targetWidth || height != targetHeight)) {
        ...
        if (SUCCEEDED(hr)) {
            hr = pScaler->Initialize(pConverter, targetWidth, targetHeight, WICBitmapInterpolationModeFant);
            if (SUCCEEDED(hr)) {
                pSource = pScaler;
                width = targetWidth;
                height = targetHeight;
