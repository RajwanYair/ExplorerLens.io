...
class ExampleDecoder : public IThumbnailDecoder
{
  public:
    ...
    {
        return false;
    }  // Change to true if GPU-accelerated
  ...
  private:
    ...
    std::vector<uint8_t> DecodeImageData(const uint8_t* fileData, size_t fileSize, int* outWidth, int* outHeight) const;
    ...
    std::vector<uint8_t> ScaleRGBA(const uint8_t* srcPixels, int srcWidth, int srcHeight, int dstWidth,
                                   int dstHeight) const;
    ...
    HBITMAP CreateBitmapFromBGRA(const uint8_t* pixels, uint32_t width, uint32_t height) const;

    /// Calculate aspect-preserving dimensions
    void CalculateAspectDimensions(int sourceWidth, int sourceHeight, uint32_t maxWidth, uint32_t maxHeight,
