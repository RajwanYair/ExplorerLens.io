...
#pragma pack(pop)
    ...
    HRESULT ExtractCompositeImage(const uint8_t* data, size_t size, const PSDHeader& header, HBITMAP* phBitmap);
    HRESULT ExtractThumbnailResource(const uint8_t* data, size_t size, HBITMAP* phBitmap);

    // Format validation
    bool IsPSDFormat(const wchar_t* path);

    // Bitmap creation
    HBITMAP CreateBitmapFromRGB(const uint8_t* pixels, uint32_t width, uint32_t height, uint16_t channels);
