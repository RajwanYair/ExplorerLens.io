...
public:
    ...
    SidecarMetadataDecoderConfig GetConfig() const { return m_config; }
    ...
    std::string GetSidecarPath(const std::string& imagePath) const {
        ...
        if (dot == std::string::npos) return imagePath + m_config.sidecarExtension;
        return imagePath.substr(0, dot) + m_config.sidecarExtension;
    }

    bool HasCrop(const SidecarData& data) const { return data.hasCrop; }
    bool HasRotation(const SidecarData& data) const { return data.hasRotation; }
