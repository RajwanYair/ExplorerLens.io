...
{
    switch (s) {
        case KTXSupercompression::None:
            return "None";
        case KTXSupercompression::BasisLZ:
            return "BasisLZ";
        case KTXSupercompression::Zstd:
            return "Zstandard";
        case KTXSupercompression::ZLIB:
            return "ZLIB";
        default:
            return "Unknown";
    }
...
struct KTXTextureInfo
{
    ...
    bool IsValid() const
    {
        return width > 0 && height > 0 && version != KTXVersion::Unknown;
    }
    bool Is3D() const
    {
        return depth > 1;
    ...
    uint32_t BestMipForThumbnail(uint32_t targetSize) const
    {
        ...
        while (mip < mipLevels - 1) {
            ...
            if (nextW < targetSize && nextH < targetSize)
                break;
            w = nextW;
            h = nextH;
            mip++;
...
struct KTXExtensions
{
    ...
    static KTXVersion VersionFromExtension(const std::string& ext)
    {
        ...
        std::transform(lower.begin(), lower.end(), lower.begin(),
                       [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        if (lower == ".ktx")
            return KTXVersion::KTX1;
        if (lower == ".ktx2")
            return KTXVersion::KTX2;
...
struct KTXDecodeResult
{
    ...
    {
        return status == KTXDecodeStatus::Success;
    }
...
class KTXTextureDecoder
{
  public:
    ...
    KTXDecodeResult DecodeThumbnail(const std::string& filePath, uint32_t maxSize = 256) const
    {
    ...
    static KTXTextureDecoder Create()
    {
        return KTXTextureDecoder();
    }
  ...
  private:
    ...
    // Decode paths
    KTXDecodeResult DecodeUncompressed(const std::vector<uint8_t>& data, const KTXTextureInfo& info,
                                       uint32_t mipLevel) const;
    KTXDecodeResult DecodeBlockCompressed(const std::vector<uint8_t>& data, const KTXTextureInfo& info,
                                          uint32_t mipLevel) const;

    // BC1 (DXT1) block decompression
    void DecompressBC1Block(const uint8_t* block, uint8_t* output, uint32_t outputStride) const;
};
