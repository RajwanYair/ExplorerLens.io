...
public:
    ...
    static const wchar_t* GetName() { return L"SnappyFrameDecoder"; }
    ...
    bool DetectMagic(const uint8_t* data, size_t size) const {
        ...
        if (chunkLen < 6 || 4 + chunkLen > size) return false;
    ...
    SnappyFrameInfo ParseFrames(const uint8_t* data, size_t size) const {
        ...
        while (offset + 4 <= size) {
            ...
            if (chunkLen == 0 && chunkType != 0xFE) break;
    ...
    SnappyStats GetStats() const { return m_stats; }
