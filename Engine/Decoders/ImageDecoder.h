...
class ImageDecoder : public IThumbnailDecoder
{
  ...
  private:
    ...
    HRESULT DecodeFromStream(IStream* pStream, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap);

    // Convert WIC bitmap to HBITMAP
    HRESULT CreateHBITMAPFromWIC(IWICBitmapSource* pSource, HBITMAP* phBitmap);

    // Apply EXIF orientation
    HRESULT ApplyEXIFOrientation(IWICBitmapFrameDecode* pFrame, IWICBitmapSource** ppCorrected);
