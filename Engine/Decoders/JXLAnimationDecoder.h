...
class JXLAnimationDecoder
{
  public:
    ...
    JXLDecodeResult DecodeCoverFrame(const uint8_t* data, size_t size, const JXLDecodeOptions& opts = {}) const;

    // Decode an arbitrary frame by index.
    JXLDecodeResult DecodeFrame(const uint8_t* data, size_t size, uint32_t frameIndex,
  ...
  private:
    ...
    JXLDecodeResult DecodeInternal(const uint8_t* data, size_t size, uint32_t frameIndex, bool coverOnly,
                                   const JXLDecodeOptions& opts) const;

    void ApplyBT2390Tonemap(std::vector<uint8_t>& pixels, uint32_t width, uint32_t height, float sourcePeakNits,
