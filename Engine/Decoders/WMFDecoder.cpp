...
Gdiplus::RectF WMFDecoder::CalculateRenderRect(
    uint32_t targetSize, float sourceWidth, float sourceHeight) {

    if (sourceWidth <= 0.0f || sourceHeight <= 0.0f) {
        return Gdiplus::RectF(0, 0,
            static_cast<float>(targetSize), static_cast<float>(targetSize));
    }

    float scale = (std::min)(
        static_cast<float>(targetSize) / sourceWidth,
