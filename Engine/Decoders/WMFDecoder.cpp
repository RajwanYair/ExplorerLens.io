...
bool WMFDecoder::CanDecode(const wchar_t* filePath) const
{
    ...
    for (uint32_t i = 0; i < s_extensionCount; ++i) {
        ...
        if (path.length() >= extLen && path.compare(path.length() - extLen, extLen, s_extensions[i]) == 0) {
            return true;
        }
    }
    return false;
...
HRESULT WMFDecoder::Decode(const wchar_t* filePath, uint32_t requestedSize, HBITMAP& hBitmap)
{
...
Gdiplus::RectF WMFDecoder::CalculateRenderRect(uint32_t targetSize, float sourceWidth, float sourceHeight)
{
    if (sourceWidth <= 0.0f || sourceHeight <= 0.0f) {
        return Gdiplus::RectF(0, 0, static_cast<float>(targetSize), static_cast<float>(targetSize));
    }

    float scale =
        (std::min)(static_cast<float>(targetSize) / sourceWidth, static_cast<float>(targetSize) / sourceHeight);
...
HRESULT WMFDecoder::RenderMetafile(const wchar_t* filePath, uint32_t size, HBITMAP& hBitmap)
{
    // Initialize GDI+
    Gdiplus::GdiplusStartupInput gdipInput;
    ULONG_PTR gdipToken = 0;
    Gdiplus::Status status = Gdiplus::GdiplusStartup(&gdipToken, &gdipInput, nullptr);
