...
{
    static const char* names[] = {"Unknown", "GIF89a", "APNG", "WebP-Anim", "AVIF-Seq", "JXL-Anim"};
...
{
    static const char* names[] = {"First", "Middle", "MostComplex", "LeastBlurry", "SceneChange", "Adaptive"};
...
struct AnimationFrame
{
    ...
    float GetQualityScore() const
    {
        ...
        return sharpness * 0.5f + complexity * 0.3f + (isKeyframe ? 0.2f : 0.0f);
    }
};
...
struct AnimatedImageInfo
{
    ...
    double GetFPS() const
    {
        return (totalDurationMs > 0 && frameCount > 1) ? (static_cast<double>(frameCount) * 1000.0 / totalDurationMs)
                                                       : 0.0;
    }

    double GetAverageBytesPerFrame() const
    {
        return (frameCount > 0) ? (static_cast<double>(totalDataSize) / frameCount) : 0.0;
    }
};
...
class AnimatedImageDecoder
{
  public:
    ...
    void SetStrategy(FrameSelectionStrategy strategy)
    {
        m_strategy = strategy;
    }
    FrameSelectionStrategy GetStrategy() const
    {
        return m_strategy;
    }
    ...
    static AnimatedImageFmt DetectFormat(const uint8_t* data, size_t size)
    {
        ...
        // GIF89a
        if (size >= 6 && data[0] == 'G' && data[1] == 'I' && data[2] == 'F' && data[3] == '8' && data[4] == '9'
            && data[5] == 'a') {
    ...
    bool ParseAnimation(const uint8_t* data, size_t size)
    {
        ...
        if (m_info.format == AnimatedImageFmt::Unknown)
            return false;
        ...
        switch (m_strategy) {
            ...
            case FrameSelectionStrategy::MostComplex: {
                auto it = std::max_element(
                    ...
                    [](const AnimationFrame& a, const AnimationFrame& b) { return a.complexity < b.complexity; });
                return static_cast<uint32_t>(std::distance(m_frames.begin(), it));
            }
            case FrameSelectionStrategy::LeastBlurry: {
                auto it = std::max_element(
                    m_frames.begin(), m_frames.end(),
                    [](const AnimationFrame& a, const AnimationFrame& b) { return a.sharpness < b.sharpness; });
                return static_cast<uint32_t>(std::distance(m_frames.begin(), it));
            }
    ...
    const AnimatedImageInfo& GetInfo() const
    {
        return m_info;
    }
    ...
    const AnimatedDecodeStats& GetStats() const
    {
        return m_stats;
    }
  ...
  private:
    ...
    bool ParseAPNG(const uint8_t* data, size_t size)
    {
        ...
        m_info.width = 320;
        m_info.height = 240;
        m_info.frameCount = (std::max)(static_cast<uint32_t>(1), static_cast<uint32_t>(size / 8000));
        PopulateFrameList();
        return true;
    }
    ...
    bool ParseWebPAnim(const uint8_t* data, size_t size)
    {
        ...
        m_info.width = 320;
        m_info.height = 240;
        m_info.frameCount = (std::max)(static_cast<uint32_t>(1), static_cast<uint32_t>(size / 10000));
        PopulateFrameList();
        return true;
    }
    ...
    uint32_t SelectAdaptive() const
    {
        ...
        for (size_t i = 1; i < m_frames.size(); i++) {
            if (m_frames[i].sceneChangeMagnitude > 0.3f && m_frames[i].GetQualityScore() > 0.4f) {
                return static_cast<uint32_t>(i);
            }
        }
        ...
        auto it = std::max_element(
            m_frames.begin(), m_frames.end(),
            [](const AnimationFrame& a, const AnimationFrame& b) { return a.GetQualityScore() < b.GetQualityScore(); });
        return static_cast<uint32_t>(std::distance(m_frames.begin(), it));
    }
