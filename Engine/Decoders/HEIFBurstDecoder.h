...
class HEIFBurstDecoder
{
  public:
    ...
    HEIFDecodeResult DecodePrimary(const uint8_t* data, size_t size, const HEIFDecodeOptions& opts = {}) const;

    // Decode a specific item by itemId (e.g. burst frame N).
    HEIFDecodeResult DecodeItem(const uint8_t* data, size_t size, uint32_t itemId,
  ...
  private:
    ...
    HEIFDecodeResult DecodeInternal(const uint8_t* data, size_t size, uint32_t itemId,
                                    const HEIFDecodeOptions& opts) const;

    bool ParseGainMap(const uint8_t* data, size_t size, uint32_t primaryItemId, std::vector<uint8_t>& gainPixels,
                      uint32_t& gainWidth, uint32_t& gainHeight) const;
};
