...
public:
  ...
  static uint32_t CountFrames(const uint8_t* data, size_t size,
    ...
    case AnimatedFormat::WebPAnim: {
      ...
      while (pos + 8 <= size) {
        if (data[pos] == 'A' && data[pos + 1] == 'N' &&
          data[pos + 2] == 'M' && data[pos + 3] == 'F')
          ++count;
        uint32_t cs = static_cast<uint32_t>(data[pos + 4])
        ...
        if (cs & 1) ++advance; // RIFF word alignment
        if (advance < 8) break; // overflow guard
        pos += advance;
      }
      break;
    }
