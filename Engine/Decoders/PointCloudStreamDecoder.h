...
class PointCloudStreamDecoder
{
  public:
    ...
    inline std::vector<uint8_t> GenerateDensityThumbnail(const LASPoint* points, size_t pointCount, double minX,
                                                         double minY, double maxX, double maxY, uint32_t thumbWidth,
                                                         uint32_t thumbHeight) const
    {
        ...
        if (!points || pointCount == 0 || thumbWidth == 0 || thumbHeight == 0)
            return thumbnail;
    ...
    {
        return std::string("LAS ") + std::to_string(header.versionMajor) + "." + std::to_string(header.versionMinor)
               + " | Points: " + std::to_string(header.numberOfPoints)
               + " | Format: " + std::to_string(header.pointDataFormatId);
    }
