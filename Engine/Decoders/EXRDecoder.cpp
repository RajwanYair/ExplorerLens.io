...
const wchar_t** EXRDecoder::GetSupportedExtensions() const
{
    return m_extensions;
}
...
HRESULT EXRDecoder::EnsureWICFactory()
{
    ...
    if (!s_wicFactory) {
        return CoCreateInstance(CLSID_WICImagingFactory, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&s_wicFactory));
    }
    return S_OK;
}
...
HRESULT EXRDecoder::DecodeWithWIC(const wchar_t* path, uint32_t maxSize, HBITMAP* phBitmap)
{
    ...
    hr = s_wicFactory->CreateDecoderFromFilename(path, nullptr, GENERIC_READ, WICDecodeMetadataCacheOnDemand, &decoder);
    if (FAILED(hr))
        return hr;  // No EXR codec available
