...
HRESULT AVIFDecoder::Decode(const ThumbnailRequest &request,
  ...
  HRESULT hr =
      DecodeFromFile(request.filePath, request.width, request.height, &hBitmap);
...
HRESULT AVIFDecoder::DecodeFromFile(const wchar_t *path, UINT targetWidth,
  ...
  *phBitmap = nullptr;
  ...
  hr = finalSource->CopyPixels(nullptr, stride, bufferSize,
...
HRESULT AVIFDecoder::DecodeFromMemory(const BYTE *data, size_t size,
