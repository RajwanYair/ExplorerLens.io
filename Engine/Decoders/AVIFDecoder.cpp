...
// AVIFDecoder handles only AVIF (AV1 codec via libavif+dav1d).
// HEIF/HEIC formats are handled by HEIFDecoder (HEVC codec via
// libheif+libde265) — do not overlap.
const wchar_t* AVIFDecoder::m_extensions[] = {
    L".avif",
    nullptr  // Null terminator
};

const uint32_t AVIFDecoder::m_extensionCount = 1;
...
HRESULT AVIFDecoder::DecodeFromFile(const wchar_t* path, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
    ...
    *phBitmap = nullptr;
    ...
    if (targetWidth > 0 && targetHeight > 0 && (sourceWidth != targetWidth || sourceHeight != targetHeight)) {
        ...
        hr = scaler->Initialize(converter.Get(), targetWidth, targetHeight, WICBitmapInterpolationModeFant);
...
HRESULT AVIFDecoder::DecodeFromMemory(const BYTE* data, size_t size, UINT targetWidth, UINT targetHeight,
                                      HBITMAP* phBitmap)
{
