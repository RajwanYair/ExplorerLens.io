...
public:
    ...
    static const wchar_t* GetName() { return L"FBXInspector"; }
    ...
    bool DetectASCIIMagic(const uint8_t* data, size_t size) const {
        ...
        std::string text(reinterpret_cast<const char*>(data),
            std::min(size, static_cast<size_t>(256)));
    ...
    FBXFileInfo Inspect(const uint8_t* data, size_t size) const {
        ...
        if (DetectBinaryMagic(data, size)) {
            ...
            while (offset + 13 <= size) {
                ...
                if (endOffset == 0 || endOffset > size) break;

                uint32_t numProps = 0;
                memcpy(&numProps, data + offset + 4, 4);

                uint8_t nameLen = data[offset + 12];
                if (offset + 13 + nameLen > size) break;
                ...
                if (node.name == "Objects") info.objectCount++;
                if (node.name == "Geometry") info.geometryCount++;
                if (node.name == "Material") info.materialCount++;
    ...
    FBXStats GetStats() const { return m_stats; }
