...
bool HDRDecoder::IsHDRFormat(const wchar_t* path)
{
    ...
    if (!data || fileSize < 11)
        return false;
    return memcmp(data.get(), "#?RADIANCE", 10) == 0 || memcmp(data.get(), "#?RGBE", 6) == 0;
}
...
const wchar_t** HDRDecoder::GetSupportedExtensions() const
{
    return m_extensions;
}
...
HRESULT HDRDecoder::DecodeFromFile(const wchar_t* path, HBITMAP* phBitmap)
{
    ...
    *phBitmap = nullptr;
    ...
    if (!fileData || fileSize < 32)
        return E_FAIL;

    // Security: File size limit
    if (!Security::ValidateFileSize(fileSize, Security::MAX_HDR_FILE_SIZE))
        return E_FAIL;
...
HRESULT HDRDecoder::ParseHDR(const uint8_t* data, size_t size, float** ppPixels, uint32_t* pWidth, uint32_t* pHeight)
{
    ...
    // Security: Safe float pixel buffer allocation (3 floats per pixel)
    size_t floatBufSize = 0;
    if (!Security::SafeMulTriple(static_cast<size_t>(width), static_cast<size_t>(height), 3 * sizeof(float),
                                 floatBufSize)) {
        return E_FAIL;
    }
    if (floatBufSize > Security::MAX_DECODE_ALLOCATION)
        return E_FAIL;

    *ppPixels = new (std::nothrow) float[static_cast<size_t>(width) * height * 3];
    if (!*ppPixels)
...
HRESULT HDRDecoder::DecodeScanlineRLE(const uint8_t*& p, const uint8_t* end, RGBE* scanline, uint32_t width)
{
    if (p + 4 > end)
        return E_FAIL;
...
HBITMAP HDRDecoder::CreateBitmapFromFloat(const float* pixels, uint32_t width, uint32_t height)
{
    BITMAPINFO bmi = {};
