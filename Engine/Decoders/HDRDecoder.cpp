...
HRESULT HDRDecoder::DecodeFromFile(const wchar_t* path, HBITMAP* phBitmap) {
    ...
    *phBitmap = nullptr;
    ...
    if (!Security::ValidateFileSize(fileSize, Security::MAX_HDR_FILE_SIZE)) return E_FAIL;
...
HRESULT HDRDecoder::ParseHDR(const uint8_t* data, size_t size,
    ...
    if (!Security::SafeMulTriple(static_cast<size_t>(width), static_cast<size_t>(height),
        3 * sizeof(float), floatBufSize)) {
        return E_FAIL;
    }
    if (floatBufSize > Security::MAX_DECODE_ALLOCATION) return E_FAIL;
