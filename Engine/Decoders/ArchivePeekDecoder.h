...
public:
    void Configure(const ArchivePeekConfig& config) { m_config = config; }
    ...
    float ScoreEntry(const ArchiveEntry& entry) const {
        float score = 0.0f;
        if (entry.isImageFile) score += 10.0f;
        if (entry.isDirectory) return 0.0f;
        if (entry.uncompressedSize > m_config.maxEntrySize) return 0.0f;
        // Prefer named images (cover, thumb, preview)
        std::wstring lower = entry.name;
        for (auto& c : lower) c = towlower(c);
        if (lower.find(L"cover") != std::wstring::npos) score += 5.0f;
        if (lower.find(L"thumb") != std::wstring::npos) score += 3.0f;
        if (lower.find(L"preview") != std::wstring::npos) score += 3.0f;
    ...
    ArchivePeekConfig GetConfig() const { return m_config; }
