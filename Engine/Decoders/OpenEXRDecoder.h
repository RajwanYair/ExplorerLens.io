...
public:
    ...
    inline EXRHeader ParseHeader(const uint8_t* data, size_t size) const {
        EXRHeader header;
        if (!data || size < 8) return header;

        header.magic = ReadU32LE(data, 0);
        if (header.magic != EXR_MAGIC) return header;
    ...
    inline std::vector<uint8_t> ToneMapToLDR(const float* hdrData, uint32_t width, uint32_t height,
        ...
        if (!hdrData || pixelCount == 0) return ldr;
...
private:
    ...
    inline float ComputeLogAverageLuminance(const float* data, uint32_t width, uint32_t height,
        uint32_t channels) const {
        size_t pixelCount = static_cast<size_t>(width) * height;
        double logSum = 0.0;
        size_t count = 0;
        for (size_t i = 0; i < pixelCount; ++i) {
            ...
            if (channels > 1) lum += 0.7152f * data[idx + 1];
            if (channels > 2) lum += 0.0722f * data[idx + 2];
            if (lum > 0.0001f) {
