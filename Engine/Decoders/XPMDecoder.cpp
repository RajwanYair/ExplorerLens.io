...
// Decode XPM3 format
//==========================================================================
XPMDecoder::DecodeResult XPMDecoder::Decode(const std::string& filePath, uint32_t targetWidth) const
{
    (void)targetWidth;
    DecodeResult result;
    std::ifstream file(filePath);
    ...
    for (size_t i = colorStart; i < lines.size() && colorsRead < numColors; ++i) {
        ...
        if (cPos != std::string::npos) {
            ...
            if (tabPos != std::string::npos)
                colorVal = colorVal.substr(0, tabPos);
    ...
    for (size_t i = pixelStart; i < lines.size() && row < h; ++i) {
        std::string content = ExtractQuoted(lines[i]);
        if (content.size() < static_cast<size_t>(w) * cpp)
            continue;
