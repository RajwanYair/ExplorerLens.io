...
HRESULT PluginDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result)
{
    ...
    HBITMAP hBitmap =
        CreateBitmapFromPixels(pluginResult.pixels, pluginResult.width, pluginResult.height, pluginResult.stride);
...
bool PluginDecoder::IsArchiveDecoder() const
{
    ...
    for (size_t i = 0; info->supported_extensions[i] != nullptr; ++i) {
        ...
        if (strcmp(ext, ".zip") == 0 || strcmp(ext, ".rar") == 0 || strcmp(ext, ".7z") == 0 || strcmp(ext, ".tar") == 0
            || strcmp(ext, ".gz") == 0 || strcmp(ext, ".bz2") == 0 || strcmp(ext, ".xz") == 0
            || strcmp(ext, ".cbz") == 0 || strcmp(ext, ".cbr") == 0 || strcmp(ext, ".cb7") == 0) {
            return true;
        }
    }
...
HBITMAP PluginDecoder::CreateBitmapFromPixels(const uint8_t* pixels, uint32_t width, uint32_t height, uint32_t stride)
{
