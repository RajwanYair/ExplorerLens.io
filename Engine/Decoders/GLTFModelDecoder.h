...
struct MeshComplexity
{
    ...
    {
        return totalTriangles > kComplexityThreshold;
    }
};
...
{
    switch (p) {
        case GLTFRenderPath::FullPBR:
            return "FullPBR";
        case GLTFRenderPath::WireframeFallback:
            return "WireframeFallback";
        case GLTFRenderPath::TextureFlatmap:
            return "TextureFlatmap";
        case GLTFRenderPath::ShapeOnly:
            return "ShapeOnly";
        default:
            return "Unknown";
    }
...
struct GLTFDecodeResult
{
    ...
    {
        return renderPath == GLTFRenderPath::WireframeFallback || renderPath == GLTFRenderPath::TextureFlatmap;
    }
};
...
struct GLTFModelDecoder
{
    ...
    {
        if (!hasD3D11)
            return GLTFRenderPath::ShapeOnly;
        if (c.IsComplex() && c.hasPBRMaterial)
            return GLTFRenderPath::TextureFlatmap;
        if (c.IsComplex())
            return GLTFRenderPath::WireframeFallback;
    ...
    GLTFDecodeResult Decode(const uint8_t* data, size_t size, uint32_t targetW, uint32_t targetH,
                            bool hasD3D11 = true) const
    {
        ...
        size_t jsonLen = 0;
        if (size >= 20 && data[0] == 'g' && data[1] == 'l' && data[2] == 'T' && data[3] == 'F') {
            // GLB: header(12) + chunkLen(4) + chunkType(4) + JSON
            uint32_t c0Len = static_cast<uint32_t>(data[12]) | (static_cast<uint32_t>(data[13]) << 8)
        ...
        auto findKey = [&](const std::string& key, size_t s, size_t e) -> size_t {
            std::string pat = "\"" + key + "\"";
            size_t kp = json.find(pat, s);
            if (kp == std::string::npos || kp >= e)
                return std::string::npos;
            size_t cp = json.find(':', kp + pat.size());
            if (cp == std::string::npos || cp >= e)
                return std::string::npos;
            return skipWS(cp + 1);
        };
        auto matchBracket = [&](size_t p, char op, char cl) -> size_t {
            if (p >= json.size() || json[p] != op)
                return std::string::npos;
            int d = 1;
                    ...
                    while (p < me) {
                        ...
                        if (json[p] == '{') {
                            ...
                            if (oe == std::string::npos)
                                break;
                            ++matCnt;
                            if (json.find("pbrMetallicRoughness", p) < oe)
                                hasPBR = true;
                            p = oe + 1;
                        } else {
                            ++p;
        ...
        if (!r.success)
            r.errorMsg = "No meshes or accessors found in glTF";
        r.decodeMs = 0.0;
        return r;
    }
};
