...
HRESULT AudioDecoder::Decode(const ThumbnailRequest& request,
    ...
    if (SUCCEEDED(hr) && result.hBitmap != nullptr) {
        BITMAP bm;
...
HRESULT AudioDecoder::ExtractAlbumArt(const wchar_t* filePath,
    ...
    *phBitmap = nullptr;
    ...
    if (fileSize >= 10 && data[0] == 'I' && data[1] == 'D' && data[2] == '3') {
        const HRESULT hr = ExtractAlbumArtMP3(data.get(), fileSize, phBitmap);
    ...
    if (ext != nullptr && _wcsicmp(ext, L".wma") == 0) {
        const HRESULT hr = ExtractAlbumArtWMA(filePath, phBitmap);
        if (SUCCEEDED(hr) && *phBitmap != nullptr) {
            return S_OK;
        }
    }
...
HRESULT AudioDecoder::ExtractAlbumArtPropertySystem(const wchar_t* filePath,
    ...
    CoUninitialize();
    return (*phBitmap != nullptr) ? S_OK : E_FAIL;
}
