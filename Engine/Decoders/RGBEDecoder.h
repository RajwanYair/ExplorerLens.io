...
enum class HDREncoding : uint8_t { RGBE, XYZE };
...
public:
    ...
    static const wchar_t* GetName() { return L"RGBEDecoder"; }
    ...
    bool DetectMagic(const uint8_t* data, size_t size) const {
        ...
        return (memcmp(data, "#?RADIANCE", 10) == 0) ||
            (size >= 6 && memcmp(data, "#?RGBE", 6) == 0);
    }
    ...
    RGBEHeader ParseHeader(const uint8_t* data, size_t size) const {
        RGBEHeader hdr;
        if (!data || size < 10) return hdr;

        std::string text(reinterpret_cast<const char*>(data),
            std::min(size, static_cast<size_t>(2048)));
    ...
    void RGBEToFloat(uint8_t r, uint8_t g, uint8_t b, uint8_t e,
        float& outR, float& outG, float& outB) const {
        if (e == 0) { outR = outG = outB = 0.0f; return; }
    ...
    RGBEStats GetStats() const { return m_stats; }
