...
#pragma pack(pop)
    ...
    HRESULT DecodePCXData(const uint8_t* data, size_t dataSize, std::vector<uint8_t>& pixels, uint32_t& width,
                          uint32_t& height);

    /// RLE decode a single scanline
    static size_t DecodeRLEScanline(const uint8_t* src, size_t srcSize, uint8_t* dst, uint32_t bytesPerLine);
};
