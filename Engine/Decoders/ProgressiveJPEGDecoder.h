...
inline const char* JPEGScanTypeToString(JPEGScanType type) {
    static const char* names[] = {
        "Unknown", "Baseline", "Progressive-DC", "Progressive-AC",
...
struct JPEGScanInfo {
    ...
    bool IsDCOnly() const { return spectralStart == 0 && spectralEnd == 0; }
    bool IsACRefinement() const { return spectralStart > 0 && bitHigh > 0; }
};
...
public:
    ...
    void SetEarlyExitEnabled(bool enabled) { m_earlyExit = enabled; }
    bool IsEarlyExitEnabled() const { return m_earlyExit; }
    ...
    /// Get scan info
    const std::vector<JPEGScanInfo>& GetScans() const { return m_scans; }
    uint32_t GetScanCount() const { return static_cast<uint32_t>(m_scans.size()); }
    const ProgressiveDecodeResult& GetResult() const { return m_result; }
...
private:
    float EstimateScanQuality(const JPEGScanInfo& scan, uint32_t scanIndex) const {
        ...
        if (scanIndex == 1) return 0.15f;  // DC only
        if (scanIndex == 2) return 0.35f;  // First AC coefficients
        if (scanIndex == 3) return 0.55f;  // More AC detail
        if (scanIndex == 4) return 0.70f;  // Good quality
        if (scanIndex == 5) return 0.82f;  // Near-final
        if (scanIndex >= 6) return (std::min)(1.0f, 0.85f + 0.025f * (scanIndex - 6));
        return 0.1f;
    }
