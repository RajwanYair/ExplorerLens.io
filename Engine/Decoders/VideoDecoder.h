...
class VideoDecoder : public IThumbnailDecoder
{
  public:
    ...
    // Configuration
    void SetSeekPosition(float normalizedPos)
    {
        m_seekPosition = normalizedPos;
    }
    float GetSeekPosition() const
    {
  ...
  private:
    ...
    HRESULT ExtractFrameMF(const wchar_t* filePath, uint32_t width, uint32_t height, HBITMAP* phBitmap);
    ...
    HRESULT ExtractFrameShell(const wchar_t* filePath, uint32_t width, uint32_t height, HBITMAP* phBitmap);
