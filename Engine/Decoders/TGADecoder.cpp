...
// Static members
const wchar_t* TGADecoder::m_extensions[] = {
 L".tga",
 L".tpic"
};

const uint32_t TGADecoder::m_extensionCount =
...
HRESULT TGADecoder::DecodeFromFile(const wchar_t* path, HBITMAP* phBitmap) {
    ...
    *phBitmap = nullptr;
    ...
    size_t pixBufSize = 0;
    if (!Security::ValidatePixelAllocation(header->width, header->height,
        header->bitsPerPixel / 8, pixBufSize)) {
        return E_FAIL;
    }
    ...
    // Create bitmap
    *phBitmap = CreateBitmapFromRGB(pixels, header->width, header->height,
        header->bitsPerPixel, flipY);
...
HRESULT TGADecoder::DecodeUncompressed(const uint8_t* data, const TGAHeader& header,
    uint8_t** ppPixels, uint32_t* pStride) {
    ...
    *ppPixels = new (std::nothrow) uint8_t[pixelDataSize];
    if (!*ppPixels) return E_OUTOFMEMORY;
    *pStride = stride;
...
HRESULT TGADecoder::DecodeRLE(const uint8_t* data, const TGAHeader& header,
    uint8_t** ppPixels, uint32_t* pStride) {
    if (!Security::ValidatePtr(ppPixels, pStride, data)) {
        return E_POINTER;
    }
    ...
    while (pixelsDecoded < totalPixels) {
        ...
        if (packetType) {
            ...
            for (uint32_t i = 0; i < pixelCount; ++i) {
                if (dest + bytesPerPixel > destEnd) break;
            ...
            if (dest + byteCount > destEnd) break;
...
bool TGADecoder::IsTGAFormat(const uint8_t* data, size_t size) {
    ...
    // Basic validation
    if (header->colorMapType > 1) return false;
    if (header->imageType > 11) return false;
...
HBITMAP TGADecoder::CreateBitmapFromRGB(const uint8_t* pixels, uint32_t width,
    uint32_t height, uint32_t bpp, bool flipY) {
