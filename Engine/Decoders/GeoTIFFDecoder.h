...
class GeoTIFFDecoder
{
  public:
    ...
    void SetBandMapping(const BandConfig& config)
    {
        m_bandConfig = config;
    }
    void SetComposite(GeoTIFFBandComposite composite)
    {
        m_composite = composite;
    }
    const GeoTIFFMetadata& GetMetadata() const
    {
        return m_metadata;
    }
                               ...
                               float percentClipHigh = 98.0f) const
    {
        ...
        for (uint32_t v = 0; v < 256; ++v) {
            ...
            if (cumulative >= percentClipLow) {
                lo = static_cast<uint8_t>(v);
                break;
            }
        }
        cumulative = 0.0f;
        for (int v = 255; v >= 0; --v) {
            cumulative += static_cast<float>(histogram[v]) / total * 100.0f;
            if (cumulative >= (100.0f - percentClipHigh)) {
                hi = static_cast<uint8_t>(v);
                break;
            }
        }
        if (lo >= hi)
  ...
  private:
    bool ParseHeader()
    {
        return m_metadata.width > 0;
    }
    bool DecodeBands()
    {
        return !m_bandData.empty();
    }
