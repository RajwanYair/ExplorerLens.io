...
const wchar_t* VideoDecoder::m_extensions[] = {
    L".mp4",  L".mkv", L".avi", L".wmv", L".mov", L".flv", L".webm", L".m4v", L".mpg",  L".mpeg", L".ts", L".mts",
    L".m2ts", L".3gp", L".3g2", L".vob", L".ogv", L".rm",  L".rmvb", L".asf", L".divx", L".xvid", nullptr};
...
HRESULT VideoDecoder::ExtractFrameMF(const wchar_t* filePath, uint32_t width, uint32_t height, HBITMAP* phBitmap)
{
    if (!phBitmap)
        return E_INVALIDARG;
    *phBitmap = nullptr;
    ...
    hr = pReader->SetCurrentMediaType(static_cast<DWORD>(MF_SOURCE_READER_FIRST_VIDEO_STREAM), nullptr, pOutputType);
    if (FAILED(hr))
        goto cleanup;
...

    {
        ...
        hr = pReader->ReadSample(static_cast<DWORD>(MF_SOURCE_READER_FIRST_VIDEO_STREAM), 0, &streamIndex, &flags,
                                 &timestamp, &pSample);
        ...
        if (SUCCEEDED(hr) && pSample) {
            ...
            if (SUCCEEDED(hr) && pBuffer) {
                ...
                if (SUCCEEDED(hr) && pData) {
                    ...
                    hr = pReader->GetCurrentMediaType(static_cast<DWORD>(MF_SOURCE_READER_FIRST_VIDEO_STREAM),
                                                      &pActualType);
...
HRESULT VideoDecoder::ExtractFrameShell(const wchar_t* filePath, uint32_t width, uint32_t height, HBITMAP* phBitmap)
{
    if (!phBitmap)
        return E_INVALIDARG;
    *phBitmap = nullptr;
...
bool VideoDecoder::GetMetadata(const wchar_t* filePath, VideoMetadata& metadata)
{
    ...
    if (SUCCEEDED(hr) && pReader) {
        ...
        hr = pReader->GetCurrentMediaType(static_cast<DWORD>(MF_SOURCE_READER_FIRST_VIDEO_STREAM), &pType);
