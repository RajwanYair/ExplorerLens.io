...
const wchar_t* ImageDecoder::m_extensions[] = {
    L".jpg", L".jpeg", L".jpe", L".jfif", L".png", L".bmp", L".dib", L".gif", L".tif", L".tiff",
...
HRESULT ImageDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result)
{
    ...
    HRESULT hr = DecodeFromFile(request.filePath, request.width, request.height, &hBitmap);
...
HRESULT ImageDecoder::DecodeFromFile(const wchar_t* path, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
    ...
    *phBitmap = nullptr;
    ...
    if (targetWidth > 0 && targetHeight > 0 && (sourceWidth != targetWidth || sourceHeight != targetHeight)) {
        ...
        hr = scaler->Initialize(orientedSource.Get(), targetWidth, targetHeight, WICBitmapInterpolationModeFant);
...
HRESULT ImageDecoder::DecodeFromStream(IStream* pStream, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
    if (!pStream || !phBitmap) {
        return E_INVALIDARG;
    }
    ...
    *phBitmap = nullptr;
    ...
    ComPtr<IWICBitmapDecoder> decoder;
    HRESULT hr = factory->CreateDecoderFromStream(pStream, nullptr, WICDecodeMetadataCacheOnDemand, &decoder);
    ...
    if (targetWidth > 0 && targetHeight > 0) {
        ...
        if (SUCCEEDED(hr) && (sourceWidth != targetWidth || sourceHeight != targetHeight)) {
            ...
            if (SUCCEEDED(hr)) {
                hr =
                    scaler->Initialize(orientedSource.Get(), targetWidth, targetHeight, WICBitmapInterpolationModeFant);
                if (SUCCEEDED(hr)) {
                    finalSource = scaler;
                }
            }
...
HRESULT ImageDecoder::ApplyEXIFOrientation(IWICBitmapFrameDecode* pFrame, IWICBitmapSource** ppCorrected)
{
