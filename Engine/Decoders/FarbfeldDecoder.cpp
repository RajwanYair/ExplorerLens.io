...
bool FarbfeldDecoder::CanDecode(const wchar_t* filePath) const
{
    ...
    return path.length() >= 3 && path.compare(path.length() - 3, 3, L".ff") == 0;
}

HRESULT FarbfeldDecoder::Decode(const wchar_t* filePath, uint32_t requestedSize, HBITMAP& hBitmap)
{
    ...
    if (fileSize < 16)
        return E_FAIL;  // Magic (8) + width (4) + height (4)
    ...
    // Validate magic
    if (!Security::ValidateMagic(data.data(), fileSize, MAGIC, 8))
        return E_FAIL;
    ...
    if (!Security::SafeMulTriple(static_cast<size_t>(width), static_cast<size_t>(height), 8, pixelDataSize))
        return E_FAIL;
    uint64_t expectedSize64 = 0;
    if (!Security::SafeAdd64(16, pixelDataSize, expectedSize64))
        return E_FAIL;
    if (fileSize < static_cast<size_t>(expectedSize64))
        return E_FAIL;

    // Security: Safe pixel buffer allocation
    size_t bgraSize = 0;
    if (!Security::ValidatePixelAllocation(width, height, 4, bgraSize))
        return E_FAIL;
