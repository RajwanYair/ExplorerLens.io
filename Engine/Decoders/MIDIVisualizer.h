...
public:
    ...
    static const wchar_t* GetName() { return L"MIDIVisualizer"; }
    ...
    MIDIFileInfo ParseHeader(const uint8_t* data, size_t size) const {
        ...
        for (uint16_t t = 0; t < info.trackCount && offset + 8 <= size; ++t) {
            if (memcmp(data + offset, "MTrk", 4) != 0) break;
            uint32_t trackLen = (data[offset + 4] << 24) | (data[offset + 5] << 16) |
            ...
            while (pos < tEnd) {
                ...
                while (pos < tEnd && (data[pos] & 0x80)) pos++;
                pos++;
                if (pos >= tEnd) break;
                ...
                else if ((status & 0xF0) == 0x80) { pos += 3; }
                else if ((status & 0xF0) == 0xB0 || (status & 0xF0) == 0xE0) { pos += 3; }
                else if ((status & 0xF0) == 0xC0 || (status & 0xF0) == 0xD0) { pos += 2; }
                else { pos++; }
    ...
    MIDIStats GetStats() const { return m_stats; }
