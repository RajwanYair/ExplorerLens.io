...
{
    switch (f) {
        case GridArchiveFormat::Unknown:
            return "Unknown";
        case GridArchiveFormat::ZIP:
            return "ZIP";
        case GridArchiveFormat::RAR:
            return "RAR";
        case GridArchiveFormat::SevenZip:
            return "7z";
        case GridArchiveFormat::TAR:
            return "TAR";
        case GridArchiveFormat::CBZ:
            return "CBZ";
        case GridArchiveFormat::CBR:
            return "CBR";
        case GridArchiveFormat::CB7:
            return "CB7";
        case GridArchiveFormat::CBT:
            return "CBT";
    }
    return "Unknown";
}
...
{
    return f == GridArchiveFormat::CBZ || f == GridArchiveFormat::CBR || f == GridArchiveFormat::CB7
           || f == GridArchiveFormat::CBT;
}
...
inline GridArchiveFormat DetectArchiveFormat(const std::string& ext)
{
    ...
    if (lower == ".zip")
        return GridArchiveFormat::ZIP;
    if (lower == ".rar")
        return GridArchiveFormat::RAR;
    if (lower == ".7z")
        return GridArchiveFormat::SevenZip;
    if (lower == ".tar" || lower == ".tar.gz" || lower == ".tgz")
        return GridArchiveFormat::TAR;
    if (lower == ".cbz")
        return GridArchiveFormat::CBZ;
    if (lower == ".cbr")
        return GridArchiveFormat::CBR;
    if (lower == ".cb7")
        return GridArchiveFormat::CB7;
    if (lower == ".cbt")
        return GridArchiveFormat::CBT;
...
{
    return {".zip", ".rar", ".7z", ".tar", ".tar.gz", ".tgz", ".cbz", ".cbr", ".cb7", ".cbt"};
}
...
{
  public:
    explicit GridLayoutEngine(uint32_t canvasSize = 256) : canvasSize_(canvasSize) {}

    uint32_t CanvasSize() const
    {
        return canvasSize_;
    }
    ...
    std::vector<GridCell> Layout2x2()
    {
        ...
        return {{0, 0, cellW, cellH, 0, false},
    ...
    std::vector<GridCell> LayoutCoverPlusThree()
    {
        ...
        return {{0, 0, coverW, static_cast<float>(canvasSize_), 0, true},
    ...
    std::vector<GridCell> GenerateLayout(GridLayoutMode mode)
    {
        switch (mode) {
            ...
            case GridLayoutMode::StripHorizontal: {
                ...
                return {{0, 0, w, static_cast<float>(canvasSize_), 0, false},
...
struct ArchiveGridConfig
{
    ...
    static ArchiveGridConfig Default()
    {
        return ArchiveGridConfig{};
    }
...
struct ArchiveGridResult
{
    ...
    {
        return imagesFound > 0 && success;
    }

    std::string Summary() const
    {
        std::ostringstream ss;
        ss << ArchiveFormatName(format) << ": " << imagesDecoded << "/" << imagesFound << " images"
