...
public:
    ...
    static const wchar_t* GetName() { return L"APNGDecoder"; }
    ...
    uint32_t ParseFrameCount(const uint8_t* data, size_t size) const {
        if (!data || size < 33) return 0;
        // Verify PNG signature
        static const uint8_t sig[] = { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A };
        if (memcmp(data, sig, 8) != 0) return 0;

        size_t offset = 8;
        while (offset + 12 <= size) {
            uint32_t chunkLen = (data[offset] << 24) | (data[offset + 1] << 16) |
                (data[offset + 2] << 8) | data[offset + 3];
            char type[5] = {};
            memcpy(type, data + offset + 4, 4);

            if (strcmp(type, "acTL") == 0 && chunkLen >= 8) {
                uint32_t numFrames = (data[offset + 8] << 24) | (data[offset + 9] << 16) |
    ...
    double ComputeEntropy(const uint8_t* pixels, size_t byteCount) const {
        if (!pixels || byteCount == 0) return 0.0;
    ...
    APNGStats GetStats() const { return m_stats; }
