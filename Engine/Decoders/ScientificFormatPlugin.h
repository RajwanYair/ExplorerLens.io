...
{
    switch (f) {
        case ScientificFormat::FITS:
            return "FITS";
        case ScientificFormat::NIfTI:
            return "NIfTI";
        case ScientificFormat::NIfTIGZ:
            return "NIfTI.gz";
        default:
            return "Unknown";
    }
...
static inline ScientificFormat DetectScientificFormat(const std::string& ext)
{
    if (ext == ".fits" || ext == ".fit" || ext == ".fts")
        return ScientificFormat::FITS;
    if (ext == ".nii")
        return ScientificFormat::NIfTI;
...
struct NIfTIHeader
{
    ...
    uint32_t MiddleSlice() const
    {
        return dimZ > 0 ? dimZ / 2 : 0;
    }
};
...
{
    switch (l) {
        case ScientificLUT::Grayscale:
            return "Grayscale";
        case ScientificLUT::Viridis:
            return "Viridis";
        case ScientificLUT::Plasma:
            return "Plasma";
        case ScientificLUT::Heat:
            return "Heat";
        case ScientificLUT::Turbo:
            return "Turbo";
        default:
            return "Custom";
    }
