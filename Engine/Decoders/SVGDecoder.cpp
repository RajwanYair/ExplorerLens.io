...
HRESULT SVGDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result) {
    ...
    HRESULT hr = RenderSVGToHBITMAP(request.filePath, request.width,
...
HRESULT SVGDecoder::RenderSVGToHBITMAP(const wchar_t* filePath, uint32_t width,
                                         uint32_t height, HBITMAP* phBitmap) {
    if (!filePath || !phBitmap) return E_INVALIDARG;
    *phBitmap = nullptr;

    // Read file
    size_t fileSize = 0;
    auto fileData = ReadFileData(filePath, fileSize);
    if (!fileData || fileSize == 0) return E_FAIL;
...
bool SVGDecoder::DecompressSVGZ(const std::vector<uint8_t>& compressed, std::string& svgContent) {
    ...
    typedef LONG(WINAPI* RtlDecompressBufferFn)(
    ...
    auto RtlDecompressBuffer = reinterpret_cast<RtlDecompressBufferFn>(
    ...
    if (flags & 0x08) { // FNAME - skip null-terminated string
        while (offset < compressed.size() && compressed[offset] != 0) offset++;
        offset++;
    }
...
bool SVGDecoder::ExtractSVGDimensions(const std::string& svgContent,
...
HBITMAP SVGDecoder::CreateSVGPlaceholder(uint32_t width, uint32_t height,
    ...
    int rectCount = 0, circleCount = 0, pathCount = 0, textCount = 0;
    {
        ...
        size_t pos = 0;
        while ((pos = lower.find("<rect", pos)) != std::string::npos) { rectCount++; pos++; }
        pos = 0;
        while ((pos = lower.find("<circle", pos)) != std::string::npos) { circleCount++; pos++; }
        pos = 0;
        while ((pos = lower.find("<path", pos)) != std::string::npos) { pathCount++; pos++; }
        pos = 0;
        while ((pos = lower.find("<text", pos)) != std::string::npos) { textCount++; pos++; }
    }
    ...
    if (totalElements == 0) totalElements = 3; // Default
    ...
    if (rectCount > 0 || totalElements > 1) {
        ...
        graphics.FillRectangle(&rectBrush, contentX + contentW * 0.5f,
                               contentY + contentH * 0.2f,
                               contentW * 0.35f, contentH * 0.3f);
    }
    ...
    if (pathCount > 0 || totalElements > 0) {
        ...
        Gdiplus::PointF pts[] = {
            {contentX + contentW * 0.1f, contentY + contentH * 0.7f},
...
std::unique_ptr<uint8_t[]> SVGDecoder::ReadFileData(const wchar_t* path, size_t& fileSize) {
    ...
    DWORD bytesRead = 0;
    if (!ReadFile(hFile, buffer.get(), static_cast<DWORD>(fileSize), &bytesRead, nullptr) ||
        bytesRead != fileSize) {
        CloseHandle(hFile);
        return nullptr;
    }
