...
class SpreadsheetPreviewDecoder
{
  public:
    static const wchar_t* FormatName(SpreadsheetFormat f)
    {
        switch (f) {
            ...
            case SpreadsheetFormat::ODS:
                return L"OpenDocument";
            case SpreadsheetFormat::Numbers:
                return L"Numbers";
            default:
                return L"Unknown";
        }
    ...
    static SpreadsheetFormat DetectFormat(const std::wstring& ext)
    {
        ...
        if (ext == L".tsv" || ext == L".tab")
            return SpreadsheetFormat::TSV;
        if (ext == L".xlsx")
            return SpreadsheetFormat::XLSX;
        if (ext == L".xls")
            return SpreadsheetFormat::XLS;
        if (ext == L".ods")
            return SpreadsheetFormat::ODS;
        if (ext == L".numbers")
            return SpreadsheetFormat::Numbers;
        return SpreadsheetFormat::CSV;
    }

    static constexpr size_t FormatCount()
    {
        return static_cast<size_t>(SpreadsheetFormat::COUNT);
    }
    static constexpr size_t CellTypeCount()
    {
        return static_cast<size_t>(CellDataType::COUNT);
    }
};
