...
class DPXDecoder : public IThumbnailDecoder
{
  public:
        ...
        for (uint32_t y = 0; y < thumbH; ++y) {
            ...
            for (uint32_t x = 0; x < thumbW; ++x) {
                ...
                if (hdr.bitDepth == 10) {
                    ...
                    size_t wordOffset = (pixelIndex / 1) * 4;  // 1 pixel per 32-bit word for DPX
                    ...
                    size_t bytesPerPixel = static_cast<size_t>(hdr.channels) * (hdr.bitDepth / 8);
                    size_t offset = (static_cast<size_t>(srcY) * hdr.width + srcX) * bytesPerPixel;
    ...
    static constexpr uint32_t TransferTypeCount()
    {
        return 6;
    }
    ...
    static DPXHeader ParseDPXHeader(const uint8_t* data, size_t size)
    {
        ...
        hdr.valid =
            (hdr.width > 0 && hdr.height > 0 && hdr.width <= 32768 && hdr.height <= 32768 && hdr.imageOffset < size);
        return hdr;
    }
    ...
    {
        return static_cast<uint8_t>((static_cast<uint32_t>(logValue) * 255u) / 1023u);
    }
