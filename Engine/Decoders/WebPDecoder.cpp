...
HRESULT WebPDecoder::DecodeFromFile(const wchar_t* path, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
...
HRESULT WebPDecoder::DecodeFromMemory(const BYTE* data, size_t size, UINT targetWidth, UINT targetHeight,
                                      HBITMAP* phBitmap)
{
    ...
    *phBitmap = nullptr;
    ...
    HBITMAP hBitmap = CreateBitmapFromRGBA(config.output.u.RGBA.rgba, config.output.width, config.output.height);
...
void WebPDecoder::AddAnimationBadge(HBITMAP hBitmap, int width, int height)
{
    ...
    POINT pts[3] = {{x + badgeSize2 / 3, y + badgeSize2 / 4},
