...
public:
    ...
    const wchar_t* GetName() const override { return L"TGADecoder"; }
...
private:
    ...
    HRESULT DecodeUncompressed(const uint8_t* data, const TGAHeader& header,
                               uint8_t** ppPixels, uint32_t* pStride);
    HRESULT DecodeRLE(const uint8_t* data, const TGAHeader& header,
                     uint8_t** ppPixels, uint32_t* pStride);
