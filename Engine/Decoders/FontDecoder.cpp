...
const wchar_t* FontDecoder::m_extensions[] = {L".ttf", L".otf", L".woff", L".woff2",
                                              L".ttc", L".fon", L".fnt",  nullptr};
...
HRESULT FontDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result)
{
    ...
    HRESULT hr = RenderFontPreview(request.filePath, request.width, request.height, &result.hBitmap);

    // Try Shell font thumbnail if DirectWrite failed
    if (FAILED(hr) || !result.hBitmap) {
        hr = ExtractFontPreviewShell(request.filePath, request.width, request.height, &result.hBitmap);
    }
...
HRESULT FontDecoder::RenderFontPreview(const wchar_t* filePath, uint32_t width, uint32_t height, HBITMAP* phBitmap)
{
    if (!phBitmap)
        return E_INVALIDARG;
    *phBitmap = nullptr;
    ...
    hr = pDWriteFactory->CreateFontFace(fontFaceType, 1, fontFiles, 0, DWRITE_FONT_SIMULATIONS_NONE, &pFontFace);
    pFontFile->Release();
    if (FAILED(hr)) {
        pDWriteFactory->Release();
        CoUninitialize();
    ...
    hr = pDWriteFactory->CreateTextFormat(L"Arial", nullptr, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,
                                          DWRITE_FONT_STRETCH_NORMAL, fontSize, L"en-us", &pTextFormat);
    if (SUCCEEDED(hr)) {
        ...
        hr = pDWriteFactory->CreateTextLayout(sampleText, textLength, pTextFormat, static_cast<float>(width),
                                              static_cast<float>(height), &pTextLayout);
        if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
        hr = pConverter->Initialize(pWICBitmap, GUID_WICPixelFormat32bppBGR, WICBitmapDitherTypeNone, nullptr, 0.0,
...
HRESULT FontDecoder::ExtractFontPreviewShell(const wchar_t* filePath, uint32_t width, uint32_t height, HBITMAP* phBitmap)
{
    if (!phBitmap)
        return E_INVALIDARG;
    *phBitmap = nullptr;
...
HBITMAP FontDecoder::CreateFontPlaceholder(uint32_t width, uint32_t height, const wchar_t* filePath)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr) && isSupportedFontType) {
                ...
                IDWriteFontFile* fontFiles[] = {pFontFile};
                IDWriteFontFace* pFontFace = nullptr;
                hr = pDWriteFactory->CreateFontFace(fontFaceType, 1, fontFiles, 0, DWRITE_FONT_SIMULATIONS_NONE,
                                                    &pFontFace);
                if (SUCCEEDED(hr)) {
                    // Get font metrics
                    DWRITE_FONT_METRICS fontMetrics;
