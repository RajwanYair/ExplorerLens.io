...
public:
    ...
    inline bool IsNeRFConfig(const uint8_t* data, size_t size) const {
        ...
        return (content.find("camera_angle_x") != std::string::npos ||
            content.find("transform_matrix") != std::string::npos ||
            content.find("frames") != std::string::npos);
    }
    ...
    inline NeRFSceneInfo ParseConfig(const uint8_t* data, size_t size) const {
        ...
        if (info.aabbScale == 0) info.aabbScale = 1;
    ...
    inline std::vector<uint8_t> GenerateCameraVisualization(const NeRFSceneInfo& info,
        uint32_t width, uint32_t height) const {
        std::vector<uint8_t> thumbnail(static_cast<size_t>(width) * height * 3, 25);
        if (width == 0 || height == 0 || info.frames.empty()) return thumbnail;
        ...
        DrawDot(thumbnail.data(), width, height,
            static_cast<int>(cx), static_cast<int>(cy), 6, 255, 100, 50);
    ...
    inline std::string FormatToString(NeRFFormat fmt) const {
        switch (fmt) {
        case NeRFFormat::InstantNGP:     return "Instant-NGP";
        case NeRFFormat::Nerfies:        return "Nerfies";
        case NeRFFormat::Mip360:         return "Mip-NeRF 360";
        case NeRFFormat::TensoRF:        return "TensoRF";
        case NeRFFormat::GaussianSplat:  return "3D Gaussian Splatting";
        default:                         return "Unknown NeRF";
        }
    }
...
private:
    ...
    inline NeRFFormat DetectFormat(const std::string& content) const {
        if (content.find("aabb_scale") != std::string::npos) return NeRFFormat::InstantNGP;
        if (content.find("nerfies") != std::string::npos) return NeRFFormat::Nerfies;
        if (content.find("ply_file_path") != std::string::npos) return NeRFFormat::GaussianSplat;
    ...
    inline float ExtractFloatValue(const std::string& json, const std::string& key) const {
        ...
        while (pos < json.size()) {
            char c = json[pos];
            if (c >= '0' && c <= '9') {
                if (hasDot) { divisor *= 10.0f; result += (c - '0') / divisor; }
                else result = result * 10.0f + (c - '0');
            }
    ...
    inline void DrawDot(uint8_t* pixels, uint32_t w, uint32_t h, int cx, int cy, int r,
