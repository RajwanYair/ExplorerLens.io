...
// Static members
const wchar_t* ICODecoder::m_extensions[] = {
 L".ico",
 L".cur"
};

const uint32_t ICODecoder::m_extensionCount = 
...
HRESULT ICODecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result) {
 ...
 HRESULT hr = DecodeFromFile(request.filePath, 
 request.width, 
 request.height,
 &result.hBitmap);
...
HRESULT ICODecoder::DecodeFromFile(const wchar_t* path, UINT targetWidth, 
 ...
 *phBitmap = nullptr;
 ...
 // Copy pixels
 UINT stride = width * 4;
 hr = converter->CopyPixels(nullptr, stride, stride * height, 
...
HRESULT ICODecoder::SelectBestFrame(IWICBitmapDecoder* decoder, UINT targetSize,
