...
enum class XZCheckType : uint8_t {
    None = 0x00, CRC32 = 0x01, CRC64 = 0x04, SHA256 = 0x0A, Unknown = 0xFF
};
...
public:
    ...
    static const wchar_t* GetName() { return L"XZStreamDecoder"; }
    ...
    XZStreamInfo ParseStream(const uint8_t* data, size_t size) const {
        ...
        if (DetectXZMagic(data, size)) {
            ...
            if (size >= 8) {
                ...
                switch (checkByte) {
                case 0x00: info.checkType = XZCheckType::None; break;
                case 0x01: info.checkType = XZCheckType::CRC32; break;
                case 0x04: info.checkType = XZCheckType::CRC64; break;
                case 0x0A: info.checkType = XZCheckType::SHA256; break;
                default: info.checkType = XZCheckType::Unknown; break;
                }
            }
            info.blockCount = 1;
        }
            ...
            if (size >= 5) {
                ...
                uint8_t bits = 0;
                while ((1u << bits) < dictSize && bits < 32) bits++;
    ...
    XZStats GetStats() const { return m_stats; }
