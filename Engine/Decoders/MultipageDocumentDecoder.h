...
public:
    ...
    MultipageDocumentDecoderConfig GetConfig() const { return m_config; }
    ...
    bool IsMultipage(uint32_t pageCount) const { return pageCount > 1; }

    std::string FormatPageLabel(uint32_t current, uint32_t total) const {
        if (!m_config.showPageCount || total <= 1) return "";
