...
class HDRDecoder : public IThumbnailDecoder
{
  ...
  private:
    ...
    HRESULT ParseHDR(const uint8_t* data, size_t size, float** ppPixels, uint32_t* pWidth, uint32_t* pHeight);
    HRESULT DecodeScanlineRLE(const uint8_t*& p, const uint8_t* end, RGBE* scanline, uint32_t width);
