                              ...
                              uint8_t* pChannels)
{
    ...
    if (!Security::ValidateFileSize(size, Security::MAX_IMAGE_FILE_SIZE)) {
        return E_FAIL;
    }
    ...
    // Security: Safe pixel buffer allocation with overflow check
    size_t pixelBufSize = 0;
    if (!Security::ValidatePixelAllocation(width, height, 4, pixelBufSize)) {
        return E_FAIL;
    }
