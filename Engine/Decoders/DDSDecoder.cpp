...
HRESULT DDSDecoder::DecodeFromFile(const wchar_t* path, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
    if (!phBitmap)
        return E_POINTER;
    *phBitmap = nullptr;
    ...
    if (targetWidth > 0 && targetHeight > 0 && (width > targetWidth || height > targetHeight)) {
        ...
        if (SUCCEEDED(hr)) {
            hr = scaler->Initialize(converter.Get(), outW, outH, WICBitmapInterpolationModeHighQualityCubic);
            if (SUCCEEDED(hr))
                source = scaler.Get();
        }
    }
    ...
    *phBitmap = CreateDIBSection(hdc, &bmi, DIB_RGB_COLORS, &pBits, nullptr, 0);
    ...
    UINT stride = outW * 4;
    hr = source->CopyPixels(nullptr, stride, stride * outH, static_cast<BYTE*>(pBits));
    if (FAILED(hr)) {
