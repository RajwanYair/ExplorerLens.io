...
public:
    ...
    static const wchar_t* GetName() { return L"BrotliStreamInspector"; }
    ...
    BrotliStreamInfo Inspect(const uint8_t* data, size_t size) const {
        ...
        if ((firstByte & 0x01) == 0) {
            ...
            if (n == 0) info.windowBits = 17;
            else if (n <= 6) info.windowBits = 16 + n;
            else info.windowBits = 24;
        }
    ...
    BrotliStats GetStats() const { return m_stats; }
