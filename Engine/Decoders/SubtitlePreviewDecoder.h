...
enum class SubtitleFormat : uint8_t { SRT, ASS, VTT, SSA, Unknown };
...
public:
    ...
    static const wchar_t* GetName() { return L"SubtitlePreviewDecoder"; }
    ...
    SubtitleFormat DetectFormat(const wchar_t* ext) const {
        if (!ext) return SubtitleFormat::Unknown;
        std::wstring e(ext);
        for (auto& c : e) c = towlower(c);
        if (e == L".srt") return SubtitleFormat::SRT;
        if (e == L".ass" || e == L".ssa") return SubtitleFormat::ASS;
        if (e == L".vtt") return SubtitleFormat::VTT;
        return SubtitleFormat::Unknown;
    }

    /// Parse SRT timestamp "HH:MM:SS,mmm" to seconds.
    double ParseSRTTimestamp(const std::string& ts) const {
        if (ts.size() < 12) return 0.0;
    ...
    SubtitleFileInfo ParseSRT(const std::string& content) const {
        ...
        while (pos < content.size()) {
            ...
            info.cueCount++;
            if (cue.endTime > info.totalDuration) info.totalDuration = cue.endTime;
            if (info.sampleCues.size() < 5) info.sampleCues.push_back(cue);
        }
        return info;
    }

    SubtitleStats GetStats() const { return m_stats; }
