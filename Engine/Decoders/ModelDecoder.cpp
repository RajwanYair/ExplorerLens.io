...
namespace Engine {
 ...
 const_cast<wchar_t*>(L".ply"),
 const_cast<wchar_t*>(L".dae"),
 const_cast<wchar_t*>(L".3ds"),
 const_cast<wchar_t*>(L".fbx"),
 nullptr
 };
 ...
 _wcsicmp(ext, L".ply") == 0 ||
 _wcsicmp(ext, L".dae") == 0 ||
 _wcsicmp(ext, L".3ds") == 0 ||
 _wcsicmp(ext, L".fbx") == 0);
 }
 ...
 size_t fileSize = 0;
 auto data = ReadFileData(filePath, fileSize);
 if (!data || fileSize < 10) return false;

 // Parse PLY header
 std::string header(reinterpret_cast<const char*>(data.get()),
 std::min(fileSize, static_cast<size_t>(8192)));

 // Verify magic
 if (header.substr(0, 3) != "ply") return false;
 ...
 std::string fmt;
 lineStream >> fmt;
 if (fmt == "binary_little_endian") { isBinary = true; isLittleEndian = true; }
 else if (fmt == "binary_big_endian") { isBinary = true; isLittleEndian = false; }
 // else ascii
 }
 ...
 if (elemType == "vertex") { vertexCount = count; inVertexElement = true; }
 else { 
 if (elemType == "face") faceCount = count; 
 ...
 if (headerEnd == 0 || vertexCount == 0) return false;

 // Cap vertex count for thumbnail safety
 if (vertexCount > 2000000) vertexCount = 2000000;

 if (isBinary) {
 return LoadPLYBinary(data.get(), headerEnd, vertexProps, vertexCount, faceCount, isLittleEndian, mesh);
 }
 else {
 return LoadPLYAscii(reinterpret_cast<const char*>(data.get()), headerEnd, vertexProps, vertexCount, faceCount, mesh);
 }
 }
 ...
 if (vertexProps[i] == "x") xIdx = static_cast<int>(i);
 else if (vertexProps[i] == "y") yIdx = static_cast<int>(i);
 else if (vertexProps[i] == "z") zIdx = static_cast<int>(i);
 else if (vertexProps[i] == "nx") nxIdx = static_cast<int>(i);
 else if (vertexProps[i] == "ny") nyIdx = static_cast<int>(i);
 else if (vertexProps[i] == "nz") nzIdx = static_cast<int>(i);
 }
 ...
 if (!std::getline(dataStream, line)) break;
 std::istringstream lineStream(line);
 ...
 if (static_cast<int>(values.size()) < propCount) continue;
 ...
 for (int j = 0; j < vertCount; ++j) lineStream >> faceIndices[j];
 // Fan triangulation
 ...
 // Verify COLLADA
 if (content.find("<COLLADA") == std::string::npos &&
 content.find("<collada") == std::string::npos) return false;
 ...
 auto extractFloatArray = [](const std::string& xml, const std::string& startTag,
 ...
 float val;
 while (iss >> val) out.push_back(val);
 return !out.empty();
 };
 ...
 if (faPos == std::string::npos) break;
 ...
 if (sourceBlock.find("position") != std::string::npos ||
 sourceBlock.find("Position") != std::string::npos ||
 sourceBlock.find("POSITION") != std::string::npos) {
 found = extractFloatArray(content, content.substr(faPos, content.find('>', faPos) - faPos + 1), positions);
 if (found) break;
 }
 }
 ...
 float val;
 while (iss >> val) positions.push_back(val);
 if (!positions.empty()) found = true;
 }
 }
 }
 searchPos = faPos + 1;
 ...
 if (positions.size() < 9) return false; // Need at least 3 vertices (9 floats)

 // Extract indices from <p> element inside <triangles> or <polylist>
 size_t triPos = content.find("<triangles");
 if (triPos == std::string::npos) triPos = content.find("<polylist");
 ...
 if (searchEnd == std::string::npos) searchEnd = triPos + 500;
 size_t endTag = content.find("</triangles>", triPos);
 if (endTag == std::string::npos) endTag = content.find("</polylist>", triPos);
 ...
 if (inputCount > 0) indexStride = static_cast<uint32_t>(inputCount);
 }
 }
 ...
 Vertex v = {};
 v.position = DirectX::XMFLOAT3(
 positions[i + j * 3], positions[i + j * 3 + 1], positions[i + j * 3 + 2]);
 mesh.vertices.push_back(v);
 }
 }
 }
 ...
 DirectX::XMFLOAT3 e1(v1.position.x - v0.position.x,
 v1.position.y - v0.position.y,
 v1.position.z - v0.position.z);
 DirectX::XMFLOAT3 e2(v2.position.x - v0.position.x,
 ...
 float dot = normal.x * light.direction.x +
 normal.y * light.direction.y +
 normal.z * light.direction.z;
 if (dot < 0.0f) dot = -dot; // Two-sided lighting
