...
//------------------------------------------------------------------------------
bool DICOMDecoder::ParseDataElement(const uint8_t *data, size_t size,
 size_t &offset) {
 if (offset + 8 > size)
 return false;
 ...
 case 0x0010: // Rows
 if (length >= 2)
 m_info.rows = ReadUint16LE(data, offset);
 break;
 case 0x0011: // Columns
 if (length >= 2)
 m_info.columns = ReadUint16LE(data, offset);
 break;
 case 0x0100: // BitsAllocated
 if (length >= 2)
 m_info.bitsAllocated = ReadUint16LE(data, offset);
 break;
 case 0x0101: // BitsStored
 if (length >= 2)
 m_info.bitsStored = ReadUint16LE(data, offset);
 break;
 case 0x0102: // HighBit
 if (length >= 2)
 m_info.highBit = ReadUint16LE(data, offset);
 break;
 case 0x0103: // PixelRepresentation
 if (length >= 2)
 m_info.pixelRepresentation = ReadUint16LE(data, offset);
 break;
 case 0x0002: // SamplesPerPixel
 if (length >= 2)
 m_info.samplesPerPixel = ReadUint16LE(data, offset);
 break;
 case 0x0004: { // PhotometricInterpretation
 std::string pi(reinterpret_cast<const char *>(data + offset), length);
 // Trim trailing spaces
 while (!pi.empty() && pi.back() == ' ')
 pi.pop_back();
 if (pi == "MONOCHROME1")
 m_info.photometric = DICOMPhotometric::Monochrome1;
 else if (pi == "MONOCHROME2")
 m_info.photometric = DICOMPhotometric::Monochrome2;
 else if (pi == "RGB")
 m_info.photometric = DICOMPhotometric::RGB;
 else if (pi == "YBR_FULL")
 m_info.photometric = DICOMPhotometric::YBR_Full;
 else if (pi == "PALETTE COLOR")
 m_info.photometric = DICOMPhotometric::Palette;
 break;
 }
 case 0x1050: { // WindowCenter
 std::string val(reinterpret_cast<const char *>(data + offset), length);
 try {
 m_info.windowLevel.windowCenter = std::stod(val);
 m_info.windowLevel.isDefault = false;
 } catch (...) {
 }
 break;
 }
 ...
 std::string val(reinterpret_cast<const char *>(data + offset), length);
 try {
 m_info.windowLevel.windowWidth = std::stod(val);
 m_info.windowLevel.isDefault = false;
 } catch (...) {
 }
 break;
...
bool DICOMDecoder::ParseHeader(const uint8_t *data, size_t size) {
 ...
 while (offset < size) {
 if (!ParseDataElement(data, size, offset))
 break;
 }
...
//------------------------------------------------------------------------------
std::vector<uint8_t>
DICOMDecoder::ExtractThumbnail(const uint8_t *data, size_t size,
 uint32_t thumbWidth,
 ...
 size_t srcIdx =
 m_info.pixelDataOffset + (srcY * m_info.columns + srcX) * 2;
 if (srcIdx + 1 < size) {
 ...
 size_t srcIdx =
 m_info.pixelDataOffset + (srcY * m_info.columns + srcX) * 3;
 if (srcIdx + 2 < size) {
