...
struct GeoExtent
{
    ...
    inline double CenterLon() const
    {
        return (minLon + maxLon) * 0.5;
    }
    inline double CenterLat() const
    {
...
class GeospatialTileDecoder
{
  public:
    ...
    inline std::vector<uint8_t> GenerateMapPreview(const GeoExtent& extent, uint32_t width, uint32_t height) const
    {
    ...
    {
        switch (fmt) {
            case GeoTileFormat::MBTiles:
                return "MBTiles";
            case GeoTileFormat::GeoTIFF:
                return "GeoTIFF";
            case GeoTileFormat::GeoPackage:
                return "GeoPackage";
            case GeoTileFormat::Shapefile:
                return "Shapefile";
            default:
                return "Unknown";
        }
  ...
  private:
    ...
    inline void DrawGridLines(uint8_t* pixels, uint32_t w, uint32_t h, const GeoExtent& ext) const
    {
        for (double lon = -180.0; lon <= 180.0; lon += 30.0) {
            if (lon < ext.minLon || lon > ext.maxLon)
                continue;
        ...
        for (double lat = -90.0; lat <= 90.0; lat += 30.0) {
            if (lat < ext.minLat || lat > ext.maxLat)
                continue;
