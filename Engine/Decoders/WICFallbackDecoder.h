...
struct WICFallbackStats {
    ...
    double successRate() const { return totalAttempts > 0 ? 100.0 * successCount / totalAttempts : 0.0; }
};

class WICFallbackDecoder {
public:
    void Configure(const WICFallbackConfig& config) { m_config = config; }

    bool IsSupported(const std::wstring& extension) const {
        if (extension == L".bmp" || extension == L".png" || extension == L".jpg" ||
            extension == L".jpeg" || extension == L".tif" || extension == L".tiff" ||
            extension == L".gif" || extension == L".ico" || extension == L".dds" ||
            extension == L".wdp" || extension == L".jxr")
            return true;
        return false;
    }

    WICCodecType DetectCodec(const std::wstring& extension) const {
        if (extension == L".bmp") return WICCodecType::BMP;
        if (extension == L".png") return WICCodecType::PNG;
        if (extension == L".jpg" || extension == L".jpeg") return WICCodecType::JPEG;
        if (extension == L".tif" || extension == L".tiff") return WICCodecType::TIFF;
        if (extension == L".gif") return WICCodecType::GIF;
        if (extension == L".ico") return WICCodecType::ICO;
        if (extension == L".dds") return WICCodecType::DDS;
        if (extension == L".wdp" || extension == L".jxr") return WICCodecType::WMPhoto;
        return WICCodecType::Unknown;
    }

    bool ValidateDimensions(uint32_t width, uint32_t height) const {
        return width > 0 && height > 0 &&
            width <= m_config.maxImageDimension &&
            height <= m_config.maxImageDimension;
    }

    WICFallbackStats GetStats() const { return m_stats; }
