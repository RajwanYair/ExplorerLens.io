...
const uint32_t RAWDecoder::m_extensionCount = static_cast<uint32_t>(sizeof(m_extensions) / sizeof(m_extensions[0]));
...
RAWDecoder::RAWDecoder() : m_pImpl(std::make_unique<Impl>())
{
...
HRESULT RAWDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result)
{
    ...
    LOG_INFO(L"RAWDecoder: Decoding %s (%dx%d)", request.filePath, request.width, request.height);

    // Try embedded thumbnail first (fast path)
    HRESULT hr = ExtractEmbeddedThumbnail(request.filePath, request.width, request.height, &result.hBitmap);
    ...
    hr = DecodeFullRAW(request.filePath, request.width, request.height, &result.hBitmap);
...
HRESULT RAWDecoder::DecodeFromFile(const wchar_t* path, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
...
HRESULT RAWDecoder::ExtractEmbeddedThumbnail(const wchar_t* path, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
    if (!phBitmap) {
        return E_POINTER;
    }
    ...
    *phBitmap = NULL;
    ...
    try {
        ...
        if (thumbnail->type == LIBRAW_IMAGE_JPEG) {
            ...
            *phBitmap = CreateBitmapFromRGB(thumbnail->data, thumbnail->width, thumbnail->height,
...
HRESULT RAWDecoder::DecodeFullRAW(const wchar_t* path, UINT targetWidth, UINT targetHeight, HBITMAP* phBitmap)
{
    if (!phBitmap) {
        return E_POINTER;
    }
    ...
    *phBitmap = NULL;
    ...
    try {
        ...
        // Create bitmap from RGB data
        *phBitmap = CreateBitmapFromRGB(image->data, image->width, image->height, rawProcessor.imgdata.sizes.flip);
...
HBITMAP RAWDecoder::ScaleToTarget(HBITMAP hSource, UINT targetWidth, UINT targetHeight)
{
    ...
    if (static_cast<UINT>(bm.bmWidth) <= targetWidth && static_cast<UINT>(bm.bmHeight) <= targetHeight) {
        return hSource;
    }
...
const wchar_t* RAWDecoder::m_extensions[] = {L".cr2", L".cr3", L".nef", L".arw", L".dng", L".orf", L".rw2", L".raf"};
const uint32_t RAWDecoder::m_extensionCount = 8;

RAWDecoder::RAWDecoder() {}
RAWDecoder::~RAWDecoder() {}
bool RAWDecoder::CanDecode(const wchar_t*)
{
    return false;
}
HRESULT RAWDecoder::Decode(const ThumbnailRequest&, ThumbnailResult&)
{
    return E_NOTIMPL;
}
bool RAWDecoder::GetRAWMetadata(const wchar_t*, RAWMetadata&)
{
    return false;
}
DecoderInfo RAWDecoder::GetInfo() const
