...
bool SGIDecoder::IsSGIExtension(const std::string& ext)
{
    ...
    return lower == ".sgi" || lower == ".rgb" || lower == ".rgba" || lower == ".bw" || lower == ".int"
           || lower == ".inta";
}
...
SGIDecoder::ImageInfo SGIDecoder::ReadInfo(const std::string& filePath) const
{
    ...
    if (info.bytesPerChannel != 1 && info.bytesPerChannel != 2)
        return info;
    ...
    // Validate ranges
    if (info.width == 0 || info.width > 16384)
        return info;
    if (info.height == 0 || info.height > 16384)
        return info;
...
//==========================================================================
size_t SGIDecoder::DecodeRLEScanline(const uint8_t* src, size_t srcLen, uint8_t* dst, size_t dstLen)
{
    size_t si = 0, di = 0;
...
//==========================================================================
SGIDecoder::DecodeResult SGIDecoder::Decode(const std::string& filePath, uint32_t targetWidth) const
{
    (void)targetWidth;
    DecodeResult result;
    ...
    if (!ExplorerLens::Engine::Security::ValidateFileSize(fileSize,
                                                          ExplorerLens::Engine::Security::MAX_IMAGE_FILE_SIZE)) {
    ...
    // Security: Validate pixel allocation with overflow check
    size_t pixBufSize = 0;
    if (!ExplorerLens::Engine::Security::ValidatePixelAllocation(w, h, 4, pixBufSize)) {
        result.error = "Image dimensions too large";
        return result;
    }

    result.width = w;
    result.height = h;
    result.pixelData.resize(pixBufSize);
    ...
    if (info.storage == SGIStorageType::Verbatim && info.bytesPerChannel == 1) {
        ...
        for (uint32_t y = 0; y < h; ++y) {
            ...
            for (uint32_t x = 0; x < w; ++x) {
                ...
                uint8_t r = 0, g = 0, b = 0, a = 255;
                if (ch >= 1)
                    r = channelData[srcIdx];
                if (ch >= 2)
                    g = channelData[planeSize + srcIdx];
                if (ch >= 3)
                    b = channelData[2 * planeSize + srcIdx];
                if (ch >= 4)
                    a = channelData[3 * planeSize + srcIdx];
