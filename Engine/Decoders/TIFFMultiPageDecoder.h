...
class TIFFMultiPageDecoder
{
  public:
    ...
    TIFFDecodeResult DecodeBestPage(const uint8_t* data, size_t size, const TIFFDecodeOptions& opts = {}) const;

    // Decode a specific page by 0-based IFD index.
    TIFFDecodeResult DecodePage(const uint8_t* data, size_t size, uint32_t pageIndex,
    ...
    static bool LooksLikeTIFF(const uint8_t* data, size_t size)
    {
        ...
        if (!le && !be)
            return false;
        uint16_t magic =
            le ? static_cast<uint16_t>(data[2] | (data[3] << 8)) : static_cast<uint16_t>((data[2] << 8) | data[3]);
  ...
  private:
    ...
    TIFFDecodeResult DecodeInternal(const uint8_t* data, size_t size, uint32_t ifdIndex,
                                    const TIFFDecodeOptions& opts) const;

    static void CMYKtoSRGB(std::vector<uint8_t>& pixels, uint32_t width, uint32_t height);
};
