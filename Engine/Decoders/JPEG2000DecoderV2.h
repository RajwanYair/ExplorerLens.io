...
public:
    ...
    JP2V2DecodeResult Decode(const uint8_t* data, size_t dataSize,
        const JP2V2DecodeOptions& options = {}) {
        ...
        size_t pixelSize = static_cast<size_t>(result.outputWidth) *
            result.outputHeight * result.outputChannels;
        ...
        result.decodeTimeMs = static_cast<uint64_t>(
    ...
    bool Validate() const {
        ...
        for (uint32_t i = 0; i <= 5; ++i) {
            if (!SupportsProfile(static_cast<JP2V2Profile>(i))) return false;
        }
        return true;
    }
    ...
    JP2V2ImageInfo ParseHeader(const uint8_t* data, size_t dataSize) const {
        ...
        for (size_t i = 0; i + 10 < dataSize; ++i) {
            if (data[i] == 0xFF && data[i + 1] == 0x51) {
                if (i + 10 < dataSize) {
                    info.width = (static_cast<uint32_t>(data[i + 6]) << 24) |
                        (static_cast<uint32_t>(data[i + 7]) << 16) |
                        (static_cast<uint32_t>(data[i + 8]) << 8) |
                        data[i + 9];
                    if (i + 14 < dataSize) {
                        info.height = (static_cast<uint32_t>(data[i + 10]) << 24) |
                            (static_cast<uint32_t>(data[i + 11]) << 16) |
                            (static_cast<uint32_t>(data[i + 12]) << 8) |
                            data[i + 13];
                    }
                }
                break;
        ...
        if (info.width == 0) info.width = 256;
        if (info.height == 0) info.height = 256;
        return info;
    }
