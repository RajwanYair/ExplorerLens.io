...
{
    size_t result = 0;
    if (!SafeMulTriple(static_cast<size_t>(width), static_cast<size_t>(height), static_cast<size_t>(bytesPerPixel),
                       result)) {
        return 0;
    }
    return result;
...
inline bool ValidateDimensions(uint32_t width, uint32_t height, uint32_t maxDim = MAX_THUMBNAIL_DIMENSION) noexcept
{
    if (width == 0 || height == 0)
        return false;
...
inline bool ValidatePixelAllocation(uint32_t width, uint32_t height, uint32_t bytesPerPixel, size_t& outSize) noexcept
{
    if (!ValidateDimensions(width, height))
        return false;
    outSize = SafePixelBufferSize(width, height, bytesPerPixel);
    if (outSize == 0 || outSize > MAX_DECODE_ALLOCATION)
        return false;
    return true;
}
...
inline bool ValidateBufferRead(size_t offset, size_t count, size_t itemSize, size_t bufferSize) noexcept
{
    size_t totalBytes = 0;
    if (!SafeMul64(count, itemSize, reinterpret_cast<uint64_t&>(totalBytes)))
        return false;
...
inline bool ValidateMagic(const uint8_t* data, size_t dataSize, const void* magic, size_t magicSize) noexcept
{
    if (!data || dataSize < magicSize)
        return false;
    return memcmp(data, magic, magicSize) == 0;
}

/// Check magic bytes at an arbitrary offset.
inline bool ValidateMagicAt(const uint8_t* data, size_t dataSize, size_t offset, const void* magic,
                            size_t magicSize) noexcept
{
    if (!ValidateBufferAccess(offset, magicSize, dataSize))
        return false;
...
struct SafeRLEReader
{
    ...
    bool CanReadSrc(size_t n) const noexcept
    {
        return src + n <= srcEnd;
    }
    bool CanWriteDst(size_t n) const noexcept
    {
        return dst + n <= dstEnd;
    }
    size_t SrcRemaining() const noexcept
    {
        return static_cast<size_t>(srcEnd - src);
    }
    size_t DstRemaining() const noexcept
    {
        return static_cast<size_t>(dstEnd - dst);
    }
...
enum class DecoderSandboxLevel : uint8_t {
    None,
    Standard,
    Strict,
    Paranoid
...
{
    switch (l) {
        case DecoderSandboxLevel::None:
            return "None";
        case DecoderSandboxLevel::Standard:
            return "Standard";
        case DecoderSandboxLevel::Strict:
            return "Strict";
        case DecoderSandboxLevel::Paranoid:
            return "Paranoid";
        default:
            return "Unknown";
    }
...
enum class SandboxResourceLimit : uint8_t {
    Memory,
    CPU,
    Disk,
    NetworkBandwidth
...
{
    switch (r) {
        case SandboxResourceLimit::Memory:
            return "Memory";
        case SandboxResourceLimit::CPU:
            return "CPU";
        case SandboxResourceLimit::Disk:
            return "Disk";
        case SandboxResourceLimit::NetworkBandwidth:
