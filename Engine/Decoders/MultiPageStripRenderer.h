...
{
    static const char* names[] = {"HorizontalStrip", "VerticalStack", "Grid2x2", "CoverPlusPeek", "FanSpread"};
...
struct StripRenderConfig
{
    ...
    static StripLayout AutoSelectLayout(uint32_t pageCount, double /*avgAspectRatio*/)
    {
        ...
        if (pageCount == 2)
            return StripLayout::HorizontalStrip;
        if (pageCount <= 4)
            return StripLayout::Grid2x2;
        return StripLayout::CoverPlusPeek;
    }
};
...
class MultiPageStripRenderer
{
  public:
    ...
    StripRenderConfig GetConfig() const
    {
        return m_config;
    }
    ...
    StripComposition ComputeComposition() const
    {
        ...
        uint32_t pagesToRender = (std::min)(m_config.maxPages, static_cast<uint32_t>(m_pages.size()));
    ...
    static StripLayout RecommendLayout(const std::wstring& formatType, uint32_t pageCount)
    {
        if (pageCount == 1)
            return StripLayout::CoverPlusPeek;
  ...
  private:
    ...
    void ComputeCoverPeekLayout(StripComposition& result, uint32_t count) const
    {
        ...
        if (peekCount > 0) {
            float peekHeight =
                (m_config.outputHeight - (peekCount - 1) * m_config.pageGap) / static_cast<float>(peekCount);
