...
enum class CertEncoding : uint8_t {
    PEM,
    DER,
    PKCS7,
    PKCS12,
...
class CertificateViewer
{
  public:
    ...
    bool CanDecode(const wchar_t* ext) const
    {
        ...
        for (auto& c : e)
            c = towlower(c);
        return e == L".pem" || e == L".crt" || e == L".cer" || e == L".der" || e == L".pfx" || e == L".p12"
               || e == L".p7b" || e == L".p7c";
    }
    ...
    CertEncoding DetectEncoding(const uint8_t* data, size_t size) const
    {
        ...
        if (memcmp(data, "-----BEGIN", 10) == 0)
            return CertEncoding::PEM;
        // DER starts with ASN.1 SEQUENCE tag (0x30)
        if (data[0] == 0x30)
            return CertEncoding::DER;
        return CertEncoding::Unknown;
    }
    ...
    {
        CertViewerInfo cert;
        if (pemText.find("-----BEGIN CERTIFICATE-----") == std::string::npos)
            return cert;
        cert.isValid = true;
        ...
        if (start != std::string::npos && end != std::string::npos) {
            ...
            if (derSize > 1500)
                cert.keySizeBits = 4096;
            else if (derSize > 800)
                cert.keySizeBits = 2048;
            else if (derSize > 400)
                cert.keySizeBits = 1024;
            else
                cert.keySizeBits = 256;  // Likely ECDSA
        }
