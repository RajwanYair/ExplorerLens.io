...
const wchar_t* PPMDecoder::m_extensions[] = {L".ppm", L".pgm", L".pbm", L".pnm", L".pam", L".pfm"};
...
const wchar_t** PPMDecoder::GetSupportedExtensions() const
{
    return m_extensions;
}
    ...
    *phBitmap = nullptr;
    ...
    if (magic[0] == 'P') {
        switch (magic[1]) {
            case '1':
            case '4':
                return DecodePBM(data.get(), fileSize, phBitmap);
            case '2':
            case '5':
                return DecodePGM(data.get(), fileSize, phBitmap);
            case '3':
            case '6':
                return DecodePPM(data.get(), fileSize, phBitmap);
        }
    }
    return E_FAIL;
}
...
HRESULT PPMDecoder::DecodePPM(const uint8_t* data, size_t size, HBITMAP* phBitmap)
{
    ...
    size_t bgraSize = 0;
    if (!Security::ValidatePixelAllocation(static_cast<uint32_t>(width), static_cast<uint32_t>(height), 4, bgraSize))
        return E_FAIL;
...
HRESULT PPMDecoder::DecodePFM(const uint8_t* data, size_t size, HBITMAP* phBitmap)
{
    ...
    if ((const char*)(fp + floatsNeeded) > end)
        return E_FAIL;
    ...
    if (!littleEndian) {
        ...
        for (size_t i = 0; i < floatsNeeded; ++i) {
            ...
            bits =
                ((bits >> 24) & 0xFF) | ((bits >> 8) & 0xFF00) | ((bits << 8) & 0xFF0000) | ((bits << 24) & 0xFF000000);
