...
enum class MDBlockType : uint8_t {
    Heading1, Heading2, Heading3, Paragraph, CodeBlock,
    ListItem, BlockQuote, HorizontalRule, Image, Table
};
...
public:
    ...
    static const wchar_t* GetName() { return L"MarkdownPreviewRenderer"; }
    ...
    std::vector<MDBlock> Parse(const std::wstring& content) const {
        ...
        if (content.empty()) return blocks;
        ...
        while (pos < content.size()) {
            ...
            if (line.find(L"### ") == 0) { block.type = MDBlockType::Heading3; block.text = line.substr(4); }
            else if (line.find(L"## ") == 0) { block.type = MDBlockType::Heading2; block.text = line.substr(3); }
            else if (line.find(L"# ") == 0) { block.type = MDBlockType::Heading1; block.text = line.substr(2); }
            else if (line.find(L"```") == 0) { block.type = MDBlockType::CodeBlock; block.text = line; }
            else if (line.find(L"- ") == 0 || line.find(L"* ") == 0) { block.type = MDBlockType::ListItem; block.text = line.substr(2); }
            else if (line.find(L"> ") == 0) { block.type = MDBlockType::BlockQuote; block.text = line.substr(2); }
            else if (line == L"---" || line == L"***") { block.type = MDBlockType::HorizontalRule; }
            else { block.type = MDBlockType::Paragraph; block.text = line; }
    ...
    MarkdownDocInfo GetDocInfo(const std::vector<MDBlock>& blocks) const {
        ...
        for (const auto& b : blocks) {
            ...
            case MDBlockType::Heading3:
                info.headingCount++;
                if (info.title.empty()) info.title = b.text;
                break;
            case MDBlockType::CodeBlock: info.codeBlockCount++; break;
            case MDBlockType::Paragraph: info.paragraphCount++; break;
            case MDBlockType::ListItem: info.listItemCount++; break;
            default: break;
            }
        }
        return info;
    ...
    MarkdownStats GetStats() const { return m_stats; }
