...
class HDF5ThumbnailDecoder
{
  public:
    ...
    bool DecodeFromFile(const std::wstring& filePath, uint32_t targetWidth, uint32_t targetHeight)
    {
        ...
        if (m_decoded && !m_datasets.empty())
            AutoSelectDataset();
        return m_decoded;
    }

    const std::vector<HDF5DatasetInfo>& ListDatasets() const
    {
        return m_datasets;
    }
    ...
    void SetColorMap(HDF5ColorMap colorMap)
    {
        m_colorMap = colorMap;
    }
    HDF5ColorMap GetColorMap() const
    {
        return m_colorMap;
    }
    ...
    void AutoSelectDataset()
    {
        ...
        for (const auto& ds : m_datasets) {
            ...
            if (ds.type == HDF5DatasetType::Image)
                score *= 10;
            else if (ds.rank == 2)
                score *= 5;
            if (score > bestScore) {
                bestScore = score;
                m_selectedDataset = ds.name;
            }
        }
    }

    const std::string& GetSelectedDataset() const
    {
  ...
  private:
    ...
    bool MapToColormap(std::vector<uint8_t>& /*rgbOut*/) const
    {
        return true;
    }
