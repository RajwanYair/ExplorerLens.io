...
public:
    ...
    const wchar_t* GetName() const override { return L"RAWDecoder"; }
...
private:
    ...
    HRESULT ExtractEmbeddedThumbnail(const wchar_t* path, UINT targetWidth,
                                     UINT targetHeight, HBITMAP* phBitmap);
    
    // Full RAW decode with demosaicing (slow but high quality)
    HRESULT DecodeFullRAW(const wchar_t* path, UINT targetWidth,
                         UINT targetHeight, HBITMAP* phBitmap);
