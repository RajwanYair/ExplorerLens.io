...
HRESULT ExampleDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result)
{
    ...
    LogDebug(L"ExampleDecoder::Decode() - File: %s, Target: %ux%u", request.filePath, request.width, request.height);
    ...
    std::vector<uint8_t> rgbaPixels = DecodeImageData(fileData.data(), fileData.size(), &imageWidth, &imageHeight);
    ...
    LogDebug(L"ExampleDecoder::Decode() - Decoded %dx%d image (%zu bytes)", imageWidth, imageHeight, rgbaPixels.size());
    ...
    if (request.flags & ThumbnailFlags::PreserveAspect) {
        CalculateAspectDimensions(imageWidth, imageHeight, request.width, request.height, targetWidth, targetHeight);

        LogDebug(L"ExampleDecoder::Decode() - Aspect-preserving scale: %dx%d -> %ux%u", imageWidth, imageHeight,
    ...
    if (imageWidth == (int)targetWidth && imageHeight == (int)targetHeight) {
        ...
        scaledPixels = ScaleRGBA(rgbaPixels.data(), imageWidth, imageHeight, targetWidth, targetHeight);

        LogDebug(L"ExampleDecoder::Decode() - Scaled image to %ux%u", targetWidth, targetHeight);
    }
    ...
    HBITMAP hBitmap = CreateBitmapFromBGRA(scaledPixels.data(), targetWidth, targetHeight);
    ...
    LogDebug(L"ExampleDecoder::Decode() - SUCCESS: Created %ux%u thumbnail", targetWidth, targetHeight);
...
std::vector<uint8_t> ExampleDecoder::DecodeImageData(const uint8_t* /*fileData*/, size_t /*fileSize*/, int* outWidth,
                                                     int* outHeight) const
{
    //==========================================================================
    ...
    *outHeight = 512;
    ...
    for (int y = 0; y < *outHeight; y++) {
        for (int x = 0; x < *outWidth; x++) {
            ...
            pixels[idx + 0] = static_cast<uint8_t>((x * 255) / *outWidth);   // R
            pixels[idx + 1] = static_cast<uint8_t>((y * 255) / *outHeight);  // G
            pixels[idx + 2] = 128;                                           // B
...
HRESULT ExampleDecoder::ReadFileToBuffer(const wchar_t* filePath, std::vector<uint8_t>& buffer) const
{
...
std::vector<uint8_t> ExampleDecoder::ScaleRGBA(const uint8_t* srcPixels, int srcWidth, int srcHeight, int dstWidth,
                                               int dstHeight) const
{
...
HBITMAP ExampleDecoder::CreateBitmapFromBGRA(const uint8_t* pixels, uint32_t width, uint32_t height) const
{
    ...
    void* pBits = nullptr;
    HBITMAP hBitmap = CreateDIBSection(nullptr,         // Device context (null = screen DC)
                                       &bmi,            // Bitmap info
...
void ExampleDecoder::CalculateAspectDimensions(int sourceWidth, int sourceHeight, uint32_t maxWidth, uint32_t maxHeight,
