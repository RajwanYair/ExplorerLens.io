...
VTFDecoder::TextureInfo VTFDecoder::ReadInfo(const std::string& filePath) const
{
    ...
    if (hdr.signature[0] != 'V' || hdr.signature[1] != 'T' || hdr.signature[2] != 'F' || hdr.signature[3] != '\0') {
        return info;
    }
...
//==========================================================================
VTFDecoder::DecodeResult VTFDecoder::Decode(const std::string& filePath, uint32_t targetWidth) const
{
    DecodeResult result;
    ...
    if (!ExplorerLens::Engine::Security::ValidateFileSize(fileSize,
                                                          ExplorerLens::Engine::Security::MAX_TEXTURE_FILE_SIZE)) {
        result.error = "File too large";
        return result;
    }

    // Security: Dimension validation
    if (!ExplorerLens::Engine::Security::ValidateDimensions(info.width, info.height)) {
        result.error = "Texture dimensions too large";
        return result;
    }
    ...
    uint32_t bestMip = 0;
    {
        ...
        const uint32_t maxMip = (info.mipmapCount > 0) ? static_cast<uint32_t>(info.mipmapCount - 1) : 0u;
        while (bestMip < maxMip) {
            uint32_t nextW = (w > 1) ? w / 2 : 1;
            uint32_t nextH = (h > 1) ? h / 2 : 1;
            if (nextW <= targetWidth && nextH <= targetWidth)
                break;
            w = nextW;
            h = nextH;
            bestMip++;
...
//==========================================================================
void VTFDecoder::DecompressBC1Block(const uint8_t* block, uint8_t* output, uint32_t outputStride)
{
...
//==========================================================================
void VTFDecoder::DecompressDXT1(const uint8_t* src, uint8_t* dst, uint32_t width, uint32_t height)
{
...
//==========================================================================
void VTFDecoder::DecompressDXT5(const uint8_t* src, uint8_t* dst, uint32_t width, uint32_t height)
{
    uint32_t blocksW = (width + 3) / 4;
    uint32_t blocksH = (height + 3) / 4;
