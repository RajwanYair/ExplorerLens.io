...
class HEIFDecoder : public IThumbnailDecoder
{
  ...
  private:
    ...
    uint8_t* DecodeHEIFImage(const uint8_t* fileData, size_t dataSize, uint32_t targetWidth, uint32_t targetHeight,
    ...
    /// </summary>
    uint8_t* ExtractEmbeddedThumbnail(const uint8_t* fileData, size_t dataSize, uint32_t& outWidth, uint32_t& outHeight,
    ...
    static constexpr const wchar_t* s_extensions[10] = {L".heif",          L".heic", L".hif",  L".heifs",
                                                        L".heics",         L".avci", L".avcs", L".heif-sequence",
