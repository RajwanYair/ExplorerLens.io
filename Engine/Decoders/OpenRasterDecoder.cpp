...
//==========================================================================
static bool FindFileInZIP(const std::vector<uint8_t>& zipData, const std::string& targetName,
                          std::vector<uint8_t>& outData)
{
    ...
    while (pos + sizeof(ZIPLocalFileHeader) < zipData.size()) {
        ...
        if (nameEnd > zipData.size())
            break;

        std::string fileName(reinterpret_cast<const char*>(zipData.data() + nameStart), hdr->fileNameLength);
...
//==========================================================================
static bool DecodePNGFromMemory(const std::vector<uint8_t>& pngData, uint32_t& outWidth, uint32_t& outHeight,
...
OpenRasterDecoder::ImageInfo OpenRasterDecoder::ReadInfo(const std::string& filePath) const
{
    ...
    if (fileSize < 30 || fileSize > 500 * 1024 * 1024)
        return info;  // 500MB max
    file.seekg(0);
...
// Decode OpenRaster
//==========================================================================
OpenRasterDecoder::DecodeResult OpenRasterDecoder::Decode(const std::string& filePath, uint32_t targetWidth) const
{
    (void)targetWidth;
    DecodeResult result;
    std::ifstream file(filePath, std::ios::binary | std::ios::ate);
