...
const wchar_t* EPSDecoder::s_extensions[] = {L".eps", L".epsf", L".ps", nullptr};
...
bool EPSDecoder::CanDecode(const wchar_t* filePath)
{
    ...
    return (_wcsicmp(ext, L".eps") == 0 || _wcsicmp(ext, L".epsf") == 0 || _wcsicmp(ext, L".ps") == 0);
}
...
const wchar_t* EPSDecoder::GetName() const
{
    return L"EPSDecoder";
}
uint32_t EPSDecoder::GetExtensionCount() const
{
    return s_extensionCount;
}
bool EPSDecoder::SupportsGPU() const
{
    return false;
}
bool EPSDecoder::IsArchiveDecoder() const
{
    return false;
}
...
HRESULT EPSDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result)
{
    ...
    if (fileSize >= sizeof(EPSBinaryHeader)) {
        ...
        if (header->magic == EPS_BINARY_MAGIC) {
            ...
            if (header->tiffOffset > 0 && header->tiffLength > 0) {
                result.hBitmap = ExtractTIFFPreview(data.get(), fileSize, header->tiffOffset, header->tiffLength,
                                                    request.width, request.height);
            }

            // Try WMF preview if TIFF not available
            if (!result.hBitmap && header->wmfOffset > 0 && header->wmfLength > 0) {
                result.hBitmap = ExtractWMFPreview(data.get(), fileSize, header->wmfOffset, header->wmfLength,
                                                   request.width, request.height);
            }

            // Parse BoundingBox from PS section
            if (header->psOffset > 0 && header->psLength > 0 && header->psOffset + header->psLength <= fileSize) {
                ParseBoundingBox(reinterpret_cast<const char*>(data.get() + header->psOffset), header->psLength,
                                 bbWidth, bbHeight, bbWidth, bbHeight);
                // bbWidth/bbHeight will be overwritten properly below
                float llx, lly, urx, ury;
                if (ParseBoundingBox(reinterpret_cast<const char*>(data.get() + header->psOffset), header->psLength,
                                     llx, lly, urx, ury)) {
                    bbWidth = urx - llx;
                    bbHeight = ury - lly;
                }
    ...
    if (!result.hBitmap) {
        if (fileSize >= 2 && data.get()[0] == '%' && data.get()[1] == '!') {
            ...
            if (ParseBoundingBox(reinterpret_cast<const char*>(data.get()),
                                 std::min(fileSize, static_cast<size_t>(4096)), llx, lly, urx, ury)) {
                bbWidth = urx - llx;
                bbHeight = ury - lly;
            }
        }
    ...
    if (!result.hBitmap) {
        result.hBitmap = CreateEPSPlaceholder(request.width, request.height, request.filePath, bbWidth, bbHeight);
    }
...
HBITMAP EPSDecoder::ExtractTIFFPreview(const uint8_t* data, size_t size, uint32_t tiffOffset, uint32_t tiffLength,
                                       uint32_t width, uint32_t height)
{
    if (static_cast<size_t>(tiffOffset) + tiffLength > size)
        return nullptr;
...
HBITMAP EPSDecoder::ExtractWMFPreview(const uint8_t* data, size_t size, uint32_t wmfOffset, uint32_t wmfLength,
                                      uint32_t width, uint32_t height)
{
    if (static_cast<size_t>(wmfOffset) + wmfLength > size)
        return nullptr;
...
bool EPSDecoder::ParseBoundingBox(const char* psText, size_t length, float& llx, float& lly, float& urx, float& ury)
{
...
HBITMAP EPSDecoder::CreateEPSPlaceholder(uint32_t width, uint32_t height, const wchar_t* filePath, float bbWidth,
                                         float bbHeight)
{
    HDC hdc = GetDC(NULL);
    ...
    RECT pageRect = {margin, margin, static_cast<LONG>(width) - margin, static_cast<LONG>(height) - margin};
    ...
    HFONT hFont = CreateFontW(static_cast<int>(height * 0.12f), 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET,
    ...
    if (bbWidth > 0 && bbHeight > 0) {
        HFONT smallFont =
            CreateFontW(static_cast<int>(height * 0.07f), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET,
                        OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, L"Segoe UI");
