...
enum class RegExportValueType : uint8_t {
    Sz, Dword, Qword, Binary, ExpandSz, MultiSz, None, Unknown
};
...
public:
    ...
    static const wchar_t* GetName() { return L"RegistryExportViewer"; }
    ...
    uint32_t DetectVersion(const std::string& firstLine) const {
        if (firstLine.find("Windows Registry Editor Version 5.00") != std::string::npos) return 5;
        if (firstLine.find("REGEDIT4") != std::string::npos) return 4;
        return 0;
    }
    ...
    RegistryExportInfo Parse(const std::string& content) const {
        ...
        if (content.size() < 10) return info;

        size_t pos = 0;
        size_t lineEnd = content.find('\n', pos);
        if (lineEnd == std::string::npos) return info;

        std::string firstLine = content.substr(0, lineEnd);
        info.version = DetectVersion(firstLine);
        if (info.version == 0) return info;
        info.isValid = true;
    ...
    RegistryExportStats GetStats() const { return m_stats; }
