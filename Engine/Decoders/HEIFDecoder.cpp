...
HRESULT DecodeHEIFWithWIC(const wchar_t* filePath, uint32_t targetWidth, uint32_t targetHeight,
                          ExplorerLens::Engine::ThumbnailResult& result)
{
    ...
    HRESULT hr = wicFactory->CreateDecoderFromFilename(filePath, nullptr, GENERIC_READ, WICDecodeMetadataCacheOnDemand,
                                                       &decoder);
    if (FAILED(hr)) {
        return hr;
    }
...
HEIFDecoder::HEIFDecoder() : m_preferEmbeddedThumbnail(true) {}
...
bool HEIFDecoder::CanDecode(const wchar_t* filePath)
{
    ...
    return (_wcsicmp(ext, L".heif") == 0 || _wcsicmp(ext, L".heic") == 0 || _wcsicmp(ext, L".hif") == 0
            || _wcsicmp(ext, L".heifs") == 0 || _wcsicmp(ext, L".heics") == 0 || _wcsicmp(ext, L".avci") == 0
...
HRESULT HEIFDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result)
{
    ...
    if (m_preferEmbeddedThumbnail) {
        pixels = ExtractEmbeddedThumbnail(fileData.get(), fileSize, decodedWidth, decodedHeight, channels);
    }

    // Fallback: decode full image
    if (!pixels) {
        pixels = DecodeHEIFImage(fileData.get(), fileSize, request.width, request.height, decodedWidth, decodedHeight,
                                 channels);
    }
...
bool HEIFDecoder::VerifyHEIFSignature(const uint8_t* data, size_t size) const
{
    ...
    if (size >= 12) {
        ...
        for (size_t i = 16; i < size - 4 && i < 64; i += 4) {
            ...
            if (strncmp(compatBrand, "heic", 4) == 0 || strncmp(compatBrand, "heix", 4) == 0
...
uint8_t* HEIFDecoder::DecodeHEIFImage(const uint8_t* fileData, size_t dataSize, uint32_t targetWidth,
                                      ...
                                      uint32_t& outChannels)
{
    ...
    // Read HEIF data
    struct heif_error err = heif_context_read_from_memory_without_copy(ctx, fileData, dataSize, nullptr);
    ...
    err = heif_decode_image(handle, &image, heif_colorspace_RGB,
                            hasAlpha ? heif_chroma_interleaved_RGBA : heif_chroma_interleaved_RGB, nullptr);
    ...
    // Get pixel data
    int stride = 0;
    const uint8_t* srcData = heif_image_get_plane_readonly(image, heif_channel_interleaved, &stride);
    ...
    for (int y = 0; y < imageHeight; ++y) {
        memcpy(pixels + y * imageWidth * outChannels, srcData + y * stride, imageWidth * outChannels);
    }
...
uint8_t* HEIFDecoder::ExtractEmbeddedThumbnail(const uint8_t* fileData, size_t dataSize, uint32_t& outWidth,
                                               uint32_t& outHeight, uint32_t& outChannels)
{
#ifdef HAS_LIBHEIF
    // Initialize HEIF context
    struct heif_context* ctx = heif_context_alloc();
    ...
    err = heif_decode_image(thumbHandle, &thumbImage, heif_colorspace_RGB,
                            hasAlpha ? heif_chroma_interleaved_RGBA : heif_chroma_interleaved_RGB, nullptr);
    ...
    // Get pixel data
    int stride = 0;
    const uint8_t* srcData = heif_image_get_plane_readonly(thumbImage, heif_channel_interleaved, &stride);
    ...
    for (uint32_t y = 0; y < outHeight; ++y) {
        memcpy(pixels + y * outWidth * outChannels, srcData + y * stride, outWidth * outChannels);
    }
...
HBITMAP HEIFDecoder::CreateHBITMAPFromRGBA(const uint8_t* pixels, uint32_t width, uint32_t height, uint32_t channels)
{
