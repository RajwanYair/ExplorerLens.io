  ...
  public:
    ...
    LaTeXDocInfo Parse(const std::wstring& source) const
    {
        ...
        while (pos < source.size()) {
            ...
            // Count environments
            if (line.find(L"\\begin{equation") != std::wstring::npos || line.find(L"$$") != std::wstring::npos)
                info.equationCount++;
            if (line.find(L"\\begin{figure") != std::wstring::npos)
                info.figureCount++;
            if (line.find(L"\\begin{table") != std::wstring::npos)
                info.tableCount++;
            if (line.find(L"\\cite{") != std::wstring::npos)
                info.citationCount++;
