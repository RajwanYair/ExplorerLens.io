...
class EPSDecoder : public IThumbnailDecoder
{
  ...
  private:
    ...
    HBITMAP ExtractTIFFPreview(const uint8_t* data, size_t size, uint32_t tiffOffset, uint32_t tiffLength,
                               uint32_t width, uint32_t height);

    // Extract embedded WMF preview from EPS binary header
    HBITMAP ExtractWMFPreview(const uint8_t* data, size_t size, uint32_t wmfOffset, uint32_t wmfLength, uint32_t width,
                              uint32_t height);

    // Parse BoundingBox from PostScript text
    bool ParseBoundingBox(const char* psText, size_t length, float& llx, float& lly, float& urx, float& ury);

    // Create placeholder thumbnail showing file info
    HBITMAP CreateEPSPlaceholder(uint32_t width, uint32_t height, const wchar_t* filePath, float bbWidth,
                                 float bbHeight);
