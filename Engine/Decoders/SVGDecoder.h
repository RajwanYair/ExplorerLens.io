...
class SVGDecoder : public IThumbnailDecoder
{
  ...
  private:
    // SVG rendering
    HRESULT RenderSVGToHBITMAP(const wchar_t* filePath, uint32_t width, uint32_t height, HBITMAP* phBitmap);
    ...
    bool ExtractSVGDimensions(const std::string& svgContent, uint32_t& outWidth, uint32_t& outHeight);

    // Render placeholder with SVG icon representation
    HBITMAP CreateSVGPlaceholder(uint32_t width, uint32_t height, const std::string& svgContent);
