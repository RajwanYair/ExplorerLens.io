...
GeoFormat GeospatialDecoder::DetectFormat(const uint8_t* data, size_t size)
{
    ...
    if (data[0] == 'P' && data[1] == 'K')
        return GeoFormat::KMZ;
    if (size >= 5 && memcmp(data, "<?xml", 5) == 0) {
        std::string content(reinterpret_cast<const char*>(data), std::min(size, (size_t)512));
        if (content.find("<kml") != std::string::npos)
            return GeoFormat::KML;
    }
    // GeoJSON
    if (data[0] == '{') {
        std::string content(reinterpret_cast<const char*>(data), std::min(size, (size_t)256));
...
double GeospatialDecoder::DistanceKm(const GeoCoordinate& a, const GeoCoordinate& b)
{
    ...
    double aVal = sin(dLat / 2) * sin(dLat / 2) + cos(lat1) * cos(lat2) * sin(dLon / 2) * sin(dLon / 2);
...
const wchar_t* GeospatialDecoder::GetProjectionName(GeoProjection proj)
{
    switch (proj) {
        case GeoProjection::WGS84:
            return L"WGS 84 (EPSG:4326)";
        case GeoProjection::WebMercator:
            return L"Web Mercator (EPSG:3857)";
        case GeoProjection::UTM:
            return L"UTM";
        case GeoProjection::Custom:
            return L"Custom";
        default:
            return L"Unknown";
    }
