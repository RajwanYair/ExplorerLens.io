...
HRESULT DocumentDecoder::Decode(const ThumbnailRequest& request, ThumbnailResult& result) {
    ...
    if (FAILED(hr) || !result.hBitmap) {
        hr = ExtractThumbnailShell(request.filePath, request.width,
...
HRESULT DocumentDecoder::ExtractEPUBCover(const wchar_t* filePath, uint32_t width,
                                          uint32_t height, HBITMAP* phBitmap) {
    if (!phBitmap) return E_INVALIDARG;
    *phBitmap = nullptr;
...
HRESULT DocumentDecoder::ExtractMOBICover(const wchar_t* filePath, uint32_t width,
                                          uint32_t height, HBITMAP* phBitmap) {
    if (!phBitmap) return E_INVALIDARG;
    *phBitmap = nullptr;
    ...
    if (bytesRead < 100) return E_FAIL;
    ...
    if (jpegStart == std::string::npos) return E_FAIL;
    ...
    if (jpegEnd == std::string::npos || jpegEnd <= jpegStart) return E_FAIL;
...
bool DocumentDecoder::IsMobiFormat(const wchar_t* path) {
    if (!path) return false;
    const wchar_t* ext = PathFindExtensionW(path);
    return ext && (_wcsicmp(ext, L".mobi") == 0 || 
                   _wcsicmp(ext, L".azw") == 0 || 
                   _wcsicmp(ext, L".azw3") == 0);
}
