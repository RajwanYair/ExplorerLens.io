...
public:
 ...
 switch (f) {
 case StructuredDataFormat::CSV: return L"CSV";
 case StructuredDataFormat::JSON: return L"JSON";
 case StructuredDataFormat::YAML: return L"YAML";
 case StructuredDataFormat::TOML: return L"TOML";
 case StructuredDataFormat::XML: return L"XML";
 case StructuredDataFormat::INI: return L"INI";
 ...
 switch (s) {
 case DataPreviewStyle::TreeView: return L"Tree View";
 case DataPreviewStyle::TableView: return L"Table View";
 case DataPreviewStyle::KeyValue: return L"Key-Value";
 case DataPreviewStyle::RawText: return L"Raw Text";
 default: return L"Unknown";
 }
 }

 static StructuredDataFormat DetectFormat(const std::wstring& ext) {
 if (ext == L".csv") return StructuredDataFormat::CSV;
 if (ext == L".json" || ext == L".jsonc") return StructuredDataFormat::JSON;
 if (ext == L".yaml" || ext == L".yml") return StructuredDataFormat::YAML;
 if (ext == L".toml") return StructuredDataFormat::TOML;
 if (ext == L".xml") return StructuredDataFormat::XML;
 if (ext == L".ini" || ext == L".cfg") return StructuredDataFormat::INI;
 return StructuredDataFormat::JSON;
 }

 static constexpr size_t FormatCount() { return static_cast<size_t>(StructuredDataFormat::COUNT); }
 static constexpr size_t ValueTypeCount() { return static_cast<size_t>(JSONValueType::COUNT); }
