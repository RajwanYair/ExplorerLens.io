...
enum class USDLayerType { USDA, USDC, USDZ, Unknown };
...
public:
    static USDLayerType DetectLayerType(const std::wstring& path) {
        ...
        if (dot == std::wstring::npos) return USDLayerType::Unknown;
        std::wstring ext = path.substr(dot + 1);
        std::transform(ext.begin(), ext.end(), ext.begin(), ::towlower);
        if (ext == L"usda") return USDLayerType::USDA;
        if (ext == L"usdc") return USDLayerType::USDC;
        if (ext == L"usdz") return USDLayerType::USDZ;
