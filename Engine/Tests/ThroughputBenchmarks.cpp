...
static const TestImage TEST_IMAGES[] = {
    { L"sample-4k.jpg",      L"JPEG",   3840, 2160 },
    { L"sample-4k.png",      L"PNG",    3840, 2160 },
    { L"sample-4k.webp",     L"WebP",   3840, 2160 },
    { L"sample-4k.avif",     L"AVIF",   3840, 2160 },
    { L"sample-4k.heic",     L"HEIF",   3840, 2160 },
    { L"sample-4k.jxl",      L"JXL",    3840, 2160 },
    { L"sample-large.psd",   L"PSD",    4096, 4096 },
    { L"sample.svg",         L"SVG",    1024, 1024 },
...
static void BM_Memory_PeakHeap(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (int i = 0; i < 100; ++i) {
            HBITMAP hBitmap = decoder.Decode(path.c_str(), ctx);
            if (hBitmap) bitmaps.push_back(hBitmap);
        }
