...
public:
    ...
    static std::vector<FuzzFixture> GenerateAll(int countPerFormat = 10) {
        ...
        // Archive formats
        GenerateCorruptFiles(fixtures, tempDir, "ZIP", L".zip",
            {0x50, 0x4B, 0x03, 0x04}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "RAR", L".rar",
            {0x52, 0x61, 0x72, 0x21, 0x1A, 0x07}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "7Z", L".7z",
            {0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "TAR", L".tar",
            {0x75, 0x73, 0x74, 0x61, 0x72}, countPerFormat);
        
        // Comic book archives (same magic as underlying format)
        GenerateCorruptFiles(fixtures, tempDir, "CBZ", L".cbz",
            {0x50, 0x4B, 0x03, 0x04}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "CBR", L".cbr",
            {0x52, 0x61, 0x72, 0x21, 0x1A, 0x07}, countPerFormat);
        
        // Image formats
        GenerateCorruptFiles(fixtures, tempDir, "WEBP", L".webp",
            {0x52, 0x49, 0x46, 0x46}, countPerFormat);  // RIFF header
        GenerateCorruptFiles(fixtures, tempDir, "PNG", L".png",
            {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "JPEG", L".jpg",
            {0xFF, 0xD8, 0xFF, 0xE0}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "GIF", L".gif",
            {0x47, 0x49, 0x46, 0x38, 0x39, 0x61}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "BMP", L".bmp",
            {0x42, 0x4D}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "TIFF", L".tif",
            {0x49, 0x49, 0x2A, 0x00}, countPerFormat);  // Little-endian TIFF
        
        // Modern image formats
        GenerateCorruptFiles(fixtures, tempDir, "JXL", L".jxl",
            {0xFF, 0x0A}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "AVIF", L".avif",
            {0x00, 0x00, 0x00, 0x1C, 0x66, 0x74, 0x79, 0x70}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "HEIF", L".heif",
            {0x00, 0x00, 0x00, 0x1C, 0x66, 0x74, 0x79, 0x70}, countPerFormat);
        
        // Professional formats
        GenerateCorruptFiles(fixtures, tempDir, "PSD", L".psd",
            {0x38, 0x42, 0x50, 0x53}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "EXR", L".exr",
            {0x76, 0x2F, 0x31, 0x01}, countPerFormat);
        GenerateCorruptFiles(fixtures, tempDir, "HDR", L".hdr",
            {0x23, 0x3F, 0x52, 0x41, 0x44, 0x49, 0x41, 0x4E}, countPerFormat); // #?RADIAN
        
        // Document formats
        GenerateCorruptFiles(fixtures, tempDir, "PDF", L".pdf",
            {0x25, 0x50, 0x44, 0x46, 0x2D}, countPerFormat);  // %PDF-
        GenerateCorruptFiles(fixtures, tempDir, "EPUB", L".epub",
            {0x50, 0x4B, 0x03, 0x04}, countPerFormat);  // ZIP-based
    ...
    static FuzzFixture GenerateSingle(
...
private:
    ...
    static void GenerateCorruptFiles(
        std::vector<FuzzFixture>& fixtures,
        ...
        int count)
    {
        ...
        std::vector<CorruptionType> types = {
            CorruptionType::TRUNCATED_HEADER,
            CorruptionType::RANDOM_PAYLOAD,
            CorruptionType::ZERO_FILL,
            CorruptionType::OVERSIZED_FIELD,
            CorruptionType::NULL_BYTES,
        ...
        for (int i = 0; i < count; i++) {
            ...
            fixtures.push_back(
                CreateCorruptFile(tempDir, format, extension, magic, corruption, i));
        }
    }

    /// Create a single corrupt file
    static FuzzFixture CreateCorruptFile(
        const std::filesystem::path& tempDir,
        const std::string& format,
        const std::wstring& extension,
        const std::vector<uint8_t>& magic,
        ...
        int index)
    {
        ...
        auto filename = std::filesystem::path(
            L"fuzz_" + std::to_wstring(index) + L"_" + 
