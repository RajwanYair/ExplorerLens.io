// Sprint 155 — ARM64 Build Config — GTest
...
TEST(ARM64BuildConfig, DefaultInventoryHasZlib) {
    ...
    for (const auto& e : inv.entries)
        if (e.name == "zlib") { found = true; break; }
    EXPECT_TRUE(found);
}

TEST(ARM64BuildConfig, CMakeParamsNotEmpty) {
    CMakeARM64Parameters p;
    p.generator = "Ninja";
    EXPECT_FALSE(p.generator.empty());
}
...
TEST(ARM64BuildConfig, LibraryStatusEnumCoverage) {
    auto s = LibraryARM64Status::Verified;
    EXPECT_EQ(static_cast<uint32_t>(s), 1u);
}

TEST(ARM64BuildConfig, ArchitectureEnumARM64Value) {
    EXPECT_EQ(static_cast<uint32_t>(TargetArchitecture::ARM64), 2u);
}
...
TEST(ARM64BuildConfig, CurrentArchitectureOnX64IsX64OrRelated) {
    // On the build machine this should be x64 or x86
    auto arch = CurrentArchitecture();
    bool isKnown = (arch == TargetArchitecture::x64 || arch == TargetArchitecture::x86 ||
