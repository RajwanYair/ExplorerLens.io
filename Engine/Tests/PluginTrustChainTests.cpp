...
{
    EXPECT_LT(static_cast<uint32_t>(PluginTrustLevel::Trusted), static_cast<uint32_t>(PluginTrustLevel::Blocked));
}
...
TEST(PluginTrustChain, ValidatorOfficialPublisher)
{
    ...
    auto result = v.Validate(sig);
    EXPECT_GE(static_cast<uint32_t>(result.level), static_cast<uint32_t>(PluginTrustLevel::Trusted));
}
