// Sprint 168 — Hot-Mode Directory Engine — GTest
...
TEST(HotModeDirectoryEngine, LowFilesNotHotMode) {
    ...
    for (int i = 0; i < 10; ++i) snap.entries.push_back({ "file" + std::to_string(i) });
...
TEST(HotModeDirectoryEngine, ManyFilesIsHotMode) {
    HotModeDirectoryEngine eng;
    DirectorySnapshot snap;
    for (int i = 0; i < 60; ++i) snap.entries.push_back({ "file" + std::to_string(i) });
...
TEST(HotModeDirectoryEngine, PreWarmBatchSucceeds) {
    HotModeDirectoryEngine eng;
    DirectorySnapshot snap;
    for (int i = 0; i < 5; ++i) snap.entries.push_back({ "f" + std::to_string(i) });
