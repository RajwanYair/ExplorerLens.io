...
void CorpusTestRunner::SetFileFilter(std::function<bool(const std::filesystem::path&)> filter)
{
...
CorpusTestRunner::RunReport CorpusTestRunner::Run() const
{
    ...
    for (const auto& dir : m_corpusDirs) {
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(dir, ec)) {
            ...
            if (m_filter && !m_filter(entry.path())) {
                continue;
            }
            ...
            if (m_maxFiles > 0 && files.size() >= m_maxFiles)
                break;
        }
        if (m_maxFiles > 0 && files.size() >= m_maxFiles)
            break;
    ...
    for (const auto& filePath : files) {
        ...
        result.extension = ToLower(WStringToUtf8(filePath.extension().wstring()));
        if (!result.extension.empty() && result.extension[0] == '.') {
            result.extension.erase(0, 1);
        }

        auto it = formatTable.find(result.extension);
        result.format = (it != formatTable.end()) ? Utf8ToWString(it->second) : L"Unknown";
    ...
    report.totalDurationMs = std::chrono::duration<double, std::milli>(wallEnd - wallStart).count();
    report.averageDurationMs = report.passed > 0 ? report.totalDurationMs / report.passed : 0.0;
...
bool CorpusTestRunner::TryDecodeFile(const std::filesystem::path& path, std::wstring& formatOut, std::wstring& errorOut)
{
    ...
    if (h[0] == 0xFF && h[1] == 0xD8) {
        formatOut = L"JPEG";
    } else if (h[0] == 0x89 && h[1] == 0x50) {
        formatOut = L"PNG";
    } else if (h[0] == 0x47 && h[1] == 0x49) {
        formatOut = L"GIF";
    } else if (h[0] == 0x42 && h[1] == 0x4D) {
        formatOut = L"BMP";
    } else if (h[0] == 0x52 && h[1] == 0x49 && h[2] == 0x46 && h[3] == 0x46) {
        formatOut = L"RIFF";
    } else if (h[0] == 0x25 && h[1] == 0x50 && h[2] == 0x44 && h[3] == 0x46) {
        formatOut = L"PDF";
    } else if (h[0] == 0x50 && h[1] == 0x4B) {
        formatOut = L"ZIP";
    } else if (h[0] == 0x52 && h[1] == 0x61 && h[2] == 0x72 && h[3] == 0x21) {
        formatOut = L"RAR";
    } else if (h[0] == 0x37 && h[1] == 0x7A) {
        formatOut = L"7-Zip";
    }
...
bool CorpusTestRunner::WriteHtmlReport(const std::filesystem::path& outputPath, const RunReport& report)
{
    ...
    for (const auto& r : report.results) {
        ...
        if (r.passed) {
            formatStats[fmt].first++;
        } else {
            formatStats[fmt].second++;
        }
    }
    ...
    out << "<!DOCTYPE html>\n<html lang=\"en\">\n<head>\n"
        ...
        << "<p>Engine: " << report.engineVersion << " &mdash; Generated: " << report.generatedAt << "</p>\n"
        << "<table>\n"
        ...
        << "<td>" << std::fixed << std::setprecision(1) << report.totalDurationMs << "</td></tr>\n</table>\n"
    ...
    for (const auto& r : report.results) {
        ...
        out << "<tr>"
            << "<td title=\"" << path8 << "\">" << std::filesystem::path(r.filePath).filename().string() << "</td>"
            << "<td>" << fmt8 << "</td>"
...
bool CorpusTestRunner::WriteCsvReport(const std::filesystem::path& outputPath, const RunReport& report)
{
    std::ofstream out(outputPath);
    if (!out.is_open()) {
        return false;
    ...
    for (const auto& r : report.results) {
        ...
        out << escape(WStringToUtf8(r.filePath)) << "," << escape(WStringToUtf8(r.format)) << ","
            << (r.passed ? "PASS" : "FAIL") << "," << std::fixed << std::setprecision(3) << r.durationMs << ","
            << r.fileSizeBytes << "," << escape(WStringToUtf8(r.errorMessage)) << "\n";
    }
    return out.good();
}
...
void CorpusTestRunner::PrintSummary(const RunReport& report)
{
    std::wcout << L"\n=== Integration Test Summary ===\n"
               ...
               << L"  Time    : " << std::fixed << std::setprecision(1) << report.totalDurationMs << L" ms\n"
               << L"================================\n\n";
}
