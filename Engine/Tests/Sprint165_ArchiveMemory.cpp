// Sprint 165 — Archive Memory Compactor — GTest
...
TEST(ArchiveMemoryCompactor, TrackSlabIncreasesTotal) {
    ...
    MemorySlab s; s.sizeBytes = 1024; s.state = SlabState::Active;
    c.TrackSlab(s);
...
TEST(ArchiveMemoryCompactor, FreeSlabNotCounted) {
    ArchiveMemoryCompactor c;
    MemorySlab s; s.sizeBytes = 1024; s.state = SlabState::Free;
    c.TrackSlab(s);
...
TEST(ArchiveMemoryCompactor, CompactReducesBytes) {
    ...
    MemorySlab s; s.sizeBytes = 1024 * 1024; s.state = SlabState::Evictable;
    c.TrackSlab(s);
...
TEST(ArchiveMemoryCompactor, CompactionReportBytesBefore) {
    ...
    ArchiveMemoryCompactor c(cfg);
    MemorySlab s; s.sizeBytes = 65536; s.state = SlabState::Evictable;
    c.TrackSlab(s);
    auto report = c.Compact();
