...
void BenchmarkIPCOverhead()
{
    ...
    // Create test pipe
    std::wstring pipe_name = L"\\\\.\\pipe\\ExplorerLens_Benchmark_" + std::to_wstring(GetCurrentProcessId());
...
void BenchmarkSharedMemory()
{
    ...
    for (size_t size : test_sizes) {
        ...
        std::cout << " Write: " << std::fixed << std::setprecision(2) << write_throughput << " MB/s\n";
...
void BenchmarkThroughput()
{
    ...
    std::cout << " Throughput: " << std::setprecision(1) << throughput << " operations/sec\n";
    std::cout << " Avg Latency: " << std::setprecision(2) << (1000.0 / throughput) << " ms/operation\n\n";
