...
public:
    ...
    [[nodiscard]] int CompareWithBaseline(const BenchmarkReport& current,
...
private:
    static BenchmarkSample MeasureDecodeStats(
        const std::string& filePath, uint32_t iterations, bool clearCache);
...
#define BENCH_MS(label, iterations, expr) \
    do { \
        ...
        for (uint32_t _i = 0; _i < (iterations); ++_i) { expr; } \
