  ...
  private:
    E2ETestResult RunCase(const E2ETestCase& tc, uint32_t idx)
    {
        ...
        if (r.wallMs > tc.maxLatencyMs)
            r.failReason = "Latency " + std::to_string(r.wallMs) + " ms exceeded " + std::to_string(tc.maxLatencyMs)
                           + " ms budget";
...
#define E2E_LOAD_CORPUS(harness, dir) (harness).LoadCorpus(L##dir, 50.f)
