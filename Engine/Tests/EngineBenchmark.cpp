...
int main()
{
    ...
    std::wcout << L"Throughput: " << std::fixed << std::setprecision(1)
               << (duration.count() > 0 ? successCount * 1000.0 / duration.count() : 0.0) << L" images/sec\n";

    // Benchmark 4: Different sizes
    PrintHeader(L"Benchmark 4: Different Thumbnail Sizes");
    if (!testImages.empty()) {
        const std::wstring& testImage = testImages[0];
        const std::vector<std::pair<int, int>> sizes = {{96, 96}, {128, 128}, {256, 256}, {512, 512}};
    ...
    std::vector<FormatTest> formats = {
        {L"JPEG", L".jpg"},  {L"PNG", L".png"},       {L"WebP", L".webp"}, {L"JPEG XL", L".jxl"}, {L"AVIF", L".avif"},
        {L"HEIF", L".heif"}, {L"RAW (CR2)", L".cr2"}, {L"ZIP", L".zip"},   {L"RAR", L".rar"},     {L"CBZ", L".cbz"}};
    ...
    for (const auto& format : formats) {
        ...
        std::wcout << std::left << std::setw(15) << format.name << std::right << std::setw(10) << std::fixed
                   << std::setprecision(3) << avgUs << L" μs/detection" << std::setw(15) << std::fixed
                   << std::setprecision(0) << (1000000.0 / avgUs) << L" det/sec" << L" ["
                   << (detectedType != DetectedFormat::Unknown ? L"OK" : L"FAIL") << L"]\n";
    }
    ...
    std::vector<ScaleTest> sizes = {
        {L"HD (1920x1080)", 1920, 1080}, {L"4K (3840x2160)", 3840, 2160}, {L"8K (7680x4320)", 7680, 4320}};
    ...
    for (const auto& test : sizes) {
        ...
        if (cpu.hasAVX2) {
            ...
            for (int i = 0; i < scaleIterations; i++) {
                SIMD::SIMDScaler::ScaleBGRA_Bilinear_AVX2(srcData.data(), test.width, test.height, srcStride,
            ...
            std::wcout << std::left << std::setw(25) << test.size << L"AVX2: " << std::right << std::setw(8)
                       << std::fixed << std::setprecision(2) << avgMs << L" ms " << std::setw(6) << std::fixed
                       << std::setprecision(1) << mpixPerSec << L" Mpix/s\n";
        }

        // Test scalar fallback
        QueryPerformanceCounter(&start);
        for (int i = 0; i < scaleIterations; i++) {
            SIMD::SIMDScaler::ScaleBGRA_Bilinear_Scalar(srcData.data(), test.width, test.height, srcStride,
                                                        dstData.data(), thumbWidth, thumbHeight, dstStride);
        }
        QueryPerformanceCounter(&end);
        avgMs = ((end.QuadPart - start.QuadPart) * 1000.0 / freq.QuadPart) / scaleIterations;
        ...
        std::wcout << std::left << std::setw(25) << L"" << L"Scalar: " << std::right << std::setw(8) << std::fixed
                   << std::setprecision(2) << avgMs << L" ms " << std::setw(6) << std::fixed << std::setprecision(1)
                   << mpixPerSec << L" Mpix/s\n\n";
    }
    ...
    std::wcout << L"Cache Hits: " << cacheHits << L" (" << std::fixed << std::setprecision(1)
               << (totalRequests > 0 ? cacheHits * 100.0 / totalRequests : 0.0) << L"%)\n";
