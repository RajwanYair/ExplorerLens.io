...
TEST_F(SecurityTestFixture, IsolationMode_UnsignedPlugin_UsesPluginHost)
{
    ...
    IsolationMode mode = IsolationModeSelector::Instance().DetermineMode(L"unsigned_plugin", unsigned_plugin);

    EXPECT_EQ(mode, IsolationMode::PluginHost) << "Unsigned plugins must run in PluginHost for security";
}
...
TEST_F(SecurityTestFixture, IsolationMode_TrustedPlugin_CanUseInWorker)
{
    ...
    IsolationMode mode = IsolationModeSelector::Instance().DetermineMode(L"TrustedTestPlugin", trusted_plugin);
...
TEST_F(SecurityTestFixture, IsolationMode_UserPreference_Respected)
{
    ...
    IsolationMode mode = IsolationModeSelector::Instance().DetermineMode(L"TestPlugin", plugin);

    EXPECT_EQ(mode, IsolationMode::PluginHost) << "User preference should override default mode";
}
...
TEST_F(SecurityTestFixture, IsolationMode_MinimumMode_Enforced)
{
    ...
    EXPECT_TRUE(min_mode == IsolationMode::PluginHost || min_mode == IsolationMode::InWorker)
...
TEST_F(SecurityTestFixture, PluginHost_ProcessCreation_Succeeds)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        EXPECT_TRUE(hr == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND) || hr == HRESULT_FROM_WIN32(ERROR_PATH_NOT_FOUND))
...
TEST_F(SecurityTestFixture, IPC_NamedPipe_ConnectionEstablished)
{
    ...
    std::wstring pipe_name = L"\\\\.\\pipe\\ExplorerLens_Test_" + std::to_wstring(GetCurrentProcessId());
...
TEST_F(SecurityTestFixture, IPC_SharedMemory_ReadWrite)
{
    ...
    if (shared_mem.IsValid()) {
        ...
        EXPECT_TRUE(shared_mem.Write(test_data, data_len, 0)) << "Should write to shared memory";

        // Read back
        char read_buffer[256] = {};
        EXPECT_TRUE(shared_mem.Read(read_buffer, data_len, 0)) << "Should read from shared memory";

        EXPECT_STREQ(test_data, read_buffer) << "Read data should match written data";
    }
}
...
TEST_F(SecurityTestFixture, Crash_Detection_RegistersPlugin)
{
    ...
    EXPECT_TRUE(handler.IsPluginDisabled(test_plugin)) << "Plugin should be disabled after crash";

    // Check crash count
    EXPECT_GT(handler.GetCrashCount(test_plugin), 0u) << "Crash count should be incremented";
}
...
TEST_F(SecurityTestFixture, Crash_MultiplePlugins_IndependentTracking)
{
    ...
    EXPECT_FALSE(handler.IsPluginDisabled(plugin2)) << "Unrelated plugin should not be disabled";
}
...
TEST_F(SecurityTestFixture, JobObject_CreationAndConfiguration)
{
    ...
    if (job_mgr.IsValid()) {
        ...
        // Set process limit
        EXPECT_TRUE(job_mgr.SetProcessCountLimit(1)) << "Should set process count limit";
    }
}
...
TEST_F(SecurityTestFixture, JobObject_ProcessRestrictions)
{
    ...
    if (job_mgr.IsValid()) {
        ...
        EXPECT_TRUE(job_mgr.ApplyTokenRestrictions()) << "Should apply token restrictions";

        // Query limits to verify
        uint64_t memory_limit = 0;
        uint64_t cpu_limit_ms = 0;

        EXPECT_TRUE(job_mgr.QueryLimits(&memory_limit, &cpu_limit_ms)) << "Should query job limits";
    }
}
