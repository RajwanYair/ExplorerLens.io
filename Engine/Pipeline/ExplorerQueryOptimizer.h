...
struct QueryOptimizerStats {
    ...
    double hitRate() const { return totalQueries > 0 ? 100.0 * prefetchHits / totalQueries : 0.0; }
};

class ExplorerQueryOptimizer {
public:
    QueryPattern DetectPattern(const std::deque<std::wstring>& recentPaths) const {
        if (recentPaths.size() < 2) return QueryPattern::Batch;
        ...
        for (const auto& p : recentPaths) {
            if (parentOf(p) != firstParent) { sameFolder = false; break; }
        }
        if (!sameFolder) return QueryPattern::Random;
    ...
    QueryOptimizerStats GetStats() const { return m_stats; }
