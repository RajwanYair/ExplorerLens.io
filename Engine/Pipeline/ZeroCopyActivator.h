...
public:
    static ZeroCopyActivator& Instance() { static ZeroCopyActivator s; return s; }

    void SetConfig(const ZeroCopyConfig& config) { m_config = config; }
    const ZeroCopyConfig& GetConfig() const { return m_config; }

    bool Activate(uint64_t imageSizeBytes) {
        if (m_active) return true;
        if (!m_config.gpuMappingEnabled) return false;
        if (imageSizeBytes > m_config.maxStagingBufferSize) return false;
    ...
    ZCStage GetCurrentStage() const { return m_currentStage; }
    ...
    bool FallbackCopy(uint64_t sizeBytes) {
        if (!m_config.fallbackToMemcpy) return false;
    ...
    ZeroCopyMetrics GetMetrics() const { return m_metrics; }
    ...
    bool ShouldUseZeroCopy(uint64_t imageSizeBytes) const {
        if (!m_config.gpuMappingEnabled) return false;
        if (imageSizeBytes < m_config.minImageSizeForZC) return false;
        if (imageSizeBytes > m_config.maxStagingBufferSize) return false;
        return true;
    }
    ...
    bool Validate() const {
        if (m_config.maxStagingBufferSize == 0) return false;
        if (m_config.stagingBufferCount == 0 || m_config.stagingBufferCount > 16) return false;
        if (m_config.minImageSizeForZC > m_config.maxStagingBufferSize) return false;
        if (m_metrics.zeroCopyRatio < 0.0f || m_metrics.zeroCopyRatio > 1.0f) return false;
        return true;
    }
...
private:
    ...
    void RecordTransfer(bool zeroCopy) {
        ...
        if (zeroCopy) m_metrics.zeroCopyTransfers++;
        UpdateRatio();
        RotateBuffer();
    }
    ...
    void UpdateRatio() {
        if (m_metrics.totalTransfers > 0) {
            m_metrics.zeroCopyRatio = static_cast<float>(m_metrics.zeroCopyTransfers)
                / static_cast<float>(m_metrics.totalTransfers);
        }
    }
