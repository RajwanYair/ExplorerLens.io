...
public:
    ...
    ComputeTarget SelectTarget(const std::wstring& /*filePath*/, uint32_t fileSize) const {
        if (!m_gpuAvailable) return ComputeTarget::CPUMulti;
        if (fileSize > m_gpuThresholdBytes) return ComputeTarget::GPUDecode;
        if (fileSize > m_hybridThresholdBytes) return ComputeTarget::Hybrid;
    ...
    DistributionPlan PlanDistribution(const std::vector<WorkUnit>& units) const {
        ...
        for (const auto& unit : units) {
            ...
            case ComputeTarget::GPUDecode: plan.gpuWorkUnits++; break;
            case ComputeTarget::Hybrid: plan.hybridWorkUnits++; break;
            default: plan.cpuWorkUnits++; break;
            }
        }
        return plan;
    }
    ...
    void SetGPUThreshold(uint32_t bytes) { m_gpuThresholdBytes = bytes; }
    void SetHybridThreshold(uint32_t bytes) { m_hybridThresholdBytes = bytes; }

    DistributorMetrics GetMetrics() const { return m_metrics; }
