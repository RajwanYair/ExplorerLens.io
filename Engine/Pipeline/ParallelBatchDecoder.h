...
public:
 ...
 uint64_t SubmitBatch(const std::vector<std::wstring>& files,
 BatchPriority priority = BatchPriority::Immediate,
 BatchProgressCallback progressCb = nullptr,
 BatchCompleteCallback completeCb = nullptr);
 ...
 /// Get configuration
 const BatchDecoderConfig& GetConfig() const { return m_config; }
...
private:
 ...
 std::unordered_map<std::wstring, std::vector<size_t>> GroupByFormat(
