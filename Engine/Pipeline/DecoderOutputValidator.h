...
static const wchar_t* OutputValidationFailureName(OutputValidationFailure f) {
    static const wchar_t* names[] = {
        L"None", L"NullBitmap", L"ZeroDimensions", L"DimensionMismatch",
        L"NullPixelData", L"AllBlackPixels", L"AllWhitePixels",
        L"InvalidBitDepth", L"CorruptedHeader", L"TruncatedData"
    };
...
public:
    ...
    OutputValidationResult ValidateBitmap(HBITMAP hBitmap,
    ...
    // Get stats
    uint64_t GetTotalValidated() const { return m_stats.totalValidated.load(std::memory_order_relaxed); }
    uint64_t GetTotalPassed() const { return m_stats.totalPassed.load(std::memory_order_relaxed); }
    uint64_t GetTotalFailed() const { return m_stats.totalFailed.load(std::memory_order_relaxed); }
...
private:
    ...
    void ValidatePixelContent(const BITMAP& bm, OutputValidationResult& result) {
        if (!bm.bmBits || bm.bmBitsPixel < 24) return;
        ...
        result.uniqueColorRatio = (sampleCount > 0)
            ? (static_cast<double>(uniqueColors) / static_cast<double>(sampleCount))
            : 0.0;
