...
public:
    ...
    DecodeThroughputRegulatorConfig GetConfig() const { return m_config; }
    ...
    void Refill(uint32_t elapsedMs) {
        ...
        m_availableTokens = (m_availableTokens + newTokens > m_config.maxBurst)
            ? m_config.maxBurst : m_availableTokens + newTokens;
    }

    uint32_t GetAvailableTokens() const { return m_availableTokens; }
