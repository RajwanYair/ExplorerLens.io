...
public:
    ...
    StreamingThumbnailEmitterConfig GetConfig() const { return m_config; }
    std::string GetName() const { return m_config.label; }

    enum class Phase : uint8_t { None, LowRes, FullRes, Complete };

    Phase GetCurrentPhase() const { return m_phase; }

    bool AdvancePhase() {
        switch (m_phase) {
        case Phase::None:    m_phase = Phase::LowRes;  return true;
        case Phase::LowRes:  m_phase = Phase::FullRes; return true;
        case Phase::FullRes: m_phase = Phase::Complete; return true;
        case Phase::Complete: return false;
        }
        return false;
    }

    void Reset() { m_phase = Phase::None; }
