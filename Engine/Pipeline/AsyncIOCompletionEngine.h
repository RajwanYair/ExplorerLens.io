...
using ReadCallback = std::function<void(const std::wstring& path,
...
public:
    ...
    inline void ReadFileRange(const std::wstring& path, uint64_t offset,
        ...
        if (readSize == 0) {
            ...
            if (remaining > 256ULL * 1024 * 1024) remaining = 256ULL * 1024 * 1024;
        ...
        if (!ok) {
            ...
            if (err != ERROR_IO_PENDING) {
                ...
                if (ctx->callback) ctx->callback(ctx->filePath, {}, false);
    ...
    inline AsyncIOMetrics GetMetrics() {
        ...
        m.avgReadLatencyMs = (m_completedReads > 0)
            ? (m_totalLatencyMs / static_cast<double>(m_completedReads))
            : 0.0;
        ReleaseSRWLockShared(&m_lock);
        return m;
    }
    ...
    uint32_t GetPending()       const noexcept { return m_pendingCount.load(); }
    uint64_t GetTotalSubmitted() const noexcept { return m_metrics.totalReadsIssued; }
    uint64_t GetCompletedCount() const noexcept { return m_completedReads; }
...
private:
    ...
    inline void WorkerProc() {
        ...
        while (m_running.load()) {
            ...
            if (!ok || numEntries == 0) continue;

            for (ULONG i = 0; i < numEntries; ++i) {
                if (entries[i].lpCompletionKey == SHUTDOWN_KEY) return;
                if (!entries[i].lpOverlapped) continue;

                auto* ctx = reinterpret_cast<AsyncReadContext*>(
                    reinterpret_cast<char*>(entries[i].lpOverlapped)
                    - offsetof(AsyncReadContext, overlapped));

                LARGE_INTEGER endTick;
                QueryPerformanceCounter(&endTick);
                double latencyMs = static_cast<double>(endTick.QuadPart - ctx->submitTick.QuadPart)
                ...
                if (ctx->callback) {
                    if (success) {
                        std::vector<uint8_t> data(ctx->buffer.begin(),
                            ctx->buffer.begin() + transferred);
    ...
    inline void RemoveContext(AsyncReadContext* ctx) {
        ...
        if (it != m_activeContexts.end()) m_activeContexts.erase(it);
    }
