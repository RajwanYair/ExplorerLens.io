...
struct ThumbnailWorkItem
{
    ...
    {
        return viewportIndex >= 0;
    }
    bool IsCancelled() const
    {
        return cancelled || priority == WorkPriority::Cancelled;
    }
};
...
struct WorkSchedulerViewport
{
    ...
    {
        return index >= (firstVisibleIndex - prefetchBefore) && index <= (lastVisibleIndex + prefetchAfter)
               && !IsInViewport(index);
    }
...
struct WorkSchedulerStats
{
    ...
    {
        return totalSubmitted > 0 ? static_cast<double>(totalCompleted) / totalSubmitted : 0.0;
    }

    double CancellationRate() const
    {
        return totalSubmitted > 0 ? static_cast<double>(totalCancelled) / totalSubmitted : 0.0;
    }
};
...
{
  public:
    explicit ExplorerWorkScheduler(SchedulerConfig config = SchedulerConfig::Default()) : m_config(config) {}
  ...
  private:
    void CancelOutOfViewportLocked()
    {
        ...
        while (!m_queue.empty()) {
            auto item = m_queue.top();
            m_queue.pop();
            if (item.viewportIndex < 0 || m_viewport.IsInViewport(item.viewportIndex)
