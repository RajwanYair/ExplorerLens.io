...
public:
        ...
        ExportProgressCallback  progress = nullptr)
    {
        ...
        if (sizes.empty()) sizes.push_back({sourceWidth, sourceHeight, "thumb"});
        ...
        for (const auto& sz : sizes) {
            ...
            et.encodingMs = EncodeThumb(sourceBGRA, sourceWidth, sourceHeight,
                                        sz.width, sz.height, job.format, job.quality,
                                        et.outputPath, et.fileSizeBytes);
    ...
    uint64_t GetTotalExported() const { return m_totalExported; }
    ...
    static bool FormatSupportsLossless(ExportFormat fmt) {
        return fmt == ExportFormat::PNG || fmt == ExportFormat::BMP ||
               fmt == ExportFormat::TIFF || fmt == ExportFormat::WebP ||
               fmt == ExportFormat::AVIF;
    }
...
private:
    ...
    static std::wstring BuildOutputPath(
        const std::wstring& dir, const std::wstring& src,
        const ExportSize& sz, ExportFormat fmt)
    {
        (void)src;
    ...
    static double EncodeThumb(
        const uint8_t* src, uint32_t sw, uint32_t sh,
        uint32_t dw, uint32_t dh,
        ExportFormat fmt, ExportQuality q,
        const std::wstring& dest, uint64_t& outSize)
    {
        // Placeholder: actual encode via libjpeg-turbo / libpng / libwebp / libavif
        (void)src; (void)sw; (void)sh; (void)dw; (void)dh; (void)fmt; (void)q; (void)dest;
