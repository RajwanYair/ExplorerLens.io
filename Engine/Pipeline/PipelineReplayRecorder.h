...
class PipelineReplayRecorder
{
  public:
    ...
    void RecordEvent(ReplayEventType type, const std::wstring& detail = L"", uint64_t dataSize = 0)
    {
        if (!m_recording)
            return;
        ReplayEvent e;
        ...
        if (type == ReplayEventType::Error)
            m_session.hasErrors = true;
    }
    ...
    ReplaySession EndSession()
    {
        ...
        for (auto& e : m_session.events) {
            if (e.type == ReplayEventType::FileOpen)
                m_session.fileCount++;
        }
    ...
    {
        switch (t) {
            case ReplayEventType::FileOpen:
                return L"FileOpen";
            case ReplayEventType::FormatDetected:
                return L"FormatDetected";
            case ReplayEventType::DecodeBegin:
                return L"DecodeBegin";
            case ReplayEventType::DecodeEnd:
                return L"DecodeEnd";
            case ReplayEventType::ResizeBegin:
                return L"ResizeBegin";
            case ReplayEventType::ResizeEnd:
                return L"ResizeEnd";
            case ReplayEventType::CacheHit:
                return L"CacheHit";
            case ReplayEventType::CacheMiss:
                return L"CacheMiss";
            case ReplayEventType::Error:
                return L"Error";
            case ReplayEventType::Timeout:
                return L"Timeout";
            default:
                return L"Unknown";
        }
    }
    static size_t EventTypeCount()
    {
        return static_cast<size_t>(ReplayEventType::COUNT);
    }
