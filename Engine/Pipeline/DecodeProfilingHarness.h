...
struct DecodeProfile
{
    ...
    double AvgMs() const
    {
        return totalCalls > 0 ? totalMs / totalCalls : 0.0;
    }
    double SuccessRate() const
    {
        return totalCalls > 0 ? 100.0 * successCount / totalCalls : 0.0;
    }
};
...
class DecodeProfilingHarness
{
  public:
    class ScopedProfile
    {
      public:
        ...
        void SetPeakMemory(uint64_t bytes)
        {
            m_peakMem = bytes;
        }
  ...
  private:
    void Record(const std::string& decoder, double ms, bool success, uint64_t peakMem)
    {
        ...
        if (ms > p.maxMs)
            p.maxMs = ms;
        if (peakMem > p.peakMemoryBytes)
            p.peakMemoryBytes = peakMem;
    }
