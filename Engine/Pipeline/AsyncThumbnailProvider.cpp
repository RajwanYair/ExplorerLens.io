...
AsyncThumbnailProvider::AsyncThumbnailProvider(const ThumbnailProviderConfig& config) : m_config(config) {}
...
void AsyncThumbnailProvider::Shutdown()
{
    ...
    for (auto& [id, request] : m_requests) {
        if (request.state == RequestState::Queued || request.state == RequestState::InProgress) {
...
uint64_t AsyncThumbnailProvider::SubmitRequest(const std::wstring& filePath, uint32_t requestedSize,
...
uint32_t AsyncThumbnailProvider::CancelRequestsForFile(const std::wstring& filePath)
{
...
bool AsyncThumbnailProvider::WaitForRequest(uint64_t requestId, uint32_t timeoutMs)
{
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        {
            ...
            if (it == m_requests.end())
                return false;
            if (it->second.state == RequestState::Completed || it->second.state == RequestState::Failed
                || it->second.state == RequestState::Cancelled || it->second.state == RequestState::TimedOut) {
                return true;
            }
        }
    ...
    // Request timed out
    {
        ...
        auto it = m_requests.find(requestId);
        if (it != m_requests.end()
            && (it->second.state == RequestState::Queued || it->second.state == RequestState::InProgress)) {
...
AsyncDecodeResult AsyncThumbnailProvider::DecodeSynchronous(const std::wstring& filePath, uint32_t requestedSize)
{
    AsyncDecodeResult result;
    ...
    result.decodeTimeMs = std::chrono::duration<double, std::milli>(endTime - startTime).count();
...
AsyncProviderStats AsyncThumbnailProvider::GetStats() const
{
    ...
    if (stats.completedRequests > 0) {
        stats.avgDecodeTimeMs = m_totalDecodeTimeMs.load() / static_cast<double>(stats.completedRequests);
    }
...
bool AsyncThumbnailProvider::IsRunning() const
{
...
const wchar_t* AsyncThumbnailProvider::GetPriorityName(DecodePriority priority)
{
    switch (priority) {
...
void AsyncThumbnailProvider::WorkerCallback(AsyncProviderDecodeItem& request)
{
    ...
    auto endTime = std::chrono::steady_clock::now();
    double elapsed = std::chrono::duration<double, std::milli>(endTime - request.startTime).count();
...
void AsyncThumbnailProvider::CheckTimeouts()
{
    ...
    for (auto& [id, request] : m_requests) {
        if (request.state == RequestState::InProgress) {
            auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - request.startTime).count();
...
uint64_t AsyncThumbnailProvider::FindDuplicateRequest(const std::wstring& filePath, uint32_t size)
{
    std::lock_guard<std::mutex> lock(m_requestMutex);
    for (const auto& [id, request] : m_requests) {
        if (request.filePath == filePath && request.requestedSize == size
            && (request.state == RequestState::Queued || request.state == RequestState::InProgress)) {
            return id;
        }
    }
    return 0;
