...
DetectedFormat FormatDetector::DetectFromExtension(const wchar_t *extension) {
  ...
  if (StrEqualI(ext, L".raw") || StrEqualI(ext, L".cr2") ||
      StrEqualI(ext, L".nef") || StrEqualI(ext, L".arw") ||
      StrEqualI(ext, L".dng"))
  ...
  if (StrEqualI(ext, L".ppm") || StrEqualI(ext, L".pgm") ||
      StrEqualI(ext, L".pbm") || StrEqualI(ext, L".pnm") ||
  ...
  if (StrEqualI(ext, L".wmv") || StrEqualI(ext, L".mov") ||
      StrEqualI(ext, L".flv") || StrEqualI(ext, L".webm") ||
      StrEqualI(ext, L".mpg") || StrEqualI(ext, L".mpeg") ||
      StrEqualI(ext, L".ts") || StrEqualI(ext, L".mts") ||
      StrEqualI(ext, L".m2ts") || StrEqualI(ext, L".3gp") ||
      StrEqualI(ext, L".vob") || StrEqualI(ext, L".ogv") ||
      StrEqualI(ext, L".asf"))
  ...
  if (StrEqualI(ext, L".wma") || StrEqualI(ext, L".aac") ||
      StrEqualI(ext, L".m4a") || StrEqualI(ext, L".ogg") ||
      StrEqualI(ext, L".opus") || StrEqualI(ext, L".wav") ||
      StrEqualI(ext, L".aiff") || StrEqualI(ext, L".aif") ||
  ...
  if (StrEqualI(ext, L".epub"))
    return DetectedFormat::DocumentEPUB;
  if (StrEqualI(ext, L".mobi") || StrEqualI(ext, L".azw") ||
      StrEqualI(ext, L".azw3") || StrEqualI(ext, L".fb2"))
    return DetectedFormat::DocumentEPUB; // eBook category
  if (StrEqualI(ext, L".docx") || StrEqualI(ext, L".doc") ||
      StrEqualI(ext, L".rtf") || StrEqualI(ext, L".odt") ||
      StrEqualI(ext, L".xps") || StrEqualI(ext, L".oxps") ||
      StrEqualI(ext, L".djvu") || StrEqualI(ext, L".djv"))
    return DetectedFormat::DocumentPDF; // Doc category
  if (StrEqualI(ext, L".xlsx") || StrEqualI(ext, L".xls") ||
      StrEqualI(ext, L".ods") || StrEqualI(ext, L".pptx") ||
      StrEqualI(ext, L".ppt") || StrEqualI(ext, L".odp"))
    return DetectedFormat::DocumentPDF;

  // Font formats
  if (StrEqualI(ext, L".ttf") || StrEqualI(ext, L".otf") ||
      StrEqualI(ext, L".woff") || StrEqualI(ext, L".woff2") ||
      StrEqualI(ext, L".ttc") || StrEqualI(ext, L".fon") ||
      StrEqualI(ext, L".fnt"))
...
DetectedFormat FormatDetector::DetectFromSignature(const wchar_t *filePath) {
  ...
  DWORD bytesRead = 0;
  BOOL success =
      ReadFile(hFile, signature, sizeof(signature), &bytesRead, nullptr);
  CloseHandle(hFile);
  ...
  if ((signature[0] == 0x49 && signature[1] == 0x49 && signature[2] == 0x2A &&
       signature[3] == 0x00) ||
      (signature[0] == 0x4D && signature[1] == 0x4D && signature[2] == 0x00 &&
       signature[3] == 0x2A)) {
    return DetectedFormat::ImageTIFF;
  }
  ...
  if (signature[0] == 0x52 && signature[1] == 0x49 && signature[2] == 0x46 &&
      signature[3] == 0x46 && bytesRead >= 12 && signature[8] == 0x57 &&
  ...
  // PDF: 25 50 44 46 (%PDF)
  if (signature[0] == 0x25 && signature[1] == 0x50 && signature[2] == 0x44 &&
      signature[3] == 0x46) {
    return DetectedFormat::DocumentPDF;
  }
...
bool FormatDetector::IsImageFormat(const wchar_t *extension) const {
  DetectedFormat type =
      const_cast<FormatDetector *>(this)->DetectFromExtension(extension);

  return (type >= DetectedFormat::ImageJPEG &&
          type <= DetectedFormat::ImageSVG);
}
...
bool FormatDetector::IsArchiveFormat(const wchar_t *extension) const {
  ...
  return (type >= DetectedFormat::ArchiveZIP &&
          type <= DetectedFormat::ArchiveTAR);
}
...
bool FormatDetector::IsDocumentFormat(const wchar_t *extension) const {
  ...
  return (type >= DetectedFormat::DocumentPDF &&
          type <= DetectedFormat::DocumentEPUB);
}
