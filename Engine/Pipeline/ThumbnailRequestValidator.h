...
static const wchar_t* RequestValidationErrorName(RequestValidationError e)
{
    static const wchar_t* names[] = {
        L"None",           L"NullPath",          L"EmptyPath",       L"PathTooLong",  L"InvalidCharacters",
        L"ZeroDimensions", L"OversizedWidth",    L"OversizedHeight", L"FileNotFound", L"ZeroFileSize",
...
class ThumbnailRequestValidator
{
  public:
    ...
    RequestValidationResult Validate(const wchar_t* filePath, uint32_t requestedWidth, uint32_t requestedHeight)
    {
    ...
    // Get stats
    RequestValidationStats GetStats() const
    {
        return m_stats;
    }

    // Get limits
    const RequestValidationLimits& GetLimits() const
    {
        return m_limits;
    }
