...
struct ZeroCopyBuffer
{
    ...
    bool IsValid() const
    {
        return ptr != nullptr && sizeBytes > 0;
    }
    bool CanUploadToGPU() const
    {
        return IsZeroCopyCapable(origin) && IsValid();
    }
};
...
struct GPUUploadDescriptor
{
    ...
    bool IsReady() const
    {
        return srcBuffer && srcBuffer->CanUploadToGPU() && widthPx > 0 && heightPx > 0;
    }
};
...
struct PipelineZeroCopyStats
{
    ...
    {
        return bytesCopied == 0 && bytesHandedOff > 0;
    }
};
...
class ZeroCopyPipeline
{
  public:
    ...
    static bool UploadToGPU(const GPUUploadDescriptor& desc, PipelineZeroCopyStats& stats)
    {
        if (!desc.IsReady())
            return false;
