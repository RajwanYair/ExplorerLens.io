...
public:
    ...
    static const wchar_t* GetName() { return L"MemoryMappedDecodePath"; }
    ...
    MappedFileView MapFile(const wchar_t* filePath) const {
        MappedFileView view;
        if (!filePath) return view;

        view.hFile = CreateFileW(filePath, GENERIC_READ, FILE_SHARE_READ,
        ...
        view.data = static_cast<const uint8_t*>(
            MapViewOfFile(view.hMapping, FILE_MAP_READ, 0, 0, 0));
        if (!view.data) {
    ...
    MMapDecodeStats GetStats() const { return m_stats; }
