...
class ZeroCopyRenderer
{
  public:
    ...
    [[nodiscard]] Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> Upload(std::span<const uint8_t> bgra, uint32_t width,
                                                                          uint32_t height, uint32_t stride);

    // Async variant — queues upload to worker thread, invokes callback on completion.
    void UploadAsync(std::span<const uint8_t> bgra, uint32_t width, uint32_t height, uint32_t stride,
                     std::function<void(Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>)> callback);

    [[nodiscard]] const UploadStats& Stats() const noexcept
    {
        return m_stats;
    }
