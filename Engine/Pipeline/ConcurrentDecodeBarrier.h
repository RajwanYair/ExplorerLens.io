...
public:
    explicit ConcurrentDecodeBarrier(uint32_t participantCount = 1)
        : m_targetCount(participantCount) {
    }
    ...
    bool Wait(uint32_t timeoutMs = 5000) {
        std::unique_lock<std::mutex> lock(m_mutex);
        if (m_state == BarrierState::Released) return true;
    ...
    void Reset(uint32_t newParticipantCount = 0) {
        ...
        m_arrivedCount = 0;
        if (newParticipantCount > 0) m_targetCount = newParticipantCount;
    ...
    BarrierState GetState() const { return m_state; }
    uint32_t GetArrivedCount() const { return m_arrivedCount; }
    uint32_t GetTargetCount() const { return m_targetCount; }
    BarrierMetrics GetMetrics() const { return m_metrics; }
