...
inline const char* JobPriorityName(JobPriority p) {
    switch (p) {
        ...
        case JobPriority::Idle:     return "Idle";
...
struct ThumbnailJob {
    ...
    bool IsComplete() const { return IsTerminalStatus(status); }
    bool IsSuccess()  const { return status == JobStatus::Completed; }
    bool IsFailed()   const { return status == JobStatus::Failed; }
...
struct BatchRequest {
    ...
    size_t FileCount() const { return filePaths.size(); }
...
struct ProgressInfo {
    ...
    double PercentComplete() const {
        ...
        return (static_cast<double>(completedJobs + failedJobs + cancelledJobs)
                / static_cast<double>(totalJobs)) * 100.0;
    }
    ...
    double EstimatedRemainingMs() const {
        size_t done = completedJobs + failedJobs + cancelledJobs;
        if (done == 0 || elapsedMs <= 0.0) return 0.0;
...
struct BatchResult {
    ...
    std::string Summary() const {
        ...
        ss << "Batch #" << batchId << ": "
           << succeeded << "/" << totalJobs << " succeeded"
           << " (" << std::fixed << std::setprecision(1) << SuccessRate() << "%), "
           << failed << " failed, " << cancelled << " cancelled, "
           << std::setprecision(0) << totalTimeMs << " ms total, "
...
struct RateLimitConfig {
    ...
    bool IsWithinLimits(size_t currentQueueDepth, size_t currentActive) const {
        if (currentQueueDepth >= maxQueueDepth) return false;
        if (currentActive >= maxConcurrentJobs)  return false;
        return true;
    }
};
...
public:
    ...
    explicit BatchProcessor(const RateLimitConfig& config)
        : m_rateConfig(config) {}
    ...
    size_t TotalSubmitted()  const { return m_totalSubmitted.load(); }
    size_t TotalCompleted()  const { return m_totalCompleted.load(); }
    size_t TotalFailed()     const { return m_totalFailed.load(); }
    size_t ActiveJobs()      const { return m_activeJobs.load(); }
    ...
    void SetJobCompleteCallback(JobCompleteCallback cb) { m_jobCompleteCb = std::move(cb); }

    //--- Simulated Processing (for testing) ---

    bool ProcessNextJob(ThumbnailJob& job) {
        if (m_paused.load() || m_cancelled.load()) return false;
        if (!m_rateConfig.IsWithinLimits(m_queue.Size(), m_activeJobs.load())) {
            // Rate limited — can still pop if under concurrent limit
            if (m_activeJobs.load() >= m_rateConfig.maxConcurrentJobs) return false;
        }

        if (!m_queue.TryPop(job)) return false;
    ...
    const RateLimitConfig& GetRateConfig() const { return m_rateConfig; }
    void SetRateConfig(const RateLimitConfig& config) { m_rateConfig = config; }
