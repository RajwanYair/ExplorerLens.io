...
public:
    ...
    static inline const wchar_t* StrategyName(PrefetchStrategy s) {
        switch (s) {
        case PrefetchStrategy::None:                 return L"Disabled";
        case PrefetchStrategy::LinearAhead:          return L"Linear Ahead";
        case PrefetchStrategy::BidirectionalWindow:  return L"Bidirectional Window";
        case PrefetchStrategy::AccessFrequency:      return L"Access Frequency";
        case PrefetchStrategy::MLPredicted:          return L"ML Predicted";
    ...
    static inline const wchar_t* DirectionName(ScrollDirection d) {
        switch (d) {
        case ScrollDirection::Unknown:   return L"Unknown";
        case ScrollDirection::Down:      return L"Down";
        case ScrollDirection::Up:        return L"Up";
        case ScrollDirection::PageDown:  return L"Page Down";
        case ScrollDirection::PageUp:    return L"Page Up";
        case ScrollDirection::JumpToEnd: return L"Jump to End";
        default:                         return L"Unknown";
        }
    }

    /// Calculate prefetch range based on viewport and scroll direction.
    static inline void CalcPrefetchRange(
        uint32_t viewportStart, uint32_t viewportSize,
        ...
        outEnd = static_cast<uint32_t>(
            (end > static_cast<int32_t>(totalItems))
            ? totalItems : static_cast<uint32_t>(end));
    }
    ...
    inline std::vector<std::wstring> PredictNext(uint32_t count) {
        ...
        for (const auto& pair : dirCounts) {
            if (pair.second >= 3 && predictions.size() < count) {
                ...
                for (auto& sib : siblings) {
                    ...
                    if (accessed.find(sib) == accessed.end() &&
        ...
        if (predictions.size() < count && m_history.size() >= 3) {
            ...
            if (ascending && seqCount >= 2) {
                ...
                auto pos = std::lower_bound(siblings.begin(), siblings.end(),
                    m_history.back());
                if (pos != siblings.end()) ++pos; // move past current
                while (pos != siblings.end() && predictions.size() < count) {
                    if (accessed.find(*pos) == accessed.end() &&
        ...
        if (predictions.size() < count && !m_history.empty()) {
            ...
            if (topCount >= 2 && !topExt.empty()) {
                ...
                for (auto& sib : siblings) {
                    if (predictions.size() >= count) break;
                    if (GetExtension(sib) == topExt &&
                        accessed.find(sib) == accessed.end() &&
                        seen.find(sib) == seen.end()) {
                        predictions.push_back(sib);
                        seen.insert(sib);
                    }
        ...
        m_stats.avgPredictionLatencyUs =
            (m_stats.avgPredictionLatencyUs * (m_stats.predictionsMade - 1)
                + latUs) / static_cast<double>(m_stats.predictionsMade);
    ...
    inline void StartPrefetching(
        std::function<void(const std::wstring&)> prefetchFn) {
    ...
    inline void RecordRequest(const std::wstring& filePath) {
        ...
        m_stats.hitRate = CalcHitRate(m_stats.prefetchHits,
            m_stats.prefetchMisses);
    }
...
private:
    ...
    static inline std::wstring GetExtension(const std::wstring& path) {
        ...
        if (slashPos != std::wstring::npos && dotPos < slashPos) return {};
    ...
    static inline std::vector<std::wstring> EnumerateDirectory(
    ...
    inline void PrefetchWorker() {
        ...
        while (!m_stopPrefetch.load(std::memory_order_acquire)) {
            ...
            for (const auto& path : predictions) {
                if (m_stopPrefetch.load(std::memory_order_acquire)) break;
