...
ParallelBatchDecoder::ParallelBatchDecoder() : m_config() {}

ParallelBatchDecoder::ParallelBatchDecoder(const BatchDecoderConfig& config) : m_config(config) {}
...
void ParallelBatchDecoder::Shutdown()
{
    ...
    for (auto& [batchId, results] : m_batchResults) {
        for (auto& item : results) {
            if (item.status == BatchItemStatus::Pending || item.status == BatchItemStatus::Decoding) {
...
uint64_t ParallelBatchDecoder::SubmitBatch(const std::vector<std::wstring>& files, BatchPriority priority,
                                           BatchProgressCallback progressCb, BatchCompleteCallback completeCb)
{
    (void)priority;
    (void)progressCb;
    (void)completeCb;
...
ParallelismLevel ParallelBatchDecoder::GetParallelismForFormat(const std::wstring& extension) const
{
...
ParallelismLevel ParallelBatchDecoder::ClassifyFormat(const std::wstring& extension)
{
    ...
    if (extension == L".zip" || extension == L".rar" || extension == L".7z" || extension == L".cbz"
        || extension == L".cbr" || extension == L".cb7" || extension == L".tar" || extension == L".gz") {
        return ParallelismLevel::SerialOnly;
    }

    // GPU-dependent formats — limited by GPU resources
    if (extension == L".dds" || extension == L".ktx" || extension == L".ktx2" || extension == L".vtf") {
        return ParallelismLevel::LimitedParallel;
    }

    // External library formats — library-specific limits
    if (extension == L".cr2" || extension == L".nef" || extension == L".arw" || extension == L".dng"
        || extension == L".orf" || extension == L".rw2" || extension == L".heif" || extension == L".heic"
        || extension == L".avif") {
        return ParallelismLevel::LimitedParallel;
    }
...
{
    switch (level) {
        case ParallelismLevel::FullParallel:
            return L"FullParallel";
        case ParallelismLevel::LimitedParallel:
            return L"LimitedParallel";
        case ParallelismLevel::SerialOnly:
            return L"SerialOnly";
        case ParallelismLevel::Adaptive:
            return L"Adaptive";
        default:
            return L"Unknown";
    }
...
{
    switch (priority) {
        case BatchPriority::Immediate:
            return L"Immediate";
        case BatchPriority::Background:
            return L"Background";
        case BatchPriority::CacheWarm:
            return L"CacheWarm";
        default:
            return L"Unknown";
    }
...
std::unordered_map<std::wstring, std::vector<size_t>> ParallelBatchDecoder::GroupByFormat(
