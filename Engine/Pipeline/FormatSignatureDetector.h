...
{
    static const char* names[] = {"Unknown", "Image", "Archive",    "Document",   "Video",   "Audio",
...
class FormatSignatureDetector
{
  public:
    ...
    {
        return m_stats.registeredSignatures;
    }
    ...
    const SignatureDetectorStats& GetStats() const
    {
        return m_stats;
    }
  ...
  private:
    float MatchSignature(const uint8_t* data, size_t size, const FormatSignature& sig) const
    {
        if (!data || size == 0)
            return 0.0f;
        if (sig.signatureOffset + sig.magicBytes.size() > size)
            return 0.0f;
        ...
        if (sig.hasSecondary && sig.secondaryOffset + sig.secondaryBytes.size() <= size) {
            ...
            for (size_t i = 0; i < sig.secondaryBytes.size(); i++) {
                if (sp[i] != sig.secondaryBytes[i]) {
                    secMatch = false;
                    break;
                }
            }
            if (secMatch)
                conf = (std::min)(1.0f, conf + 0.15f);
        }
    ...
    {
        // JPEG
        RegisterSignature({"JPEG", "JPEG Image", FormatClass::Image, 0, {0xFF, 0xD8, 0xFF}, {}, 0.9f, false, 0, {}});

        // PNG
        RegisterSignature({"PNG",
        ...
        // GIF87a / GIF89a
        RegisterSignature({"GIF", "GIF Image", FormatClass::Image, 0, {0x47, 0x49, 0x46, 0x38}, {}, 0.9f, false, 0, {}});

        // BMP
        RegisterSignature({"BMP", "Bitmap Image", FormatClass::Image, 0, {0x42, 0x4D}, {}, 0.7f, false, 0, {}});

        // TIFF (little-endian)
        RegisterSignature(
            {"TIFF_LE", "TIFF Image (LE)", FormatClass::Image, 0, {0x49, 0x49, 0x2A, 0x00}, {}, 0.9f, false, 0, {}});

        // TIFF (big-endian)
        RegisterSignature(
            {"TIFF_BE", "TIFF Image (BE)", FormatClass::Image, 0, {0x4D, 0x4D, 0x00, 0x2A}, {}, 0.9f, false, 0, {}});

        // WebP
        RegisterSignature({"WebP",
                           "WebP Image",
                           FormatClass::Image,
                           0,
                           {0x52, 0x49, 0x46, 0x46},
        ...
        // ZIP
        RegisterSignature(
            {"ZIP", "ZIP Archive", FormatClass::Archive, 0, {0x50, 0x4B, 0x03, 0x04}, {}, 0.9f, false, 0, {}});

        // RAR5
        RegisterSignature({"RAR5",
                           "RAR5 Archive",
                           FormatClass::Archive,
                           0,
        ...
        // PDF
        RegisterSignature(
            {"PDF", "PDF Document", FormatClass::Document, 0, {0x25, 0x50, 0x44, 0x46}, {}, 0.95f, false, 0, {}});
