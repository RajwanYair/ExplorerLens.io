...
class AsyncThumbnailProvider
{
  public:
    ...
    uint64_t SubmitRequest(const std::wstring& filePath, uint32_t requestedSize, DecodePriority priority,
                           AsyncDecodeCallback callback);
    ...
    AsyncDecodeResult DecodeSynchronous(const std::wstring& filePath, uint32_t requestedSize);
    ...
    /// Get configuration
    const ThumbnailProviderConfig& GetConfig() const
    {
        return m_config;
    }
