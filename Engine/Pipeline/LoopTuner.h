...
#define LENS_UNROLL_2(var, limit, body)          \
    {                                            \
        ...
        for (; var < var##_end2; var += 2) {     \
            { body }                             \
            { size_t var##_save = var; var = var##_save + 1; body var = var##_save; } \
        }                                        \
        for (var = var##_end2; var < (limit); ++var) { body } \
    }
...
#define LENS_UNROLL_4(idx, count, body)          \
    do {                                         \
        ...
        for (idx = _u4_end; idx < _u4_count; ++idx) { body } \
    } while(0)
...
#define LENS_UNROLL_8(idx, count, body)          \
    do {                                         \
        ...
        for (idx = _u8_end; idx < _u8_count; ++idx) { body } \
    } while(0)
...
struct TileConfig {
    ...
    static TileConfig ForPixelFormat(size_t bytesPerPixel, size_t imageWidth) {
        ...
        if (rowBytes > 0) {
            cfg.tileWidth = (std::min)(imageWidth,
                cfg.l1CacheBytes / (bytesPerPixel * cfg.tileHeight));
            if (cfg.tileWidth < 8) cfg.tileWidth = 8;
...
public:
    ...
    void ProcessImage(size_t imageWidth, size_t imageHeight,
                      const TileCallback& callback) {
        if (!callback || imageWidth == 0 || imageHeight == 0) return;
    ...
    void ProcessRowBatches(size_t totalRows, size_t batchSize,
        ...
        if (!callback || totalRows == 0) return;
        if (batchSize == 0) batchSize = m_config.tileHeight;
    ...
    void SetConfig(const TileConfig& config) { m_config = config; }

    /// Get processing statistics
    const TileProcessorStats& GetStats() const { return m_stats; }

    /// Reset statistics
    void ResetStats() { m_stats.Reset(); }

    /// Calculate optimal tile count for an image
    static size_t CalculateTileCount(size_t imageWidth, size_t imageHeight,
...
public:
    ...
    bool Enqueue(uint32_t opType, size_t offset, size_t length) {
        if (m_count >= MaxOps) return false;
    ...
    uint64_t TotalFlushed() const { return m_totalFlushed; }

    /// Get number of flush calls
    uint64_t FlushCount() const { return m_flushCount; }

    /// Check if batch is full
    bool IsFull() const { return m_count >= MaxOps; }

    /// Clear without executing
    void Clear() { m_count = 0; }

    /// Maximum batch size
    static constexpr size_t MaxBatchSize() { return MaxOps; }
