...
enum class DecodeWindowContext { Foreground, Background, Minimized };
enum class FPIDecodePriority      { Idle = 0, Low = 1, Normal = 2, High = 3, Realtime = 4 };
...
public:
    ...
    void SetForegroundWindow(uint32_t hwnd) noexcept { m_foregroundHwnd = hwnd; }
    uint32_t GetForegroundWindow() const noexcept    { return m_foregroundHwnd; }
    ...
    void SetForegroundPriority(FPIDecodePriority p) noexcept { m_foregroundPriority = p; }
    void SetBackgroundPriority(FPIDecodePriority p) noexcept { m_backgroundPriority = p; }
    FPIDecodePriority ForegroundPriority() const noexcept    { return m_foregroundPriority; }
    FPIDecodePriority BackgroundPriority() const noexcept    { return m_backgroundPriority; }
