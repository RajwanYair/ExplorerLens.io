...
{
    static const char* names[] = {"Placeholder", "Header", "TinyPreview", "LowRes", "MediumRes", "FullRes", "Enhanced"};
...
struct StreamingDecodeStats
{
    ...
    {
        return (totalDecodes > 0) ? (static_cast<double>(progressiveDecodes) / totalDecodes * 100.0) : 0.0;
    }
};
...
{
    static const char* names[] = {"Unknown", "JPEG", "PNG", "WebP", "BMP", "GIF", "TIFF"};
...
class StreamingDecodeEngine
{
  public:
    ...
    explicit StreamingDecodeEngine(const StreamingDecodeConfig& config) : m_config(config) {}

    // ====================================================================
    // Configuration
    // ====================================================================

    const StreamingDecodeConfig& GetConfig() const
    {
        return m_config;
    }
    void SetConfig(const StreamingDecodeConfig& config)
    {
        m_config = config;
    }
    ...
    inline uint32_t BeginDecode(const std::wstring& filePath, uint64_t fileSize)
    {
    ...
    inline ProgressiveResult FeedChunk(uint32_t sessionId, const StreamChunk& chunk)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        ...
        if (achievableLevel > session.currentLevel || chunk.isFinal) {
            ...
            result.decodeTimeMs = std::chrono::duration<double, std::milli>(now - session.startTime).count();

            if (!session.firstPaintReported && result.level >= DecodeLoD::TinyPreview) {
                uint64_t n = m_stats.totalDecodes;
                m_stats.avgFirstPaintMs = (m_stats.avgFirstPaintMs * static_cast<double>(n - 1) + result.decodeTimeMs)
    ...
    inline void EndDecode(uint32_t sessionId)
    {
        ...
        if (it != m_v14Sessions.end()) {
            auto now = std::chrono::steady_clock::now();
            double totalMs = std::chrono::duration<double, std::milli>(now - it->second.startTime).count();
            uint64_t n = m_stats.totalDecodes;
            m_stats.avgFullDecodeMs =
                (m_stats.avgFullDecodeMs * static_cast<double>(n - 1) + totalMs) / static_cast<double>(n);
    ...
    {
        if (pct >= 1.0f)
            return DecodeLoD::FullRes;
        if (pct >= 0.5f)
            return DecodeLoD::MediumRes;
        if (pct >= 0.125f)
            return DecodeLoD::LowRes;
        if (pct >= 0.01f)
            return DecodeLoD::TinyPreview;
        if (pct > 0.0f)
            return DecodeLoD::Header;
    ...
    {
        static const float scores[] = {0.0f, 0.05f, 0.15f, 0.35f, 0.65f, 0.95f, 1.0f};
    ...
    inline uint64_t BeginStream(const std::wstring& filePath, uint32_t targetWidth, uint32_t targetHeight)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            if (GetFileSizeEx(hFile, &fsize)) {
                session.estimatedTotalBytes = static_cast<uint64_t>(fsize.QuadPart);
            }

            // Read first 64 bytes for format detection
            uint8_t headerBuf[64] = {};
            DWORD bytesRead = 0;
            if (ReadFile(hFile, headerBuf, 64, &bytesRead, nullptr) && bytesRead >= 4) {
                session.format = DetectFormatFromHeader(headerBuf, bytesRead);
                session.accumulatedData.assign(headerBuf, headerBuf + bytesRead);
                session.bytesReceived = bytesRead;

                // Parse dimensions if enough header data
                if (session.format == StreamFileFormat::PNG && bytesRead >= 24) {
                    ParsePngDimensions(headerBuf, bytesRead, session.detectedWidth, session.detectedHeight);
                }
                if (session.format == StreamFileFormat::JPEG && bytesRead >= 16) {
                    ParseJpegSOF(headerBuf, bytesRead, session.detectedWidth, session.detectedHeight);
                }
                if (session.format == StreamFileFormat::BMP && bytesRead >= 26) {
                    ParseBmpDimensions(headerBuf, bytesRead, session.detectedWidth, session.detectedHeight);
                }
    ...
    inline bool FeedData(uint64_t sessionId, const uint8_t* data, size_t size)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        ...
        session.accumulatedData.insert(session.accumulatedData.end(), data, data + size);
        session.bytesReceived += size;
        m_stats.totalBytesStreamed += size;

        // Try to detect format if still unknown
        if (session.format == StreamFileFormat::Unknown && session.accumulatedData.size() >= 12) {
            session.format = DetectFormatFromHeader(session.accumulatedData.data(), session.accumulatedData.size());
        }

        // Parse JPEG SOF if we haven't found dimensions yet
        if (session.format == StreamFileFormat::JPEG && session.detectedWidth == 0
            && session.accumulatedData.size() >= 16) {
            ParseJpegSOF(session.accumulatedData.data(), session.accumulatedData.size(), session.detectedWidth,
                         session.detectedHeight);
        }

        // Update phase based on data received
        if (session.estimatedTotalBytes > 0) {
            float ratio = static_cast<float>(session.bytesReceived) / static_cast<float>(session.estimatedTotalBytes);
            if (ratio >= 1.0f) {
    ...
    inline bool GetPartialResult(uint64_t sessionId, std::vector<uint8_t>& outRGBA, uint32_t& outWidth,
                                 uint32_t& outHeight)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        ...
        if (session.partialResult.empty())
            return false;
    ...
    inline StreamingDecodeStats GetStats() const
    {
        return m_stats;
    }
  ...
  private:
    ...
    static inline StreamFileFormat DetectFormatFromHeader(const uint8_t* data, size_t size)
    {
        if (size < 4)
    ...
    static inline void ParseJpegSOF(const uint8_t* data, size_t size, uint32_t& outW, uint32_t& outH)
    {
    ...
    static inline void ParsePngDimensions(const uint8_t* data, size_t size, uint32_t& outW, uint32_t& outH)
    {
        ...
        if (data[12] == 'I' && data[13] == 'H' && data[14] == 'D' && data[15] == 'R') {
            outW = (static_cast<uint32_t>(data[16]) << 24) | (static_cast<uint32_t>(data[17]) << 16)
                   | (static_cast<uint32_t>(data[18]) << 8) | data[19];
            outH = (static_cast<uint32_t>(data[20]) << 24) | (static_cast<uint32_t>(data[21]) << 16)
    ...
    static inline void ParseBmpDimensions(const uint8_t* data, size_t size, uint32_t& outW, uint32_t& outH)
    {
        ...
        if (size < 26)
            return;
        outW = static_cast<uint32_t>(data[18]) | (static_cast<uint32_t>(data[19]) << 8)
               | (static_cast<uint32_t>(data[20]) << 16) | (static_cast<uint32_t>(data[21]) << 24);
        int32_t rawH =
            static_cast<int32_t>(static_cast<uint32_t>(data[22]) | (static_cast<uint32_t>(data[23]) << 8)
    ...
    inline void UpdatePartialResult(StreamingSession& session)
    {
        ...
        float progress = 0.0f;
        if (session.estimatedTotalBytes > 0) {
            progress = static_cast<float>(session.bytesReceived) / static_cast<float>(session.estimatedTotalBytes);
            if (progress > 1.0f)
                progress = 1.0f;
        }

        // Compute base intensity from progress (lighter when less data)
        uint8_t baseIntensity = static_cast<uint8_t>(128.0f + 127.0f * progress);

        // Generate a simple gradient pattern to visualize progress
        for (uint32_t y = 0; y < h; ++y) {
            float yFrac = static_cast<float>(y) / static_cast<float>(h);
            uint8_t rowVal = static_cast<uint8_t>(static_cast<float>(baseIntensity) * (0.5f + 0.5f * yFrac));
    ...
    inline DecodeLoD DetermineLevel(const V14Session& session) const
    {
        if (session.fileSize == 0)
            return DecodeLoD::Placeholder;
        float ratio = static_cast<float>(session.bytesReceived) / static_cast<float>(session.fileSize);
