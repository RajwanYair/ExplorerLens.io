...
inline const char* ZeroCopyModeToString(ZeroCopyMode mode) {
    static const char* names[] = {
        "Disabled", "MappedUpload", "DirectStorage",
        "MemoryMapped", "PinnedMemory"
    };
...
struct ZeroCopyStats {
    ...
    double GetZeroCopyRate() const {
        return (totalTransfers > 0)
            ? (static_cast<double>(zeroCopyTransfers) / totalTransfers * 100.0)
            : 0.0;
    }
    ...
    double GetBandwidthSavingPercent() const {
        ...
        return (totalPossible > 0)
            ? (static_cast<double>(bytesSavedByCopy) / totalPossible * 100.0)
            : 0.0;
    }
};
...
public:
    ...
    ZeroCopyMode GetActiveMode() const { return m_supportedMode; }
    ...
    bool TransferToGPU(const void* decoderOutput, uint64_t size,
        uint64_t gpuDestination) {
        ...
        m_stats.avgTransferTimeUs =
            (m_stats.avgTransferTimeUs * (m_stats.totalTransfers - 1) + elapsedUs)
            / m_stats.totalTransfers;
    ...
    ZeroCopyStats GetStats() const { return m_stats; }
