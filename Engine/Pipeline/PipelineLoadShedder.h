...
class PipelineLoadShedder
{
  public:
    ...
    PipelineLoadShedderConfig GetConfig() const
    {
        return m_config;
    }
    ...
    Decision Evaluate(uint32_t currentQueueDepth, uint32_t requestPriority) const
    {
        ...
        if (currentQueueDepth >= m_config.shedThreshold && requestPriority < m_config.minPriorityToKeep)
    ...
    uint64_t GetTotalShed() const
    {
        return m_totalShed;
    }
    uint64_t GetTotalAccepted() const
    {
        return m_totalAccepted;
    }
