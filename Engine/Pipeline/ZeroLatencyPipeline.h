...
public:
    ...
    ZLPResult Process(const ZLPRequest& req) {
        ...
        if (result.success) m_metrics.requestsCompleted++;
        else                m_metrics.requestsFailed++;
        if (result.usedGPUPath) m_metrics.gpuPathHits++;
        else                    m_metrics.cpuFallbackHits++;
    ...
    const ZLPMetrics& GetMetrics() const { return m_metrics; }

    void ResetMetrics() { m_metrics = {}; }

    bool IsGPUPathAvailable() const { return m_gpuAvailable; }
    ...
    static uint32_t RecommendedThumbSize(uint64_t fileSizeBytes) {
        ...
        if (fileSizeBytes > 50ULL * 1024 * 1024) return 512;
        if (fileSizeBytes > 10ULL * 1024 * 1024) return 320;
        return 256;
    }
