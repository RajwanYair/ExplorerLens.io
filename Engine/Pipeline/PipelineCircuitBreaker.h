...
static const wchar_t* PipelineCircuitStateName(PipelineCircuitState s) {
    switch (s) {
    case PipelineCircuitState::Closed:   return L"Closed";
    case PipelineCircuitState::Open:     return L"Open";
    case PipelineCircuitState::HalfOpen: return L"HalfOpen";
...
private:
    ...
    void EvaluateTrip() {
        uint64_t wTotal = m_windowTotal.load(std::memory_order_relaxed);
        if (wTotal < m_config.minimumRequests) return;
    ...
    void TryRecover() {
        ...
        if (probes < m_config.halfOpenMaxRequests) return;
