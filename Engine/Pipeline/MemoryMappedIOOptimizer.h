...
class MemoryMappedIOOptimizer
{
  public:
    ...
    static inline uint64_t AlignOffset(uint64_t offset, uint64_t granularity = 65536)
    {
    ...
    explicit MemoryMappedIOOptimizer(size_t maxCached = 16) : m_maxCached(maxCached)
    {
    ...
    inline MappedFile MapFile(const std::wstring& path)
    {
        ...
        HANDLE hMapping = CreateFileMappingW(
            hFile, nullptr, PAGE_READONLY, static_cast<DWORD>(static_cast<uint64_t>(fileSize.QuadPart) >> 32),
            static_cast<DWORD>(static_cast<uint64_t>(fileSize.QuadPart) & 0xFFFFFFFFu), nullptr);
        if (!hMapping) {
            CloseHandle(hFile);
            ReleaseSRWLockExclusive(&m_lock);
            return {};
    ...
    inline MappedFile MapFileRange(const std::wstring& path, uint64_t offset, size_t length)
    {
        HANDLE hFile = CreateFileW(path.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr,
        ...
        void* view = MapViewOfFile(hMapping, FILE_MAP_READ, offHigh, offLow, mapLen);
        if (!view) {
            CloseHandle(hMapping);
            CloseHandle(hFile);
            return {};
        ...
        result.size = (length <= mapLen - extraBytes) ? length : (mapLen - extraBytes);
        result.hFile = hFile;
        result.hMapping = hMapping;
    ...
    inline bool PrefetchRegion(const MappedFile& file, size_t offset, size_t prefetchSize)
    {
        if (!file.data || offset + prefetchSize > file.size)
            return false;

        // Dynamically load PrefetchVirtualMemory (available on Win8+)
        using PrefetchVMFn = BOOL(WINAPI*)(HANDLE, ULONG_PTR, PWIN32_MEMORY_RANGE_ENTRY, ULONG);

        static PrefetchVMFn pfn = []() -> PrefetchVMFn {
            HMODULE hKernel = GetModuleHandleW(L"kernel32.dll");
            if (!hKernel)
                return nullptr;
            return reinterpret_cast<PrefetchVMFn>(GetProcAddress(hKernel, "PrefetchVirtualMemory"));
        }();
        ...
        entry.VirtualAddress = const_cast<void*>(static_cast<const void*>(file.data + offset));
