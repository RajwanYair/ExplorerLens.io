...
class RetryPolicyEngine
{
  public:
    ...
    RetryConfig& Config() noexcept
    {
        return m_cfg;
    }
    const RetryConfig& Config() const noexcept
    {
        return m_cfg;
    }
    const RetryEngineStats& Stats() const noexcept
    {
        return m_stats;
    }
    ...
    {
        return m_cfg.maxRetries;
    }
    double BaseDelay() const noexcept
    {
        return m_cfg.baseDelayMs;
    }
  ...
  private:
    double ComputeDelay(int attempt)
    {
        ...
        if (m_cfg.enableJitter) {
            std::uniform_real_distribution<double> dist(d * (1.0 - m_cfg.jitterFactor), d * (1.0 + m_cfg.jitterFactor));
            d = dist(m_rng);
        }
        return d;
    }
