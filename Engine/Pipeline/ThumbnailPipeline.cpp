...
ProfileComponent GetDecoderProfileComponent(const IThumbnailDecoder* decoder)
{
    ...
    if (_wcsicmp(name, L"WebPDecoder") == 0)
        return ProfileComponent::DECODE_WEBP;
    if (_wcsicmp(name, L"AVIFDecoder") == 0)
        return ProfileComponent::DECODE_AVIF;
    if (_wcsicmp(name, L"ArchiveDecoder") == 0)
        return ProfileComponent::DECODE_ARCHIVE;
    if (_wcsicmp(name, L"JXLDecoder") == 0)
        return ProfileComponent::DECODE_JXL;
    if (_wcsicmp(name, L"HEIFDecoder") == 0)
        return ProfileComponent::DECODE_HEIF;
    if (_wcsicmp(name, L"RAWDecoder") == 0)
        return ProfileComponent::DECODE_RAW;
    if (_wcsicmp(name, L"ICODecoder") == 0)
        return ProfileComponent::DECODE_ICO;
    if (_wcsicmp(name, L"TGADecoder") == 0)
        return ProfileComponent::DECODE_TGA;
    if (_wcsicmp(name, L"QOIDecoder") == 0)
        return ProfileComponent::DECODE_QOI;
    if (_wcsicmp(name, L"PSDDecoder") == 0)
        return ProfileComponent::DECODE_PSD;
    if (_wcsicmp(name, L"DDSDecoder") == 0)
        return ProfileComponent::DECODE_DDS;
    if (_wcsicmp(name, L"HDRDecoder") == 0)
        return ProfileComponent::DECODE_HDR;
    if (_wcsicmp(name, L"PPMDecoder") == 0)
        return ProfileComponent::DECODE_PPM;
    if (_wcsicmp(name, L"EXRDecoder") == 0)
        return ProfileComponent::DECODE_EXR;
    if (_wcsicmp(name, L"SVGDecoder") == 0)
        return ProfileComponent::DECODE_SVG;
    if (_wcsicmp(name, L"VideoDecoder") == 0)
        return ProfileComponent::DECODE_VIDEO;
    if (_wcsicmp(name, L"AudioDecoder") == 0)
        return ProfileComponent::DECODE_AUDIO;
    if (_wcsicmp(name, L"PDFDecoder") == 0)
        return ProfileComponent::DECODE_PDF;
    if (_wcsicmp(name, L"DocumentDecoder") == 0)
        return ProfileComponent::DECODE_DOCUMENT;
    if (_wcsicmp(name, L"FontDecoder") == 0)
        return ProfileComponent::DECODE_FONT;
  ...
  public:
    ...
    ThumbnailResult GenerateThumbnail(const ThumbnailRequest& request)
    {
        ...
        if (config.enableCache && cacheProvider) {
            ...
            if (SUCCEEDED(cacheProvider->Get(request.filePath, request.width, request.height, &cachedBitmap))) {
                // Cache hit!
        ...
        if (SUCCEEDED(result.status) && config.enableCache && cacheProvider && result.hBitmap) {
            ...
            cacheProvider->Put(request.filePath, result.width, result.height, result.hBitmap);
    ...
    void GetStatistics(uint64_t& outTotalRequests, uint64_t& outCacheHits, uint64_t& outCacheMisses,
...
ThumbnailPipeline::ThumbnailPipeline() : m_impl(std::make_unique<Impl>()) {}
...
std::vector<ThumbnailResult> ThumbnailPipeline::GenerateThumbnailsBatch(const std::vector<ThumbnailRequest>& requests)
{
    std::vector<ThumbnailResult> results(requests.size());

    if (!m_impl->initialized || requests.empty())
        return results;
    ...
    while (remaining > 0) {
        ...
        for (size_t i = 0; i < batchSize; ++i) {
            ...
            futures.push_back(std::async(
                std::launch::async, [this, &requests, idx]() { return m_impl->GenerateThumbnail(requests[idx]); }));
        }
...
void ThumbnailPipeline::GetStatistics(uint64_t& totalRequests, uint64_t& cacheHits, uint64_t& cacheMisses,
