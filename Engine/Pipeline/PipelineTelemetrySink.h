...
public:
    explicit PipelineTelemetrySink(SinkTelemetryLevel level = SinkTelemetryLevel::Standard)
        : m_level(level) {
    }

    void RecordEvent(const SinkTelemetryEvent& event) {
        if (m_level == SinkTelemetryLevel::None) return;
        std::lock_guard<std::mutex> lock(m_mutex);
        m_events.push_back(event);
        m_totalDurationMs += event.durationMs;
        if (event.success) m_successCount++;
        else m_failureCount++;
        if (event.durationMs > m_maxDurationMs) m_maxDurationMs = event.durationMs;
    }
    ...
    std::vector<SinkTelemetryEvent> GetEvents(size_t maxCount = 100) const {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (m_events.size() <= maxCount) return m_events;
        return std::vector<SinkTelemetryEvent>(
            m_events.end() - static_cast<ptrdiff_t>(maxCount),
            m_events.end());
    }

    void SetLevel(SinkTelemetryLevel level) { m_level = level; }
    SinkTelemetryLevel GetLevel() const { return m_level; }
