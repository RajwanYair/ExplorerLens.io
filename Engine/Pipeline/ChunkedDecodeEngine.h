...
public:
    ...
    void Configure(const ChunkedDecodeConfig& config) { m_config = config; }

    uint32_t CalculateChunkCount(uint32_t imageHeight) const {
        if (m_config.chunkHeight == 0) return 1;
        return (imageHeight + m_config.chunkHeight - 1) / m_config.chunkHeight;
    }

    ChunkDescriptor GetChunk(uint32_t index, uint32_t imageWidth,
        uint32_t imageHeight, uint32_t bpp,
        const uint8_t* fullData) const {
        ChunkDescriptor cd;
        cd.offsetY = index * m_config.chunkHeight;
        cd.height = (cd.offsetY + m_config.chunkHeight > imageHeight)
            ? (imageHeight - cd.offsetY)
            : m_config.chunkHeight;
    ...
    bool FitsInBudget(uint32_t imageWidth, uint32_t bpp) const {
        uint64_t chunkBytes = static_cast<uint64_t>(imageWidth) * (bpp / 8) *
            m_config.chunkHeight * m_config.maxPendingChunks;
        return chunkBytes <= m_config.memoryBudgetBytes;
    }

    ChunkedDecodeStats GetStats() const { return m_stats; }
