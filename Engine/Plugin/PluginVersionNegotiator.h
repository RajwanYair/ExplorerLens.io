...
struct SemanticVersion
{
    ...
    constexpr SemanticVersion(uint32_t maj, uint32_t min, uint32_t pat) : major(maj), minor(min), patch(pat) {}
...
inline const wchar_t* CompatibilityResultName(CompatibilityResult r)
{
    switch (r) {
        ...
        case CompatibilityResult::MinorMismatch:
            return L"MinorMismatch";
        case CompatibilityResult::MajorBreaking:
            return L"MajorBreaking";
...
class PluginVersionNegotiator
{
  public:
    ...
    SemanticVersion GetSDKVersion() const
    {
        return SDK_VERSION;
    }
    ...
    CompatibilityResult CheckCompatibility(const SemanticVersion& pluginVersion, const SemanticVersion& sdkVersion) const
    {
    ...
    std::vector<MigrationStep> GetMigrationPath(const SemanticVersion& fromVer, const SemanticVersion& toVer) const
    {
    ...
    std::wstring GetUpgradeSuggestion(const SemanticVersion& pluginVersion) const
    {
        ...
        switch (compat) {
            ...
            case CompatibilityResult::MinorMismatch:
                return L"Minor version mismatch. Plugin may work but upgrade recommended to " + SDK_VERSION.ToString()
                       + L".";
            case CompatibilityResult::MajorBreaking:
                return L"Major version mismatch. Plugin must be recompiled for SDK " + SDK_VERSION.ToString() + L".";
            case CompatibilityResult::Unknown:
            default:
    ...
    const std::vector<MigrationStep>& GetMigrationSteps() const
    {
        return m_migrationSteps;
    }
  ...
  private:
    ...
    {
        m_migrationSteps.push_back({SemanticVersion{13, 0, 0}, SemanticVersion{14, 0, 0},
                                    L"v13 to v14: Plugin API restructured, ICADDecoderPlugin added.", true});
        m_migrationSteps.push_back({SemanticVersion{14, 0, 0}, SemanticVersion{15, 0, 0},
