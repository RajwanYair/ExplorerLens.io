...
class PluginNamedPipeBridge
{
  public:
    ...
    inline bool StartServer(const std::wstring& pluginId)
    {
        ...
        m_hPipe = CreateNamedPipeW(m_pipeName.c_str(), PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
    ...
    {
        if (!m_serverRunning.load() || !m_hPipe)
            return false;
    ...
    {
        return SendRaw(static_cast<uint32_t>(PipeMessageType::Response), data.data(),
                       static_cast<uint32_t>(data.size()));
    }

    /// Send a typed message with message type.
    inline bool SendTypedMessage(PipeMessageType type, const std::vector<uint8_t>& data)
    {
        return SendRaw(static_cast<uint32_t>(type), data.data(), static_cast<uint32_t>(data.size()));
    }

    /// Receive a length-prefixed message (blocking up to timeoutMs).
    inline bool ReceiveMessage(std::vector<uint8_t>& outData, uint32_t timeoutMs = 5000)
    {
        uint32_t msgType = 0;
    ...
    inline bool ReceiveTypedMessage(uint32_t& outMsgType, std::vector<uint8_t>& outData, uint32_t timeoutMs = 5000)
    {
    ...
    inline void SetMessageCallback(std::function<void(uint32_t msgType, const std::vector<uint8_t>& payload)> fn)
    {
  ...
  private:
    ...
    inline bool ReceiveRaw(uint32_t& outMsgType, std::vector<uint8_t>& outData, uint32_t timeoutMs)
    {
        if (!m_hPipe || !m_clientConnected.load())
            return false;
    ...
    static inline bool BuildCurrentUserDACL(SECURITY_ATTRIBUTES* sa, PSECURITY_DESCRIPTOR* ppSD)
    {
        ...
        BOOL ok = ConvertStringSecurityDescriptorToSecurityDescriptorW(sddl.c_str(), SDDL_REVISION_1, ppSD, nullptr);
        if (!ok)
            return false;
