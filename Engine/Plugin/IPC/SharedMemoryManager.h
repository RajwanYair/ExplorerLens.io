...
class SharedMemoryManager
{
  public:
    ...
    static std::unique_ptr<SharedMemorySection> CreateForFile(uint64_t correlation_id, const void* file_data,
                                                              uint64_t file_size);

    // Open shared memory for file data (client side)
    static std::unique_ptr<SharedMemorySection> OpenForFile(const std::string& name, uint64_t expected_size);

    // Create shared memory for bitmap data (server side)
    static std::unique_ptr<SharedMemorySection> CreateForBitmap(uint64_t correlation_id, const void* bitmap_data,
                                                                uint64_t bitmap_size);

    // Open shared memory for bitmap data (client side)
    static std::unique_ptr<SharedMemorySection> OpenForBitmap(const std::string& name, uint64_t expected_size);
