...
inline const wchar_t* PluginAuditSeverityName(PluginAuditSeverity s) {
    switch (s) {
        case PluginAuditSeverity::Info:          return L"Info";
        case PluginAuditSeverity::Warning:       return L"Warning";
        case PluginAuditSeverity::SecurityEvent: return L"SecurityEvent";
        case PluginAuditSeverity::Violation:     return L"Violation";
        case PluginAuditSeverity::Critical:      return L"Critical";
...
public:
    ...
    void Append(PluginAuditSeverity severity,
    ...
    std::vector<PluginAuditEntry> QueryByPluginId(const std::wstring& pluginId) const {
        ...
        std::vector<PluginAuditEntry> filtered;
        for (auto& e : ordered) {
            if (e.pluginId == pluginId) filtered.push_back(e);
        }
        return filtered;
    }

    // Query entries by time range
    std::vector<PluginAuditEntry> QueryByTimeRange(
