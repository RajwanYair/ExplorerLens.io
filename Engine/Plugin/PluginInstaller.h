...
class PluginInstaller
{
  public:
    ...
    PluginInstallResult Install(const std::string& pkgPath, InstallProgressCallback progress = nullptr);

    // Update an already-installed plugin (installs new over old; keeps config).
    PluginInstallResult Update(const std::string& pluginId, const std::string& newPkgPath,
