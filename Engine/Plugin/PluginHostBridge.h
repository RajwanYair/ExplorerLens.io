...
class PluginHostBridge
{
  public:
    ...
    PluginHostState GetState() const
    {
        return m_state.load();
    }
    ...
    /// Get crash history
    const PluginCrashReport& GetLastCrashReport() const
    {
        return m_lastCrash;
    }
    ...
    bool RequestDecode(const wchar_t* pluginName, const wchar_t* filePath, uint32_t cx, uint32_t cy)
    {
        if (m_state != PluginHostState::Running)
            return false;
    ...
    {
        switch (s) {
            case PluginHostState::NotStarted:
                return "NotStarted";
            case PluginHostState::Starting:
                return "Starting";
            case PluginHostState::Running:
                return "Running";
            case PluginHostState::Suspended:
                return "Suspended";
            case PluginHostState::Crashed:
                return "Crashed";
            case PluginHostState::ShuttingDown:
                return "ShuttingDown";
            case PluginHostState::Stopped:
                return "Stopped";
            default:
                return "Unknown";
        }
    ...
    static constexpr uint32_t GetStateCount()
    {
        return 7;
    }
  ...
  private:
    ...
    bool LaunchProcess()
    {
        ...
        std::wstring cmdLine = m_config.executablePath + L" --pipe=" + m_config.pipeName;

        if (!CreateProcessW(nullptr, cmdLine.data(), nullptr, nullptr, FALSE, CREATE_SUSPENDED, nullptr, nullptr, &si,
                            &pi)) {
            return false;
        }
