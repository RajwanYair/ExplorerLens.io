...
//------------------------------------------------------------------------------
PluginSemVer PluginSemVer::Parse(const std::wstring& str)
{
    PluginSemVer v;
    ...
    try {
        v.minor = static_cast<uint16_t>(std::stoi(str.substr(pos1 + 1, pos2 - pos1 - 1)));
    } catch (...) {}
    size_t pos3 = str.find(L'-', pos2 + 1);
    if (pos3 != std::wstring::npos) {
        try {
            v.patch = static_cast<uint16_t>(std::stoi(str.substr(pos2 + 1, pos3 - pos2 - 1)));
        } catch (...) {}
...
//------------------------------------------------------------------------------
PluginMarketplaceV2::PluginMarketplaceV2() : m_catalogUrl(L"https://plugins.explorerlens.dev/v1") {}

PluginMarketplaceV2::PluginMarketplaceV2(const std::wstring& catalogUrl) : m_catalogUrl(catalogUrl) {}
...
std::vector<PluginListing> PluginMarketplaceV2::CheckUpdates() const
{
    ...
    for (const auto& installed : m_installed) {
        for (const auto& catalog : m_catalog) {
            if (catalog.id == installed.listing.id && catalog.version >= installed.installedVersion
...
//------------------------------------------------------------------------------
bool PluginMarketplaceV2::VerifyHash(const std::wstring& filePath, const std::wstring& expectedHash)
{
    ...
    status = BCryptHashData(hHash, fileData.data(), static_cast<ULONG>(fileData.size()), 0);
    if (status != 0) {
