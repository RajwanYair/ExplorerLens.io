...
class PluginHandle
{
  public:
    ...
    // Decode operations
    PluginErrorCode Decode(const DecodeRequest* request, DecodeResult* result,
                           PluginProgressCallback progress = nullptr);

    void FreeResult(DecodeResult* result);

    // Optional operations
    PluginErrorCode GetMetadata(const std::filesystem::path& file_path, ImageMetadata* metadata);

    PluginErrorCode GetThumbnail(const std::filesystem::path& file_path, DecodeResult* result);
...
class PluginManager
{
  public:
    ...
    PluginErrorCode DecodeFile(const std::filesystem::path& file_path, uint32_t target_width, uint32_t target_height,
                               DecodeResult* result, PluginProgressCallback progress = nullptr);

    // Create IThumbnailDecoder wrapper for a plugin with automatic mode selection
    std::unique_ptr<Engine::PluginDecoder> CreateDecoderForPlugin(const std::string& plugin_name);

    // Create IThumbnailDecoder wrapper for best plugin for file
    std::unique_ptr<Engine::PluginDecoder> CreateDecoderForFile(const std::filesystem::path& file_path);
...
class ManagerPluginDiscovery
{
  public:
    ...
    static std::vector<std::filesystem::path> FindPlugins(const std::filesystem::path& directory,
...
struct PluginManifest
{
    ...
    static std::optional<PluginManifest> LoadFromFile(const std::filesystem::path& manifest_path);

    static std::optional<PluginManifest> LoadFromJSON(const std::string& json_data);
};
