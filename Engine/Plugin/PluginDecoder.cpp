...
PluginDecoder::PluginDecoder(PluginHandle* plugin_handle, const std::wstring& plugin_id)
...
PluginDecoder::PluginDecoder(const std::filesystem::path& plugin_path, const std::wstring& plugin_id)
...
HRESULT PluginDecoder::DecodeInWorker(const ThumbnailRequest& request, ThumbnailResult& result)
{
    if (!plugin_handle_ || !plugin_handle_->IsLoaded()) {
        return E_POINTER;
    }
    ...
    result.hBitmap = CreateHBITMAPFromPixels(plugin_result.pixels, plugin_result.width, plugin_result.height,
...
HRESULT PluginDecoder::DecodeInPluginHost(const ThumbnailRequest& request, ThumbnailResult& result)
{
    if (!host_client_) {
        return E_POINTER;
    }
    ...
    DecodeResult plugin_result = {};
    IPC::IPCErrorCode ipc_error = host_client_->RequestThumbnail(plugin_request, plugin_result);
...
HBITMAP PluginDecoder::CreateHBITMAPFromPixels(const uint8_t* pixels, uint32_t width, uint32_t height,
                                               PixelFormat format)
{
    ...
    if (hBitmap && bits) {
        ...
        if (format == PIXEL_FORMAT_RGBA32) {
            ...
            // Direct copy
            for (uint32_t y = 0; y < height; y++) {
                memcpy(dest + y * dest_stride, pixels + y * width * 4, width * 4);
            }
...
std::unique_ptr<PluginDecoder> PluginDecoderFactory::CreateDecoder(PluginHandle* plugin_handle,
                                                                   const std::wstring& plugin_id,
                                                                   const std::filesystem::path& plugin_path)
{
    // Use IsolationModeSelector to determine mode
    IsolationMode mode = IsolationModeSelector::Instance().DetermineMode(plugin_id, plugin_path);

    return CreateDecoderWithMode(plugin_handle, plugin_id, plugin_path, mode);
}

std::unique_ptr<PluginDecoder> PluginDecoderFactory::CreateDecoderWithMode(PluginHandle* plugin_handle,
                                                                           const std::wstring& plugin_id,
                                                                           const std::filesystem::path& plugin_path,
                                                                           IsolationMode mode)
{
