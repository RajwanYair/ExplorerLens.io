...
bool JobObjectManager::SetBasicLimits(const JobObjectLimits& limits) {
 ...
 basic_limits.PerProcessUserTimeLimit = MillisecondsToFileTime(
 limits.per_process_user_time_limit_ms);
 }
...
bool JobObjectManager::SetNotificationLimits(const JobObjectLimits& limits) {
 ...
 if (limits.process_memory_limit > 0) {
 notification_limits.JobMemoryLimit = static_cast<uint64_t>(
 limits.job_memory_limit * 0.9);
