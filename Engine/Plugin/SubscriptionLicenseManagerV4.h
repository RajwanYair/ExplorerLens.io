...
struct SeatAllocation {
    ...
    [[nodiscard]] bool HasCapacity() const noexcept { return Available() > 0; }
};
...
public:
    ...
    [[nodiscard]] bool CheckSeat(const std::string& pluginId,
                                 const std::string& userId) const;

    // Allocate a seat lease; returns nullopt if no capacity.
    [[nodiscard]] std::optional<SeatLease> AllocateSeat(const std::string& pluginId,
...
private:
    ...
    [[nodiscard]] bool VerifySignature(const JWTEntitlement& ent,
                                       const std::string& rawToken) const noexcept;
