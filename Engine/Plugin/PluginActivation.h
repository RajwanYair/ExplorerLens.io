...
public:
    ...
    size_t PluginCount() const { return scannedPlugins_.size(); }

    void Clear() { scannedPlugins_.clear(); }
...
public:
    ...
    IPCMessage Receive() {
        if (!sentMessages_.empty()) {
            ...
            IPCMessage response;
            response.type = (last.type == IPCMessageType::Ping)
                ? IPCMessageType::HealthCheck
                : IPCMessageType::DecodeResult;
