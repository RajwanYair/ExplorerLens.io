...
class PluginDecoder : public IThumbnailDecoder
{
  public:
    ...
    explicit PluginDecoder(PluginHandle* plugin_handle, const std::wstring& plugin_id);

    /// Constructor for PluginHost mode (isolated execution)
    /// @param plugin_path Path to plugin DLL
    /// @param plugin_id Unique identifier for the plugin
    PluginDecoder(const std::filesystem::path& plugin_path, const std::wstring& plugin_id);
    ...
    {
        return info_.supportedExtensions;
    }
    uint32_t GetExtensionCount() const override
    {
        return info_.extensionCount;
    }
    bool SupportsGPU() const override
    {
        return info_.supportsGPU;
    }
    bool IsArchiveDecoder() const override
    {
        return info_.isArchiveDecoder;
    }
    ...
    IsolationMode GetIsolationMode() const
    {
        return isolation_mode_;
    }

    /// Get plugin ID
    const std::wstring& GetPluginId() const
    {
        return plugin_id_;
    }
    ...
    {
        return isolation_mode_ == IsolationMode::PluginHost;
    }
  ...
  private:
    ...
    HBITMAP CreateHBITMAPFromPixels(const uint8_t* pixels, uint32_t width, uint32_t height, PixelFormat format);
...
class PluginDecoderFactory
{
  public:
    ...
    static std::unique_ptr<PluginDecoder> CreateDecoder(PluginHandle* plugin_handle, const std::wstring& plugin_id,
                                                        const std::filesystem::path& plugin_path);

    /// Create a decoder with explicit mode
    static std::unique_ptr<PluginDecoder> CreateDecoderWithMode(PluginHandle* plugin_handle,
