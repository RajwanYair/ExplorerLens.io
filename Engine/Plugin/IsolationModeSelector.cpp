...
void IsolationModeSelector::LoadConfiguration()
{
    ...
    HKEY key;
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Plugins", 0, KEY_READ, &key) != ERROR_SUCCESS) {
        return;
    }
    ...
    if (RegQueryValueExW(key, L"TrustedPlugins", nullptr, &type, reinterpret_cast<BYTE*>(buffer), &buffer_size)
            == ERROR_SUCCESS
        && type == REG_MULTI_SZ) {
        wchar_t* ptr = buffer;
        while (*ptr) {
    ...
    while (RegEnumValueW(key, index++, value_name, &value_name_size, nullptr, &type, nullptr, nullptr)
           == ERROR_SUCCESS) {
        if (type == REG_DWORD && wcsstr(value_name, L"AllowInWorker_") == value_name) {
            DWORD allow = 0;
            DWORD size = sizeof(allow);
            if (RegQueryValueExW(key, value_name, nullptr, nullptr, reinterpret_cast<BYTE*>(&allow), &size)
                == ERROR_SUCCESS) {
    ...
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Policies\\ExplorerLens\\Plugins", 0, KEY_READ, &key)
        == ERROR_SUCCESS) {
        // Minimum isolation mode
        DWORD min_mode = 1;  // Default: PluginHost
        DWORD size = sizeof(min_mode);
        if (RegQueryValueExW(key, L"MinIsolationMode", nullptr, nullptr, reinterpret_cast<BYTE*>(&min_mode), &size)
            == ERROR_SUCCESS) {
        ...
        if (RegQueryValueExW(key, L"AllowedPlugins", nullptr, &type, reinterpret_cast<BYTE*>(buffer), &buffer_size)
                == ERROR_SUCCESS
            && type == REG_MULTI_SZ) {
            wchar_t* ptr = buffer;
            while (*ptr) {
        ...
        buffer_size = sizeof(buffer);
        if (RegQueryValueExW(key, L"DeniedPlugins", nullptr, &type, reinterpret_cast<BYTE*>(buffer), &buffer_size)
                == ERROR_SUCCESS
            && type == REG_MULTI_SZ) {
            wchar_t* ptr = buffer;
            while (*ptr) {
...
void IsolationModeSelector::SaveConfiguration()
{
    ...
    HKEY key;
    if (RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Plugins", 0, nullptr, 0, KEY_WRITE, nullptr, &key,
                        nullptr)
        != ERROR_SUCCESS) {
        return;
    }
    ...
    RegSetValueExW(key, L"TrustedPlugins", 0, REG_MULTI_SZ, reinterpret_cast<const BYTE*>(multi_sz.c_str()),
    ...
    for (const auto& pair : user_preferences_) {
        ...
        RegSetValueExW(key, value_name.c_str(), 0, REG_DWORD, reinterpret_cast<const BYTE*>(&allow), sizeof(allow));
    }
...
bool IsTrustedVendor(const std::wstring& vendor)
{
    ...
    static const std::unordered_set<std::wstring> trusted_vendors = {
        L"Microsoft Corporation", L"Adobe Systems Incorporated", L"Google LLC", L"Apple Inc.",
...
std::wstring GetPluginPublisher(const std::filesystem::path& plugin_path)
{
    ...
    if (!CryptQueryObject(CERT_QUERY_OBJECT_FILE, plugin_path.c_str(), CERT_QUERY_CONTENT_FLAG_PKCS7_SIGNED_EMBED,
                          CERT_QUERY_FORMAT_FLAG_BINARY, 0, &encoding, &content_type, &format_type, &cert_store,
                          &crypt_msg, nullptr)) {
        return L"";
    }
    ...
    PCCERT_CONTEXT cert_context =
        CertFindCertificateInStore(cert_store, encoding, 0, CERT_FIND_SUBJECT_CERT, &cert_info, nullptr);

    std::wstring publisher;

    if (cert_context) {
        // Get subject name
        DWORD name_size = CertGetNameStringW(cert_context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, nullptr, 0);

        if (name_size > 0) {
            std::vector<wchar_t> name_buffer(name_size);
            CertGetNameStringW(cert_context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, name_buffer.data(), name_size);
