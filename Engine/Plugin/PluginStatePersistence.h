...
{
  public:
    explicit PluginStatePersistence(PersistenceConfig config = {}) : m_config(config) {}

    void SetState(const std::string& pluginId, const std::string& key, const std::string& value)
    {
    ...
    std::string GetState(const std::string& pluginId, const std::string& key, const std::string& defaultValue = "") const
    {
        auto pit = m_pluginStates.find(pluginId);
        if (pit == m_pluginStates.end())
            return defaultValue;
    ...
    void SetConfig(const PersistenceConfig& config)
    {
        m_config = config;
    }
    PersistenceConfig GetConfig() const
    {
        return m_config;
    }
    uint64_t GetSaveCount() const
    {
        return m_saveCount;
    }
  ...
  private:
    std::unordered_map<std::string, std::unordered_map<std::string, PluginStateEntry>> m_pluginStates;
