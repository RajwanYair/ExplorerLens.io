...
public:
    ...
    IsolationMode DetermineMode(const std::wstring& plugin_id,
...
inline const wchar_t* GetIsolationModeName(IsolationMode mode) {
    switch (mode) {
        case IsolationMode::InWorker: return L"In-Worker (Direct)";
        case IsolationMode::PluginHost: return L"PluginHost (Sandboxed)";
