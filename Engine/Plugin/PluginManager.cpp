...
PluginHandle::PluginHandle(const std::filesystem::path& path) : path_(path)
{
    LoadPlugin();
}
...
bool PluginHandle::LoadPlugin()
{
    ...
    get_info_ = reinterpret_cast<decltype(get_info_)>(GetProcAddress(static_cast<HMODULE>(module_), "plugin_get_info"));
    init_ = reinterpret_cast<decltype(init_)>(GetProcAddress(static_cast<HMODULE>(module_), "plugin_init"));
    cleanup_ = reinterpret_cast<decltype(cleanup_)>(GetProcAddress(static_cast<HMODULE>(module_), "plugin_cleanup"));
    can_decode_ =
        reinterpret_cast<decltype(can_decode_)>(GetProcAddress(static_cast<HMODULE>(module_), "plugin_can_decode"));
    decode_ = reinterpret_cast<decltype(decode_)>(GetProcAddress(static_cast<HMODULE>(module_), "plugin_decode"));
    free_result_ =
        reinterpret_cast<decltype(free_result_)>(GetProcAddress(static_cast<HMODULE>(module_), "plugin_free_result"));

    // Optional function pointers
    get_metadata_ =
        reinterpret_cast<decltype(get_metadata_)>(GetProcAddress(static_cast<HMODULE>(module_), "plugin_get_metadata"));
...
PluginErrorCode PluginHandle::Decode(const DecodeRequest* request, DecodeResult* result, PluginProgressCallback progress)
{
...
PluginErrorCode PluginHandle::GetMetadata(const std::filesystem::path& file_path, ImageMetadata* metadata)
{
...
PluginErrorCode PluginHandle::GetThumbnail(const std::filesystem::path& file_path, DecodeResult* result)
{
...
PluginErrorCode PluginManager::DecodeFile(const std::filesystem::path& file_path, uint32_t target_width,
                                          uint32_t target_height, DecodeResult* result, PluginProgressCallback progress)
{
    if (!result) {
        return PLUGIN_ERROR_INVALID_PARAMETER;
    }
...
std::unique_ptr<Engine::PluginDecoder> PluginManager::CreateDecoderForPlugin(const std::string& plugin_name)
{
    // Get plugin handle
    ...
    return Engine::PluginDecoderFactory::CreateDecoder(plugin_handle, plugin_id, plugin_path);
}

std::unique_ptr<Engine::PluginDecoder> PluginManager::CreateDecoderForFile(const std::filesystem::path& file_path)
{
    // Find suitable plugin
...
std::vector<std::filesystem::path> ManagerPluginDiscovery::FindPlugins(const std::filesystem::path& directory,
                                                                       bool recursive)
{
...
std::optional<PluginManifest> PluginManifest::LoadFromFile(const std::filesystem::path& manifest_path)
{
