...
{
    return static_cast<PackPluginCapability>(static_cast<uint32_t>(a) | static_cast<uint32_t>(b));
}
...
{
    return {"com.explorerlens.ref.minimal",
...
struct EmbeddedMetadata
{
    ...
    bool HasGPS() const
    {
        return latitudeDeg.has_value() && longitudeDeg.has_value();
    }
...
{
    return {"com.explorerlens.ref.watermark",
            "Watermark Overlay",
            ...
            {".jpg", ".jpeg", ".png", ".webp", ".tiff", ".bmp", ".heic", ".avif", ".jxl", ".gif", ".dng"},
...
struct ReferencePluginPack
{
    ...
    {
        if (minimalGenerator.id == id)
            return &minimalGenerator;
        if (metadataReader.id == id)
            return &metadataReader;
        if (watermarkOverlay.id == id)
