...
bool PluginHostClient::StartPluginHost(const std::wstring& plugin_path, uint32_t timeout_ms)
{
    ...
    oss << L"ExplorerLens-PluginHost-" << GetCurrentProcessId() << L"-" << std::hex << std::setw(16)
        << std::setfill(L'0') << GenerateCorrelationId();
    pipe_name_ = oss.str();
...
IPC::IPCErrorCode PluginHostClient::RequestThumbnail(const DecodeRequest& request, DecodeResult& result)
{
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    IPC::IPCThumbnailRequest ipc_request;
    ...
    memcpy(payload.data() + sizeof(ipc_request), file_path_wide.c_str(), ipc_request.filePathLength * sizeof(wchar_t));
    ...
    if (ipc_response->bitmapDataSize > 0
        && ipc_response->resultCode == static_cast<uint32_t>(IPC::IPCErrorCode::SUCCESS)) {
...
bool PluginHostClient::CanDecode(const std::wstring& file_path)
{
    ...
    memcpy(payload.data() + sizeof(query), file_path.c_str(), query.filePathLength * sizeof(wchar_t));
...
bool PluginHostClient::HasCrashed() const
{
    ...
    if (GetExitCodeProcess(process_handle_, &exit_code)) {
        if (exit_code != STILL_ACTIVE && exit_code != 0) {
            ...
            return exit_code == STATUS_ACCESS_VIOLATION || exit_code == STATUS_STACK_OVERFLOW
                   || exit_code == STATUS_INTEGER_DIVIDE_BY_ZERO || exit_code == STATUS_ILLEGAL_INSTRUCTION;
        }
    }
...
bool PluginHostClient::SpawnPluginHostProcess()
{
    ...
    cmd_line << L"\"" << plugin_host_path << L"\" " << L"--plugin \"" << plugin_path_ << L"\" " << L"--pipe \""
             << pipe_name_ << L"\" " << L"--timeout " << timeout_ms_;
    ...
    if (!CreateProcessW(plugin_host_path.c_str(), &cmd_line_str[0], nullptr, nullptr, FALSE,
...
bool PluginHostClient::ConnectToPipe()
{
    ...
    for (int i = 0; i < 10; ++i) {
        pipe_handle_ =
            CreateFileW(full_pipe_name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
...
bool PluginHostClient::SendMessage(const IPC::MessageHeader& header, const void* payload, uint32_t size)
{
    if (pipe_handle_ == INVALID_HANDLE_VALUE) {
        return false;
    }
...
bool PluginHostClient::ReceiveMessage(IPC::MessageHeader& header, std::vector<uint8_t>& payload, uint32_t timeout_ms)
{
    if (pipe_handle_ == INVALID_HANDLE_VALUE) {
        return false;
    }
