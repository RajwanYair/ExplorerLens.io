...
class PluginHostClient
{
  public:
    ...
    bool StartPluginHost(const std::wstring& plugin_path, uint32_t timeout_ms = IPC::DEFAULT_REQUEST_TIMEOUT);

    // Send decode request
    IPC::IPCErrorCode RequestThumbnail(const DecodeRequest& request, DecodeResult& result);
  ...
  private:
    ...
    // IPC
    bool SendMessage(const IPC::MessageHeader& header, const void* payload, uint32_t size);
    bool ReceiveMessage(IPC::MessageHeader& header, std::vector<uint8_t>& payload, uint32_t timeout_ms);
