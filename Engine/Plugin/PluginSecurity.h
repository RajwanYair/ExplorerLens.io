...
struct SandboxPolicySpec {
    ...
    static SandboxPolicySpec Strict()    { SandboxPolicySpec s; s.preset = SandboxPolicyPreset::Strict; s.limits = {128ULL*1024*1024, 10, 128, true}; return s; }
    static SandboxPolicySpec Standard()  { SandboxPolicySpec s; s.preset = SandboxPolicyPreset::Standard; s.limits = {256ULL*1024*1024, 25, 256, true}; return s; }
    static SandboxPolicySpec Developer() { SandboxPolicySpec s; s.preset = SandboxPolicyPreset::Developer; s.limits = {512ULL*1024*1024, 50, 512, true}; return s; }
};

enum class TeardownReason : uint8_t { NormalExit, TimeoutKill, CrashTerminate };
...
inline std::string ToString(TeardownReason r) {
    switch (r) {
        ...
        case TeardownReason::CrashTerminate:  return "CrashTerminate";
    }
    return "Unknown";
}
...
struct SandboxTeardownResult {
    ...
    int32_t exitCode = 0;
    bool WasClean() const noexcept { return reason == TeardownReason::NormalExit && exitCode == 0; }
};

struct HandleLeakReport {
    uint32_t leaked = 0;
    bool HasLeak() const noexcept { return leaked > 0; }
    std::string Summary() const { return "leaked=" + std::to_string(leaked); }
};

class SandboxPolicyValidator {
public:
    explicit SandboxPolicyValidator(const SandboxPolicySpec& policy) : m_policy(policy) {}
    bool IsValid() const noexcept { return m_policy.limits.maxMemoryBytes > 0 && m_policy.limits.maxCPUPercent <= 100; }
private:
...
public:
    static PluginRuntimeValidator Create() { return {}; }
    uint32_t InvalidTransitionCount() const noexcept { return 0; }
    ...
    bool IsValidTransition(StateEnum from, StateEnum to) const noexcept {
        ...
        if (t == 7) return true;  // -> Faulted is always valid
        if (t == f + 1) return true;
        if (f == 6 && t == 0) return true;  // Stopping -> Unloaded
        return false;
    }
};
