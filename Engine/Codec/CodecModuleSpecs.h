...
inline std::string GenerateCodecManifest() {
    ...
    for (size_t i = 0; i < specs.size(); i++) {
        ...
        // Convert wchar_t to narrow for JSON (ASCII-safe codec names)
        std::string dll;
        dll.reserve(wcslen(s.dllName));
        for (const wchar_t* p = s.dllName; *p; ++p)
            dll.push_back(static_cast<char>(*p & 0x7F));
        json += " \"dll\": \"" + dll + "\",\n";

        json += " \"extensions\": [";
        for (size_t j = 0; j < s.extensions.size(); j++) {
            std::string ext;
            ext.reserve(s.extensions[j].size());
        ...
        for (size_t j = 0; j < s.dependencies.size(); j++) {
            std::string dep;
            dep.reserve(s.dependencies[j].size());
