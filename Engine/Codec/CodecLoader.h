...
struct CodecHandle
{
    ...
    bool IsLoaded() const
    {
        return state == CodecState::Ready && hModule != nullptr;
    }
    ...
    void TouchTimestamp()
    {
        lastUseTimestamp.store(static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
...
class CodecLoader
{
  public:
    ...
    //--------------------------------------------------------------------------
    explicit CodecLoader(const CodecLoaderConfig& config = {}) : m_config(config) {}
    ...
    {
        auto handle = GetHandle(codecId);
        if (!handle || !handle->IsLoaded() || !handle->pfnGetHealth)
            return false;
    ...
    CodecLoaderStats GetStats() const
    {
        return m_stats;
    }
  ...
  private:
    ...
    uint32_t LoadCodecIfNeeded(CodecHandle* handle)
    {
        ...
        if (handle->state == CodecState::Error)
        ...
        handle->pfnInitialize =
            reinterpret_cast<PFN_DtCodec_Initialize>(::GetProcAddress(handle->hModule, DTCODEC_FN_INITIALIZE));
        handle->pfnGetModuleInfo =
            reinterpret_cast<PFN_DtCodec_GetModuleInfo>(::GetProcAddress(handle->hModule, DTCODEC_FN_GETMODULEINFO));
        ...
        handle->pfnFreeResult =
            reinterpret_cast<PFN_DtCodec_FreeResult>(::GetProcAddress(handle->hModule, DTCODEC_FN_FREERESULT));
        handle->pfnShutdown =
            reinterpret_cast<PFN_DtCodec_Shutdown>(::GetProcAddress(handle->hModule, DTCODEC_FN_SHUTDOWN));

        // Optional health
        handle->pfnGetHealth =
            reinterpret_cast<PFN_DtCodec_GetHealth>(::GetProcAddress(handle->hModule, DTCODEC_FN_GETHEALTH));

        // Check mandatory
        if (!handle->pfnInitialize || !handle->pfnGetModuleInfo || !handle->pfnDecodeThumbnail || !handle->pfnFreeResult
            || !handle->pfnShutdown) {
    ...
    {
        if (GetCurrentMemoryEstimate() <= m_config.memoryBudgetBytes)
            return;
        ...
        std::sort(candidates.begin(), candidates.end(),
                  [](const Candidate& a, const Candidate& b) { return a.idleMs > b.idleMs; });
    ...
    std::string ExtractJsonString(const std::string& json, size_t searchFrom, const std::string& key)
    {
    ...
    std::string ExtractJsonNumber(const std::string& json, size_t searchFrom, const std::string& key)
    {
        std::string search = "\"" + key + "\"";
        size_t keyPos = json.find(search, searchFrom);
        if (keyPos == std::string::npos)
    ...
    void RegisterBuiltInExtensions()
    {
        ...
        static const wchar_t* wicExts[] = {L".jpg", L".jpeg", L".jpe",  L".jfif", L".png", L".bmp", L".dib",
                                           L".gif", L".tif",  L".tiff", L".ico",  L".cur",  // ICODecoder (WIC-based)
