...
class PredictiveCacheEngine
{
  public:
    ...
    explicit PredictiveCacheEngine(const PredictionConfig& config) : m_config(config)
    {
        ::InitializeSRWLock(&m_srwLock);
    }
    ...
    {
        if (m_decayRunning.exchange(true))
            return;
    ...
    std::vector<CachePrediction> PredictNext(const std::wstring& currentDir)
    {
        ...
        std::sort(predictions.begin(), predictions.end(),
                  [](const CachePrediction& a, const CachePrediction& b) { return a.confidence > b.confidence; });
    ...
    const PredictionConfig& GetConfig() const
    {
        return m_config;
    }
  ...
  private:
    ...
    float ComputeProbabilityLocked(const std::wstring& path) const
    {
        ...
        for (const auto& [p, e] : m_accessTable) {
            if (e.decayedFrequency > maxFreq)
                maxFreq = e.decayedFrequency;
        }
        float freqScore = entry.decayedFrequency / maxFreq;

        // Combined score with configurable weights
        float combined =
            recencyScore * m_config.recencyWeight + freqScore * m_config.frequencyWeight + entry.directoryBoost;
        ...
        for (const auto& [dir, count] : dirCounts) {
            if (count >= m_config.directoryPatternWindow) {
                for (auto& [path, entry] : m_accessTable) {
                    ...
                    std::wstring pathDir = (pathLastSlash != std::wstring::npos) ? path.substr(0, pathLastSlash) : path;
                    if (pathDir == dir) {
    ...
    float ComputeProfilePredictionLocked(const DirectoryProfile& profile, const std::wstring& currentDir,
                                         uint64_t nowMs) const
    {
        ...
        for (const auto& [p, prof] : m_profiles) {
            if (prof.accessFrequency > maxFreq)
                maxFreq = prof.accessFrequency;
        }
    ...
    uint32_t EstimateFileCountLocked(const DirectoryProfile& profile) const
    {
        for (auto it = m_history.rbegin(); it != m_history.rend(); ++it) {
            if (it->directoryPath == profile.path)
                return it->fileCount;
        }
        return 50;
    }
