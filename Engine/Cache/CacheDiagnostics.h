...
{
    static const wchar_t* names[] = {L"Excellent", L"Good", L"Fair", L"Poor", L"Critical"};
...
class CacheDiagnostics
{
  public:
    ...
    {
        return (m_totalMemory > 0) ? (static_cast<double>(m_usedMemory.load(std::memory_order_relaxed))
                                      / static_cast<double>(m_totalMemory))
                                   : 0.0;
    }
    ...
    CacheDiagnosticReport GenerateReport() const
    {
        ...
        report.overallHitRate =
            (totalAccess > 0) ? (static_cast<double>(report.totalHits) / static_cast<double>(totalAccess)) : 0.0;
