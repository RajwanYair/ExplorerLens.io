...
class CacheKeyGenerator
{
  public:
    ...
    /// </summary>
    static std::wstring GenerateWithTime(const wchar_t* filePath, uint32_t width, uint32_t height, FILETIME modTime)
    {
        if (!filePath)
            return L"";

        uint64_t hash = HashFNV1a(filePath);
        uint64_t timeStamp = (static_cast<uint64_t>(modTime.dwHighDateTime) << 32) | modTime.dwLowDateTime;
