...
class PersistentDiskCache
{
  public:
    ...
    bool Put(const std::wstring& filePath, uint32_t width, uint32_t height, const uint8_t* data, uint32_t dataSize,
             double decodeCostMs, const std::wstring& formatName);

    /// Retrieve a cached thumbnail
    bool Get(const std::wstring& filePath, uint32_t& width, uint32_t& height, std::vector<uint8_t>& data);
    ...
    /// Get configuration
    const DiskCacheConfig& GetConfig() const
    {
        return m_config;
    }
    ...
    static std::wstring GenerateCacheKey(const std::wstring& filePath, uint32_t thumbnailSize);
...
{
    switch (p) {
        case PersistenceEvictionPolicy::LRU:
            return "LRU";
        case PersistenceEvictionPolicy::SizeBased:
            return "SizeBased";
        case PersistenceEvictionPolicy::TimeBased:
            return "TimeBased";
        case PersistenceEvictionPolicy::AccessCount:
            return "AccessCount";
        default:
            return "Unknown";
    }
...
class ThumbnailPersistenceLayer
{
  public:
    ...
    {
        if (!m_initialized || !data || dataSize == 0)
            return false;
