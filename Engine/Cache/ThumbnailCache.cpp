...
std::wstring ThumbnailCache::GenerateKey(const wchar_t* filePath, uint32_t width, uint32_t height) const
{
    ...
    std::wstringstream ss;
    ss << filePath << L"|" << metadata.fileSize << L"|" << metadata.lastModified << L"|" << width << L"x" << height;
...
HRESULT ThumbnailCache::OptimizeCache()
{
    ...
    wchar_t msg[256];
    swprintf_s(msg, L"Cache optimized: %u entries, saved %llu KB", optimizedCount, savedBytes / 1024);
...
HRESULT ThumbnailCache::DefragmentCache()
{
    ...
    wchar_t msg[256];
    swprintf_s(msg, L"Cache defragmented: %zu orphaned entries removed, %u MB tracked", removedCount,
               static_cast<uint32_t>(m_currentSizeBytes / (1024 * 1024)));
    OutputDebugStringW(msg);
