...
inline std::string ToString(MemoryPressureTier t) {
    switch (t) {
    case MemoryPressureTier::None:     return "None";
    case MemoryPressureTier::Low:      return "Low";
    case MemoryPressureTier::Medium:   return "Medium";
    case MemoryPressureTier::High:     return "High";
    case MemoryPressureTier::Critical: return "Critical";
    default:                           return "Unknown";
    }
}
...
struct SystemMemorySnapshot {
    ...
    MemoryPressureTier PressureTier() const {
        if (totalPhysicalBytes == 0) return MemoryPressureTier::None;
        double usagePercent = 100.0 * (1.0 - static_cast<double>(availableBytes) / static_cast<double>(totalPhysicalBytes));
        if (usagePercent < 50.0) return MemoryPressureTier::None;
        if (usagePercent < 70.0) return MemoryPressureTier::Low;
        if (usagePercent < 85.0) return MemoryPressureTier::Medium;
        if (usagePercent < 95.0) return MemoryPressureTier::High;
...
public:
    ...
    void EnableAutoUpdate(uint32_t intervalMs = 5000) {
        if (m_autoUpdateRunning.exchange(true)) return;
    ...
    size_t TotalBudget() const { return m_baseBudget; }

private:
    void RecalculateBudgetLocked() {
        double shrinkFactor = 1.0;
        switch (m_currentTier) {
        case MemoryPressureTier::None:     shrinkFactor = 1.0;  break;
        case MemoryPressureTier::Low:      shrinkFactor = 0.75; break; // shrink by 25%
        case MemoryPressureTier::Medium:   shrinkFactor = 0.50; break; // shrink by 50%
        case MemoryPressureTier::High:     shrinkFactor = 0.25; break; // shrink by 75%
        case MemoryPressureTier::Critical: shrinkFactor = 0.10; break; // shrink by 90%
        }
