...
{
  public:
    void SetAlgorithm(CacheCompressionAlgo algo)
    {
        m_algo = algo;
    }
    CacheCompressionAlgo GetAlgorithm() const
    {
        return m_algo;
    }

    void SetMinSizeForCompression(uint32_t bytes)
    {
        m_minSize = bytes;
    }
    uint32_t MinSizeForCompression() const
    {
        return m_minSize;
    }
    ...
    {
        switch (m_algo) {
            case CacheCompressionAlgo::LZ4Fast:
                return inputSize * 60 / 100;
            case CacheCompressionAlgo::LZ4HC:
                return inputSize * 50 / 100;
            case CacheCompressionAlgo::Zstd1:
                return inputSize * 45 / 100;
            case CacheCompressionAlgo::Zstd3:
                return inputSize * 35 / 100;
            default:
                return inputSize;
        }
    }
    ...
    void RecordCompression(uint32_t inSize, uint32_t outSize, double ms)
    {
        ...
        m_stats.avgCompressMs =
            (m_stats.avgCompressMs * (m_stats.entriesCompressed - 1) + ms) / m_stats.entriesCompressed;
        m_stats.ratio = (m_stats.uncompressedBytes > 0) ? static_cast<float>(m_stats.compressedBytes)
                                                              / static_cast<float>(m_stats.uncompressedBytes)
                                                        : 1.0f;
    }

    const CompressionStats& Stats() const
    {
        return m_stats;
    }
    ...
    {
        switch (a) {
            case CacheCompressionAlgo::None:
                return L"None";
            case CacheCompressionAlgo::LZ4Fast:
                return L"LZ4Fast";
            case CacheCompressionAlgo::LZ4HC:
    ...
    static size_t AlgorithmCount()
    {
        return static_cast<size_t>(CacheCompressionAlgo::COUNT);
    }
