...
public:
    void SetAlgorithm(CacheCompression algo) { m_algo = algo; }
    CacheCompression GetAlgorithm() const { return m_algo; }
    ...
    bool ShouldCompress(uint32_t entrySize) const {
        return m_algo != CacheCompression::None && entrySize >= m_minSize;
    }

    uint32_t EstimateCompressedSize(uint32_t inputSize) const {
        switch (m_algo) {
        case CacheCompression::LZ4Fast: return inputSize * 60 / 100;
        case CacheCompression::LZ4HC:   return inputSize * 50 / 100;
        case CacheCompression::Zstd1:   return inputSize * 45 / 100;
        case CacheCompression::Zstd3:   return inputSize * 35 / 100;
    ...
    static const wchar_t* AlgorithmName(CacheCompression a) {
        switch (a) {
        case CacheCompression::None:    return L"None";
        case CacheCompression::LZ4Fast: return L"LZ4Fast";
        case CacheCompression::LZ4HC:   return L"LZ4HC";
        case CacheCompression::Zstd1:   return L"Zstd1";
        case CacheCompression::Zstd3:   return L"Zstd3";
        default: return L"Unknown";
        }
    }
    static size_t AlgorithmCount() { return static_cast<size_t>(CacheCompression::COUNT); }

private:
    CacheCompression m_algo = CacheCompression::LZ4Fast;
