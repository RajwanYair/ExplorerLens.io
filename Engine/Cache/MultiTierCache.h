...
class BloomFilter
{
  public:
    ...
    explicit BloomFilter(uint32_t expectedElements = 100000, double falsePositiveRate = 0.01);
    ...
    /// Stats
    uint32_t GetInsertedCount() const
    {
...
inline BloomFilter::BloomFilter(uint32_t expectedElements, double falsePositiveRate) : m_insertedCount(0)
{
    // Ensure sane bounds
    ...
    if (m_hashCount > 20)
        m_hashCount = 20;  // Cap at 20 hash functions
...
inline bool BloomFilter::Deserialize(const std::vector<uint8_t>& buffer)
{
    ...
    if (buffer.size() < 12 + expectedBytes)
        return false;
...
{
    if (m_insertedCount == 0 || m_bitCount == 0)
        return 0.0;
    // FPR ≈ (1 - e^(-k*n/m))^k
...
class ICacheTier
{
  public:
    ...
    virtual HRESULT Get(const std::wstring& cacheKey, std::vector<uint8_t>& outData) = 0;

    /// Store bitmap data
    virtual HRESULT Put(const std::wstring& cacheKey, const std::vector<uint8_t>& data) = 0;
...
class MemoryCacheTier : public ICacheTier
{
  public:
    ...
    StorageTier GetTier() const override
    {
...
class SQLiteCacheTier : public ICacheTier
{
  public:
    ...
    explicit SQLiteCacheTier(const std::wstring& dbPath = L"", uint64_t maxSizeBytes = 2ULL * 1024 * 1024 * 1024);
    ~SQLiteCacheTier() override;

    StorageTier GetTier() const override
    {
        return StorageTier::SQLite;
    }
    const wchar_t* GetName() const override
    {
        return L"SQLite WAL";
...
{
  public:
    explicit DiskCacheTier(const std::wstring& cacheDir = L"", uint64_t maxSizeBytes = 4ULL * 1024 * 1024 * 1024);
...
class MultiTierCacheManager
{
  ...
  private:
    ...
    void PromoteToTier(size_t targetTierIndex, const std::wstring& key, const std::vector<uint8_t>& data);
};
...
inline HRESULT MemoryCacheTier::Get(const std::wstring& cacheKey, std::vector<uint8_t>& outData)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
inline HRESULT MemoryCacheTier::Put(const std::wstring& cacheKey, const std::vector<uint8_t>& data)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
inline SQLiteCacheTier::~SQLiteCacheTier()
{
    Close();
}
...
inline HRESULT SQLiteCacheTier::Get(const std::wstring& /*cacheKey*/, std::vector<uint8_t>& /*outData*/)
{
    ++m_stats.missCount;
    return S_FALSE;  // Not available without sqlite3
}

inline HRESULT SQLiteCacheTier::Put(const std::wstring& /*cacheKey*/, const std::vector<uint8_t>& /*data*/)
{
    return E_NOTIMPL;
}
...
inline HRESULT SQLiteCacheTier::EnableWALMode()
{
    return E_NOTIMPL;
}
inline HRESULT SQLiteCacheTier::Vacuum()
{
    return E_NOTIMPL;
}
inline HRESULT SQLiteCacheTier::CreateOptimalIndexes()
{
    return E_NOTIMPL;
}
...
inline HRESULT SQLiteCacheTier::Open()
{
    return E_NOTIMPL;
}
inline void SQLiteCacheTier::Close()
{
    m_db = nullptr;
}
...
inline HRESULT DiskCacheTier::Get(const std::wstring& cacheKey, std::vector<uint8_t>& outData)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    std::wstring filePath = KeyToFilePath(cacheKey);
    ...
    DWORD bytesRead = 0;
    BOOL ok = ReadFile(hFile, outData.data(), static_cast<DWORD>(fileSize.QuadPart), &bytesRead, nullptr);
    CloseHandle(hFile);
...
inline HRESULT DiskCacheTier::Put(const std::wstring& cacheKey, const std::vector<uint8_t>& data)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    ...
    std::wstring filePath = KeyToFilePath(cacheKey);
    HANDLE hFile =
        CreateFileW(filePath.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (hFile == INVALID_HANDLE_VALUE)
        return E_FAIL;
    DWORD bytesWritten = 0;
    BOOL ok = WriteFile(hFile, data.data(), static_cast<DWORD>(data.size()), &bytesWritten, nullptr);
    CloseHandle(hFile);
    if (!ok)
        return E_FAIL;
    ++m_stats.insertCount;
...
inline HRESULT MultiTierCacheManager::Get(const std::wstring& cacheKey, std::vector<uint8_t>& outData)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
inline HRESULT MultiTierCacheManager::Put(const std::wstring& cacheKey, const std::vector<uint8_t>& data)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
inline MultiTierCacheManager::CacheDashboard MultiTierCacheManager::GetDashboard() const
{
    ...
    dashboard.overallHitRate = totalTotal > 0 ? static_cast<double>(totalHits) / totalTotal : 0.0;
    dashboard.bloomFilterSavingsPercent =
        m_bloomChecks > 0 ? (static_cast<double>(m_bloomNegatives) / m_bloomChecks) * 100.0 : 0.0;
    return dashboard;
}
...
inline void MultiTierCacheManager::PromoteToTier(size_t targetTierIndex, const std::wstring& key,
