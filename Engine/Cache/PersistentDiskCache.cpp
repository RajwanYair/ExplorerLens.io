...
constexpr int WSQL_OPEN_RW_CR = 0x00000006;
static void (*const WSQL_TRANSIENT)(void*) =
    reinterpret_cast<void (*)(void*)>(static_cast<intptr_t>(-1));
...
WinSqlite3& Sql()
{
    static WinSqlite3 sql;
    return sql;
}

#define LOAD_SQL_FN(sym, field) \
    sql.field = reinterpret_cast<decltype(sql.field)>(GetProcAddress(sql.hDll, (sym))); \
    if (!sql.field) { FreeLibrary(sql.hDll); sql.hDll = nullptr; return false; }
...
bool LoadWinSqlite3()
{
    ...
    sql.hDll = LoadLibraryW(L"winsqlite3.dll");
    if (!sql.hDll)
        return false;
...
bool PersistentDiskCache::Open()
{
    ...
    if (m_config.cacheDirPath.empty()) {
        ...
        if (GetEnvironmentVariableW(L"LOCALAPPDATA", buffer, MAX_PATH) > 0)
            m_config.cacheDirPath = std::wstring(buffer) + L"\\ExplorerLens\\Cache";
        else
            m_config.cacheDirPath = L"C:\\ExplorerLensCache";
    }

    CreateDirectoryW(m_config.cacheDirPath.c_str(), nullptr);
    m_sqliteLoaded = OpenSqliteDb(m_config.cacheDirPath + L"\\catalog.db");
    m_isOpen = true;
    return true;
}
...
bool PersistentDiskCache::OpenSqliteDb(const std::wstring& dbPath)
{
    ...
    auto& sql = Sql();
    int result = sql.open_v2(utf8Path.c_str(), reinterpret_cast<sqlite3_t**>(&m_sqliteDb), WSQL_OPEN_RW_CR, nullptr);
    if (result != WSQL_OK) {
        m_sqliteDb = nullptr;
        return false;
    }
    ...
    const char* schema =
        ...
        "id INTEGER PRIMARY KEY, file_path TEXT NOT NULL, cache_key TEXT NOT NULL UNIQUE, "
        "file_mtime INTEGER NOT NULL DEFAULT 0, file_size INTEGER NOT NULL DEFAULT 0, "
        "width INTEGER NOT NULL, height INTEGER NOT NULL, data_blob BLOB, "
        "data_size INTEGER NOT NULL DEFAULT 0, crc32 INTEGER NOT NULL DEFAULT 0, "
        "decoder_id TEXT, decode_ms REAL NOT NULL DEFAULT 0, "
        "created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')), "
        "last_hit INTEGER NOT NULL DEFAULT (strftime('%s','now')), hit_count INTEGER NOT NULL DEFAULT 0);"
    ...
    result = sql.exec(static_cast<sqlite3_t*>(m_sqliteDb), schema, nullptr, nullptr, &errorMessage);
    if (errorMessage)
        sql.free_(errorMessage);
    if (result != WSQL_OK) {
                              ...
                              uint32_t dataSize, double decodeCostMs, const std::wstring& formatName)
{
    ...
    std::wstring key = GenerateCacheKey(filePath, size);
...
bool PersistentDiskCache::SqlitePut(const std::wstring& key, const std::wstring& filePath, uint32_t width,
                                    uint32_t height, const uint8_t* data, uint32_t dataSize, double decodeCostMs,
                                    const std::wstring& formatName)
{
    ...
    const char* statement =
        ...
        "created_at, last_hit, hit_count) VALUES (?,?,?,?,?,?,?,?,?,strftime('%s','now'),strftime('%s','now'),0);";
    sqlite3_stmt_t* stmt = nullptr;
    auto& sql = Sql();
    if (sql.prepare_v2(static_cast<sqlite3_t*>(m_sqliteDb), statement, -1, &stmt, nullptr) != WSQL_OK || !stmt)
        return false;

    std::string utf8Path = WideToUtf8(filePath);
    std::string utf8Key = WideToUtf8(key);
    std::string utf8Format = WideToUtf8(formatName);
    ...
    sql.bind_int64(stmt, 7, static_cast<long long>(ComputeCRC32(data, dataSize)));
    sql.bind_text(stmt, 8, utf8Format.c_str(), -1, WSQL_TRANSIENT);
                              ...
                              std::vector<uint8_t>& data)
{
    ...
    for (uint32_t size : {256u, 512u, 128u, 1024u}) {
        auto key = GenerateCacheKey(filePath, size);
                                    ...
                                    std::vector<uint8_t>& data)
{
    ...
    const char* statement =
        ...
        "FROM thumbnail_cache WHERE file_path = ? ORDER BY last_hit DESC LIMIT 1;";
    sqlite3_stmt_t* stmt = nullptr;
    auto& sql = Sql();
    if (sql.prepare_v2(static_cast<sqlite3_t*>(m_sqliteDb), statement, -1, &stmt, nullptr) != WSQL_OK || !stmt)
        return false;
    ...
    if (sql.step(stmt) == WSQL_ROW) {
        ...
        const void* blob = sql.column_blob(stmt, 3);
        if (blob && blobBytes > 0) {
            data.resize(static_cast<size_t>(blobBytes));
            memcpy(data.data(), blob, static_cast<size_t>(blobBytes));
        }
        found = true;
        ...
        const char* updateStatement =
            ...
            " WHERE file_path = ?;";
        sqlite3_stmt_t* updateStmt = nullptr;
        if (sql.prepare_v2(static_cast<sqlite3_t*>(m_sqliteDb), updateStatement, -1, &updateStmt, nullptr) == WSQL_OK) {
            sql.bind_text(updateStmt, 1, utf8Path.c_str(), -1, WSQL_TRANSIENT);
            sql.step(updateStmt);
            sql.finalize(updateStmt);
        }
        ...
        entry.cacheWriteTime = now;
        entry.lastAccessTime = now;
        entry.cacheKey = GenerateCacheKey(filePath, (width > height) ? width : height);
