...
static_assert(alignof(CacheAligned<int>) == CACHE_LINE_BYTES,
    "CacheAligned must have cache-line alignment");
static_assert(sizeof(CacheAligned<int>) >= CACHE_LINE_BYTES,
    "CacheAligned must occupy at least one cache line");
...
static_assert(alignof(PaddedAtomic<uint64_t>) == CACHE_LINE_BYTES,
    "PaddedAtomic must have cache-line alignment");
static_assert(sizeof(PaddedAtomic<uint64_t>) >= CACHE_LINE_BYTES,
...
public:
    ...
    static constexpr size_t FootprintBytes() { return sizeof(CacheLineArray); }

    /// Bytes per element (includes padding)
    static constexpr size_t BytesPerElement() { return sizeof(CacheAligned<T>); }

    /// Verify all elements are cache-line aligned
    bool AllAligned() const {
        for (size_t i = 0; i < N; ++i) {
            if (!m_elements[i].IsProperlyAligned()) return false;
        }
        return true;
    }
...
constexpr bool MayFalseShare(size_t offsetA, size_t sizeA,
                              size_t offsetB, size_t /*sizeB*/) {
