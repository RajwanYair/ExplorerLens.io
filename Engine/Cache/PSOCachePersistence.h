...
public:
    ...
    bool Insert(const PSODescription& desc, const uint8_t* blobData, uint32_t blobSize) {
        if (!blobData || blobSize == 0 || blobSize > PSO_MAX_BLOB_SIZE) return false;
...
private:
    ...
    bool LoadFromDisk() {
        HANDLE hFile = CreateFileW(m_cachePath.c_str(), GENERIC_READ,
    ...
    bool WriteToDisk() {
        ...
        HANDLE hFile = CreateFileW(tempPath.c_str(), GENERIC_WRITE,
        ...
        m_header.checksum = ComputeCRC32(m_entries.data(),
            m_entries.size() * sizeof(PSODiskEntry), m_blobs.data(), m_blobs.size());

        DWORD written = 0;
        WriteFile(hFile, &m_header, sizeof(m_header), &written, nullptr);
        if (!m_entries.empty()) {
            WriteFile(hFile, m_entries.data(),
                static_cast<DWORD>(m_entries.size() * sizeof(PSODiskEntry)), &written, nullptr);
        }
        if (!m_blobs.empty()) {
            WriteFile(hFile, m_blobs.data(),
                static_cast<DWORD>(m_blobs.size()), &written, nullptr);
        }
