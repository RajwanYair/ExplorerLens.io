...
public:
    static const wchar_t* CacheStateName(PSOCacheState s) {
        switch (s) {
            case PSOCacheState::NotCached:   return L"Not Cached";
            case PSOCacheState::Compiling:   return L"Compiling";
            case PSOCacheState::Cached:      return L"Cached";
            case PSOCacheState::Invalidated: return L"Invalidated";
    ...
    static const wchar_t* PipelineTypeName(PipelineType t) {
        switch (t) {
            case PipelineType::Graphics:   return L"Graphics";
            case PipelineType::Compute:    return L"Compute";
            case PipelineType::MeshShader: return L"Mesh Shader";
            case PipelineType::WorkGraph:  return L"Work Graph";
            default: return L"Unknown";
        }
    }
    ...
    static const wchar_t* WarmupStrategyName(PSOWarmupStrategy w) {
        switch (w) {
            case PSOWarmupStrategy::Lazy:       return L"Lazy";
            case PSOWarmupStrategy::Eager:      return L"Eager";
            case PSOWarmupStrategy::Background: return L"Background";
            default: return L"Unknown";
        }
    }

    static constexpr size_t CacheStateCount()     { return static_cast<size_t>(PSOCacheState::COUNT); }
    static constexpr size_t PipelineTypeCount()   { return static_cast<size_t>(PipelineType::COUNT); }
    static constexpr size_t WarmupStrategyCount() { return static_cast<size_t>(PSOWarmupStrategy::COUNT); }

    static PSOCacheStats DefaultStats() { return PSOCacheStats{}; }

    static bool IsHitRateGood(const PSOCacheStats& s) {
        if (s.hitCount + s.missCount == 0) return true;
