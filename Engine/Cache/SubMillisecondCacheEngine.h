...
class SubMillisecondCacheEngine
{
  public:
    ...
    explicit SubMillisecondCacheEngine(uint32_t capacity = DEFAULT_CAPACITY, CacheHashAlgo algo = CacheHashAlgo::FNV1a)
    ...
    inline bool Get(const std::wstring& key, std::vector<uint8_t>& outData)
    {
        ...
        for (uint8_t probe = 0; probe < MAX_PROBE; ++probe) {
            ...
            if (entry.hash == h && entry.key == key) {
                ...
                double elapsedMs = static_cast<double>(now.QuadPart - entry.insertTimeTick) * 1000.0
            ...
            if (entry.probeDistance < probe)
                break;  // robin-hood invariant
        }
        ...
        double lookupUs = static_cast<double>(endTick.QuadPart - startTick.QuadPart) * 1000000.0
                          / static_cast<double>(m_freq.QuadPart);
    ...
    inline void EvictExpired()
    {
        ...
        for (uint32_t i = 0; i < m_capacity; ++i) {
            if (!m_table[i].occupied)
                continue;
            if (m_table[i].ttlMs == 0)
                continue;  // no expiry
            double elapsedMs = static_cast<double>(now.QuadPart - m_table[i].insertTimeTick) * 1000.0
                               / static_cast<double>(m_freq.QuadPart);
    ...
    inline CacheHashAlgo GetHashAlgorithm() const noexcept
    {
        return m_algo;
    }
  ...
  private:
    ...
    inline void RemoveAtInternal(uint32_t slot)
    {
        ...
        for (uint8_t i = 1; i < MAX_PROBE; ++i) {
            ...
            if (!m_table[next].occupied || m_table[next].probeDistance == 0)
                break;
