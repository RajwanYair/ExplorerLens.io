...
enum class HTTPMethod { GET, POST, PUT, DELETE_HTTP };
enum class RESTServerState { Stopped, Running, Error };
...
public:
    explicit RESTThumbnailServer(RESTServerConfig config = {})
    ...
    bool Start() {
        if (m_state == RESTServerState::Running) return true;
        m_state = RESTServerState::Running;
        return true;
    }
    void Stop() noexcept { m_state = RESTServerState::Stopped; }
    bool IsRunning() const noexcept { return m_state == RESTServerState::Running; }
    RESTServerState State() const noexcept { return m_state; }
    const RESTServerConfig& Config() const noexcept { return m_config; }
