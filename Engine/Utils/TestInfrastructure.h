...
inline bool TestInfrastructureAvailable()
{
    ...
    static_assert(sizeof(IntegrationTestFramework) > 0, "IntegrationTestFramework missing");
    static_assert(sizeof(TestSuiteExpansion) > 0, "TestSuiteExpansion missing");
    static_assert(sizeof(CodeCoverageIntegration) > 0, "CodeCoverageIntegration missing");
    return true;
}

/// Count of consolidated test infrastructure modules.
inline constexpr uint32_t TestInfrastructureModuleCount()
{
    return 3;
}
