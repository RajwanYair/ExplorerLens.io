...
public:
    ...
    LocaleFallbackResult Resolve(const std::string& locale,
                                  const std::string& key) const {
        for (const auto& candidate : BuildChain(locale)) {
            auto locIt = m_store.find(candidate);
            if (locIt == m_store.end()) continue;
    ...
    static std::vector<std::string> BuildChain(const std::string& locale) {
        ...
        if (locale != "en") chain.push_back("en");
        return chain;
    }

    size_t LocaleCount()                  const noexcept { return m_store.size(); }
    bool   HasLocale(const std::string& l) const noexcept { return m_store.count(l) > 0; }
