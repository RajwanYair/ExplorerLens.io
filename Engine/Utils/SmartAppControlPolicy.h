...
enum class SACPolicyMode {
    Enforcement,
    Evaluation,
    Off
};
enum class SACBinaryTrustLevel {
    Trusted,
    Untrusted,
    Unknown,
    Blocked
...
{
  public:
    explicit SmartAppControlPolicy(SACPolicyMode mode = SACPolicyMode::Enforcement) : m_mode(mode) {}
    ...
    void AddTrustedPublisher(const std::string& cn)
    {
        m_trustedPublishers.push_back(cn);
    }
    void SetMode(SACPolicyMode mode) noexcept
    {
        m_mode = mode;
    }
    SACPolicyMode Mode() const noexcept
    {
        return m_mode;
    }
    ...
    {
        switch (mode) {
            case SACPolicyMode::Enforcement:
                return "Enforcement";
            case SACPolicyMode::Evaluation:
                return "Evaluation";
            case SACPolicyMode::Off:
                return "Off";
        }
        return "Unknown";
    }
