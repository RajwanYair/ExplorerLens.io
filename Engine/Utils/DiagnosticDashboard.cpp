...
const wchar_t* DiagnosticDashboard::GetCategoryName(MetricCategory cat)
{
    switch (cat) {
        ...
        case MetricCategory::Network:
            return L"Network";
        case MetricCategory::Decoder:
            return L"Decoder";
        case MetricCategory::Cache:
            return L"Cache";
        default:
            return L"Unknown";
    }
...
{
    switch (level) {
        case DiagHealthLevel::Healthy:
            return L"Healthy";
        case DiagHealthLevel::Warning:
            return L"Warning";
        case DiagHealthLevel::Degraded:
            return L"Degraded";
        case DiagHealthLevel::Critical:
            return L"Critical";
        case DiagHealthLevel::Unknown:
            return L"Unknown";
        default:
            return L"Unknown";
    }
...
DiagHealthLevel DiagnosticDashboard::EvaluateHealth(double value, double threshold) const
{
    ...
    if (ratio < 0.6)
        return DiagHealthLevel::Healthy;
    if (ratio < 0.8)
        return DiagHealthLevel::Warning;
    if (ratio < 0.95)
        return DiagHealthLevel::Degraded;
    return DiagHealthLevel::Critical;
}

void DiagnosticDashboard::RecordMetric(const std::wstring& name, MetricCategory cat, double value, double threshold)
{
    MetricPoint mp;
    mp.name = name;
    mp.category = cat;
    ...
    mp.timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
...
HealthSnapshot DiagnosticDashboard::GetSnapshot() const
{
    ...
    for (const auto& m : m_metrics) {
        if (m.health == DiagHealthLevel::Warning)
            snap.warningCount++;
        if (m.health == DiagHealthLevel::Critical)
            snap.criticalCount++;
