...
MigrationResult ConfigMigrationEngine::Migrate(ConfigVersion from,
 ConfigVersion to) {
 ...
 result.migrationTimeMs =
 std::chrono::duration<double, std::milli>(end - start).count();
 result.success = true;
 return result;
}

void ConfigMigrationEngine::ApplyRule(const MigrationRule &rule,
 MigrationResult &result) {
 switch (rule.action) {
 ...
 m_migratedConfig[rule.targetKey.empty() ? rule.sourceKey
 : rule.targetKey] =
...
void ConfigMigrationEngine::AddDefaults(ConfigVersion targetVersion,
 MigrationResult &result) {
...
bool ConfigMigrationEngine::CreateBackup(
 const std::wstring & /*backupPath*/) const {
