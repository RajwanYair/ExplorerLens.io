...
enum class I18nPluralCategory { Zero, One, Two, Few, Many, Other };
enum class I18nLocaleDir { LTR, RTL };
...
public:
    explicit I18nRuntimeEngine(const std::string& defaultLocale = "en-US")
        : m_activeLocale(defaultLocale) {}
    ...
    bool SetLocale(const std::string& locale) {
        if (m_catalogues.find(locale) == m_catalogues.end() && locale != "en-US") return false;
    ...
    I18nLocaleDir GetDirection() const noexcept {
        for (const auto& [_, info] : m_localeInfo) {
            if (info.tag == m_activeLocale) return info.direction;
        }
    ...
    const std::string& ActiveLocale() const noexcept { return m_activeLocale; }
    size_t CatalogueCount()           const noexcept { return m_catalogues.size(); }
