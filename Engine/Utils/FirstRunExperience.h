...
static const OOBEStepInfo k_OOBESteps[] = {
    ...
    {OOBEStep::LicenseAgreement, L"License Agreement", L"Review and accept the End User License Agreement.", false,
     false},
    {OOBEStep::RegistrationChoice, L"Choose How to Start",
     L"Start a 30-day Pro trial, enter a license key, or use the Community edition.", true, false},
    {OOBEStep::ShellRegistration, L"Register Shell Extension", L"Register ExplorerLens with Windows Explorer.", true,
     true},
    ...
    {OOBEStep::Complete, L"Setup Complete", L"ExplorerLens is ready.  Restart Explorer to see thumbnails.", false,
     false}};
...
class FirstRunExperience
{
  public:
    ...
    {
        HKEY hk = nullptr;
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens", 0, KEY_READ, &hk) != ERROR_SUCCESS)
            return true;
        DWORD val = 0, sz = sizeof(val);
        bool hasFlag = (RegQueryValueExW(hk, L"SetupComplete", nullptr, nullptr, reinterpret_cast<BYTE*>(&val), &sz)
                        == ERROR_SUCCESS)
                       && val;
        RegCloseKey(hk);
        return !hasFlag;
    ...
    {
        HKEY hk = nullptr;
        RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens", 0, nullptr, 0, KEY_WRITE, nullptr, &hk, nullptr);
        if (!hk)
            return;
        DWORD one = 1;
        RegSetValueExW(hk, L"SetupComplete", 0, REG_DWORD, reinterpret_cast<const BYTE*>(&one), sizeof(DWORD));
        RegCloseKey(hk);
    }
    ...
    {
        HKEY hk = nullptr;
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens", 0, KEY_WRITE, &hk) != ERROR_SUCCESS)
            return;
    ...
    void SetLicenseKey(const std::wstring& key)
    {
        m_state.licenseKey = key;
    }
    void SetTrialChosen(bool t)
    {
        m_state.trialStarted = t;
    }
  ...
  private:
    ...
    {
        if (m_onStep)
            m_onStep(m_state, CurrentStepInfo());
    }
