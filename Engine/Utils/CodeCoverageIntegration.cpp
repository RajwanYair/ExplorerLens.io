...
CodeCoverageIntegration::CodeCoverageIntegration() : m_thresholds(CoverageThresholds::ForCI()) {}

CodeCoverageIntegration::CodeCoverageIntegration(const CoverageThresholds& thresholds) : m_thresholds(thresholds) {}

std::wstring CodeCoverageIntegration::GenerateCoverageCommand(const std::wstring& testExecutable,
...
CoverageReport CodeCoverageIntegration::ParseCoverageReport(const std::wstring& xmlPath) const
{
...
std::vector<FuzzTargetConfig> CodeCoverageIntegration::GenerateFuzzTargets() const
{
    ...
    for (const auto& decoder : decoders) {
        ...
        config.targetTypes = {FuzzTargetType::HeaderParsing, FuzzTargetType::FullDecode, FuzzTargetType::MalformedInput};
...
ModuleCoverage CodeCoverageIntegration::GetModuleCoverage(const std::wstring& moduleName,
...
CoverageReport CodeCoverageIntegration::MergeReports(const std::vector<CoverageReport>& reports) const
{
    ...
    merged.overallLineCoverage = totalLines > 0 ? (100.0 * coveredLines / totalLines) : 0.0;
    merged.overallBranchCoverage = totalBranches > 0 ? (100.0 * coveredBranches / totalBranches) : 0.0;
    merged.overallFunctionCoverage = totalFunctions > 0 ? (100.0 * coveredFunctions / totalFunctions) : 0.0;
...
{
    switch (type) {
        case FuzzTargetType::HeaderParsing:
            return L"HeaderParsing";
        case FuzzTargetType::FullDecode:
            return L"FullDecode";
        case FuzzTargetType::MetadataExtraction:
            return L"MetadataExtraction";
        case FuzzTargetType::MalformedInput:
            return L"MalformedInput";
        case FuzzTargetType::BoundaryValues:
            return L"BoundaryValues";
        default:
            return L"Unknown";
    }
}
...
{
    return {L"ImageDecoder", L"WebPDecoder",       L"AVIFDecoder", L"JXLDecoder",        L"HEIFDecoder",
            L"RAWDecoder",   L"ICODecoder",        L"TGADecoder",  L"QOIDecoder",        L"PSDDecoder",
            L"DDSDecoder",   L"HDRDecoder",        L"PPMDecoder",  L"EXRDecoder",        L"SVGDecoder",
            L"PDFDecoder",   L"FontDecoder",       L"WMFDecoder",  L"PCXDecoder",        L"FarbfeldDecoder",
            L"EPSDecoder",   L"KTXTextureDecoder", L"VTFDecoder",  L"OpenRasterDecoder", L"XCFDecoder",
            L"SGIDecoder",   L"XPMDecoder"};
}
