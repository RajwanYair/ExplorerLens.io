...
struct COMRegistryEntry
{
    ...
    {
        switch (status) {
            case COMRegistryStatus::Valid:
                return "Valid";
            case COMRegistryStatus::Missing:
                return "Missing";
            case COMRegistryStatus::WrongValue:
                return "WrongValue";
            case COMRegistryStatus::AccessDenied:
                return "AccessDenied";
        }
        return "Unknown";
    }
...
{
  public:
    static const char* LensClsid() noexcept
    {
        return LENS_CLSID;
    }
  ...
  private:
    std::vector<COMRegistryEntry> BuildExpectedEntries() const
    {
        return {
            ...
            {std::string("CLSID\\") + LENS_CLSID + "\\InProcServer32", "ThreadingModel", "Apartment",
             COMRegistryStatus::Missing},
        };
    }
    ...
    {
        if (entry.status == COMRegistryStatus::AccessDenied)
            return false;
