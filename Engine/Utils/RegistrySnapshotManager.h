...
{
    switch (value) {
        case SnapshotScope::CurrentUser:
            return "CurrentUser";
        case SnapshotScope::LocalMachine:
            return "LocalMachine";
        case SnapshotScope::ClassesRoot:
            return "ClassesRoot";
...
{
    switch (value) {
        case SnapshotAction::Backup:
            return "Backup";
        case SnapshotAction::Restore:
            return "Restore";
        case SnapshotAction::Compare:
            return "Compare";
...
class RegistrySnapshotManager
{
  public:
    ...
    bool CreateSnapshot(SnapshotScope scope, const std::wstring& outputPath, const std::string& description = "")
    {
        ...
        if (m_snapshots.size() >= MAX_SNAPSHOTS) {
            ...
            auto oldest = std::min_element(
                ...
                [](const RegistrySnapshot& a, const RegistrySnapshot& b) { return a.timestampMs < b.timestampMs; });
        ...
        switch (scope) {
            case SnapshotScope::CurrentUser:
                snapshot.keyCount = 150;
                snapshot.sizeBytes = 32768;
                break;
            case SnapshotScope::LocalMachine:
                snapshot.keyCount = 80;
                snapshot.sizeBytes = 16384;
                break;
            case SnapshotScope::ClassesRoot:
                snapshot.keyCount = 200;
                snapshot.sizeBytes = 65536;
                break;
            case SnapshotScope::All:
                snapshot.keyCount = 430;
                snapshot.sizeBytes = 114688;
                break;
        }
    ...
    uint64_t GetTotalCreated() const
    {
        return m_totalCreated;
    }
    uint64_t GetTotalRestored() const
    {
        return m_totalRestored;
    }
    uint32_t GetLastRestoredId() const
    {
        return m_lastRestoredId;
    }
