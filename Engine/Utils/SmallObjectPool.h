...
{
  public:
    explicit PoolPtr(SmallObjectPool<T>* pool = nullptr) : m_pool(pool), m_ptr(pool ? pool->Allocate() : nullptr) {}
    ...
    PoolPtr(PoolPtr&& other) noexcept : m_pool(other.m_pool), m_ptr(other.m_ptr)
    {
        other.m_ptr = nullptr;
    }
