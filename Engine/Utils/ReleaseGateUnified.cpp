...
ReleaseGateV3::ReleaseGateV3(const ReleaseThresholdsV92& thresholds) : m_thresholds(thresholds) {}
...
ReleaseGateVerdict ReleaseGateV3::ComputeVerdict(uint32_t passed, uint32_t failed, bool hasBlockers) const
{
    if (hasBlockers)
...
ReleaseGateResult ReleaseGateV3::Evaluate() const
{
    ...
    for (const auto& m : m_measurements) {
        ...
        result.failedKPIs++;
        failed++;
        if (m.dimension == ReleaseKPIDimension::BuildQuality || m.dimension == ReleaseKPIDimension::Security
    ...
    result.overallScore = result.totalKPIs > 0 ? (100.0 * passed / result.totalKPIs) : 0.0;
...
std::wstring ReleaseGateV3::GenerateChecklist() const
{
    ...
    for (const auto& m : m_measurements) {
        oss << L" [" << (m.passed ? L"PASS" : L"FAIL") << L"] " << m.name << L": " << m.value << m.unit << L"\n";
    }
    return oss.str();
}
...
{
    std::wostringstream oss;
    oss << L"Release Notes " << result.version << L" — Gate " << GetVerdictName(result.verdict) << L"\n";
    oss << L" Score: " << result.overallScore << L"% (" << result.passedKPIs << L"/" << result.totalKPIs << L" KPIs)\n";
    return oss.str();
}
...
ReleaseGateV10::ReleaseGateV10(const ReleaseThresholdsV10& thresholds) : m_thresholds(thresholds) {}
...
V10ReleaseResult ReleaseGateV10::Evaluate() const
{
    ...
    add(m_shellRegistrations >= m_thresholds.minShellRegistrations, L"ShellRegistrations");
    add((uint32_t)m_gpuBackends.size() >= m_thresholds.minGPUBackends, L"GPUBackends");

    result.passed = (result.passedChecks == result.totalChecks);
    result.overallScore = result.totalChecks > 0 ? (100.0 * result.passedChecks / result.totalChecks) : 0.0;
...
{
    std::wostringstream oss;
    oss << L"Release Notes v10.0.0 — " << (result.passed ? L"PASSED" : L"FAILED") << L"\n";
