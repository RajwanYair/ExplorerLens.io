...
struct CrashBudget
{
    ...
    {
        return currentCrashes > maxCrashes || currentTimeouts > maxTimeouts || currentMemoryLeaks > maxMemoryLeaks;
    }
    ...
    static CrashBudget ZeroTolerance()
    {
        return {};
    }
...
struct FuzzConfig
{
    ...
    static FuzzConfig Quick()
    {
        ...
        c.timeoutMs = 2000;
        c.strategies = {MutationStrategy::BitFlip, MutationStrategy::HeaderCorrupt, MutationStrategy::Truncation};
        return c;
    }
...
struct FuzzStats
{
    ...
    {
        return totalIterations > 0 ? static_cast<double>(totalCrashes) / totalIterations : 0.0;
    }

    double IterationsPerSecond() const
    {
        return totalDurationMs > 0 ? totalIterations / (totalDurationMs / 1000.0) : 0.0;
    }

    std::string Summary() const
    {
        return "Iterations=" + std::to_string(totalIterations) + " Crashes=" + std::to_string(totalCrashes)
...
class ByteMutator
{
  public:
    ...
    std::vector<uint8_t> Mutate(const std::vector<uint8_t>& input, MutationStrategy strategy)
    {
        if (input.empty())
            return input;
        auto output = input;
...
{
  public:
    explicit ContinuousFuzzEngine(FuzzConfig config = FuzzConfig::Quick()) : m_config(config), m_mutator(42) {}
    ...
    void SetDecoder(DecoderFn fn)
    {
        m_decoderFn = std::move(fn);
    }

    FuzzExecution RunSingle(const std::vector<uint8_t>& input, const std::string& decoderName, MutationStrategy strategy)
    {
        FuzzExecution exec;
        exec.id = ++m_nextId;
        ...
        exec.durationMs = std::chrono::duration<double, std::milli>(end - start).count();
    ...
    {
        return m_config.budget.IsExhausted();
    }
    bool IsBudgetClean() const
    {
        return m_config.budget.IsClean();
    }

    FuzzStats GetStats() const
    {
        return m_stats;
    }
    FuzzConfig GetConfig() const
    {
        return m_config;
    }
  ...
  private:
    void RecordResult(const FuzzExecution& exec)
    {
        ...
        switch (exec.result) {
            case FuzzResult::NoError:
                m_stats.totalNoErrors++;
                break;
            case FuzzResult::ErrorHandled:
                m_stats.totalErrorsHandled++;
                break;
            case FuzzResult::Crash:
            case FuzzResult::Assertion:
                m_stats.totalCrashes++;
                m_stats.uniqueCrashes++;
                break;
            case FuzzResult::Timeout:
            case FuzzResult::Hang:
    ...
    static bool InvokeSEHGuarded(std::function<bool(const uint8_t*, size_t)>& fn, const uint8_t* data, size_t size,
                                 bool& completed)
    {
        __try {
...
class FuzzingCampaign
{
  public:
    ...
    std::string GetWinAFLCommand(const FuzzCampaignConfig& config, const char* targetExe) const
    {
        std::string cmd = "afl-fuzz.exe";
        cmd += " -i " + std::string(config.corpusDir ? config.corpusDir : "corpus");
        cmd += " -o " + std::string(config.artifactDir ? config.artifactDir : "findings");
    ...
    const FuzzCrashArtifact* GetCrashes() const
    {
        return m_crashes;
    }
    uint32_t GetCrashCount() const
    {
        return m_crashCount;
    }
...
{
    static const char* names[] = {"ZeroFill", "RandomBytes",   "ValidHeaderGarbage", "Truncated",
                                  "BitFlip",  "OversizeField", "NullTerminated",     "RepeatingPattern"};
    auto idx = static_cast<uint32_t>(s);
    return idx < static_cast<uint32_t>(CorruptionStrategy::MaxEnum) ? names[idx] : "Unknown";
}
...
struct FuzzCampaignReport
{
    ...
    {
        return totalIterations > 0 ? 100.0 * sehExceptions / totalIterations : 0.0;
    }

    double GracefulRate() const
    {
        return totalIterations > 0 ? 100.0 * gracefulRejects / totalIterations : 0.0;
    }
};
...
{
  public:
    explicit CorruptPayloadGenerator(uint64_t seed = 0) : m_rng(seed ? seed : std::random_device{}()) {}

    std::vector<uint8_t> Generate(CorruptionStrategy strategy, const std::string& extension, size_t targetSize)
    {
        switch (strategy) {
...
class SEHIsolationWrapper
{
  public:
    ...
    static ExecutionResult Execute(std::function<bool()> callable, uint32_t /*timeoutMs*/ = 5000)
    {
...
class WorkerIsolationMonitor
{
  public:
    ...
    explicit WorkerIsolationMonitor(uint32_t maxFailuresBeforeRecycle = 10, double maxFailureRatePercent = 5.0)
...
{
  public:
    using DecoderCallback = std::function<bool(const uint8_t* data, size_t size, const std::string& extension)>;

    explicit FuzzCampaignRunner(uint64_t seed = 42) : m_generator(seed) {}

    // Attach a decode callback that simulates the thumbnail pipeline
    void SetDecoderCallback(DecoderCallback cb)
    {
        m_decoder = std::move(cb);
    }

    // Run fuzzing campaign with specified iterations per format
    FuzzCampaignReport Run(uint64_t iterationsPerFormat, const std::vector<std::string>& formats)
    {
        ...
        report.totalElapsedMs = std::chrono::duration<double, std::milli>(campaignEnd - campaignStart).count();
        report.avgIterationMs = report.totalIterations > 0 ? report.totalElapsedMs / report.totalIterations : 0.0;
    ...
    FuzzCampaignReport QuickSmoke(uint64_t totalIterations)
    {
        ...
        auto end = std::chrono::steady_clock::now();
        report.totalElapsedMs = std::chrono::duration<double, std::milli>(end - start).count();
        report.avgIterationMs = report.totalIterations > 0 ? report.totalElapsedMs / report.totalIterations : 0.0;

        return report;
    }

    size_t SupportedFormatCount() const
    {
        return GetKnownMagicBytes().size();
    }
  ...
  private:
    FuzzResult RunSingleIteration(uint64_t id, const std::string& ext)
    {
        ...
        auto sehResult = SEHIsolationWrapper::Execute(
            [&]() { return m_decoder ? m_decoder(payload.data(), payload.size(), ext) : false; });
    ...
    void AccumulateResult(FuzzCampaignReport& report, const FuzzResult& res)
    {
        ...
        if (res.sehCaught)
            report.sehExceptions++;
        if (res.gracefulReject)
            report.gracefulRejects++;
        if (res.circuitBroken)
            report.circuitBreaks++;
        if (res.crashed) {
            report.crashes++;
...
class MemoryLeakRegressionHarness
{
  public:
    ...
    static LeakTestResult RunLeakTest(std::function<void()> decodeFunc, uint64_t iterations = 100)
    {
        ...
        result.workingSetGrowthMB =
            static_cast<double>(result.after.workingSetBytes - result.before.workingSetBytes) / (1024.0 * 1024.0);
        result.privateGrowthMB =
            static_cast<double>(result.after.privateBytes - result.before.privateBytes) / (1024.0 * 1024.0);
        result.growthPerIterKB = (result.workingSetGrowthMB * 1024.0) / iterations;
        result.passedLeakCheck = result.growthPerIterKB < LeakTestResult::MAX_GROWTH_PER_ITER_KB;
