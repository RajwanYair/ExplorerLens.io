...
{
    switch (policy) {
        case RotationPolicy::SizeBased:
            return L"Size Based";
        case RotationPolicy::TimeBased:
            return L"Time Based";
        case RotationPolicy::CountBased:
            return L"Count Based";
...
{
    switch (comp) {
        case LogCompression::None:
            return L"None";
        case LogCompression::GZip:
            return L"GZip";
        case LogCompression::Zstd:
            return L"Zstd";
...
void LogRotationEngine::AddRotatedFile(const std::wstring& path, uint64_t size)
{
    ...
    file.timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
