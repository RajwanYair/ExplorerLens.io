...
uint64_t NotificationEngine::Send(NotifyType type, const std::wstring& title, const std::wstring& message,
                                  NotifyPriority priority)
{
    ...
    if (priority == NotifyPriority::Critical)
        n.durationMs *= 3;
    else if (priority == NotifyPriority::High)
        n.durationMs *= 2;
...
uint32_t NotificationEngine::GetPendingCount() const
{
    ...
    for (const auto& n : m_notifications) {
        if (n.state == NotifyState::Pending)
            count++;
    }
    return count;
}
...
{
    switch (type) {
        case NotifyType::BatchComplete:
            return L"Batch Complete";
        case NotifyType::UpdateAvailable:
            return L"Update Available";
        case NotifyType::DecoderError:
            return L"Decoder Error";
        case NotifyType::CacheCleared:
            return L"Cache Cleared";
        case NotifyType::PluginLoaded:
            return L"Plugin Loaded";
        case NotifyType::LicenseExpiring:
            return L"License Expiring";
        case NotifyType::SystemWarning:
            return L"System Warning";
        default:
            return L"Unknown";
    }
}
...
{
    switch (state) {
        case NotifyState::Pending:
            return L"Pending";
        case NotifyState::Displayed:
            return L"Displayed";
        case NotifyState::Dismissed:
            return L"Dismissed";
        case NotifyState::Expired:
            return L"Expired";
        case NotifyState::Clicked:
            return L"Clicked";
        default:
            return L"Unknown";
    }
