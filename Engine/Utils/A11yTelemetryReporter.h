...
class A11yTelemetryReporter
{
  public:
    ...
    void RecordScreenReaderSession()
    {
        ++m_record.screenReaderSessions;
    }
    void RecordHighContrastSession()
    {
        ++m_record.highContrastSessions;
    }
    void RecordKeyboardNavEvent()
    {
        ++m_record.keyboardNavEvents;
    }
    void RecordAltTextGenerated()
    {
        ++m_record.altTextGenerated;
    }
    void RecordWCAGAuditPassed()
    {
        ++m_record.wcagAuditsPassed;
    }
    void RecordWCAGAuditFailed()
    {
        ++m_record.wcagAuditsFailed;
    }

    void RecordCaptionQuality(float score)
    {
        uint64_t n = m_record.wcagAuditsPassed + m_record.wcagAuditsFailed + 1;
        m_record.avgCaptionQuality = (m_record.avgCaptionQuality * (n - 1) + score) / static_cast<float>(n);
    }

    void RecordContrastRatio(float ratio)
    {
        uint64_t n = m_record.screenReaderSessions + m_record.highContrastSessions + 1;
        m_record.avgContrastRatio = (m_record.avgContrastRatio * (n - 1) + ratio) / static_cast<float>(n);
    ...
    A11yTelemetryReport GenerateReport(const std::string& format = "json") const
    {
        ...
        rpt.payload = "{\"screenReader\":" + std::to_string(m_record.screenReaderSessions)
                      ...
                      + ",\"altTextGen\":" + std::to_string(m_record.altTextGenerated) + ",\"wcagPassRate\":"
