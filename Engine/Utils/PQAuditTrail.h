...
struct PQAuditEntry {
    ...
    std::string EventName() const noexcept {
        switch (event) {
        case PQCryptoEvent::KeyGenerated:      return "KeyGenerated";
        case PQCryptoEvent::KeyEncapsulated:   return "KeyEncapsulated";
        case PQCryptoEvent::KeyDecapsulated:   return "KeyDecapsulated";
        case PQCryptoEvent::SignatureCreated:  return "SignatureCreated";
        case PQCryptoEvent::SignatureVerified: return "SignatureVerified";
        case PQCryptoEvent::KeyRotated:        return "KeyRotated";
        case PQCryptoEvent::PolicyViolation:   return "PolicyViolation";
        }
        return "Unknown";
    }
};
...
public:
    ...
    PQAuditVerifyResult VerifyChain() const {
        ...
        for (const auto& e : m_entries) {
            if (e.previousHash != prevHash) {
                result.brokenLinks++;
                if (result.firstBrokenAt.empty()) result.firstBrokenAt = e.isoTimestamp;
            }
    ...
    const std::vector<PQAuditEntry>& Entries() const noexcept { return m_entries; }
    int EntryCount() const noexcept { return static_cast<int>(m_entries.size()); }
...
private:
    ...
    static std::string ComputeHash(const PQAuditEntry& e) {
        ...
        for (char c : combined) { h ^= static_cast<unsigned char>(c); h *= 16777619u; }
        std::ostringstream oss;
        oss << std::hex << h;
        return oss.str();
    }
