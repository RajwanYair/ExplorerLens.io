...
class UsageStats
{
  public:
    ...
    void RecordCacheHit()
    {
        Record(UsageEvent::CacheHit, L"", 1);
        m_cacheHits++;
    }
    void RecordCacheMiss()
    {
        Record(UsageEvent::CacheMiss, L"", 1);
        m_cacheMisses++;
    }
    void RecordGpuDecode()
    {
        Record(UsageEvent::GpuDecodeUsed, L"", 1);
    }
    ...
    UsageSnapshot BuildSnapshot() const
    {
        ...
        if (total > 0)
            snap.cacheHitRate100 = (m_cacheHits * 10000) / total;
        ...
        if (pfn)
            pfn(&osv);
        snap.osVersion = std::to_wstring(osv.dwMajorVersion) + L"." + std::to_wstring(osv.dwMinorVersion) + L"."
