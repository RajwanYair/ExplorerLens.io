...
struct ModuleCoverageReport
{
    ...
    {
        return totalFunctions > 0 ? static_cast<float>(coveredFunctions) * 100.0f / static_cast<float>(totalFunctions)
                                  : 0.0f;
    }
    float GetBranchCoverage() const
    {
        return totalBranches > 0 ? static_cast<float>(coveredBranches) * 100.0f / static_cast<float>(totalBranches)
                                 : 0.0f;
    }
};
...
class CodeCoverageConfig
{
  public:
    ...
    std::string GetOpenCppCoverageCommand(const char* testExe, const char* outputDir = "coverage") const
    {
    ...
    const CodeCoverageThresholds& GetThresholds() const
    {
        return m_thresholds;
    }
    void SetThresholds(const CodeCoverageThresholds& t)
    {
        m_thresholds = t;
    }
    ...
    static const CoverageExclusion* GetExclusions()
    {
        static const CoverageExclusion exclusions[] = {
            {"external\\*", "Third-party libraries"}, {"gtest\\*", "Test framework"},
            {"build\\*", "Build artifacts"},          {"Engine\\Tests\\*", "Test code itself"},
            {"packages\\*", "NuGet packages"},        {"*_generated.*", "Auto-generated code"}};
        return exclusions;
    }
    ...
    {
        static const ModuleTarget targets[] = {{"Engine/Core", 85.0f},
