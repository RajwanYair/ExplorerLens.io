...
{
    switch (d) {
        case ValidationDomain::Decoder:
            return "Decoder";
        case ValidationDomain::Memory:
            return "Memory";
        case ValidationDomain::Cache:
            return "Cache";
        case ValidationDomain::Plugin:
            return "Plugin";
        case ValidationDomain::ARM64:
            return "ARM64";
        case ValidationDomain::Pipeline:
            return "Pipeline";
        case ValidationDomain::Integration:
            return "Integration";
        default:
            return "Unknown";
    }
...
struct MatrixCell
{
    ...
    {
        return caseCount > 0 ? 100.0 * passCount / caseCount : 0.0;
    }
};
...
struct MatrixValidationReport
{
    ...
    static MatrixValidationReport CreateMock(bool allPass = true)
    {
        ...
        const ValidationDomain domains[] = {ValidationDomain::Decoder, ValidationDomain::Memory, ValidationDomain::Cache,
                                            ValidationDomain::Plugin, ValidationDomain::Pipeline};
...
enum class QATestCategory : uint8_t {
    Unit = 0,
    Integration,
    Performance,
    Fuzz,
...
enum class QADefectSeverity : uint8_t {
    Critical = 0,
    High,
    Medium,
    Low,
...
enum class QAShipSignal : uint8_t {
    Ship = 0,
    ConditionalShip,
    Hold,
    Block,
...
class QualityAssuranceV2
{
  public:
    ...
    {
        switch (c) {
            case QATestCategory::Unit:
                return L"Unit Tests";
            case QATestCategory::Integration:
                return L"Integration Tests";
            case QATestCategory::Performance:
                return L"Performance Tests";
            case QATestCategory::Fuzz:
                return L"Fuzz Tests";
            case QATestCategory::Regression:
                return L"Regression Tests";
            case QATestCategory::Accessibility:
                return L"Accessibility Tests";
            default:
                return L"Unknown";
        }
    }
    ...
    {
        switch (s) {
            case QADefectSeverity::Critical:
                return L"Critical";
            case QADefectSeverity::High:
                return L"High";
            case QADefectSeverity::Medium:
                return L"Medium";
            case QADefectSeverity::Low:
                return L"Low";
            case QADefectSeverity::Info:
                return L"Info";
            default:
                return L"Unknown";
        }
    ...
    {
        switch (s) {
            case QAShipSignal::Ship:
                return L"SHIP";
            case QAShipSignal::ConditionalShip:
                return L"CONDITIONAL SHIP";
            case QAShipSignal::Hold:
                return L"HOLD";
            case QAShipSignal::Block:
                return L"BLOCK";
            default:
                return L"Unknown";
        }
    }
    static constexpr size_t TestCategoryCount()
    {
        return static_cast<size_t>(QATestCategory::COUNT);
    }
    static constexpr size_t DefectSeverityCount()
    {
        return static_cast<size_t>(QADefectSeverity::COUNT);
    }
    static constexpr size_t ShipSignalCount()
    {
        return static_cast<size_t>(QAShipSignal::COUNT);
    }
    static QAShipSignal Evaluate(const QAFinalMatrix& m)
    {
        if (m.criticalDefects > 0)
            return QAShipSignal::Block;
        if (m.overallPassPct < 99.0f)
            return QAShipSignal::Hold;
        if (m.defectDensity > 0.5f)
            return QAShipSignal::ConditionalShip;
...
{
    const char* warningId = nullptr;  ///< e.g., "C26495" or "cppcoreguidelines-init-variables"
...
class StaticAnalysisGate
{
  public:
    ...
    bool CheckGate(uint32_t securityWarnings, uint32_t correctnessWarnings, uint32_t concurrencyWarnings) const
    {
        if (m_thresholds.failOnSecurityWarning && securityWarnings > m_thresholds.maxSecurityWarnings)
            return false;
        if (m_thresholds.failOnCorrectnessWarning && correctnessWarnings > m_thresholds.maxCorrectnessWarnings)
            return false;
        if (m_thresholds.failOnConcurrencyWarning && concurrencyWarnings > m_thresholds.maxConcurrencyWarnings)
            return false;
        return true;
    }
    ...
    static const WarningSuppression* GetSuppressions()
    {
        static const WarningSuppression suppressions[] = {
            ...
            {"C26812", nullptr, "Prefer enum class — already using enum class everywhere in new code",
             WarningSeverity::Warning, WarningCategory::Modernization, false, nullptr},
            {"C6387", "LENSShell/*", "SAL annotations sometimes over-report on COM interfaces",
             WarningSeverity::Warning, WarningCategory::Correctness, true, ""},
            {"cppcoreguidelines-owning-memory", nullptr, "COM raw pointers managed by AddRef/Release",
             WarningSeverity::Note, WarningCategory::Readability, false, nullptr},
            {"performance-no-int-to-ptr", nullptr, "MAKEINTRESOURCE and Windows API macros require int-to-ptr",
    ...
    const AnalysisThresholds& GetThresholds() const
    {
        return m_thresholds;
    }
    void SetThresholds(const AnalysisThresholds& t)
    {
        m_thresholds = t;
    }
