...
DocGenerationResult AutoDocGenerator::GenerateAll(const std::wstring& outputDir, DocFormat format)
{
    ...
    for (uint8_t i = 0; i < static_cast<uint8_t>(DocSection::SectionCount); i++) {
        ...
        if (!content.empty()) {
            ...
            result.files.push_back(outputDir + L"\\" + GetSectionName(section) + GetFormatExtension(format));
        }
    }
...
const wchar_t* AutoDocGenerator::GetSectionName(DocSection section)
{
    switch (section) {
        case DocSection::Overview:
            return L"Overview";
        case DocSection::Installation:
            return L"Installation";
        case DocSection::Configuration:
            return L"Configuration";
        case DocSection::Decoders:
            return L"Decoders";
        case DocSection::GPUPipeline:
            return L"GPU Pipeline";
        case DocSection::PluginAPI:
            return L"Plugin API";
        case DocSection::Testing:
            return L"Testing";
        case DocSection::Performance:
            return L"Performance";
        case DocSection::Changelog:
            return L"Changelog";
        default:
            return L"Unknown";
    }
...
const wchar_t* AutoDocGenerator::GetFormatName(DocFormat format)
{
    switch (format) {
        ...
        case DocFormat::HTML:
            return L"HTML";
        case DocFormat::ReStructuredText:
            return L"reStructuredText";
        case DocFormat::AsciiDoc:
            return L"AsciiDoc";
        default:
            return L"Unknown";
    }
