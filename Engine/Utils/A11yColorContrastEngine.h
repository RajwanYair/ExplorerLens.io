...
enum class A11yWCAGLevel {
    AA_Normal,
    AA_Large,
    AAA_Normal,
    AAA_Large
};
enum class ContrastDecision {
    Pass,
    Fail
};
  ...
  public:
    ...
    A11yContrastResult Evaluate(ColorRGBA8 fg, ColorRGBA8 bg, A11yWCAGLevel level) const
    {
        ...
        std::string summary = std::string(dec == ContrastDecision::Pass ? "PASS" : "FAIL") + " "
                              + std::to_string(static_cast<int>(ratio * 10) / 10.0).substr(0, 3) + ":1 (need "
    ...
    {
        switch (level) {
            case A11yWCAGLevel::AA_Normal:
                return WCAGThresholds::AA_NORMAL;
            case A11yWCAGLevel::AA_Large:
                return WCAGThresholds::AA_LARGE;
            case A11yWCAGLevel::AAA_Normal:
                return WCAGThresholds::AAA_NORMAL;
            case A11yWCAGLevel::AAA_Large:
                return WCAGThresholds::AAA_LARGE;
        }
        return 4.5;
    }
