...
const ArgDefinition* CommandLineInterface::FindDefinition(const std::wstring& name) const
{
    for (const auto& def : m_definitions) {
        if (def.longName == name || def.shortName == name)
            return &def;
        // Also match normalized
        if (NormalizeName(def.longName) == NormalizeName(name))
            return &def;
    }
    return nullptr;
}
...
std::wstring CommandLineInterface::GenerateHelp() const
{
    ...
    for (const auto& def : m_definitions) {
        ...
        if (def.type != ArgType::Flag)
            ss << L" <" << GetArgTypeName(def.type) << L">";
        ss << L"\n " << def.description;
        if (def.required)
            ss << L" (required)";
        if (!def.defaultValue.empty())
            ss << L" [default: " << def.defaultValue << L"]";
        ss << L"\n";
    }
    return ss.str();
}
...
const wchar_t* CommandLineInterface::GetParseStatusName(ParseStatus status)
{
    switch (status) {
        case ParseStatus::Success:
            return L"Success";
        case ParseStatus::UnknownArgument:
            return L"Unknown Argument";
        case ParseStatus::MissingValue:
            return L"Missing Value";
        case ParseStatus::MissingRequired:
            return L"Missing Required";
        case ParseStatus::InvalidValue:
            return L"Invalid Value";
        case ParseStatus::HelpRequested:
            return L"Help Requested";
        default:
            return L"Unknown";
    }
}
