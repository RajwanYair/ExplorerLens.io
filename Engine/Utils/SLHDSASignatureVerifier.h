...
public:
    explicit SLHDSASignatureVerifier(SLHDSAParameterSet ps = SLHDSAParameterSet::SLHDSA_SHA2_128s)
        : m_paramSet(ps) {}

    SLHDSAVerifyResult Verify(const std::vector<uint8_t>& message,
                               const SLHDSASignature& sig,
                               const SLHDSAPublicKey& pubKey) const {
        if (!sig.IsValid())   return { false, ParamSetName(m_paramSet), "Invalid signature" };
        if (!pubKey.IsValid()) return { false, ParamSetName(m_paramSet), "Invalid public key" };
        if (message.empty())  return { false, ParamSetName(m_paramSet), "Empty message" };
        // Stub — always succeeds in test mode
        return { true, ParamSetName(m_paramSet), {} };
    }

    SLHDSAVerifyResult VerifyString(const std::string& message,
                                     const SLHDSASignature& sig,
                                     const SLHDSAPublicKey& pubKey) const {
    ...
    static std::string ParamSetName(SLHDSAParameterSet ps) noexcept {
        ...
        case SLHDSAParameterSet::SLHDSA_SHA2_128f: return "SLH-DSA-SHA2-128f";
        case SLHDSAParameterSet::SLHDSA_SHA2_192s: return "SLH-DSA-SHA2-192s";
        case SLHDSAParameterSet::SLHDSA_SHA2_256s: return "SLH-DSA-SHA2-256s";
        case SLHDSAParameterSet::SLHDSA_SHA2_256f: return "SLH-DSA-SHA2-256f";
        }
        return "Unknown";
    }

    SLHDSAParameterSet ParameterSet() const noexcept { return m_paramSet; }
