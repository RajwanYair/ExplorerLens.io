...
struct OAuthValidationResult {
    ...
    TokenClaims           claims;
    std::string           errorMsg;
    bool Ok() const noexcept { return status == TokenValidationStatus::Valid; }

    std::string StatusName() const noexcept {
        switch (status) {
        case TokenValidationStatus::Valid:            return "Valid";
        case TokenValidationStatus::Expired:          return "Expired";
        case TokenValidationStatus::InvalidSignature: return "InvalidSignature";
        case TokenValidationStatus::MalformedToken:   return "MalformedToken";
        case TokenValidationStatus::MissingIssuer:    return "MissingIssuer";
        case TokenValidationStatus::Revoked:          return "Revoked";
        }
        return "Unknown";
    }
};
...
public:
    explicit OAuthTokenValidator(
        std::string expectedIssuer   = "https://login.microsoftonline.com",
        std::string expectedAudience = "ExplorerLens",
    ...
    void SetAllowedScope(const std::string& scope) { m_allowedScopes.push_back(scope); }
    bool HasRequiredScope(const TokenClaims& claims, const std::string& scope) const noexcept {
        for (const auto& s : claims.scopes) if (s == scope) return true;
        return false;
    }
