...
{
  public:
    explicit QuantumSafeKeyRotator(KeyRotationPolicy policy = {}) : m_policy(std::move(policy)) {}
    ...
    {
        if (!m_policy.autoRotate)
            return false;
        if (rec.encapCount >= m_policy.maxEncapsulations)
            return true;
        auto now =
            std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    ...
    const KeyRotationPolicy& Policy() const noexcept
    {
        return m_policy;
    }
