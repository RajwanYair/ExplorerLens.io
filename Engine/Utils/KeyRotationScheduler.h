...
class KeyRotationScheduler
{
  public:
    ...
    void Schedule(const std::string& keyId, uint32_t intervalDays, int64_t firstRotationMs, uint32_t jitterMs = 300000)
    {
    ...
    void SetDueCallback(RotationDueCallback cb)
    {
    ...
    uint32_t Tick(int64_t nowMs)
    {
        ...
        for (auto& e : m_schedule) {
            ...
            if (nowMs >= e.nextRotationMs) {
                if (m_callback)
                    m_callback(e.keyId);
                e.nextRotationMs = nowMs + static_cast<int64_t>(e.intervalDays) * 86400000LL
