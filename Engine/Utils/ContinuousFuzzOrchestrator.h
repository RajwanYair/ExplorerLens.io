...
{
    static const char* names[] = {"BitFlip",       "ByteReplace",    "BlockShuffle", "HeaderCorrupt",
...
{
    static const char* names[] = {"None", "Info", "Low", "Medium", "High", "Critical"};
    auto idx = static_cast<uint8_t>(s);
...
{
    static const char* names[] = {"Import", "Minimize", "Merge", "Export", "Distill"};
...
struct FuzzCampaignStats
{
    ...
    {
        return (totalExecutions > 0) ? (static_cast<double>(uniqueCrashes) / totalExecutions * 1000000.0) : 0.0;
    }
};
...
class ContinuousFuzzOrchestrator
{
  public:
    ...
    void RecordCrash(const std::string& decoder, CrashSeverity severity, const std::string& inputHash,
                     uint64_t inputSize)
    {
        // Check for duplicate
    ...
    const FuzzCampaignStats& GetStats() const
    {
        return m_stats;
    }

    /// Get all crashes
    const std::vector<FuzzCrashReport>& GetCrashes() const
    {
        return m_crashes;
    }

    /// Get corpus entries
    const std::vector<CorpusEntry>& GetCorpus() const
    {
        return m_corpus;
    }
    ...
    uint32_t MinimizeCorpus()
    {
        ...
        m_corpus.erase(
            std::remove_if(m_corpus.begin(), m_corpus.end(), [](const CorpusEntry& e) { return !e.isInteresting; }),
            m_corpus.end());
        m_stats.corpusSize = m_corpus.size();
    ...
    /// Get target decoders
    const std::vector<std::string>& GetTargetDecoders() const
    {
        return m_targetDecoders;
    }
