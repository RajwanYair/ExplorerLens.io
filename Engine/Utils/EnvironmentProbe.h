...
inline const char* CIProviderToString(CIProvider p)
{
    switch (p) {
        ...
        case CIProvider::AzureDevOps:
            return "Azure DevOps";
        case CIProvider::Jenkins:
            return "Jenkins";
        case CIProvider::Other:
            return "Other";
        default:
            return "None";
    }
...
inline CIProvider DetectCIProvider()
{
    ...
    if (!SafeGetEnvVar("JENKINS_URL").empty())
...
struct ProbedWindowsVersion
{
    ...
    std::string ToString() const
    {
        if (!detected)
            return "(unknown)";
        return "Windows " + std::to_string(majorVersion) + "." + std::to_string(minorVersion) + " build "
    ...
    bool IsWindows10_1809OrLater() const
    {
        if (!detected)
            return false;
        return (majorVersion > 10) || (majorVersion == 10 && buildNumber >= 17763);
    }
};
...
{
    switch (level) {
        case GPUFeatureLevel::D3D11_FeatureLevel_9_1:
            return "9.1";
        case GPUFeatureLevel::D3D11_FeatureLevel_10_0:
            return "10.0";
        case GPUFeatureLevel::D3D11_FeatureLevel_11_0:
            return "11.0";
        case GPUFeatureLevel::D3D11_FeatureLevel_11_1:
            return "11.1";
        case GPUFeatureLevel::D3D11_FeatureLevel_12_0:
            return "12.0";
        case GPUFeatureLevel::D3D11_FeatureLevel_12_1:
            return "12.1";
        default:
            return "None";
    }
...
struct GPUCapabilities
{
    ...
    std::string ToString() const
    {
        ...
        return "GPU: " + adapterDescription + " (Feature Level " + GPUFeatureLevelToString(featureLevel) + ")";
    }
};
...
inline GPUCapabilities DetectGPUCapabilities()
{
    ...
    typedef HRESULT(WINAPI * PFN_EL_D3D11CreateDevice)(void* pAdapter,
                                                       UINT DriverType,  // D3D_DRIVER_TYPE
                                                       HMODULE Software, UINT Flags, const UINT* pFeatureLevels,
                                                       UINT FeatureLevels, UINT SDKVersion, void** ppDevice,
                                                       UINT* pFeatureLevel, void** ppImmediateContext);

    auto pD3D11CreateDevice = reinterpret_cast<PFN_EL_D3D11CreateDevice>(GetProcAddress(hD3D11, "D3D11CreateDevice"));
    ...
    if (pD3D11CreateDevice) {
        ...
        HRESULT hr = pD3D11CreateDevice(nullptr,  // default adapter
                                        1,        // D3D_DRIVER_TYPE_HARDWARE
                                        nullptr,  // no software module
                                        0,        // flags
                                        featureLevels, _countof(featureLevels),
                                        7,  // D3D11_SDK_VERSION
                                        &pDevice, &achievedLevel, &pContext);
        ...
        if (SUCCEEDED(hr) && pDevice) {
            ...
            switch (achievedLevel) {
                case 0xc100:
                    caps.featureLevel = GPUFeatureLevel::D3D11_FeatureLevel_12_1;
                    break;
                case 0xc000:
                    caps.featureLevel = GPUFeatureLevel::D3D11_FeatureLevel_12_0;
                    break;
                case 0xb100:
                    caps.featureLevel = GPUFeatureLevel::D3D11_FeatureLevel_11_1;
                    break;
                case 0xb000:
                    caps.featureLevel = GPUFeatureLevel::D3D11_FeatureLevel_11_0;
                    break;
                case 0xa000:
                    caps.featureLevel = GPUFeatureLevel::D3D11_FeatureLevel_10_0;
                    break;
                case 0x9100:
                    caps.featureLevel = GPUFeatureLevel::D3D11_FeatureLevel_9_1;
                    break;
                default:
                    caps.featureLevel = GPUFeatureLevel::None;
                    break;
            }
...
inline DisplayDPIInfo DetectDisplayDPI()
{
    ...
    if (hUser32) {
        ...
        auto pGetDpi = reinterpret_cast<GetDpiForSystemPtr>(GetProcAddress(hUser32, "GetDpiForSystem"));
        if (pGetDpi) {
            UINT dpi = pGetDpi();
            info.dpiX = dpi;
            info.dpiY = dpi;
