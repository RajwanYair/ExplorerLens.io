...
class MalformedInputHandler
{
  public:
    ...
    static bool CheckMagicBytes(const uint8_t* data, uint32_t size, const std::wstring& expectedFormat);

    /// Get safe decode dimensions (clamped)
    static void ClampDimensions(uint32_t& width, uint32_t& height, uint32_t maxWidth, uint32_t maxHeight);

    /// Get config
    const MalformedInputConfig& GetConfig() const
    {
        return m_config;
    }
