...
struct TierEntitlements
{
    ...
    static constexpr uint32_t Pro = Community | static_cast<uint32_t>(LicenseFeature::GpuAcceleration)
                                    | static_cast<uint32_t>(LicenseFeature::AdvancedFormats)
                                    | static_cast<uint32_t>(LicenseFeature::BatchProcessing);
    static constexpr uint32_t Enterprise =
        Pro | static_cast<uint32_t>(LicenseFeature::EnterpriseGPO) | static_cast<uint32_t>(LicenseFeature::CloudSync)
        | static_cast<uint32_t>(LicenseFeature::AISmartCrop) | static_cast<uint32_t>(LicenseFeature::PrioritySupport);
...
class LicenseManager
{
  public:
    ...
    {
        HKEY hk = nullptr;
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\License", 0, KEY_READ, &hk) != ERROR_SUCCESS)
            return false;
        ...
        if (RegQueryValueExW(hk, L"KeyHash", nullptr, &type, reinterpret_cast<BYTE*>(buf), &sz) == ERROR_SUCCESS)
            m_info.keyHash = buf;

        sz = sizeof(buf);
        if (RegQueryValueExW(hk, L"Licensee", nullptr, &type, reinterpret_cast<BYTE*>(buf), &sz) == ERROR_SUCCESS)
            m_info.licensee = buf;

        DWORD tierVal = 0;
        sz = sizeof(tierVal);
        if (RegQueryValueExW(hk, L"Tier", nullptr, &type, reinterpret_cast<BYTE*>(&tierVal), &sz) == ERROR_SUCCESS)
    ...
    {
        HKEY hk = nullptr;
        RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\License", 0, nullptr, 0, KEY_WRITE, nullptr, &hk,
                        nullptr);
        if (!hk)
            return false;

        RegSetValueExW(hk, L"KeyHash", 0, REG_SZ, reinterpret_cast<const BYTE*>(info.keyHash.c_str()),
                       static_cast<DWORD>((info.keyHash.size() + 1) * sizeof(wchar_t)));
        RegSetValueExW(hk, L"Licensee", 0, REG_SZ, reinterpret_cast<const BYTE*>(info.licensee.c_str()),
                       static_cast<DWORD>((info.licensee.size() + 1) * sizeof(wchar_t)));
        DWORD tierVal = static_cast<DWORD>(info.tier);
        RegSetValueExW(hk, L"Tier", 0, REG_DWORD, reinterpret_cast<const BYTE*>(&tierVal), sizeof(DWORD));
        RegCloseKey(hk);
    ...
    {
        if (m_info.tier != LicenseTier::Trial)
            return false;
        SYSTEMTIME now = {};
        GetLocalTime(&now);
        ...
        uExp.LowPart = ftExp.dwLowDateTime;
        uExp.HighPart = ftExp.dwHighDateTime;
    ...
    {
        HKEY hk = nullptr;
        RegCreateKeyExW(HKEY_LOCAL_MACHINE, L"Software\\ExplorerLens\\Trial", 0, nullptr, 0, KEY_READ | KEY_WRITE,
                        nullptr, &hk, nullptr);
        if (!hk)
            return false;
        DWORD started = 0;
        DWORD sz = sizeof(started);
        RegQueryValueExW(hk, L"Started", nullptr, nullptr, reinterpret_cast<BYTE*>(&started), &sz);
        if (started) {
            RegCloseKey(hk);
            return false;
        }  // Trial already used
        started = 1;
        RegSetValueExW(hk, L"Started", 0, REG_DWORD, reinterpret_cast<const BYTE*>(&started), sizeof(DWORD));
        RegCloseKey(hk);
    ...
    void RefreshEntitlements()
    {
        switch (m_info.tier) {
            case LicenseTier::Pro:
                m_entitlements = TierEntitlements::Pro;
                break;
            case LicenseTier::Enterprise:
                m_entitlements = TierEntitlements::Enterprise;
                break;
            case LicenseTier::Trial:
                m_entitlements = TierEntitlements::Trial;
                break;
            default:
                m_entitlements = TierEntitlements::Community;
                break;
        }
        if (m_onChange)
            m_onChange(m_info);
