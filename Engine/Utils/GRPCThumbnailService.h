...
enum class GRPCServiceState { Stopped, Starting, Running, Error };
enum class ThumbnailCodec     { PNG, JPEG, WEBP, AVIF };
...
public:
    ...
    explicit GRPCThumbnailService(GRPCServiceConfig config = {})
    ...
    bool Start() {
        if (m_state == GRPCServiceState::Running) return true;
        m_state = GRPCServiceState::Running;
        return true;
    }

    void Stop() noexcept { m_state = GRPCServiceState::Stopped; }
    bool IsRunning() const noexcept { return m_state == GRPCServiceState::Running; }
    GRPCServiceState State() const noexcept { return m_state; }
    const GRPCServiceConfig& Config() const noexcept { return m_config; }

    GRPCThumbnailResponse HandleRequest(const GRPCThumbnailRequest& req) {
        if (!m_handler) return { false, {}, 0, 0, ThumbnailCodec::PNG, "No handler" };
        return m_handler(req);
    }

    std::string ServiceName() const noexcept { return "ExplorerLens.ThumbnailService"; }
