...
enum class FIPSAlgorithmStatus { Approved, Deprecated, NonApproved };
...
struct FIPSAlgorithmEntry {
    ...
    std::string StatusName() const noexcept {
        switch (status) {
        case FIPSAlgorithmStatus::Approved:    return "Approved";
        case FIPSAlgorithmStatus::Deprecated:  return "Deprecated";
        case FIPSAlgorithmStatus::NonApproved: return "NonApproved";
        }
        return "Unknown";
    }
};
...
public:
    ...
    std::vector<std::string> ApprovedAlgorithms() const {
        ...
        for (const auto& [name, entry] : m_algorithms)
            if (entry.status == FIPSAlgorithmStatus::Approved) result.push_back(name);
        return result;
    }

    size_t AlgorithmCount() const noexcept { return m_algorithms.size(); }
    bool IsFIPSMode() const noexcept { return m_fipsMode; }
    void SetFIPSMode(bool enabled) noexcept { m_fipsMode = enabled; }
