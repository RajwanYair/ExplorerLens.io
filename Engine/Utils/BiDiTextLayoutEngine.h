...
class BiDiTextLayoutEngine
{
  public:
    ...
    BiDiLayoutResult Analyse(const std::wstring& text, BiDiBaseDirection baseDir = BiDiBaseDirection::Auto) const
    {
        if (text.empty())
            return {false, {}, baseDir, 0, "Empty text"};
  ...
  private:
    static BiDiBaseDirection ResolveDirection(const std::wstring& text, BiDiBaseDirection baseDir) noexcept
    {
        if (baseDir != BiDiBaseDirection::Auto)
            return baseDir;
        return ContainsRTL(text) ? BiDiBaseDirection::RTL : BiDiBaseDirection::LTR;
    }

    static std::vector<BiDiRun> SegmentRuns(const std::wstring& text, BiDiBaseDirection dir)
    {
