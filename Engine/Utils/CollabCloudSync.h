...
enum class CloudSyncTarget { SharePoint, OneDrive, OneDriveBusiness };
enum class CollabSyncDirection    { Upload, Download, Bidirectional };
enum class SyncStatus       { Idle, Syncing, Success, Conflict, Error };
...
struct CollabSyncResult {
    ...
    int          conflicts     = 0;
    std::string  errorMsg;
    bool Ok() const noexcept { return status == SyncStatus::Success; }
};
...
using CloudHttpPostFn = std::function<bool(const std::string& url,
...
public:
    ...
    void SetHttpFunction(CloudHttpPostFn fn) { m_httpFn = std::move(fn); }
    ...
    CollabSyncResult Sync(const CloudSyncRequest& request) {
        ...
        const bool httpOk = m_httpFn(
            BuildGraphUrl(request),
            request.accessToken,
    ...
    CloudSyncStats GetStats() const noexcept { return m_stats; }
    void ResetStats() noexcept { m_stats = {}; }

    std::string TargetName(CloudSyncTarget t) const noexcept {
        switch (t) {
        case CloudSyncTarget::SharePoint:       return "SharePoint";
        case CloudSyncTarget::OneDrive:         return "OneDrive";
        case CloudSyncTarget::OneDriveBusiness: return "OneDriveBusiness";
        }
        return "Unknown";
    }
