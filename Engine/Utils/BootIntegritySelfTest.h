...
enum class IntegrityTestResult {
    Pass,
    Skip,
    Fail,
    NotRun
...
struct IntegrityTestCase
{
    ...
    {
        return result == IntegrityTestResult::Fail;
    }
};
  ...
  public:
    ...
    BootIntegrityReport RunAll()
    {
        ...
        for (auto& t : m_tests) {
            ...
            t.elapsedMs = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count();
    ...
    const BootIntegrityReport& LastReport() const noexcept
    {
        return m_lastReport;
    }
    ...
    int TestCount() const noexcept
    {
        return (int)m_tests.size();
    }
    bool IsHealthy() const noexcept
    {
        return m_lastReport.IsHealthy();
    }
    void Reset() noexcept
    {
        m_tests.clear();
