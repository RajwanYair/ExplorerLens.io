...
enum class NavigationDirection { Up, Down, Left, Right, Next, Previous, First, Last };
enum class KeyboardModifier    { None = 0, Shift = 1, Ctrl = 2, Alt = 4 };
...
struct NavigationResult {
    ...
    std::string errorMsg;
    bool Ok() const noexcept { return found; }
};
...
public:
    ...
    NavigationResult Navigate(const std::string& currentId, NavigationDirection dir) const {
        ...
        if (it == m_nodes.end()) return { false, {}, false, "Node not found: " + currentId };
        ...
        if (nit == it->second.neighbors.end()) {
            ...
            if (dir == NavigationDirection::Next || dir == NavigationDirection::Previous) {
                ...
                if (!wrapped.empty()) return { true, wrapped, true, {} };
            }
    ...
    std::vector<std::string> TabOrder() const {
        ...
        for (const auto& [id, n] : m_nodes) if (!n.disabled) order.push_back({ n.tabIndex, id });
        std::sort(order.begin(), order.end());
        std::vector<std::string> result;
        for (auto& [_, id] : order) result.push_back(id);
        return result;
    }
