...
MappedFileInfo MemorySafetyIntegration::MapFile(const std::wstring& filePath, AccessPattern pattern) const
{
    MappedFileInfo info;
...
SafeBuffer MemorySafetyIntegration::CreateSafeBuffer(const uint8_t* data, size_t size)
{
    SafeBuffer buffer;
    if (data && size > 0) {
...
bool MemorySafetyIntegration::ValidateAccess(const SafeBuffer& buffer, size_t offset, size_t length)
{
...
MemorySafetyReport MemorySafetyIntegration::RunDecoderSafetyCheck(const std::wstring& decoderName) const
{
    (void)decoderName;
...
const wchar_t* MemorySafetyIntegration::GetSanitizerName(SanitizerMode mode)
{
    switch (mode) {
        ...
        case SanitizerMode::MemorySanitizer:
            return L"MemorySanitizer";
        case SanitizerMode::UndefinedBehavior:
            return L"UndefinedBehavior";
        case SanitizerMode::ThreadSanitizer:
            return L"ThreadSanitizer";
        default:
            return L"Unknown";
    }
}
