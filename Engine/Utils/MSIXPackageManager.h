...
{
    return static_cast<PackageCapability>(static_cast<uint32_t>(a) | static_cast<uint32_t>(b));
}
...
struct MSIXConfig
{
    ...
    std::wstring description = L"GPU-accelerated thumbnail provider for 200+ formats";
    ...
    PackageCapability capabilities =
        PackageCapability::RunFullTrust | PackageCapability::ShellExtension | PackageCapability::COMServer;
...
class MSIXPackageManager
{
  public:
    ...
    bool SignPackage(const std::wstring& packagePath, const std::wstring& certPath) const;
    ...
    /// Get config
    const MSIXConfig& GetConfig() const
    {
        return m_config;
    }
