...
enum class ScreenReaderType    { NVDA, JAWS, Narrator, VoiceOver, Orca, Unknown };
enum class LiveRegionPoliteness { Off, Polite, Assertive, Rude };
...
public:
    ...
    void SetAnnounceFn(AnnounceFn fn) { m_announceFn = std::move(fn); }
    void SetActiveReader(ScreenReaderType type) noexcept { m_activeReader = type; }
    void SetAvailable(bool available) noexcept { m_available = available; }

    bool Announce(const std::string& text,
                  LiveRegionPoliteness politeness = LiveRegionPoliteness::Polite) {
        if (!m_available || m_activeReader == ScreenReaderType::Unknown) return false;
        ScreenReaderAnnouncement ann { text, politeness, false, true };
        if (m_announceFn) return m_announceFn(ann);
        return false;
    }

    bool AnnounceThumbnailReady(const std::wstring& fileName) {
        std::string msg = "Thumbnail ready: ";
        for (wchar_t c : fileName) msg += (c < 128) ? static_cast<char>(c) : '?';
        return Announce(msg, LiveRegionPoliteness::Polite);
    }

    bool IsAvailable() const noexcept { return m_available; }
    ScreenReaderType ActiveReader() const noexcept { return m_activeReader; }

    static std::string ReaderName(ScreenReaderType t) noexcept {
        switch (t) {
        case ScreenReaderType::NVDA:     return "NVDA";
        case ScreenReaderType::JAWS:     return "JAWS";
        case ScreenReaderType::Narrator: return "Narrator";
        case ScreenReaderType::VoiceOver: return "VoiceOver";
        case ScreenReaderType::Orca:     return "Orca";
        case ScreenReaderType::Unknown:  return "Unknown";
        }
        return "Unknown";
    }
