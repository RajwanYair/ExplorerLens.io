...
enum class AccessibilityPlatform {
    UIAutomation,
    ATSPI2,
    NSAccessibility
};
...
class AccessibilityNavigatorV3
{
  public:
    ...
    void SetInvokeCallback(AccessibilityInvokeCallback cb)
    {
        m_invokeCallback = std::move(cb);
    }

    bool Invoke(const std::string& automationId)
    {
        auto hit = FindById(automationId);
        if (!hit.found || !hit.element.isEnabled)
            return false;
        if (m_invokeCallback)
            m_invokeCallback(hit.element);
        return true;
    }
    ...
    AccessibilityPlatform Platform() const noexcept
    {
        return m_platform;
    }
    ...
    {
        switch (t) {
            case ANV3ControlType::Button:
                return "Button";
            case ANV3ControlType::List:
                return "List";
            case ANV3ControlType::ListItem:
                return "ListItem";
            case ANV3ControlType::DataGrid:
                return "DataGrid";
            case ANV3ControlType::DataItem:
                return "DataItem";
            case ANV3ControlType::Text:
                return "Text";
            case ANV3ControlType::Edit:
                return "Edit";
            default:
                return "Control";
        }
