...
enum class PlatformOS { Windows, MacOS, Linux, Unknown };
...
public:
    ...
    const PlatformCapabilities& GetCapabilities() const { return m_caps; }

    bool SupportsGPUAcceleration() const {
        return m_caps.hasDirectX11 || m_caps.hasDirectX12
            || m_caps.hasVulkan    || m_caps.hasMetal;
    }
    ...
    std::string GetPlatformSummary() const {
        std::string s = "OS=";
        switch (m_caps.os) {
        case PlatformOS::Windows: s += "Windows"; break;
        case PlatformOS::MacOS:   s += "macOS";   break;
        case PlatformOS::Linux:   s += "Linux";   break;
