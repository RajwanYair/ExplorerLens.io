...
bool AuditLogger::ReadEnabledFromRegistry() const
{
 ...
 DWORD type = 0;
 result = RegQueryValueExW(hKey, L"AuditLogging", nullptr,
...
void AuditLogger::LogEvent(AuditEvent event, const std::wstring& details)
{
 ...
 log_file_ << FormatTimestamp()
 << L" [" << EventLabel(event) << L"] "
 << details << L"\n";
 log_file_.flush();
}
...
void AuditLogger::LogPluginUsage(const std::wstring& pluginId,
...
void AuditLogger::RotateIfNeeded()
{
 ...
 dir / (stem + L"." + std::to_wstring(MAX_ROTATED_FILES) + ext);
 ...
 dir / (stem + L"." + std::to_wstring(i) + ext);
 std::filesystem::path dst = 
 dir / (stem + L"." + std::to_wstring(i + 1) + ext);
...
const wchar_t* AuditLogger::EventLabel(AuditEvent event)
{
 switch (event)
 {
 case AuditEvent::ThumbnailRequested: return L"THUMBNAIL_REQ";
 case AuditEvent::ThumbnailGenerated: return L"THUMBNAIL_OK";
 case AuditEvent::ThumbnailFailed: return L"THUMBNAIL_FAIL";
 case AuditEvent::PluginLoaded: return L"PLUGIN_LOAD";
 case AuditEvent::PluginUnloaded: return L"PLUGIN_UNLOAD";
 case AuditEvent::PluginFailed: return L"PLUGIN_ERROR";
 case AuditEvent::PluginCrashed: return L"PLUGIN_CRASH";
 case AuditEvent::CacheHit: return L"CACHE_HIT";
 case AuditEvent::CacheMiss: return L"CACHE_MISS";
 case AuditEvent::CachePurge: return L"CACHE_PURGE";
 case AuditEvent::DecoderError: return L"DECODER_ERROR";
 case AuditEvent::SecurityViolation: return L"SECURITY";
 case AuditEvent::ConfigurationChanged: return L"CONFIG";
