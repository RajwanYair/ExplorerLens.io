...
struct HighContrastColors
{
    ...
    double ContrastRatio(COLORREF fg, COLORREF bg) const
    {
        auto luminance = [](COLORREF c) -> double {
            ...
            return 0.2126 * sRGB(GetRValue(c)) + 0.7152 * sRGB(GetGValue(c)) + 0.0722 * sRGB(GetBValue(c));
        };
  ...
  public:
    ...
    HCTheme DetectTheme() const
    {
        ...
        if (hc.lpszDefaultScheme) {
            ...
            if (scheme.find(L"White") != std::wstring::npos && scheme.find(L"Black") != std::wstring::npos) {
    ...
    COLORREF AdaptColor(COLORREF original) const
    {
        ...
        int lum = (GetRValue(original) * 299 + GetGValue(original) * 587 + GetBValue(original) * 114) / 1000;
