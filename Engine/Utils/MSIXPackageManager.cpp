//==============================================================================
// MSIXPackageManager.cpp — Sprint 195
...
std::wstring MSIXPackageManager::GenerateManifest() const {
    ...
    xml += L"<Package\n";
    xml += L"  xmlns=\"http://schemas.microsoft.com/appx/manifest/foundation/windows10\"\n";
    xml += L"  xmlns:uap=\"http://schemas.microsoft.com/appx/manifest/uap/windows10\"\n";
    xml += L"  xmlns:rescap=\"http://schemas.microsoft.com/appx/manifest/foundation/windows10/restrictedcapabilities\"\n";
    xml += L"  xmlns:desktop=\"http://schemas.microsoft.com/appx/manifest/desktop/windows10\"\n";
    xml += L"  xmlns:com=\"http://schemas.microsoft.com/appx/manifest/com/windows10\">\n\n";
    ...
    xml += L"    <PublisherDisplayName>" + m_config.publisherDisplayName + L"</PublisherDisplayName>\n";
    ...
    xml += L"                        MinVersion=\"" + m_config.minWindowsVersion + L"\"\n";
    ...
    if (HasCapability(m_config.capabilities, PackageCapability::COMServer)) {
        ...
        xml += L"      <com:ComServer>\n";
        xml += L"        <com:SurrogateServer DisplayName=\"DarkThumbs Thumbnail Provider\">\n";
        xml += L"          <com:Class Id=\"9E6ECB90-5A61-42BD-B851-D3297D9C7F39\"\n";
        xml += L"                     Path=\"CBXShell.dll\"\n";
...
std::wstring MSIXPackageManager::GenerateAppInstaller() const {
    ...
    if (m_config.includeARM64 && !m_config.includex64) arch = L"arm64";
...
// Build / Sign
//==============================================================================
PackageBuildResult MSIXPackageManager::BuildPackage(const std::wstring& outputDir) const {
...
bool MSIXPackageManager::SignPackage(const std::wstring& packagePath,
    ...
    if (packagePath.empty() || certPath.empty()) return false;
    if (m_config.signing == SigningMode::None) return true;
    // In full implementation: shell out to signtool.exe sign
    return true;
}

bool MSIXPackageManager::ValidateManifest(const std::wstring& manifestXml) const {
    if (manifestXml.empty()) return false;
...
const wchar_t* MSIXPackageManager::GetChannelName(PackageChannel channel) {
    switch (channel) {
    case PackageChannel::Stable:   return L"Stable";
    case PackageChannel::Beta:     return L"Beta";
    case PackageChannel::Dev:      return L"Dev";
    case PackageChannel::Canary:   return L"Canary";
    case PackageChannel::Internal: return L"Internal";
...
const wchar_t* MSIXPackageManager::GetSigningName(SigningMode mode) {
    switch (mode) {
    case SigningMode::None:         return L"None";
    case SigningMode::SelfSigned:   return L"SelfSigned";
    case SigningMode::Authenticode: return L"Authenticode";
    case SigningMode::StoreSigned:  return L"StoreSigned";
    case SigningMode::AzureTrusted: return L"AzureTrusted";
    default: return L"Unknown";
    }
}
...
const wchar_t* MSIXPackageManager::GetPackageTypeName(PackageType type) {
    switch (type) {
    case PackageType::MSIX:               return L"MSIX";
    case PackageType::MSIXBundle:         return L"MSIXBundle";
    case PackageType::AppX:               return L"AppX";
    case PackageType::SparsePackage:      return L"SparsePackage";
    case PackageType::MSIX_Appinstaller:  return L"Appinstaller";
    default: return L"Unknown";
    }
}
