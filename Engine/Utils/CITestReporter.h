...
{
    switch (cat) {
        case CIFailureCategory::Assertion:
            return "Assertion";
        case CIFailureCategory::Timeout:
            return "Timeout";
        case CIFailureCategory::Crash:
            return "Crash";
        case CIFailureCategory::ResourceExhaustion:
            return "ResourceExhaustion";
        case CIFailureCategory::EnvironmentIssue:
...
class CITestReporter
{
  public:
    ...
    explicit CITestReporter(const std::string& suiteName) : m_suiteName(suiteName)
    {
    ...
    void AddResult(const std::string& name, bool passed, double durationMs = 0.0, const std::string& failureMsg = "",
    ...
    {
        auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(m_endTime - m_startTime).count();
        return static_cast<double>(elapsed) / 1000.0;
    }

    const std::vector<CITestEntry>& Results() const
    {
        return m_results;
    }
    ...
    std::string ExportJUnitXML() const
    {
        ...
        xml << "<testsuites tests=\"" << TotalTests() << "\" failures=\"" << FailedTests() << "\" time=\"" << std::fixed
            << std::setprecision(3) << (TotalDurationMs() / 1000.0) << "\">\n";

        xml << "  <testsuite name=\"" << EscapeXML(m_suiteName) << "\" tests=\"" << TotalTests() << "\" failures=\""
            << FailedTests() << "\" time=\"" << std::fixed << std::setprecision(3) << (TotalDurationMs() / 1000.0)
            << "\">\n";

        for (const auto& r : m_results) {
            xml << "    <testcase name=\"" << EscapeXML(r.name) << "\" classname=\""
                << EscapeXML(r.suiteName.empty() ? m_suiteName : r.suiteName) << "\" time=\"" << std::fixed
                << std::setprecision(3) << (r.durationMs / 1000.0) << "\"";

            if (r.passed) {
                xml << "/>\n";
            } else {
                xml << ">\n";
                xml << "      <failure message=\"" << EscapeXML(r.failureMessage) << "\" type=\""
                    << CIFailureCategoryToString(r.failureCategory) << "\"/>\n";
                xml << "    </testcase>\n";
            }
        }
    ...
    std::string ExportJSON() const
    {
        ...
        json << "  \"totalDurationMs\": " << std::fixed << std::setprecision(2) << TotalDurationMs() << ",\n";
        json << "  \"results\": [\n";
        ...
        for (size_t i = 0; i < m_results.size(); ++i) {
            ...
            json << "      \"durationMs\": " << std::fixed << std::setprecision(2) << r.durationMs << ",\n";
    ...
    std::string ExportConsoleSummary() const
    {
        ...
        out << "\n" << BOLD << "======== Test Report: " << m_suiteName << " ========" << RESET << "\n";
