...
class CodeCoverageIntegration
{
  public:
    ...
    std::wstring GenerateCoverageCommand(const std::wstring& testExecutable, const std::wstring& outputDir) const;
    ...
    ModuleCoverage GetModuleCoverage(const std::wstring& moduleName, const CoverageReport& report) const;

    /// Merge multiple coverage reports
    CoverageReport MergeReports(const std::vector<CoverageReport>& reports) const;

    /// Get threshold configuration
    const CoverageThresholds& GetThresholds() const
    {
        return m_thresholds;
    }
