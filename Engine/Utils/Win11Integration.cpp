...
void Win11Integration::DetectOS() {
    ...
    if (m_versionInfo.isWin11_24H2) m_versionInfo.displayName = L"Windows 11 24H2";
    else if (m_versionInfo.isWin11_23H2) m_versionInfo.displayName = L"Windows 11 23H2";
    else if (m_versionInfo.isWin11_22H2) m_versionInfo.displayName = L"Windows 11 22H2";
    else if (m_versionInfo.isWin11) m_versionInfo.displayName = L"Windows 11";
    else m_versionInfo.displayName = L"Windows 10";
...
std::vector<Win11Feature> Win11Integration::GetAvailableFeatures() const {
    ...
    for (uint8_t i = 0; i < static_cast<uint8_t>(Win11Feature::FeatureCount); i++) {
        ...
        if (IsFeatureAvailable(f)) features.push_back(f);
    }
    return features;
}
...
uint32_t Win11Integration::GetAvailableFeatureCount() const {
    uint32_t count = 0;
    for (uint8_t i = 0; i < static_cast<uint8_t>(Win11Feature::FeatureCount); i++) {
        if (IsFeatureAvailable(static_cast<Win11Feature>(i))) count++;
    }
    return count;
}
...
//------------------------------------------------------------------------------
std::vector<SnapZone> Win11Integration::GetSnapZones(
    uint32_t monitorWidth, uint32_t monitorHeight) const
{
    std::vector<SnapZone> zones;
    if (!m_versionInfo.isWin11) return zones;
...
const wchar_t* Win11Integration::GetFeatureName(Win11Feature feature) {
    switch (feature) {
        ...
        case Win11Feature::MicaMaterial:   return L"Mica Material";
        case Win11Feature::SnapLayouts:    return L"Snap Layouts";
        case Win11Feature::Widgets:        return L"Widgets";
        case Win11Feature::DarkMode:       return L"Dark Mode";
        case Win11Feature::DirectStorage:  return L"DirectStorage";
        case Win11Feature::AutoHDR:        return L"Auto HDR";
        case Win11Feature::WindowedGames:  return L"Windowed Games";
...
const wchar_t* Win11Integration::GetMicaModeName(MicaMode mode) {
    switch (mode) {
        case MicaMode::None:    return L"None";
        case MicaMode::Mica:    return L"Mica";
        case MicaMode::MicaAlt: return L"Mica Alt";
        case MicaMode::Acrylic: return L"Acrylic";
        default: return L"Unknown";
    }
}
