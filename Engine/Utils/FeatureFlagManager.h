...
class FeatureFlagManager
{
  public:
    ...
    std::wstring GetString(const std::wstring& name, const std::wstring& fallback = L"") const
    {
        auto rec = Resolve(name);
    ...
    void Reload()
    {
        ...
        for (auto& [name, rec] : m_flags) {
            ...
            if (LoadFromRegistry(HKEY_LOCAL_MACHINE, L"Software\\Policies\\ExplorerLens\\Flags", name, rec)) {
                rec.source = FlagSource::GroupPolicy;
            } else if (LoadFromRegistry(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Flags", name, rec)) {
    ...
    void ApplyCloudPayload(const std::unordered_map<std::wstring, std::wstring>& payload)
    {
        std::lock_guard<std::mutex> lk(m_mtx);
        for (const auto& [key, val] : payload) {
            ...
            if (it->second.source == FlagSource::GroupPolicy)
                continue;
        ...
        for (const auto& [name, rec] : m_flags) {
            ...
            switch (rec.source) {
                case FlagSource::Default:
                    out += L"default";
                    break;
                case FlagSource::UserHKCU:
                    out += L"user";
                    break;
                case FlagSource::CloudConfig:
                    out += L"cloud";
                    break;
                case FlagSource::GroupPolicy:
                    out += L"gpo";
                    break;
            }
            out += L"]\n";
  ...
  private:
    ...
    bool LoadFromRegistry(HKEY root, const wchar_t* subKey, const std::wstring& name, FlagRecord& rec)
    {
        HKEY hk = nullptr;
        if (RegOpenKeyExW(root, subKey, 0, KEY_READ, &hk) != ERROR_SUCCESS)
            return false;
        bool found = false;
        if (std::holds_alternative<bool>(rec.value) || std::holds_alternative<int32_t>(rec.value)) {
            DWORD val = 0, sz = sizeof(val), type = 0;
            if (RegQueryValueExW(hk, name.c_str(), nullptr, &type, reinterpret_cast<BYTE*>(&val), &sz)
                == ERROR_SUCCESS) {
            ...
            wchar_t buf[512] = {};
            DWORD sz = sizeof(buf);
            if (RegQueryValueExW(hk, name.c_str(), nullptr, nullptr, reinterpret_cast<BYTE*>(buf), &sz)
                == ERROR_SUCCESS) {
    ...
    {
        HKEY hk = nullptr;
        RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\ExplorerLens\\Flags", 0, nullptr, 0, KEY_WRITE, nullptr, &hk,
                        nullptr);
        if (!hk)
            return;
        if (type == REG_DWORD) {
            DWORD d = _wtoi(val.c_str());
            RegSetValueExW(hk, name.c_str(), 0, REG_DWORD, reinterpret_cast<const BYTE*>(&d), sizeof(DWORD));
        } else {
            RegSetValueExW(hk, name.c_str(), 0, REG_SZ, reinterpret_cast<const BYTE*>(val.c_str()),
