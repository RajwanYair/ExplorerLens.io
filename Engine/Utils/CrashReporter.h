...
class CrashReporter
{
  public:
    ...
    void SetDumpDirectory(const std::wstring& dir)
    {
        m_dumpDir = dir;
    }
    ...
    {
        m_dumpType = static_cast<DWORD>(type);
    }

    // Update live context (call from decode pipeline)
    void UpdateContext(const CrashContext& ctx)
    {
        m_context = ctx;
    }
    ...
    {
        return WriteDump(GetCurrentProcess(), GetCurrentProcessId(), nullptr, EXCEPTION_EXECUTE_HANDLER, false);
    }
    ...
    static void InvokeCbSafe(std::function<void(const ReporterCrashReport&)>& fn,
  ...
  private:
    ...
    {
        auto& inst = Instance();
        inst.WriteDump(GetCurrentProcess(), GetCurrentProcessId(), ep, EXCEPTION_EXECUTE_HANDLER, true);

        return inst.m_prevFilter ? inst.m_prevFilter(ep) : EXCEPTION_CONTINUE_SEARCH;
    }

    ReporterCrashReport WriteDump(HANDLE hProc, DWORD pid, PEXCEPTION_POINTERS ep, DWORD unusedFlags, bool notifyCbs)
    {
        (void)unusedFlags;
        ...
        swprintf_s(fname, MAX_PATH, L"%s\\ExplorerLens-%04d%02d%02d-%02d%02d%02d.dmp", m_dumpDir.c_str(), st.wYear,
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            report.writtenToDisk = (MiniDumpWriteDump(hProc, pid, hFile, static_cast<MINIDUMP_TYPE>(m_dumpType),
