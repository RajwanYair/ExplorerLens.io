...
bool SIMDScaler::ScaleBGRA(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight, uint32_t srcStride,
                           uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight, uint32_t dstStride,
                           ScalingQuality quality)
{
    ...
    switch (quality) {
        case ScalingQuality::NearestNeighbor:
            ...
            return ScaleBGRA_NearestNeighbor(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                             dstStride);

        case ScalingQuality::Bicubic:
            // Bicubic scaling (higher quality, sharp edges)
            if (cpu.hasAVX2) {
                return ScaleBGRA_Bicubic_AVX2(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                              dstStride);
            } else {
                return ScaleBGRA_Bicubic_Scalar(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                                dstStride);
            }

        case ScalingQuality::Lanczos:
            // Lanczos3 scaling (best quality, minimal ringing)
            return ScaleBGRA_Lanczos3_Scalar(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                             dstStride);
        ...
        default:
            ...
            if (cpu.hasAVX2) {
                return ScaleBGRA_Bilinear_AVX2(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                               dstStride);
            } else if (cpu.hasSSE41) {
                return ScaleBGRA_Bilinear_SSE41(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                                dstStride);
            } else {
                return ScaleBGRA_Bilinear_Scalar(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                                 dstStride);
            }
    }
}
...
bool SIMDScaler::ScaleBGRA_Bilinear_AVX2(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                         uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                         uint32_t dstStride)
{
    ...
    for (uint32_t y = 0; y < dstHeight; ++y) {
        ...
        for (; x < simdCount; x += simdWidth) {
            ...
            __m256 srcXVec = _mm256_set_ps((x + 7) * xRatio, (x + 6) * xRatio, (x + 5) * xRatio, (x + 4) * xRatio,
...
bool SIMDScaler::ScaleBGRA_Bilinear_SSE41(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                          uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                          uint32_t dstStride)
{
    // For now, fall back to scalar implementation
    // A full SSE4.1 implementation would be similar to AVX2 but with 128-bit registers
    return ScaleBGRA_Bilinear_Scalar(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight, dstStride);
}
...
bool SIMDScaler::ScaleBGRA_Bilinear_Scalar(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                           uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                           uint32_t dstStride)
{
    const float xRatio = static_cast<float>(srcWidth - 1) / static_cast<float>(dstWidth);
...
bool SIMDScaler::ScaleBGRA_Bicubic_Scalar(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                          uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                          uint32_t dstStride)
{
    ...
    for (uint32_t dy = 0; dy < dstHeight; ++dy) {
        ...
        for (uint32_t dx = 0; dx < dstWidth; ++dx) {
            ...
            for (int32_t ky = -1; ky <= 2; ++ky) {
                ...
                if (sy < 0)
                    sy = 0;
                if (sy >= static_cast<int32_t>(srcHeight))
                    sy = srcHeight - 1;
                ...
                for (int32_t kx = -1; kx <= 2; ++kx) {
                    ...
                    if (sx < 0)
                        sx = 0;
                    if (sx >= static_cast<int32_t>(srcWidth))
                        sx = srcWidth - 1;
...
bool SIMDScaler::ScaleBGRA_Bicubic_AVX2(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                        uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                        uint32_t dstStride)
{
    // For now, fall back to scalar implementation
    // Full AVX2 bicubic is complex and requires careful horizontal SIMD optimization
    return ScaleBGRA_Bicubic_Scalar(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight, dstStride);
}
...
bool SIMDScaler::ScaleBGRA_NearestNeighbor(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                           uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                           uint32_t dstStride)
{
...
bool SIMDScaler::ScaleBGRA_Lanczos3_Scalar(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                           uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                           uint32_t dstStride)
{
    const float xRatio = static_cast<float>(srcWidth) / static_cast<float>(dstWidth);
...
bool SIMDScaler::ScaleBGRA_BoxFilter_Scalar(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                            uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                            uint32_t dstStride)
{
...
bool SIMDScaler::ScaleBGRA_BoxFilter_SSE41(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight,
                                           uint32_t srcStride, uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight,
                                           uint32_t dstStride)
{
    if (!srcData || !dstData || srcWidth == 0 || srcHeight == 0 || dstWidth == 0 || dstHeight == 0) {
        ...
        for (uint32_t dx = 0; dx < dstWidth; ++dx) {
            ...
            for (uint32_t sy = srcY0; sy < srcY1; ++sy) {
                ...
                for (uint32_t i = 0; i < simdWidth; i += 4) {
                    ...
                    __m128i pixels = _mm_loadu_si128(reinterpret_cast<const __m128i*>(srcRow + (sx + i) * 4));
                ...
                for (uint32_t i = simdWidth; i < boxWidth; ++i) {
                    ...
                    accumBG = _mm_add_epi32(accumBG, _mm_setr_epi32(pixel[0], pixel[1], 0, 0));
                    accumRA = _mm_add_epi32(accumRA, _mm_setr_epi32(pixel[2], pixel[3], 0, 0));
                    ++count;
                }
            }
...
bool SIMDScaler::ScaleBGRA_BoxFilter(const uint8_t* srcData, uint32_t srcWidth, uint32_t srcHeight, uint32_t srcStride,
                                     uint8_t* dstData, uint32_t dstWidth, uint32_t dstHeight, uint32_t dstStride)
{
    if (HardwareCapabilities::Get().GetCPU().hasSSE41) {
        return ScaleBGRA_BoxFilter_SSE41(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight,
                                         dstStride);
    }
    return ScaleBGRA_BoxFilter_Scalar(srcData, srcWidth, srcHeight, srcStride, dstData, dstWidth, dstHeight, dstStride);
}
...
void SIMDScaler::ConvertRGBAtoBGRA_AVX2(const uint8_t* rgba, uint8_t* bgra, uint32_t pixelCount)
{
    ...
    const __m256i shuffleMask = _mm256_setr_epi8(2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15, 2, 1, 0, 3, 6, 5,
                                                 4, 7, 10, 9, 8, 11, 14, 13, 12, 15);
...
void SIMDScaler::ConvertRGBAtoBGRA_Scalar(const uint8_t* rgba, uint8_t* bgra, uint32_t pixelCount)
{
...
void SIMDScaler::ConvertRGBtoBGR_AVX2(const uint8_t* rgb, uint8_t* bgr, uint32_t pixelCount)
{
...
void SIMDScaler::ConvertRGBtoBGR_Scalar(const uint8_t* rgb, uint8_t* bgr, uint32_t pixelCount)
{
    for (uint32_t i = 0; i < pixelCount; i++) {
