...
MalformedInputHandler::MalformedInputHandler()
    : m_config() {}

MalformedInputHandler::MalformedInputHandler(const MalformedInputConfig& config)
    : m_config(config) {}
...
bool MalformedInputHandler::IsGIF(const uint8_t* data, uint32_t size) {
    return size >= 6 && (std::memcmp(data, GIF87_MAGIC, 6) == 0 ||
                         std::memcmp(data, GIF89_MAGIC, 6) == 0);
}
...
bool MalformedInputHandler::IsWebP(const uint8_t* data, uint32_t size) {
    return size >= 12 && std::memcmp(data, WEBP_RIFF, 4) == 0 &&
           std::memcmp(data + 8, WEBP_SIG, 4) == 0;
}

//------------------------------------------------------------------------------
bool MalformedInputHandler::CheckMagicBytes(const uint8_t* data, uint32_t size,
                                             const std::wstring& expectedFormat) {
    if (!data || size == 0) return false;
    if (expectedFormat == L"PNG")  return IsPNG(data, size);
    if (expectedFormat == L"JPEG") return IsJPEG(data, size);
    if (expectedFormat == L"GIF")  return IsGIF(data, size);
    if (expectedFormat == L"BMP")  return IsBMP(data, size);
    if (expectedFormat == L"ZIP")  return IsZIP(data, size);
    if (expectedFormat == L"PDF")  return IsPDF(data, size);
    if (expectedFormat == L"WEBP") return IsWebP(data, size);
...
bool MalformedInputHandler::AreDimensionsSafe(uint32_t width, uint32_t height) const {
    ...
    if (width > m_config.bombLimits.maxImageWidth) return false;
    if (height > m_config.bombLimits.maxImageHeight) return false;
    uint64_t pixels = static_cast<uint64_t>(width) * height;
    return pixels <= m_config.bombLimits.maxPixelCount;
}

bool MalformedInputHandler::IsCompressionRatioSafe(uint64_t compressed,
                                                    uint64_t decompressed) const {
    if (compressed == 0) return false;
    if (decompressed > m_config.bombLimits.maxDecompressedSize) return false;
...
//------------------------------------------------------------------------------
void MalformedInputHandler::ClampDimensions(uint32_t& width, uint32_t& height,
...
const wchar_t* MalformedInputHandler::GetCorruptionName(CorruptionType type) {
    switch (type) {
        case CorruptionType::None:               return L"None";
        case CorruptionType::TruncatedFile:      return L"TruncatedFile";
        case CorruptionType::InvalidMagic:       return L"InvalidMagic";
        case CorruptionType::CorruptHeader:      return L"CorruptHeader";
        case CorruptionType::InvalidDimensions:  return L"InvalidDimensions";
        case CorruptionType::DecompressionBomb:   return L"DecompressionBomb";
        case CorruptionType::MalformedMetadata:  return L"MalformedMetadata";
        case CorruptionType::CircularReference:  return L"CircularReference";
        case CorruptionType::UnsupportedVersion: return L"UnsupportedVersion";
        case CorruptionType::EncryptedContent:   return L"EncryptedContent";
        case CorruptionType::ZeroByteFile:       return L"ZeroByteFile";
        case CorruptionType::ExcessiveNesting:   return L"ExcessiveNesting";
        case CorruptionType::InvalidChecksum:    return L"InvalidChecksum";
...
const wchar_t* MalformedInputHandler::GetSeverityName(ValidationSeverity severity) {
    switch (severity) {
        case ValidationSeverity::Info:     return L"Info";
        case ValidationSeverity::Warning:  return L"Warning";
        case ValidationSeverity::Error:    return L"Error";
        case ValidationSeverity::Critical: return L"Critical";
        default:                            return L"Unknown";
    }
}
