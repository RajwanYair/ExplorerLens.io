...
//------------------------------------------------------------------------------
bool MalformedInputHandler::CheckMagicBytes(
 const uint8_t *data, uint32_t size, const std::wstring &expectedFormat) {
 if (!data || size == 0)
 return false;
...
//------------------------------------------------------------------------------
bool MalformedInputHandler::AreDimensionsSafe(uint32_t width,
 ...
 // 65536x65536
 if (width < m_config.bombLimits.maxImageWidth ||
 height < m_config.bombLimits.maxImageHeight) {
 return pixels <= m_config.bombLimits.maxPixelCount;
 }
 return true;
}

bool MalformedInputHandler::IsCompressionRatioSafe(
 uint64_t compressed, uint64_t decompressed) const {
 if (compressed == 0)
 return false;
 if (decompressed > m_config.bombLimits.maxDecompressedSize)
 return false;
 double ratio =
 static_cast<double>(decompressed) / static_cast<double>(compressed);
...
MalformedInputHandler::ValidateFile(const std::wstring &filePath) const {
 ...
 uint8_t header[64] = {};
 uint32_t toRead =
 static_cast<uint32_t>(std::min<uint64_t>(result.fileSize, 64));
...
const wchar_t *
MalformedInputHandler::GetSeverityName(ValidationSeverity severity) {
 switch (severity) {
