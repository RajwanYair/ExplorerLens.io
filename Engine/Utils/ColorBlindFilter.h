...
public:
    ...
    [[nodiscard]] std::vector<uint8_t> ApplyCopy(
    ...
    static bool MeetsWCAGContrastUnderSimulation(uint32_t fgBGRA, uint32_t bgBGRA,
                                                  ColorBlindType type,
                                                  float minContrast = 4.5f) noexcept;

    [[nodiscard]] ColorBlindConfig Config() const noexcept { return m_cfg; }
    void SetConfig(ColorBlindConfig cfg) noexcept { m_cfg = cfg; }
