...
enum class OpenAPISchemaType { String, Integer, Boolean, Array, Object };
enum class OpenAPIOutputFormat { YAML, JSON };
...
public:
    ...
    void SetTitle(const std::string& title)     { m_spec.title   = title; }
    void SetVersion(const std::string& version) { m_spec.version = version; }
    ...
    const OpenAPISpec& Spec() const noexcept { return m_spec; }
    size_t EndpointCount() const noexcept { return m_spec.endpoints.size(); }
...
private:
    ...
    std::string GenerateJSON() const {
        std::ostringstream oss;
        oss << "{\"openapi\":\"3.1.0\",\"info\":{\"title\":\"" << m_spec.title
            << "\",\"version\":\"" << m_spec.version << "\"},\"paths\":{";
        bool first = true;
        for (const auto& ep : m_spec.endpoints) {
            if (!first) oss << ",";
