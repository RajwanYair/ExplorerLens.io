...
class ConfigMigrationEngine
{
  public:
    ...
    std::map<std::wstring, std::wstring> GetMigratedConfig() const
    {
        return m_migratedConfig;
    }
    const std::vector<MigrationRule>& GetRules() const
    {
        return m_rules;
    }
    ...
    {
        return static_cast<uint32_t>(ConfigVersion::VersionCount);
    }
