...
{
  public:
    explicit MLKEMKeyEncapsulator(MLKEMSecurityLevel level = MLKEMSecurityLevel::MLKEM768) : m_level(level) {}
    ...
    MLKEMKeyPair GenerateKeyPair() const
    {
        ...
        size_t pubSize = (m_level == MLKEMSecurityLevel::MLKEM512)   ? 800
                         : (m_level == MLKEMSecurityLevel::MLKEM768) ? 1184
                                                                     : 1568;
        size_t privSize = (m_level == MLKEMSecurityLevel::MLKEM512)   ? 1632
                          : (m_level == MLKEMSecurityLevel::MLKEM768) ? 2400
                                                                      : 3168;
    ...
    MLKEMDecapsulationResult Decapsulate(const MLKEMPrivateKey& privKey, const std::vector<uint8_t>& ciphertext) const
    {
    ...
    {
        switch (level) {
            case MLKEMSecurityLevel::MLKEM512:
                return "ML-KEM-512";
            case MLKEMSecurityLevel::MLKEM768:
                return "ML-KEM-768";
            case MLKEMSecurityLevel::MLKEM1024:
                return "ML-KEM-1024";
        }
        return "Unknown";
    }

    MLKEMSecurityLevel Level() const noexcept
    {
        return m_level;
    }
