...
{
    static const char* names[] = {"clang-tidy", "MSVC /analyze", "cppcheck", "PVS-Studio", "Coverity"};
...
class StaticAnalysisCIGate
{
  public:
    ...
    void SetThresholds(const SAGateThresholds& t)
    {
        m_thresholds = t;
    }
    const SAGateThresholds& GetThresholds() const
    {
        return m_thresholds;
    }
    ...
    SAGateResult Evaluate() const
    {
        ...
        for (const auto& f : m_findings) {
            if (m_thresholds.diffAwareOnly && !f.isNewInDiff)
                continue;
            ...
            switch (f.severity) {
                case SAFindingSeverity::Critical:
                    result.criticals++;
                    break;
                case SAFindingSeverity::Error:
                    result.errors++;
                    break;
                case SAFindingSeverity::Warning:
                    result.warnings++;
                    break;
                case SAFindingSeverity::Note:
                    result.notes++;
                    break;
            }
        }
        ...
        // Determine verdict
        if (result.criticals > m_thresholds.maxCriticals || result.errors > m_thresholds.maxErrors) {
    ...
    /// Get all findings
    const std::vector<SAFinding>& GetFindings() const
    {
        return m_findings;
    }
