...
inline const char* RemoteSessionTypeToString(RemoteSessionType type) {
    static const char* names[] = {
        "Local", "RDP", "Citrix", "VNC", "VMware", "AnyDesk", "Unknown"
    };
...
inline const char* BandwidthTierToString(BandwidthTier tier) {
    static const char* names[] = {
        "Local", "HighSpeed", "Broadband", "LowBand", "Constrained"
    };
...
public:
    ...
    bool IsRemoteSession() const { return m_stats.isRemote; }

    /// Get the session type
    RemoteSessionType GetSessionType() const { return m_stats.sessionType; }

    /// Get the current rendering profile
    const RemoteRenderProfile& GetProfile() const { return m_profile; }
    ...
    uint8_t GetOptimalJPEGQuality() const { return m_profile.jpegQuality; }

    /// Should GPU rendering be used?
    bool ShouldUseGPU() const { return !m_profile.disableGPU; }

    /// Get statistics
    const RemoteDesktopStats& GetStats() const { return m_stats; }
...
private:
    ...
    RemoteSessionType DetectProtocol() const {
        ...
        if (::GetEnvironmentVariableW(L"SESSIONNAME", sessionName, size)) {
            ...
            if (name.find(L"RDP") != std::wstring::npos) return RemoteSessionType::RDP;
            if (name.find(L"ICA") != std::wstring::npos) return RemoteSessionType::Citrix;
        }
