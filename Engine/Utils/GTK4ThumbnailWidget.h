...
class GTK4ThumbnailWidget
{
  public:
    ...
    void SetClickCallback(GTK4ClickCallback cb)
    {
        m_onClick = std::move(cb);
    }

    GTK4RenderOutput RenderWidget(const std::vector<uint8_t>& rgbaSource, uint32_t srcW, uint32_t srcH)
    {
        GTK4RenderOutput out;
        if (!m_ready || rgbaSource.empty() || srcW == 0 || srcH == 0) {
            out.errorCode = "INVALID_INPUT";
            return out;
        }
