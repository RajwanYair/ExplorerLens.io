...
enum class ComplianceRegulation : uint8_t {
    GDPR = 0,
    CCPA,
    HIPAA,
    SOX,
    ...
    static const wchar_t* RegulationName(ComplianceRegulation r)
    {
        switch (r) {
            ...
            case ComplianceRegulation::SOX:
                return L"SOX";
            case ComplianceRegulation::ISO27001:
                return L"ISO 27001";
            default:
                return L"Unknown";
        }
    ...
    static const wchar_t* AuditEventTypeName(AuditEventType t)
    {
        switch (t) {
            ...
            case AuditEventType::Export:
                return L"Export";
            case AuditEventType::Share:
                return L"Share";
            case AuditEventType::Consent:
                return L"Consent";
            default:
                return L"Unknown";
        }
    }
    static constexpr size_t RegulationCount()
    {
        return static_cast<size_t>(ComplianceRegulation::COUNT);
    }
    static constexpr size_t DataClassCount()
    {
        return static_cast<size_t>(DataClassification::COUNT);
    }
    static constexpr size_t AuditEventCount()
    {
        return static_cast<size_t>(AuditEventType::COUNT);
    }
};
...
inline std::wstring LicenseToSPDX(LicenseType lic)
{
    switch (lic) {
        ...
        case LicenseType::BSD_3_Clause:
            return L"BSD-3-Clause";
        case LicenseType::Apache_2_0:
            return L"Apache-2.0";
        case LicenseType::LGPL_2_1:
            return L"LGPL-2.1-only";
        case LicenseType::GPL_2_0:
            return L"GPL-2.0-only";
        case LicenseType::Zlib:
            return L"Zlib";
        case LicenseType::Public_Domain:
            return L"CC0-1.0";
        case LicenseType::Proprietary:
            return L"LicenseRef-Proprietary";
        default:
            return L"NOASSERTION";
    }
}
...
struct DependencyProvenance
{
    ...
    {
        return !name.empty() && !version.empty() && !source_url.empty() && !sha256_hash.empty()
...
class DependencyRegistry
{
  ...
  private:
    ...
    {
        Register({L"libjxl", L"0.11.1", L"https://github.com/libjxl/libjxl", L"v0.11.1", L"", LicenseType::BSD_3_Clause,
                  L"Organization: Google LLC", L"pkg:github/libjxl/libjxl@v0.11.1"});

        Register({L"libheif", L"1.19.5", L"https://github.com/nickel-org/libheif", L"v1.19.5", L"",
                  LicenseType::LGPL_2_1, L"Organization: Dirk Farin", L"pkg:github/nickel-org/libheif@v1.19.5"});

        Register({L"libwebp", L"1.5.0", L"https://chromium.googlesource.com/webm/libwebp", L"v1.5.0", L"",
                  LicenseType::BSD_3_Clause, L"Organization: Google LLC", L"pkg:github/nickel-org/libwebp@v1.5.0"});

        Register({L"LibRaw", L"0.21.3", L"https://github.com/LibRaw/LibRaw", L"0.21.3", L"", LicenseType::LGPL_2_1,
                  L"Organization: LibRaw LLC", L"pkg:github/LibRaw/LibRaw@0.21.3"});

        Register({L"libavif", L"1.3.0", L"https://github.com/AOMediaCodec/libavif", L"v1.3.0", L"",
                  LicenseType::BSD_3_Clause, L"Organization: Alliance for Open Media",
                  L"pkg:github/AOMediaCodec/libavif@v1.3.0"});

        Register({L"zlib", L"1.3.1", L"https://github.com/madler/zlib", L"v1.3.1", L"", LicenseType::Zlib,
                  L"Organization: Jean-loup Gailly / Mark Adler", L"pkg:github/madler/zlib@v1.3.1"});

        Register({L"zstd", L"1.5.7", L"https://github.com/facebook/zstd", L"v1.5.7", L"", LicenseType::BSD_3_Clause,
                  L"Organization: Meta Platforms", L"pkg:github/facebook/zstd@v1.5.7"});

        Register({L"lz4", L"1.10.0", L"https://github.com/lz4/lz4", L"v1.10.0", L"", LicenseType::BSD_3_Clause,
                  L"Organization: Yann Collet", L"pkg:github/lz4/lz4@v1.10.0"});

        Register({L"lzma", L"26.00", L"https://7-zip.org/sdk.html", L"26.00", L"", LicenseType::Public_Domain,
                  L"Organization: Igor Pavlov", L"pkg:generic/lzma-sdk@26.00"});

        Register({L"minizip-ng", L"4.0.10", L"https://github.com/zlib-ng/minizip-ng", L"4.0.10", L"", LicenseType::Zlib,
                  L"Organization: zlib-ng contributors", L"pkg:github/zlib-ng/minizip-ng@4.0.10"});

        Register({L"unrar", L"7.2.2", L"https://www.rarlab.com/rar_add.htm", L"7.2.2", L"", LicenseType::Proprietary,
                  L"Organization: Alexander L. Roshal", L"pkg:generic/unrar@7.2.2"});
    }
};
  ...
  public:
    ...
    std::wstring Generate(const std::vector<DependencyProvenance>& deps) const
    {
        switch (config_.format) {
            case SBOMFormat::SPDX_2_3:
                return GenerateSPDX(deps);
            case SBOMFormat::CycloneDX_1_5:
                return GenerateCycloneDX(deps);
            default:
                return GenerateSPDX(deps);
        }
    }
...
class CIPolicyGate
{
  public:
    struct ReleaseChecklist
    {
        ...
        {
            return all_binaries_signed && sbom_present && sbom_valid && all_deps_have_hashes && all_deps_have_provenance;
        }

        uint32_t PassedChecks() const
        {
            uint32_t n = 0;
            if (all_binaries_signed)
                n++;
            if (sbom_present)
                n++;
            if (sbom_valid)
                n++;
            if (all_deps_have_hashes)
                n++;
            if (all_deps_have_provenance)
                n++;
        ...
        static constexpr uint32_t TotalChecks()
        {
            return 7;
        }
    };
    ...
    ReleaseChecklist Evaluate(const std::vector<DependencyProvenance>& deps, bool has_sbom, bool binaries_signed,
                              bool manifest_signed, bool repro_build) const
    {
        ...
        for (auto& d : deps) {
            if (d.sha256_hash.empty())
                cl.all_deps_have_hashes = false;
            if (!d.IsComplete())
                cl.all_deps_have_provenance = false;
        }
    ...
    std::vector<CIPolicyViolation> CheckViolations(const ReleaseChecklist& cl) const
    {
        ...
        if (!cl.all_binaries_signed) {
            violations.push_back(
                {CIPolicyViolation::Severity::Error, L"SIGN-001", L"Not all binaries are Authenticode-signed", L""});
        }
        if (!cl.sbom_present) {
            violations.push_back(
                {CIPolicyViolation::Severity::Error, L"SBOM-001", L"SBOM not generated for release artifacts", L""});
        }
        if (!cl.all_deps_have_hashes) {
            violations.push_back({CIPolicyViolation::Severity::Error, L"HASH-001",
                                  L"Some dependencies missing SHA256 hash verification", L""});
        }
        if (!cl.all_deps_have_provenance) {
            violations.push_back({CIPolicyViolation::Severity::Error, L"PROV-001",
                                  L"Incomplete dependency provenance (missing source/license/hash)", L""});
        }
        if (!cl.manifest_signed) {
            violations.push_back({CIPolicyViolation::Severity::Warning, L"SIGN-002",
                                  L"SHA256SUMS manifest not GPG/sigstore signed", L""});
        }
        if (!cl.reproducible_build) {
            violations.push_back(
                {CIPolicyViolation::Severity::Warning, L"REPRO-001", L"Build reproducibility not verified", L""});
        }
...
class ComplianceReleaseManifest
{
  public:
    ...
    uint32_t ArtifactCount() const
    {
        return static_cast<uint32_t>(artifacts_.size());
    }

    const std::vector<ComplianceArtifact>& GetArtifacts() const
    {
        return artifacts_;
    }
...
class SBOMDocumentGenerator
{
  public:
    ...
    static const SBOMComponent& GetComponent(uint32_t index)
    {
        static const SBOMComponent components[] = {
            {"zlib", "1.3.1", "Jean-loup Gailly", "https://zlib.net", LicenseType::MIT, "Zlib", nullptr,
             "pkg:github/madler/zlib@1.3.1", true, false},
            {"lz4", "1.10.0", "Yann Collet", "https://github.com/lz4/lz4", LicenseType::MIT, "BSD-2-Clause", nullptr,
             "pkg:github/lz4/lz4@1.10.0", true, false},
            {"zstd", "1.5.7", "Meta / Yann Collet", "https://github.com/facebook/zstd", LicenseType::MIT,
             "BSD-3-Clause", nullptr, "pkg:github/facebook/zstd@1.5.7", true, false},
            {"lzma-sdk", "26.00", "Igor Pavlov", "https://7-zip.org/sdk.html", LicenseType::PublicDomain,
             "LicenseRef-LZMA-SDK", nullptr, nullptr, true, false},
            {"minizip-ng", "4.0.10", "Nathan Moinvaziri", "https://github.com/zlib-ng/minizip-ng", LicenseType::Zlib,
             "Zlib", nullptr, "pkg:github/zlib-ng/minizip-ng@4.0.10", true, false},
            {"unrar", "7.2.2", "Alexander Roshal", "https://www.rarlab.com", LicenseType::Custom, "LicenseRef-UnRAR",
             nullptr, nullptr, true, false},
            {"libwebp", "1.5.0", "Google", "https://chromium.googlesource.com/webm/libwebp", LicenseType::MIT,
             "BSD-3-Clause", nullptr, "pkg:github/nicedoc/webp@1.5.0", true, false},
            {"libavif", "1.3.0", "AOMedia", "https://github.com/AOMediaCodec/libavif", LicenseType::MIT, "BSD-2-Clause",
             nullptr, "pkg:github/AOMediaCodec/libavif@1.3.0", true, false},
            {"libjxl", "0.11.1", "Google", "https://github.com/libjxl/libjxl", LicenseType::MIT, "BSD-3-Clause",
             nullptr, "pkg:github/libjxl/libjxl@0.11.1", true, false},
            {"libheif", "1.19.5", "Dirk Farin", "https://github.com/nicedoc/libheif", LicenseType::LGPL21,
             "LGPL-2.1-only", nullptr, "pkg:github/nicedoc/libheif@1.19.5", true, false},
            {"libde265", "1.0.15", "Dirk Farin", "https://github.com/nicedoc/libde265", LicenseType::LGPL21,
             "LGPL-2.1-only", nullptr, "pkg:github/nicedoc/libde265@1.0.15", true, false},
            {"dav1d", "1.5.1", "VideoLAN", "https://code.videolan.org/videolan/dav1d", LicenseType::MIT, "BSD-2-Clause",
             nullptr, "pkg:github/nicedoc/dav1d@1.5.1", true, false},
            {"libraw", "0.21.3", "LibRaw LLC", "https://www.libraw.org", LicenseType::LGPL21, "LGPL-2.1-only", nullptr,
             "pkg:github/libraw/libraw@0.21.3", true, false},
            {"bzip2", "1.0.8", "Julian Seward", "https://sourceware.org/bzip2", LicenseType::MIT, "bzip2-1.0.6",
             nullptr, nullptr, true, false},
            {"xz-utils", "5.6.4", "Lasse Collin", "https://tukaani.org/xz", LicenseType::PublicDomain, "0BSD", nullptr,
             nullptr, true, false},
            {"highway", "1.2.0", "Google", "https://github.com/google/highway", LicenseType::Apache2, "Apache-2.0",
             nullptr, "pkg:github/google/highway@1.2.0", false, false},
            {"brotli", "1.1.0", "Google", "https://github.com/google/brotli", LicenseType::MIT, "MIT", nullptr,
             "pkg:github/google/brotli@1.1.0", false, false},
            {"lcms2", "2.16", "Marti Maria", "https://github.com/mm2/Little-CMS", LicenseType::MIT, "MIT", nullptr,
             "pkg:github/mm2/Little-CMS@2.16", false, false},
        };
    ...
    bool GenerateSBOM(const wchar_t* outputPath, SBOMOutputFormat format = SBOMOutputFormat::SPDX_JSON)
    {
        FILE* fp = nullptr;
  ...
  private:
    ...
    void WriteSPDX(FILE* fp)
    {
        ...
        fprintf(fp, " \"documentNamespace\": \"https://explorerlens.io/sbom/v15.0.0\",\n");
        fprintf(fp, " \"packages\": [\n");
    ...
    void WriteCSV(FILE* fp)
    {
        ...
        for (uint32_t i = 0; i < COMPONENT_COUNT; ++i) {
            const auto& c = GetComponent(i);
            fprintf(fp, "%s,%s,%s,%s,%s\n", c.name, c.version, LicenseName(c.license), c.supplier,
                    c.isDirectDependency ? "Yes" : "No");
        }
    }
};
...
class DecodeRateLimiter
{
  public:
    ...
    bool TryAcquire()
    {
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - m_windowStart).count();
        if (elapsed >= 1000) {
            m_windowStart = now;
    ...
    {
        return m_totalAccepted.load(std::memory_order_relaxed);
    }
    uint64_t TotalRejected() const
    {
        return m_totalRejected.load(std::memory_order_relaxed);
    }
  ...
  private:
    DecodeRateLimiter() : m_windowStart(std::chrono::steady_clock::now()) {}
