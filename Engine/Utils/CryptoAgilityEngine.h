...
enum class CryptoRole        { KeyExchange, Signature, SymmetricEncryption, Hash, MAC };
enum class CryptoPreference  { Classical, PostQuantum, Hybrid, Auto };
...
public:
    explicit CryptoAgilityEngine(CryptoPreference pref = CryptoPreference::Hybrid)
        : m_preference(pref) {
    ...
    CryptoNegotiationResult Negotiate(CryptoRole role,
                                      const std::vector<std::string>& clientSupported) const {
        const CryptoAlgorithmDescriptor* best = nullptr;
        for (const auto& id : clientSupported) {
            auto it = m_algorithms.find(id);
            if (it == m_algorithms.end()) continue;
            const auto& algo = it->second;
            if (algo.role != role) continue;
            if (!algo.fipsApproved && m_requireFIPS) continue;
            if (m_preference == CryptoPreference::PostQuantum && !algo.postQuantum) continue;
            if (m_preference == CryptoPreference::Classical   &&  algo.postQuantum) continue;
            if (!best || algo.priority < best->priority) best = &algo;
        }
        if (!best) return { false, {}, role, false, "No mutually supported algorithm" };
        return { true, best->id, role, best->fipsApproved, {} };
    }

    void SetRequireFIPS(bool require) noexcept { m_requireFIPS = require; }
    void SetPreference(CryptoPreference pref) noexcept { m_preference = pref; }
    size_t AlgorithmCount() const noexcept { return m_algorithms.size(); }
