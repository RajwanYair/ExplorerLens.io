//==============================================================================
// DarkThumbs Engine — Sprint 253: Test Infrastructure Upgrade
...
public:
    ...
    static std::wstring GetCoverageCommand(CoverageTool tool) {
        switch (tool) {
            case CoverageTool::OpenCppCoverage:
                return L"OpenCppCoverage.exe --sources Engine\\ --modules EngineTests.exe -- build\\Tests\\Release\\EngineTests.exe";
            case CoverageTool::MSVCProfiler:
                return L"vsinstr.exe /coverage EngineTests.exe && vsperfmon.exe /coverage /output:coverage.coverage";
            case CoverageTool::LLVMCov:
                return L"llvm-profdata merge -output=default.profdata default.profraw && llvm-cov report EngineTests.exe";
            default:
    ...
    static const wchar_t* CoverageToolName(CoverageTool t) {
        switch (t) {
            case CoverageTool::OpenCppCoverage: return L"OpenCppCoverage";
            case CoverageTool::MSVCProfiler:    return L"MSVC Profiler";
            case CoverageTool::LLVMCov:         return L"LLVM Coverage";
            case CoverageTool::Manual:          return L"Manual";
    ...
    static const wchar_t* SanitizerModeName(SanitizerMode m) {
        switch (m) {
            ...
            case SanitizerMode::AddressSanitizer:  return L"ASAN";
            case SanitizerMode::UndefinedBehavior: return L"UBSAN";
            case SanitizerMode::ThreadSanitizer:   return L"TSAN";
            case SanitizerMode::MemorySanitizer:   return L"MSAN";
            case SanitizerMode::Full:              return L"Full";
            default: return L"Unknown";
        }
    }
    ...
    static std::wstring GetSanitizerFlags(SanitizerMode m) {
        switch (m) {
            case SanitizerMode::AddressSanitizer: return L"/fsanitize=address";
            case SanitizerMode::Full:             return L"/fsanitize=address /RTC1";
            default: return L"";
        }
    }
    ...
    std::vector<TestCorpusFile> GetCorpusFor(const std::wstring& ext) const {
        ...
        for (auto& f : m_corpus) {
            if (f.extension == ext) result.push_back(f);
        }
        return result;
    }
    ...
    CoverageThresholds GetReleaseThresholds() const { return { 85.0f, 65.0f, 90.0f }; }
