...
class LogRotationEngine
{
  public:
    ...
    static uint32_t GetPolicyCount()
    {
        return static_cast<uint32_t>(RotationPolicy::COUNT);
    }

    /// Set rotation configuration
    void SetConfig(const RotationConfig& config)
    {
        m_config = config;
    }
    ...
    const std::vector<RotatedLogFile>& GetRotatedFiles() const
    {
        return m_rotatedFiles;
    }
