...
void PythonSDK::PopulateDecoders() {
    ...
    auto addDecoder = [&](const std::wstring& name, const std::wstring& desc,
                          ...
                          bool requiresLib = false, const std::wstring& libVer = L"") {
    ...
    addDecoder(L"ImageDecoder", L"Standard image formats (WIC)", {L".jpg", L".png", L".bmp", L".gif", L".tiff"});
    addDecoder(L"WebPDecoder", L"WebP images", {L".webp"}, true, L"1.5.0");
    addDecoder(L"AVIFDecoder", L"AVIF images", {L".avif"}, true, L"1.3.0");
    addDecoder(L"HEIFDecoder", L"HEIF/HEIC images", {L".heif", L".heic"}, true, L"1.19.5");
    addDecoder(L"JXLDecoder", L"JPEG XL images", {L".jxl"}, true, L"0.11.1");
    addDecoder(L"RAWDecoder", L"Camera RAW formats", {L".cr2", L".nef", L".arw", L".dng"}, true, L"0.21.3");
    ...
    addDecoder(L"ArchiveDecoder", L"Archive thumbnails", {L".zip", L".7z", L".rar", L".cbz", L".cbr"});
    ...
    addDecoder(L"DocumentDecoder", L"Document thumbnails", {L".docx", L".pptx", L".epub"});
    addDecoder(L"ModelDecoder", L"3D model preview", {L".obj", L".stl", L".gltf"});
...
//------------------------------------------------------------------------------
PythonThumbnailResult PythonSDK::GenerateFromBuffer(const uint8_t* data, size_t size,
...
//------------------------------------------------------------------------------
std::wstring PythonSDK::DetectFormatFromBuffer(const uint8_t* data, size_t size) const {
    if (!data || size < 4) return L"";
    // Magic byte detection
    if (size >= 8 && data[0] == 0x89 && data[1] == 'P' && data[2] == 'N' && data[3] == 'G')
        return L"PNG";
    if (data[0] == 0xFF && data[1] == 0xD8) return L"JPEG";
    if (data[0] == 'R' && data[1] == 'I' && data[2] == 'F' && data[3] == 'F') return L"WebP";
    if (data[0] == '%' && data[1] == 'P' && data[2] == 'D' && data[3] == 'F') return L"PDF";
    return L"Unknown";
}
...
BatchResult PythonSDK::ProcessBatch(std::vector<BatchEntry>& entries,
    ...
    for (uint32_t i = 0; i < entries.size(); ++i) {
        ...
        if (callback) callback(i + 1, result.total, entry.inputPath.c_str());
    ...
    if (result.total > 0) result.avgTimeMs = result.totalTimeMs / result.total;
    return result;
}

//------------------------------------------------------------------------------
std::wstring PythonSDK::GetVersion() {
    return L"10.0.0";
}
...
std::wstring PythonSDK::GenerateCtypesStub() {
    return L"import ctypes\n"
           L"lib = ctypes.CDLL('DarkThumbsEngine.dll')\n"
           L"lib.DarkThumbs_Init.restype = ctypes.c_void_p\n"
           L"lib.DarkThumbs_GetVersion.restype = ctypes.c_wchar_p\n"
           L"lib.DarkThumbs_GetDecoderCount.argtypes = [ctypes.c_void_p]\n"
           L"lib.DarkThumbs_GetDecoderCount.restype = ctypes.c_uint32\n";
}

std::wstring PythonSDK::GeneratePybindWrapper() {
    return L"#include <pybind11/pybind11.h>\n"
           L"namespace py = pybind11;\n"
           L"PYBIND11_MODULE(darkthumbs, m) {\n"
           L"    m.def(\"version\", &DarkThumbs_GetVersion);\n"
           L"}\n";
}
...
static DarkThumbs::Engine::PythonSDK* g_sdk = nullptr;

void* DarkThumbs_Init() {
    g_sdk = new DarkThumbs::Engine::PythonSDK();
    g_sdk->Initialize();
    return g_sdk;
}
...
int DarkThumbs_GenerateThumbnail(void* handle, const wchar_t* path,
    ...
    auto sdk = static_cast<DarkThumbs::Engine::PythonSDK*>(handle);
    if (!sdk || !path) return -1;
...
void DarkThumbs_FreePixels(uint8_t* pixels) { delete[] pixels; }

const wchar_t* DarkThumbs_GetVersion() {
    static std::wstring ver = DarkThumbs::Engine::PythonSDK::GetVersion();
    return ver.c_str();
}
...
void DarkThumbs_Destroy(void* handle) {
    delete static_cast<DarkThumbs::Engine::PythonSDK*>(handle);
}
