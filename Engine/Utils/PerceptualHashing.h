...
struct HashEntry {
 ...
 bool HasPHash() const { return pHash.IsValid(); }
 bool HasDHash() const { return dHash.IsValid(); }
 bool HasBothHashes() const { return HasPHash() && HasDHash(); }
};
...
struct DuplicateGroup {
 ...
 bool HasDuplicates() const { return members.size() > 1; }

 uint64_t TotalFileSize() const {
 uint64_t total = 0;
 for (auto& m : members) total += m.fileSize;
 return total;
 }
 ...
 for (auto& m : members) {
 total += m.fileSize;
 if (m.fileSize > maxSize) maxSize = m.fileSize;
 }
...
public:
 ...
 std::vector<SimilarityMatch> FindSimilar(
 const PerceptualHash& query, uint32_t threshold = 10) const
 {
 ...
 if (dist <= threshold) {
 results.push_back({
 entry.path, entry.hash, dist,
 query.Similarity(entry.hash), entry.fileSize
 });
 }
 }
 ...
 m_entries[i].path, m_entries[i].hash, 0,
 100.0, m_entries[i].fileSize
 });
 visited[i] = true;
 ...
 if (dist <= threshold) {
 group.members.push_back({
 m_entries[j].path, m_entries[j].hash, dist,
 m_entries[i].hash.Similarity(m_entries[j].hash),
 m_entries[j].fileSize
 });
 visited[j] = true;
 }
 }
...
struct DuplicateScanResult {
 ...
 if (wastedBytes < 1024) return std::to_string(wastedBytes) + " B";
 if (wastedBytes < 1048576) return std::to_string(wastedBytes / 1024) + " KB";
 if (wastedBytes < 1073741824) return std::to_string(wastedBytes / 1048576) + " MB";
 return std::to_string(wastedBytes / 1073741824) + " GB";
 }

 std::string Summary() const {
 return std::to_string(totalDuplicates) + " duplicates in " +
 std::to_string(duplicateGroups) + " groups, wasting " +
 WastedSizeHuman() + " (" +
 std::to_string(static_cast<int>(DuplicateRate())) + "% duplicate rate)";
 }
};
...
inline std::string ExportDuplicatesCSV(const std::vector<DuplicateGroup>& groups) {
 ...
 << m.hash.ToHex() << ","
 << m.hammingDistance << ","
 << std::fixed << std::setprecision(1) << m.similarityPct << ","
 << m.fileSize << "\n";
 }
 }
 return csv.str();
...
inline std::string ExportDuplicatesJSON(const std::vector<DuplicateGroup>& groups) {
 ...
 auto& m = g.members[mi];
 json << " {\"path\": \"" << m.filePath
 << "\", \"hash\": \"" << m.hash.ToHex()
 << "\", \"distance\": " << m.hammingDistance
 << ", \"similarity\": " << std::fixed << std::setprecision(1) << m.similarityPct
 << ", \"size\": " << m.fileSize << "}";
 if (mi < g.members.size() - 1) json << ",";
 json << "\n";
 }
 json << " ]\n }";
 if (gi < groups.size() - 1) json << ",";
 json << "\n";
 }
 json << " ]\n}\n";
 return json.str();
