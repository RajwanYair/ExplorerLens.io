...
enum class MemoryScheduleAction {
    FullPrealloc,
    PartialPrealloc,
    LazyAlloc,
    Emergency
...
{
  public:
    explicit ThermalAwareMemoryScheduler(ThermalMemoryConfig config = {}) : m_config(std::move(config)) {}
    ...
    ThermalScheduleDecision Evaluate(float thermalC) const noexcept
    {
        ...
        if (thermalC >= m_config.criticalThresholdC) {
            zone = TAMZone::Critical;
            action = MemoryScheduleAction::Emergency;
            allocMB = 16;
            delay = true;
        } else if (thermalC >= m_config.hotThresholdC) {
            zone = TAMZone::Hot;
            action = MemoryScheduleAction::LazyAlloc;
            allocMB = m_config.lazyAllocMB;
            delay = true;
        } else if (thermalC >= m_config.warmThresholdC) {
            zone = TAMZone::Warm;
            action = MemoryScheduleAction::PartialPrealloc;
            allocMB = m_config.partialPreallocMB;
        } else {
            zone = TAMZone::Comfortable;
            action = MemoryScheduleAction::FullPrealloc;
            allocMB = m_config.fullPreallocMB;
        }
    ...
    {
        switch (z) {
            case TAMZone::Comfortable:
                return "Comfortable";
            case TAMZone::Warm:
                return "Warm";
            case TAMZone::Hot:
                return "Hot";
            case TAMZone::Critical:
                return "Critical";
        }
        return "Unknown";
    }
    ...
    {
        switch (a) {
            case MemoryScheduleAction::FullPrealloc:
                return "FullPrealloc";
            case MemoryScheduleAction::PartialPrealloc:
                return "PartialPrealloc";
            case MemoryScheduleAction::LazyAlloc:
                return "LazyAlloc";
            case MemoryScheduleAction::Emergency:
                return "Emergency";
        }
        return "Unknown";
    }

    const ThermalMemoryConfig& Config() const noexcept
    {
        return m_config;
    }
