...
public:
    ...
    MemoryTrimPolicyConfig GetConfig() const { return m_config; }
    std::string GetName() const { return m_config.label; }

    enum class TrimLevel : uint8_t { None, Gentle, Moderate, Aggressive };

    TrimLevel EvaluateTrimLevel(uint32_t pressureTier, bool isIdle) const {
        if (pressureTier >= 4) return TrimLevel::Aggressive;
        if (pressureTier >= 2) return TrimLevel::Moderate;
        if (isIdle) return TrimLevel::Gentle;
        return TrimLevel::None;
    }

    uint32_t GetTrimPercent(TrimLevel level) const {
        switch (level) {
        case TrimLevel::Gentle: return 10;
        case TrimLevel::Moderate: return 30;
        case TrimLevel::Aggressive: return m_config.trimPercentOnPressure;
        default: return 0;
        }
    }
