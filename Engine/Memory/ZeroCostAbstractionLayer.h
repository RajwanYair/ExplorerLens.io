...
struct alignas(CACHE_LINE_SIZE) CacheAlignedArray {
    ...
    inline const T& operator[](size_t index) const { return elements[index]; }
...
public:
    ...
    inline std::string DescribeLayout() const {
        ...
        std::string desc = "Size=" + std::to_string(info.Size) +
            " Align=" + std::to_string(info.Alignment) +
            " CacheLines=" + std::to_string(info.CacheLinesUsed) +
            " Waste=" + std::to_string(info.WastedBytes) +
            " Copyable=" + (info.IsTriviallyCopyable ? "yes" : "no") +
