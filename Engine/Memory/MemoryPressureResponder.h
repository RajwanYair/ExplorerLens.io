...
public:
    ...
    MemoryPressureResponderConfig GetConfig() const { return m_config; }
    std::string GetName() const { return m_config.label; }

    enum class PressureLevel : uint8_t { None, Low, Medium, High, Critical };

    PressureLevel Evaluate(uint32_t memoryUsagePercent) const {
        if (memoryUsagePercent >= 95) return PressureLevel::Critical;
        if (memoryUsagePercent >= m_config.highThresholdPercent) return PressureLevel::High;
        if (memoryUsagePercent >= m_config.lowThresholdPercent) return PressureLevel::Medium;
        if (memoryUsagePercent >= 50) return PressureLevel::Low;
    ...
    ResponseAction GetAction(PressureLevel level) const {
        switch (level) {
        case PressureLevel::Critical: return { true, true, true, true };
        case PressureLevel::High:     return { true, true, true, false };
        case PressureLevel::Medium:   return { true, false, false, false };
        case PressureLevel::Low:      return { false, false, false, false };
        default:                      return {};
        }
    }
