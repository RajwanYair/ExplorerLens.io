...
public:
    ...
    NUMANodeAllocatorConfig GetConfig() const { return m_config; }
    std::string GetName() const { return m_config.label; }

    uint32_t GetNodeCount() const { return m_nodeCount; }
    bool IsNUMAAvailable() const { return m_nodeCount > 1; }

    uint32_t GetPreferredNode() const {
        return m_config.autoDetect ? m_detectedNode : m_config.preferredNode;
    }

    void SetDetectedNode(uint32_t node) { m_detectedNode = node; }
    ...
    AllocationStats GetStats() const { return m_stats; }
