...
class PoolAllocatorMetrics
{
  public:
    ...
    void UpdateCapacity(uint64_t totalBytes, uint64_t usedBytes)
    {
        ...
        if (usedBytes > m_summary.peakUsedBytes)
            m_summary.peakUsedBytes = usedBytes;
        m_summary.fragmentationRatio = totalBytes > 0 ? 1.0 - (static_cast<double>(usedBytes) / totalBytes) : 0.0;
    }
        ...
        for (auto& b : m_summary.buckets) {
            if (b.blockSize == blockSize) {
                if (isAlloc) {
                    ...
                    if (b.freeBlocks > 0)
                        b.freeBlocks--;
                    if (b.UsedBlocks() > b.peakUsed)
                        b.peakUsed = b.UsedBlocks();
                } else {
                    b.deallocationCount++;
                    b.freeBlocks++;
                }
