...
class ZeroFragmentationHeap
{
  public:
    ...
    ZeroFragmentationHeapConfig GetConfig() const
    {
        return m_config;
    }
    ...
    uint32_t FindSizeClass(uint64_t requestedSize) const
    {
        for (uint32_t i = 0; i < m_sizeClasses.size(); ++i)
            if (m_sizeClasses[i] >= requestedSize)
                return i;
    ...
    {
        return m_totalAllocated > 0 ? 1.0 - static_cast<double>(m_usedBytes) / m_totalAllocated : 0.0;
    }
    ...
    uint32_t GetSizeClassCount() const
    {
        return static_cast<uint32_t>(m_sizeClasses.size());
    }
