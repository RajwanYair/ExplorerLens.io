...
inline const char* SlabClassName(SlabClass c)
{
    switch (c) {
        case SlabClass::Tiny:
            return "Tiny (≤64x64)";
        case SlabClass::Small:
            return "Small (≤128x128)";
        case SlabClass::Medium:
            return "Medium (≤256x256)";
        case SlabClass::Large:
            return "Large (≤512x512)";
        case SlabClass::XLarge:
            return "XLarge (≤1024x1024)";
        case SlabClass::Huge:
            return "Huge (>1024x1024)";
        default:
            return "Unknown";
    }
}
...
inline size_t SlabClassBufferSize(SlabClass c)
{
    switch (c) {
        case SlabClass::Tiny:
            return 64 * 64 * 4;  // 16 KB
        case SlabClass::Small:
            return 128 * 128 * 4;  // 64 KB
        case SlabClass::Medium:
            return 256 * 256 * 4;  // 256 KB
        case SlabClass::Large:
            return 512 * 512 * 4;  // 1 MB
        case SlabClass::XLarge:
            return 1024 * 1024 * 4;  // 4 MB
        case SlabClass::Huge:
            return 2048 * 2048 * 4;  // 16 MB
        default:
            return 0;
    }
}
...
inline SlabClass ClassifyDimension(uint32_t width, uint32_t height)
{
    ...
    if (maxDim <= 64)
        return SlabClass::Tiny;
    if (maxDim <= 128)
        return SlabClass::Small;
    if (maxDim <= 256)
        return SlabClass::Medium;
    if (maxDim <= 512)
        return SlabClass::Large;
    if (maxDim <= 1024)
        return SlabClass::XLarge;
...
struct PooledBuffer
{
    ...
    {
        if (data)
            std::fill(data, data + capacity, static_cast<uint8_t>(0));
    }
};
...
struct BufferPoolStats
{
    ...
    {
        return "Acquires=" + std::to_string(totalAcquires) + " Releases=" + std::to_string(totalReleases)
...
struct BufferPoolConfig
{
    ...
    static BufferPoolConfig Default()
    {
        return {};
    }
...
{
  public:
    explicit BufferPool(BufferPoolConfig config = BufferPoolConfig::Default()) : m_config(config)
    {
    ...
    BufferPoolConfig GetConfig() const
    {
        return m_config;
    }
  ...
  private:
    void UpdateMemoryStats()
    {
        ...
        if (total > m_stats.peakMemoryBytes)
            m_stats.peakMemoryBytes = total;
    }
