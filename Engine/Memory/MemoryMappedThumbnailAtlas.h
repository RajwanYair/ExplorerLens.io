...
class MemoryMappedThumbnailAtlas
{
  public:
    ...
    bool Create(const std::wstring& path, uint64_t reserveSizeBytes = 64 * 1024 * 1024)
    {
        ...
        m_filePath = path;
        m_fileHandle = CreateFileW(path.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS,
        ...
        // Create file mapping
        m_mappingHandle =
            CreateFileMappingW(m_fileHandle, nullptr, PAGE_READWRITE, static_cast<DWORD>(reserveSizeBytes >> 32),
        ...
        m_basePtr = static_cast<uint8_t*>(MapViewOfFile(m_mappingHandle, FILE_MAP_ALL_ACCESS, 0, 0, 0));
        if (!m_basePtr) {
            Close();
            return false;
        }
    ...
    bool Open(const std::wstring& path)
    {
        ...
        m_filePath = path;
        m_fileHandle = CreateFileW(path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                                   FILE_ATTRIBUTE_NORMAL, nullptr);
        if (m_fileHandle == INVALID_HANDLE_VALUE)
            return false;
        ...
        m_basePtr = static_cast<uint8_t*>(MapViewOfFile(m_mappingHandle, FILE_MAP_READ, 0, 0, 0));
        if (!m_basePtr) {
            Close();
            return false;
        }
    ...
    bool Store(uint64_t keyHash, const uint8_t* data, uint32_t size, uint16_t width, uint16_t height)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (!m_basePtr || m_dataOffset + size > m_mappedSize)
            return false;
