...
enum class SPEStrategy { Sequential, MLPredicted, ScrollVelocity, Hybrid };
enum class SPEPriority { Urgent, Speculative, Background };
...
public:
    explicit SmartPrefetchEngine(SPEStrategy strategy = SPEStrategy::Hybrid)
        : m_strategy(strategy) {}

    void SetPrefetchIssuer(PrefetchIssuer fn) { m_issuer = std::move(fn); }

    void NotifyViewedFile(const std::wstring& filePath) {
        m_recentAccess.push_back(filePath);
        if (m_recentAccess.size() > 32) m_recentAccess.pop_front();
    }

    std::vector<SPECandidate> Predict(const std::vector<std::wstring>& visibleFiles,
                                           float scrollVelocity = 0.0f) const {
        ...
        for (size_t i = 0; i < visibleFiles.size() && i < 8; ++i) {
            ...
            candidates.push_back({ visibleFiles[i], conf,
                (i < 3) ? SPEPriority::Urgent : SPEPriority::Speculative, 256 });
        }
        (void)scrollVelocity;
        return candidates;
    }
    ...
    void NotifyHit(const std::wstring&)  noexcept { m_stats.hits++; }
    void NotifyMiss(const std::wstring&) noexcept { m_stats.misses++; }

    const SPEStats& Stats()    const noexcept { return m_stats; }
    void SetMinConfidence(double v) noexcept { m_minConfidence = v; }
