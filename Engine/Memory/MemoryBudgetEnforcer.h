...
public:
    ...
    uint64_t GetCurrentWorkingSet() {
        ...
        if (GetProcessMemoryInfo(GetCurrentProcess(), reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(&pmc), sizeof(pmc))) {
            ...
            if (ws > m_peakWorkingSet) m_peakWorkingSet = ws;
            return ws;
        }
        return 0;
    }
    ...
    MemoryBudgetStatus GetStatus() {
        ...
        status.utilizationPct = (m_config.budgetBytes > 0)
    ...
    // Update budget
    void SetBudget(uint64_t bytes) { m_config.budgetBytes = bytes; }

    // Get budget
    uint64_t GetBudget() const { return m_config.budgetBytes; }
...
private:
    ...
    BudgetEnforcementLevel ComputeLevel(uint64_t workingSet) const {
        ...
        if (ratio >= m_config.emergencyThreshold) return BudgetEnforcementLevel::Emergency;
        if (ratio >= m_config.strictThreshold)    return BudgetEnforcementLevel::Strict;
        if (ratio >= m_config.moderateThreshold)  return BudgetEnforcementLevel::Moderate;
