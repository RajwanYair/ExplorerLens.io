...
class MemoryDefragmenter
{
  public:
    ...
    void SetRelocationCallback(std::function<void(void* oldBase, void* newBase, size_t size)> fn)
    {
        AcquireSRWLockExclusive(&m_lock);
    ...
    DefragResult Defragment()
    {
        ...
        std::sort(m_regions.begin(), m_regions.end(), [](const MemoryRegion& a, const MemoryRegion& b) {
            return reinterpret_cast<uintptr_t>(a.base) < reinterpret_cast<uintptr_t>(b.base);
        });
        ...
        m_regions.erase(
            std::remove_if(m_regions.begin(), m_regions.end(), [](const MemoryRegion& r) { return r.size == 0; }),
            m_regions.end());
    ...
    void EnableAutoDefrag(double fragmentationThreshold = 0.3, uint32_t checkIntervalMs = 10000)
    {
        DisableAutoDefrag();
        ...
        m_autoDefragThread = std::thread([this]() {
            while (m_autoDefragEnabled.load(std::memory_order_acquire)) {
                std::this_thread::sleep_for(std::chrono::milliseconds(m_autoDefragIntervalMs));
    ...
    void CoalesceFreeLocked()
    {
        ...
        for (size_t i = 0; i + 1 < m_regions.size();) {
            if (!m_regions[i].inUse && !m_regions[i + 1].inUse) {
                uintptr_t endOfI = reinterpret_cast<uintptr_t>(m_regions[i].base) + m_regions[i].size;
                uintptr_t startOfNext = reinterpret_cast<uintptr_t>(m_regions[i + 1].base);

                if (endOfI == startOfNext) {
                    // Merge: extend region i, remove region i+1
                    m_regions[i].size += m_regions[i + 1].size;
                    m_regions.erase(m_regions.begin() + static_cast<ptrdiff_t>(i + 1));
