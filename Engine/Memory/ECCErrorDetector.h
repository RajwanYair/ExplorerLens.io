...
struct ECCStatus
{
    bool eccSupported;
    bool eccEnabled;
    uint64_t totalSingleBit;
    uint64_t totalDoubleBit;
};
class ECCErrorDetector
{
  public:
    ECCStatus QueryStatus() const
    {
        return {false, false, 0, 0};
    }
    bool HasCorrectedErrors() const
    {
        return m_records.size() > 0 && m_records[0].singleBitErrors > 0;
    }
    bool HasUncorrectedErrors() const
    {
        return false;
    }
    std::vector<ECCErrorRecord> GetHistory() const
    {
        return m_records;
    }
    void ClearHistory()
