...
struct HotModeThresholds
{
    ...
    static HotModeThresholds Default()
    {
        return {};
    }
...
class HotModeDirectoryEngine
{
  public:
    ...
    DirectorySnapshot IndexDirectory(const std::string& path) const
    {
        ...
        HANDLE hFind = FindFirstFileExW(searchPattern.c_str(),
        ...
        if (hFind != INVALID_HANDLE_VALUE) {
            do {
                ...
                int mblen = WideCharToMultiByte(CP_UTF8, 0, findData.cFileName, -1, nullptr, 0, nullptr, nullptr);
                std::string fileName(static_cast<size_t>(mblen), '\0');
                WideCharToMultiByte(CP_UTF8, 0, findData.cFileName, -1, fileName.data(), mblen, nullptr, nullptr);
                ...
                entry.lastModified = (static_cast<uint64_t>(findData.ftLastWriteTime.dwHighDateTime) << 32)
        ...
        std::sort(snap.entries.begin(), snap.entries.end(),
                  [](const DirectoryIndexEntry& a, const DirectoryIndexEntry& b) { return a.filePath < b.filePath; });

        // Set index timestamp (FILETIME now)
        FILETIME ft{};
        GetSystemTimeAsFileTime(&ft);
        snap.indexTimestamp = (static_cast<uint64_t>(ft.dwHighDateTime) << 32) | ft.dwLowDateTime;
        snap.hotMode = IsHotModeDirectory(snap);
        return snap;
    }

    /// Pre-warm a batch of files from the directory snapshot.
    /// Checks each file's extension against known thumbnail-capable formats.
    /// Matching files are marked cached with an estimated VRAM cost of
    /// 4 * 256 * 256 bytes (256 KB BGRA thumbnail).  Non-matching files
    /// are skipped and counted as failures.  Wall-clock time is measured
    /// via QueryPerformanceCounter.
    ///
    /// Recognised extensions:
    ///   .jpg .jpeg .png .bmp .gif .webp .tiff .tif .ico .svg .psd
    ///   .avif .heic .heif .jxl .exr .hdr .dds
    ///   .raw .cr2 .nef .arw .dng .orf .rw2
    PreWarmBatchResult PreWarmBatch(DirectorySnapshot& snap, uint32_t startIndex, uint32_t count) const
    {
        ...
        for (uint32_t i = startIndex; i < end; ++i) {
            ...
            // Known thumbnail-capable image extensions
            static const char* knownExts[] = {".jpg", ".jpeg", ".png", ".bmp", ".gif",  ".webp", ".tiff",
                                              ".tif", ".ico",  ".svg", ".psd", ".avif", ".heic", ".heif",
                                              ".jxl", ".exr",  ".hdr", ".dds", ".raw",  ".cr2",  ".nef",
                                              ".arw", ".dng",  ".orf", ".rw2"};
            bool matched = false;
            for (const char* known : knownExts) {
                if (ext == known) {
                    matched = true;
                    break;
                }
            ...
            if (matched) {
                entry.thumbnailCached = true;
                entry.prewarmPending = false;
                constexpr size_t THUMB_BYTES = 4 * 256 * 256;
                result.bytesUsed += THUMB_BYTES;
        ...
        result.totalMs =
            static_cast<double>(tEnd.QuadPart - tStart.QuadPart) * 1000.0 / static_cast<double>(freq.QuadPart);
        return result;
    }
    ...
    const HotModeThresholds& Thresholds() const
    {
        return m_thresholds;
    }
