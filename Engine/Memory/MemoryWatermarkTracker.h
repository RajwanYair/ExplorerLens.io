...
class MemoryWatermarkTracker
{
  public:
    ...
    void Configure(const MemWatermarkConfig& config)
    {
        m_config = config;
    }
    ...
    WatermarkLevel Evaluate(uint64_t currentBytes)
    {
        ...
        if (newLevel != m_lastLevel) {
            m_crossings++;
            if (m_callback)
                m_callback(newLevel, currentBytes);
        }
    ...
    WatermarkSnapshot GetSnapshot() const
    {
        ...
        s.utilizationPercent = m_config.maxBytes > 0 ? 100.0 * m_lastBytes / m_config.maxBytes : 0.0;
