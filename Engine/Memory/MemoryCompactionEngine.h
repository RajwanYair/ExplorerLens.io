...
class MemoryCompactionEngine
{
  public:
    ...
    CompactionResult RunCompaction(EngineCompactionTrigger trigger)
    {
        ...
        result.fragmentationAfter =
            result.blocksConsolidated > 0 ? m_currentFragmentation * 0.5f : m_currentFragmentation;
    ...
    void SetConfig(const CompactionConfig& config)
    {
        m_config = config;
    }
    CompactionConfig GetConfig() const
    {
        return m_config;
    }
    uint64_t GetTotalCompactions() const
    {
        return m_totalCompactions;
    }
