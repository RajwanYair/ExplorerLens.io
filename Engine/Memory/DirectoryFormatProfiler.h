...
class DirectoryFormatProfiler
{
  public:
    ...
    // ─── Profile Directory ───────────────────────────────────────
    ProfilerDirProfile ProfileDirectory(const std::string& dirPath, const std::vector<std::string>& files) const
    {
        ...
        std::sort(profile.histogram.begin(), profile.histogram.end(),
                  [](const ExtensionCount& a, const ExtensionCount& b) { return a.count > b.count; });
        ...
        if (!profile.histogram.empty()) {
            ...
            for (const auto& ec : profile.histogram) {
                if (ec.family == profile.dominantFamily)
                    familyCount += ec.count;
            }
            profile.dominantRatio =
                profile.totalFiles > 0 ? static_cast<double>(familyCount) / profile.totalFiles : 0.0;
    ...
    {
        switch (family) {
            case DirFormatFamily::LightweightImage:
                return FamilyMemoryBudget::LightweightImage();
            case DirFormatFamily::ModernImage:
                return FamilyMemoryBudget::ModernImage();
            case DirFormatFamily::RawPhoto:
                return FamilyMemoryBudget::RawPhoto();
            case DirFormatFamily::HDRImage:
                return FamilyMemoryBudget::HDRImage();
            case DirFormatFamily::Archive:
                return FamilyMemoryBudget::Archive();
            default:
                return FamilyMemoryBudget::ModernImage();  // Fallback
        }
    }

    size_t FamilyMapSize() const
    {
        return m_familyMap.size();
    }
