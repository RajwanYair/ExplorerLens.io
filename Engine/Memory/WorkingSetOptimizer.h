...
public:
    ...
    void RecordSnapshot(const WorkingSetSnapshot& snapshot) {
        ...
        m_metrics.avgWorkingSetMB = static_cast<double>(m_totalBytes) /
            m_metrics.snapshotCount / (1024.0 * 1024.0);
    ...
    void RecordLock() { m_metrics.lockCount++; }
    ...
    bool IsGrowthTrending() const {
        if (m_snapshots.size() < 5) return false;
