...
TEST(Diag, CategoryName_Coverage) {
...
TEST(Diag, ExportStatusName_Coverage) {
...
TEST(Diag, ExportResult_Summary) {
    ExportResult r;
...
TEST(Diag, Exporter_ExportSuccess) {
    auto exp = DiagnosticsExporter::Create();
...
TEST(Diag, Exporter_ExportNoData) {
    auto exp = DiagnosticsExporter::Create();
    auto result = exp.Export();
    EXPECT_EQ(result.status, ExportStatus::NoData);
    EXPECT_FALSE(result.IsSuccess());
}
TEST(Diag, Exporter_FilterSensitive) {
