...
TEST(Sprint127_COMApartment, AuditEntry_MismatchNotCompliant) {
    InterfaceAuditEntry e;
    e.declaredModel = ApartmentType::STA;
...
TEST(Sprint127_COMApartment, AuditEntry_GlobalStateNotCompliant) {
    InterfaceAuditEntry e;
    e.declaredModel = ApartmentType::STA;
    e.actualModel = ApartmentType::STA;
...
TEST(Sprint127_COMApartment, Auditor_CreateWithKnownInterfaces) {
    auto auditor = COMApartmentAuditor::Create();
    EXPECT_GE(auditor.InterfaceCount(), 3u);
}
TEST(Sprint127_COMApartment, Auditor_RunAudit_AllCompliant) {
    auto auditor = COMApartmentAuditor::Create();
...
TEST(Sprint127_COMApartment, Auditor_RunAudit_ViolationDetected) {
    auto auditor = COMApartmentAuditor::Create();
...
TEST(Sprint127_COMApartment, TestMatrix_GeneratesScenarios) {
    auto auditor = COMApartmentAuditor::Create();
    auto matrix = auditor.GenerateTestMatrix();
    EXPECT_GE(matrix.size(), 5u);
    EXPECT_EQ(matrix[0].sourceApartment, ApartmentType::STA);
}
TEST(Sprint127_COMApartment, StabilityImprovements_Exist) {
    auto auditor = COMApartmentAuditor::Create();
    auto fixes = auditor.GetImprovements();
    EXPECT_GE(fixes.size(), 5u);
}
TEST(Sprint127_COMApartment, ThreadSafetyValidator_SingleThread) {
...
TEST(Sprint127_COMApartment, ThreadSafetyValidator_CrossThread) {
    ThreadSafetyValidator v;
...
TEST(Sprint127_COMApartment, ThreadSafetyValidator_Reset) {
    ThreadSafetyValidator v;
    v.RecordAccess(1, "ITest");
    v.Reset();
    EXPECT_EQ(v.UniqueThreadCount("ITest"), 0u);
}
TEST(Sprint127_COMApartment, AuditResult_ComplianceZeroInterfaces) {
