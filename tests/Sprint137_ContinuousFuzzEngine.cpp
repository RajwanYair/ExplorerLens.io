...
TEST(Sprint137_Fuzz, Engine_AddCorpus) {
    auto engine = ContinuousFuzzEngine::Create();
    CorpusEntry entry;
...
TEST(Sprint137_Fuzz, Engine_BudgetClean) {
    auto engine = ContinuousFuzzEngine::Create();
    EXPECT_TRUE(engine.IsBudgetClean());
    EXPECT_FALSE(engine.IsBudgetExhausted());
}
TEST(Sprint137_Fuzz, Stats_Summary) {
    auto engine = ContinuousFuzzEngine::Create();
...
TEST(Sprint137_Fuzz, CorpusEntry_IsInteresting) {
    CorpusEntry e;
