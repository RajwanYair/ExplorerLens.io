...
TEST_F(LocalizationTest, FiveLanguagesSupported) {
    EXPECT_EQ(languages.size(), 5u)
        << "Requires exactly 5 language packs";
}
...
TEST(AccessibilityIntegrationTest, FrameworkHeaderExists) {
    ...
    bool exists = fs::exists("Engine/Utils/AccessibilityEngine.h") ||
        fs::exists("Engine\\Utils\\AccessibilityEngine.h");
    EXPECT_TRUE(exists) << "AccessibilityEngine.h must exist for this module (merged from AccessibilityFramework.h)";
}
