...
// ---------------------------------------------------------------------------
// Targets these professional/VFX formats via OIIO:
...
TEST_F(OIIOIntegrationTest, VcpkgCanAddOIIO) {
    ...
    // When  is fully implemented, vcpkg.json should include it
    ...
    if (!hasOiio) {
        GTEST_SKIP() << "OpenImageIO not yet added to vcpkg.json — planned for this module full implementation";
    }
    EXPECT_TRUE(hasOiio);
}
...
TEST_F(OIIOIntegrationTest, TextureCacheConcept) {
    ...
    //
    // This test validates the concept; actual integration is Deliverable.
...
TEST_F(OIIOIntegrationTest, BenchmarkComparisonContract) {
    // Deliverable: Benchmark OIIO vs existing decoders
