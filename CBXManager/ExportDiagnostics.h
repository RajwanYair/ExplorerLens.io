// ============================================================================
// ExportDiagnostics.h - Sprint 8 Enhancement
...
private:
    ...
    static std::wstring GetTempDiagnosticsPath() {
        ...
        path += L"DarkThumbs_Diagnostics_";
    ...
    static bool ExportSystemInfo(const std::wstring& filePath) {
        ...
        file << L"=== DarkThumbs System Information ===\n\n";
        ...
        if (hMod) {
            ...
            if (RtlGetVersion) {
                ...
                file << L"OS Version: " << osInfo.dwMajorVersion << L"." 
                     << osInfo.dwMinorVersion << L" Build " << osInfo.dwBuildNumber << L"\n";
            }
        }
        ...
        switch (si.wProcessorArchitecture) {
            case PROCESSOR_ARCHITECTURE_AMD64: file << L"x64\n"; break;
            case PROCESSOR_ARCHITECTURE_ARM64: file << L"ARM64\n"; break;
            case PROCESSOR_ARCHITECTURE_INTEL: file << L"x86\n"; break;
            default: file << L"Unknown\n"; break;
        }
        ...
        // DarkThumbs Version
        file << L"\n=== DarkThumbs Build Information ===\n";
        file << L"Version: 7.0.0\n";
    ...
    static bool ExportCircuitBreakers(const std::wstring& filePath) {
        ...
        file << L"After 5 consecutive failures, a decoder is temporarily disabled.\n\n";
        ...
        if (status.empty()) {
            ...
            for (const auto& [decoderName, state] : status) {
                ...
                file << L"  Last Failure: " << FormatTimestamp(stats.lastFailureTime) << L"\n\n";
            }
        }
    ...
    static bool ExportRegistrySettings(const std::wstring& filePath) {
        ...
        // Read DarkThumbs registry settings
        HKEY hKey;
    ...
    static bool ExportEventLogs(const std::wstring& filePath) {
        ...
        // This is a placeholder - full ETW integration happens in Sprint 12
    ...
    static bool ExportPerformanceMetrics(const std::wstring& filePath) {
        ...
        file << L"  Decode time by format: See benchmarks/baseline-v7.0.0.json\n";
    ...
    static bool CreateZipBundle(const std::wstring& sourceDir, const std::wstring& zipPath) {
        ...
        if (!bundle.is_open()) return false;

        bundle << L"DarkThumbs Diagnostics Bundle\n";
        bundle << L"=============================\n\n";

        // Read and append all diagnostic files
        std::vector<std::wstring> files = {
            L"system_info.txt",
            L"decoder_health.txt",
            L"circuit_breakers.txt",
            L"registry_settings.txt",
            L"event_logs.txt",
            L"performance.txt",
            L"gpu_info.txt"
        };
