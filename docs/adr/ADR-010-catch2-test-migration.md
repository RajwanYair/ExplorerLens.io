...
**Status:** Rejected  
**Rejected:** 2026-07-29 — `catch2-tests.yml` CI workflow deleted; `BUILD_CATCH2_TESTS` option
removed from `Engine/Tests/CMakeLists.txt`; custom `TEST()`/`RUN_TEST()` framework retained.  
**Date:** 2026-07-19 (proposed)  

## Context

ExplorerLens has ~4877 unit tests using a custom macro-based framework (`TEST()`, `RUN_TEST()`,  
...
1. All new test files go into `Engine/Tests/Catch2Tests/`
1. Existing `EngineTests*.cpp` files remain unchanged (custom macros)
1. Corpus validation tests use Catch2 exclusively (`CorpusValidationTests.cpp`)
1. Future decoder tests, integration tests, and regression tests use Catch2
