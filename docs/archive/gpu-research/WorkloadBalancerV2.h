...
enum class WorkloadBalancerV2Algorithm {
    WorkStealing,
    RoundRobin,
    WeightedCapacity,
    AffinityBased
};
enum class WorkloadQueuePriority {
    Realtime,
    High,
    Normal,
    Background
...
class WorkloadBalancerV2
{
  public:
    ...
    {
        if (m_adapters.empty())
            return {false, -1, 0, "No adapters registered"};
    ...
    void NotifyComplete(int adapterId)
    {
        for (auto& a : m_adapters) {
            if (a.adapterId == adapterId && a.queueDepth > 0) {
                a.queueDepth--;
                break;
            }
        }
    ...
    int DispatchedCount() const noexcept
    {
        return m_dispatched.load();
    }
    ...
    {
        switch (a) {
            case WorkloadBalancerV2Algorithm::WorkStealing:
                return "WorkStealing";
            case WorkloadBalancerV2Algorithm::RoundRobin:
                return "RoundRobin";
            case WorkloadBalancerV2Algorithm::WeightedCapacity:
                return "WeightedCapacity";
            case WorkloadBalancerV2Algorithm::AffinityBased:
                return "AffinityBased";
        }
        return "Unknown";
    }
  ...
  private:
    int PickAdapter(const WorkloadItem&)
    {
        ...
        for (int i = 1; i < static_cast<int>(m_adapters.size()); ++i) {
            if (m_adapters[i].queueDepth < m_adapters[best].queueDepth)
                best = i;
        }
        return best;
    }
