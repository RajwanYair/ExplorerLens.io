...
class VulkanRayTracingPreview
{
  public:
    ...
    inline std::vector<uint8_t> RenderPreview(const RTScene& scene, const RTRenderConfig& config) const
    {
        ...
        if (scene.vertices.empty() || scene.triangles.empty())
            return framebuffer;
        ...
        for (uint32_t y = 0; y < config.height; ++y) {
            for (uint32_t x = 0; x < config.width; ++x) {
                ...
                std::array<float, 3> dir =
                    Normalize({forward[0] + u * right[0] + v * up[0], forward[1] + u * right[1] + v * up[1],
    ...
    inline bool ValidateScene(const RTScene& scene) const
    {
        ...
        for (const auto& tri : scene.triangles) {
            if (tri.v0 >= maxIdx || tri.v1 >= maxIdx || tri.v2 >= maxIdx)
                return false;
        }
        return true;
    }
  ...
  private:
    ...
    inline Vec3 Sub(Vec3 a, Vec3 b) const
    {
        return {a[0] - b[0], a[1] - b[1], a[2] - b[2]};
    }
    inline Vec3 Add(Vec3 a, Vec3 b) const
    {
        return {a[0] + b[0], a[1] + b[1], a[2] + b[2]};
    }
    inline Vec3 Mul(Vec3 a, float s) const
    {
        return {a[0] * s, a[1] * s, a[2] * s};
    }
    inline float Dot(Vec3 a, Vec3 b) const
    {
        return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
    }
    ...
    inline HitInfo IntersectScene(const RTScene& scene, Vec3 origin, Vec3 dir) const
    {
        ...
        for (size_t i = 0; i < scene.triangles.size(); ++i) {
            ...
            if (t > 0.001f && t < closest.t) {
                ...
                closest.materialIdx =
                    static_cast<uint32_t>(i % (std::max)(static_cast<size_t>(1), scene.materials.size()));
                closest.hit = true;
            }
        }
        return closest;
    ...
    inline Vec3 TraceRay(const RTScene& scene, Vec3 origin, Vec3 dir, uint32_t depth, bool shadows) const
    {
        ...
        RTMaterial mat =
            scene.materials.empty() ? RTMaterial{} : scene.materials[hitInfo.materialIdx % scene.materials.size()];
        ...
        float ambient = 0.15f;
        return {mat.albedo[0] * (ambient + NdotL * shadowFactor * scene.lightIntensity * scene.lightColor[0]),
