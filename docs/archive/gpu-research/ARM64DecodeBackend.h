...
class ARM64DecodeBackend
{
  public:
    ...
    ARM64SIMDResult DecodeStride(const uint8_t* src, uint32_t width, uint32_t height, uint8_t* dst)
    {
        (void)src;
        (void)dst;
        ARM64SIMDResult r;
    ...
    bool HasNEON() const
    {
        return m_caps.hasNEON;
    }
    ARM64DecodeMode GetMode() const
    {
        return m_cfg.mode;
    }
    const ARM64DecodeConfig& GetConfig() const
    {
        return m_cfg;
    }
    void SetConfig(const ARM64DecodeConfig& cfg)
    {
        m_cfg = cfg;
    }
    const ARM64Capabilities& GetCapabilities() const
    {
        return m_caps;
    }
    void Reset()
