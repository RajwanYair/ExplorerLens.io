...
class GPUOccupancyCalculator
{
  public:
    ...
    GPUOccupancyCalculatorConfig GetConfig() const
    {
        return m_config;
    }
    ...
    OccupancyResult Calculate(uint32_t registersUsed, uint32_t sharedMemKB, uint32_t threadsPerBlock,
                              uint32_t maxWarpsPerSM = 48) const
    {
        OccupancyResult r;
        r.maxWarps = maxWarpsPerSM;
        uint32_t warpsPerBlock = (threadsPerBlock + 31) / 32;
        r.activeWarps = warpsPerBlock;
        if (r.activeWarps > r.maxWarps)
            r.activeWarps = r.maxWarps;
        r.theoreticalOccupancy = r.maxWarps > 0 ? static_cast<float>(r.activeWarps) / r.maxWarps : 0.0f;
