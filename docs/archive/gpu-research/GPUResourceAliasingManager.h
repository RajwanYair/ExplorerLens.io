...
{
  public:
    uint32_t Register(uint64_t vramBytes)
    {
        m_regions.push_back(vramBytes);
        return static_cast<uint32_t>(m_regions.size() - 1);
    }
    bool Alias(uint32_t srcId, uint32_t dstId)
    {
        (void)srcId;
        (void)dstId;
        return true;
    }
    void Barrier(ResourceAlias al)
    {
        m_barriers.push_back(al);
    }
