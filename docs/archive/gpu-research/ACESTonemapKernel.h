...
class ACESTonemapKernel
{
  public:
    ...
    void SetCurve(ACESCurveVariant curve)
    {
        m_curve = curve;
    }
    ACESCurveVariant GetCurve() const
    {
        return m_curve;
    }

    // Set grading parameters applied before tone mapping
    void SetGrading(const ACESGradingParams& grading)
    {
        m_grading = grading;
    }
    const ACESGradingParams& GetGrading() const
    {
        return m_grading;
    }
    ...
    bool Validate() const
    {
        ...
        if (testR < 0.0f || testR > 0.01f)
            return false;
        if (testG < 0.5f || testG > 1.0f)
            return false;
        if (testB < 0.9f || testB > 1.0f)
            return false;
        if (testG >= testB)
            return false;  // Monotonicity: f(1) < f(10)
        return true;
    }
