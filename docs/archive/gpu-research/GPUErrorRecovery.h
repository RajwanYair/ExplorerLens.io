...
class GPUErrorRecovery
{
  public:
    ...
    GPURecoveryAction DetermineAction(const GPUErrorEvent& error) const
    {
        ...
        switch (error.type) {
            ...
            case GPUErrorType::DeviceHung:
                return error.consecutiveCount >= 2 ? GPURecoveryAction::FallbackAPI : GPURecoveryAction::ResetDevice;
    ...
    void OnFallback(FallbackCallback cb)
    {
        m_fallbackCb = std::move(cb);
    }
    ...
    GPURecoveryStats GetStats() const
    {
        return m_stats;
    }
