...
class ShaderCacheCompiler
{
  public:
    ...
    // CompileShader
    // ================================================================
    inline bool CompileShader(const std::string& hlslSource, const std::string& entryPoint, const std::string& target,
                              std::vector<uint8_t>& outBytecode)
    {
        ...
        HRESULT hr = m_pfnCompile(hlslSource.data(), hlslSource.size(), nullptr, nullptr, nullptr, entryPoint.c_str(),
                                  target.c_str(), D3DCOMPILE_OPTIMIZATION_LEVEL3, 0, &blob, &err);
        if (err)
            err->Release();
    ...
    // ================================================================
    inline std::string MakeCacheKey(const std::string& source, const std::string& entryPoint, const std::string& target)
    {
        AcquireShared();
        EnsureModules();
    ...
    // SaveToCache
    // ================================================================
    inline bool SaveToCache(const std::string& key, const std::vector<uint8_t>& bytecode)
    {
        AcquireExclusive();
        EnsureCacheDir();
        ...
        bool ok = false;
        if (f.is_open()) {
            f.write(reinterpret_cast<const char*>(bytecode.data()), static_cast<std::streamsize>(bytecode.size()));
            ok = f.good();
            f.close();
        }
    ...
    // LoadFromCache
    // ================================================================
    inline bool LoadFromCache(const std::string& key, std::vector<uint8_t>& outBytecode)
    {
        AcquireShared();
    ...
    inline void PurgeExpired(uint32_t maxAgeDays = 30)
    {
        ...
        uint64_t cutoff = nowU.QuadPart - static_cast<uint64_t>(maxAgeDays) * 24ULL * 3600ULL * 10000000ULL;
  ...
  private:
    ...
    using PFN_BCryptOpenAlgorithmProvider = NTSTATUS(WINAPI*)(BCRYPT_ALG_HANDLE*, LPCWSTR, LPCWSTR, ULONG);
    using PFN_BCryptCreateHash = NTSTATUS(WINAPI*)(BCRYPT_ALG_HANDLE, BCRYPT_HASH_HANDLE*, PUCHAR, ULONG, PUCHAR, ULONG,
                                                   ULONG);
    using PFN_BCryptHashData = NTSTATUS(WINAPI*)(BCRYPT_HASH_HANDLE, PUCHAR, ULONG, ULONG);
    using PFN_BCryptFinishHash = NTSTATUS(WINAPI*)(BCRYPT_HASH_HANDLE, PUCHAR, ULONG, ULONG);
    using PFN_BCryptDestroyHash = NTSTATUS(WINAPI*)(BCRYPT_HASH_HANDLE);
    using PFN_BCryptCloseAlgorithmProvider = NTSTATUS(WINAPI*)(BCRYPT_ALG_HANDLE, ULONG);
    ...
    inline void EnsureModules()
    {
        ...
        if (m_hCompiler && !m_pfnCompile) {
            m_pfnCompile = reinterpret_cast<PFN_D3DCompile>(::GetProcAddress(m_hCompiler, "D3DCompile"));
        }
        if (!m_hBcrypt) {
            m_hBcrypt = ::LoadLibraryW(L"bcrypt.dll");
        }
        if (m_hBcrypt && !m_pfnBCryptOpen) {
            m_pfnBCryptOpen = reinterpret_cast<PFN_BCryptOpenAlgorithmProvider>(
                ::GetProcAddress(m_hBcrypt, "BCryptOpenAlgorithmProvider"));
            ...
            m_pfnBCryptDestroy =
                reinterpret_cast<PFN_BCryptDestroyHash>(::GetProcAddress(m_hBcrypt, "BCryptDestroyHash"));
            m_pfnBCryptClose = reinterpret_cast<PFN_BCryptCloseAlgorithmProvider>(
                ::GetProcAddress(m_hBcrypt, "BCryptCloseAlgorithmProvider"));
        }
    }

    inline void EnsureCacheDir()
    {
        if (m_cacheDirReady && !m_cacheDir.empty())
            return;
    ...
    {
        EnsureModules();
        if (!m_pfnBCryptOpen || !m_pfnBCryptCreate || !m_pfnBCryptHashData || !m_pfnBCryptFinish || !m_pfnBCryptDestroy
            || !m_pfnBCryptClose)
        ...
        m_pfnBCryptHashData(hHash, reinterpret_cast<PUCHAR>(const_cast<char*>(data.data())),
