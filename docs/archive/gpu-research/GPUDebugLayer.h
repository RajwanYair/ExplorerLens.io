...
class GPUDebugLayer
{
  public:
    ...
    void SetBreakOnError(bool brk)
    {
        m_breakOnError = brk;
    }
    bool GetBreakOnError() const
    {
        return m_breakOnError;
    }
    ...
    void RecordMessage(const GPUDebugMessage& msg)
    {
        ...
        if (msg.severity == GPUDebugSeverity::Error)
            m_stats.errorCount++;
        if (msg.severity == GPUDebugSeverity::Warning)
            m_stats.warningCount++;
        if (msg.category == GPUDebugCategory::ResourceLeak)
            m_stats.leakCount++;
    }

    const std::vector<GPUDebugMessage>& GetMessages() const
    {
        return m_messages;
    }
    const GPUDebugStats& GetStats() const
    {
        return m_stats;
    }
    ...
    bool HasLeaks() const
    {
        return m_stats.leakCount > 0;
    }

    static size_t SeverityCount()
    {
        return static_cast<size_t>(GPUDebugSeverity::COUNT);
    }
    static size_t CategoryCount()
    {
        return static_cast<size_t>(GPUDebugCategory::COUNT);
    }
