...
public:
    ...
    VisualQueryPruneResult PruneSearchSpace(const VisualQueryHint& hint, const std::vector<uint32_t>& candidateIds,
                                            std::vector<uint32_t>& prunedOut) const noexcept
    {
        ...
        res.pruneMs = 0.1f;
        res.estimatedSpeedup = (res.prunedCandidates > 0) ? static_cast<float>(res.originalCandidates)
                                                                / static_cast<float>(res.prunedCandidates)
                                                          : 1.0f;
        return res;
    }
