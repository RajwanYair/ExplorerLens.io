...
## Issue
The installation script hangs at "Registering COM DLL" when running `regsvr32.exe /s CBXShell.dll`.
...
```powershell
regsvr32.exe /s "C:\Users\ryair\OneDrive - Intel Corporation\Documents\MyScripts\DarkThumbs\x64\Release\CBXShell.dll"
```

### Verbose Registration (diagnostics)
```powershell
regsvr32.exe "C:\Users\ryair\OneDrive - Intel Corporation\Documents\MyScripts\DarkThumbs\x64\Release\CBXShell.dll"
```
...
3. **DllRegisterServer Implementation**
...
1. **Try Manual Registration**
   ...
   regsvr32.exe "x64\Release\CBXShell.dll"
   ```
...
3. **Test from Local Disk**
   ```powershell
   # Copy DLL to local disk
   Copy-Item "x64\Release\CBXShell.dll" "C:\Temp\CBXShell.dll"
   regsvr32.exe "C:\Temp\CBXShell.dll"
   ```

4. **Verify Visual C++ Redistributable**
   ```powershell
...
# Shows:
# [DRY RUN] Would create: C:\Program Files\DarkThumbs
# [DRY RUN] Would copy: CBXShell.dll
# [DRY RUN] Would register: C:\Program Files\DarkThumbs\CBXShell.dll
# [DRY RUN] Command: regsvr32.exe /s "C:\Program Files\DarkThumbs\CBXShell.dll"
```
