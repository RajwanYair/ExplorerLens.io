# ExplorerLens.io Build Scripts Reference
## Complete Build System Documentation

> **Note:** This document was written for the v2.0 CMD-based build system (November 2025).
> The build system has since been replaced by PowerShell scripts (`Build-MSVC.ps1`, etc.).
> See [.github/copilot-instructions.md](../../.github/copilot-instructions.md) for current build commands.

**Last Updated:** March 2026 
**Build System Version:** 3.0 (PowerShell + CMake Presets)
...
│ ├── build-cbxshell.cmd # Build LENSShell.dll
│ ├── build-cbxmanager.cmd # Build LENSManager.exe
...
2. Builds LENSShell.dll with all compression support
3. Builds LENSManager.exe
...
**Output:** `LENSShell\x64\Release\LENSShell.dll`
...
**Output:** `LENSManager\x64\Release\LENSManager.exe`
...
2. Build LENSShell.dll
3. Build LENSManager.exe
...
dir LENSShell\x64\Release\LENSShell.dll
dir LENSManager\x64\Release\LENSManager.exe
```
...
**Solution:** Rebuild all libraries, or disable LTCG in LENSShell.vcxproj
...
rmdir /s /q LENSShell\x64
rmdir /s /q LENSManager\x64
...
if not exist "LENSShell\x64\Release\LENSShell.dll" exit /b 1
if not exist "LENSManager\x64\Release\LENSManager.exe" exit /b 1
...
3. Run `LENSManager.exe` for configuration
