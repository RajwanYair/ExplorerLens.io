# ExplorerLens.io Installer Options - Open Source Solutions
...
ExplorerLens.io now supports professional Windows installation using industry-standard **open-source installer tools**. This document describes the available options and how to use them.
...
**File**: `install/ExplorerLens.io-Installer.iss` 
**Output**: `ExplorerLens.io-v5.2.0-x64-Setup.exe` (~2 MB)
...
2. **Compile the Installer**:
 ...
 "C:\Program Files (x86)\Inno Setup 6\Compil32.exe" install\ExplorerLens.io-Installer.iss
 
 REM Option B: Using command line
 "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" install\ExplorerLens.io-Installer.iss
 ```

3. **Output**:
 ```
 release-packages\ExplorerLens.io-v5.2.0-x64-Setup.exe
 ```
...
**File**: `install/ExplorerLens.io-Installer.nsi` 
**Output**: `ExplorerLens.io-v5.2.0-x64-Setup.exe`
...
2. **Compile the Installer**:
 ...
 "C:\Program Files (x86)\NSIS\makensis.exe" install\ExplorerLens.io-Installer.nsi
 ```

3. **Output**:
 ```
 ExplorerLens.io-v5.2.0-x64-Setup.exe (in project root)
 ```
...
4. **Copy DLL to System32**:
 - LENSShell.dll → C:\Windows\System32\

5. **Register COM Component**:
 ```cmd
 regsvr32 /s "C:\Windows\System32\LENSShell.dll"
 ```

6. **Create Shortcuts**:
 - Start Menu → ExplorerLens.io
 - Desktop → ExplorerLens.io Config (optional)
...
2. **Unregister COM**:
 ```cmd
 regsvr32 /s /u "C:\Windows\System32\LENSShell.dll"
 ```
3. **Delete System32\LENSShell.dll**
4. **Delete Program Files\ExplorerLens.io**
...
### Inno Setup
```cmd
ExplorerLens.io-v5.2.0-x64-Setup.exe /VERYSILENT /NORESTART
```

### NSIS
```cmd
ExplorerLens.io-v5.2.0-x64-Setup.exe /S
```
...
signtool sign /f MyCert.pfx /p MyPassword /t http://timestamp.digicert.com ExplorerLens.io-v5.2.0-x64-Setup.exe

REM Verify signature
signtool verify /pa ExplorerLens.io-v5.2.0-x64-Setup.exe
```
...
- [ ] **Install on clean Windows 10**
 ...
 - LENSManager.exe launches

- [ ] **Verify COM Registration**
 - Open folder with images
 - Thumbnails generate correctly
 - Right-click context menu shows ExplorerLens.io options
...
2. **Create release package**:
 ```
 ExplorerLens.io-v5.2.0-x64-Setup.exe (installer)
 ExplorerLens.io-v5.2.0-x64-Portable.zip (portable version)
 ```

3. **Upload to GitHub**:
 - Tag: `v5.2.0`
 - Title: `ExplorerLens.io v5.2.0 - Pure 64-bit Release`
...
```powershell
Get-FileHash ExplorerLens.io-v5.2.0-x64-Setup.exe -Algorithm SHA256 | 
...
regsvr32 "C:\Windows\System32\LENSShell.dll"
```
...
**Recommended**: Use **Inno Setup** for ExplorerLens.io installation.
