 ...
 * Multi-Page Thumbnail Collage Implementation for DarkThumbs
...
ThumbnailCollage::CollageMode ThumbnailCollage::GetCollageModeFromRegistry() {
    ...
    if (ERROR_SUCCESS == regKey.Open(HKEY_CURRENT_USER, CBX_APP_KEY, KEY_READ)) {
...
bool ThumbnailCollage::SetCollageModeToRegistry(CollageMode mode) {
    ...
    if (ERROR_SUCCESS == regKey.Create(HKEY_CURRENT_USER, CBX_APP_KEY)) {
