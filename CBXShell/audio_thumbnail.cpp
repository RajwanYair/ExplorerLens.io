/******************************************************************************
 * audio_thumbnail.cpp
 * Audio Thumbnail Generation Implementation for DarkThumbs
...
namespace DarkThumbs
{
    ...
    HBITMAP AudioThumbnail::ExtractAlbumArtPropertySystem(const std::wstring &audioPath)
    {
    ...
    HBITMAP AudioThumbnail::GenerateWaveform(const std::wstring &audioPath, int width, int height)
    {
    ...
    HBITMAP AudioThumbnail::GenerateWaveformPCM(const std::wstring &audioPath, int width, int height)
    {
    ...
    bool AudioThumbnail::ExtractAudioSamples(const std::wstring &audioPath, std::vector<float> &samples, int maxSamples)
    {
        // Simple WAV file reader
        HANDLE hFile = CreateFileW(audioPath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
        ...
        WAVHeader header;
        DWORD bytesRead;
        if (!ReadFile(hFile, &header, sizeof(WAVHeader), &bytesRead, NULL) || bytesRead != sizeof(WAVHeader))
        {
            CloseHandle(hFile);
            return false;
        }
        ...
        while (ReadFile(hFile, chunkID, 4, &bytesRead, NULL) && bytesRead == 4)
        {
            ...
            if (memcmp(chunkID, "data", 4) == 0)
            {
                ...
                int samplesPerPixel = max(1, (int)(chunkSize / header.blockAlign / maxSamples));
                int samplesRead = 0;
                ...
                while (samplesRead < maxSamples && ReadFile(hFile, buffer.data(), (DWORD)buffer.size(), &bytesRead, NULL) && bytesRead > 0)
                {
    ...
    HBITMAP AudioThumbnail::DrawWaveform(const std::vector<float> &samples, int width, int height, COLORREF waveColor)
    {
        if (samples.empty())
            return NULL;
    ...
    bool AudioThumbnail::GetMetadata(const std::wstring &audioPath, AudioMetadata &metadata)
    {
        // Initialize COM if needed
        static bool comInitialized = false;
        if (!comInitialized)
        ...
        // Title
        if (SUCCEEDED(propStore->GetValue(PKEY_Title, &propVar)) && propVar.vt == VT_LPWSTR)
        {
        ...
        if (SUCCEEDED(propStore->GetValue(PKEY_Music_Artist, &propVar)))
        {
            ...
            else if (propVar.vt == (VT_VECTOR | VT_LPWSTR) && propVar.calpwstr.cElems > 0)
            {
        ...
        // Album
        if (SUCCEEDED(propStore->GetValue(PKEY_Music_AlbumTitle, &propVar)) && propVar.vt == VT_LPWSTR)
        {
        ...
        if (SUCCEEDED(propStore->GetValue(PKEY_Media_Duration, &propVar)) && propVar.vt == VT_UI8)
        {
            metadata.duration = (int)(propVar.uhVal.QuadPart / 10000000); // Convert to seconds
        }
        PropVariantClear(&propVar);

        // Bitrate
        if (SUCCEEDED(propStore->GetValue(PKEY_Audio_EncodingBitrate, &propVar)) && propVar.vt == VT_UI4)
        {
        ...
        // Sample Rate
        if (SUCCEEDED(propStore->GetValue(PKEY_Audio_SampleRate, &propVar)) && propVar.vt == VT_UI4)
        {
        ...
        // Channels
        if (SUCCEEDED(propStore->GetValue(PKEY_Audio_ChannelCount, &propVar)) && propVar.vt == VT_UI4)
        {
