...
function Test-VcpkgInstalled {
    ...
    # Check machine-wide installation locations only.
    ...
    foreach ($path in $commonPaths) {
        ...
        if (Test-Path -LiteralPath $vcpkgExe -PathType Leaf) {
            Write-Host "✓ machine-wide vcpkg found at: $path" -ForegroundColor Green
            $vcpkgDir = $path
...
function Get-VcpkgPath {
    ...
    # Check machine-wide locations only.
    $machineVcpkgRoot = [Environment]::GetEnvironmentVariable('VCPKG_ROOT', 'Machine')
    $commonPaths = @(
        $machineVcpkgRoot,
        'C:\ProgramData\vcpkg',
    ...
    foreach ($path in $commonPaths) {
        if ($path -and (Test-Path -LiteralPath (Join-Path $path "vcpkg.exe") -PathType Leaf)) {
            return $path
        }
    }
...
function Install-VcpkgIfNeeded {
    ...
    .PARAMETER InstallPath
        Directory to install vcpkg (default: C:\ProgramData\vcpkg)
    .PARAMETER Force
    ...
    if ($normalizedInstallPath -notmatch '^C:\\(ProgramData|Program Files)(\\|$)' -and $normalizedInstallPath -ne 'C:\vcpkg') {
        throw 'vcpkg must be installed in a machine-wide location such as C:\ProgramData\vcpkg.'
    }
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        throw 'Machine-wide vcpkg installation requires an elevated PowerShell session. Re-run this script as Administrator.'
    }
    ...
    try {
        ...
        [Environment]::SetEnvironmentVariable('VCPKG_ROOT', $InstallPath, 'Machine')
        ...
        Write-Host ""
        Write-Host "Machine VCPKG_ROOT and PATH were updated for all users." -ForegroundColor Gray
        Write-Host ""
...
function Install-VcpkgPackage {
    ...
    .PARAMETER PackageName
        Name of the package to install (e.g., "zlib:x64-windows-static-md")
    .PARAMETER Triplet
        vcpkg triplet (default: x64-windows-static-md)
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
