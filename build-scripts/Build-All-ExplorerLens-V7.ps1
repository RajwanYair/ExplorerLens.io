...
<#
.SYNOPSIS
    ExplorerLens v7.0 - Master Build Script (High Performance)
...
.EXAMPLE
    .\Build-All-ExplorerLens-V7.ps1
    .\Build-All-ExplorerLens-V7.ps1 -Clean
    .\Build-All-ExplorerLens-V7.ps1 -UseVcpkg
...
# Build output paths
$script:BuildPaths = @{
    LENSShell     = "x64\$Configuration\LENSShell.dll"
    LENSManager   = "x64\$Configuration\LENSManager.exe"
...
function Build-MainProject {
    Write-BuildLog "Building ExplorerLens main project..." -Level Progress
    
    Push-Location $ProjectRoot
    try {
        if ($Clean) {
            Write-BuildLog "Performing clean build..." -Level Info
            Invoke-ParallelBuild -Solution "LENSShell.sln" `
                -Target "Clean" `
        ...
        Write-BuildLog "Building LENSShell solution..." -Level Progress
        Invoke-ParallelBuild -Solution "LENSShell.sln" `
            -Configuration $Configuration `
            -Platform "x64" `
            -MaxParallel $Parallel
        ...
        # Verify outputs
        $dllPath = Join-Path $ProjectRoot $BuildPaths.LENSShell
        $exePath = Join-Path $ProjectRoot $BuildPaths.LENSManager
        
        if (Test-Path $dllPath) {
            $size = (Get-Item $dllPath).Length / 1MB
            Write-BuildLog "LENSShell.dll: $([math]::Round($size, 2)) MB" -Level Success
        } else {
        ...
        if (Test-Path $exePath) {
            ...
            Write-BuildLog "LENSManager.exe: $([math]::Round($size, 2)) MB" -Level Success
        } else {
            throw "LENSManager.exe not found at $exePath"
        }
...
function Main {
    ...
    Write-Host "║  ExplorerLens v7.0 - Master Build Script (High Performance)   ║" -ForegroundColor Cyan
    Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    ...
    try {
        ...
        Test-PathExists -Path "LENSShell.sln" -Description "Solution file"
        ...
        Write-Host "  → LENSShell.dll:    $($BuildPaths.LENSShell)" -ForegroundColor White
        Write-Host "  → LENSManager.exe:  $($BuildPaths.LENSManager)" -ForegroundColor White
        Write-Host ""
